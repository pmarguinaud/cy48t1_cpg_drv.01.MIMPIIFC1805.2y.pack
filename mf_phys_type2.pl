...
for my $r (@r)
  { 
    ...
    while (my ($x, $y) = each (%{ $X{$r} }))
      {
        next unless ($y);
        (my $z = $y) =~ s/^Z//o;
        my @expr = &F ('//call-stmt/arg-spec/arg[./named-E[string(N)="?"]]', $x, $d);
        for my $expr (@expr)
          {
            my ($call) = &Fxtran::stmt ($expr);
            my ($p) = &F ('./procedure-designator', $call, 1);
            if ($F90{$p})
              {
                if ($expr->nextSibling)
                  {
                    $expr->nextSibling->unbindNode;
                  }
                $expr->unbindNode ();
              }
          }
        ...
        my ($arg) = &F ('./object/file/program-unit/subroutine-stmt/dummy-arg-LT/arg-N[string (.)="?"]', $x, $d);
        ...
        my ($en_decl) = &F ('//EN-decl[string(EN-N)="?"]', $x, $d);
    ...
    my @code = grep { ! (m/^\s*\&\s*\&\s*$/o) } split (m/\n/o, $d->textContent);
    my $code = join ("\n", @code, '');
    'FileHandle'->new (">$F90{$r}")->print ($code);
  }
