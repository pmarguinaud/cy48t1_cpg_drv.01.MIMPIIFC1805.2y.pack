...
{
  my $name = shift;
  return $name =~ m/^(?:APL_ARPEGE|MF_PHYS_)/o ? (fillcolor => 'green',  style => 'filled') : ();
}
...
while (my ($k, $v) = each (%g))
  {
    ...
    for ($k)
      {
        s/RRTM_TAUMOL\d+/RRTM_TAUMOL/o;
        s/SRTM_TAUMOL\d+/SRTM_TAUMOL/o;
      }
    $g->add_node (name => $k, label => $k . ($L{$k} ? "\n$L{$k}" : ''), shape => 'box', &color ($k)) unless ($seen_nodes{$k}++);
#   $g->add_node (name => $k, label => "$k", shape => 'box', &color ($k));
    for my $v (@$v)
      {   
        ...
        $g->add_edge (from => $k, to => $v) unless ($seen_edges{$k}{$v}++);
print "$k -- $v\n";
      }   
  }
