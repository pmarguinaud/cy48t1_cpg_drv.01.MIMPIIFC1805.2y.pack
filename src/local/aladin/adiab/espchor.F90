  ...
  DO JSP=KSTA,KEND
    DO JLEV=1,NFLEVG
      ...
      IF (LNHDYN) THEN
        ...
        IF (YRDYNA%LNHX) PSPTNDHD_SNHX(JLEV,JSP)=-PSPSNHXG(JLEV,JSP)
      ENDIF
    ENDDO
  ENDDO
  IF (YQ%LSP) THEN
...
!cdir nodep
  ...
  IF (YRDYNA%LNHX.AND.YRDYNA%LSLHD_SVD.AND.(.NOT.YRDYNA%LSLHD_STATIC)) THEN
    DO JSP=KSTA,KEND
      DO JLEV=1,NFLEVG
!cdir nodep
        PSPSNHXG(JLEV,JSP)= PSPSNHXG(JLEV,JSP)&
  ...
  DO JSP=KSTA,KEND
    DO JLEV=1,NFLEVG
      ...
      IF (LNHDYN) THEN
        ...
        IF (YRDYNA%LNHX) PSPTNDHD_SNHX(JLEV,JSP)=PSPTNDHD_SNHX(JLEV,JSP)+PSPSNHXG(JLEV,JSP)
      ENDIF
    ENDDO
  ENDDO
  IF (YQ%LSP) THEN
