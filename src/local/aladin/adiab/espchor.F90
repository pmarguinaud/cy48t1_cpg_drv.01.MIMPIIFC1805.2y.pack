  ...
  DO JSP=KSTA,KEND
    DO JLEV=1,NFLEVG
      ...
      IF (LNHDYN) THEN
        ...
        IF (LNHX) PSPTNDHD_SNHX(JLEV,JSP)=-PSPSNHXG(JLEV,JSP)
      ENDIF
    ENDDO
  ENDDO
  IF (YQ%LSP) THEN
...
!cdir nodep
  ...
  IF (LNHX.AND.LSLHD_SVD.AND.(.NOT.LSLHD_STATIC)) THEN
    DO JSP=KSTA,KEND
      DO JLEV=1,NFLEVG
!cdir nodep
        PSPSNHXG(JLEV,JSP)= PSPSNHXG(JLEV,JSP)&
  ...
  DO JSP=KSTA,KEND
    DO JLEV=1,NFLEVG
      ...
      IF (LNHDYN) THEN
        ...
        IF (LNHX) PSPTNDHD_SNHX(JLEV,JSP)=PSPTNDHD_SNHX(JLEV,JSP)+PSPSNHXG(JLEV,JSP)
      ENDIF
    ENDDO
  ENDDO
  IF (YQ%LSP) THEN
