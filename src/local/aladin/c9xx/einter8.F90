...
!       Each value is interpolated from the 4 nearest points of the initial gri
...
!          from INTER8  (M. DEQUE 18 JULY 95.)
!          and  EINTER2 (J.F. GELEYN, M. DEQUE 1 FEB 91.)
...
IF (LHOOK) CALL DR_HOOK('EINTER8',0,ZHOOK_HANDLE)
...
IF ( YRCLI%LGLOBE .AND. ((KLON0-4-YRCLI%NGLOBX)*(KLAT0-4-YRCLI%NGLOBY) /= 0) )&
 & WRITE(KULOUT,'('' EINTER8 : CAUTION !'',/,&
 & '' EITHER YOU ARE USING OLD E923, OR'',&
 & '' THERE IS A BUG IN THE SIZE OF DATASET (ARGUMENT 2 OR 3)'')')  

IF (YRCLI%LGLOBE) THEN
! GLOBAL input grid: fill the margins to sew the spheric domain
! along X (longitude): take the other border (360 deg longit shift)
! along Y (latitude): take the value coming opposite, at 180 degrees
!                     longitude shift.
! KLON0 includes the margins: KLON0=NGLOBX+4
  IPAUT=(KLON0-4)/2
  DO JFL=1,KNCH
    DO JI=3,IPAUT+2
...
! Coordinates of grid lower left corner in a global equivalent one:
  DO JFL=1,KNCH
    DO JI=3,KLON0-3
...
IF (LRPLANE) THEN
  IF (LMRT .AND. (ABS(ELAT0) >= ZEPS)) THEN
    WRITE(KULOUT,*) 'EINTER8: ELAT0=',ELAT0,&
    & ' MUST BE EQUAL ZERO IF LMRT IS TRUE!'
    CALL ABOR1('EINTER8: LMRT & ELAT0 INCONSISTENT')
  ENDIF 
...
! ZXCO(1:KNCO) are the longitudes of the points into the box
! ZYCO(1:KNCO) are the latitudes  of the points into the box

DO JJ=1,KLAT
  DO JI=1,KLON

!  Loop in the box : the interpolation values are localised.
    DO JM=1,KNCO

      IM=JM+(JI-1+(JJ-1)*KLON)*KNCO
! NGLOBX grid points cover 2*RPI radians longitude
! zx is the x global grid coordinate in grid mesh units beginning at 0
      ZX=MOD( ZGX/RPI/2.0_JPRB*(ZXCO(IM)+2.0_JPRB*RPI) , ZGX )
! zx=ii1-0.5 is at grid position ixx=3
      ZX=ZX - II1 + 3.5_JPRB
      IXX(JM)=INT(ZX)
! fractional coordinate inside a mesh:
      ZXX=ZX-IXX(JM)
      ZCX=1.0_JPRB-ZXX
...
! zy=ij1-0.5 is at grid position iyy=3
      ZY=ZY - IJ1 + 3.5_JPRB
      IYY(JM)=INT(ZY)
! fractional coordinate inside a mesh:
      ZYY=ZY-IYY(JM)
...
WRITE(6,'(''----- fin de EINTER8  - OK!'')')

!         ---------------------------------------------------------------------

END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('EINTER8',1,ZHOOK_HANDLE)
