...
! Allocate YDGEOMETRY%YRGSGEOM_NB:
...
! Allocate YDGEOMETRY%YRGSGEOM:
ALLOCATE(YDGEOMETRY%YRGSGEOM(NGPBLKS))

! Copy items to non-blocked form of YDGEOMETRY%YRGSGEOM
YDGEOMETRY%YRGSGEOM_NB%RCORI(:)    =ZZRCORI(:)
YDGEOMETRY%YRGSGEOM_NB%RCORIC(:)   =ZZRCORIC(:)
YDGEOMETRY%YRGSGEOM_NB%GEMU(:)     =ZZGEMU(:)
YDGEOMETRY%YRGSGEOM_NB%GSQM2(:)    =ZZGSQM2(:)
YDGEOMETRY%YRGSGEOM_NB%GELAM(:)    =ZZGELAM(:)
YDGEOMETRY%YRGSGEOM_NB%GELAT(:)    =ZZGELAT(:)
YDGEOMETRY%YRGSGEOM_NB%GECLO(:)    =ZZGECLO(:)
YDGEOMETRY%YRGSGEOM_NB%GESLO(:)    =ZZGESLO(:)
YDGEOMETRY%YRGSGEOM_NB%GM(:)       =ZZGM(:)
YDGEOMETRY%YRGSGEOM_NB%GMAPPA(:)   =RSTRET*RSTRET ! provisional setting, must be revised for LMRT=T.
YDGEOMETRY%YRGSGEOM_NB%GOMVRL(:)   =ZZGOMVRL(:)
YDGEOMETRY%YRGSGEOM_NB%GOMVRM(:)   =ZZGOMVRM(:)
YDGEOMETRY%YRGSGEOM_NB%GNORDL(:)   =ZZGNORDL(:)
YDGEOMETRY%YRGSGEOM_NB%GNORDM(:)   =ZZGNORDM(:)
YDGEOMETRY%YRGSGEOM_NB%GNORDLCL(:) =ZZGNORDLCL(:)
YDGEOMETRY%YRGSGEOM_NB%GNORDMCL(:) =ZZGNORDMCL(:)
YDGEOMETRY%YRGSGEOM_NB%GNORDMCM(:) =ZZGNORDMCM(:)
YDGEOMETRY%YRGSGEOM_NB%GAW(:)      =ZZGAW(:)
YDGEOMETRY%YRGSGEOM_NB%NGPLAT(:)   =IIGPLAT(:)
YDGEOMETRY%YRGSGEOM_NB%NUNIQUEGP(:)=IIUNIQUEGP(:)

! Point to items in YDGEOMETRY%YRGSGEOM
...
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RCOLON(NGPTOT,NGPBLKS))
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RSILON(NGPTOT,NGPBLKS))
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RINDX(NGPTOT,NGPBLKS))
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RINDY(NGPTOT,NGPBLKS))
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RATATH(NGPTOT,NGPBLKS))
ALLOCATE(YDGEOMETRY%YRCSGEOM_B%RATATX(NGPTOT,NGPBLKS))
