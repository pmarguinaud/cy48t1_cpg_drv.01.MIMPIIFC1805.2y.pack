SUBROUTINE ETRANSDIR_MDL(YDGEOMETRY,YDGFL,YDGMV,CDCONF,KNFTHER,YDSP)

!**** *ETRANSDIR_MDL * - Direct transforms for model

!     Purpose.  Perform direct transform (gridpoint to spectral)
!     --------

!**   Interface.  CALL ETRANSDIR_MDL(...)
!     ---------- 
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('ETRANSDIR_MDL',0,ZHOOK_HANDLE)
...
! Configuration where grid-point VOR,DIV transformed to spectral (for Jb calc.)
  IF(NUMSPFLDS1>0) THEN
    CALL EDIR_TRANS(PSPSC2=ZSP(1:IST,:),PSPSC3A=YDSP%SP3D(:,:,1:2+KNFTHER),&
     ...
     & PGP3A=GMVT1(:,:,1:IDIMT1,:),PGP3B=YDGFL%GFLT1(:,:,IOFF+1:IOFF+IDIM3,:))  
  ELSE
    CALL EDIR_TRANS(PSPSC2=ZSP(1:IST,:),PSPSC3A=YDSP%SP3D(:,:,1:2+KNFTHER),&
     & KRESOL=NRESOL,KPROMA=NPROMA,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3A=NBSETLEV,&
     & PGP2=GMVT1S(:,1:YT1%NDIMS,:),&
     & PGP3A=GMVT1(:,:,1:IDIMT1,:))  
  ENDIF
...
! Normal configuration (u,v grid-point to vor,div spectral)
  IF(NUMSPFLDS1>0) THEN
    CALL EDIR_TRANS(PSPVOR=YDSP%VOR,PSPDIV=YDSP%DIV,PSPSC2=ZSP(1:IST,:),PSPSC3A=YDSP%HV,&
     ...
     & PGP3A=GMVT1(:,:,3:IDIMT1,:),PGP3B=YDGFL%GFLT1(:,:,IOFF+1:IOFF+IDIM3,:),&
  ...
  ELSE
    CALL EDIR_TRANS(PSPVOR=YDSP%VOR,PSPDIV=YDSP%DIV,PSPSC2=ZSP(1:IST,:),PSPSC3A=YDSP%HV,&
     ...
     & KVSETSC3A=NBSETLEV,&
     & PGPUV=GMVT1(:,:,1:2,:),PGP2=GMVT1S(:,1:YT1%NDIMS,:),&
     & PGP3A=GMVT1(:,:,3:IDIMT1,:),PMEANU=YDSP%UB,PMEANV=YDSP%VB)
  ENDIF
ELSE
! Normal configuration (u,v grid-point to vor,div spectral)
  IF(NUMSPFLDS1>0) THEN
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('ETRANSDIR_MDL',1,ZHOOK_HANDLE)
END SUBROUTINE ETRANSDIR_MDL
