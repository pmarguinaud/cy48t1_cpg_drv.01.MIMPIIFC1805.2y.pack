...
USE YOMDYNA      , ONLY : LPC_CHEAP, LPC_FULL
...
LLSLPHY= LSLPHY.AND.LLDIAB.AND.((LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.LPC_FULL))
LLINC=.FALSE.
LLEXCLUDE=.TRUE.
...
IF( .NOT.(LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
...
IF( .NOT.(LPC_CHEAP.AND.(NCURRENT_ITER > 0)) .AND. (&
