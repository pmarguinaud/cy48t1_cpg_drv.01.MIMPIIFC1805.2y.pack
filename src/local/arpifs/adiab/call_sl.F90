...
LLSLPHY= LSLPHY.AND.LLDIAB.AND.((YRDYNA%LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.YRDYNA%LPC_FULL))
LLINC=.FALSE.
LLEXCLUDE=.TRUE.
...
IF( .NOT.(YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
...
IF( .NOT.(YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) .AND. (&
