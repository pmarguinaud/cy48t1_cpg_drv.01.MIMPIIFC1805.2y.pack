...
IF (CDPART (3:3) == 'X') THEN
  ...
  IF (NCURRENT_ITER == 0) THEN
    ...
    & YDMODEL, YDCPG_DDH%DDHI, PGMV, PGFL, YDCPG_SL2%ZVIEW(:, MSLB2VVEL), PSD_XA, PSD_PF,                  &
    & PGMVTNDSI_DDH, PGMVTNDHD_DDH, PGFLTNDHD_DDH, YDCPG_DDH%DHCV, YDDDH, YDCPG_MISC%FTCNS        )
  ENDIF
  ...
  IF (LSLAG) THEN
    CALL CPG_DYN_SLG (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_DYN0, YDCPG_DYN9, &
                    & YDVARS, YDFIELDS%YRGMV, YDMODEL, SLHDA(YDCPG_BNDS%KSTGLO), SLHDD0(YDCPG_BNDS%KSTGLO), PGFL, &
                    & KSETTLOFF, PGMV, PGMVS, YDCPG_SL1%ZVIEW, YDCPG_SL2%ZVIEW, PGMVT1, PGMVT1S, PGMVTNDSI_DDH)
  ELSE
  ...
  CALL CPG_END(YDGEOMETRY, YDVARS, YDCPG_BNDS, YDCPG_OPTS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, YDMODEL%YRML_PHY_EC%YREPHY, &
  & YDMODEL%YRML_GCONF, YDDYN, YDMODEL%YRML_PHY_MF, GM(1), YDCPG_MISC%QS, YDMF_PHYS_SURF%GSP_RR%PT_T9,          &
  & PGFL, PGMV, PGMVS, PTRAJ_PHYS)
