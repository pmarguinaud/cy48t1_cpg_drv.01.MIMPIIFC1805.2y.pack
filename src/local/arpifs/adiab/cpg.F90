...
SUBROUTINE CPG(YDGEOMETRY,YDCPG_DIM,YDTMP,YDCPG_TND,YDCPG_MISC,&
 & YDCPG_PHY0,YDCPG_PHY9,YDMF_PHYS,YDMF_PHYS_TMP,YDAPLPAR_TMP,YDCPG_DDH,YDCPG_DYN0,YDCPG_DYN9,&
...
INTEGER(KIND=JPIM),INTENT(OUT)   :: KSETTLOFF(YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PGFLSLP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIMSLP)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSAVTEND(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_PHY_G%YRSLPHY%NVTEND)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PGMVTNDHD_DDH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG, &
 & 2+YDMODEL%YRML_GCONF%YRDIMF%NFTHER)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PGFLTNDHD_DDH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PGPSDT2D(YDCPG_DIM%KLON,YSPPT%YGPSDT(1)%NG2D)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSD_PF(YDCPG_DIM%KLON,YDFIELDS%YRSURF%YSD_PFD%NLEVS,YDFIELDS%YRSURF%YSD_PFD%NDIM) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGMV(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDFIELDS%YRGMV%NDIMGMV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGMVS(YDCPG_DIM%KLON,YDFIELDS%YRGMV%NDIMGMVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGFL(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIM)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGFLPC(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIMPC)
REAL(KIND=JPRB),   INTENT(INOUT) :: PGFLPT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIMPT)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSD_XA(YDCPG_DIM%KLON,YDFIELDS%YRSURF%YSD_XAD%NLEVS,YDFIELDS%YRSURF%YSD_XAD%NDIM)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGMVTNDSI_DDH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG, &
 & YDMODEL%YRML_DIAG%YRMDDH%NDIMSIGMV)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PB1(YDSL%NASLB1,YDMODEL%YRML_DYN%YRPTRSLB1%NFLDSLB1)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PB2(YDCPG_DIM%KLON,YDMODEL%YRML_DYN%YRPTRSLB2%NFLDSLB2)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGMVT1(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDFIELDS%YRGMV%YT1%NDIM)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGMVT1S(YDCPG_DIM%KLON,YDFIELDS%YRGMV%YT1%NDIMS)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGFLT1(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIM1)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PEXTRA(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_PHY_G%YRDPHY%NVEXTRDYN)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGMVTNDSL_DDH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG, &
 & 2+YDMODEL%YRML_GCONF%YRDIMF%NFTHER)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PGFLTNDSL_DDH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NUMFLDS)
TYPE (TRAJ_PHYS_TYPE),INTENT(INOUT) :: PTRAJ_PHYS
TYPE (TRAJ_SLAG_TYPE),INTENT(INOUT) :: PTRAJ_SLAG
TYPE(TYP_DDH)        , INTENT(INOUT) :: YDDDH

REAL(KIND=JPRB) :: ZSLBUF1AU(YDCPG_DIM%KLON,YDMODEL%YRML_DYN%YRPTRSLB1%NFLDSLB1)        ! local version of SLBUF1
...
#include "mf_phys_par.intfb.h"
#include "mf_phys_par_new.intfb.h"
#include "mf_phys_mpa.intfb.h"
#include "mf_phys_sim.intfb.h"

!     ------------------------------------------------------------------

IF (LHOOK) CALL DR_HOOK('CPG', 0, ZHOOK_HANDLE)
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM, YDDIMV=>YDGEOMETRY%YRDIMV, YDGEM=>YDGEOMETRY%YRGEM, YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_DIM%KBL), &
...
YDCPG_MISC%GPAR(:,:)=0.0_JPRB

CALL CPG_GP(YDGEOMETRY, YDCPG_DIM, YDTMP%CPG_GP, YDCPG_TND, YDCPG_MISC, YDCPG_DYN0, YDCPG_DYN9, &
& YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, LD_DFISTEP, LDFSTEP, LDDIAB, PDT, PTE, PGFL,       &
& PGMVTNDSL_DDH, PGFLTNDSL_DDH, YDCPG_MISC%GPAR, PB2, PGFLT1, YDCPG_MISC%KOZO, YDDDH)
...
IF (LDSLPHY.AND.((LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.LPC_FULL))) THEN
  ...
  & YDCPG_DIM%KFDIA, LDFSTEP, PDTPHY, ZSLBUF1AU(1, MSLB1UP9), ZSLBUF1AU(1, MSLB1VP9), ZSLBUF1AU(1, MSLB1TP9), &
  & ZSLBUF1AU(1, MSLB1GFLP9), PSAVTEND, PGFLSLP)
ENDIF
...
!*       4.4.1   Call MF unlagged physics (predictor only if PC scheme).
  ...
  IF (YDMODEL%YRML_PHY_MF%YRSIMPHL%LSIMPH) THEN
    CALL MF_PHYS_SIM(YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,            &
    ...
    & ZSLBUF1AU, PB2, PGMVT1, PGFLT1, YDCPG_MISC%GPAR, PTRAJ_PHYS, YDDDH, YDCPG_MISC%FTCNS)
  ELSEIF (YDMODEL%YRML_PHY_MF%YRARPHY%LMPA) THEN
    CALL MF_PHYS_MPA(YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,            &
    & YDMF_PHYS_TMP, YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, &
    & YDFIELDS%YRCFU, YDFIELDS%YRXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, YDCPG_MISC%KOZO, PGPSDT2D,      &
    & ZSLBUF1AU, PB2, PGMVT1, PGFLT1, YDCPG_MISC%GPAR, PTRAJ_PHYS, YDDDH, YDCPG_MISC%FTCNS)
  ELSE
    ...
    IF (LLNEW) THEN
    CALL MF_PHYS_PAR_NEW(  YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,                    &
    & YDMF_PHYS_TMP, YDAPLPAR_TMP, YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, &
    & YDFIELDS%YRCFU, YDFIELDS%YRXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, YDCPG_MISC%KOZO, PGPSDT2D,                    &
    & ZSLBUF1AU, PB2, PGMVT1, PGFLT1, YDCPG_MISC%GPAR, PTRAJ_PHYS, YDDDH, YDCPG_MISC%FTCNS)
    ELSE
    CALL MF_PHYS_PAR(  YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,                        &
    & YDMF_PHYS_TMP, YDAPLPAR_TMP, YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, &
    & YDFIELDS%YRCFU, YDFIELDS%YRXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, YDCPG_MISC%KOZO, PGPSDT2D,                    &
    & ZSLBUF1AU, PB2, PGMVT1, PGFLT1, YDCPG_MISC%GPAR, PTRAJ_PHYS, YDDDH, YDCPG_MISC%FTCNS)
    ENDIF
...
IF ( LPC_FULL.AND.(.NOT.LPC_CHEAP).AND.LSLAG.AND.(LMPHYS.OR.LSIMPH) ) THEN
  ...
  CALL CPG_PT_ULP(YDGEOMETRY, YDFIELDS%YRGMV, YGFL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, LLGET, NFLSA,   &
  & NFLEN, PTENDU=ZSLBUF1AU(1, ISLB1U9), PTENDV=ZSLBUF1AU(1, ISLB1V9), PTENDT=ZSLBUF1AU(1, ISLB1T9),  &
  & PTENDVD=ZSLBUF1AU(1, ISLB1VD9), PTENDGFL=ZSLBUF1AU(1, ISLB1GFL9), PTENDSP=ZSLBUF1AU(1, MSLB1SP9), &
...
IF (NCURRENT_ITER == 0) THEN
  CALL CPG_DIA(YDGEOMETRY, YDCPG_DIM, YDCPG_TND, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF,   &
  & YDVARS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, YDFIELDS%YRCFU, YDFIELDS%YRXFU, YDMODEL, YDCPG_DDH%DDHI, &
  & LDCONFX, LDDIAB, LDFSTEP, LD_DFISTEP, PGMV, PGFL, PB2(1, MSLB2VVEL), PSD_XA, YDCPG_MISC%GPAR,     &
...
& PGMV, PGMVS, ZSLBUF1AU, PB2, PGMVT1, PGMVT1S, PGFLT1, PGMVTNDSI_DDH, PTRAJ_SLAG)
...
& PGFL, YDCPG_MISC%GPAR, PGMV, PGMVS, PTRAJ_PHYS)
