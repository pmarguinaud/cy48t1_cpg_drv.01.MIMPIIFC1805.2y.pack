...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG(YDGEOMETRY,YDCPG_DIM,YDTMP,YDCPG_TND,YDCPG_SLB,YDCPG_MISC,&
...
TYPE(CPG_SLB_TYPE),INTENT(INOUT) :: YDCPG_SLB
...
IF (LDUSEPB1) THEN
  DO JFLD=1,NFLDSLB1
    YDCPG_SLB%ZVIEW(1:YDCPG_DIM%KFDIA,JFLD)=0.0_JPRB
  ENDDO
ENDIF
...
IF (LDSLPHY.AND.((LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.LPC_FULL))) THEN
  ...
  & YDCPG_DIM%KFDIA, LDFSTEP, PDTPHY, YDCPG_SLB%ZVIEW(:, MSLB1UP9), YDCPG_SLB%ZVIEW(:, MSLB1VP9), YDCPG_SLB%ZVIEW(:, MSLB1TP9), &
  & YDCPG_SLB%ZVIEW(:, MSLB1GFLP9), PSAVTEND, PGFLSLP)
ENDIF
...
IF( (LMPHYS.OR.LSIMPH) .AND. (NCURRENT_ITER == 0) ) THEN  
  CALL MF_PHYS (YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS, YDMF_PHYS_TMP, &
              & YDAPLPAR_TMP, YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS,     &
              & LDCONFX, PDTPHY, PGPSDT2D, PGFL, PB2, PGMVT1, PGFLT1, PTRAJ_PHYS, YDDDH, YDCPG_SLB%ZVIEW)
ENDIF
...
IF ( LPC_FULL.AND.(.NOT.LPC_CHEAP).AND.LSLAG.AND.(LMPHYS.OR.LSIMPH) ) THEN
  ...
  & NFLEN, PTENDU=YDCPG_SLB%ZVIEW(:, ISLB1U9), PTENDV=YDCPG_SLB%ZVIEW(:, ISLB1V9), PTENDT=YDCPG_SLB%ZVIEW(:, ISLB1T9),  &
  & PTENDVD=YDCPG_SLB%ZVIEW(:, ISLB1VD9), PTENDGFL=YDCPG_SLB%ZVIEW(:, ISLB1GFL9), PTENDSP=YDCPG_SLB%ZVIEW(:, MSLB1SP9), &
...
& PGMV, PGMVS, YDCPG_SLB%ZVIEW, PB2, PGMVT1, PGMVT1S, PGFLT1, PGMVTNDSI_DDH, PTRAJ_SLAG)

IF (LDUSEPB1) THEN
  CALL CPG_PB1 (YDCPG_DIM, YDMODEL, YDSL, PB1, YDCPG_SLB%ZVIEW)
ENDIF
