...
TYPE(CPG_DIM_TYPE),INTENT(INOUT) :: YDCPG_DIM
...
IF (YDCPG_DIM%KIDIA   /= IST    )  CALL ABOR1 ('YDCPG_DIM%KIDIA   /= IST    ')
IF (YDCPG_DIM%KFDIA   /= IEND   )  CALL ABOR1 ('YDCPG_DIM%KFDIA   /= IEND   ')
IF (YDCPG_DIM%KBL     /= IBL    )  CALL ABOR1 ('YDCPG_DIM%KBL     /= IBL    ')
IF (YDCPG_DIM%KSTGLO  /= KSTGLO )  CALL ABOR1 ('YDCPG_DIM%KSTGLO  /= KSTGLO ')
IF (YDCPG_DIM%KGPTOT  /= NGPTOT )  CALL ABOR1 ('YDCPG_DIM%KGPTOT  /= NGPTOT ')
IF (YDCPG_DIM%KGPCOMP /= KGPCOMP)  CALL ABOR1 ('YDCPG_DIM%KGPCOMP /= KGPCOMP')
IF (YDCPG_DIM%KFDIE   /= IENDC  )  CALL ABOR1 ('YDCPG_DIM%KFDIE   /= IENDC  ')
IF (YDCPG_DIM%KIDIE   /= ISTC   )  CALL ABOR1 ('YDCPG_DIM%KIDIE   /= ISTC   ')

CALL CPG_GP(YDGEOMETRY,YDTMP%CPG_GP,YDCPG_TND,YDCPG_MISC,YDCPG_DYN0,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
!*       4.4.1   Call MF unlagged physics (predictor only if PC scheme).
  ...
  CALL MF_PHYS(YDGEOMETRY,YDCPG_MISC,YDCPG_PHY0,YDCPG_PHY9,YDMF_PHYS,YDCPG_DYN0,YDCPG_DYN9,&
...
IF (YSD_VF%YLSM%LSET) THEN
  YDCPG_MISC%LSM(IST:IEND)=YDMF_PHYS_SURF%GSD_VF%PLSM(IST:IEND)
ELSE
  YDCPG_MISC%LSM(IST:IEND)=0.0_JPRB
ENDIF
IF (YSP_RR%YT%LSET) THEN
  YDCPG_MISC%TSOL(IST:IEND)=YDMF_PHYS_SURF%GSP_RR%PT_T0(IST:IEND)
ELSE
