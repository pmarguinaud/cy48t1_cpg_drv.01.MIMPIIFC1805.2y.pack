...
USE DDH_MIX            , ONLY : TYP_DDH, SETDDH, CLEANDDH, RESET_DDHFLEX
...
IF (CDPART (1:1) == 'X') THEN
  ...
  IF (YDCPG_OPTS%LSLPHY.AND.((YRDYNA%LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.YRDYNA%LPC_FULL))) THEN
...
IF (CDPART (3:3) == 'X') THEN
  ...
  IF (LMPHYS.OR.LSIMPH) THEN
    ...
    IF (YRDYNA%LPC_FULL.AND.(.NOT.YRDYNA%LPC_CHEAP).AND.LSLAG) THEN
  ...
  IF (YRDYNA%LSLDIA .AND. (NCURRENT_ITER == NSITER)) THEN
  ...
  IF (LSLAG) THEN
    CALL CPG_DYN_SLG (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_DYN0, YDCPG_DYN9, &
                    & YDVARS, YDCPG_SL1, YDCPG_SL2, YDFIELDS%YRGMV, YDMODEL, SLHDA(YDCPG_BNDS%KSTGLO), SLHDD0(YDCPG_BNDS%KSTGLO), PGFL, &
