...
! DDH
REAL(KIND=JPRB), TARGET :: ZAUX3D(0:YDCPG_DIM%KFLEVG,YDMODEL%YRML_DIAG%YRMDDH%NDHIDH,YDDDH%NFIELDS3D_AUTO)
REAL(KIND=JPRB), TARGET :: ZAUX2D(YDMODEL%YRML_DIAG%YRMDDH%NDHIDH,YDDDH%NFIELDS2D_AUTO)
REAL(KIND=JPRB), TARGET :: ZAUXSM(YDCPG_DIM%KLON,1,YDCPG_DIM%KFLEVG,13)
INTEGER(KIND=JPIM) :: IDDHI(YDCPG_DIM%KLON)                  ! cf. KDDHI in CPG_GP
...
IF (LSDDH) THEN
  IF (NCURRENT_ITER == 0) CALL GPINIDDH(YDGEOMETRY,YDMDDH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,IDDHI,YDCPG_MISC%DHSF,YDCPG_DDH%DHCV,YDCPG_MISC%NEB,YDCPG_MISC%CLCT,YDCPG_DIM%KSTGLO)
