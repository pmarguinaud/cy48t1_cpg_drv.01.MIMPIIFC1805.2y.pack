...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDCPG_TND,YDCPG_SL1,YDCPG_SL2,YDCPG_MISC,&
...
IF (CDPART (1:1) == 'X') THEN
  ...
  CALL CPG_GP(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDTMP%CPG_GP, YDCPG_TND, YDCPG_MISC, YDCPG_GPAR, YDCPG_DYN0, YDCPG_DYN9, &
  ...
  IF (LDSLPHY.AND.((LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.LPC_FULL))) THEN
    ...
    & YDCPG_BNDS%KFDIA, LDFSTEP, PDTPHY, YDCPG_SL1%ZVIEW(:, MSLB1UP9), YDCPG_SL1%ZVIEW(:, MSLB1VP9), YDCPG_SL1%ZVIEW(:, MSLB1TP9), &
...
IF (CDPART (3:3) == 'X') THEN
  ...
  !     assumed to be zero and it is ignored.
      ...
      CALL CPG_PT_ULP(YDGEOMETRY, YDFIELDS%YRGMV, YGFL, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NCURRENT_ITER > 0, NFLSA,   &
  ...
  IF (NCURRENT_ITER == 0) THEN
    CALL CPG_DIA(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_GPAR, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF,   &
  ...
  CALL CPG_DYN(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_DYN0, YDCPG_DYN9, YDVARS, YDFIELDS%YRGMV, &
  & YDMODEL, PBETADT, PDT, SLHDA(YDCPG_BNDS%KSTGLO), SLHDD0(YDCPG_BNDS%KSTGLO), PGFL, KSETTLOFF, PGFLPC,       &
  ...
  CALL CPG_END(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDFIELDS%YRGMV, YDFIELDS%YRSURF, YDMODEL%YRML_PHY_EC%YREPHY, YDMODEL%YRML_GCONF, &
