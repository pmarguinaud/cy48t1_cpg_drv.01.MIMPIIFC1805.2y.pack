...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_SL1, YDCPG_SL2,            &
...
USE CPG_TYPE_MOD       , ONLY : CPG_DYN_TYPE, CPG_PHY_TYPE, CPG_MISC_TYPE, &
                              & CPG_TND_TYPE, CPG_DDH_TYPE, CPG_SL1_TYPE,  &
...
IF (CDPART (1:1) == 'X') THEN
  ...
  CALL CPG_GP(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_GPAR,  &
...
IF (CDPART (3:3) == 'X') THEN
  ...
  & YDVARS, YDFIELDS%YRGMV, YDMODEL, SLHDA(YDCPG_BNDS%KSTGLO), SLHDD0(YDCPG_BNDS%KSTGLO), &
  ...
  & YDMODEL%YRML_GCONF, YDDYN, YDMODEL%YRML_PHY_MF, GM(1), YDCPG_MISC%QS, YDMF_PHYS_SURF%GSP_RR%PT_T9, &
