...
& YDSL, PGFLSLP, PSAVTEND, PGMVTNDHD_DDH, PGFLTNDHD_DDH, PGPSDT2D, PSD_PF,                            &
...
IF (CDPART (1:1) == 'X') THEN
  ...
  & YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, YDCPG_OPTS%L_DFISTEP, YDCPG_OPTS%LFSTEP, YDCPG_OPTS%LDIAB, &
  & YDCPG_OPTS%ZDT, YDCPG_OPTS%ZTE, PGFL, PGMVTNDSL_DDH, PGFLTNDSL_DDH, YDCPG_SL2%ZVIEW, PGFLT1, YDCPG_MISC%KOZO, YDDDH)
  ...
  IF (YDCPG_OPTS%LSLPHY.AND.((LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.LPC_FULL))) THEN
    CALL EC_PHYS_LSLPHY(  YDGEOMETRY, YDSLPHY, YDMODEL%YRML_GCONF, YDMODEL%YRML_PHY_MF%YRPHY2, YDCPG_BNDS%KIDIA, &
    & YDCPG_BNDS%KFDIA, YDCPG_OPTS%LFSTEP, YDCPG_OPTS%ZDTPHY, YDCPG_SL1%ZVIEW(:, MSLB1UP9), YDCPG_SL1%ZVIEW(:, MSLB1VP9),             &
...
IF (CDPART (2:2) == 'X') THEN
  ...
  IF (LMPHYS.OR.LSIMPH) THEN
    ...
    IF (NCURRENT_ITER == 0) THEN  
      ...
      & YDMODEL, YDFIELDS, YDCPG_OPTS%LCONFX, YDCPG_OPTS%ZDTPHY, PGPSDT2D, PGFL, PGMVT1, PGFLT1, PTRAJ_PHYS, YDDDH)
    ENDIF
...
IF (CDPART (3:3) == 'X') THEN
  ...
  IF (NCURRENT_ITER == 0) THEN
    ...
    & YDMODEL, YDCPG_DDH%DDHI, YDCPG_OPTS%LCONFX, YDCPG_OPTS%LDIAB, YDCPG_OPTS%LFSTEP, YDCPG_OPTS%L_DFISTEP, PGMV, PGFL, YDCPG_SL2%ZVIEW(:, MSLB2VVEL), &
  ...
  IF (LSLDIA .AND. (NCURRENT_ITER == NSITER)) THEN
    ...
    CALL CPDYSLDIA(YDGEOMETRY, YDDPHY, YDMODEL%YRML_GCONF, NPROMA, YDCPG_OPTS%LFSTEP, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
  ...
  & YDVARS, YDFIELDS%YRGMV, YDMODEL, YDCPG_OPTS%ZBETADT, YDCPG_OPTS%ZDT, SLHDA(YDCPG_BNDS%KSTGLO), SLHDD0(YDCPG_BNDS%KSTGLO), &
  ...
  & YDMODEL%YRML_GCONF, YDDYN, YDMODEL%YRML_PHY_MF, YDCPG_OPTS%LCONFX, YDCPG_OPTS%LDIAB, GM(1), YDCPG_MISC%QS, YDMF_PHYS_SURF%GSP_RR%PT_T9, &
