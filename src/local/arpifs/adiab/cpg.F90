...
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZDIFCQ(:,:), ZDIFCQL(:,:), ZDIFCQN(:,:), ZDIFCS(:,:), ZDIFTQ(:,:), ZDIFTQL(:,:), ZDIFTQN(:,:), ZDIFTS(:,:), ZFCCQL(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFCCQN(:,:), ZFCSQL(:,:), ZFCSQN(:,:), ZFCQLNG(:,:), ZFCQNNG(:,:), ZFCQNG(:,:), ZFPLCL(:,:), ZFPLCN(:,:), ZFPLCG(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFPLCH(:,:), ZFPLSL(:,:), ZFPLSN(:,:), ZFPLSG(:,:), ZFPLSH(:,:), ZFPFPSL(:,:), ZFPFPSN(:,:), ZFPFPSG(:,:), ZFPFPCL(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFPFPCN(:,:), ZFPEVPSL(:,:), ZFPEVPSN(:,:), ZFPEVPSG(:,:), ZFPEVPCL(:,:), ZFPEVPCN(:,:), ZFPEVPCG(:,:), ZFRSO(:,:,:), ZFRTH(:,:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZSTRCU(:,:), ZSTRCV(:,:), ZSTRDU(:,:), ZSTRDV(:,:), ZSTRTU(:,:), ZSTRTV(:,:), ZSTRMU(:,:), ZSTRMV(:,:), ZDIFCQLC(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZDIFCQIC(:,:), ZFIMCC (:,:), ZFEDQLC(:,:), ZFEDQIC(:,:), ZFEDQRC(:,:), ZFEDQSC(:,:), ZFCNEGQLC(:,:), ZFCNEGQIC(:,:), ZFCNEGQRC(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFCNEGQSC(:,:), ZFRMH(:,:), ZFCHOZ(:,:), ZFDIS (:,:), ZFHPSL(:,:), ZFHPSN(:,:), ZFHPSG(:,:), ZFHPCL(:,:), ZFHPCN(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFHSCL(:,:), ZFHSCN(:,:), ZFHPCG(:,:), ZFHSSL(:,:), ZFHSSN(:,:), ZFEPFP(:,:), ZFCMPCQ(:,:), ZFCMPSN(:,:), ZFHSSG(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFCMPSL(:,:), ZFRSOC(:,:), ZFRTHC(:,:), ZFCHSP(:,:), ZFCLL(:,:), ZFCLN(:,:), ZFCS(:,:), ZFEVL(:,:), ZFEVN(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFEVV(:), ZFLWSP(:), ZFTR(:), ZFRSODS(:), ZFRSOPS(:), ZFRSDNI(:), ZFRSGNI(:), ZFRTHDS(:), ZFONTE(:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFGEL(:), ZFGELS(:), ZALB(:), ZGZ0(:), ZGZ0H(:), ZRUISL(:), ZRUISP(:), ZRUISS(:), ZFRSOPT(:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZFRSOLU(:), ZQCLS(:), ZTCLS(:), ZUCLS(:), ZVCLS(:), ZNUCLS(:), ZNVCLS(:), ZRHCLS(:), ZMRT(:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZCLCH(:), ZCLCM(:), ZCLCL(:), ZCLCC(:), ZCLPH(:), ZVEIN(:), ZDRNSHF(:), ZCAPE(:), ZCTOP(:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZMOCON(:), ZUGST(:), ZVGST(:), ZCT(:), ZTENDU(:,:), ZTENDV(:,:), ZFCQRNG(:,:), ZFCQSNG(:,:), ZFCQGNG(:,:)
REAL (KIND=JPRB), POINTER, CONTIGUOUS :: ZDIAGH(:), ZVISICLD(:), ZFLASH(:), ZVISIHYD(:), ZMXCLWC(:), ZTPWCLS(:), ZCUCONVCA(:), ZNLCONVCA(:)
...
ZDIFCQ(1:,0:)    => YDMF_PHYS%DIFCQ
ZDIFCQL(1:,0:)   => YDMF_PHYS%DIFCQL
ZDIFCQN(1:,0:)   => YDMF_PHYS%DIFCQN
ZDIFCS(1:,0:)    => YDMF_PHYS%DIFCS
ZDIFTQ(1:,0:)    => YDMF_PHYS%DIFTQ
ZDIFTQL(1:,0:)   => YDMF_PHYS%DIFTQL
ZDIFTQN(1:,0:)   => YDMF_PHYS%DIFTQN
ZDIFTS(1:,0:)    => YDMF_PHYS%DIFTS
ZFCCQL(1:,0:)    => YDMF_PHYS%FCCQL
ZFCCQN(1:,0:)    => YDMF_PHYS%FCCQN
ZFCSQL(1:,0:)    => YDMF_PHYS%FCSQL
ZFCSQN(1:,0:)    => YDMF_PHYS%FCSQN
ZFCQLNG(1:,0:)   => YDMF_PHYS%FCQLNG
ZFCQNNG(1:,0:)   => YDMF_PHYS%FCQNNG
ZFCQNG(1:,0:)    => YDMF_PHYS%FCQNG
ZFPLCL(1:,0:)    => YDMF_PHYS%FPLCL
ZFPLCN(1:,0:)    => YDMF_PHYS%FPLCN
ZFPLCG(1:,0:)    => YDMF_PHYS%FPLCG
ZFPLCH(1:,0:)    => YDMF_PHYS%FPLCH
ZFPLSL(1:,0:)    => YDMF_PHYS%FPLSL
ZFPLSN(1:,0:)    => YDMF_PHYS%FPLSN
ZFPLSG(1:,0:)    => YDMF_PHYS%FPLSG
ZFPLSH(1:,0:)    => YDMF_PHYS%FPLSH
ZFPFPSL(1:,0:)   => YDMF_PHYS%FPFPSL
ZFPFPSN(1:,0:)   => YDMF_PHYS%FPFPSN
ZFPFPSG(1:,0:)   => YDMF_PHYS%FPFPSG
ZFPFPCL(1:,0:)   => YDMF_PHYS%FPFPCL
ZFPFPCN(1:,0:)   => YDMF_PHYS%FPFPCN
ZFPEVPSL(1:,0:)  => YDMF_PHYS%FPEVPSL
ZFPEVPSN(1:,0:)  => YDMF_PHYS%FPEVPSN
ZFPEVPSG(1:,0:)  => YDMF_PHYS%FPEVPSG
ZFPEVPCL(1:,0:)  => YDMF_PHYS%FPEVPCL
ZFPEVPCN(1:,0:)  => YDMF_PHYS%FPEVPCN
ZFPEVPCG(1:,0:)  => YDMF_PHYS%FPEVPCG
ZFRSO(1:,0:,1:)  => YDMF_PHYS%FRSO
ZFRTH(1:,0:,1:)  => YDMF_PHYS%FRTH
ZSTRCU(1:,0:)    => YDMF_PHYS%STRCU
ZSTRCV(1:,0:)    => YDMF_PHYS%STRCV
ZSTRDU(1:,0:)    => YDMF_PHYS%STRDU
ZSTRDV(1:,0:)    => YDMF_PHYS%STRDV
ZSTRTU(1:,0:)    => YDMF_PHYS%STRTU
ZSTRTV(1:,0:)    => YDMF_PHYS%STRTV
ZSTRMU(1:,0:)    => YDMF_PHYS%STRMU
ZSTRMV(1:,0:)    => YDMF_PHYS%STRMV
ZDIFCQLC(1:,0:)  => YDMF_PHYS%DIFCQLC
ZDIFCQIC(1:,0:)  => YDMF_PHYS%DIFCQIC
ZFIMCC (1:,0:)   => YDMF_PHYS%FIMCC
ZFEDQLC(1:,0:)   => YDMF_PHYS%FEDQLC
ZFEDQIC(1:,0:)   => YDMF_PHYS%FEDQIC
ZFEDQRC(1:,0:)   => YDMF_PHYS%FEDQRC
ZFEDQSC(1:,0:)   => YDMF_PHYS%FEDQSC
ZFCNEGQLC(1:,0:) => YDMF_PHYS%FCNEGQLC
ZFCNEGQIC(1:,0:) => YDMF_PHYS%FCNEGQIC
ZFCNEGQRC(1:,0:) => YDMF_PHYS%FCNEGQRC
ZFCNEGQSC(1:,0:) => YDMF_PHYS%FCNEGQSC
ZFRMH(1:,0:)     => YDMF_PHYS%FRMH
ZFCHOZ(1:,0:)    => YDMF_PHYS%FCHOZ
ZFDIS (1:,0:)    => YDMF_PHYS%FDIS
ZFHPSL(1:,0:)    => YDMF_PHYS%FHPSL
ZFHPSN(1:,0:)    => YDMF_PHYS%FHPSN
ZFHPSG(1:,0:)    => YDMF_PHYS%FHPSG
ZFHPCL(1:,0:)    => YDMF_PHYS%FHPCL
ZFHPCN(1:,0:)    => YDMF_PHYS%FHPCN
ZFHSCL(1:,0:)    => YDMF_PHYS%FHSCL
ZFHSCN(1:,0:)    => YDMF_PHYS%FHSCN
ZFHPCG(1:,0:)    => YDMF_PHYS%FHPCG
ZFHSSL(1:,0:)    => YDMF_PHYS%FHSSL
ZFHSSN(1:,0:)    => YDMF_PHYS%FHSSN
ZFEPFP(1:,0:)    => YDMF_PHYS%FEPFP
ZFCMPCQ(1:,0:)   => YDMF_PHYS%FCMPCQ
ZFCMPSN(1:,0:)   => YDMF_PHYS%FCMPSN
ZFHSSG(1:,0:)    => YDMF_PHYS%FHSSG
ZFCMPSL(1:,0:)   => YDMF_PHYS%FCMPSL
ZFRSOC(1:,0:)    => YDMF_PHYS%FRSOC
ZFRTHC(1:,0:)    => YDMF_PHYS%FRTHC
ZFCHSP           => YDMF_PHYS%FCHSP
ZFCLL            => YDMF_PHYS%FCLL
ZFCLN            => YDMF_PHYS%FCLN
ZFCS             => YDMF_PHYS%FCS
ZFEVL            => YDMF_PHYS%FEVL
ZFEVN            => YDMF_PHYS%FEVN
ZFEVV            => YDMF_PHYS%FEVV
ZFLWSP           => YDMF_PHYS%FLWSP
ZFTR             => YDMF_PHYS%FTR
ZFRSODS          => YDMF_PHYS%FRSODS
ZFRSOPS          => YDMF_PHYS%FRSOPS
ZFRSDNI          => YDMF_PHYS%FRSDNI
ZFRSGNI          => YDMF_PHYS%FRSGNI
ZFRTHDS          => YDMF_PHYS%FRTHDS
ZFONTE           => YDMF_PHYS%FONTE
ZFGEL            => YDMF_PHYS%FGEL
ZFGELS           => YDMF_PHYS%FGELS
ZALB             => YDMF_PHYS%ALB
ZGZ0             => YDMF_PHYS%GZ0
ZGZ0H            => YDMF_PHYS%GZ0H
ZRUISL           => YDMF_PHYS%RUISL
ZRUISP           => YDMF_PHYS%RUISP
ZRUISS           => YDMF_PHYS%RUISS
ZFRSOPT          => YDMF_PHYS%FRSOPT
ZFRSOLU          => YDMF_PHYS%FRSOLU
ZQCLS            => YDMF_PHYS%QCLS
ZTCLS            => YDMF_PHYS%TCLS
ZUCLS            => YDMF_PHYS%UCLS
ZVCLS            => YDMF_PHYS%VCLS
ZNUCLS           => YDMF_PHYS%NUCLS
ZNVCLS           => YDMF_PHYS%NVCLS
ZRHCLS           => YDMF_PHYS%RHCLS
ZMRT             => YDMF_PHYS%MRT
ZCLCH            => YDMF_PHYS%CLCH
ZCLCM            => YDMF_PHYS%CLCM
ZCLCL            => YDMF_PHYS%CLCL
ZCLCC            => YDMF_PHYS%CLCC
ZCLPH            => YDMF_PHYS%CLPH
ZVEIN            => YDMF_PHYS%VEIN
ZDRNSHF          => YDMF_PHYS%DRNSHF
ZCAPE            => YDMF_PHYS%CAPE
ZCTOP            => YDMF_PHYS%CTOP
ZMOCON           => YDMF_PHYS%MOCON
ZUGST            => YDMF_PHYS%UGST
ZVGST            => YDMF_PHYS%VGST
ZCT              => YDMF_PHYS%CT
ZTENDU           => YDMF_PHYS%TENDU
ZTENDV           => YDMF_PHYS%TENDV
ZFCQRNG(1:,0:)   => YDMF_PHYS%FCQRNG
ZFCQSNG(1:,0:)   => YDMF_PHYS%FCQSNG
ZFCQGNG(1:,0:)   => YDMF_PHYS%FCQGNG
ZDIAGH           => YDMF_PHYS%DIAGH
ZVISICLD         => YDMF_PHYS%VISICLD
ZFLASH           => YDMF_PHYS%FLASH
ZVISIHYD         => YDMF_PHYS%VISIHYD
ZMXCLWC          => YDMF_PHYS%MXCLWC
ZTPWCLS          => YDMF_PHYS%TPWCLS
ZCUCONVCA        => YDMF_PHYS%CUCONVCA
ZNLCONVCA        => YDMF_PHYS%NLCONVCA
