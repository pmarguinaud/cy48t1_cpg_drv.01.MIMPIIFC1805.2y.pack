...
IF ((NSTEP >= 0).AND.LSDDH.AND.(.NOT.LINITER).AND.(.NOT.LDCONFX)) THEN 
  ...
  IF(LAGPHY.OR.(.NOT.LDLDIAB)) THEN
    ...
    LLPHY=.FALSE.
    CALL GPRH(.FALSE.,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,1.0_JPRB,0.0_JPRB,&
    ...
    CALL CPDYDDH(YDVAB,YDGMV,YDMODEL%YRML_GCONF,YDDPHY,YDMODEL%YRML_DIAG,YDPHY,NPROMA,NPROMNH,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,&
  ...
  ELSE
    ...
    IF(.NOT.(LMPHYS.OR.LEPHYS)) THEN
      LLPHY=.FALSE.
      CALL GPRH(.FALSE.,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,&
...
!CALL POSINSUB('CPG_DIA',IPOS)
    ...
    IF((LMPHYS.OR.LEPHYS).AND..NOT.LMPA.AND..NOT.LFLEXDIA) THEN
      CALL CPPHDDH (YDMODEL%YRML_DIAG,YDEPHY,YDRIP,YDMODEL%YRML_PHY_MF,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,.TRUE.,&
...
!CALL POSINSUB('CPG_DIA',IPOS)
  ...
  IF (LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      CALL CPCUDDH_OMP(YDMDDH,YDMODEL%YRML_DIAG%YRTDDH,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,KDDHI,YDDDH,LLDYN)
    ELSE
      CALL CPCUDDH(YDMODEL%YRML_DIAG,YDRIP,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,KDDHI,YDCPG_MISC%DHSF,RDDH_FIELD,RDDHSURF_FIELD,LLDYN,LLPHY,NTOTFIELD)
    ENDIF 
  ELSE 
    CALL CPCUDDH(YDMODEL%YRML_DIAG,YDRIP,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,KDDHI,YDCPG_MISC%DHSF,PDHCV,ZDHCS,LLDYN,LLPHY,NDHCVSUN)
  ENDIF 
...
IF(LLCALL_CPCFU)THEN  
  CALL CPCFU(YDGEOMETRY,YDCFU,YDRIP,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_BNDS%KIDIE,YDCPG_BNDS%KFDIE,YDCPG_BNDS%KSTGLO,LDLFSTEP,&
...
IF(LLCALL_CUMCPLDM) THEN  
  ILEVCO=NFLEVG
  CALL CUMCPL(YDGEOMETRY%YRDIM,YDRIP,YDPHY,ILEVCO,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_BNDS%KSTGLO,NTSSG,&
...
IF (LLCALL_CPXFU) THEN
  ...
  ENDIF
  CALL CPXFU(YDGEOMETRY,YDXFU,YDPHY%YRDPRECIPS,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_BNDS%KIDIE,YDCPG_BNDS%KFDIE,YDCPG_BNDS%KSTGLO,&
...
IF (LDIRCLSMOD) THEN
  CALL CPCLS_ASSIM(YDGEOMETRY,YDSURF,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA, &
