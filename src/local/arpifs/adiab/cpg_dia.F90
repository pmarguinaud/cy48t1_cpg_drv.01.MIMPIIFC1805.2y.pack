...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_DIA(YDGEOMETRY,YDCPG_DIM, YDCPG_OPTS,YDCPG_TND,YDCPG_MISC,YDCPG_GPAR,YDMF_PHYS,&
 & YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDGMV,YDSURF,YDCFU,YDXFU,YDMODEL,&
 !---------------------------------------------------------------------
 ! - INPUT .
 & KDDHI,&
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDVAB=>YDGEOMETRY%YRVAB, &
 &  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_DIM%KBL), YDCSGEOM=>YDGEOMETRY%YRCSGEOM(YDCPG_DIM%KBL), YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY, &
...
IF ((NSTEP >= 0).AND.LSDDH.AND.(.NOT.LINITER).AND.(.NOT.LDCONFX)) THEN 
  ...
  IF(LAGPHY.OR.(.NOT.LDLDIAB)) THEN
    ...
    LLPHY=.FALSE.
    CALL GPRH(.FALSE.,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,1.0_JPRB,0.0_JPRB,&
     & YDVARS%Q%T0,YDVARS%T%T0,YDCPG_DYN0%PREF,ZQSAT,ZRH)  
  
    ZDUM1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=0.0_JPRB
  
    IF(YA%LACTIVE) THEN
      ZNEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDVARS%A%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ELSE
      IF (LMPA) THEN
        ZNEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=0.0_JPRB    ! ???
      ELSE
        ZNEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDCPG_MISC%NEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
      ENDIF
    ENDIF
    ...
    CALL CPDYDDH(YDVAB,YDGMV,YDMODEL%YRML_GCONF,YDDPHY,YDMODEL%YRML_DIAG,YDPHY,NPROMA,NPROMNH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,&
  ...
  ELSE
    ...
    IF(.NOT.(LMPHYS.OR.LEPHYS)) THEN
      LLPHY=.FALSE.
      CALL GPRH(.FALSE.,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,&
    ...
    ELSE
      IF (LMPA) THEN
        CALL GPRH(.FALSE.,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,&
         & 1.0_JPRB,0.0_JPRB,YDVARS%Q%T0,YDVARS%T%T0,YDCPG_DYN0%PREF,ZQSAT,ZRH)  
      ELSE
        ZRH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDCPG_MISC%RH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
      ENDIF
      LLPHY=.TRUE.
    ENDIF

    IF (LMPA) THEN
      ZQS(1:NPROMA) =  YDCPG_GPAR%ZVIEW(1:NPROMA,MVQS)
      ZTSOL(1:NPROMA) = YDCPG_GPAR%ZVIEW(1:NPROMA,MVTS)
    ELSE 
...
!CALL POSINSUB('CPG_DIA',IPOS)
    ...
    IF((LMPHYS.OR.LEPHYS).AND..NOT.LMPA.AND..NOT.LFLEXDIA) THEN
      CALL CPPHDDH (YDMODEL%YRML_DIAG,YDEPHY,YDRIP,YDMODEL%YRML_PHY_MF,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,.TRUE.,&
...
!CALL POSINSUB('CPG_DIA',IPOS)
  ...
  IF (LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      CALL CPCUDDH_OMP(YDMDDH,YDMODEL%YRML_DIAG%YRTDDH,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,KDDHI,YDDDH,LLDYN)
    ELSE
      CALL CPCUDDH(YDMODEL%YRML_DIAG,YDRIP,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,KDDHI,YDCPG_MISC%DHSF,RDDH_FIELD,RDDHSURF_FIELD,LLDYN,LLPHY,NTOTFIELD)
    ENDIF 
  ELSE 
    CALL CPCUDDH(YDMODEL%YRML_DIAG,YDRIP,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,KDDHI,YDCPG_MISC%DHSF,PDHCV,ZDHCS,LLDYN,LLPHY,NDHCVSUN)
  ENDIF 
...
IF (LLCALL_CPCFU.OR.LLCALL_CPXFU.OR.LLCALL_CUMCPLDM) THEN
  DO JLEV=0,NFLEVG
    ZSTRCU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRCU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ZSTRCV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRCV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ZSTRDU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRDU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ZSTRDV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRDV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ZSTRTU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRTU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ZSTRTV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS%OUT%STRTV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
  ENDDO
ENDIF

! * Z(U,V)CLS and Z(U,V)GST are used for XFU.
IF (LLCALL_CPXFU .OR. LDIRCLSMOD) THEN
  ZUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%UCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ZVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%VCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ZNUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%NUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ZNVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%NVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ZUGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%UGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ZVGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%VGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
ENDIF
...
IF (LDLDIAB.OR.(LSIMPH.AND.(.NOT.LTRAJPS))) THEN  
  IF (LLCALL_CPCFU.OR.LLCALL_CPXFU.OR.LLCALL_CUMCPLDM) THEN
    DO JLEV=0,NFLEVG
      ZSTRCU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRCU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      ZSTRCV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRCV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      ZSTRDU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRDU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      ZSTRDV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRDV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      ZSTRTU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRTU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      ZSTRTV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=ZSTRTV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ENDDO
  ENDIF
  IF (LLCALL_CPXFU .OR. LDIRCLSMOD) THEN
    ZUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ZVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ZNUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZNUCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ZNVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZNVCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ZUGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZUGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
    ZVGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=ZVGST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ENDIF
ENDIF
...
IF(LLCALL_CPCFU)THEN  
  CALL CPCFU(YDGEOMETRY,YDCFU,YDRIP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KIDIE,YDCPG_DIM%KFDIE,YDCPG_DIM%KSTGLO,LDLFSTEP,&
...
IF(LLCALL_CUMCPLDM) THEN  
  ILEVCO=NFLEVG
  CALL CUMCPL(YDGEOMETRY%YRDIM,YDRIP,YDPHY,ILEVCO,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KSTGLO,NTSSG,&
...
IF (LLCALL_CPXFU) THEN
  IF (LDPRECIPS) THEN
    ZDPRECIPS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NDTPREC)=YDMF_PHYS_SURF%GSD_XP%PPRECIP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NDTPREC)
  ENDIF
  IF (LDPRECIPS2) THEN
    ZDPRECIPS2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NDTPREC2)=YDMF_PHYS_SURF%GSD_XP2%PPRECIP2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NDTPREC2)
  ENDIF
  CALL CPXFU(YDGEOMETRY,YDXFU,YDPHY%YRDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KIDIE,YDCPG_DIM%KFDIE,YDCPG_DIM%KSTGLO,&
...
IF (LDIRCLSMOD) THEN
  CALL CPCLS_ASSIM(YDGEOMETRY,YDSURF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, &
