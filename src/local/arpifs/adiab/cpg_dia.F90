...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_DIA(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDGMV,YDSURF,YDCFU,YDXFU,YDMODEL,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
!CALL POSINSUB('CPG_DIA',IPOS)
    ...
    IF((LMPHYS.OR.LEPHYS).AND..NOT.LMPA.AND..NOT.LFLEXDIA) THEN
      CALL CPPHDDH (YDMODEL%YRML_DIAG,YDEPHY,YDRIP,YDMODEL%YRML_PHY_MF,NPROMA,KST,KEND,NFLEVG,.TRUE.,&
       ...
       & YDCPG_DYN0%PHI,YDMF_PHYS%UCLS,YDMF_PHYS%VCLS,YDMF_PHYS%TCLS,YDMF_PHYS%QCLS,YDMF_PHYS%CLPH,YDMF_PHYS%ALB,YDMF_PHYS%GZ0,YDMF_PHYS%GZ0H,&
       & PDHCV,ZDHCS,PFTCNS)
    ENDIF
...
IF(LLCALL_CPCFU)THEN  
  CALL CPCFU(YDGEOMETRY,YDCFU,YDRIP,KST,KEND,KSTC,KENDC,KSTGLO,LDLFSTEP,&
   ...
   & YDCPG_DYN0%XYB%ZVIEW(:,:,YYTXYB0%M_DELP),PNEB,PQICE,PQLI,YDCPG_DYN0%PRE(1:,NFLEVG),YDMF_PHYS_SURF%GSP_SG%PF_T0,&
   & YDVARS%O3%T0,YDVARS%Q%T0,YDMF_PHYS_SURF%GSP_RR%PW_T0,YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
...
IF(LLCALL_CUMCPLDM) THEN  
  ...
  CALL CUMCPL(YDGEOMETRY%YRDIM,YDRIP,YDPHY,ILEVCO,KST,KEND,KSTGLO,NTSSG,&
   ...
   & YDMF_PHYS%RUISS,YDMF_PHYS%RUISP,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDMF_PHYS%DRNSHF,YDMF_PHYS%ALB,&
   & ZUCLS,ZVCLS)
ENDIF
