...
& YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDGMV, YDSURF, YDCFU, YDXFU, YDMODEL, KDDHI, LDCONFX, LDLDIAB, &
& LDLFSTEP, LD_DFISTEP, PGMV, PGFL, POMEGA, PEXT, PCOVPTOT, PGMVTNDSI, PGMVTNDHD, PGFLTNDHD, PDHCV,  &
& YDDDH, PFTCNS)
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDVAB=>YDGEOMETRY%YRVAB, &
 &  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_BNDS%KBL), YDCSGEOM=>YDGEOMETRY%YRCSGEOM(YDCPG_BNDS%KBL), YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY, &
...
IF ((NSTEP >= 0).AND.LSDDH.AND.(.NOT.LINITER).AND.(.NOT.LDCONFX)) THEN 
  ...
  IF(LAGPHY.OR.(.NOT.LDLDIAB)) THEN
    ...
    LLPHY=.FALSE.
    CALL GPRH(.FALSE., NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, 1.0_JPRB, 0.0_JPRB, YDVARS%Q%T0, &
    & YDVARS%T%T0, YDCPG_DYN0%PREF, ZQSAT, ZRH)  
  
    ZDUM1(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=0.0_JPRB
  
    IF(YA%LACTIVE) THEN
      ZNEB(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=YDVARS%A%T0(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)
    ELSE
      IF (LMPA) THEN
        ZNEB(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=0.0_JPRB    ! ???
      ELSE
        ZNEB(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=YDCPG_MISC%NEB(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)
      ENDIF
    ENDIF
    ...
    CALL CPDYDDH(YDVAB, YDGMV, YDMODEL%YRML_GCONF, YDDPHY, YDMODEL%YRML_DIAG, YDPHY, NPROMA, NPROMNH,                 &
    & YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, YDGSGEOM, YDCSGEOM, YDCPG_DYN0%XYB%ZVIEW, YDCPG_DYN0%RCP%ZVIEW,     &
  ...
  ELSE
    ...
    ELSE
      IF (LMPA) THEN
        ...
        & YDVARS%T%T0, YDCPG_DYN0%PREF, ZQSAT, ZRH)  
      ELSE
        ZRH(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=YDCPG_MISC%RH(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)
      ENDIF
      LLPHY=.TRUE.
    ENDIF
...
!CALL POSINSUB('CPG_DIA',IPOS)
    ...
    IF((LMPHYS.OR.LEPHYS).AND..NOT.LMPA.AND..NOT.LFLEXDIA) THEN
      CALL CPPHDDH (YDMODEL%YRML_DIAG, YDEPHY, YDRIP, YDMODEL%YRML_PHY_MF, NPROMA, YDCPG_BNDS%KIDIA,                          &
      & YDCPG_BNDS%KFDIA, NFLEVG, .TRUE., YDGSGEOM, ZUZGEO, ZVMGEO, YDMF_PHYS_SURF%GSP_SG%PF_T0, YDMF_PHYS_SURF%GSP_SB%PT_T0, &
...
!CALL POSINSUB('CPG_DIA',IPOS)
  ...
  IF (LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      CALL CPCUDDH_OMP(YDMDDH, YDMODEL%YRML_DIAG%YRTDDH, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
      & NFLEVG, KDDHI, YDDDH, LLDYN)
    ELSE
      CALL CPCUDDH(YDMODEL%YRML_DIAG, YDRIP, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, &
      & KDDHI, YDCPG_MISC%DHSF, RDDH_FIELD, RDDHSURF_FIELD, LLDYN, LLPHY, NTOTFIELD)
    ENDIF 
  ELSE 
    CALL CPCUDDH(YDMODEL%YRML_DIAG, YDRIP, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, KDDHI, &
    & YDCPG_MISC%DHSF, PDHCV, ZDHCS, LLDYN, LLPHY, NDHCVSUN)
  ENDIF 
...
IF (LDLDIAB.OR.(LSIMPH.AND.(.NOT.LTRAJPS))) THEN  
  IF (LLCALL_CPCFU.OR.LLCALL_CPXFU.OR.LLCALL_CUMCPLDM) THEN
    DO JLEV=0,NFLEVG
      ZSTRCU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRCU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
      ZSTRCV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRCV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
      ZSTRDU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRDU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
      ZSTRDV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRDV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
      ZSTRTU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRTU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
      ZSTRTV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)=ZSTRTV(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JLEV)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ENDDO
  ENDIF
  IF (LLCALL_CPXFU .OR. LDIRCLSMOD) THEN
    ZUCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZUCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ZVCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZVCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ZNUCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZNUCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ZNVCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZNVCLS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ZUGST(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZUGST(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
    ZVGST(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=ZVGST(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)/YDGSGEOM%GM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
  ENDIF
ENDIF
...
IF(LLCALL_CPCFU)THEN  
  CALL CPCFU(YDGEOMETRY, YDCFU, YDRIP, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_BNDS%KIDIE, YDCPG_BNDS%KFDIE,                &
  & YDCPG_BNDS%KSTGLO, LDLFSTEP, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS,          &
  ...
  & YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRTHC, YDMF_PHYS%OUT%FDIS(:, NFLEVG), ZSTRCU, ZSTRCV, ZSTRDU,                           &
  & ZSTRDV, ZSTRTU, ZSTRTV, YDCPG_DYN0%XYB%DELP, YDCPG_MISC%NEB, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDCPG_DYN0%PRE(1:, NFLEVG), &
...
IF(LLCALL_CUMCPLDM) THEN  
  ILEVCO=NFLEVG
  CALL CUMCPL(YDGEOMETRY%YRDIM, YDRIP, YDPHY, ILEVCO, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_BNDS%KSTGLO,             &
  & NTSSG, ZSTRTU(1, NFLEVG), ZSTRTV(1, NFLEVG), YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FCLL,             &
  & YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FPLCL(:, NFLEVG),       &
  & YDMF_PHYS%OUT%FPLCN(:, NFLEVG), YDMF_PHYS%OUT%FPLSL(:, NFLEVG), YDMF_PHYS%OUT%FPLSN(:, NFLEVG), YDMF_PHYS%OUT%RUISS, &
...
IF (LLCALL_CPXFU) THEN
  IF (LDPRECIPS) THEN
    ZDPRECIPS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NDTPREC)=YDMF_PHYS_SURF%GSD_XP%PPRECIP(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NDTPREC)
  ENDIF
  IF (LDPRECIPS2) THEN
    ZDPRECIPS2(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NDTPREC2)=YDMF_PHYS_SURF%GSD_XP2%PPRECIP2(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NDTPREC2)
  ENDIF
  CALL CPXFU(YDGEOMETRY, YDXFU, YDPHY%YRDPRECIPS, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_BNDS%KIDIE,               &
...
IF (LDIRCLSMOD) THEN
  CALL CPCLS_ASSIM(YDGEOMETRY, YDSURF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDMF_PHYS_SURF%GSP_CL%PUCLS_T1,             &
