...
TYPE(CPG_DYN_TYPE), ALLOCATABLE :: YLCPG_DYN0_TID (:), YLCPG_DYN9_TID (:)
TYPE(CPG_PHY_TYPE), ALLOCATABLE :: YLCPG_PHY0_TID (:), YLCPG_PHY9_TID (:)

TYPE(CPG_TND_TYPE) :: YLCPG_TND
TYPE(CPG_MISC_TYPE):: YLCPG_MISC
TYPE(CPG_TMP_TYPE) :: YLCPG_TMP

TYPE(CPG_TND_TYPE),  ALLOCATABLE :: YLCPG_TND_TID (:)
TYPE(CPG_MISC_TYPE), ALLOCATABLE :: YLCPG_MISC_TID (:)
TYPE(CPG_TMP_TYPE),  ALLOCATABLE :: YLCPG_TMP_TID (:)

TYPE(CPG_OPTS_TYPE):: YLCPG_OPTS
TYPE(CPG_BNDS_TYPE):: YLCPG_BNDS
TYPE(CPG_DDH_TYPE) :: YLCPG_DDH

TYPE(CPG_DDH_TYPE), ALLOCATABLE :: YLCPG_DDH_TID (:)

TYPE(CPG_SL1_TYPE)  :: YLCPG_SL1
TYPE(CPG_SL2_TYPE)  :: YLCPG_SL2

TYPE(CPG_SL1_TYPE), ALLOCATABLE :: YLCPG_SL1_TID (:)
TYPE(CPG_SL2_TYPE), ALLOCATABLE :: YLCPG_SL2_TID (:)

TYPE(CPG_GPAR_TYPE) :: YLCPG_GPAR
TYPE(MF_PHYS_TYPE)  :: YLMF_PHYS

TYPE(CPG_GPAR_TYPE), ALLOCATABLE :: YLCPG_GPAR_TID (:)
TYPE(MF_PHYS_TYPE),  ALLOCATABLE :: YLMF_PHYS_TID (:)


INTEGER (KIND=JPIM) :: NTID, ITID

LOGICAL, ALLOCATABLE :: LLUPDATE (:)
...
ALLOCATE (YLCPG_DYN0_TID (NTID), SOURCE=YLCPG_DYN0)
ALLOCATE (YLCPG_PHY0_TID (NTID), SOURCE=YLCPG_PHY0)
...
ALLOCATE (YLCPG_SL1_TID (NTID), SOURCE=YLCPG_SL1)
ALLOCATE (YLCPG_SL2_TID (NTID), SOURCE=YLCPG_SL2)
...
ALLOCATE (YLCPG_DYN9_TID (NTID), SOURCE=YLCPG_DYN9)
ALLOCATE (YLCPG_PHY9_TID (NTID), SOURCE=YLCPG_PHY9)

CALL YLCPG_TMP%INIT (YDFIELDS%REGISTRY, NLEV=NFLEVG, PERSISTENT=LLPERSISTENT)

ALLOCATE (YLCPG_TMP_TID (NTID), SOURCE=YLCPG_TMP)

CALL YLCPG_DDH%INIT (YDFIELDS%REGISTRY, NLEV=NFLEVG, YDMODEL=YDMODEL, YDDDH=YDDDH, PERSISTENT=LLPERSISTENT)

ALLOCATE (YLCPG_DDH_TID (NTID), SOURCE=YLCPG_DDH)
...
ALLOCATE (YLCPG_TND_TID  (NTID), SOURCE=YLCPG_TND)
ALLOCATE (YLCPG_MISC_TID (NTID), SOURCE=YLCPG_MISC)
ALLOCATE (YLCPG_GPAR_TID (NTID), SOURCE=YLCPG_GPAR)
...
ALLOCATE (YLMF_PHYS_TID (NTID), SOURCE=YLMF_PHYS)
...
!$OMP&  FIRSTPRIVATE (YDVARS, YLMF_PHYS_SURF, YLCPG_BNDS, ITID)
DO JKGLO=1,KGPCOMP,NPROMA
  ...
  CALL YLCPG_DYN0_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
  CALL YLCPG_DYN9_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
  CALL YLMF_PHYS_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
  CALL YLCPG_SL2_TID (ITID)%UPDATE_VIEW (IBL)

  IF (LLUPDATE (ITID)) THEN
    CALL YLCPG_PHY0_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
    CALL YLCPG_PHY9_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
    CALL YLCPG_TND_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
    CALL YLCPG_TMP_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
    CALL YLCPG_MISC_TID (ITID)%UPDATE_VIEW (BLOCK_INDEX=IBL)
    CALL YLCPG_GPAR_TID (ITID)%UPDATE_VIEW (IBL)
    CALL YLCPG_SL1_TID (ITID)%UPDATE_VIEW (IBL)
    CALL YLCPG_DDH_TID (ITID)%UPDATE_VIEW (IBL)
    LLUPDATE (ITID) = .FALSE.
  ENDIF
  ...
  CALL CPG(YDGEOMETRY, YLCPG_BNDS, YLCPG_OPTS, YLCPG_TMP_TID (ITID), YLCPG_TND_TID (ITID), YLCPG_SL1_TID (ITID),           &
  & YLCPG_SL2_TID (ITID), YLCPG_MISC_TID (ITID), YLCPG_GPAR_TID (ITID), YLCPG_PHY0_TID (ITID), YLCPG_PHY9_TID (ITID),      &
  & YLMF_PHYS_TID (ITID), YLCPG_DDH_TID (ITID), YLCPG_DYN0_TID (ITID), YLCPG_DYN9_TID (ITID),                              &
  & YLMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, LLCONFX, LD_DFISTEP, LLFSTEP, LLDIAB, LLSLPHY,                              &
