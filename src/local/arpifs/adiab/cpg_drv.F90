...
CALL YLCPG_MISC%INIT (YDFIELDS%REGISTRY, YDCPG_DIM=YLCPG_DIM, PERSISTENT=LLPERSISTENT)
CALL YLCPG_GPAR%INIT (YDFIELDS%REGISTRY, YLCPG_DIM, YDMODEL%YRML_PHY_MF%YRPARAR, PERSISTENT=LLPERSISTENT)

CALL YLMF_PHYS%OUT%INIT (YDFIELDS%REGISTRY, YDCPG_DIM=YLCPG_DIM, PERSISTENT=LLPERSISTENT)
...
CALL YLMF_PHYS%GRA%INIT (YDFIELDS%REGISTRY, YDCPG_DIM=YLCPG_DIM, LDMF_PHYS=LMPHYS.OR.LSIMPH, PG=PGRADIENT, PERSISTENT=LLPERSISTENT)
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LMPHYS .OR. LSIMPH) THEN
  ...
  ! From aplpar.F90
  IF (.NOT. YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM) THEN 
    YDMODEL%YRML_PHY_RAD%YRERAD%NAER=0
    YDMODEL%YRML_PHY_RAD%YRERAD%NRADFR=YDMODEL%YRML_GCONF%YRRIP%NSTOP+1
  ENDIF
  ...
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LDPRECIPS) THEN
    YDMODEL%YRML_PHY_MF%YRPHY%YRDPRECIPS%NDTPRECCUR=INT(MOD(ZSTATI/TSTEP,REAL(NDTPREC)))+1_JPIM
  ENDIF
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LDPRECIPS2) THEN
    YDMODEL%YRML_PHY_MF%YRPHY%YRDPRECIPS%NDTPRECCUR2=INT(MOD(ZSTATI/TSTEP,REAL(NDTPREC2)))+1_JPIM
  ENDIF
