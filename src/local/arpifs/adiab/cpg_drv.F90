...
IF (LSETTLSVF.AND.NSTEP>NSTART.AND.LSETFSTAT) THEN
  ...
  DO JLEV=1,NFLEVSF
    DO IBL=1,NGPBLKS
      ISETTLOFF(JLEV)=ISETTLOFF(JLEV)+SUM(IWSETTLOFF(:,JLEV,IBL))
    ENDDO
  ENDDO
...
IF (YDMODEL%YRML_DYN%YRDYNA%LMIXETTLS.AND.NSTEP>NSTART) THEN
  IF (LRPLANE) THEN
    ...
    IF (MYPROC == 1) THEN
      DO JLEV=1,NFLEVG
        IOFF    = JLEV
        ZPD_STAT= REAL(ISETTLOFF(IOFF),JPRB)/REAL(ING,JPRB) * 100.0_JPRB
        IF (YDMODEL%YRML_DYN%YRDYNA%LMIXETTLS_PRINT.AND.(NCURRENT_ITER==0)) THEN
