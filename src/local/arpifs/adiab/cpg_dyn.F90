...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_DYN(YDGEOMETRY,YDCPG_DIM, YDCPG_OPTS,YDCPG_TND,YDCPG_MISC,YDCPG_DYN0,YDCPG_DYN9,YDVARS,YDGMV,&
 ! --- INPUT ----------------------------------------------------------
 & YDMODEL,PBETADT,PDT,&
 & PSLHDA,PSLHDD0,&
 & PGFL,&
...
!*       2.1   Semi-lagrangian dynamics (not lagged part).
  ...
  CALL LACDYN(YDGEOMETRY,YDCPG_DYN0,YDCPG_DYN9,YDGMV,&
   ! --- INPUT ----------------------------------------------------------------
   & YDMODEL,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,PBETADT,PDT,PSLHDA,PSLHDD0,&
   & YDCPG_DIM%KBL,&
...
!*       2.2   Simple physics.
  ...
  IF ((.NOT.(LMPHYS.OR.LEPHYS)).AND.LSPHLC) THEN
    IF (LTWOTL) THEN
      CALL VDIFLCZ(YDVDF,YDVAB,YDPHLC,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NPROMA,NFLEVG,PDT,YDCPG_MISC%LSM,&
...
!*       3.1   Dynamics with Eulerian advection treatment 
  ...
  CALL CPEULDYN(YDGEOMETRY,YDGMV,&
   ! --- INPUT ----------------------------------------------------
   & YDEPHY,YGFL,YDMODEL%YRML_DYN,YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,PBETADT,PDT,&
   & YDCPG_DIM%KBL,&
...
IF (LTRAJSAVE .AND. LTRAJSLAG) THEN
  ...
  ELSE
    ZTSTEP=TSTEP
  ENDIF
  PTRAJ_SLAG%PUL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1U9+1-NFLSA:MSLB1U9+NFLEVG-NFLSA)
  PTRAJ_SLAG%PVL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1V9+1-NFLSA:MSLB1V9+NFLEVG-NFLSA)
  PTRAJ_SLAG%PTL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1T9+1-NFLSA:MSLB1T9+NFLEVG-NFLSA)
  DO JGFL=1,NUMFLDS
    IF(YCOMP(JGFL)%LADV) THEN
      JJOFF=(NFLEN-NFLSA+1)*(YCOMP(JGFL)%MP_SL1-1)-NFLSA
      PTRAJ_SLAG%PGFLL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,YCOMP(JGFL)%MP_SL1) =&
         & PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1GFL9+1+JJOFF:MSLB1GFL9+NFLEVG+JJOFF)
  ...
  PTRAJ_SLAG%PCL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1C9+1-NFLSA:MSLB1C9+NFLEVG-NFLSA)/ZTSTEP
  PTRAJ_SLAG%PUL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1U0+1-NFLSA:MSLB1U0+NFLEVG-NFLSA)/ZTSTEP
  PTRAJ_SLAG%PVL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1V0+1-NFLSA:MSLB1V0+NFLEVG-NFLSA)/ZTSTEP
  PTRAJ_SLAG%PTL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1T0+1-NFLSA:MSLB1T0+NFLEVG-NFLSA)/ZTSTEP
  IF (LADVF.AND.L2TLFF) THEN
    PTRAJ_SLAG%PUT15(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDVARS%U%T1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    PTRAJ_SLAG%PVT15(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDVARS%V%T1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
  ENDIF
  PTRAJ_SLAG%PURL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1UR0+1-NFLSA:MSLB1UR0+NFLEVG-NFLSA)
  PTRAJ_SLAG%PVRL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1VR0+1-NFLSA:MSLB1VR0+NFLEVG-NFLSA)
  IF (LSLDP_CURV) THEN
    PTRAJ_SLAG%PZRL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1ZR0+1-NFLSA:MSLB1ZR0+NFLEVG-NFLSA)
  ENDIF
  PTRAJ_SLAG%PWRL05(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1WR0+1-NFLSA:MSLB1WR0+NFLEVG-NFLSA)
  IF (LSLDP_RK) THEN
    PTRAJ_SLAG%PURL005(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1UR00+1-NFLSA:MSLB1UR00+NFLEVG-NFLSA)
    PTRAJ_SLAG%PVRL005(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1VR00+1-NFLSA:MSLB1VR00+NFLEVG-NFLSA)
    IF (LSLDP_CURV) THEN
      PTRAJ_SLAG%PZRL005(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1ZR00+1-NFLSA:MSLB1ZR00+NFLEVG-NFLSA)
    ENDIF
    PTRAJ_SLAG%PWRL005(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1WR00+1-NFLSA:MSLB1WR00+NFLEVG-NFLSA)
  ENDIF
  PTRAJ_SLAG%PURL5(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2URL:MSLB2URL+NFLEVG-1)
  PTRAJ_SLAG%PVRL5(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2VRL:MSLB2VRL+NFLEVG-1)
  PTRAJ_SLAG%PWRL5(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2WRL:MSLB2WRL+NFLEVG-1)
  IF (LSLHD.AND.(.NOT.LSLHD_STATIC)) THEN
    PTRAJ_SLAG%PKAPPA5(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2KAPPA:MSLB2KAPPA+NFLEVG-1)
    IF (LSLHDHEAT) THEN
      PTRAJ_SLAG%PKAPPAT5(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2KAPPAT:MSLB2KAPPAT+NFLEVG-1)
    ENDIF
  ENDIF
  IF (LSETTLSVF) THEN
    PTRAJ_SLAG%PWRL95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDCPG_DYN9%WRL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
  ENDIF
  PTRAJ_SLAG%PX95(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PB1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB1SP9)
