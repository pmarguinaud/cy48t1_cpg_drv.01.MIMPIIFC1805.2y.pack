SUBROUTINE CPG_END(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDGMV, YDSURF, YDEPHY, YDML_GCONF, YDDYN, &
& YDML_PHY_MF, LDCONFX, LDLDIAB, PGM, PQS, PRRT9, PGFL, PGPAR, PGMV, PGMVS, PTRAJ_PHYS)
...
& )
ASSOCIATE(NDIM=>YGFL%NDIM, NPROMA=>YDDIM%NPROMA, NFLEVG=>YDDIMV%NFLEVG, NCURRENT_ITER=>YDDYN%NCURRENT_ITER,       &
& LAGPHY=>YDEPHY%LAGPHY, RSTRET=>YDGEM%RSTRET, NDIMGMV=>YDGMV%NDIMGMV, NDIMGMVS=>YDGMV%NDIMGMVS, YT0=>YDGMV%YT0,  &
...
IF (NCURRENT_ITER == 0) THEN
  IF ((.NOT.LDCONFX).AND.LDLDIAB) THEN  
    IF (LTWOTL) THEN
      CALL GPPOPER(YDDYN, 'SET0TO1', YDSURF, KBL=YDCPG_BNDS%KBL)
    ELSE
      CALL GPPOPER(YDDYN, 'PHTFILT', YDSURF, KBL=YDCPG_BNDS%KBL)
    ENDIF
  ENDIF
ENDIF
...
IF (LTWOTL.AND.LTENC) THEN
  PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT9%MSP)=PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT0%MSP)
  PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT9%MSPL)=PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT0%MSPL)
  PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT9%MSPM)=PGMVS(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,YT0%MSPM)
ENDIF
...
IF(NCURRENT_ITER == 0)THEN
  IF (LMSE.AND.NGPAR/=0) THEN
    GPARBUF (:, :, 1+(YDCPG_BNDS%KSTGLO-1)/YDGEOMETRY%YRDIM%NPROMA) = PGPAR(1:NPROMA,1:NGPAR)
