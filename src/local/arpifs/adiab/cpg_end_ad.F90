SUBROUTINE CPG_END_AD(YDGEOMETRY,YDGMV, &
 !---------------------------------------------------------------------
 ! - INPUT in TL code.
...
!**** *CPG_END_AD* - Grid point calculations: end of non lagged part, AD code.

!     Purpose.
!     --------
!           Grid point calculations: end of non lagged part, AD code.
...
!   K. Yessad: 10-Apr-2006 Move some zeroing into CPG_ZERO_AD.
!   K. Yessad: 10-Apr-2006 PGPHL,PGPHM also in hydrostatic model.
...
IF (LHOOK) CALL DR_HOOK('CPG_END_AD',0,ZHOOK_HANDLE)
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('CPG_END_AD',1,ZHOOK_HANDLE)
END SUBROUTINE CPG_END_AD
