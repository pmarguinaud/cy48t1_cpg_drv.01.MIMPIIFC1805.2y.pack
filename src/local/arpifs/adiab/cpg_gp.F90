...
IF(LLSTR) THEN
  ...
  CALL GPMPFC(YDFIELDS%YRGMV,YDMODEL%YRML_GCONF,YDDYN,NPROMA,NFLEVG,KST,KEND,IFLAG,YDGSGEOM%GM,PGMV,PGMVS,PGFL,&
            & P0U=PGMV(:,:,YT0%MU), P0V=PGMV(:,:,YT0%MV), P0DIV=PGMV(:,:,YT0%MDIV), P0TL=PGMV(:,:,YT0%MTL), &
            & P0TM=PGMV(:,:,YT0%MTM), P9U=PGMV(:,:,YT9%MU), P9V=PGMV(:,:,YT9%MV), P0UL=PGMV(:,:,YT0%MUL), &
            & P0VL=PGMV(:,:,YT0%MVL), P0VOR=PGMV(:,:,YT0%MVOR), P0SPDL=PGMV(:,:,YT0%MSPDL), P0SPDM=PGMV(:,:,YT0%MSPDM), &
            & P0SVDL=PGMV(:,:,YT0%MSVDL), P0SVDM=PGMV(:,:,YT0%MSVDM), P0NHXL=PGMV(:,:,YT0%MNHXL), P0NHXM=PGMV(:,:,YT0%MNHXM), &
            & P9DIV=PGMV(:,:,YT9%MDIV), P9TL=PGMV(:,:,YT9%MTL), P9TM=PGMV(:,:,YT9%MTM), P9SPDL=PGMV(:,:,YT9%MSPDL), &
            & P9SPDM=PGMV(:,:,YT9%MSPDM), P9SVDL=PGMV(:,:,YT9%MSVDL), P9SVDM=PGMV(:,:,YT9%MSVDM), P0SPL=PGMVS(:,YT0%MSPL), &
            & P0SPM=PGMVS(:,YT0%MSPM), P9SPL=PGMVS(:,YT9%MSPL), P9SPM=PGMVS(:,YT9%MSPM))
ENDIF
...
CALL GPINISLB(&
 ...
 & PGWFT0=YDCPG_DYN0%GWFT,PGDW0=ZGDW0,PGWS0=YDCPG_DYN0%GWHT(:,NFLEVG))
!& PGWFT0=YDCPG_DYN0%GWFT,PGDW0=YDCPG_DYN0%GDW,PGWS0=YDCPG_DYN0%GWHT(:,NFLEVG)) ! TODO : LTWOTL does not work with this line
