...
YDCPG_DYN0%OROGL(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=YDOROG%OROGL(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)*YDVARS%GEOMETRY%GM%T0(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
YDCPG_DYN0%OROGM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=YDOROG%OROGM(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)*YDVARS%GEOMETRY%GM%T0(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
IF(LNHDYN) THEN
  DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ZGM2 = YDVARS%GEOMETRY%GM%T0(JROF)**2
    Z_OROGLL_T0(JROF)=YDOROG%OROGLL(JROF)*ZGM2
    Z_OROGMM_T0(JROF)=YDOROG%OROGMM(JROF)*ZGM2
    Z_OROGLM_T0(JROF)=YDOROG%OROGLM(JROF)*ZGM2
  ENDDO
ENDIF
...
IF (LNHQE) THEN
  ...
  & LLGPXX, YDCPG_OPTS%LDIAB, LMPA, YDOROG%OROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0, Z_GDW_T0,               &
  & Z_GWHT_T0, PGFL, YDCPG_TND%ZVIEW)
ELSEIF (LNHEE) THEN
  ! * NHEE model:
...
ELSE
  ...
  & LLUVH, LLGPXX, YDCPG_OPTS%LDIAB, LMPA, YDOROG%OROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0,                 &
  & Z_GDW_T0, Z_GWHT_T0, PGFL, YDCPG_TND%ZVIEW)
ENDIF
