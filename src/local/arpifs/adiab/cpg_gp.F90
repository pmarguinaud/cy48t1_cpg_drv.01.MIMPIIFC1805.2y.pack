...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP(YDGEOMETRY,YDCPG_DIM,YDTMP,YDCPG_TND,YDCPG_MISC,YDCPG_DYN0,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,&
 & YDMODEL,YDFIELDS,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
ASSOCIATE(NUMFLDS=>YGFL%NUMFLDS, &
 ...
 & YSP_SBD=>YDFIELDS%YRSURF%YSP_SBD, &
...
IF (NSTEP > 0.AND.LTENC) THEN
  CALL ETENC(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_LBC,YDFIELDS%YRELBC_FIELDS,&
   & LD_DFISTEP,YDCPG_DIM%KFDIE,YDCPG_DIM%KSTGLO,YDVARS%SP%T0,&
...
IF (YDMODEL%YRML_DYN%YRSPNG%LNSPONGE) CALL GPNSPNG(YGFL,YDMODEL%YRML_DYN%YRSPNG,NPROMA,NFLEVG,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,PGFL)
...
IF(NCURRENT_ITER == 0) THEN
  CALL GPTF2(YDGEOMETRY,YDFIELDS%YRGMV,&
   ! --- INPUT ---------------------------------------------------------
   & YDMODEL%YRML_GCONF,YDDYN,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,LDLFSTEP,&
...
YDCPG_DYN0%OROGL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDOROG%OROGL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
YDCPG_DYN0%OROGM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDOROG%OROGM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
IF(LNHDYN) THEN
...
IF(LLSTR) THEN
  IFLAG=0
  CALL GPMPFC(YDFIELDS%YRGMV,YDMODEL%YRML_GCONF,YDDYN,NPROMA,NFLEVG,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,IFLAG,YDGSGEOM%GM,PGFL=PGFL,&
...
CALL GP_SPV(YDGEOMETRY,YDDYN,YDSIMPHL,.FALSE.,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,&
...
IF (LSFORC .AND. LSPS_FRC) THEN
  CALL CP_FORCING_PS(YDGEOMETRY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DYN0%PRE)
ENDIF
...
!*     1.6   SURFACE VARIABLES.
  ...
  IF (LDLDIAB.OR.LSPHLC.OR.LSIMPH) THEN
    YDCPG_MISC%QS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=0.0_JPRB
    IF(LDLFSTEP .AND. .NOT. LTWOTL) THEN
      CALL GPPOPER(YDDYN,'SET9TO0',YDFIELDS%YRSURF,KBL=YDCPG_DIM%KBL)
    ENDIF
  ENDIF

  ! for AROME
  IF (LMSE.AND.NGPAR/=0) THEN
    PGPAR(1:NPROMA,1:NGPAR) = GPARBUF (:, :, 1+(YDCPG_DIM%KSTGLO-1)/YDGEOMETRY%YRDIM%NPROMA)
  ENDIF
...
!*     1.8   OZON PHYSICO-CHEMICAL PROPERTIES AND SUBGRID SURFACE TEMPERATURE.
  ...
  IF (LDLDIAB) THEN
    IF(NTOZ3D > 0.OR.NTOZ2D > 0.OR.NTOZ1D > 0.AND.(.NOT.LEO3CH)) THEN
      CALL GPINOZST(YDGEOMETRY,YDMODEL%YRML_CHEM%YROZO,YDDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KSTGLO,PKOZO)
    ENDIF
  ENDIF
...
!*     1.9  NUDGING.
 ...
 IF(LNUDG.AND.(.NOT.LMSE)) THEN
    CALL UPDSST(YDMODEL%YRML_AOC%YRMCC,YDMODEL%YRML_PHY_MF%YRPHY1,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFNUDG,YSP_SBD%NLEVS,YDCPG_DIM%KBL,&
     & XPNUDG,&
...
IF (LNHQE) THEN
  ...
  CALL CPG_GP_NHQE(YDGEOMETRY,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
   & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KSTGLO,LLGPXX,LDLDIAB,LMPA,&
   & YDCPG_DIM%KBL,YDOROG%OROG,&
   !---------------------------------------------------------------------
   ! - INPUT/OUTPUT .
   & PGFL,&
   !---------------------------------------------------------------------
...
ELSEIF (LNHEE) THEN
  ...
  CALL CPG_GP_NHEE(YDGEOMETRY,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
   & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KSTGLO,LLGPXX,LDLDIAB,LMPA,&
   & YDCPG_DIM%KBL,YDOROG%OROG,&
...
ELSE
  ...
  CALL CPG_GP_HYD(YDGEOMETRY,YDCPG_DIM,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
   & LLUVH,LLGPXX,LDLDIAB,LMPA,&
   & YDOROG%OROG,&
   !---------------------------------------------------------------------
   ! - INPUT/OUTPUT .
   & PGFL,&
...
CALL GPINISLB(&
 & YDGEOMETRY,YGFL,YDDYN,YDPTRSLB2,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,PTE,.FALSE.,&
 & PGFL,&
...
IF (NCURRENT_ITER == 0) THEN
  IF (LDLDIAB.OR.LSIMPH) THEN
    CALL GPPOPER(YDDYN,'SET1TO9',YDFIELDS%YRSURF,KBL=YDCPG_DIM%KBL)
  ENDIF
ENDIF
...
IF (LSFORC) THEN
  ...
  ENDIF
  CALL CP_FORCING(YDGEOMETRY,YDMODEL,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,PDT,YDCPG_DYN0%PRE,YDCPG_DYN0%PREF,ZP1FORC,&
...
IF (LRSLDDH.AND.LSLAG) THEN
  ...
  IF (LTWOTL) THEN
    PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_U)=-YDVARS%U%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_V)=-YDVARS%V%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    IF (NFTHER > 0) PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_T)=-YDVARS%T%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    IF (LNHDYN)THEN !!! ?????? ky: LNHDYN or LNHEE there?
      PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_PD)=-YDVARS%SPD%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ENDIF
    IF (LNHDYN)THEN
      PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_VD)=-YDVARS%SVD%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
      IF (LNHX) PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_NHX)=-YDCPG_DYN0%NHX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ENDIF
    DO JGFL=1,NUMFLDS
      IF (YCOMP(JGFL)%LADV) THEN
        PGFLTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,JGFL)=-PGFL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,YCOMP(JGFL)%MP)
      ENDIF
    ENDDO
  ELSE
    PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_U)=-YDVARS%U%T9(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_V)=-YDVARS%V%T9(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    IF (NFTHER > 0) PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_T)=-YDVARS%T%T9(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    IF (LNHDYN)THEN !!! ?????? ky: LNHDYN or LNHEE there?
      PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_PD)=-YDVARS%SPD%T9(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ENDIF
    IF (LNHDYN)THEN
      PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_VD)=-YDVARS%SVD%T9(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
      IF (LNHX) PGMVTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,MSLDDH_NHX)=-YDCPG_DYN9%NHX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ENDIF
    DO JGFL=1,NUMFLDS
      IF (YCOMP(JGFL)%LADV) THEN
        PGFLTNDSL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,JGFL)=-PGFL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG,YCOMP(JGFL)%MP9)
      ENDIF
    ENDDO
  ENDIF
