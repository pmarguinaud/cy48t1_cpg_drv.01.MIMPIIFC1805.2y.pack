...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_MISC, YDCPG_GPAR,     &
& YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, &
...
IF (NSTEP > 0.AND.LTENC) THEN
  ...
  & YDCPG_OPTS%L_DFISTEP, YDCPG_BNDS%KFDIE, YDCPG_BNDS%KSTGLO, YDVARS%SP%T0, YDVARS%SP%DL, YDVARS%SP%DM, YDVARS%SP%T9, &
...
IF (LNHQE) THEN
  ...
  & YDMODEL, YDFIELDS%YRGMV, LLGPXX, YDCPG_OPTS%LDIAB, LMPA, YDOROG%OROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, PGFL, YDCPG_TND%ZVIEW)
ELSEIF (LNHEE) THEN
  ! * NHEE model:
  LLGPXX=.NOT.(LSLAG.AND.(NVDVAR == 4 .OR. NVDVAR == 5).AND.ND4SYS==2)
  CALL CPG_GP_NHEE(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDCPG_DYN9, &
  & YDMODEL, YDFIELDS%YRGMV, LLGPXX, YDCPG_OPTS%LDIAB, LMPA, YDOROG%OROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, PGFL, YDCPG_TND%ZVIEW)
ELSE
  ! * Hydrostatic model:
  ...
  & YDFIELDS%YRGMV, LLUVH, LLGPXX, YDCPG_OPTS%LDIAB, LMPA, YDOROG%OROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, PGFL, YDCPG_TND%ZVIEW)
ENDIF
...
CALL GPINISLB(  YDGEOMETRY, YGFL, YDDYN, YDPTRSLB2, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZTE, .FALSE.,      &
...
& YDCPG_MISC%QSNOW, PGWFT0=YDCPG_DYN0%GWFT, PGDW0=Z_GDW_T0, PGWS0=Z_GWHT_T0(:, NFLEVG)) 
...
IF (LSFORC) THEN
  ...
  ENDIF
  CALL CP_FORCING(YDGEOMETRY, YDMODEL, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZDT, YDCPG_DYN0%PRE, YDCPG_DYN0%PREF, &
