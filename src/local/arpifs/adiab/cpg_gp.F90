...
& YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, YDCPG_SL2, YDDDH, PP1FORC)
...
USE CPG_TYPE_MOD   , ONLY : CPG_DYN_TYPE, CPG_MISC_TYPE, &
                          & CPG_TND_TYPE, CPG_GPAR_TYPE, CPG_SL2_TYPE
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM, YDDIMV=>YDGEOMETRY%YRDIMV, YDGEM=>YDGEOMETRY%YRGEM, YDPTRSLB2=>YDMODEL%YRML_DYN%YRPTRSLB2,   &
& YDPARAR=>YDMODEL%YRML_PHY_MF%YRPARAR, YDMDDH=>YDMODEL%YRML_DIAG%YRMDDH, YDDPHY=>YDMODEL%YRML_PHY_G%YRDPHY,                    &
& YDDIMF=>YDMODEL%YRML_GCONF%YRDIMF, YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY, YDLDDH=>YDMODEL%YRML_DIAG%YRLDDH,                      &
& YGFL=>YDMODEL%YRML_GCONF%YGFL, YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY, YDARPHY=>YDMODEL%YRML_PHY_MF%YRARPHY,                        &
& YDDYN=>YDMODEL%YRML_DYN%YRDYN, YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL, YDPHLC=>YDMODEL%YRML_PHY_SLIN%YRPHLC                   &
&                    )
...
& LSPHLC=>YDPHLC%LSPHLC, LSIMPH=>YDSIMPHL%LSIMPH,                               &
...
IF (LTENC.AND..NOT.YDCPG_OPTS%LTWOTL) THEN
  CALL ABOR1('CPG_GP: ABOR1 CALLED: LTENC.AND..NOT.LTWOTL')
ENDIF
IF (YDCPG_OPTS%NSTEP > 0.AND.LTENC) THEN
...
IF(NCURRENT_ITER == 0) THEN
  IF (YDCPG_OPTS%LTWOTL) THEN
    CALL GPTF2_EXPL_2TL (YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%LFSTEP, &
                       & P0U=YDVARS%U%T0, P0V=YDVARS%V%T0, P9U=YDVARS%U%T9, P9V=YDVARS%V%T9)
  ELSE
    CALL GPTF2_EXPL_3TL_PART1 (YDGEOMETRY, YDMODEL%YRML_GCONF, YDDYN, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
   & YDCPG_OPTS%LFSTEP, YDCPG_OPTS%LNHDYN, YDCPG_OPTS%NVDVAR, &
...
IF(LLSTR) THEN
  IFLAG=0
  CALL GPMPFC_EXPL(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LNHXDER, YDCPG_OPTS%LPC_FULL, YDCPG_OPTS%LTWOTL, YDVARS, YDMODEL%YRML_GCONF, YDDYN, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,          &
  & IFLAG, YDVARS%GEOMETRY%GM%T0)
ENDIF

CALL GP_SPV(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LPC_FULL, YDCPG_OPTS%LTWOTL, YDGEOMETRY, YDDYN, YDSIMPHL, .FALSE., YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SP%T0, YDVARS%SP%DL, &
...
!*     1.6   SURFACE VARIABLES.
  ...
  IF (YDCPG_OPTS%LDIAB.OR.LSPHLC.OR.LSIMPH) THEN
    ...
    IF(YDCPG_OPTS%LFSTEP .AND. .NOT. YDCPG_OPTS%LTWOTL) THEN
...
!*     1.9  NUDGING.
 ...
 IF(YDCPG_OPTS%LNUDG.AND.(.NOT.LMSE)) THEN
    ...
    & YDCPG_OPTS%NFNUDG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, YDCPG_BNDS%KBL, YDCPG_OPTS%XPNUDG, YDMF_PHYS_SURF%GSP_RR%PT_T0, YDMF_PHYS_SURF%GSP_SB%PT_T0,             &
...
IF (YDCPG_OPTS%LNHQE) THEN
  ! * NHQE model:
  LLGPXX=.NOT.(YDCPG_OPTS%LSLAG.AND.YDCPG_OPTS%NVDVAR == 4.AND.YDCPG_OPTS%ND4SYS==2)
  CALL CPG_GP_NHQE(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDCPG_DYN9, YDMODEL, &
  & LLGPXX, YDCPG_OPTS%LDIAB, LMPA, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0,             &
  & Z_GDW_T0, Z_GWHT_T0, YDCPG_TND, PP1FORC)
ELSEIF (YDCPG_OPTS%LNHEE) THEN
  ! * NHEE model:
  LLGPXX=.NOT.(YDCPG_OPTS%LSLAG.AND.(YDCPG_OPTS%NVDVAR == 4 .OR. YDCPG_OPTS%NVDVAR == 5).AND.YDCPG_OPTS%ND4SYS==2)
...
ELSE
  ...
  LLUVH=LLGPXX.OR.YDCPG_OPTS%LSLHD
  CALL CPG_GP_HYD(YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDCPG_DYN9, YDMODEL, &
  & LLUVH, LLGPXX, YDCPG_OPTS%LDIAB, LMPA, Z_OROGLL_T0, Z_OROGMM_T0,                  &
  & Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, YDCPG_TND, PP1FORC)
ENDIF
...
CALL GPINISLB_PART1_EXPL (YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LNHX, YDCPG_OPTS%LSLAG, YDCPG_OPTS%LTWOTL, YDGEOMETRY, YGFL, YDDYN, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZTE, &
...
CALL GPINISLB_PART2_EXPL (YDCPG_OPTS%LSLAG, YDCPG_OPTS%LTWOTL, YDDYN, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZTE, YDVARS)

CALL GPINISLB_PART3_EXPL (YDCPG_OPTS%LGWADV, YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, YDPTRSLB2, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
...
IF (YDCPG_OPTS%LSFORC) THEN
  IF (NCURRENT_ITER > 0 .AND. YDCPG_OPTS%LPC_FULL) THEN
  ...
  ENDIF
  CALL CP_FORCING(YDGEOMETRY, YDMODEL, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZDT, YDCPG_DYN0%PRE, &
  & YDCPG_DYN0%PREF, PP1FORC, YDVARS%U%T0, YDVARS%V%T0, YDVARS%T%T0, YDVARS%Q%T0, YDCPG_TND%ZVIEW,         &
  & YDVARS%Q%T1, YDDDH)
ENDIF
