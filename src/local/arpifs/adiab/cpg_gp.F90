...
& YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDFIELDS, PGFL, PGMVTNDSL, PGFLTNDSL, PB2, PGFLT1,    &
& PKOZO, YDDDH)
...
IF(NCURRENT_ITER == 0) THEN
  ...
  ELSE
   ...
   & YDCPG_OPTS%LFSTEP, LNHDYN, NVDVAR, &
...
IF(LLSTR) THEN
  IFLAG=0
  CALL GPMPFC_EXPL(LNHDYN, LNHXDER, LPC_FULL, LTWOTL, YDVARS, YDMODEL%YRML_GCONF, YDDYN, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,          &
  & IFLAG, YDVARS%GEOMETRY%GM%T0, P0U=YDVARS%U%T0, P0V=YDVARS%V%T0, P0DIV=YDVARS%DIV%T0, P0TL=YDVARS%T%DL, &
...
CALL GP_SPV(LNHDYN, LPC_FULL, LTWOTL, YDGEOMETRY, YDDYN, YDSIMPHL, .FALSE., YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SP%T0, YDVARS%SP%DL, &
...
!*     1.6   SURFACE VARIABLES.
  ...
  IF (YDCPG_OPTS%LDIAB.OR.LSPHLC.OR.LSIMPH) THEN
    ...
    IF(YDCPG_OPTS%LFSTEP .AND. .NOT. LTWOTL) THEN
...
IF (LNHQE) THEN
  ...
  & Z_GDW_T0, Z_GWHT_T0, YDCPG_TND, ZP1FORC)
ELSEIF (LNHEE) THEN
  ! * NHEE model:
...
ELSE
  ...
  & Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, YDCPG_TND, ZP1FORC)
ENDIF
...
CALL GPINISLB(  LGWADV, LNHDYN, LNHX, LSLAG, LTWOTL, LVERTFE, LVFE_GW, YDGEOMETRY, YGFL, YDDYN, YDPTRSLB2, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%ZTE,    &
& .FALSE., PGFL, YDVARS%U%T9, YDVARS%V%T9, YDVARS%T%T9, YDVARS%SPD%T9, YDVARS%SVD%T9, YDCPG_DYN9%NHX,      &
& YDVARS%SP%T9, YDCPG_DYN0%CTY%VVEL (:, 1:), YDCPG_DYN0%PREF, YDVARS%U%T1, YDVARS%V%T1, YDVARS%T%T1,       &
& PGFLT1, YDVARS%SPD%T1, YDVARS%SVD%T1, YDVARS%NHX%T1, YDVARS%SP%T1, PB2, YDCPG_MISC%QICE, YDCPG_MISC%QLI, &
& YDCPG_MISC%QRAIN, YDCPG_MISC%QSNOW, PGWFT0=YDCPG_DYN0%GWFT, PGDW0=Z_GDW_T0, PGWS0=Z_GWHT_T0(:, NFLEVG),  &
& PL0=YDVARS%L%T0, PL9=YDVARS%L%T9, &
& PI0=YDVARS%I%T0, PI9=YDVARS%I%T9, &
& PR0=YDVARS%R%T0, PR9=YDVARS%R%T9, &
& PS0=YDVARS%S%T0, PS9=YDVARS%S%T9) 
...
IF (LSFORC) THEN
  ...
  & YDCPG_DYN0%PREF, ZP1FORC, YDVARS%U%T0, YDVARS%V%T0, YDVARS%T%T0, YDVARS%Q%T0, YDCPG_TND%ZVIEW,         &
  & ZATND_Q, PGFLT1(:, :, YQ%MP1), YDDDH)
ENDIF
