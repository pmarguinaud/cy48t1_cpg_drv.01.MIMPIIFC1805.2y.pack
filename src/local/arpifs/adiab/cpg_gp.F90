...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDCPG_TND,YDCPG_MISC,&
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 & YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_BNDS%KBL), YDOROG=>YDGEOMETRY%YROROG(YDCPG_BNDS%KBL), &
...
IF (NSTEP > 0.AND.LTENC) THEN
  CALL ETENC(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_LBC,YDFIELDS%YRELBC_FIELDS,&
   & LD_DFISTEP,YDCPG_BNDS%KFDIE,YDCPG_BNDS%KSTGLO,YDVARS%SP%T0,&
...
IF (YDMODEL%YRML_DYN%YRSPNG%LNSPONGE) CALL GPNSPNG(YGFL,YDMODEL%YRML_DYN%YRSPNG,NPROMA,NFLEVG,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,PGFL)
...
IF(NCURRENT_ITER == 0) THEN
  CALL GPTF2(YDGEOMETRY,YDFIELDS%YRGMV,&
   ! --- INPUT ---------------------------------------------------------
   & YDMODEL%YRML_GCONF,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LDLFSTEP,&
...
IF(LLSTR) THEN
  IFLAG=0
  CALL GPMPFC(YDFIELDS%YRGMV,YDMODEL%YRML_GCONF,YDDYN,NPROMA,NFLEVG,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,IFLAG,YDGSGEOM%GM,PGFL=PGFL,&
...
CALL GP_SPV(YDGEOMETRY,YDDYN,YDSIMPHL,.FALSE.,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
...
!*     1.8   OZON PHYSICO-CHEMICAL PROPERTIES AND SUBGRID SURFACE TEMPERATURE.
  ...
  IF (LDLDIAB) THEN
    IF(NTOZ3D > 0.OR.NTOZ2D > 0.OR.NTOZ1D > 0.AND.(.NOT.LEO3CH)) THEN
      CALL GPINOZST(YDGEOMETRY,YDMODEL%YRML_CHEM%YROZO,YDDPHY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_BNDS%KSTGLO,PKOZO)
    ENDIF
  ENDIF
...
!*     1.9  NUDGING.
 ...
 IF(LNUDG.AND.(.NOT.LMSE)) THEN
    CALL UPDSST(YDMODEL%YRML_AOC%YRMCC,YDMODEL%YRML_PHY_MF%YRPHY1,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFNUDG,YSP_SBD%NLEVS,YDCPG_BNDS%KBL,&
     & XPNUDG,&
...
IF (LNHQE) THEN
  ...
  CALL CPG_GP_NHQE(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
   & LLGPXX,LDLDIAB,LMPA,&
   & YDOROG%OROG,&
...
ELSEIF (LNHEE) THEN
  ...
  CALL CPG_GP_NHEE(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
   & LLGPXX,LDLDIAB,LMPA,&
   & YDOROG%OROG,&
...
ELSE
  ...
  CALL CPG_GP_HYD(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDFIELDS%YRGMV,&
   !---------------------------------------------------------------------
   ! - INPUT .
...
CALL GPINISLB(&
 & YDGEOMETRY,YGFL,YDDYN,YDPTRSLB2,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,PTE,.FALSE.,&
 & PGFL,&
...
IF (LSFORC) THEN
  ...
  ENDIF
  CALL CP_FORCING(YDGEOMETRY,YDMODEL,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,PDT,YDCPG_DYN0%PRE,YDCPG_DYN0%PREF,ZP1FORC,&
