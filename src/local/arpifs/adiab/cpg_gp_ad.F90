...
!**** *CPG_GP_AD* - Adjoint of grid point calculations.
!                   initial part of not lagged grid-point calculations.

!     Purpose.
!     --------
!           Adjoint of grid point calculations.
!           initial part of not lagged grid-point calculations.
!           - get data in buffers.
...
!           and not coded in CPG_GP_AD are mentioned by comments.
...
!        LDLRSTEP  : .T.: first time-step (T if NSTEP < or = 0).
...
!     Author.
!     -------
!        K. Yessad, after CPGAD (old part 4 of CPGAD)
...
#include "gpctyad.intfb.h"
#include "gpgeoad.intfb.h"
#include "gpgrgeoad.intfb.h"
#include "gpgrpad.intfb.h"
#include "gpgrxybad.intfb.h"
#include "gpinislbad.intfb.h"
#include "gpmpfc_pgfl.intfb.h"
#include "gphpread.intfb.h"
#include "gprcpad.intfb.h"
#include "gprtad.intfb.h"
#include "gp_spvad.intfb.h"
#include "gptf2ad.intfb.h"
#include "gp_tndlagadiab_uv_ad.intfb.h"

!     ------------------------------------------------------------------

IF (LHOOK) CALL DR_HOOK('CPG_GP_AD',0,ZHOOK_HANDLE)
...
LLNHDYN=.FALSE. ! AD of NH not coded

! * Set to zero some local arrays.
ZGPHL(KST:KEND,0:NFLEVG)=0.0_JPRB
ZGPHM(KST:KEND,0:NFLEVG)=0.0_JPRB
ZRT0(KST:KEND,1:NFLEVG)=0.0_JPRB
ZRPRE0F(KST:KEND,1:NFLEVG)=0.0_JPRB
ZXYBDER0(KST:KEND,1:NFLEVG,1:YYTXYBDER0%NDIM)=0.0_JPRB
Z_PHI0FL(KST:KEND,1:NFLEVG)=0.0_JPRB
Z_PHI0FM(KST:KEND,1:NFLEVG)=0.0_JPRB
...
! * Update of PGP1 for predictor: not coded in the AD code.

! * Retrieve sensitivities from the PB2 buffer:

! Deep layer equations: not coded in the AD code.
  
! * Compute perturbation of vertical velocity (AD of some direct
!   code which is in GPINISLB):
DO JLEV=1,NFLEVG
  ILVVEL=MSLB2VVEL+JLEV-1
  DO JROF=KST,KEND
    PRE0F(JROF,JLEV)=PRE0F(JROF,JLEV)&
     & +PCTY5(JROF,JLEV,YYTCTY5%M_VVEL)*PB2(JROF,ILVVEL)
    PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)=PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)&
     & +PRE5F(JROF,JLEV)*PB2(JROF,ILVVEL)
    PB2(JROF,ILVVEL)=0.0_JPRB
  ENDDO
ENDDO

ZDUM(:,:) = 0.0_JPRB
CALL GPINISLBAD(&
 & YDGEOMETRY,YGFL,KST,KEND,PTE,&
 & PGFL,&
 & ZT9U,ZT9V,ZT9T,&
 & ZT9SPD,ZT9SVD,&
 & ZT9SP,&
...
!*       3.2.8 COMPUTES THE GEOPOTENTIAL HEIGHT "gz" AND ITS GRADIENT.
  ...
  IF (LSIMPH) THEN
    CALL GPGEOAD(NPROMA,KST,KEND,NFLEVG,PHI9,PHIF9,&
...
!*       3.2.3 COMPUTE "R", "Cp" AND "Kap=R/Cp".
  ...
  IF(LSIMPH) THEN
    CALL GPRCPAD(NPROMA,KST,KEND,NFLEVG,PQ=ZPQ9,PCP=PRCP9(1,1,YYTRCP9%M_CP),PR=PRCP9(1,1,YYTRCP9%M_R))
  ENDIF

!*       3.2.1 COMPUTE PRE95, PXYB95, PRE95F.

  ! * Case of finite elements: not done in the AD code (ZRPRE9F is useless).

  IF (LSIMPH) THEN
    CALL GPHPREAD(NPROMA,NFLEVG,KST,KEND,YDVAB,PRE9,PRE95,PXYB=PXYB9,PXYB5=PXYB95,PRESF=PRE9F)
  ENDIF
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PRCP0(JROF,JLEV,YYTRCP0%M_KAP)=PRCP0(JROF,JLEV,YYTRCP0%M_KAP)&
     & +PGMV5(JROF,JLEV,YT5%MT)*PCTY5(JROF,JLEV,YYTCTY5%M_VVEL)*PATND(JROF,JLEV,YYTTND%M_TNDT)
    ZT0T(JROF,JLEV)=ZT0T(JROF,JLEV)&
     & +PRCP5(JROF,JLEV,YYTRCP5%M_KAP)*PCTY5(JROF,JLEV,YYTCTY5%M_VVEL)*PATND(JROF,JLEV,YYTTND%M_TNDT)
    PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)=PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)&
     & +PRCP5(JROF,JLEV,YYTRCP5%M_KAP)*PGMV5(JROF,JLEV,YT5%MT)*PATND(JROF,JLEV,YYTTND%M_TNDT)
    PATND(JROF,JLEV,YYTTND%M_TNDT)=0._JPRB
  ENDDO
ENDDO
...
ZPSGRTL(:,:)=0.0_JPRB
ZPSGRTM(:,:)=0.0_JPRB
CALL GP_TNDLAGADIAB_UV_AD(YDGEOMETRY,YDGMV,YDEPHY,YDDYN,KST,KEND,YDGSGEOM%RCORI,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
...
IF( YDML_DYN%YRDYNA%LGRADSP ) THEN
  ...
  ZPSGRTL(KST:KEND,1:NFLEVG) = ZT0SGRTL(KST:KEND,1:NFLEVG)
  ZT0SGRTL(KST:KEND,1:NFLEVG)=0.0_JPRB
  ZPSGRTM(KST:KEND,1:NFLEVG) = ZT0SGRTM(KST:KEND,1:NFLEVG)
  ZT0SGRTM(KST:KEND,1:NFLEVG)=0.0_JPRB

  ! adjust field with filter result
  ZPSGRTL(KST:KEND,1:NFLEVG) = ZPSGRTL(KST:KEND,1:NFLEVG) + ZSGRTL(KST:KEND,1:NFLEVG)
  ZT9SGRTL(KST:KEND,1:NFLEVG) = ZT9SGRTL(KST:KEND,1:NFLEVG) - ZSGRTL(KST:KEND,1:NFLEVG)
  ZT0SGRTL(KST:KEND,1:NFLEVG) = ZT0SGRTL(KST:KEND,1:NFLEVG) + ZSGRTL(KST:KEND,1:NFLEVG)

  ZPSGRTM(KST:KEND,1:NFLEVG) = ZPSGRTM(KST:KEND,1:NFLEVG) + ZSGRTM(KST:KEND,1:NFLEVG)
  ZT9SGRTM(KST:KEND,1:NFLEVG) = ZT9SGRTM(KST:KEND,1:NFLEVG) - ZSGRTM(KST:KEND,1:NFLEVG)
  ZT0SGRTM(KST:KEND,1:NFLEVG) = ZT0SGRTM(KST:KEND,1:NFLEVG) + ZSGRTM(KST:KEND,1:NFLEVG)
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    ZT0U(JROF,JLEV)=ZT0U(JROF,JLEV)+&
     & PGMV5(JROF,JLEV,YT5%MU)*PKENE0(JROF,JLEV)
    ZT0V(JROF,JLEV)=ZT0V(JROF,JLEV)+&
     & PGMV5(JROF,JLEV,YT5%MV)*PKENE0(JROF,JLEV)
    PKENE0(JROF,JLEV)=0.0_JPRB
  ENDDO
ENDDO
...
CALL GPGRGEOAD(YDGEOMETRY%YRVERT_GEOM,LLNHDYN,&
...
! * "gz" at full levels and half levels.
CALL GPGEOAD(NPROMA,KST,KEND,NFLEVG,&
 & PHI0,PHIF0,ZT0T,PRCP0(1,1,YYTRCP0%M_R),PXYB0(1,1,YYTXYB0%M_LNPR),PXYB0(1,1,YYTXYB0%M_ALPH),&
 & PGMV5(1,1,YT5%MT),PRCP5(1,1,YYTRCP5%M_R),PXYB5(1,1,YYTXYB5%M_LNPR),PXYB5(1,1,YYTXYB5%M_ALPH),YDGEOMETRY%YRVERT_GEOM)  
...
CALL GPCTYAD(NPROMA,KST,KEND,NFLEVG,YDGEOMETRY%YRVERT_GEOM,&
 & ZT0U,ZT0V,ZT0DIV,&
...
CALL GPRTAD(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
...
CALL GPRCPAD(NPROMA,KST,KEND,NFLEVG,&
...
CALL GPGRXYBAD(NPROMA,KST,KEND,NFLEVG,YDVAB,PRE0L,PRE0M,PXYB0,ZXYBDER0,&
...
IF(LVERTFE) THEN
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
      PRE0F(JROF,JLEV)=PRE0F(JROF,JLEV)&
       & -ZRPRE0F(JROF,JLEV)*PRPREF5(JROF,JLEV)**2
    ENDDO
  ENDDO
ENDIF
...
CALL GPHPREAD(NPROMA,NFLEVG,KST,KEND,YDVAB,PRE0,PRE5,PXYB=PXYB0,PXYB5=PXYB5,PRESF=PRE0F)
...
! Not coded in the AD code (CALL UPDSSTAD).

!*       1.8  OZONE PHYSICO-CHEMICAL PROPERTIES AND SUBGRID SURFACE TEMPERATURE

! Not coded in the AD code (CALL GPINOZSTDMAD).

!*       1.7  SEMI-LAGRANGIAN MASS FIX.

! Not coded in the AD code.
...
CALL GP_SPVAD(YDGEOMETRY,YDDYN,YDSIMPHL,KST,KEND,&
...
IF(.NOT.LTWOTL .AND. NCURRENT_ITER == 0) THEN
  CALL GPTF2AD(YDGEOMETRY,YDGMV,&
   ! --- INPUT ---------------------------------------------------------
   & YDML_GCONF,YDDYN,KST,KEND,LDLRSTEP,&
...
! Not coded in the AD code (CALL GPTENCAD).

IF (LLNHDYN)  PGPPC(:,:)=0.0_JPRB

!     ------------------------------------------------------------------

END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('CPG_GP_AD',1,ZHOOK_HANDLE)
