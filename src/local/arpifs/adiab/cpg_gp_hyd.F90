...
IF(YRSPEC%LGP) THEN
  CALL GPRT(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   ...
   & ZPQL,ZPQM,&
   & ZRT0,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,PRL=ZPRSPECL,PRM=ZPRSPECM)
ELSE
  CALL GPRT(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   & YDCPG_DYN0%RCP%R,ZT0T,ZT0TL,ZT0TM,&
   & ZPQL,ZPQM,&
...
IF ( LSFORC .AND. (LSW_FRC.OR.LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDGEOMETRY,YDMODEL%YRML_GCONF,YDMODEL%YRML_PHY_MF%YRPHY2,KST,KEND, &
   & ZP1FORC,YDCPG_DYN0%PREF,ZT0T,YDCPG_DYN0%RCP%R,YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%VVEL(:,1:))
ENDIF
...
CALL GPGEO(NPROMA,KST,KEND,NFLEVG,YDCPG_DYN0%PHI,YDCPG_DYN0%PHIF,ZT0T,&
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    YDCPG_DYN0%KENE(JROF,JLEV)=0.5_JPRB*(ZT0U(JROF,JLEV)**2+ZT0V(JROF,JLEV)**2)
  ENDDO
ENDDO
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PATND(JROF,JLEV,YYTTND%M_TNDT)=&
     & YDCPG_DYN0%RCP%KAP(JROF,JLEV)*ZT0T(JROF,JLEV)*YDCPG_DYN0%CTY%VVEL(JROF,JLEV)
  ENDDO
ENDDO
...
!*     3.2.8 COMPUTES THE GEOPOTENTIAL HEIGHT "gz" AND ITS GRADIENT.
  ...
  IF(LDLDIAB.OR.LSIMPH) THEN
    ...
    CALL GPGEO(NPROMA,KST,KEND,NFLEVG,YDCPG_DYN9%PHI,YDCPG_DYN9%PHIF,ZT9T,&
