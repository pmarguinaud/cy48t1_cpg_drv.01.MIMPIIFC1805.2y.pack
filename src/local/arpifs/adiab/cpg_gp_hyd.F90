...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP_HYD(YDGEOMETRY,YDCPG_DIM,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDGMV,&
 !---------------------------------------------------------------------
 ! - INPUT .
 & LDUVH,LDGW,LDLDIAB,LDMPA,&
 & POROG,&
 !---------------------------------------------------------------------
 ! - INPUT/OUTPUT .
 & PGFL,&
...
IF (LDGW) THEN
  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,LLDER,YDVARS%U%T0,YDVARS%V%T0,YDTMP%T0%US,YDTMP%T0%VS)
ENDIF

!*     3.1.12 COMPUTES THE PRESSURE FORCE FOR THE RHS OF MOMENTUM EQN.

CALL GPGRP(YDGEOMETRY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,&
...
!*     3.2.10 COMPUTES HALF-LEVEL WINDS.
  ...
  IF (LDGW) THEN
    ...
    CALL GPHLUV(YDGEOMETRY%YRDIMV,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDVARS%U%T9,YDVARS%V%T9,YDTMP%T9%UVH%ZVIEW)
