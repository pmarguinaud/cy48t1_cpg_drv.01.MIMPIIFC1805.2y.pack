...
IF (L_RDRY_VD) THEN
  YDTMP%T0%RDT(KST:KEND,1:NFLEVG)=RD*YDVARS%T%T0(KST:KEND,1:NFLEVG)
ELSE
  YDTMP%T0%RDT(KST:KEND,1:NFLEVG)=YDTMP%T0%RT(KST:KEND,1:NFLEVG)
ENDIF
...
CALL GPCTY(YDVFE,NPROMA,KST,KEND,NFLEVG,LRUBC,YDVAB,YDVETA,&
 ...
 & PSPL=YDCPG_DYN0%PREL,PSPM=YDCPG_DYN0%PREM,PRPREF=YDTMP%T0%RPREF,PDPHYCTY=YDTMP%T0%DPHYCTY,&
 & PDELP=YDCPG_DYN0%XYB%DELP, PLNPR=YDCPG_DYN0%XYB%LNPR, PRDELP=YDCPG_DYN0%XYB%RDELP, &
 & PALPH=YDCPG_DYN0%XYB%ALPH, PRTGR=YDCPG_DYN0%XYB%RTGR, PRPRE=YDCPG_DYN0%XYB%RPRE,  &
...
IF (LDGW) THEN
  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,KST,KEND,LLDER,YDVARS%U%T0,YDVARS%V%T0,YDTMP%T0%US,YDTMP%T0%VS)
ENDIF

!*     3.1.12 COMPUTES THE PRESSURE FORCE FOR THE RHS OF MOMENTUM EQN.

CALL GPGRP(YDGEOMETRY,KST,KEND,&
 & YDTMP%T0%RT,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%XYB%ALPH,YDTMP%T0%XYBDER%ZVIEW,&
...
! Caution: must use consistent definitions of RT in ZRDT0 and ZDVER0.
  ...
  CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LLGWF0,LLGDWI,&
   ...
   & YDTMP%T0%RDT,YDTMP%T0%DVER,YDTMP%T0%GWHT,YDCPG_DYN0%GWFT)
ENDIF
...
!*     3.2.4 COMPUTE "RT".
  ...
  IF (L_RDRY_VD) THEN
    YDTMP%T9%RDT(KST:KEND,1:NFLEVG)=RD*YDVARS%T%T9(KST:KEND,1:NFLEVG)
  ELSE
...
!*     3.2.16 COMPUTES term [gw].
  ...
  IF (LDGW) THEN
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LDMPA,LLGDWI,&
     ...
     & YDTMP%T9%RDT,YDTMP%T9%DVER,YDTMP%T9%GWHT,YDCPG_DYN9%GWFT)
  ENDIF
