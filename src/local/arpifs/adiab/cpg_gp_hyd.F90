...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP_HYD(YDGEOMETRY,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDGMV,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
ASSOCIATE(NPROMA=>YDDIM%NPROMA,NFLEVG=>YDDIMV%NFLEVG, &
 & NDIM=>YGFL%NDIM, YQ=>YGFL%YQ,YRSPEC=>YGFL%YRSPEC, YFORC=>YGFL%YFORC, YPHYCTY => YGFL%YPHYCTY, &
...
ZPGFLYPHYCTY=> YDVARS%PHYCTY%T0
ZPQL        => YDVARS%Q%DL
ZPQM        => YDVARS%Q%DM
ZPRSPECL    => YDVARS%RSPEC%DL
ZPRSPECM    => YDVARS%RSPEC%DM
ZT0DIV      => YDVARS%DIV%T0
ZT0EDOT     => YDVARS%EDOT%T0
ZT0SGRTL    => YDVARS%SGRTL%T0
ZT0SGRTM    => YDVARS%SGRTM%T0
ZT0T        => YDVARS%T%T0
ZT0TL       => YDVARS%T%DL
ZT0TM       => YDVARS%T%DM
ZT0U        => YDVARS%U%T0
...
ZT9SGRTL    => YDVARS%SGRTL%T9
ZT9SGRTM    => YDVARS%SGRTM%T9
ZT9T        => YDVARS%T%T9
ZT9U        => YDVARS%U%T9
ZT9V        => YDVARS%V%T9
...
IF (YGFL%YPHYCTY%LACTIVE) THEN
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
      ZDPHYCTY(JROF,JLEV)=YDVARS%PHYCTY%T0(JROF,JLEV)
    ENDDO
  ENDDO
ELSE
  ZDPHYCTY(:,:)=0.0_JPRB
...
CALL GPCTY(YDVFE,NPROMA,KST,KEND,NFLEVG,LRUBC,YDVAB,YDVETA,&
 ...
 & PSPL=YDCPG_DYN0%PREL,PSPM=YDCPG_DYN0%PREM,PRPREF=ZRPRE0F,PDPHYCTY=ZDPHYCTY,&
 & PDELP=YDCPG_DYN0%XYB%DELP, PLNPR=YDCPG_DYN0%XYB%LNPR, PRDELP=YDCPG_DYN0%XYB%RDELP, &
 & PALPH=YDCPG_DYN0%XYB%ALPH, PRTGR=YDCPG_DYN0%XYB%RTGR, PRPRE=YDCPG_DYN0%XYB%RPRE,  &
...
IF (LDGW) THEN
  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,KST,KEND,LLDER,YDVARS%U%T0,YDVARS%V%T0,ZUS0,ZVS0)
ENDIF
...
!*     3.1.14 COMPUTES dver.
  ...
  IF (L_RDRY_VD) THEN
    ...
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        ZDVER0(JROF,JLEV)=-(YDCPG_DYN0%RCP%R(JROF,JLEV)/RD)*(YDVARS%DIV%T0(JROF,JLEV)&
         & +ZNHXT0(JROF,JLEV)+(1.0_JPRB-YDCPG_DYN0%RCP%KAP(JROF,JLEV))*YDCPG_DYN0%CTY%VVEL(JROF,JLEV))
      ENDDO
    ENDDO
  ELSE
    ...
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        ZDVER0(JROF,JLEV)=-(YDVARS%DIV%T0(JROF,JLEV)&
         & +ZNHXT0(JROF,JLEV)+(1.0_JPRB-YDCPG_DYN0%RCP%KAP(JROF,JLEV))*YDCPG_DYN0%CTY%VVEL(JROF,JLEV))
      ENDDO
    ENDDO
  ENDIF
...
! Caution: must use consistent definitions of RT in ZRDT0 and ZDVER0.
  ...
  CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LLGWF0,LLGDWI,&
   ...
   & ZRDT0,ZDVER0,ZGWHT0,YDCPG_DYN0%GWFT)
ENDIF
...
!*     3.2.14 COMPUTES dver.
    ...
    IF (L_RDRY_VD) THEN
      ...
      DO JLEV=1,NFLEVG
        DO JROF=KST,KEND
          ZDVER9(JROF,JLEV)=-(YDCPG_DYN0%RCP%R(JROF,JLEV)/RD)*(YDVARS%DIV%T9(JROF,JLEV)&
           & +ZNHXT9(JROF,JLEV)+(1.0_JPRB-YDCPG_DYN0%RCP%KAP(JROF,JLEV))*YDCPG_DYN0%CTY%VVEL(JROF,JLEV))
        ENDDO
      ENDDO
    ELSE
      ! dver_hyd=-(D+X+(cv/cp)(omega/prehyd))
      ...
      DO JLEV=1,NFLEVG
        DO JROF=KST,KEND
          ZDVER9(JROF,JLEV)=-(YDVARS%DIV%T9(JROF,JLEV)&
           & +ZNHXT9(JROF,JLEV)+(1.0_JPRB-YDCPG_DYN0%RCP%KAP(JROF,JLEV))*YDCPG_DYN0%CTY%VVEL(JROF,JLEV))
        ENDDO
      ENDDO
    ENDIF
...
!*     3.2.16 COMPUTES term [gw].
  ...
  IF (LDGW) THEN
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LDMPA,LLGDWI,&
     ...
     & ZRDT9,ZDVER9,ZGWHT9,YDCPG_DYN9%GWFT)
  ENDIF
