...
& YDGMV, LDGPXX, LDLDIAB, LDMPA, POROG, Z_OROGLL_T0, Z_OROGMM_T0, Z_OROGLM_T0, Z_GDW_T0, Z_GWHT_T0, &
& PGFL, PATND)
...
IF (LNHEE_REFINE_GRP) THEN
  ...
  CALL GNHEE_REFINE_GRP(YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, Z_RT_T0, YDCPG_DYN0%RTL, YDCPG_DYN0%RTM, &
...
LLDER=.TRUE.
IF (NVDVAR==3 .OR. NVDVAR==4) THEN
  CALL GNHGRGW(YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, LLDER, Z_RDT_T0,                            &
  ...
  & POROGM=YDCPG_DYN0%OROGM, POROGLM=Z_OROGLM_T0, POROGLL=Z_OROGLL_T0, POROGMM=Z_OROGMM_T0, PGWFL=YDCPG_DYN0%GWFL,         &
...
CALL GNHD3(NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%DIV%T0, Z_NHXS_T0, Z_DVER_T0, YDCPG_DYN0%RCP%R, &
...
& YDCPG_BNDS%KFDIA, YDGSGEOM%RCORI, YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, Z_PSGRTL_T0, Z_PSGRTM_T0, YDVARS%U%T0,    &
& YDVARS%V%T0, PATND(1, 1, YYTTND%M_TNDU), PATND(1, 1, YYTTND%M_TNDV), PATND(1, 1, YYTTND%M_TNDU_NOC),          &
...
IF (LNHEE_REFINE_PREH_BBC) THEN
  ...
  & Z_PDEP_T0, Z_WH2F_T0, Z_PSGRTL_T0, Z_PSGRTM_T0, Z_SGRTSL_T0, Z_SGRTSM_T0, PATND(1, 1, YYTTND%M_TNDU),    &
  & PATND(1, 1, YYTTND%M_TNDV), PATND(1, 1, YYTTND%M_TNDU_NOC), PATND(1, 1, YYTTND%M_TNDV_NOC),              &
...
IF (LNHEE_SVDLAPL_FIRST) THEN
  ...
  IF (LGWADV) THEN
    ...
    & Z_OROGLM_T0, Z_OROGLL_T0, Z_OROGMM_T0, Z_US_T0, Z_VS_T0, Z_TNDUS_T0, Z_TNDVS_T0, YDCPG_DYN0%XYB%LNPR,       &
    & YDCPG_DYN0%XYB%ALPH, YDCPG_DYN0%PREF, YDCPG_DYN0%PRE, Z_PDEP_T0, PATND(1, 1, YYTTND%M_TNDGW)                &
    &   )
  ENDIF
ELSE
  ...
  ELSE IF (LVFE_GW) THEN
    ...
    & POROGM=YDCPG_DYN0%OROGM, POROGLM=Z_OROGLM_T0, POROGLL=Z_OROGLL_T0, POROGMM=Z_OROGMM_T0, PUS=Z_US_T0,           &
...
IF (.NOT.LGWADV) THEN
  ...
  IF (LNHEE_SVDLAPL_FIRST) THEN
    ...
    & YDCPG_DYN0%UVH%VH, PATND(1, 1, YYTTND%M_TNDU), PATND(1, 1, YYTTND%M_TNDV), Z_TNDUS_T0,                        &
...
!*     3.2.13a COMPUTES TERM "NHX".
  ...
  IF (LDGPXX) THEN
    ...
    & Z_UVH_VH_T9, Z_NHXS_T0, PNHPPI=Z_NHPPI_T0)
    IF (NVDVAR == 3 .OR. NVDVAR == 4) THEN
      ! NHX = NHX_S
...
!*     3.2.16 COMPUTES term [gw] or [gW].
  ...
  ELSEIF (NVDVAR==5) THEN
    ...
    & Z_ZERO_T0, Z_ZERO_T0, YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH, Z_ZERO_T0, Z_ZERO_T0, Z_RDT_T9, &
