...
CALL GPHPRE_EXPL(YDCPG_OPTS%LAPRXPK, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%NDLNPR, YDCPG_OPTS%RHYDR0, YDCPG_OPTS%TOPPRES, &
...
IF(YDCPG_OPTS%LVERTFE) THEN
  ...
  ENDDO
ENDIF
CALL GPGRXYB_EXPL(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%NDLNPR, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                                  &
...
CALL GNHPRE(YDCPG_OPTS%NPDVAR, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T0, &
...
ENDDO
CALL GNHPREH(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDCPG_OPTS%NPDVAR, YDGEOMETRY, NPROMA,                         &
...
CALL GNHGRPRE(YDCPG_OPTS%NPDVAR, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%XYB%RTGR, &
...
CALL GPCTY_EXPL(YDCPG_OPTS%LVERTFE, YDVFE, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, YDCPG_OPTS%LRUBC,      &
...
& YDCPG_DYN0%UVH%WWI (1:, 1:), LDVERINT=YDCPG_OPTS%LVEREGINT)
...
IF (YDCPG_OPTS%LNHEE_REFINE_GRP) THEN
  CALL GPFLWI(YDGEOMETRY%YRDIMV, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH, &
  & YDCPG_DYN0%PRE, YDCPG_DYN0%PREF, YDCPG_DYN0%XYB%DELP, Z_WH2F_T0, LDVERINT=YDCPG_OPTS%LVEREGINT)
...
IF (YDCPG_OPTS%NVDVAR == 5 .AND. YDCPG_OPTS%LGWADV) THEN
...
ELSEIF (YDCPG_OPTS%NVDVAR == 4) THEN
...
IF (YDCPG_OPTS%NVDVAR==3 .OR. YDCPG_OPTS%NVDVAR==4) THEN
  LLGWF0=.TRUE.
  LLGDWI=.FALSE.
  IF (YDCPG_OPTS%LGWADV.AND.YDCPG_OPTS%LVFE_GW) THEN
  ...
  ELSE
    ...
    IF (YDCPG_OPTS%LGWADV) YDCPG_DYN0%GWT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=P_GWHT_T0(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,0:NFLEVG-1)
  ENDIF

  IF (YDCPG_OPTS%LTWOTL.AND.YDCPG_OPTS%LRDBBC) THEN
...
CALL GNHD3(YDCPG_OPTS%L_RDRY_VD, YDMODEL%YRCST, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%DIV%T0, &
& Z_NHXS_T0, Z_DVER_T0, YDCPG_DYN0%RCP%R, RMAX_D3, LBOUND_D3, Z_Z3DIVG_T0)

!*     3.1.19 COMPUTES pre/(Rd T prehyd [Delta log(prehyd)]) at t.

IF (.NOT.YDCPG_OPTS%LGWADV) THEN
  IF (YDCPG_OPTS%L_RDRY_VD) THEN
...
IF (.NOT.YDCPG_OPTS%LGWADV.OR.YDCPG_OPTS%LNHEE_SVDLAPL_FIRST.OR.YDCPG_OPTS%LNHEE_REFINE_PREH_BBC) THEN
...
IF (YDCPG_OPTS%LNHEE_REFINE_PREH_BBC) THEN
...
IF (YDCPG_OPTS%LNHEE_SVDLAPL_FIRST) THEN
  ...
  IF (YDCPG_OPTS%LGWADV) THEN
    ...
    CALL GNHEE_TNDLAGADIAB_GW(YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%NVDVAR, YDMODEL%YRCST, &
...
ELSE
  ...
  IF (YDCPG_OPTS%LGWADV.AND.YDCPG_OPTS%LVFE_GW) THEN
    CALL GNH_TNDLAGADIAB_GW(YDCPG_OPTS%LGWADV, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDCPG_OPTS%LVFE_GW,     &
...
CALL GNH_TNDLAGADIAB_SPD(YDCPG_OPTS%NPDVAR, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
...
IF (.NOT.YDCPG_OPTS%LGWADV) THEN
  ...
  IF (YDCPG_OPTS%LNHEE_SVDLAPL_FIRST) THEN
    ...
    CALL GNHEE_TNDLAGADIAB_SVD(YDCPG_OPTS%LVEREGINT, YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, &
    & YDCPG_OPTS%NVDVAR, YDMODEL%YRCST, YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL,             &
  ...
  ELSE
    ...
    !   Dataflow may be tricky, and not always convenient to test future variable changes.
    CALL GNH_TNDLAGADIAB_SVD(YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%NVDVAR, YDMODEL%YRCST,        &
...
CALL CPG_GP_NHEE_T9 (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDMODEL, LDGPXX, YDCPG_OPTS%LGWADV, &
& YDCPG_OPTS%LRDBBC, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%L_RDRY_VD, YDCPG_OPTS%NVDVAR,            &
