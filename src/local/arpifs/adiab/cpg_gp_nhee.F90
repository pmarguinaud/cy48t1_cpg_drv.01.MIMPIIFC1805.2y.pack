...
IF (LNHEE_REFINE_PREH_BBC) THEN
  ...
  & Z_PDEP_T0, Z_WH2F_T0, Z_PSGRTL_T0, Z_PSGRTM_T0, Z_SGRTSL_T0, Z_SGRTSM_T0, PATND(:, :, YYTTND%M_TNDU),    &
  & PATND(:, :, YYTTND%M_TNDV), PATND(:, :, YYTTND%M_TNDU_NOC), PATND(:, :, YYTTND%M_TNDV_NOC),              &
...
IF (LNHEE_SVDLAPL_FIRST) THEN
  ...
  IF (LGWADV) THEN
    ...
    & YDCPG_DYN0%XYB%ALPH, YDCPG_DYN0%PREF, YDCPG_DYN0%PRE, Z_PDEP_T0, PATND(:, :, YYTTND%M_TNDGW)                &
    &                     )
  ENDIF
ELSE
...
& Z_Z3DIVG_T0, YDCPG_DYN0%CTY%VVEL(:, 1:), PATND(:, :, YYTTND%M_TNDPD) )
...
IF (.NOT.LGWADV) THEN
  ...
  IF (LNHEE_SVDLAPL_FIRST) THEN
    ...
    & YDCPG_DYN0%UVH%VH, PATND(:, :, YYTTND%M_TNDU), PATND(:, :, YYTTND%M_TNDV), Z_TNDUS_T0,                        &
    & Z_TNDVS_T0, YDCPG_DYN0%RDPHI, Z_GWHL_T0, Z_GWHM_T0, YDCPG_DYN0%GWFL, YDCPG_DYN0%GWFM,                         &
    & Z_DVER_T0, Z_DVERW_T0, YDVARS%DIV%T0, Z_Z3DIVG_T0, PATND(:, :, YYTTND%M_TNDVD), YDCPG_DYN0%DBBC               &
    &                            )
  ELSE
    ! * rhs of (gw) eqn computed first, then Laplacian term in "dver" eqn computed.
    ...
    & YDCPG_DYN0%XYB%DELP, YDCPG_DYN0%XYB%LNPR, PATND(:, :, YYTTND%M_TNDVD) )
  ENDIF
ENDIF
