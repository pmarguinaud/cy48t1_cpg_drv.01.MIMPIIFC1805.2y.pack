...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP_NHEE(YDGEOMETRY,YDCPG_BNDS, YDCPG_OPTS,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDGMV,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
CALL GPGRXYB(NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,.FALSE.,YDVAB,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,&
           ...
           & PALPH=YDCPG_DYN0%XYB%ALPH, PRTGR=YDCPG_DYN0%XYB%RTGR, PRPRE=YDCPG_DYN0%XYB%RPRE,  &
           & PRPP=YDCPG_DYN0%XYB%RPP, &
           & PCOEFD_DER=YDTMP%T0%XYBDER%COEFD, PLNPRL_DER=YDTMP%T0%XYBDER%LNPRL, PLNPRM_DER=YDTMP%T0%XYBDER%LNPRM, &
           & PCOEFA_DER=YDTMP%T0%XYBDER%COEFA, PCOEFAPL_DER=YDTMP%T0%XYBDER%COEFAPL, PALPHPLL_DER=YDTMP%T0%XYBDER%ALPHPLL, &
           & PALPHPLM_DER=YDTMP%T0%XYBDER%ALPHPLM, PALPHL_DER=YDTMP%T0%XYBDER%ALPHL, PALPHM_DER=YDTMP%T0%XYBDER%ALPHM)

!*     3.1.3 COMPUTE "R", "Cp" AND "kap=R/Cp".

CALL GPRCP(NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,PGFL=PGFL,PCP=YDCPG_DYN0%RCP%CP,&
 & PR=YDCPG_DYN0%RCP%R,PKAP=YDCPG_DYN0%RCP%KAP)

!*     3.1.4 COMPUTES "RT" AND ITS GRADIENT.

IF(YRSPEC%LGP) THEN
  CALL GPRT(LSPRT,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,RD,RV,&
...
CALL GNHGRPRE(YDGEOMETRY,NPROMA,NFLEVG,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,&
...
CALL GPCTY(YDVFE,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,LRUBC,YDVAB,YDVETA,&
...
IF(LGWDIAGS_ON) THEN
  CALL UPDATE_GWDIAG(YDGEOMETRY,YDMODEL%YRML_GCONF%YRRIP,NPROMA,NFLEVG,YDCPG_BNDS%KFDIA,NSTEP,YDCPG_BNDS%KSTGLO,YDCPG_DYN0%PRE,YDCPG_DYN0%CTY%DIVDP(:,1:))
ENDIF
...
CALL GPGRGEO(YDGEOMETRY,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,NFLEVG,&
 & YDTMP%T0%RT,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,&
...
CALL GPHLWI(YDGEOMETRY%YRDIMV,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_DYN0%XYB%LNPR,&
...
LLDER=.TRUE.
CALL GPUVS(NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LLDER,&
...
IF (LNHEE_REFINE_GRP) THEN
  CALL GPFLWI(YDGEOMETRY%YRDIMV,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA, &
  ...
  CALL GNHEE_REFINE_GRP(YDGEOMETRY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
   & YDTMP%T0%RT,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,&
   & YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%XYB%ALPH,&
   & PRNHPPI=YDTMP%T0%RNHPPI,PQCHAL=YDCPG_DYN0%QCHAL,PQCHAM=YDCPG_DYN0%QCHAM,PHIHL=YDTMP%T0%GPHL,PHIHM=YDTMP%T0%GPHM, &
   & PDEP=YDTMP%T0%PDEP,PREF=YDCPG_DYN0%PREF,PREH=YDCPG_DYN0%PRE,PWWH2F=YDTMP%T0%WH2F,&
   & PSGRTL=YDTMP%T0%PSGRTL,PSGRTM=YDTMP%T0%PSGRTM, &
   & PALPHPLL_DER=YDTMP%T0%XYBDER%ALPHPLL, PALPHPLM_DER=YDTMP%T0%XYBDER%ALPHPLM)
ELSE
  CALL GNHEE_GRP(YDGEOMETRY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
   & YDTMP%T0%RT,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,&
   & YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%XYB%ALPH,&
   & PDELNHPRE=YDTMP%T0%DELNHPRE,PLNNHPREFL=YDTMP%T0%LNNHPREFL,PLNNHPREFM=YDTMP%T0%LNNHPREFM,&
   & PRNHPPI=YDTMP%T0%RNHPPI,PQCHAL=YDCPG_DYN0%QCHAL,PQCHAM=YDCPG_DYN0%QCHAM,&
...
IF (LDGPXX) THEN
  CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDTMP%T0%GPHL,YDTMP%T0%GPHM,YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,&
  ...
  ELSEIF (NVDVAR == 5) THEN
    ! NHX = NHX_S - (d13 - dver)
    CALL GNHEE_SVDINCR13(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,&
...
IF (NVDVAR==3 .OR. NVDVAR==4) THEN
  ...
  IF (LGWADV.AND.LVFE_GW) THEN
    ...
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LLGWF0,LLGDWI,&
...
LLDER=.TRUE.
IF (NVDVAR==3 .OR. NVDVAR==4) THEN
  CALL GNHGRGW(YDGEOMETRY,NPROMA,NFLEVG,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LLDER,&
...
CALL GNHD3(NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
...
CALL GP_TNDLAGADIAB_UV(YDGEOMETRY,YDGMV,YDMODEL%YRML_PHY_EC%YREPHY,YDMODEL%YRML_DYN%YRDYN,&
 & YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDGSGEOM%RCORI,&
...
IF (.NOT.LGWADV.OR.LNHEE_SVDLAPL_FIRST.OR.LNHEE_REFINE_PREH_BBC) THEN
  CALL GNH_TNDLAGADIAB_UVS(NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LRPLANE,&
...
IF (LNHEE_REFINE_PREH_BBC) THEN
  ...
  CALL GNHEE_REFINE_PREH(YDGEOMETRY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
...
IF (LNHEE_SVDLAPL_FIRST) THEN
  ...
  IF (LGWADV) THEN
    ...
    CALL GNHEE_TNDLAGADIAB_GW(YDGEOMETRY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
     & YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,YDTMP%T0%OROGLM,YDTMP%T0%OROGLL,YDTMP%T0%OROGMM,&
...
ELSE
  ...
  IF (LGWADV.AND.LVFE_GW) THEN
    CALL GNH_TNDLAGADIAB_GW(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
...
CALL GNH_TNDLAGADIAB_SPD(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
...
IF (.NOT.LGWADV) THEN
  ...
  IF (LNHEE_SVDLAPL_FIRST) THEN
    ...
    CALL GNHEE_TNDLAGADIAB_SVD(YDGEOMETRY,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
     & YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,YDTMP%T0%OROGLM,YDTMP%T0%OROGLL,YDTMP%T0%OROGMM,&
  ...
  ELSE
    ...
    !   Dataflow may be tricky, and not always convenient to test future variable changes.
    CALL GNH_TNDLAGADIAB_SVD(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,&
     & YDVARS%U%T0,YDVARS%V%T0,YDCPG_DYN0%UVH%UH,YDCPG_DYN0%UVH%VH,&
...
!*     3.2.10 COMPUTES HALF-LEVEL WINDS.
  ...
  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LLDER,YDVARS%U%T9,YDVARS%V%T9,YDTMP%T9%US,YDTMP%T9%VS)
...
!*     3.2.16 COMPUTES term [gw] or [gW].
  ...
  IF (NVDVAR==3 .OR. NVDVAR==4) THEN
    ...
    IF (LGWADV.AND.LVFE_GW) THEN
      ...
      CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LDMPA,LLGDWI,&
