...
SUBROUTINE CPG_GP_NHEE(YDGEOMETRY,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDGMV,&
 ...
 & PHI0,PHIF0,PHI0FL,PHI0FM,PGWFT0,PGWHT0,PGWT0,PGWS,PGWFL,PGWFM,PGDW0,PKENE0,&
 & PRT0L,PRT0M,&
 & PRE9F,PNHPRE9F,PNHPRE9H,PHI9,PHIF9,PGWFT9,PGWHT9,PGWT9,PGDW9,&
 & PATND,PDBBC,PRDPHI,&
...
IF (NVDVAR==3 .OR. NVDVAR==4) THEN
  ...
  IF (LGWADV.AND.LVFE_GW) THEN
    ...
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LLGWF0,LLGDWI,&
     ...
     & ZRDT0,ZDVER0,PGWHT0,YDCPG_DYN0%GWFT,PRNHPPI=ZRNHPPI0,PGDW=PGDW0)

    YDCPG_DYN0%GWT(KST:KEND,1:NFLEVG)=YDCPG_DYN0%GWFT(KST:KEND,1:NFLEVG)
  ELSE
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LLGWF0,LLGDWI,&
     & POROGL,POROGM,YDCPG_DYN0%XYB%LNPR,YDCPG_DYN0%XYB%ALPH,ZUS0,ZVS0,&
     & ZRDT0,ZDVER0,PGWHT0,YDCPG_DYN0%GWFT,PRNHPPI=ZRNHPPI0)

    IF (LGWADV) YDCPG_DYN0%GWT(KST:KEND,1:NFLEVG)=PGWHT0(KST:KEND,0:NFLEVG-1)
  ENDIF

  IF (LTWOTL.AND.LRDBBC) THEN
    ! PGWS contains (gw)_surf
    YDCPG_DYN0%GWS(KST:KEND)=PGWHT0(KST:KEND,NFLEVG)
  ENDIF
ELSEIF (NVDVAR==5) THEN
  ...
  CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LLGWF0,LLGDWI,&
   ...
   & ZRDT0,ZDVERW0,PGWHT0,YDCPG_DYN0%GWFT,PRNHPPI=ZRNHPPI0)
  IF (LGWADV) THEN
    YDCPG_DYN0%GWT(KST:KEND,1:NFLEVG)=PGWHT0(KST:KEND,0:NFLEVG-1)
  ENDIF
...
IF (.NOT.LGWADV) THEN
  ...
  ELSE
    ...
    CALL GNH_TNDLAGADIAB_SVD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
     & ZT0U,ZT0V,YDCPG_DYN0%UVH%UH,YDCPG_DYN0%UVH%VH,&
     & YDCPG_DYN0%RDPHI,PGWHT0,YDCPG_DYN0%GWFT,ZGWHL,ZGWHM,YDCPG_DYN0%GWFL,YDCPG_DYN0%GWFM,ZDVER0,ZT0DIV,Z3DIVG,&
  ...
  IF (NVDVAR==3 .OR. NVDVAR==4) THEN
    ...
    IF (LGWADV.AND.LVFE_GW) THEN
      ...
      CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LDMPA,LLGDWI,&
       ...
       & ZRDT9,ZDVER9,PGWHT9,YDCPG_DYN9%GWFT,PRNHPPI=ZRNHPPI9,PGDW=PGDW9)

      YDCPG_DYN9%GWT(KST:KEND,1:NFLEVG)=YDCPG_DYN9%GWFT(KST:KEND,1:NFLEVG)
    ELSE
      CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LDMPA,LLGDWI,&
       & POROGL,POROGM,YDCPG_DYN9%XYB%LNPR,YDCPG_DYN9%XYB%ALPH,ZUS9,ZVS9,&
       & ZRDT9,ZDVER9,PGWHT9,YDCPG_DYN9%GWFT,PRNHPPI=ZRNHPPI9)

      IF (LGWADV) YDCPG_DYN9%GWT(KST:KEND,1:NFLEVG)=PGWHT9(KST:KEND,0:NFLEVG-1)
    ENDIF

    IF (LRDBBC) THEN
      ! PGWS contains (gw)_surf
      YDCPG_DYN0%GWS(KST:KEND)=PGWHT9(KST:KEND,NFLEVG)
    ENDIF
  ELSEIF (NVDVAR==5) THEN
    ! Case LVFE_GW is ignored and not coded.
    ! -- Modified W
    ...
    CALL GPGW(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,LDMPA,LLGDWI,&
     ...
     & ZRDT9,ZDVERW9,PGWHT9,YDCPG_DYN9%GWFT,PRNHPPI=ZRNHPPI9)
