...
& LBOUND_D3=>YDMODEL%YRML_DYN%YRDYN%LBOUND_D3, RMAX_D3=>YDMODEL%YRML_DYN%YRDYN%RMAX_D3, RD => YDCST%RD,  &
& RV => YDCST%RV)
...
CALL GPHPRE_EXPL(YDMODEL%YRML_DYN%YRDYNA%LAPRXPK, YDCPG_OPTS%LVERTFE, YDMODEL%YRML_DYN%YRDYNA%NDLNPR, YDMODEL%YRML_DYN%YRDYNA%RHYDR0, YDCPG_OPTS%TOPPRES, &
& YDMODEL%YRCST, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVAB, YDCPG_DYN0%PRE, PRESF=YDCPG_DYN0%PREF, &
...
IF(YDCPG_OPTS%LVERTFE) THEN
  ...
  ENDDO
ENDIF
CALL GPGRXYB_EXPL(YDCPG_OPTS%LVERTFE, YDMODEL%YRML_DYN%YRDYNA%NDLNPR, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                                  &
& NFLEVG, .FALSE., YDVAB, YDCPG_DYN0%PREL, YDCPG_DYN0%PREM, PDELP=YDCPG_DYN0%XYB%DELP, PLNPR=YDCPG_DYN0%XYB%LNPR,                     &
& PRDELP=YDCPG_DYN0%XYB%RDELP, PALPH=YDCPG_DYN0%XYB%ALPH, PRTGR=YDCPG_DYN0%XYB%RTGR, PRPRE=YDCPG_DYN0%XYB%RPRE,                       &
& PRPP=YDCPG_DYN0%XYB%RPP, PCOEFD_DER=Z_XYBDER_COEFD_T0, PLNPRL_DER=Z_XYBDER_LNPRL_T0, PLNPRM_DER=Z_XYBDER_LNPRM_T0,                  &
& PCOEFA_DER=Z_XYBDER_COEFA_T0, PCOEFAPL_DER=Z_XYBDER_COEFAPL_T0, PALPHPLL_DER=Z_XYBDER_ALPHPLL_T0, PALPHPLM_DER=Z_XYBDER_ALPHPLM_T0, &
...
IF(YRSPEC%LGP) THEN
  CALL GPRT(YDCPG_OPTS%LSPRT, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, RD, RV, YDCPG_DYN0%RCP%R,   &
  & YDVARS%T%T0, YDVARS%T%DL, YDVARS%T%DM, YDVARS%Q%DL, YDVARS%Q%DM, Z_RT_T0, YDCPG_DYN0%RTL, YDCPG_DYN0%RTM, &
...
CALL GNHPRE(YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T0, &
& YDCPG_DYN0%PREF, PNHPREF=YDCPG_DYN0%NHPREF, PNHPPI=Z_NHPPI_T0, PRNHPPI=Z_RNHPPI_T0, PDEP=Z_PDEP_T0          &
& )
DO JLEV=1,NFLEVG
...
ENDDO
CALL GNHPREH(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, NPROMA,                         &
& NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T0, YDCPG_DYN0%PRE, YDCPG_DYN0%XYB%DELP, YDCPG_DYN0%XYB%LNPR, &
& Z_NHPPI_T0, YDCPG_DYN0%NHPREF, YDCPG_DYN0%NHPREH, Z_DELNHPRE_T0  )
CALL GNHGRPRE(YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%XYB%RTGR, &
& YDCPG_DYN0%PREL, YDCPG_DYN0%PREM, YDCPG_DYN0%NHPREF, YDVARS%SPD%DL, YDVARS%SPD%DM, Z_NHPREL_T0,                     &
& Z_NHPREM_T0, Z_LNNHPREFL_T0, Z_LNNHPREFM_T0, YDCPG_DYN0%QCHAL, YDCPG_DYN0%QCHAM)
...
CALL GPCTY_EXPL(YDCPG_OPTS%LVERTFE, YDVFE, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, YDMODEL%YRML_DYN%YRDYNA%LRUBC,      &
& YDVAB, YDVETA, YDVARS%U%T0, YDVARS%V%T0, YDVARS%DIV%T0, YDVARS%EDOT%T0, PSPL=YDCPG_DYN0%PREL, PSPM=YDCPG_DYN0%PREM, &
& PRPREF=Z_RPREF_T0, PDELP=YDCPG_DYN0%XYB%DELP, PLNPR=YDCPG_DYN0%XYB%LNPR, PRDELP=YDCPG_DYN0%XYB%RDELP,               &
...
IF(LGWDIAGS_ON) THEN
  CALL UPDATE_GWDIAG(YDGEOMETRY, YDMODEL%YRML_GCONF%YRRIP, NPROMA, NFLEVG, YDCPG_BNDS%KFDIA, YDCPG_OPTS%NSTEP, &
...
IF ( YDCPG_OPTS%LSFORC .AND. (YDCPG_OPTS%LSW_FRC.OR.YDCPG_OPTS%LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDMODEL%YRCST, YDGEOMETRY, YDMODEL%YRML_GCONF, YDMODEL%YRML_PHY_MF%YRPHY2, YDCPG_BNDS%KIDIA,                &
  & YDCPG_BNDS%KFDIA, PP1FORC, YDCPG_DYN0%PREF, YDVARS%T%T0, YDCPG_DYN0%RCP%R, YDCPG_DYN0%CTY%EVEL, YDCPG_DYN0%CTY%VVEL(:, 1:)&
  &                         )
ENDIF
...
CALL GPGEO_EXPL(YDCPG_OPTS%LVERTFE, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG, YDCPG_DYN0%PHI,    &
& YDCPG_DYN0%PHIF, YDVARS%T%T0, Z_RRED_T0, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH, YDGEOMETRY%YRVERT_GEOM&
& )

! * "grad gz" at full levels and half levels.

CALL GPGRGEO_EXPL(YDCPG_OPTS%LVERTFE, YDGEOMETRY, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NFLEVG,                 &
& Z_RT_T0, YDCPG_DYN0%RTL, YDCPG_DYN0%RTM, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH, POROGL=YDCPG_DYN0%OROGL,         &
& POROGM=YDCPG_DYN0%OROGM, PHIFL=YDCPG_DYN0%PHIFL, PHIFM=YDCPG_DYN0%PHIFM, PHIHL=Z_GPHL_T0, PHIHM=Z_GPHM_T0,          &
...
& YDCPG_DYN0%UVH%WWI (1:, 1:), LDVERINT=YDMODEL%YRML_DYN%YRDYNA%LVEREGINT)
...
IF (YDMODEL%YRML_DYN%YRDYNA%LNHEE_REFINE_GRP) THEN
  CALL GPFLWI(YDGEOMETRY%YRDIMV, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH, &
  & YDCPG_DYN0%PRE, YDCPG_DYN0%PREF, YDCPG_DYN0%XYB%DELP, Z_WH2F_T0, LDVERINT=YDMODEL%YRML_DYN%YRDYNA%LVEREGINT)

  CALL GNHEE_REFINE_GRP_EXPL(YDCPG_OPTS%LVERTFE, YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                         &
  & Z_RT_T0, YDCPG_DYN0%RTL, YDCPG_DYN0%RTM, YDCPG_DYN0%PREL, YDCPG_DYN0%PREM, YDCPG_DYN0%XYB%RTGR, YDCPG_DYN0%XYB%ALPH, &
  & PRNHPPI=Z_RNHPPI_T0, PQCHAL=YDCPG_DYN0%QCHAL, PQCHAM=YDCPG_DYN0%QCHAM, PHIHL=Z_GPHL_T0, PHIHM=Z_GPHM_T0,             &
  & PDEP=Z_PDEP_T0, PREF=YDCPG_DYN0%PREF, PREH=YDCPG_DYN0%PRE, PWWH2F=Z_WH2F_T0, PSGRTL=Z_PSGRTL_T0,                     &
  & PSGRTM=Z_PSGRTM_T0, PALPHPLL_DER=Z_XYBDER_ALPHPLL_T0, PALPHPLM_DER=Z_XYBDER_ALPHPLM_T0)
ELSE
  CALL GNHEE_GRP(YDCPG_OPTS%LVERTFE, YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, Z_RT_T0, YDCPG_DYN0%RTL,         &
  & YDCPG_DYN0%RTM, YDCPG_DYN0%PREL, YDCPG_DYN0%PREM, YDCPG_DYN0%XYB%RDELP, YDCPG_DYN0%XYB%RTGR, YDCPG_DYN0%XYB%ALPH, &
...
IF (LDGPXX) THEN
  CALL GPXX(YDCPG_OPTS%LVERTFE, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,          &
  & Z_GPHL_T0, Z_GPHM_T0, YDCPG_DYN0%PHIFL, YDCPG_DYN0%PHIFM, YDCPG_DYN0%XYB%LNPR, Z_RT_T0, YDVARS%U%T0, &
  & YDVARS%V%T0, YDCPG_DYN0%UVH%UH, YDCPG_DYN0%UVH%VH, Z_NHXS_T0, PNHPPI=Z_NHPPI_T0)
  ...
  ELSEIF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR == 5) THEN
    ! NHX = NHX_S - (d13 - dver)
    CALL GNHEE_SVDINCR13(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA,               &
    & YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, YDCPG_DYN0%UVH%UH, YDCPG_DYN0%UVH%VH, PLNPR=YDCPG_DYN0%XYB%LNPR, &
    & PRT=Z_RDT_T0, PNHPPI=Z_NHPPI_T0, PSVDINCR13=Z_SVDINCR13_T0)
...
ELSE
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR == 4) THEN
...
IF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR == 5 .AND. YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
  CALL GPYY(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
  & YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, YDCPG_DYN0%UVH%UH, YDCPG_DYN0%UVH%VH, YDCPG_DYN0%NHY)
ELSE
...
IF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR == 3) THEN
...
ELSEIF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR == 4) THEN
...
IF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR==3 .OR. YDMODEL%YRML_DYN%YRDYNA%NVDVAR==4) THEN
  LLGWF0=.TRUE.
  LLGDWI=.FALSE.
  IF (YDMODEL%YRML_DYN%YRDYNA%LGWADV.AND.YDCPG_OPTS%LVFE_GW) THEN
    ! PGDW is required (at least for SL2TL)
    CALL GPGW(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, NFLEVG, NPROMA,               &
    & YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, LLGWF0, LLGDWI, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, YDCPG_DYN0%XYB%LNPR, &
    & YDCPG_DYN0%XYB%ALPH, Z_US_T0, Z_VS_T0, Z_RDT_T0, Z_DVER_T0, P_GWHT_T0, YDCPG_DYN0%GWFT, PRNHPPI=Z_RNHPPI_T0, &
    & PGDW=P_GDW_T0)
  ...
  ELSE
    ...
    IF (YDMODEL%YRML_DYN%YRDYNA%LGWADV) YDCPG_DYN0%GWT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:NFLEVG)=P_GWHT_T0(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,0:NFLEVG-1)
  ENDIF

  IF (YDCPG_OPTS%LTWOTL.AND.YDMODEL%YRML_DYN%YRDYNA%LRDBBC) THEN
...
ELSEIF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR==5) THEN
  ...
  LLGWF0=.TRUE.
  LLGDWI=.FALSE.
  CALL GPGW(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, &
  & YDCPG_BNDS%KFDIA, LLGWF0, LLGDWI, Z_ZERO_T0, Z_ZERO_T0, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH,                &
  & Z_ZERO_T0, Z_ZERO_T0, Z_RDT_T0, Z_DVERW_T0, P_GWHT_T0, YDCPG_DYN0%GWFT, PRNHPPI=Z_RNHPPI_T0)
...
LLDER=.TRUE.
IF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR==3 .OR. YDMODEL%YRML_DYN%YRDYNA%NVDVAR==4) THEN
  CALL GNHGRGW(YDCPG_OPTS%LVERTFE, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                         &
  & LLDER, Z_RDT_T0, Z_RDTL_T0, Z_RDTM_T0, Z_DVER_T0, Z_DVERL_T0, Z_DVERM_T0, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH,    &
...
ELSEIF (YDMODEL%YRML_DYN%YRDYNA%NVDVAR==5) THEN
  ...
  CALL GNHGRGW(YDCPG_OPTS%LVERTFE, YDGEOMETRY, NPROMA, NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                        &
  & LLDER, Z_RDT_T0, Z_RDTL_T0, Z_RDTM_T0, Z_DVERW_T0, Z_DVERL_T0, Z_DVERM_T0, YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH,  &
...
CALL GNHD3(YDMODEL%YRML_DYN%YRDYNA%L_RDRY_VD, YDMODEL%YRCST, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%DIV%T0, &
& Z_NHXS_T0, Z_DVER_T0, YDCPG_DYN0%RCP%R, RMAX_D3, LBOUND_D3, Z_Z3DIVG_T0)

!*     3.1.19 COMPUTES pre/(Rd T prehyd [Delta log(prehyd)]) at t.

IF (.NOT.YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
  IF (YDMODEL%YRML_DYN%YRDYNA%L_RDRY_VD) THEN
...
& YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%GEOMETRY%RCORI%T0, YDVARS%GEOMETRY%GNORDL%T0, YDVARS%GEOMETRY%GNORDM%T0, &
& Z_PSGRTL_T0, Z_PSGRTM_T0, YDVARS%U%T0, YDVARS%V%T0, YDCPG_TND%TNDU, YDCPG_TND%TNDV, YDCPG_TND%TNDU_NOC,             &
& YDCPG_TND%TNDV_NOC)

!*     3.1.20b COMPUTES [D V_surf/Dt].

IF (.NOT.YDMODEL%YRML_DYN%YRDYNA%LGWADV.OR.YDMODEL%YRML_DYN%YRDYNA%LNHEE_SVDLAPL_FIRST.OR.YDMODEL%YRML_DYN%YRDYNA%LNHEE_REFINE_PREH_BBC) THEN
  CALL GNH_TNDLAGADIAB_UVS(NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%LRPLANE, YDVARS%GEOMETRY%RCORI%T0, &
...
IF (YDMODEL%YRML_DYN%YRDYNA%LNHEE_REFINE_PREH_BBC) THEN
  ...
  CALL GNHEE_REFINE_PREH(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDMODEL%YRCST, YDGEOMETRY, YDCPG_BNDS%KIDIA,      &
  & YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, P_OROGLM_T0, P_OROGLL_T0, P_OROGMM_T0,                   &
  & Z_US_T0, Z_VS_T0, YDCPG_DYN0%PREF, YDCPG_DYN0%PRE, Z_PDEP_T0, Z_WH2F_T0, Z_PSGRTL_T0,                          &
  & Z_PSGRTM_T0, Z_SGRTSL_T0, Z_SGRTSM_T0, YDCPG_TND%TNDU, YDCPG_TND%TNDV, YDCPG_TND%TNDU_NOC, YDCPG_TND%TNDV_NOC, &
...
IF (YDMODEL%YRML_DYN%YRDYNA%LNHEE_SVDLAPL_FIRST) THEN
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
    ...
    IF (YDCPG_OPTS%LVFE_LAPL_BC.OR.YDCPG_OPTS%LVFE_GW) CALL ABOR1(                                   &
                                                       & ' CPG_GP_NHEE 3.1.21: option not yet coded!'&
                                                       & )

    CALL GNHEE_TNDLAGADIAB_GW(YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRML_DYN%YRDYNA%NVDVAR, YDMODEL%YRCST, &
    & YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM,                    &
...
ELSE
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LGWADV.AND.YDCPG_OPTS%LVFE_GW) THEN
    CALL GNH_TNDLAGADIAB_GW(YDMODEL%YRML_DYN%YRDYNA%LGWADV, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDCPG_OPTS%LVFE_GW,     &
    & YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRCST, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,     &
    & PDELP=YDCPG_DYN0%XYB%DELP, PDELNHPRE=Z_DELNHPRE_T0, PTNDGWF_LAP=Z_TNDGWF_LAP_T0, PTNDGWF_OTH=Z_TNDGWF_OTH_T0&
    & )
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      ...
      ENDDO
    ENDDO
  ELSEIF (YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
    CALL GNH_TNDLAGADIAB_GW(YDMODEL%YRML_DYN%YRDYNA%LGWADV, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDCPG_OPTS%LVFE_GW,            &
    & YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRCST, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,            &
    & PDEP=Z_PDEP_T0, PREF=YDCPG_DYN0%PREF, PREH=YDCPG_DYN0%PRE, PTNDGWH_LAP=Z_TNDGWH_LAP_T0, PTNDGWH_OTH=Z_TNDGWH_OTH_T0&
    &             )
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      ...
      ENDDO
    ENDDO
  ELSE IF (YDCPG_OPTS%LVFE_GW) THEN
    CALL GNH_TNDLAGADIAB_GW(YDMODEL%YRML_DYN%YRDYNA%LGWADV, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDCPG_OPTS%LVFE_GW,          &
    & YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRCST, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,          &
    & POROGL=YDCPG_DYN0%OROGL, POROGM=YDCPG_DYN0%OROGM, POROGLM=P_OROGLM_T0, POROGLL=P_OROGLL_T0, POROGMM=P_OROGMM_T0, &
    & PUS=Z_US_T0, PVS=Z_VS_T0, PTNDUS=Z_TNDUS_T0, PTNDVS=Z_TNDVS_T0, PDEP=Z_PDEP_T0, PREF=YDCPG_DYN0%PREF,            &
    & PREH=YDCPG_DYN0%PRE, PDELP=YDCPG_DYN0%XYB%DELP, PDELNHPRE=Z_DELNHPRE_T0, PTNDGWH_LAP=Z_TNDGWH_LAP_T0,            &
    & PTNDGWF_OTH=Z_TNDGWF_OTH_T0, PDBBC=YDCPG_DYN0%DBBC)
  ELSE
    DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ...
    & YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRCST, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,          &
    & POROGL=YDCPG_DYN0%OROGL, POROGM=YDCPG_DYN0%OROGM, POROGLM=P_OROGLM_T0, POROGLL=P_OROGLL_T0, POROGMM=P_OROGMM_T0, &
    & PUS=Z_US_T0, PVS=Z_VS_T0, PTNDUS=Z_TNDUS_T0, PTNDVS=Z_TNDVS_T0, PDEP=Z_PDEP_T0, PDEPS=Z_PDEPS_T0,                &
...
CALL GNH_TNDLAGADIAB_SPD(YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
& YDCPG_DYN0%RCP%KAP, Z_Z3DIVG_T0, YDCPG_DYN0%CTY%VVEL(:, 1:), YDCPG_TND%TNDPD )
...
IF (.NOT.YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LNHEE_SVDLAPL_FIRST) THEN
    ...
    IF (YDCPG_OPTS%LVERTFE) CALL ABOR1(' CPG_GP_NHEE 3.1.23: not yet coded case')

    CALL GNHEE_TNDLAGADIAB_SVD(YDMODEL%YRML_DYN%YRDYNA%LVEREGINT, YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, &
    & YDMODEL%YRML_DYN%YRDYNA%NVDVAR, YDMODEL%YRCST, YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL,             &
    & YDCPG_DYN0%OROGM, P_OROGLM_T0, P_OROGLL_T0, P_OROGMM_T0, YDCPG_DYN0%CTY%EVEL, YDCPG_DYN0%XYB%RDELP,             &
    & YDCPG_DYN0%XYB%LNPR, YDCPG_DYN0%XYB%ALPH, YDCPG_DYN0%PREF, YDCPG_DYN0%PRE, Z_PDEP_T0, YDVARS%U%T0,              &
    & YDVARS%V%T0, YDCPG_DYN0%UVH%UH, YDCPG_DYN0%UVH%VH, YDCPG_TND%TNDU, YDCPG_TND%TNDV, Z_TNDUS_T0,                  &
  ...
  ELSE
    ...
    !   Dataflow may be tricky, and not always convenient to test future variable changes.
    CALL GNH_TNDLAGADIAB_SVD(YDCPG_OPTS%LVFE_GW, YDCPG_OPTS%LVFE_LAPL_BC, YDMODEL%YRML_DYN%YRDYNA%NVDVAR, YDMODEL%YRCST,        &
    & YDGEOMETRY, NFLEVG, NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%U%T0, YDVARS%V%T0, YDCPG_DYN0%UVH%UH, &
    & YDCPG_DYN0%UVH%VH, YDCPG_DYN0%RDPHI, P_GWHT_T0, YDCPG_DYN0%GWFT, Z_GWHL_T0, Z_GWHM_T0, YDCPG_DYN0%GWFL,      &
    & YDCPG_DYN0%GWFM, Z_DVER_T0, YDVARS%DIV%T0, Z_Z3DIVG_T0, Z_TNDGWH_LAP_T0, Z_TNDGWH_OTH_T0,                    &
    & Z_TNDGWF_OTH_T0, Z_PDEP_T0, YDCPG_DYN0%PRE, YDCPG_DYN0%PREF, YDCPG_DYN0%XYB%DELP, YDCPG_DYN0%XYB%LNPR,       &
    & YDCPG_TND%TNDVD )
  ENDIF
ENDIF
...
CALL CPG_GP_NHEE_T9 (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDMODEL, LDGPXX, YDMODEL%YRML_DYN%YRDYNA%LGWADV, &
& YDMODEL%YRML_DYN%YRDYNA%LRDBBC, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDMODEL%YRML_DYN%YRDYNA%L_RDRY_VD, YDMODEL%YRML_DYN%YRDYNA%NVDVAR,            &
& Z_GPHL_T0, Z_GPHM_T0, Z_NHPPI_T0, Z_NHXS_T0, Z_SVDINCR13_T0, Z_ZERO_T0, YDCPG_DYN9, LDLDIAB, LDMPA             &
& )
