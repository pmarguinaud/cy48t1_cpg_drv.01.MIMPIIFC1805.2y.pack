SUBROUTINE CPG_GP_NHEE_T9 (YDGEOMETRY, YDCPG_BNDS, YDVARS, YDCPG_DYN0, YDMODEL, LDGPXX, LDGWADV, LDDER, LDGDWI, &
...
REAL (KIND=JPRB),     INTENT(IN)    :: P_NHPPI_T0 (YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
REAL (KIND=JPRB),     INTENT(IN)    :: P_NHXS_T0 (YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
REAL (KIND=JPRB),     INTENT(IN)    :: P_SVDINCR13_T0 (YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
REAL (KIND=JPRB),     INTENT(IN)    :: P_ZERO_T0 (YDGEOMETRY%YRDIM%NPROMA)
...
CALL GPHPRE_EXPL(YDMODEL%YRCST, YDGEOMETRY%YRDIM%NPROMA, YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRVAB, YDCPG_DYN9%PRE, PRESF=YDCPG_DYN9%PREF,   &
...
CALL GPRCP_YDVARS (YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRDIMV%NFLEVG, YDVARS=YDVARS, KGFLTYP=9, &
...
CALL GNHPRE(YDGEOMETRY, YDGEOMETRY%YRDIM%NPROMA, YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T9, YDCPG_DYN9%PREF, &
...
ENDDO
CALL GNHPREH(YDGEOMETRY, YDGEOMETRY%YRDIM%NPROMA, YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T9, YDCPG_DYN9%PRE, &
...
IF (LDGPXX) THEN
  CALL GPXX(YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, P_GPHL_T0, P_GPHM_T0, YDCPG_DYN0%PHIFL, &
  ...
  ELSEIF (KVDVAR == 5) THEN
    ! NHX = NHX_S - (d13 - dver)
    CALL GNHEE_SVDINCR13(YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL,    &
...
IF (KVDVAR == 5 .AND. LDGWADV) THEN
  CALL GPYY(YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, &
...
ELSEIF (KVDVAR==5) THEN
  ...
  ! -- Modified W
  LDGDWI=.FALSE.
  CALL GPGW(YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, LDMPA, LDGDWI,          &
