...
& YDMODEL%YRCST, YDGEOMETRY%YRDIM%NPROMA, YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,            &
& YDGEOMETRY%YRVAB, YDCPG_DYN9%PRE, PRESF=YDCPG_DYN9%PREF, PDELP=YDCPG_DYN9%XYB%DELP, PLNPR=YDCPG_DYN9%XYB%LNPR,   &
& PRDELP=YDCPG_DYN9%XYB%RDELP, PALPH=YDCPG_DYN9%XYB%ALPH, PRTGR=YDCPG_DYN9%XYB%RTGR, PRPRE=YDCPG_DYN9%XYB%RPRE,    &
& PRPP=YDCPG_DYN9%XYB%RPP)

!*     3.2.3 COMPUTE "R", "Cp" AND "Kap=R/Cp".

CALL GPRCP_EXPL (YDMODEL%YRCST, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRDIMV%NFLEVG, &
& YDVARS=YDVARS, KGFLTYP=9, PCP=YDCPG_DYN9%RCP%CP, PR=YDCPG_DYN9%RCP%R)
...
CALL GNHPRE(YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, YDGEOMETRY%YRDIM%NPROMA, YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA,        &
& YDCPG_BNDS%KFDIA, YDVARS%SPD%T9, YDCPG_DYN9%PREF, PNHPREF=YDCPG_DYN9%NHPREF, PNHPPI=Z_NHPPI_T9, PRNHPPI=Z_RNHPPI_T9, &
& PDEP=Z_PDEP_T9)
DO JLEV=1,YDGEOMETRY%YRDIMV%NFLEVG
...
ENDDO
CALL GNHPREH(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_ECMWF, YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, YDGEOMETRY%YRDIM%NPROMA,     &
& YDGEOMETRY%YRDIMV%NFLEVG, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%SPD%T9, YDCPG_DYN9%PRE, YDCPG_DYN9%XYB%DELP, &
& YDCPG_DYN9%XYB%LNPR, Z_NHPPI_T9, YDCPG_DYN9%NHPREF, YDCPG_DYN9%NHPREH, Z_DELNHPRE_T9  )
...
IF (LDLDIAB.OR.YDMODEL%YRML_PHY_MF%YRSIMPHL%LSIMPH) THEN
  ...
  CALL GPGEO_EXPL(YDCPG_OPTS%LVERTFE, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRDIMV%NFLEVG, &
  & YDCPG_DYN9%PHI, YDCPG_DYN9%PHIF, YDVARS%T%T9, Z_RRED_T9, YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH,                       &
...
IF (LDGPXX .AND. .NOT.LDVERTFE) THEN
  ...
  CALL GPHLUV_EXPL(YDGEOMETRY%YRDIMV, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%U%T9, &
  & YDVARS%V%T9, PWWI=Z_UVH_WWI_T9, PUH=Z_UVH_UH_T9, PVH=Z_UVH_VH_T9)
ENDIF

LLDER=.FALSE.
CALL GPUVS(YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
& LLDER, YDVARS%U%T9, YDVARS%V%T9, Z_US_T9, Z_VS_T9)

!*     3.2.13a COMPUTES TERM "NHX".

! caution: must use "R" and "pre/prehyd" at time t.
IF (LDGPXX) THEN
  CALL GPXX(YDCPG_OPTS%LVERTFE, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, &
  & YDCPG_BNDS%KFDIA, P_GPHL_T0, P_GPHM_T0, YDCPG_DYN0%PHIFL, YDCPG_DYN0%PHIFM, YDCPG_DYN9%XYB%LNPR,             &
  & Z_R0T_T9, YDVARS%U%T9, YDVARS%V%T9, Z_UVH_UH_T9, Z_UVH_VH_T9, P_NHXS_T0, PNHPPI=P_NHPPI_T0)
  ...
  ELSEIF (KVDVAR == 5) THEN
    ! NHX = NHX_S - (d13 - dver)
    CALL GNHEE_SVDINCR13(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG,               &
    & YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM,               &
    & Z_UVH_UH_T9, Z_UVH_VH_T9, PLNPR=YDCPG_DYN9%XYB%LNPR, PRT=Z_RDT_T9, PNHPPI=Z_NHPPI_T9, PSVDINCR13=P_SVDINCR13_T0&
    & )
...
IF (KVDVAR == 5 .AND. LDGWADV) THEN
  CALL GPYY(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, &
  & YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, Z_UVH_UH_T9, Z_UVH_VH_T9,              &
  & YDCPG_DYN9%NHY)
ELSE
...
IF (KVDVAR==3 .OR. KVDVAR==4) THEN
  ...
  IF (LDGWADV.AND.LDVFE_GW) THEN
    ...
    CALL GPGW(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG,        &
    & YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, LDMPA, LLGDWI, YDCPG_DYN0%OROGL, YDCPG_DYN0%OROGM, &
    & YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH, Z_US_T9, Z_VS_T9, Z_RDT_T9, Z_DVER_T9, Z_GWHT_T9, YDCPG_DYN9%GWFT,    &
    & PRNHPPI=Z_RNHPPI_T9, PGDW=Z_GDW_T9)
...
ELSEIF (KVDVAR==5) THEN
  ...
  LLGDWI=.FALSE.
  CALL GPGW(YDCPG_OPTS%LNHDYN, YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_GW, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, &
  & YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, LDMPA, LLGDWI, P_ZERO_T0, P_ZERO_T0,        &
  & YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH, P_ZERO_T0, P_ZERO_T0, Z_RDT_T9, Z_DVERW_T9, Z_GWHT_T9,         &
  & YDCPG_DYN9%GWFT, PRNHPPI=Z_RNHPPI_T9)
