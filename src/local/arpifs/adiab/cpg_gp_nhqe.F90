...
@PROCESS NOCHECK
#endif
SUBROUTINE CPG_GP_NHQE(YDGEOMETRY,YDTMP,YDVARS,YDCPG_DYN0,YDCPG_DYN9,YDMODEL,YDGMV,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
YDTMP%T0%ONE(KST:KEND,1:NFLEVG)=1.0_JPRB
...
IF(LVERTFE) THEN
  ...
  ENDDO
ENDIF
CALL GPGRXYB(NPROMA,KST,KEND,NFLEVG,.FALSE.,YDVAB,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,PXYBDER=YDTMP%T0%XYBDER%ZVIEW,&
           & PDELP=YDCPG_DYN0%XYB%DELP, PLNPR=YDCPG_DYN0%XYB%LNPR, PRDELP=YDCPG_DYN0%XYB%RDELP, &
           & PALPH=YDCPG_DYN0%XYB%ALPH, PRTGR=YDCPG_DYN0%XYB%RTGR, PRPRE=YDCPG_DYN0%XYB%RPRE,  &
           & PRPP=YDCPG_DYN0%XYB%RPP)
...
CALL GPRCPH(YDGEOMETRY,KST,KEND,YDCPG_DYN0%RCP%KAP,YDTMP%T0%KAPH)
...
CALL GPGEO(NPROMA,KST,KEND,NFLEVG,YDCPG_DYN0%PHI,YDCPG_DYN0%PHIF,YDTMP%T0%RT,&
...
CALL GPGRGEO(YDGEOMETRY,NPROMA,KST,KEND,NFLEVG,&
 ...
 & YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,YDTMP%T0%GPHL,YDTMP%T0%GPHM)
...
CALL GNHQE_GRP(YDGEOMETRY,KST,KEND,&
 & YDTMP%T0%RT,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,YDCPG_DYN0%PREL,YDCPG_DYN0%PREM,YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%XYB%ALPH,YDTMP%T0%XYBDER%ZVIEW,YDCPG_DYN0%PRE,&
...
IF( LGRADSP ) THEN
  ...
  YDTMP%T0%SGRTL(KST:KEND,1:NFLEVG)=YDTMP%T0%PSGRTL(KST:KEND,1:NFLEVG)&
   & - (YDVARS%SGRTL%T9(KST:KEND,1:NFLEVG) - YDVARS%SGRTL%T0(KST:KEND,1:NFLEVG))
  YDTMP%T0%SGRTM(KST:KEND,1:NFLEVG)=YDTMP%T0%PSGRTM(KST:KEND,1:NFLEVG)&
   & - (YDVARS%SGRTM%T9(KST:KEND,1:NFLEVG) - YDVARS%SGRTM%T0(KST:KEND,1:NFLEVG))

  ! store for next time-step unfiltered fields
  YDVARS%SGRTL%T0(KST:KEND,1:NFLEVG)=YDTMP%T0%PSGRTL(KST:KEND,1:NFLEVG)
  YDVARS%SGRTM%T0(KST:KEND,1:NFLEVG)=YDTMP%T0%PSGRTM(KST:KEND,1:NFLEVG)

  ! set new values for current timestep
  YDTMP%T0%PSGRTL(KST:KEND,1:NFLEVG)=YDTMP%T0%SGRTL(KST:KEND,1:NFLEVG)
  YDTMP%T0%PSGRTM(KST:KEND,1:NFLEVG)=YDTMP%T0%SGRTM(KST:KEND,1:NFLEVG)

ENDIF

YDTMP%T0%SGRTSL(KST:KEND)=YDTMP%T0%PSGRTL(KST:KEND,NFLEVG)
YDTMP%T0%SGRTSM(KST:KEND)=YDTMP%T0%PSGRTM(KST:KEND,NFLEVG)

!*     3.1.13 COMPUTES TERM "NHX".

IF (LDGPXX) THEN
  ! * compute NHX_s = "wind shear" part of NHX
  CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,YDTMP%T0%GPHL,YDTMP%T0%GPHM,YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,&
...
!*     3.2.4 COMPUTE "RT".
  ...
  YDTMP%T9%R9T(KST:KEND,1:NFLEVG)=YDVARS%T%T9(KST:KEND,1:NFLEVG)*YDCPG_DYN9%RCP%R(KST:KEND,1:NFLEVG)
  YDTMP%T9%R0T(KST:KEND,1:NFLEVG)=YDVARS%T%T9(KST:KEND,1:NFLEVG)*YDCPG_DYN0%RCP%R(KST:KEND,1:NFLEVG)
...
!*     3.2.8 COMPUTES THE GEOPOTENTIAL HEIGHT "gz" AND ITS GRADIENT.
  ...
  IF((LDLDIAB.OR.LSIMPH)) THEN
    ...
    CALL GPGEO(NPROMA,KST,KEND,NFLEVG,YDCPG_DYN9%PHI,YDCPG_DYN9%PHIF,YDTMP%T9%R9T,&
...
!*     3.2.10 COMPUTES HALF-LEVEL WINDS.
  ...
  IF (LDGPXX .AND. .NOT.LVERTFE) THEN
    ...
    YDTMP%T9%UVH%ZVIEW(KST:KEND,1:NFLEVG,YYTHW9%M_WWI)=YDCPG_DYN0%UVH%WWI(KST:KEND,1:NFLEVG)
    CALL GPHLUV(YDGEOMETRY%YRDIMV,NPROMA,KST,KEND,YDVARS%U%T9,YDVARS%V%T9,YDTMP%T9%UVH%ZVIEW)
  ENDIF

  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,KST,KEND,LLDER,YDVARS%U%T9,YDVARS%V%T9,YDTMP%T9%US,YDTMP%T9%VS)
...
!*     3.2.13 COMPUTES TERM "NHX".
  ...
  IF (LDGPXX) THEN
    ...
    CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,YDTMP%T0%GPHL,YDTMP%T0%GPHM,YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,&
     ...
     & YDVARS%U%T9,YDVARS%V%T9,YDTMP%T9%UVH%ZVIEW(:,:,YYTHW9%M_UH),YDTMP%T9%UVH%ZVIEW(:,:,YYTHW9%M_VH),&
     & YDTMP%T9%NHXS_T)
    ! * compute NHX_d = "divergence" part of NHX
    CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
