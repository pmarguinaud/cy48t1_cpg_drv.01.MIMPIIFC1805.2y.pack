...
IF(YRSPEC%LGP) THEN
  CALL GPRT(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   ...
   & ZRT0,YDCPG_DYN0%RTL,YDCPG_DYN0%RTM,PRL=PGFL(1,1,YRSPEC%MPL),PRM=PGFL(1,1,YRSPEC%MPM))
ELSE
  CALL GPRT(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   & YDCPG_DYN0%RCP%R,PGMV(1,1,YT0%MT),PGMV(1,1,YT0%MTL),PGMV(1,1,YT0%MTM),&
   & PGFL(1,1,YQ%MPL),PGFL(1,1,YQ%MPM),&
...
CALL GNHPRE(YDGEOMETRY,NPROMA,NFLEVG,KST,KEND,PGMV(1,1,YT0%MSPD),YDCPG_DYN0%PREF,&
 & PKAP=YDCPG_DYN0%RCP%KAP,&
 & PNHPREF=YDCPG_DYN0%NHPREF,PEQCHAF=ZEQCHA0F,PEIQCHAF=ZEIQCHA0F)
CALL GNHQE_PREH(YDGEOMETRY,KST,KEND,YDCPG_DYN0%PRE,PGMV(1,1,YT0%MSPD),YDCPG_DYN0%NHPREH,&
...
IF ( LSFORC .AND. (LSW_FRC.OR.LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDGEOMETRY,YDMODEL%YRML_GCONF,YDMODEL%YRML_PHY_MF%YRPHY2,KST,KEND, &
   & PGFL(1,1,YFORC(1)%MP),YDCPG_DYN0%PREF,PGMV(1,1,YT0%MT),YDCPG_DYN0%RCP%R,YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%VVEL(:,1:))
ENDIF
...
CALL GNHQE_GRP(YDGEOMETRY,KST,KEND,&
 ...
 & PGMV(1,1,YT0%MSPD),YDCPG_DYN0%QCHAL,YDCPG_DYN0%QCHAM,ZEQCHA0F,ZEQCHA0H,YDCPG_DYN0%RCP%KAP,ZKAP0H,&
...
IF (LDGPXX) THEN
  ...
  CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
   & PGMV(1,1,YT0%MU),PGMV(1,1,YT0%MV),YDCPG_DYN0%XYB%LNPR,YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%ALPH,&
...
!*     3.2.6 COMPUTE QUANTITIES FOR NHQE MODEL.
  ...
  CALL GNHPRE(YDGEOMETRY,NPROMA,NFLEVG,KST,KEND,PGMV(1,1,YT9%MSPD),YDCPG_DYN9%PREF,&
   & PKAP=YDCPG_DYN9%RCP%KAP,&
   & PNHPREF=YDCPG_DYN9%NHPREF)
  CALL GNHQE_PREH(YDGEOMETRY,KST,KEND,YDCPG_DYN9%PRE,PGMV(1,1,YT9%MSPD),YDCPG_DYN9%NHPREH)
...
!*     3.2.13 COMPUTES TERM "NHX".
  ...
  IF (LDGPXX) THEN
    ...
    ! * compute NHX_d = "divergence" part of NHX
    CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
     & PGMV(1,1,YT9%MU),PGMV(1,1,YT9%MV),YDCPG_DYN0%XYB%LNPR,YDCPG_DYN9%XYB%RDELP,YDCPG_DYN9%XYB%ALPH,&
