...
CALL GPRCPH(YDGEOMETRY,KST,KEND,PRCP0(1,1,YYTRCP0%M_KAP),ZKAP0H)
...
CALL GPGEO(NPROMA,KST,KEND,NFLEVG,PHI0,PHIF0,ZRT0,&
 & ZONE,PXYB0(1,1,YYTXYB0%M_LNPR),PXYB0(1,1,YYTXYB0%M_ALPH),&
...
CALL GNHQE_GRP(YDGEOMETRY,KST,KEND,&
 ...
 & PGMV(1,1,YT0%MSPD),PQCHA0L,PQCHA0M,ZEQCHA0F,ZEQCHA0H,PRCP0(1,1,YYTRCP0%M_KAP),ZKAP0H,&
...
IF (LDGPXX) THEN
  ...
  CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
   & PGMV(1,1,YT0%MU),PGMV(1,1,YT0%MV),PXYB0,PRE0L,PRE0M,PRCP0(1,1,YYTRCP0%M_KAP),&
   & ZNHXD_T0)
  ! * compute NHX
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
...
IF (.NOT.LGWADV) THEN
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
      PRDPHI(JROF,JLEV)=1.0_JPRB/(PXYB0(JROF,JLEV,YYTXYB0%M_LNPR)*ZRT0(JROF,JLEV))
    ENDDO
  ENDDO
ENDIF
...
IF (LGWADV) THEN
  ...
  CALL GNHQE_TNDLAGADIAB_GW(YDGEOMETRY,KST,KEND,&
   ...
   & PXYB0(1,1,YYTXYB0%M_LNPR),PXYB0(1,1,YYTXYB0%M_ALPH),PRE0F,PRE0,&
   & ZEQCHA0F,PRCP0(1,1,YYTRCP0%M_KAP),&
...
IF (.NOT.LGWADV) THEN
  ...
  CALL GNHQE_TNDLAGADIAB_SVD(YDGEOMETRY,KST,KEND,&
   & POROGL,POROGM,POROGLM,POROGLL,POROGMM,&
   & PXYB0(1,1,YYTXYB0%M_LNPR),PXYB0(1,1,YYTXYB0%M_ALPH),PRE0F,PRE0,&
   & PGMV(1,1,YT0%MU),PGMV(1,1,YT0%MV),PUVH0(1,0,YYTHW0%M_UH),PUVH0(1,0,YYTHW0%M_VH),ZTNDUS,ZTNDVS,&
   & ZEQCHA0F,PRCP0(1,1,YYTRCP0%M_KAP),&
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PATND(JROF,JLEV,YYTTND%M_TNDT)=PRCP0(JROF,JLEV,YYTRCP0%M_KAP)*PGMV(JROF,JLEV,YT0%MT)*PCTY0(JROF,JLEV,YYTCTY0%M_VVEL)
  ENDDO
ENDDO
...
!*     3.2.3 COMPUTE "R", "Cp" AND "Kap=R/Cp".
  ...
  CALL GPRCP(NPROMA,KST,KEND,NFLEVG,PGFL=PGFL,KGFLTYP=9,PCP=PRCP9(1,1,YYTRCP9%M_CP),&
   & PR=PRCP9(1,1,YYTRCP9%M_R),PKAP=PRCP9(1,1,YYTRCP9%M_KAP))

!*     3.2.4 COMPUTE "RT".

  ! ZR9T9 is R(t-dt)*Tt(t-dt); ZR0T9 is R(t)*Tt(t-dt) (for GPXX)
  ZR9T9(KST:KEND,1:NFLEVG)=PGMV(KST:KEND,1:NFLEVG,YT9%MT)*PRCP9(KST:KEND,1:NFLEVG,YYTRCP9%M_R)
...
!*     3.2.8 COMPUTES THE GEOPOTENTIAL HEIGHT "gz" AND ITS GRADIENT.
  ...
  IF((LDLDIAB.OR.LSIMPH)) THEN
    ...
    CALL GPGEO(NPROMA,KST,KEND,NFLEVG,PHI9,PHIF9,ZR9T9,&
     & ZONE,PXYB9(1,1,YYTXYB9%M_LNPR),PXYB9(1,1,YYTXYB9%M_ALPH),&
     & YDGEOMETRY%YRVERT_GEOM)
  ENDIF
...
!*     3.2.13 COMPUTES TERM "NHX".
  ...
  IF (LDGPXX) THEN
    ...
    ! * compute NHX_d = "divergence" part of NHX
    CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
     & PGMV(1,1,YT9%MU),PGMV(1,1,YT9%MV),PXYB9,PRE9L,PRE9M,PRCP9(1,1,YYTRCP9%M_KAP),&
     & ZNHXD_T9)
    ! * compute NHX
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
