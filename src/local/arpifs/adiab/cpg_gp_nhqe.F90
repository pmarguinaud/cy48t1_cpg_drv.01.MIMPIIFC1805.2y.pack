...
IF(YRSPEC%LGP) THEN
  CALL GPRT(LSPRT,NPROMA,KST,KEND,NFLEVG,RD,RV,&
   & YDCPG_DYN0%RCP%R,PGMV(:,:,YT0%MT),PGMV(:,:,YT0%MTL),PGMV(:,:,YT0%MTM),&
...
CALL GNHPRE(YDGEOMETRY,NPROMA,NFLEVG,KST,KEND,PGMV(:,:,YT0%MSPD),YDCPG_DYN0%PREF,&
 & PKAP=YDCPG_DYN0%RCP%KAP,&
 & PNHPREF=YDCPG_DYN0%NHPREF,PEQCHAF=ZEQCHA0F,PEIQCHAF=ZEIQCHA0F)
CALL GNHQE_PREH(YDGEOMETRY,KST,KEND,YDCPG_DYN0%PRE,PGMV(:,:,YT0%MSPD),YDCPG_DYN0%NHPREH,&
 & PREF=YDCPG_DYN0%PREF,PEQCHAF=ZEQCHA0F,PKAPH=ZKAP0H,&
 & PEQCHAH=ZEQCHA0H)

YDCPG_DYN0%QCHAL(KST:KEND,1:NFLEVG)=PGMV(KST:KEND,1:NFLEVG,YT0%MSPDL)
YDCPG_DYN0%QCHAM(KST:KEND,1:NFLEVG)=PGMV(KST:KEND,1:NFLEVG,YT0%MSPDM)
...
CALL GPCTY(YDVFE,NPROMA,KST,KEND,NFLEVG,LRUBC,YDVAB,YDVETA,&
 & PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),PGMV(:,:,YT0%MDIV),PGMV(:,:,YT0%MEDOT),&
...
IF ( LSFORC .AND. (LSW_FRC.OR.LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDGEOMETRY,YDMODEL%YRML_GCONF,YDMODEL%YRML_PHY_MF%YRPHY2,KST,KEND, &
   & PGFL(:,:,YFORC(1)%MP),YDCPG_DYN0%PREF,PGMV(:,:,YT0%MT),YDCPG_DYN0%RCP%R,YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%VVEL(:,1:))
ENDIF
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    YDCPG_DYN0%KENE(JROF,JLEV)=0.5_JPRB*(PGMV(JROF,JLEV,YT0%MU)**2+PGMV(JROF,JLEV,YT0%MV)**2)
  ENDDO
ENDDO
...
CALL GPHLUV(YDGEOMETRY%YRDIMV,NPROMA,KST,KEND,PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),&
          & PWWI=YDCPG_DYN0%UVH%WWI,PUH=YDCPG_DYN0%UVH%UH,PVH=YDCPG_DYN0%UVH%VH)


LLDER=.TRUE.
CALL GPUVS(NFLEVG,NPROMA,KST,KEND,LLDER,&
 & PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),ZUS0,ZVS0,&
 & PDIVF=PGMV(:,:,YT0%MDIV),PVORF=PGMV(:,:,YT0%MVOR),&
 & PUFL=PGMV(:,:,YT0%MUL),PVFL=PGMV(:,:,YT0%MVL),&
...
CALL GNHQE_GRP(YDGEOMETRY,KST,KEND,&
 ...
 & PGMV(:,:,YT0%MSPD),YDCPG_DYN0%QCHAL,YDCPG_DYN0%QCHAM,ZEQCHA0F,ZEQCHA0H,YDCPG_DYN0%RCP%KAP,ZKAP0H,&
...
IF (LDGPXX) THEN
  ...
  CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,ZGPHL,ZGPHM,YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,&
   ...
   & PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),YDCPG_DYN0%UVH%UH,YDCPG_DYN0%UVH%VH,&
   & ZNHXS_T0)
  ! * compute NHX_d = "divergence" part of NHX
  CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
   & PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),YDCPG_DYN0%XYB%LNPR,YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%ALPH,&
  ...
  ENDDO
ELSE
  YDCPG_DYN0%NHX(KST:KEND,1:NFLEVG)=PGMV(KST:KEND,1:NFLEVG,YT0%MNHX)
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    ZDVER0(JROF,JLEV)=PGMV(JROF,JLEV,YT0%MSVD)-YDCPG_DYN0%NHX(JROF,JLEV)
...
IF (.NOT.LGWADV) THEN
  ...
  CALL GNHQE_TNDLAGADIAB_SVD(YDGEOMETRY,KST,KEND,&
   & YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,YDCPG_DYN0%OROGLM,YDCPG_DYN0%OROGLL,YDCPG_DYN0%OROGMM,&
   & YDCPG_DYN0%XYB%LNPR,YDCPG_DYN0%XYB%ALPH,YDCPG_DYN0%PREF,YDCPG_DYN0%PRE,&
   & PGMV(:,:,YT0%MU),PGMV(:,:,YT0%MV),YDCPG_DYN0%UVH%UH,YDCPG_DYN0%UVH%VH,ZTNDUS,ZTNDVS,&
   & ZEQCHA0F,YDCPG_DYN0%RCP%KAP,&
...
DO JLEV=1,NFLEVG
  DO JROF=KST,KEND
    PATND(JROF,JLEV,YYTTND%M_TNDT)=YDCPG_DYN0%RCP%KAP(JROF,JLEV)*PGMV(JROF,JLEV,YT0%MT)*YDCPG_DYN0%CTY%VVEL(JROF,JLEV)
  ENDDO
ENDDO
...
!*     3.2.6 COMPUTE QUANTITIES FOR NHQE MODEL.
  ...
  CALL GNHPRE(YDGEOMETRY,NPROMA,NFLEVG,KST,KEND,PGMV(:,:,YT9%MSPD),YDCPG_DYN9%PREF,&
   & PKAP=YDCPG_DYN9%RCP%KAP,&
   & PNHPREF=YDCPG_DYN9%NHPREF)
  CALL GNHQE_PREH(YDGEOMETRY,KST,KEND,YDCPG_DYN9%PRE,PGMV(:,:,YT9%MSPD),YDCPG_DYN9%NHPREH)
...
!*     3.2.10 COMPUTES HALF-LEVEL WINDS.
  ...
  IF (LDGPXX .AND. .NOT.LVERTFE) THEN
    ...
    CALL GPHLUV(YDGEOMETRY%YRDIMV,NPROMA,KST,KEND,PGMV(:,:,YT9%MU),PGMV(:,:,YT9%MV),ZUVH9)
  ENDIF

  LLDER=.FALSE.
  CALL GPUVS(NFLEVG,NPROMA,KST,KEND,LLDER,PGMV(:,:,YT9%MU),PGMV(:,:,YT9%MV),ZUS9,ZVS9)
...
!*     3.2.13 COMPUTES TERM "NHX".
  ...
  IF (LDGPXX) THEN
    ...
    CALL GPXX(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,ZGPHL,ZGPHM,YDCPG_DYN0%PHIFL,YDCPG_DYN0%PHIFM,&
     ...
     & PGMV(:,:,YT9%MU),PGMV(:,:,YT9%MV),ZUVH9(1,0,YYTHW9%M_UH),ZUVH9(1,0,YYTHW9%M_VH),&
     & ZNHXS_T9)
    ! * compute NHX_d = "divergence" part of NHX
    CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
     & PGMV(:,:,YT9%MU),PGMV(:,:,YT9%MV),YDCPG_DYN0%XYB%LNPR,YDCPG_DYN9%XYB%RDELP,YDCPG_DYN9%XYB%ALPH,&
  ...
  ELSE
    ...
    YDCPG_DYN9%NHX(KST:KEND,1:NFLEVG)=PGMV(KST:KEND,1:NFLEVG,YT9%MNHX)
  ENDIF
...
!*     3.2.14 COMPUTES dver.
  ...
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
      ZDVER9(JROF,JLEV)=PGMV(JROF,JLEV,YT9%MSVD)-YDCPG_DYN9%NHX(JROF,JLEV)
    ENDDO
  ENDDO
