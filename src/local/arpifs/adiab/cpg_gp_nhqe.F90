...
IF ( LSFORC .AND. (LSW_FRC.OR.LSOMEGA_FRC) ) THEN
  CALL GPCTY_FORC(YDGEOMETRY,YDMODEL%YRML_GCONF,YDMODEL%YRML_PHY_MF%YRPHY2,KST,KEND, &
   & PGFL(1,1,YFORC(1)%MP),PRE0F,PGMV(1,1,YT0%MT),YDCPG_DYN0%RCP%R,YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%VVEL(:,1:))
ENDIF
...
CALL GNHQE_GRP(YDGEOMETRY,KST,KEND,&
 & ZRT0,PRT0L,PRT0M,PRE0L,PRE0M,YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%RTGR,YDCPG_DYN0%XYB%ALPH,ZXYBDER0,PRE0,&
...
IF (LDGPXX) THEN
  ...
  CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
   ...
   & YDCPG_DYN0%XYB%RTGR,PRE0L,PRE0M,YDCPG_DYN0%RCP%KAP,&
   & ZNHXD_T0)
  ! * compute NHX
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
...
ELSE
  ...
  CALL GNHQE_XXD(YDGEOMETRY,NFLEVG,NPROMA,KST,KEND,&
   & PGMV(1,1,YT0%MU),PGMV(1,1,YT0%MV),YDCPG_DYN0%XYB%LNPR,YDCPG_DYN0%XYB%RDELP,YDCPG_DYN0%XYB%ALPH,&
   & YDCPG_DYN0%XYB%RTGR,PRE0L,PRE0M,YDCPG_DYN0%RCP%KAP,ZNHXD_T0)
  ! * compute NHX_s
  DO JLEV=1,NFLEVG
    DO JROF=KST,KEND
