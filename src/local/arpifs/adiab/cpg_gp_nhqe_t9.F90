SUBROUTINE CPG_GP_NHQE_T9 (YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDCPG_DYN0, YDMODEL, LDGPXX, &
& LDGWADV, LDRDBBC, LDVERTFE, LDVFE_GW, P_GPHL_T0, P_GPHM_T0, P_ONE_T0, YDCPG_DYN9, LDLDIAB, LDMPA)
...
REAL(KIND=JPRB)       ,INTENT(INOUT)  :: P_ONE_T0(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)
...
& YDVARS=YDVARS, KGFLTYP=9, PCP=YDCPG_DYN9%RCP%CP, PR=YDCPG_DYN9%RCP%R, PKAP=YDCPG_DYN9%RCP%KAP)
...
& YDCPG_BNDS%KFDIA, YDVARS%SPD%T9, YDCPG_DYN9%PREF, PKAP=YDCPG_DYN9%RCP%KAP, PNHPREF=YDCPG_DYN9%NHPREF          &
& )
CALL GNHQE_PREH(YDCPG_OPTS%LVERTFE, YDMODEL%YRML_DYN%YRDYNA%NPDVAR, YDGEOMETRY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, &
& YDCPG_DYN9%PRE, YDVARS%SPD%T9, YDCPG_DYN9%NHPREH      )
...
IF((LDLDIAB.OR.YDMODEL%YRML_PHY_MF%YRSIMPHL%LSIMPH)) THEN
  ...
  & YDCPG_DYN9%PHI, YDCPG_DYN9%PHIF, Z_R9T_T9, P_ONE_T0, YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH, YDGEOMETRY%YRVERT_GEOM    &
  & )
ENDIF
...
IF (LDGPXX) THEN
  ...
  & Z_R0T_T9, YDVARS%U%T9, YDVARS%V%T9, Z_UVH_UH_T9, Z_UVH_VH_T9, Z_NHXS_T_T9)
  ! * compute NHX_d = "divergence" part of NHX
  CALL GNHQE_XXD(YDMODEL%YRML_DYN%YRDYNA%LRUBC, YDCPG_OPTS%LVERTFE, YDGEOMETRY, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIM%NPROMA, &
  & YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDVARS%U%T9, YDVARS%V%T9, YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%RDELP,          &
  & YDCPG_DYN9%XYB%ALPH, YDCPG_DYN9%XYB%RTGR, YDCPG_DYN9%PREL, YDCPG_DYN9%PREM, YDCPG_DYN9%RCP%KAP,                   &
  & Z_NHXD_T_T9)
  ! * compute NHX
...
IF (LDGWADV.AND.LDVFE_GW) THEN
  ...
  & YDCPG_DYN9%XYB%LNPR, YDCPG_DYN9%XYB%ALPH, Z_US_T9, Z_VS_T9, Z_R9T_T9, Z_DVER_T9, Z_GWHT_T9, YDCPG_DYN9%GWFT,    &
  & PGDW=Z_GDW_T9)
