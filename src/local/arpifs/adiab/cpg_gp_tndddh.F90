...
IF (LTWOTL) THEN
  ...
  IF (LNHDYN)THEN
    ...
    IF (LNHX) PGMVTNDSL(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDGEOMETRY%YRDIMV%NFLEVG,YDMODEL%YRML_DIAG%YRMDDH%MSLDDH_NHX)=-YDCPG_DYN0%NHX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDGEOMETRY%YRDIMV%NFLEVG)
  ENDIF
...
ELSE
  ...
  IF (LNHDYN)THEN
    ...
    IF (LNHX) PGMVTNDSL(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDGEOMETRY%YRDIMV%NFLEVG,YDMODEL%YRML_DIAG%YRMDDH%MSLDDH_NHX)=-YDCPG_DYN9%NHX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,1:YDGEOMETRY%YRDIMV%NFLEVG)
  ENDIF
  DO JGFL=1,YDMODEL%YRML_GCONF%YGFL%NUMFLDS
    IF (YDMODEL%YRML_GCONF%YGFL%YCOMP(JGFL)%LADV) THEN
