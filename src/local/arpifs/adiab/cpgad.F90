...
@PROCESS NOCHECK
#endif
SUBROUTINE CPGAD(YDGEOMETRY,YDGMV,YDGMV5,YDSURF5,&
 !---------------------------------------------------------------------
 ! - INPUT .
 & YDMODEL,LDRSTEP,LDUSEPB1,LDSTR,&
 & KGPCOMP,KSTGLO,KIBL,&
 ...
 ! - TRAJECTORY .
 & PGMV5,PGMV5S,PGFL5,&
 & PTRAJ_PHYS,PTRAJ_PHYS_TLAD,PTRAJ_SRFC,PTRAJ_CST,PTRAJ_SLAG)

!**** *CPGAD* - Adjoint grid point calculations.
...
!      F. Vana  13-Jan-2009: add SLHDA and SLHDD0 for SLHD KAPPA
...
!      T.Wilhelmsson (Apr 2012): Move OpenMP loop to cpg_drv_ad
...
USE INTDYN_MOD,ONLY : YYTTND, YYTHW5, YYTCTY5, YYTCTY0, YYTXYBDER5,&
 & YYTRCP5, YYTRCP95, YYTRCP0, YYTRCP9, YYTXYB0, YYTXYB5, YYTXYB9,&
 & YYTXYB95, YYTGMVT95, YYTGFLT95, YYTHW0
USE EINT_MOD , ONLY : SL_STRUCT
USE YOMTRAJ  , ONLY : TRAJ_PHYS_TYPE, TRAJ_PHYS_TLAD_TYPE, TRAJ_SRFC_TYPE,&
 & TRAJ_CST_TYPE, TRAJ_SLAG_TYPE
...
REAL(KIND=JPRB) :: ZPRE0(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG)                  ! cf. PRE0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPRE0F(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                   ! cf. PRE0F in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPRE0L(YDGEOMETRY%YRDIM%NPROMA)                          ! cf. PRE0L in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPRE0M(YDGEOMETRY%YRDIM%NPROMA)                          ! cf. PRE0M in CPG_GP_AD.
REAL(KIND=JPRB) :: ZRT0L(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                    ! cf. PRT0L in CPG_GP_AD.
REAL(KIND=JPRB) :: ZRT0M(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                    ! cf. PRT0M in CPG_GP_AD.
REAL(KIND=JPRB) :: ZXYB0(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YYTXYB0%NDIM)       ! cf. PXYB0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZUVH0(YDGEOMETRY%YRDIM%NPROMNH,0:YDGEOMETRY%YRDIMV%NFLEVG,YYTHW0%NDIM)     ! cf. PUVH0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZRCP0(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YYTRCP0%NDIM)       ! cf. PRCP0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPHI0(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG)                  ! cf. PHI0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPHIF0(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                   ! cf. PHIF0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZCTY0(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG,YYTCTY0%NDIM)     ! cf. PCTY0 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZKENE0(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                   ! cf. PKENE0 in CPG_GP_AD.
...
REAL(KIND=JPRB) :: ZQS(YDGEOMETRY%YRDIM%NPROMM)                             ! cf. PQS in MF_PHYS_AD.
REAL(KIND=JPRB) :: ZQS1(YDGEOMETRY%YRDIM%NPROMM)                            ! cf. PQS1 in MF_PHYS_AD.
REAL(KIND=JPRB) :: ZTS(YDGEOMETRY%YRDIM%NPROMM)                             ! cf. PTS in MF_PHYS_AD.
REAL(KIND=JPRB) :: ZTS1(YDGEOMETRY%YRDIM%NPROMM)                            ! cf. PTS1 in MF_PHYS_AD.
REAL(KIND=JPRB) :: ZSNS(YDGEOMETRY%YRDIM%NPROMM)                            ! cf. PSNS in MF_PHYS_AD.
! - auxiliary quantities at time t-dt.
REAL(KIND=JPRB) :: ZPRE9(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG)                  ! cf. PRE9 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPRE9F(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                   ! cf. PRE9F in CPG_GP_AD.
REAL(KIND=JPRB) :: ZXYB9(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YYTXYB9%NDIM)       ! cf. PXYB9 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZRCP9(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG,YYTRCP9%NDIM)       ! cf. PCP9 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPHI9(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG)                  ! cf. PHI9 in CPG_GP_AD.
REAL(KIND=JPRB) :: ZPHIF9(YDGEOMETRY%YRDIM%NPROMA,YDGEOMETRY%YRDIMV%NFLEVG)                   ! cf. PHIF9 in CPG_GP_AD.
! - other quantities.
...
#include "cpg_dyn_ad.intfb.h"
#include "cpg_end_ad.intfb.h"
#include "cpg_gp_ad.intfb.h"
#include "cpg_zero_ad.intfb.h"
#include "gpmpfc5.intfb.h"
#include "mf_physad.intfb.h"

!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('CPGAD',0,ZHOOK_HANDLE)
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 &  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL), YDDYN=>YDMODEL%YRML_DYN%YRDYN,YDPTRSLB1=>YDMODEL%YRML_DYN%YRPTRSLB1, &
 & YDPTRSLB2=>YDMODEL%YRML_DYN%YRPTRSLB2,YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL,YGFL=>YDMODEL%YRML_GCONF%YGFL, &
...
CALL CPG_ZERO_AD(&
 ! --------------------------------------------------------------------------
 ! - INPUT:
 & YDGEOMETRY,YDDYN,YDPTRGPPC,YDSIMPHL,IST,IEND,&
 ! --------------------------------------------------------------------------
 ! - OUTPUT:
 & ZRT0L,ZRT0M,ZPRE0F,ZPRE0,ZPRE0L,ZPRE0M,ZRCP0,ZPHIF0,ZPHI0,ZKENE0,&
 & ZXYB0,ZCTY0,ZUVH0,&
 & ZPRE9F,ZPRE9,ZRCP9,ZPHIF9,ZPHI9,ZXYB9,&
 & ZGPPC,ZATND)

CALL CPG_END_AD(YDGEOMETRY,YDGMV,&
 !---------------------------------------------------------------------
 ! - INPUT in TL code.
 & YDMODEL%YRML_GCONF,YDDYN,IST,IEND,GM,&
 !---------------------------------------------------------------------
 ! - INPUT/OUTPUT in TL code.
 & PGFL,PGMV,PGMVS)
...
IF (LDUSEPB1) THEN
  DO JFLD=1,NFLDSLB1
    DO JROF=KSTGLO,MIN(KSTGLO-1+NPROMA,KGPCOMP)
      ZSLBUF1AU(JROF-KSTGLO+1,JFLD)=&
       & ZSLBUF1AU(JROF-KSTGLO+1,JFLD)+PB1(YDSL%NSLCORE(JROF),JFLD)
    ENDDO
  ENDDO
ENDIF

CALL CPG_DYN_AD(YDGEOMETRY,YDGMV,YDGMV5,YDSURF5,&
 ! --- INPUT IN TL ----------------------------------------------------
...
! * Unlagged ECMWF physics (CALL EC_PHYS_AD) or
!   split ECMWF physics (CALL EC_PHYS_LSLPHY_AD) not yet coded.

! * Unlagged MF physics.
!   Remark! the management of physics for LPC_FULL PC scheme
!   (CALL CPG_PT_AD before CALL MF_PHYSAD) is not yet coded.
IF (LSIMPH.AND.(NCURRENT_ITER == 0)) THEN

  CALL MF_PHYSAD(YDGEOMETRY,YDGMV,YDGMV5,YDSURF5,&
...
! * At this level ZSLBUF1AU and PB1 are not used any longer.
!   They are reset to zero (for security).
IF (LDUSEPB1) THEN
  DO JFLD=1,NFLDSLB1
    DO JROF=KSTGLO,MIN(KSTGLO-1+NPROMA,KGPCOMP)
      ZSLBUF1AU(JROF-KSTGLO+1,JFLD)=0.0_JPRB
      PB1(YDSL%NSLCORE(JROF),JFLD)=0.0_JPRB
    ENDDO
  ENDDO
ENDIF
...
CALL CPG_GP_AD(YDGEOMETRY,YDGMV,YDGMV5,&
 !---------------------------------------------------------------------
 ! - INPUT .
 & YDMODEL%YRML_PHY_EC%YREPHY,YDMODEL%YRML_GCONF,YDMODEL%YRML_DYN,YDSIMPHL,IST,IEND,LDRSTEP,PTE,KIBL,&
 !---------------------------------------------------------------------
 ! - INPUT IN TL.
 & PGFL,&
 !---------------------------------------------------------------------
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('CPGAD',1,ZHOOK_HANDLE)
END SUBROUTINE CPGAD
