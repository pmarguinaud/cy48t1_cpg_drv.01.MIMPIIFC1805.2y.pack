...
DO JROF = KSTART, KPROF
  PEVEL(JROF,KFLEV) = PEVEL(JROF,KFLEV) + YDCST%RG*ZEVELS(JROF)
  PSDVBC(JROF,KFLEV) = PSDVBC(JROF,KFLEV) + YDCST%RG*ZEVELS(JROF)
ENDDO
...
DO JLEV = 1, KFLEV-1
  DO JROF = KSTART, KPROF
    PEVEL(JROF,JLEV) = PEVEL(JROF,JLEV) + YDCST%RG*( YDVAB%VBH(JLEV)* ZEVELS(JROF) )  
    PSDVBC(JROF,JLEV)= PSDVBC(JROF,JLEV) + YDCST%RG*( YDVAB%VBH(JLEV)* ZEVELS(JROF) )
  ENDDO
ENDDO
...
DO JROF = KSTART, KPROF
  PSPT1(JROF)=PSPT1(JROF)-PDT*YDCST%RG*(PFP(JROF,KFLEV)+ZFE(JROF,KFLEV))/PRES(JROF)  
ENDDO
