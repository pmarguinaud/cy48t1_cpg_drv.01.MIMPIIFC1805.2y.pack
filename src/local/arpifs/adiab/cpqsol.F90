SUBROUTINE CPQSOL(YDDIMV,YDPHY,KPROMA,KSTART,KPROF,PRES,PTS0,PQS,PQSATS,PQSOL)

USE YOMDIMV  , ONLY : TDIMV
USE PARKIND1 , ONLY : JPIM, JPRB
USE YOMHOOK  , ONLY : LHOOK, DR_HOOK
USE YOMCST   , ONLY : RV, RCPV, RETV, RCW, RCS, RLVTT, RLSTT, RTT,&
 & RALPW, RBETW, RGAMW, RALPS, RBETS, RGAMS, RALPD, RBETD, RGAMD  
...
DO JROF=KSTART,KPROF
  ...
  ZHU=PQS(JROF)*(1.0_JPRB+RETV*PQSATS(JROF))/(PQSATS(JROF)*&
   & (1.0_JPRB+RETV*PQS(JROF)))  

  IF (LNEIGE) THEN
    ZDELT0 = MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-PTS0(JROF)))
  ELSE
    ZDELT0 = 0.0_JPRB
  ENDIF
  ...
  PQSOL(JROF)=ZHU*ZQSATS0/(1.0_JPRB+RETV*ZQSATS0*(1.0_JPRB-ZHU))
