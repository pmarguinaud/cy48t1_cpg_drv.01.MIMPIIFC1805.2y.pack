SUBROUTINE CPTEND_NEW ( YDMODEL,KPROMA, KSTART, KPROF, KFLEV,PGNORDL,PGNORDM,&
 !  Variables 2D Input
...
DO JLEV= 0, KFLEV
  DO JROF = KSTART, KPROF
    ...
    IF (LLDPSFI) THEN
      ...
      PFHSCL(JROF,JLEV)=ZFPLCL(JROF,JLEV)*ZTI(JROF,JLEV)*RCW
      PFHSCN(JROF,JLEV)=ZFPLCN(JROF,JLEV)*ZTI(JROF,JLEV)*RCS
      PFHSSL(JROF,JLEV)=ZFPLSL(JROF,JLEV)*ZTI(JROF,JLEV)*RCW
      PFHSSN(JROF,JLEV)=ZFPLSN(JROF,JLEV)*ZTI(JROF,JLEV)*RCS
      IF (LGRAPRO) THEN
        PFHSSG(JROF,JLEV)=ZFPLSG(JROF,JLEV)*ZTI(JROF,JLEV)*RCS
      ENDIF

    ELSE

      PFHSCL(JROF,JLEV)=PFPLCL(JROF,JLEV)*ZTI(JROF,JLEV)*(RCW-RCPD)
      PFHSCN(JROF,JLEV)=PFPLCN(JROF,JLEV)*ZTI(JROF,JLEV)*(RCS-RCPD)
      PFHSSL(JROF,JLEV)=PFPLSL(JROF,JLEV)*ZTI(JROF,JLEV)*(RCW-RCPD)
      PFHSSN(JROF,JLEV)=PFPLSN(JROF,JLEV)*ZTI(JROF,JLEV)*(RCS-RCPD)
      IF (LGRAPRO) THEN
        PFHSSG(JROF,JLEV)=PFPLSG(JROF,JLEV)*ZTI(JROF,JLEV)*(RCS-RCPD)
      ENDIF

    ENDIF

    PFHPCL(JROF,JLEV)= - RLVZER * (PFCCQL(JROF,JLEV)-PFECL(JROF,JLEV))
    PFHPCN(JROF,JLEV)= - RLSZER * (PFCCQN(JROF,JLEV)-PFECN(JROF,JLEV))
    PFHPSL(JROF,JLEV)= - RLVZER * (PFCSQL(JROF,JLEV)-PFESL(JROF,JLEV))
    PFHPSN(JROF,JLEV)= - RLSZER * (PFCSQN(JROF,JLEV)-PFESN(JROF,JLEV))

    PFP(JROF,JLEV)= ZFPL(JROF,JLEV) +ZFPN(JROF,JLEV)

    IF (LGRAPRO) THEN
      PFHPSG(JROF,JLEV)= - RLSZER * (-PFESG(JROF,JLEV))
      PFHPCG(JROF,JLEV)= - RLSZER * (-PFECG(JROF,JLEV))
...
DO JLEV= 0, KFLEV
  DO JROF = KSTART, KPROF
    ZJTOT(JROF,JLEV) = PFRSO(JROF,JLEV)+PFRTH(JROF,JLEV)+PFRMH(JROF,JLEV)+&
        ...
        & RLVZER*(PFCCQL(JROF,JLEV)+PFCSQL(JROF,JLEV)-PFESL(JROF,JLEV)&
        & -PFECL(JROF,JLEV))-&
        & RLSZER*(PFCCQN(JROF,JLEV)+PFCSQN(JROF,JLEV)-PFESN(JROF,JLEV)&
        & -PFECN(JROF,JLEV))&
        & +ZFHIMCC(JROF,JLEV)
  ENDDO
ENDDO
 ...
 DO JLEV = 0, KFLEV
   DO JROF = KSTART, KPROF
     ZJTOT(JROF,JLEV) = ZJTOT(JROF,JLEV) + (RCS-RCPD)*ZFPG(JROF,JLEV)*ZTI(JROF,JLEV) +&
      & RLSZER*(PFESG(JROF,JLEV)+PFECG(JROF,JLEV))
   ENDDO
 ENDDO
ENDIF
...
IF (LFLEXDIA) THEN
   IF (LDDH_OMP) THEN
    ...
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=-RLVZER*PFCCQL(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTCCQL',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=-RLVZER*PFCSQL(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTCSQL',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=RLVZER*PFESL(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTESL',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=RLVZER*PFECL(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTECL',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=-RLSZER*PFCCQN(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTCCQN',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=-RLSZER*PFCSQN(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTCSQN',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=RLSZER*PFESN(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTESN',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=RLSZER*PFECN(KSTART:KPROF,0:KFLEV)
    CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPFLUX,'FCTECN',YDDDH)
    ZTMPFLUX(KSTART:KPROF,0:KFLEV)=RLSZER*PFESG(KSTART:KPROF,0:KFLEV)
...
IF(LMUSCLFA) THEN
  ...
  DO JLEV=1,KFLEV
    DO JROF=KSTART,KPROF
      ZQ1(JROF,JLEV)=(PTENDH(JROF,JLEV)-(RCPV-RCPD)*PTENDQ(JROF,JLEV)*PT(JROF,JLEV))/PCP(JROF,JLEV)
