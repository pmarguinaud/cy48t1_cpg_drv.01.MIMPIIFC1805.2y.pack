...
DO JLON=KSTART,KPROF
  ...
  ZCPS(JLON)=YDCST%RCPD*(1.0_JPRB-PQS(JLON))+YDCST%RCPV*PQS(JLON)
  DO JLEV=1,KFLEV
...
DO JLON=KSTART,KPROF
  P_ZFHSCL(JLON,0)=PFPLCL(JLON,0)*PT(JLON,1)*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSCN(JLON,0)=PFPLCN(JLON,0)*PT(JLON,1)*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSSL(JLON,0)=PFPLSL(JLON,0)*PT(JLON,1)*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSSN(JLON,0)=PFPLSN(JLON,0)*PT(JLON,1)*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))

  P_ZFHPCL(JLON,0)= - YDCST%RLVZER * PFCCQL(JLON,0)
  P_ZFHPCN(JLON,0)= - YDCST%RLSZER * PFCCQN(JLON,0)
  P_ZFHPSL(JLON,0)= - YDCST%RLVZER * PFCSQL(JLON,0)
  P_ZFHPSN(JLON,0)= - YDCST%RLSZER * PFCSQN(JLON,0)
...
DO JLEV = 1 , KFLEV-1
  DO JLON = KSTART , KPROF
    ...
    P_ZFHSCL(JLON,JLEV)=PFPLCL(JLON,JLEV)*ZTI*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
    P_ZFHSCN(JLON,JLEV)=PFPLCN(JLON,JLEV)*ZTI*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
    P_ZFHSSL(JLON,JLEV)=PFPLSL(JLON,JLEV)*ZTI*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
    P_ZFHSSN(JLON,JLEV)=PFPLSN(JLON,JLEV)*ZTI*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))

    P_ZFHPCL(JLON,JLEV)= - YDCST%RLVZER * PFCCQL(JLON,JLEV)
    P_ZFHPCN(JLON,JLEV)= - YDCST%RLSZER * PFCCQN(JLON,JLEV)
    P_ZFHPSL(JLON,JLEV)= - YDCST%RLVZER * PFCSQL(JLON,JLEV)
    P_ZFHPSN(JLON,JLEV)= - YDCST%RLSZER * PFCSQN(JLON,JLEV)
...
DO JLON=KSTART,KPROF
  P_ZFHSCL(JLON,KFLEV)=PFPLCL(JLON,KFLEV)*PTS(JLON)*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSCN(JLON,KFLEV)=PFPLCN(JLON,KFLEV)*PTS(JLON)*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSSL(JLON,KFLEV)=PFPLSL(JLON,KFLEV)*PTS(JLON)*(YDCST%RCW-YDCST%RCPD*(1.0_JPRB-ZDPSFI))
  P_ZFHSSN(JLON,KFLEV)=PFPLSN(JLON,KFLEV)*PTS(JLON)*(YDCST%RCS-YDCST%RCPD*(1.0_JPRB-ZDPSFI))

  P_ZFHPCL(JLON,KFLEV)= - YDCST%RLVZER * PFCCQL(JLON,KFLEV)
  P_ZFHPCN(JLON,KFLEV)= - YDCST%RLSZER * PFCCQN(JLON,KFLEV)
  P_ZFHPSL(JLON,KFLEV)= - YDCST%RLVZER * PFCSQL(JLON,KFLEV)
  P_ZFHPSN(JLON,KFLEV)= - YDCST%RLSZER * PFCSQN(JLON,KFLEV)
