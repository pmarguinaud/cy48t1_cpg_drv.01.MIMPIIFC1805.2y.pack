...
REAL(KIND=JPRB)   ,INTENT(IN) , TARGET  :: PCP(KPROMA,KFLEV) 
REAL(KIND=JPRB)   ,INTENT(IN) , TARGET  :: PDELP(KPROMA,KFLEV) 
REAL(KIND=JPRB)   ,INTENT(IN) , TARGET  :: PTT(KPROMA,KFLEV) 
REAL(KIND=JPRB)   ,INTENT(IN) , TARGET  :: PUT(KPROMA,KFLEV) 
REAL(KIND=JPRB)   ,INTENT(IN) , TARGET  :: PVT(KPROMA,KFLEV) 
...
REAL(KIND=JPRB),DIMENSION(:,:),  POINTER :: ZCP
REAL(KIND=JPRB),DIMENSION(:,:),  POINTER :: ZDELP
REAL(KIND=JPRB),DIMENSION(:,:),  POINTER :: ZTT 
REAL(KIND=JPRB),DIMENSION(:,:),  POINTER :: ZUT 
REAL(KIND=JPRB),DIMENSION(:,:),  POINTER :: ZVT 
...
DO JLEV=1,KFLEV
  DO JROF=KST,KPROF
    ZCPF = 1.0_JPRB / ( ZCP(JROF,JLEV) + PDT * ZTDCP(JROF,JLEV) )
    ZH = ZCP(JROF,JLEV) * ZTT(JROF,JLEV)
    ZDEC(JROF,JLEV) = ( ZUT(JROF,JLEV)*PTENDU_ZDEC(JROF,JLEV) &
     & + ZVT(JROF,JLEV)*PTENDV_ZDEC(JROF,JLEV) + 0.5_JPRB * PDT * &
     & ( (PTENDU_ZDEC(JROF,JLEV))**2 + (PTENDV_ZDEC(JROF,JLEV))**2 ) )
    ZRRGDELP(JROF,JLEV)=ZFLAG*ZRRG*ZDELP(JROF,JLEV)
    ZDTT1(JROF,JLEV) = ZFLAG * ( ZCPF * ( ZH - PDT*ZDEC(JROF,JLEV)&
     & + PDT * PTENDH(JROF,JLEV) ) - ZTT(JROF,JLEV) )
    IF (LPTKE) THEN
