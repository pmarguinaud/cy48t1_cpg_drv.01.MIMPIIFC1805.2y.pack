...
DO JLEV=1,KFLEV
  DO JROF=KST,KPROF
    YDMF_PHYS_NEXT_STATE%U (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) = &
  & YDMF_PHYS_NEXT_STATE%U (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) + PDT*PTENDU(JROF,JLEV)
    YDMF_PHYS_NEXT_STATE%V (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) = &
  & YDMF_PHYS_NEXT_STATE%V (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) + PDT*PTENDV(JROF,JLEV)
    YDMF_PHYS_NEXT_STATE%T (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) = &
  & YDMF_PHYS_NEXT_STATE%T (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) + ZDTT1(JROF,JLEV)
  ENDDO
ENDDO
...
IF(LNHDYN) THEN
  DO JLEV=1,KFLEV
    DO JROF=KST,KPROF
      YDMF_PHYS_NEXT_STATE%SVD (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) = &
    & YDMF_PHYS_NEXT_STATE%SVD (JROF, JLEV+1-YDCPG_DIM%ILEVT1I) + PDT*PTENDD(JROF,JLEV)
    ENDDO
  ENDDO
ENDIF
