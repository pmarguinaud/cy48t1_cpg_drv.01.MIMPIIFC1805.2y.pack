...
INTEGER(KIND=JPIM) :: JLEV, JLON, IPGFL, JGFL
...
DO JLEV=1,YDCPG_OPTS%KFLEVG
  DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ZCPF = 1.0_JPRB / ( YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(JLON,JLEV) + PDT * ZTDCP(JLON,JLEV) )
    ZH = YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(JLON,JLEV) * YDMF_PHYS_BASE_STATE%T(JLON,JLEV)
    ZDEC(JLON,JLEV) = ( YDMF_PHYS_BASE_STATE%U(JLON,JLEV)*PTENDU_ZDEC(JLON,JLEV) &
     & + YDMF_PHYS_BASE_STATE%V(JLON,JLEV)*PTENDV_ZDEC(JLON,JLEV) + 0.5_JPRB * PDT * &
     & ( (PTENDU_ZDEC(JLON,JLEV))**2 + (PTENDV_ZDEC(JLON,JLEV))**2 ) )
    ZRRGDELP(JLON,JLEV)=ZFLAG*ZRRG*YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)
    ZDTT1(JLON,JLEV) = ZFLAG * ( ZCPF * ( ZH - PDT*ZDEC(JLON,JLEV)&
     & + PDT * PTENDH(JLON,JLEV) ) - YDMF_PHYS_BASE_STATE%T(JLON,JLEV) )
    IF (LPTKE) THEN
      ZDTT1(JLON,JLEV) = ZDTT1(JLON,JLEV)&
       & - ZFLAG*ZCPF*PDT*PTENDTKE(JLON,JLEV)
    ENDIF
  ENDDO
ENDDO
...
DO JLEV=1,YDCPG_OPTS%KFLEVG
  DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    PFDIS(JLON,JLEV)= PFDIS(JLON,JLEV-1)&
     & + ZRRGDELP(JLON,JLEV)*ZDEC(JLON,JLEV)
  ENDDO
  IF (LPTKE) THEN
    DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      PFDIS(JLON,JLEV)= PFDIS(JLON,JLEV)&
       & + ZRRGDELP(JLON,JLEV)*PTENDTKE(JLON,JLEV)
    ENDDO
  ENDIF
ENDDO
