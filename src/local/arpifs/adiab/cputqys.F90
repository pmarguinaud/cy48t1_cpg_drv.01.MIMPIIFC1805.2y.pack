...
ASSOCIATE(  TSPHY=>YDPHY2%TSPHY, LFORCENL=>YDSTOPH%LFORCENL, NFORCESTART=>YDSTOPH%NFORCESTART, NFORCEEND=>YDSTOPH%NFORCEEND&
& )
!  ----------------------------------------------------------------------
...
DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
  YDMF_PHYS%OUT%FDIS(JROF,0)=0.0_JPRB
ENDDO
...
DO JLEV = 1, YDCPG_OPTS%KFLEVG
  DO JROF = YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA
  ZTDCP = (YDCST%RCPV - YDCST%RCPD) * PTENDQ(JROF,JLEV)
  ZCPF  = 1.0_JPRB / ( YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(JROF,JLEV) + PDT * ZTDCP )
  ZDEC = PDT * ( YDMF_PHYS_BASE_STATE%U(JROF,JLEV)*YDMF_PHYS%OUT%TENDU(JROF,JLEV) +&
   & YDMF_PHYS_BASE_STATE%V(JROF,JLEV)*YDMF_PHYS%OUT%TENDV(JROF,JLEV) + 0.5_JPRB * PDT * (&
   & (YDMF_PHYS%OUT%TENDU(JROF,JLEV))**2 + (YDMF_PHYS%OUT%TENDV(JROF,JLEV))**2 ))  
  ...
  YDMF_PHYS%OUT%FDIS(JROF,JLEV)= YDMF_PHYS%OUT%FDIS(JROF,JLEV-1)+ ZRDT*ZRRG*YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JROF,JLEV)*ZDEC
  
  ENDDO
ENDDO  


IF (LFORCENL.AND.(NSTEP*(TSPHY/YDCST%RHOUR)>=NFORCESTART).AND.&
               & (NSTEP*(TSPHY/YDCST%RHOUR)<=NFORCEEND)) THEN
  DO JLEV = 1, YDCPG_OPTS%KFLEVG
    DO JROF = YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA
      YDMF_PHYS%OUT%TENDU(JROF,JLEV)=YDMF_PHYS%OUT%TENDU(JROF,JLEV)+&
    &  YDMF_PHYS%FOR%U(JROF,JLEV)
      YDMF_PHYS%OUT%TENDV(JROF,JLEV)=YDMF_PHYS%OUT%TENDV(JROF,JLEV)+&
    &  YDMF_PHYS%FOR%V(JROF,JLEV)
      ZDTT1(JROF,JLEV)=ZDTT1(JROF,JLEV)+&
    &  PDT*YDMF_PHYS%FOR%T(JROF,JLEV)
      PTENDQ(JROF,JLEV)=PTENDQ(JROF,JLEV)+&
    &  YDMF_PHYS%FOR%Q(JROF,JLEV)
    ENDDO
  ENDDO
ENDIF
...
DO JLEV = 1, YDCPG_OPTS%KFLEVG
  DO JROF = YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA
    YDMF_PHYS_NEXT_STATE%U(JROF,JLEV)=YDMF_PHYS_NEXT_STATE%U(JROF,JLEV) + PDT*YDMF_PHYS%OUT%TENDU(JROF,JLEV)
    YDMF_PHYS_NEXT_STATE%V(JROF,JLEV)=YDMF_PHYS_NEXT_STATE%V(JROF,JLEV) + PDT*YDMF_PHYS%OUT%TENDV(JROF,JLEV)
