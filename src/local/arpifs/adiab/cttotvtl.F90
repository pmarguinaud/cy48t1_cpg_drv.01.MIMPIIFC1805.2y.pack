...
!**** *CTTOTVTL* - Computes T from 'TV' (TV defined as RT/Rd)
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('CTTOTVTL',0,ZHOOK_HANDLE)
...
DO JLEV = 1, NFLEVG
  DO JROF = IST, IEND
    ...
    IF (YRDYNA%LDRY_ECMWF) THEN
      PT(JROF,JLEV)=ZR5*PT(JROF,JLEV)/RD
    ELSE
      ZR=(RV-RD)*PQ(JROF,JLEV)
      PT(JROF,JLEV)=(ZR5*PT(JROF,JLEV)&
       & +ZR*PT5(JROF,JLEV))/RD
    ENDIF
  ENDDO
ENDDO
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('CTTOTVTL',1,ZHOOK_HANDLE)
