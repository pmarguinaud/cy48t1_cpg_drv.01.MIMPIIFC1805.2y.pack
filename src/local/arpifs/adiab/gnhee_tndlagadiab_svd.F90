...
IF (KVDVAR==3 .OR. KVDVAR==4 .OR. KVDVAR==5) THEN
  ...
  ELSEIF (KVDVAR==5) THEN
    ...
    CALL GNHEE_TNDLAGADIAB_FORCOROG(LDVEREGINT, YDCST, YDGEOMETRY, KST, KPROF, POROGL, POROGM, &
    & POROGLM, POROGLL, POROGMM, PUH, PVH, PTNDUF, PTNDVF, PEVEL, PRDELP, PLNPR, PALPH, ZATND13)
  
    ! * Compute the equivalent '(p-Pi)/Pi' generated by term 'S V grad(Phi_s)'.
    CALL GNHEE_PDEP_OROG(LDVERTFE, LDVFE_GW, YDGEOMETRY, KST, KPROF, ZATND13, PREF, &
    & ZPART_RESCALED_PDEPF)
