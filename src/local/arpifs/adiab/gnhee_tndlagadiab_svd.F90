...
IF (NVDVAR==3 .OR. NVDVAR==4 .OR. NVDVAR==5) THEN
  ! coded in GNH_TNDLAGADIAB_SVD but not easy to transpose there; to be studied later.
  IF (LVERTFE) CALL ABOR1(' GNHEE_TNDLAGADIAB_SVD 1.1: option not yet coded')
  ...
  CALL GNHEE_LAPL(YDGEOMETRY,KST,KPROF,ZIN,ZINS,PLNPR,PALPH,PREF,PREH,ZOUT)
