...
REAL(KIND=JPRB)   ,INTENT(IN),OPTIONAL,TARGET :: PXYBDER(KPROMA,KFLEV,YYTXYBDER%NDIM)
...
REAL(KIND=JPRB),TARGET,INTENT(OUT)   :: PGWHL(KPROMA,0:KFLEV)
REAL(KIND=JPRB),TARGET,INTENT(OUT)   :: PGWHM(KPROMA,0:KFLEV)
...
REAL(KIND=JPRB),INTENT(IN),OPTIONAL,TARGET :: PLNPRL_DER(KPROMA,KFLEV)
REAL(KIND=JPRB),INTENT(IN),OPTIONAL,TARGET :: PLNPRM_DER(KPROMA,KFLEV)
REAL(KIND=JPRB),INTENT(IN),OPTIONAL,TARGET :: PALPHL_DER(KPROMA,KFLEV)
REAL(KIND=JPRB),INTENT(IN),OPTIONAL,TARGET :: PALPHM_DER(KPROMA,KFLEV)
...
REAL(KIND=JPRB),CONTIGUOUS,POINTER :: ZLNPRL_DER(:,:), ZLNPRM_DER(:,:), ZALPHL_DER (:,:), ZALPHM_DER(:,:)
    ...
    DO JL=1,KFLEV
      DO JROF=KD,KF
        ...
        ZINL(JROF,JL)=YDGEOMETRY%YRVETA%VFE_RDETAH(JL)*(&
         ...
         & - ZRTV*PXYBDER(JROF,JL,YYTXYBDER%M_LNPRL) )
        ZINM(JROF,JL)=YDGEOMETRY%YRVETA%VFE_RDETAH(JL)*(&
         ...
         & - ZRTV*PXYBDER(JROF,JL,YYTXYBDER%M_LNPRM) )
      ENDDO
    ENDDO
  ELSE
    DO JL=1,KFLEV
      ...
      DO JL=1,KFLEV
        DO JROF=KD,KF
          ...
          PGWFL(JROF,JL)=PGWHL(JROF,JL) &
           ...
           & + ZRTV*PXYBDER(JROF,JL,YYTXYBDER%M_ALPHL)
          PGWFM(JROF,JL)=PGWHM(JROF,JL) &
           & +(PRTM(JROF,JL)*PRNHPPI(JROF,JL)*PDVER(JROF,JL) &
           & - ZRTPI*PQCHAM(JROF,JL)*PDVER(JROF,JL) &
           & + ZRTPI*PDVERM(JROF,JL) )*PALPH(JROF,JL) &
           & + ZRTV*PXYBDER(JROF,JL,YYTXYBDER%M_ALPHM)
        ENDDO
      ENDDO
    ELSE
      DO JL=1,KFLEV
