SUBROUTINE GP_KAPPAT(&
 ! --- INPUT --------------------------------------------------
 & YDDYNA, YDCST, YDGEOMETRY, YDDYN,KPROMA,KSTART,KPROF,KFLEV,&
...
REAL(KIND=JPRB) :: ZRTGR(KPROMA,KFLEV),ZRPRE (KPROMA,KFLEV)
...
CALL GPHPRE_EXPL (YDDYNA%LAPRXPK,LVERTFE,YDDYNA%NDLNPR,YDDYNA%RHYDR0,TOPPRES,YDCST,KPROMA,KFLEV,KSTART,KPROF,YDVAB,PRESH=ZPREH,PRESF=ZPREF,&
                & PDELP=ZDELP, PLNPR=ZLNPR, PRDELP=ZRDELP, &
                & PALPH=ZALPH, PRTGR=ZRTGR, PRPRE =ZRPRE , &
                & PRPP =ZRPP )
...
ELSE
  ...
  DO JLEV=1,NLEV_SPONGE
    ...
    DO JROF=KSTART,KPROF
      ...
      ZAUX=ZC*ZRDELP(JROF,JLEV)*ZPREH(JROF,KFLEV)
