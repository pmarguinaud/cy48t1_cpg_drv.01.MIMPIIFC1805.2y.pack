...
!      Dec-2003 K. Yessad  mult/div by GM has moved in GPMPFC_GMVS.
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('GP_SPVAD',0,ZHOOK_HANDLE)
...
ELSE
  IF (LTWOTL) THEN
    LLPRE9=(NCURRENT_ITER > 0 .AND. YRDYNA%LPC_FULL)
  ELSE
    LLPRE9=.TRUE.
  ENDIF
ENDIF
...
DO JROF=KST,KEND
  PRE0(JROF,NFLEVG)=PRE0(JROF,NFLEVG)+PSPT5M(JROF)*PRE0M(JROF)
  PSPT0M(JROF)=PSPT0M(JROF)+PRE5(JROF,NFLEVG)*PRE0M(JROF)
  PRE0M(JROF)=0.0_JPRB
  PRE0(JROF,NFLEVG)=PRE0(JROF,NFLEVG)+PSPT5L(JROF)*PRE0L(JROF)
  PSPT0L(JROF)=PSPT0L(JROF)+PRE5(JROF,NFLEVG)*PRE0L(JROF)
  PRE0L(JROF)=0.0_JPRB
  PSPT0(JROF)=PSPT0(JROF)+PRE5(JROF,NFLEVG)*PRE0(JROF,NFLEVG)
  PRE0(JROF,NFLEVG)=0.0_JPRB
ENDDO
IF (LLPRE9) THEN
  DO JROF=KST,KEND
    PSPT9(JROF)=PSPT9(JROF)+PRE95(JROF,NFLEVG)*PRE9(JROF,NFLEVG)
    PRE9(JROF,NFLEVG)=0.0_JPRB
  ENDDO
ENDIF
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('GP_SPVAD',1,ZHOOK_HANDLE)
END SUBROUTINE GP_SPVAD
