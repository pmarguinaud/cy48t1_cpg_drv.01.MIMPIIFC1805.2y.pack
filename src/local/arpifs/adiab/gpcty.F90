...
ELSEIF (PRESENT (PEVEL) .AND.  PRESENT (PVVEL) .AND.  PRESENT (PPSDIV) .AND. &
     &  PRESENT (PPSDVBC) .AND.  PRESENT (PDIVDP)) THEN
  ZEVEL  (1:,0:) => PEVEL  
  ZVVEL  (1:,0:) => PVVEL  
  ZPSDIV (1:,0:) => PPSDIV 
  ZPSDVBC(1:,0:) => PPSDVBC
  ZDIVDP (1:,0:) => PDIVDP 
ELSE
  CALL ABOR1 ('GPCTY: PCTY OR PEVEL/PVVEL/PPSDIV/PPSDVBC/PDIVDP REQUIRED')
ENDIF
