...
REAL(KIND=JPRB),TARGET :: ZZXYB(KPROMA,KFLEV,YYTXYB%NDIM),ZZPRESF(KPROMA,KFLEV)
...
ELSE
  ...
  IF (ASSOCIATED (ZXYB)) THEN
    ZDELP  => ZXYB (:,:,YYTXYB%M_DELP )
    ZLNPR  => ZXYB (:,:,YYTXYB%M_LNPR )
    ZRDELP => ZXYB (:,:,YYTXYB%M_RDELP)
    ZALPH  => ZXYB (:,:,YYTXYB%M_ALPH )
    ZRTGR  => ZXYB (:,:,YYTXYB%M_RTGR )
    ZRPRE  => ZXYB (:,:,YYTXYB%M_RPRE )
    ZRPP   => ZXYB (:,:,YYTXYB%M_RPP  )
  ELSE
    ZDELP  => PDELP  
    ZLNPR  => PLNPR  
    ZRDELP => PRDELP 
