  ...
  DO JLEV=1,KFLEV
    IF(LLKAP) THEN
      DO JLON=KSTART,KPROF
        ...
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)+ZKAP(JLON,JLEV)*ZPCP_R5
  ...
  DO JLEV=1,KFLEV
    ...
    IF(LLQ .AND. LLQ_THERMACT) THEN
      DO JLON=KSTART,KPROF
        ...
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQ(JLON,JLEV)=PQ(JLON,JLEV)+(RV-RD)*ZR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQL .AND. LLQL_THERMACT) THEN
      DO JLON=KSTART,KPROF
        PQL(JLON,JLEV)=PQL(JLON,JLEV)+(RCW-RCPD)*ZCP(JLON,JLEV)
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQL(JLON,JLEV)=PQL(JLON,JLEV)-RD*ZR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQI .AND. LLQI_THERMACT) THEN
      DO JLON=KSTART,KPROF
        PQI(JLON,JLEV)=PQI(JLON,JLEV)+(RCS-RCPD)*ZCP(JLON,JLEV)
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQI(JLON,JLEV)=PQI(JLON,JLEV)-RD*ZR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQR .AND. LLQR_THERMACT) THEN
      DO JLON=KSTART,KPROF
        PQR(JLON,JLEV)=PQR(JLON,JLEV)+(RCW-RCPD)*ZCP(JLON,JLEV)
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQR(JLON,JLEV)=PQR(JLON,JLEV)-RD*ZR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQS .AND. LLQS_THERMACT) THEN
      DO JLON=KSTART,KPROF
        PQS(JLON,JLEV)=PQS(JLON,JLEV)+(RCS-RCPD)*ZCP(JLON,JLEV)
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQS(JLON,JLEV)=PQS(JLON,JLEV)-RD*ZR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQG .AND. LLQG_THERMACT) THEN
      DO JLON=KSTART,KPROF
        PQG(JLON,JLEV)=PQG(JLON,JLEV)+(RCS-RCPD)*ZCP(JLON,JLEV)
        IF (.NOT. YRDYNA%LDRY_ECMWF) PQG(JLON,JLEV)=PQG(JLON,JLEV)-RD*ZR(JLON,JLEV)
      ENDDO
    ENDIF
