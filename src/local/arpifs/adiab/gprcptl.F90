...
ELSE
  DO JLEV=1,KFLEV
    ...
    IF(LLQ .AND. LLQ_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)+(RV-RD)*PQ(JLON,JLEV)
        ZCP(JLON,JLEV) = ZCP(JLON,JLEV)+(RCPV-RCPD)*PQ(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQL .AND. LLQL_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)-RD*PQL(JLON,JLEV)
        ZCP(JLON,JLEV) = ZCP(JLON,JLEV)+(RCW-RCPD)*PQL(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQI .AND. LLQI_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)-RD*PQI(JLON,JLEV)
        ZCP(JLON,JLEV) = ZCP(JLON,JLEV)+(RCS-RCPD)*PQI(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQR .AND. LLQR_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)-RD*PQR(JLON,JLEV)
        ZCP(JLON,JLEV) = ZCP(JLON,JLEV)+(RCW-RCPD)*PQR(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQS .AND. LLQS_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)-RD*PQS(JLON,JLEV)
        ZCP(JLON,JLEV) = ZCP(JLON,JLEV)+(RCS-RCPD)*PQS(JLON,JLEV)
      ENDDO
    ENDIF
    IF(LLQG .AND. LLQG_THERMACT) THEN
      DO JLON=KSTART,KPROF
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZR(JLON,JLEV)=ZR(JLON,JLEV)-RD*PQG(JLON,JLEV)
    ...
    IF (LLKAP) THEN
      DO JLON=KSTART,KPROF
        ...
        ZPCP_R2=ZPCP_R5*ZPCP_R5
        ZKAP(JLON,JLEV)= -PR5(JLON,JLEV)*ZPCP_R2*ZCP(JLON,JLEV)  
        IF (.NOT. YRDYNA%LDRY_ECMWF) ZKAP(JLON,JLEV)=ZKAP(JLON,JLEV)+ZR(JLON,JLEV)*ZPCP_R5
      ENDDO
    ENDIF
  ENDDO
ENDIF
