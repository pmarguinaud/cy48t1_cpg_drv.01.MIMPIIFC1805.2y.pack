...
DO JLEV=1,KLEV
  DO JLH=KSTART,KEND
    ...
    IF (.NOT. YRDYNA%LDRY_ECMWF) PRT(JLH,JLEV)=PRT(JLH,JLEV)+PR(JLH,JLEV)*PT5(JLH,JLEV)
    IF (LDSPRT) THEN
    ...
    ELSE
      ...
      IF (.NOT.YRDYNA%LDRY_ECMWF) PRTL(JLH,JLEV)=PRTL(JLH,JLEV)+(PRV-PRD)*PT5(JLH,JLEV)*PQL(JLH,JLEV)&
       & + PR(JLH,JLEV)*PT5L(JLH,JLEV)
      PRTM(JLH,JLEV)= (PRV-PRD)*PT(JLH,JLEV)*PQ5M(JLH,JLEV)&
       & + PR5(JLH,JLEV)*PTM(JLH,JLEV)
      IF (.NOT.YRDYNA%LDRY_ECMWF) PRTM(JLH,JLEV)=PRTM(JLH,JLEV)+(PRV-PRD)*PT5(JLH,JLEV)*PQM(JLH,JLEV)&
