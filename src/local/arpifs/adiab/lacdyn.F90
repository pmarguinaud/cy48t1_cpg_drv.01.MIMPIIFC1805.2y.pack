...
IF (LNHEE) THEN
  CALL LANHSI(YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,YRDYNA%LGWADV,YDGSGEOM%RCORI,ZREDIV,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZSPDS0,ZSPDS9,ZLPD0,ZLPD9)
ELSEIF (LNHQE) THEN
  CALL LANHQESI(YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,YRDYNA%LGWADV,YDGSGEOM%RCORI,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZLPD0,ZLPD9)
ELSE
...
IF ((NCURRENT_ITER == 0).OR. (&
 & NCURRENT_ITER > 0 .AND. YRDYNA%LPC_FULL .AND.(.NOT.YRDYNA%LPC_CHEAP) )) THEN
...
IF (LNHDYN.AND.YRDYNA%LRDBBC.AND.(.NOT.YRDYNA%LGWADV)) THEN
  ! * Remarks:
...
IF (YRDYNA%LCOMAD) THEN
  CALL LATTE_STDDIS(YDGEOMETRY,YDGMV,YDPTRSLB2,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,ZDTS2,YRDYNA%LCOMADH,YRDYNA%LCOMADV,PGMV,PB2)
ENDIF
