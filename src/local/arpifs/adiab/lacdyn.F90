SUBROUTINE LACDYN(YDGEOMETRY,YDVARS,YDCPG_BNDS, YDCPG_OPTS, YDCPG_TND, YDCPG_DYN0,YDCPG_DYN9,YDGMV,&
 ! --- INPUT ------------------------------------------------------------------
 & YDMODEL,KST,KPROF,PBETADT,PDT,PSLHDA,PSLHDD0,&
 & KIBL,&
 & PATND,PGFL,&
 ! --- INPUT/OUTPUT -----------------------------------------------------------
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 &  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL), YDDYN=>YDMODEL%YRML_DYN%YRDYN,YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY, &
...
CALL LASURE(YDGEOMETRY,YDMODEL%YRML_PHY_EC%YREPHY,YDDYN,YDMODEL%YRML_DYN%YREDYN,YDMODEL%YRML_PHY_MF%YRPHY,KST,KPROF,PBETADT,PDT, &
 & KIBL,ZDTS2,ZBT,LL2TLFF1,ZBDT,ZREDIV,ZESGP,ZESGM)
...
IF (LNHEE) THEN
  CALL LANHSI(YDGEOMETRY,YDGMV,YGFL,YDDYN,KST,KPROF,LGWADV,YDGSGEOM%RCORI,ZREDIV,PGMV,PGMVS,PGFL,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZSPDS0,ZSPDS9,ZLPD0,ZLPD9)
ELSEIF (LNHQE) THEN
  CALL LANHQESI(YDGEOMETRY,YDGMV,YGFL,YDDYN,KST,KPROF,LGWADV,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZLPD0,ZLPD9)
ELSE
  CALL LASSIE(YDGEOMETRY,YDGMV,YGFL,YDDYN,KST,KPROF,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M)
ENDIF
...
IF ((NCURRENT_ITER == 0).OR. (&
 & NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  ...
  CALL LAVENT(YDGEOMETRY,YDGMV,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,YDMODEL%YRML_PHY_MF%YRPARAR,&
   & KST,KPROF,KIBL,KSETTLOFF,LL2TLFF1, &
...
IF (LMIXETTLS) THEN
  CALL LATTE_NL(YDGEOMETRY,YDGMV,YDMODEL%YRML_DYN,YDMODEL%YRML_GCONF%YRRIP,&
   & KST,KPROF,ZDTS2,ZBDT,PGMV(1,1,YT0%MU),PGMV(1,1,YT0%MV),YDCPG_DYN0%CTY%EVEL,&
...
CALL LATTEX(YDGEOMETRY,YDGMV,YDMODEL%YRML_DIAG%YRLDDH,YDMDDH,YDMODEL%YRML_GCONF,YDMODEL%YRML_DYN,KST,KPROF,ZDTS2,ZBT,ZBDT, &
 & ZESGP,ZESGM,&
...
IF (LNHDYN.AND.LRDBBC.AND.(.NOT.LGWADV)) THEN
  ...
  !     in routine LAPINEB.
  CALL LATTE_BBC(YDGEOMETRY,YDGMV,YDMODEL%YRML_DYN,KST,KPROF,ZDTS2,ZESGP,ZESGM,YDCPG_DYN0%DBBC,YDCPG_DYN0%RDPHI,&
...
CALL LATTES(YDGEOMETRY,YDGMV,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,KST,KPROF,ZDTS2,ZBDT,ZESGP,ZESGM,&
 & KIBL,YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,&
...
IF (LSLHD) THEN
  CALL LATTE_KAPPA(YDGEOMETRY,YDGMV,YDDYN,YDPTRSLB2,KST,KPROF,ZDTS2,PSLHDA,PSLHDD0,PGMV,PGMVS,&
...
IF (LCOMAD) THEN
  CALL LATTE_STDDIS(YDGEOMETRY,YDGMV,YDPTRSLB2,KST,KPROF,ZDTS2,LCOMADH,LCOMADV,PGMV,PB2)
ENDIF
...
IF ((NCURRENT_ITER == 0).OR. (&
 & NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  CALL LAVABO(YDGEOMETRY,YGFL,YDDYN,YDPTRSLB1,KST,KPROF,LL2TLFF1,LSLPHY,PB1)
ENDIF
...
IF (LGWADV) THEN
  ...
  LLGWADV=.FALSE.
  IF (LNHEE) THEN
    CALL LANHSI(YDGEOMETRY,YDGMV,YGFL,YDDYN,KST,KPROF,LLGWADV,YDGSGEOM%RCORI,ZREDIV,PGMV,PGMVS,PGFL,&
     & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
     & ZSPDS0,ZSPDS9,ZLPD0,ZLPD9)
  ELSEIF (LNHQE) THEN
    CALL LANHQESI(YDGEOMETRY,YDGMV,YGFL,YDDYN,KST,KPROF,LLGWADV,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
     & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
     & ZLPD0,ZLPD9)
  ENDIF

  CALL LANHSIB(YDGEOMETRY,YDGMV,YDMODEL%YRML_DYN,KST,KPROF,ZBT,ZBDT,ZESGP,&
