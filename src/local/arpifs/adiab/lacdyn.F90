...
IF (LNHEE) THEN
  CALL LANHSI(YDMODEL%YRML_DYN%YRDYNA,YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,YDMODEL%YRML_DYN%YRDYNA%LGWADV,YDGSGEOM%RCORI,ZREDIV,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZSPDS0,ZSPDS9,ZLPD0,ZLPD9)
ELSEIF (LNHQE) THEN
  CALL LANHQESI(YDMODEL%YRML_DYN%YRDYNA,YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,YDMODEL%YRML_DYN%YRDYNA%LGWADV,YDGSGEOM%RCORI,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
   & ZLPD0,ZLPD9)
ELSE
  CALL LASSIE(YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,YDGSGEOM%RCORI,&
   & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M)
ENDIF
...
IF ((NCURRENT_ITER == 0).OR. (&
 & NCURRENT_ITER > 0 .AND. YDMODEL%YRML_DYN%YRDYNA%LPC_FULL .AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LPC_CHEAP) )) THEN
  ...
  CALL LAVENT(YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1, YDCPG_SL2, YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,YDMODEL%YRML_PHY_MF%YRPARAR,&
   & YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDCPG_BNDS%KBL,KSETTLOFF,LL2TLFF1, &
   & ZDTS2,YDCPG_DYN0%XYB%RDELP,&
   & YDCPG_DYN0%CTY%EVEL,PATND,YDCPG_DYN9%WRL)
...
CALL LATTEX(YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2,YDCPG_BNDS,YDCPG_OPTS,YDMODEL%YRML_DIAG%YRLDDH,YDMDDH,YDMODEL%YRML_GCONF,YDMODEL%YRML_DYN,ZDTS2,ZBT,ZBDT, &
 & ZESGP,ZESGM,&
...
IF (LNHDYN.AND.YDMODEL%YRML_DYN%YRDYNA%LRDBBC.AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LGWADV)) THEN
  ! * Remarks:
...
CALL LATTES(YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,ZDTS2,ZBDT,ZESGP,ZESGM,&
 & YDCPG_BNDS%KBL,YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,&
 & ZSDIV0,ZSDIV9,YDCPG_DYN0%CTY%PSDVBC,YDCPG_DYN0%PRE,ZMIXNL)
...
IF (YDMODEL%YRML_DYN%YRDYNA%LSLHD) THEN
  CALL LATTE_KAPPA(YDMODEL%YRML_DYN%YRDYNA,YDCST, YDGEOMETRY,YDGMV,YDDYN,YDPTRSLB2,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,ZDTS2,PSLHDA,PSLHDD0,PGMV,PGMVS,&
...
IF (YDMODEL%YRML_DYN%YRDYNA%LCOMAD) THEN
  CALL LATTE_STDDIS(YDGEOMETRY,YDGMV,YDPTRSLB2,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,ZDTS2,YDMODEL%YRML_DYN%YRDYNA%LCOMADH,YDMODEL%YRML_DYN%YRDYNA%LCOMADV,PGMV,PB2)
ENDIF
...
IF ((NCURRENT_ITER == 0).OR. (&
 & NCURRENT_ITER > 0 .AND. YDMODEL%YRML_DYN%YRDYNA%LPC_FULL .AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LPC_CHEAP) )) THEN
  CALL LAVABO(YDMODEL%YRML_DYN%YRDYNA,YDGEOMETRY,YDCPG_SL1,YGFL,YDDYN,YDPTRSLB1,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LL2TLFF1,LSLPHY,PB1)
ENDIF
...
IF (YDMODEL%YRML_DYN%YRDYNA%LGWADV) THEN
  ...
  LLGWADV=.FALSE.
  IF (LNHEE) THEN
    CALL LANHSI(YDMODEL%YRML_DYN%YRDYNA,YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,LLGWADV,YDGSGEOM%RCORI,ZREDIV,&
     & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
     & ZSPDS0,ZSPDS9,ZLPD0,ZLPD9)
  ELSEIF (LNHQE) THEN
    CALL LANHQESI(YDMODEL%YRML_DYN%YRDYNA,YDMODEL%YRCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,LLGWADV,YDGSGEOM%RCORI,&
     & ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,&
     & ZLPD0,ZLPD9)
  ENDIF

  CALL LANHSIB(YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2,YDMODEL%YRML_DYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,ZBT,ZBDT,ZESGP,&
