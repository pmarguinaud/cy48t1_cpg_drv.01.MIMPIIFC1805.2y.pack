...
!**** *LACDYNTL* Semi-Lagrangian scheme.  (tangent-linear version)
...
!        INPUT:      ('5' for corresponding trajectory variables)
...
!          PWRL95  - vertical velocity from t-dt (trajectory only)

!        INPUT/OUTPUT:   ('5' for corresponding trajectory variables)
...
!          PGFL5 to PB15: trajectory for PGFL to PB1.
...
!   F. Vana  13-Feb-2014: Two more arguments to LATTE_KAPPATL
...
! * computed in LASSIETL or LANHSITL:
...
#include "lassietl.intfb.h"
#include "lasure.intfb.h"
#include "lattestl.intfb.h"
#include "latte_kappatl.intfb.h"
#include "lattextl.intfb.h"
#include "lavabotl.intfb.h"
#include "laventtl.intfb.h"

!     ------------------------------------------------------------------

IF (LHOOK) CALL DR_HOOK('LACDYNTL',0,ZHOOK_HANDLE)
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDDIMV=>YDGEOMETRY%YRDIMV,YDGEM=>YDGEOMETRY%YRGEM, &
 &  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL), &
 & YDPTRSLB1=>YDMODEL%YRML_DYN%YRPTRSLB1,YDPTRSLB2=>YDMODEL%YRML_DYN%YRPTRSLB2,YGFL=>YDMODEL%YRML_GCONF%YGFL, &
 & YDPTRSLB15=>YDMODEL%YRML_DYN%YRPTRSLB15)
...
CALL LASURE(YDGEOMETRY,YDMODEL%YRML_PHY_EC%YREPHY,YDMODEL%YRML_DYN%YRDYN,YDMODEL%YRML_DYN%YREDYN,YDMODEL%YRML_PHY_MF%YRPHY,KST, &
 & KPROF,PBETADT,PDT,YDGSGEOM%GMAPPA,YDGSGEOM%GM,&
...
CALL LASSIETL(YDGEOMETRY,YDGMV,YDGMV5,YGFL,YDMODEL%YRML_DYN%YRDYN,KST,KPROF,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
...
CALL LAVENTTL(YDGEOMETRY,YDGMV,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,KST,KPROF,KIBL,LL2TLFF1,ZDTS2,&
...
CALL LATTEXTL(YDGEOMETRY,YDGMV,YDMODEL%YRML_DIAG%YRLDDH,YDMODEL%YRML_GCONF,YDMODEL%YRML_DYN,KST,KPROF,ZDTS2,ZBT,ZBDT,ZESGP,ZESGM,&
 & KIBL,POROGL,POROGM,&
...
CALL LATTESTL(YDGEOMETRY,YDGMV,YDGMV5,YDMODEL%YRML_GCONF%YRRIP,YDMODEL%YRML_DYN,KST,KPROF,ZDTS2,ZBDT,ZESGP,ZESGM,&
...
IF (YDMODEL%YRML_DYN%YRDYNA%LSLHD) THEN
  CALL LATTE_KAPPATL(YDGEOMETRY,YDGMV,YDGMV5,YDMODEL%YRML_DYN%YRDYN,YDPTRSLB2,KST,KPROF,ZDTS2,PSLHDA,PSLHDD0,PGMV,PGMVS,PGMV5, &
 & PGMV5S,PB2)
ENDIF

!     ------------------------------------------------------------------

!*       7.    UPPER AND LOWER LATERAL BOUNDARIES CONDITIONS.
!              ----------------------------------------------

! ky: top and bottom values of PB15 are currently filled there, but
! for ntrsltype=2 this is probably better to fill them in CPG_DYN_TL
! just after the call to GET_TRAJ_SLAG.
CALL LAVABOTL(YDGEOMETRY,YGFL,YDMODEL%YRML_DYN,KST,KPROF,LL2TLFF1,PB15,PB1)

!     ------------------------------------------------------------------

END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('LACDYNTL',1,ZHOOK_HANDLE)
