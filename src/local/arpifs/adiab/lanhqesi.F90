SUBROUTINE LANHQESI(YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDGMV,YDGFL,YDDYN,LDGWADV,PRCORI,PGMV,PGMVS,&
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  IF (.NOT.LNHQE_SIHYD) THEN
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PGAGT0L(JROF,JLEV)=PGAGT0L(JROF,JLEV)+YDCST%RD*SITR*PGMV(JROF,JLEV,YT0%MSPDL)
        PGAGT0M(JROF,JLEV)=PGAGT0M(JROF,JLEV)+YDCST%RD*SITR*PGMV(JROF,JLEV,YT0%MSPDM)
      ENDDO
    ENDDO
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    IF (.NOT.LNHQE_SIHYD) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PGAGT9L(JROF,JLEV)=PGAGT9L(JROF,JLEV)+YDCST%RD*SITR*PGMV(JROF,JLEV,YT9%MSPDL)
          PGAGT9M(JROF,JLEV)=PGAGT9M(JROF,JLEV)+YDCST%RD*SITR*PGMV(JROF,JLEV,YT9%MSPDM)
        ENDDO
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
