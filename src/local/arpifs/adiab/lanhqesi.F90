...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  IF (.NOT.YDDYNA%LNHQE_SIHYD) THEN
    ...
    CALL SILKAP(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_LAPL_BC, YDCST,YDGEOMETRY,YDDYN,YDDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T0,PLPD0,PMULFAC=ZF)
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PLPD0  (JROF,JLEV)=0.0_JPRB
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    IF (.NOT.YDDYNA%LNHQE_SIHYD) THEN
      ...
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
      CALL SILKAP(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_LAPL_BC, YDCST,YDGEOMETRY,YDDYN,YDDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T9,PLPD9(YDCPG_BNDS%KIDIA,1),PMULFAC=ZF)
    ELSE
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PLPD9  (JROF,JLEV)=0.0_JPRB
