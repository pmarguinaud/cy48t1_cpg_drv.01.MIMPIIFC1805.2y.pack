SUBROUTINE LANHQESI(YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,LDGWADV,PRCORI,&
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  IF (.NOT.YRDYNA%LNHQE_SIHYD) THEN
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ...
    CALL SILKAP(YDCST,YDGEOMETRY,YDDYN,YRDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T0,PLPD0,PMULFAC=ZF)
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PLPD0  (JROF,JLEV)=0.0_JPRB
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    IF (.NOT.YRDYNA%LNHQE_SIHYD) THEN
      ...
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
      CALL SILKAP(YDCST,YDGEOMETRY,YDDYN,YRDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T9,PLPD9(YDCPG_BNDS%KIDIA,1),PMULFAC=ZF)
    ELSE
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PLPD9  (JROF,JLEV)=0.0_JPRB
