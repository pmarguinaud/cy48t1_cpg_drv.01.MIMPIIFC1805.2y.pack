SUBROUTINE LANHQESI(YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDGMV,YDGFL,YDDYN,KST,KPROF,LDGWADV,PRCORI,PGMV,PGMVS,PGFL,&
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  CALL SITNU(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT0%MDIV),PTOD0(KST,1),PSDIV0(KST),IPROFS)
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0L(KST,1),PGMV(KST,1,YT0%MTL),PGMVS(KST,YT0%MSPL),IPROFS,NFLEVG)
  CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0M(KST,1),PGMV(KST,1,YT0%MTM),PGMVS(KST,YT0%MSPM),IPROFS,NFLEVG)
  ...
  IF (.NOT.LNHQE_SIHYD) THEN
    ...
    CALL SILKAP(YDGEOMETRY,YDDYN,LNHQE_C2,NPROMA,1,IPROFS,PGMV(KST,1,YT0%MSPD),PLPD0(KST,1),PMULFAC=ZF)
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        PLPD0  (JROF,JLEV)=0.0_JPRB
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    ! - Computation of Nu*D (SI term for continuity equation) and Tau*D (SI term for temperature equation).
    CALL SITNU(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT9%MDIV),PTOD9(KST,1),PSDIV9(KST),IPROFS)
    ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
    CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9L(KST,1),PGMV(KST,1,YT9%MTL),PGMVS(KST,YT9%MSPL),IPROFS,NFLEVG)
    CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9M(KST,1),PGMV(KST,1,YT9%MTM),PGMVS(KST,YT9%MSPM),IPROFS,NFLEVG)
    ! - Add NH contributions to (PGAGT0L,PGAGT0M).
    IF (.NOT.LNHQE_SIHYD) THEN
      DO JLEV=1,NFLEVG
        DO JROF=KST,KPROF
      ...
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
      CALL SILKAP(YDGEOMETRY,YDDYN,LNHQE_C2,NPROMA,1,IPROFS,PGMV(KST,1,YT9%MSPD),PLPD9(KST,1),PMULFAC=ZF)
    ELSE
      DO JLEV=1,NFLEVG
        DO JROF=KST,KPROF
          PLPD9  (JROF,JLEV)=0.0_JPRB
