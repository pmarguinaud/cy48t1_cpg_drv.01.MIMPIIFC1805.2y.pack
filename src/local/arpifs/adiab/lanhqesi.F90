SUBROUTINE LANHQESI(YDDYNA, YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDDYN,LDGWADV,PRCORI,&
...
ZF=YDCST%RG*YDCST%RG/(YDCST%RD*SITR)
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  CALL SITNU(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%DIV%T0,PTOD0(YDCPG_BNDS%KIDIA,1),PSDIV0(YDCPG_BNDS%KIDIA),IPROFS)
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0L(YDCPG_BNDS%KIDIA,1),YDVARS%T%DL,YDVARS%SP%DL,IPROFS,NFLEVG)
  CALL SIGAM(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0M(YDCPG_BNDS%KIDIA,1),YDVARS%T%DM,YDVARS%SP%DM,IPROFS,NFLEVG)
  ! - Add NH contributions to (PGAGT0L,PGAGT0M).
  IF (.NOT.YDDYNA%LNHQE_SIHYD) THEN
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PGAGT0L(JROF,JLEV)=PGAGT0L(JROF,JLEV)+YDCST%RD*SITR*YDVARS%SPD%DL(JROF,JLEV)
        PGAGT0M(JROF,JLEV)=PGAGT0M(JROF,JLEV)+YDCST%RD*SITR*YDVARS%SPD%DM(JROF,JLEV)
      ENDDO
    ENDDO
    ! - Computation of linear terms for vertical divergence variable.
    CALL SILKAP(YDCST,YDGEOMETRY,YDDYN,YDDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T0,PLPD0,PMULFAC=ZF)
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PLPD0  (JROF,JLEV)=0.0_JPRB
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    ! - Computation of Nu*D (SI term for continuity equation) and Tau*D (SI term for temperature equation).
    CALL SITNU(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%DIV%T9,PTOD9(YDCPG_BNDS%KIDIA,1),PSDIV9(YDCPG_BNDS%KIDIA),IPROFS)
    ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
    CALL SIGAM(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9L(YDCPG_BNDS%KIDIA,1),YDVARS%T%DL9,YDVARS%SP%DL9,IPROFS,NFLEVG)
    CALL SIGAM(YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9M(YDCPG_BNDS%KIDIA,1),YDVARS%T%DM9,YDVARS%SP%DM9,IPROFS,NFLEVG)
    ! - Add NH contributions to (PGAGT0L,PGAGT0M).
    IF (.NOT.YDDYNA%LNHQE_SIHYD) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PGAGT9L(JROF,JLEV)=PGAGT9L(JROF,JLEV)+YDCST%RD*SITR*YDVARS%SPD%DL9(JROF,JLEV)
          PGAGT9M(JROF,JLEV)=PGAGT9M(JROF,JLEV)+YDCST%RD*SITR*YDVARS%SPD%DM9(JROF,JLEV)
        ENDDO
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
      CALL SILKAP(YDCST,YDGEOMETRY,YDDYN,YDDYNA%LNHQE_C2,NPROMA,1,IPROFS,YDVARS%SPD%T9,PLPD9(YDCPG_BNDS%KIDIA,1),PMULFAC=ZF)
    ELSE
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PLPD9  (JROF,JLEV)=0.0_JPRB
