...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0L(YDCPG_BNDS%KIDIA,1),PGMV(YDCPG_BNDS%KIDIA,1,YT0%MTL),PGMVS(YDCPG_BNDS%KIDIA,YT0%MSPL),IPROFS,NFLEVG)
  CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0M(YDCPG_BNDS%KIDIA,1),PGMV(YDCPG_BNDS%KIDIA,1,YT0%MTM),PGMVS(YDCPG_BNDS%KIDIA,YT0%MSPM),IPROFS,NFLEVG)
  ...
  IF (.NOT.LNHQE_SIHYD) THEN
    ...
    CALL SILKAP(YDGEOMETRY,YDDYN,LNHQE_C2,NPROMA,1,IPROFS,PGMV(YDCPG_BNDS%KIDIA,1,YT0%MSPD),PLPD0(YDCPG_BNDS%KIDIA,1),PMULFAC=ZF)
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        PLPD0  (JROF,JLEV)=0.0_JPRB
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
    CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9L(YDCPG_BNDS%KIDIA,1),PGMV(YDCPG_BNDS%KIDIA,1,YT9%MTL),PGMVS(YDCPG_BNDS%KIDIA,YT9%MSPL),IPROFS,NFLEVG)
    CALL SIGAM(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9M(YDCPG_BNDS%KIDIA,1),PGMV(YDCPG_BNDS%KIDIA,1,YT9%MTM),PGMVS(YDCPG_BNDS%KIDIA,YT9%MSPM),IPROFS,NFLEVG)
    ! - Add NH contributions to (PGAGT0L,PGAGT0M).
    IF (.NOT.LNHQE_SIHYD) THEN
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      ...
      ENDDO
      ! - Computation of linear terms for vertical divergence variable.
      CALL SILKAP(YDGEOMETRY,YDDYN,LNHQE_C2,NPROMA,1,IPROFS,PGMV(YDCPG_BNDS%KIDIA,1,YT9%MSPD),PLPD9(YDCPG_BNDS%KIDIA,1),PMULFAC=ZF)
    ELSE
      DO JLEV=1,NFLEVG
        DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          PLPD9  (JROF,JLEV)=0.0_JPRB
