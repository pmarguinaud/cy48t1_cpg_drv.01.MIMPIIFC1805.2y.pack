SUBROUTINE LANHSI(YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDGMV,YDGFL,YDDYN,KST,KPROF,LDGWADV,PRCORI,PREDIV,PGMV,PGMVS,PGFL,&
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  DO JLEV=1,NFLEVG
    ...
    ENDDO
  ENDDO
  CALL SIPTP(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT0%MDIV),ZSVDT(KST,1),&
   & PSPDS0(KST,1),PTOD0(KST,1),PSDIV0(KST),IPROFS)
  ! - Computation of SI terms for momentum equation.
  CALL SIDD(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0L(KST,1),ZDUM(KST,1),&
   & PGMV(KST,1,YT0%MSPDL),PGMV(KST,1,YT0%MTL),PGMVS(KST,YT0%MSPL),IPROFS)
  CALL SIDD(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0M(KST,1),ZDUM(KST,1),&
   & PGMV(KST,1,YT0%MSPDM),PGMV(KST,1,YT0%MTM),PGMVS(KST,YT0%MSPM),IPROFS)
  ! - Computation of SI terms for rescaled "pseudo" vertical divergence eqn.
  CALL SISEVE(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT0%MSPD),PLPD0(KST,1),IPROFS)
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      PLPD0(JROF,JLEV)=(RG*RG/(RD*SITR))*PLPD0(JROF,JLEV)
    ENDDO
  ENDDO
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    DO JLEV=1,NFLEVG
      ...
      ENDDO
    ENDDO
    CALL SIPTP(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT9%MDIV),ZSVDT(KST,1),&
     & PSPDS9(KST,1),PTOD9(KST,1),PSDIV9(KST),IPROFS)
    ! - Computation of SI terms for momentum equation.
    CALL SIDD(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9L(KST,1),ZDUM(KST,1),&
     & PGMV(KST,1,YT9%MSPDL),PGMV(KST,1,YT9%MTL),PGMVS(KST,YT9%MSPL),IPROFS)
    CALL SIDD(YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9M(KST,1),ZDUM(KST,1),&
     & PGMV(KST,1,YT9%MSPDM),PGMV(KST,1,YT9%MTM),PGMVS(KST,YT9%MSPM),IPROFS)
    ! - Computation of SI terms for rescaled "pseudo" vertical divergence eqn.
    CALL SISEVE(YDGEOMETRY,YDDYN,NPROMA,1,PGMV(KST,1,YT9%MSPD),PLPD9(KST,1),IPROFS)
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        PLPD9(JROF,JLEV)=(RG*RG/(RD*SITR))*PLPD9(JROF,JLEV)
      ENDDO
    ENDDO
