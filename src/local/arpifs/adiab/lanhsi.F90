SUBROUTINE LANHSI(YDCST,YDGEOMETRY,YDVARS,YDCPG_BNDS,YDCPG_OPTS,YDGMV,YDGFL,YDDYN,LDGWADV,PRCORI,PREDIV,&
...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  CALL SISEVE(YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%SPD%T0,PLPD0(YDCPG_BNDS%KIDIA,1),IPROFS)
  DO JLEV=1,NFLEVG
    DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
IF (.NOT.LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    ! - Computation of SI terms for rescaled "pseudo" vertical divergence eqn.
    CALL SISEVE(YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%SPD%T9,PLPD9(YDCPG_BNDS%KIDIA,1),IPROFS)
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
