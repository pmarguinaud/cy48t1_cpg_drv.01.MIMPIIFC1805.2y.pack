...
ELSEIF (.NOT.LDGWADV) THEN
  ...
  DO JLEV=1,NFLEVG
    ...
    ENDDO
  ENDDO
  CALL SIPTP(YDCPG_OPTS%LVERTFE, YDCST,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%DIV%T0,ZSVDT(YDCPG_BNDS%KIDIA,1),&
  ...
  CALL SIDD(YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, YDDYNA,YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0L(YDCPG_BNDS%KIDIA,1),ZDUM(YDCPG_BNDS%KIDIA,1),&
   & YDVARS%SPD%DL,YDVARS%T%DL,YDVARS%SP%DL,IPROFS)
  CALL SIDD(YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, YDDYNA,YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT0M(YDCPG_BNDS%KIDIA,1),ZDUM(YDCPG_BNDS%KIDIA,1),&
   & YDVARS%SPD%DM,YDVARS%T%DM,YDVARS%SP%DM,IPROFS)
  ! - Computation of SI terms for rescaled "pseudo" vertical divergence eqn.
  CALL SISEVE(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_LAPL_BC, YDDYNA,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%SPD%T0,PLPD0(YDCPG_BNDS%KIDIA,1),IPROFS)
  DO JLEV=1,NFLEVG
    DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
IF (.NOT.YDCPG_OPTS%LTWOTL) THEN
  ...
  ELSEIF (.NOT.LDGWADV) THEN
    ...
    DO JLEV=1,NFLEVG
      ...
      ENDDO
    ENDDO
    CALL SIPTP(YDCPG_OPTS%LVERTFE, YDCST,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%DIV%T9,ZSVDT(YDCPG_BNDS%KIDIA,1),&
     & PSPDS9(YDCPG_BNDS%KIDIA,1),PTOD9(YDCPG_BNDS%KIDIA,1),PSDIV9(YDCPG_BNDS%KIDIA),IPROFS)
    ! - Computation of SI terms for momentum equation.
    CALL SIDD(YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, YDDYNA,YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9L(YDCPG_BNDS%KIDIA,1),ZDUM(YDCPG_BNDS%KIDIA,1),&
     & YDVARS%SPD%DL9,YDVARS%T%DL9,YDVARS%SP%DL9,IPROFS)
    CALL SIDD(YDCPG_OPTS%LVFE_LAPL_BC, YDCPG_OPTS%LVERTFE, YDDYNA,YDCST,YDGEOMETRY,YDDYN,NPROMA,1,PGAGT9M(YDCPG_BNDS%KIDIA,1),ZDUM(YDCPG_BNDS%KIDIA,1),&
     & YDVARS%SPD%DM9,YDVARS%T%DM9,YDVARS%SP%DM9,IPROFS)
    ! - Computation of SI terms for rescaled "pseudo" vertical divergence eqn.
    CALL SISEVE(YDCPG_OPTS%LVERTFE, YDCPG_OPTS%LVFE_LAPL_BC, YDDYNA,YDGEOMETRY,YDDYN,NPROMA,1,YDVARS%SPD%T9,PLPD9(YDCPG_BNDS%KIDIA,1),IPROFS)
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
