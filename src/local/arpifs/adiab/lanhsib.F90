...
IF (XIDT > 0.0_JPRB)THEN
  ...
  ZBDT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=(1.0_JPRB+XIDT)*PBDT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
ELSE
  ZBT=PBT
  ZBDT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=PBDT(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
ENDIF
...
IF( YDML_DYN%YRDYNA%NVDVAR == 4 .OR. YDML_DYN%YRDYNA%NVDVAR == 5 )THEN
  IF (YDML_DYN%YRDYNA%ND4SYS==1 .OR. (YDML_DYN%YRDYNA%ND4SYS==2 .AND. NCURRENT_ITER==0)) THEN
    DO JLEV=1,NFLEVG
      DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      ...
      ENDDO
    ENDDO
  ELSEIF (YDML_DYN%YRDYNA%ND4SYS==2 .AND. NCURRENT_ITER>0) THEN
    DO JLEV=1,NFLEVG
      DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        YDCPG_SL2%VDSI(JROF,JLEV)=YDCPG_SL2%VDSI(JROF,JLEV)&
...
IF (YDCPG_OPTS%NSTEP > 0 .AND. NCURRENT_ITER==0) THEN
  ...
  IF (YDML_DYN%YRDYNA%LSLINLC2) THEN
    ...
    ENDDO
  ENDIF
  IF (YDML_DYN%YRDYNA%LSLINL) THEN
    DO JLEV = 1,NFLEVG
      DO JROF = YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZMIXNL = PMIXNL(JROF, JLEV)
