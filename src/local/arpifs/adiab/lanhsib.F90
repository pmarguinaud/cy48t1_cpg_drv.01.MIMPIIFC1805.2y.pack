...
IF( YRDYNA%NVDVAR == 4 .OR. YRDYNA%NVDVAR == 5 )THEN
  IF (YRDYNA%ND4SYS==1 .OR. (YRDYNA%ND4SYS==2 .AND. NCURRENT_ITER==0)) THEN
    DO JLEV=1,NFLEVG
      DO JROF = KST,KPROF
      ...
      ENDDO
    ENDDO
  ELSEIF (YRDYNA%ND4SYS==2 .AND. NCURRENT_ITER>0) THEN
    DO JLEV=1,NFLEVG
      DO JROF = KST,KPROF
        YDCPG_SL2%VDSI(JROF,JLEV)=YDCPG_SL2%VDSI(JROF,JLEV)&
...
IF (YRDYNA%LSLINLC2 .AND. NCURRENT_ITER==0) THEN
  DO JLEV = 1,NFLEVG
    DO JROF = KST,KPROF
  ...
  ENDDO
ENDIF
IF (YRDYNA%LSLINL .AND. NCURRENT_ITER==0) THEN
  DO JLEV = 1,NFLEVG
    DO JROF = KST,KPROF
