...
ELSE
  ...
  IF( YRDYNA%LSLDIA .AND. ( YRDYNA%LPC_CHEAP.OR.(NCURRENT_ITER == NSITER) )) THEN
...
CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,ISTALAT,LFINDVSEP,YRDYNA%LSLHD,YRDYNA%LSLHDQUAD,LLINTV,&
...
IF (LNHDYN.AND.YRDYNA%LGWADV.AND.(.NOT.LVFE_GW)) THEN
