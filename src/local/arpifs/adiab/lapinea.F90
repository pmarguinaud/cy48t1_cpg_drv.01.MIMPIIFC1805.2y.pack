...
ELSE
  ...
  IF( YDML_DYN%YRDYNA%LSLDIA .AND. ( YDML_DYN%YRDYNA%LPC_CHEAP.OR.(NCURRENT_ITER == NSITER) )) THEN
...
CALL LARCINA(YDGEOMETRY,YDML_DYN,KST,KPROF,YDSL,IHVI,ISTALAT,LFINDVSEP,YDML_DYN%YRDYNA%LSLHD,YDML_DYN%YRDYNA%LSLHDQUAD,LLINTV,&
...
IF (LNHDYN.AND.YDML_DYN%YRDYNA%LGWADV.AND.(.NOT.LVFE_GW)) THEN
