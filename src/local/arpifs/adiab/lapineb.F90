...
LLUPDATE_NHX=YRDYNA%LNHX.AND.(YRDYNA%NVDVAR == 4 .OR. YRDYNA%NVDVAR == 5).AND.(.NOT.YRDYNA%LGWADV.OR.(YRDYNA%ND4SYS==2.AND.NCURRENT_ITER<NSITER))
...
IF(YRDYNA%LPC_CHEAP.AND.YRDYNA%LSETTLS) THEN
  ZGMV9_NL(:,:,:)=0.0_JPRB
ENDIF
...
IF (LNHDYN.AND.YRDYNA%LGWADV.AND.YRDYNA%LSLINL.AND.YRDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
IF ( YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
...
IF (YRDYNA%LPC_CHEAP.AND.YRDYNA%LSETTLS.AND.(NCURRENT_ITER==0)) THEN
...
ELSEIF (NVLAG == 3) THEN
  ...
  LLSLINLC2=YRDYNA%LSLINLC2.AND.YRDYNA%NGWADVSI==1.AND.(NCURRENT_ITER == 0)
...
ELSE
  DO JLEV=1,NFLEVG
    ...
    ENDDO
  ENDDO
  IF (YRDYNA%LSLINL.AND.YRDYNA%NGWADVSI==1.AND.NCURRENT_ITER==0) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        ZPP=PCCO(JROF,JLEV,YDTCCO%M_RQX)*YDGSGEOM%GNORDM(JROF)-PCCO(JROF,JLEV,YDTCCO%M_RQY)*YDGSGEOM%GNORDL(JROF)
        ZQQ=PCCO(JROF,JLEV,YDTCCO%M_RQY)*YDGSGEOM%GNORDM(JROF)+PCCO(JROF,JLEV,YDTCCO%M_RQX)*YDGSGEOM%GNORDL(JROF)
      ...
      ENDDO
    ENDDO
  ELSEIF (YRDYNA%LSLINL.AND.YRDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
IF ( .NOT. (YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
  ...
  IF ( LLCOMPUTE_OMVAK_O .AND. (.NOT.L2TLFF)&
   & .AND. YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
...
IF ( L2TLFF .AND. .NOT.(YRDYNA%LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
...
IF(YRDYNA%LGWADV.OR.YRDYNA%LRDBBC) THEN
...
IF(YRDYNA%LGWADV) THEN
  ...
  IF (YRDYNA%NVDVAR==4 .OR. YRDYNA%NVDVAR==5) THEN
    IVDVAR=3
  ELSE
    IVDVAR=YRDYNA%NVDVAR
  ENDIF

  ALLOCATE (ZSVDT1(NPROMA,NFLEVG))
  IF (YRDYNA%NVDVAR==5) ALLOCATE (ZNHYT1(NPROMA,0:NFLEVG))
  ...
  IF (LVERTFE.AND.LVFE_GW) THEN
    ...
    IF (YRDYNA%NVDVAR==3 .OR. YRDYNA%NVDVAR==4) THEN
...
IF (YRDYNA%LRDBBC.AND.(YRDYNA%NVDVAR==3 .OR. YRDYNA%NVDVAR==4)) THEN
...
IF (LNHDYN.AND.YRDYNA%LGWADV.AND.YRDYNA%NGWADVSI==2.AND.NCURRENT_ITER==0) THEN

  ! * Continuity equation:
  IF (NVLAG==3.AND.YRDYNA%LSLINLC2) THEN
    IF (LVERTFE) THEN
  ...
  IF (YRDYNA%LSLINL.AND.LNHEE) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
