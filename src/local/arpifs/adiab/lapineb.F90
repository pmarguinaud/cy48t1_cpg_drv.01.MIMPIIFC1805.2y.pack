...
USE YOMDYNA      , ONLY : LGWADV, NGWADVSI, LRDBBC, NVDVAR, LNHX,&
 &                        ND4SYS, LSLDIA, LPC_CHEAP, LSLINL, LSLINLC2, L_RDRY_VD,&
 &                        LSETTLS
...
LLUPDATE_NHX=LNHX.AND.(NVDVAR == 4 .OR. NVDVAR == 5).AND.(.NOT.LGWADV.OR.(ND4SYS==2.AND.NCURRENT_ITER<NSITER))
...
IF (LNHDYN.AND.LGWADV.AND.LSLINL.AND.NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
IF ( LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
...
IF (LPC_CHEAP.AND.LSETTLS.AND.(NCURRENT_ITER==0)) THEN
...
ELSEIF (NVLAG == 3) THEN
  ...
  LLSLINLC2=LSLINLC2.AND.NGWADVSI==1.AND.(NCURRENT_ITER == 0)
...
ELSE
  DO JLEV=1,NFLEVG
    ...
    ENDDO
  ENDDO
  IF (LSLINL.AND.NGWADVSI==1.AND.NCURRENT_ITER==0) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        ZPP=PCCO(JROF,JLEV,YDTCCO%M_RQX)*YDGSGEOM%GNORDM(JROF)-PCCO(JROF,JLEV,YDTCCO%M_RQY)*YDGSGEOM%GNORDL(JROF)
        ZQQ=PCCO(JROF,JLEV,YDTCCO%M_RQY)*YDGSGEOM%GNORDM(JROF)+PCCO(JROF,JLEV,YDTCCO%M_RQX)*YDGSGEOM%GNORDL(JROF)
      ...
      ENDDO
    ENDDO
  ELSEIF (LSLINL.AND.NGWADVSI==2.AND.NCURRENT_ITER==0) THEN
...
IF ( .NOT. (LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
  ...
  IF ( LLCOMPUTE_OMVAK_O .AND. (.NOT.L2TLFF)&
   & .AND. LPC_CHEAP.AND.(NCURRENT_ITER == 0) ) THEN
...
IF ( L2TLFF .AND. .NOT.(LPC_CHEAP.AND.(NCURRENT_ITER > 0)) ) THEN
...
IF(LGWADV) THEN
  ...
  IF (NVDVAR==5) ALLOCATE (ZNHYT1(NPROMA,0:NFLEVG))
...
IF (LRDBBC.AND.(NVDVAR==3 .OR. NVDVAR==4)) THEN
...
IF (LNHDYN.AND.LGWADV.AND.NGWADVSI==2.AND.NCURRENT_ITER==0) THEN

  ! * Continuity equation:
  IF (NVLAG==3.AND.LSLINLC2) THEN
    IF (LVERTFE) THEN
