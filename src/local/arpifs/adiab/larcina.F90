...
ELSEIF (KTIP == 2) THEN
  ...
  IF (YRDYNA%LVSPLIP) THEN
    IWIS=105
  ELSEIF (YRDYNA%LRHSVWENO) THEN
    IWIS=106
    IWENO=3
  ELSE
    IWIS=103+KHVI
...
IF (LDPLANE) THEN
  ...
  CALL ELASCAW(&
   ...
   & LDSLHD,LDSLHDQUAD,YRDYNA%LSLHD_OLD,YRDYNA%L3DTURB,&
   & YRDYNA%LCOMAD,YRDYNA%LCOMADH,YRDYNA%LCOMADV,&
...
ELSE
  ...
  CALL LASCAW(&
   ...
   & NFLSA,KSTABUF,IWIS,IHOR,IWENO,KHVI,&
   & LDSLHD,LDSLHDQUAD,YRDYNA%LSLHD_OLD,YDDYN%LSLHDHEAT,YRDYNA%L3DTURB,&
   & YRDYNA%LCOMAD,YRDYNA%LCOMADH,YRDYNA%LCOMADV,&
