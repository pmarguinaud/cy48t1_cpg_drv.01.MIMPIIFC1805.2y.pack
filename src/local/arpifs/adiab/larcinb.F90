...
USE YOMDYNA                , ONLY : LGWADV, LRDBBC, NVDVAR, LPC_FULL,&
 &                                  LSLHD_W, LSLHD_T, LSLHD_SPD, LSLHD_SVD, ND4SYS, L3DTURB,&
 &                                  LCOMAD_W, LCOMAD_T, LCOMAD_SPD, LCOMAD_SVD, LCOMAD_SP, LSLINL, &
 &                                  LSLINLC2, LPC_CHEAP, LPC_CHEAP2, LSETTLS
...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
 & LL_LIN,LCOMAD_W,LSLHD_W,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF ((NSPLTHOI /= 0).AND.(NWLAG < 4)) THEN
  ...
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
   & LL_LIN,LCOMAD_W,LLHOI2,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF(NWLAG >= 3) THEN
  ...
  IF (LSETTLS .AND. LPC_CHEAP .AND. NCURRENT_ITER == 0) THEN
    IF (LCOMAD_W) THEN
      CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
       & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKH,&
 & LL_LIN,LCOMAD_T,LSLHD_T,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF ((NSPLTHOI /= 0).AND.(NTLAG < 4)) THEN
  ...
  ENDIF
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
   & LL_LIN,LCOMAD_T,LLHOI2,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (LNHEE) THEN
  ...
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,LCOMAD_SPD,LSLHD_SPD,LVWENO_SPD,WENO_ALPHA_SPD,LLMOM,LQMPD,LQMHPD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (LNHDYN) THEN
  ...
  LLMOM=.TRUE.
  LL1=.NOT.LGWADV
  LL2=LGWADV.AND.LVFE_GW
  LL3=LGWADV.AND.(ND4SYS == 2).AND.(NCURRENT_ITER < NSITER)

  ! --- part of the RHS requiring a "high order" interpolation:
  IF (LL1.OR.LL2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,LCOMAD_SVD,LSLHD_SVD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((NSPLTHOI /= 0).AND.(NSVDLAG < 4)) THEN
    ...
    ENDIF
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
     & LL_LIN,LCOMAD_SVD,LLHOI2,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((LL1.OR.LL3).AND.(NVDVAR == 4 .OR. NVDVAR == 5)) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
     & LL_LIN,LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF(NVLAG == 2) THEN
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,LCOMAD_SP,LLNOSLHD,LVWENO_SP,WENO_ALPHA_SP,LLMOM,LQMP,LQMHP,KNOWENO,KL0,PLSCAW,PRSCAW,&
   & PB1(1,MSLB1C9),PCF)
ENDIF
...
IF (LNHDYN.AND.LRDBBC) THEN
  ...
  IF (NSVDLAG == 2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,1,NFLSA,NFLEN,IWNT,&
     & LL_LIN,LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0(1:NPROMA,NFLEVG,0:3),&
