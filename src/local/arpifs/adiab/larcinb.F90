...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
 & LL_LIN,YRDYNA%LCOMAD_W,YRDYNA%LSLHD_W,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF ((NSPLTHOI /= 0).AND.(NWLAG < 4)) THEN
  ...
  IF (NSPLTHOI == 1) THEN
    ...
    IWK=NSLDIMK
  ELSE
    LLHOI2=YRDYNA%LSLHD_W
    IWK=IWKM
  ENDIF
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
   & LL_LIN,YRDYNA%LCOMAD_W,LLHOI2,LVWENO_W,WENO_ALPHA_W,LLMOM,LQMW,LQMHW,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF(NWLAG >= 3) THEN
  ...
  IF (YRDYNA%LSETTLS .AND. YRDYNA%LPC_CHEAP .AND. NCURRENT_ITER == 0) THEN
    IF (YRDYNA%LCOMAD_W) THEN
      CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
       & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKH,&
 & LL_LIN,YRDYNA%LCOMAD_T,YRDYNA%LSLHD_T,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF ((NSPLTHOI /= 0).AND.(NTLAG < 4)) THEN
  ...
  IF (NSPLTHOI == 1) THEN
    ...
    IWK=NSLDIMK
  ELSE
    LLHOI2=YRDYNA%LSLHD_T
    IWK=IWKH
  ENDIF
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
   & LL_LIN,YRDYNA%LCOMAD_T,LLHOI2,LVWENO_T,WENO_ALPHA_T,LLMOM,LQMT,LQMHT,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (LNHEE) THEN
  ...
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,YRDYNA%LCOMAD_SPD,YRDYNA%LSLHD_SPD,LVWENO_SPD,WENO_ALPHA_SPD,LLMOM,LQMPD,LQMHPD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF (LNHDYN) THEN
  ...
  LLMOM=.TRUE.
  LL1=.NOT.YRDYNA%LGWADV
  LL2=YRDYNA%LGWADV.AND.LVFE_GW
  LL3=YRDYNA%LGWADV.AND.(YRDYNA%ND4SYS == 2).AND.(NCURRENT_ITER < NSITER)

  ! --- part of the RHS requiring a "high order" interpolation:
  IF (LL1.OR.LL2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
     & LL_LIN,YRDYNA%LCOMAD_SVD,YRDYNA%LSLHD_SVD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((NSPLTHOI /= 0).AND.(NSVDLAG < 4)) THEN
    ...
    ENDIF
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWK,&
     & LL_LIN,YRDYNA%LCOMAD_SVD,LLHOI2,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
  ...
  IF ((LL1.OR.LL3).AND.(YRDYNA%NVDVAR == 4 .OR. YRDYNA%NVDVAR == 5)) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
     & LL_LIN,YRDYNA%LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0,PLSCAW,PRSCAW,&
...
IF(NVLAG == 2) THEN
  CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWNT,&
   & LL_LIN,YRDYNA%LCOMAD_SP,LLNOSLHD,LVWENO_SP,WENO_ALPHA_SP,LLMOM,LQMP,LQMHP,KNOWENO,KL0,PLSCAW,PRSCAW,&
   & PB1(1,MSLB1C9),PCF)
ENDIF

! --- 3D part of the RHS requiring a trilinear interpolation:
IF(NVLAG == 3) THEN
  IF (YRDYNA%LCOMAD_SP) THEN
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAW(1,1,YDTLSCAW%M_WDLAMAD),PLSCAW(1,1,YDTLSCAW%M_WDLOMAD+1),KL0(1,1,1),PLSCAW(1,1,YDTLSCAW%M_WDVERMAD),&
     & PB1(1,MSLB1C9),PCF)
  ELSE
...
IF (LNHDYN.AND.YRDYNA%LRDBBC) THEN
  ...
  IF (NSVDLAG == 2) THEN
    CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,1,NFLSA,NFLEN,IWNT,&
     & LL_LIN,YRDYNA%LCOMAD_SVD,LLNOSLHD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,KNOWENO,KL0(1:NPROMA,NFLEVG,0:3),&
