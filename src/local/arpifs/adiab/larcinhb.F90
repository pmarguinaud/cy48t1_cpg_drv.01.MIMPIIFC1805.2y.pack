...
IF (YRDYNA%LPC_FULL.AND.YRDYNA%LPC_CHEAP2.AND.(NCURRENT_ITER<NSITER)) THEN
  LL_LIN=.TRUE.
ELSE
  LL_LIN=.FALSE.
ENDIF
...
CALL LAITRE_GMV(YDML_DYN,KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,IWKM,&
 & LL_LIN,YRDYNA%LCOMAD_SVD,YRDYNA%LSLHD_SVD,LVWENO_SVD,WENO_ALPHA_SVD,LLMOM,LQMVD,LQMHVD,INOWENOH,KL0H(1,1,0),PLSCAWH,PRSCAWH,&
 & PB1(1,MSLB1VD9),PGWF)
...
IF (YRDYNA%LSETTLS .AND. YRDYNA%LPC_CHEAP .AND. NSVDLAG == 3) THEN
  IF (YRDYNA%LCOMAD_SVD) THEN
    CALL LAITLI(KASLB1,NPROMA,KST,KPROF,NFLEVG,NFLSA,NFLEN,&
     & PLSCAWH(1,1,YDML_DYN%YYTLSCAWH%M_WDLAMAD),PLSCAWH(1,1,YDML_DYN%YYTLSCAWH%M_WDLOMAD+1),&
     & KL0H(1,1,1),PLSCAWH(1,1,YDML_DYN%YYTLSCAWH%M_WDVERMAD),&
