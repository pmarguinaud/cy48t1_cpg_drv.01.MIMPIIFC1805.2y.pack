SUBROUTINE LATTE_KAPPA(YDCST, YDGEOMETRY,YDGMV,YDDYN,YDPTRSLB2,KST,KPROF,PDTS2,PSLHDA,PSLHDD0,PGMV,PGMVS,&
...
! Heat
IF (LSLHDHEAT) THEN
  CALL GP_KAPPAT(YDCST, YDGEOMETRY,YDDYN,NPROMA,KST,KPROF,NFLEVG,PDTS2,&
...
IF (YRDYNA%SLHDKMIN /= YRDYNA%SLHDKREF) THEN

  !  Detection of tropopause level
  CALL TROPO_TEP(YDCST,YDGEOMETRY,NPROMA,KST,KPROF,NFLEVG,&
