SUBROUTINE LATTE_KAPPA(YDDYNA, YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2,YDGMV,YDDYN,YDPTRSLB2,KST,KPROF,PDTS2,PSLHDA,PSLHDD0,PGMV,PGMVS,&
...
! Momentum
CALL GP_KAPPA(YDDYNA, YDVAB,YDDYN,NPROMA,KST,KPROF,NFLEVG,PDTS2,&
 & PGMV(:,:,YT0%MUL),PGMV(:,:,YT0%MVL),PGMV(:,:,YT0%MVOR),PGMV(:,:,YT0%MDIV),&
 & PUVH0(1,0,YYTHW0%M_UH),PUVH0(1,0,YYTHW0%M_VH),PRES0,PGMVS(:,YT0%MSPL),PGMVS(:,YT0%MSPM),&
 & PRDELP,PSLHDA,PSLHDD0,ZKAPPA)

! Heat
IF (LSLHDHEAT) THEN
  CALL GP_KAPPAT(YDDYNA, YDCST, YDGEOMETRY,YDDYN,NPROMA,KST,KPROF,NFLEVG,PDTS2,&
   & PGMV(:,:,YT0%MT),PGMV(:,:,YT0%MTL),PGMV(:,:,YT0%MTM),&
   & PGMVS(:,YT0%MSP),PGMVS(:,YT0%MSPL),PGMVS(:,YT0%MSPM),&
...
IF (YDDYNA%SLHDKMIN /= YDDYNA%SLHDKREF) THEN
  ...
  CALL TROPO_TEP(YDDYNA, YDCST,YDGEOMETRY,NPROMA,KST,KPROF,NFLEVG,&
...
DO JLEV=1,NLEV_SPONGE
  YDCPG_SL2%KAPPA(KST:KPROF,JLEV)=SLHD_MASK_U(JLEV)*ZKAPPA(KST:KPROF,JLEV)
  IF (LSLHDHEAT) YDCPG_SL2%KAPPAT(KST:KPROF,JLEV)= &
   &   SLHD_MASK_T(JLEV)*ZKAPPAT(KST:KPROF,JLEV)
ENDDO
DO JLEV=NLEV_SPONGE+1,NFLEVG
  YDCPG_SL2%KAPPA(KST:KPROF,JLEV)=0._JPRB
  IF (LSLHDHEAT) YDCPG_SL2%KAPPAT(KST:KPROF,JLEV)= 0._JPRB
ENDDO
