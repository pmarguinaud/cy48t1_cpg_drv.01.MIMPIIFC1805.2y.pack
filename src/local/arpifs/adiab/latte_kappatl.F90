SUBROUTINE LATTE_KAPPATL(YDGEOMETRY,YDGMV,YDGMV5,YDDYN,YDPTRSLB2,KST,KPROF,PDTS2,PSLHDA,PSLHDD0,&
 & PGMV,PGMVS,PGMV5,PGMV5S,PB2)

!**** *LATTE_KAPPATL* input for SLHD diffusion. (tangent-liner version)
...
!        INPUT/OUTPUT:
!          PB2     - "SLBUF2" buffer (containing both: increments + trajectory)
...
#include "gp_kappatl.intfb.h"
#include "gp_kappattl.intfb.h"

!     -------------------------------------------------------

IF (LHOOK) CALL DR_HOOK('LATTE_KAPPATL',0,ZHOOK_HANDLE)
...
!        Momentum
CALL GP_KAPPATL(YDDYN,NPROMA,KST,KPROF,NFLEVG,PDTS2,&
...
!        Heat
IF (LSLHDHEAT) THEN
  CALL GP_KAPPATTL(YDGEOMETRY,YDDYN,NPROMA,KST,KPROF,NFLEVG,PDTS2,&
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('LATTE_KAPPATL',1,ZHOOK_HANDLE)
END SUBROUTINE LATTE_KAPPATL
