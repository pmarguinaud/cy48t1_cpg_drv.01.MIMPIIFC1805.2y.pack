SUBROUTINE LATTES(YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2, YDRIP,YDML_DYN,KST,KPROF,PDTS2,PBDT,PESGP,PESGM,&
 & KIBL,POROGL,POROGM,&
...
& YDVFE=>YDGEOMETRY%YRVFE,   YDOROG=>YDGEOMETRY%YROROG(KIBL), YDDYN=>YDML_DYN%YRDYN)
  ...
  IF(.NOT.LLCT)THEN
    ...
    IF(NVLAG == 2 .OR. NVLAG == 3)THEN
      ...
      DO JROF=KST,KPROF
        ...
        YDVARS%SP%T1(JROF)=YDVARS%SP%T1(JROF)-ZCMSLP*YDOROG%OROG(JROF)
      ENDDO
    ENDIF
