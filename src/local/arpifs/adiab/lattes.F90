SUBROUTINE LATTES(YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1,YDCPG_SL2, YDGMV,YDRIP,YDML_DYN,KST,KPROF,PDTS2,PBDT,PESGP,PESGM,&
 & KIBL,POROGL,POROGM,&
...
IF (LTWOTL) THEN
  ...
  IF(.NOT.LLCT)THEN
    ...
    DO JLEV=1,NFLEVG
        ...
        ELSEIF (YDML_DYN%YRDYNA%LSETTLS) THEN
          ...
          IF (YDML_DYN%YRDYNA%LPC_CHEAP) THEN
            DO JROF=KST,KPROF
              ...
              YDCPG_SL1%C9(JROF,JLEV)=YDCPG_SL1%C9(JROF,JLEV)+ZNESC
              YDCPG_SL1%C9_NL(JROF,JLEV)=ZMIXNL*ZSETTLS
            ENDDO
          ELSE
            DO JROF=KST,KPROF
              ZNESC  =PESGM*ZMOY1SP(JROF,JLEV)
      ...
      IF( YDML_DYN%YRDYNA%LPC_FULL )THEN
        ...
        YDVARS%CSPNL%T9(KST:KPROF,JLEV)=ZMOY1SP(KST:KPROF,JLEV)
      ENDIF
    ...
    IF(NVLAG == 2 .OR. NVLAG == 3)THEN
      !dir$ ivdep
      DO JROF=KST,KPROF
        YDCPG_SL1%SP9(JROF)=YDCPG_SL1%SP9(JROF)+YDVARS%SP%T0(JROF)&
  ...
  ELSE
    ...
    IF (.NOT.LLCTC) THEN
      !dir$ ivdep
      DO JROF=KST,KPROF
        YDCPG_SL1%SP9(JROF)=YDCPG_SL1%SP9(JROF)+YDVARS%SP%T9(JROF)&
         & +ZCMSLP*YDOROG%OROG(JROF)
      ENDDO
    ENDIF
  ...
  IF( XIDT > 0.0_JPRB )THEN
    DO JROF=KST,KPROF
      PGMVT1S(JROF,YT1%MSP)=PGMVT1S(JROF,YT1%MSP)-ZXIGP*YDCPG_SL2%SPSI(JROF)
      YDCPG_SL2%SPSI(JROF)=ZXIGP*YDCPG_SL2%SPSI(JROF)
    ENDDO
  ELSE
    DO JROF=KST,KPROF
      PGMVT1S(JROF,YT1%MSP)=PGMVT1S(JROF,YT1%MSP)-PESGP*YDCPG_SL2%SPSI(JROF)
