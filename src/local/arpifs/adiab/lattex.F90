...
IF(.NOT.YDCPG_OPTS%LTWOTL)THEN
  ...
  IF( YDCPG_OPTS%LNHEE ) ALLOCATE( ZSPDSI9(NPROMNH,NFLEVG))
  IF( YDCPG_OPTS%LNHDYN ) ALLOCATE( ZVWVSI9(NPROMNH,NFLEVG))
ENDIF
...
IF (LRSIDDH) THEN
  ...
  ENDDO
  IF (YDCPG_OPTS%LNHDYN) THEN !! ?????? LNHDYN or LNHEE?
...
IF (YDCPG_OPTS%LTWOTL) THEN
  ...
  CALL LATTEX_DNT(YDCPG_OPTS%NSTEP, YDML_DYN%YRDYNA,YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,LLSETTLSW,NWLAG,PESGP,PESGM,&
...
DO JLEV=1,NFLEVG
  ...
  DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
    ZMOY1T(JROF,JLEV)=ZMOY1T(JROF,JLEV)&
     & +PDTS2*YDCPG_TND%TNDT(JROF,JLEV)&
...
IF (YDCPG_OPTS%LTWOTL) THEN
  ...
  CALL LATTEX_DNT(YDCPG_OPTS%NSTEP, YDML_DYN%YRDYNA,YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDML_DYN%YRDYNA%LSETTLS,NTLAG,PESGP,PESGM,&
...
IF (YDCPG_OPTS%LNHEE) THEN
  ...
  IF (YDCPG_OPTS%LTWOTL) THEN
    ...
    CALL LATTEX_DNT(YDCPG_OPTS%NSTEP, YDML_DYN%YRDYNA,YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDML_DYN%YRDYNA%LSETTLS,NSPDLAG,PESGP,PESGM,&
...
IF (YDCPG_OPTS%LNHDYN.AND.(.NOT.YDML_DYN%YRDYNA%LGWADV)) THEN
  ...
  IF (YDCPG_OPTS%LTWOTL) THEN
    ...
    CALL LATTEX_DNT(YDCPG_OPTS%NSTEP, YDML_DYN%YRDYNA,YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YDML_DYN%YRDYNA%LSETTLS,NSVDLAG,PESGP,PESGM,&
    ...
    IF ((YDML_DYN%YRDYNA%NVDVAR==4 .OR. YDML_DYN%YRDYNA%NVDVAR==5) .AND. YDML_DYN%YRDYNA%ND4SYS==1) THEN
      ...
      IF (.NOT.LLCT) THEN
        ...
        IF(YDML_DYN%YRDYNA%LNESC.OR.(YDCPG_OPTS%NSTEP <= NFOST)) THEN
          ...
          DO JLEV=1,NFLEVG
            ...
            ENDDO
          ENDDO
        ELSEIF (.NOT.YDML_DYN%YRDYNA%LNESC.AND.(YDCPG_OPTS%NSTEP > NFOST).AND.YDML_DYN%YRDYNA%LSETTLS) THEN
...
IF (YDCPG_OPTS%LNHDYN.AND.YDML_DYN%YRDYNA%LGWADV) THEN
...
IF (YDCPG_OPTS%LTWOTL) THEN
  CALL LATTEX_GFL_2TL (YDGEOMETRY, YDCPG_BNDS, YDML_GCONF, YDML_DYN, PGFL, YDCPG_SL1%ZVIEW, LLCT, LLCTC, LLTDIABLIN)
ELSE
  CALL LATTEX_GFL_3TL (YDGEOMETRY, YDCPG_BNDS, YDML_GCONF, YDML_DYN, PGFL, YDCPG_SL1%ZVIEW, LLTDIABLIN)
ENDIF
...
IF (.NOT.LLCTC) THEN
  CALL LATTEX_GFL_VSPLTRANS (YDGEOMETRY, YDCPG_BNDS, YDML_GCONF, YDML_DYN, YDCPG_SL1%ZVIEW, LLTDIABLIN)
ENDIF
