...
LLCT =YRDYNA%LPC_FULL .AND. NCURRENT_ITER > 0  ! corrector step
LLCTC=YRDYNA%LPC_CHEAP .AND. NCURRENT_ITER > 0

LLVD5=(YRDYNA%NVDVAR==5)
...
IF (LTWOTL) THEN
  ...
  CALL LATTEX_DNT(YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YRDYNA%LSETTLS,NTLAG,PESGP,PESGM,&
...
IF (LNHEE) THEN
  ...
  IF (LTWOTL) THEN
    ...
    CALL LATTEX_DNT(YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YRDYNA%LSETTLS,NSPDLAG,PESGP,PESGM,&
...
IF (LNHDYN.AND.(.NOT.YRDYNA%LGWADV)) THEN
  ...
  IF (LTWOTL) THEN
    ...
    CALL LATTEX_DNT(YDGEOMETRY,YDLDDH,YDRIP,YDDYN,YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA,YRDYNA%LSETTLS,NSVDLAG,PESGP,PESGM,&
    ...
    IF ((YRDYNA%NVDVAR==4 .OR. YRDYNA%NVDVAR==5) .AND. YRDYNA%ND4SYS==1) THEN

      IF (.NOT.LLCT) THEN
        ! * predictor step for LPC_FULL or normal SI step.
        IF(YRDYNA%LNESC.OR.(NSTEP <= NFOST)) THEN
          ...
          DO JLEV=1,NFLEVG
            ...
            ENDDO
          ENDDO
        ELSEIF (.NOT.YRDYNA%LNESC.AND.(NSTEP > NFOST).AND.YRDYNA%LSETTLS) THEN
    ...
    ELSEIF ((YRDYNA%NVDVAR==4 .OR. YRDYNA%NVDVAR==5) .AND. YRDYNA%ND4SYS==2) THEN
...
IF (LNHDYN.AND.YRDYNA%LGWADV) THEN
  ...
  IF (LTWOTL) THEN
    ...
    IF( YRDYNA%LPC_FULL.AND.NCURRENT_ITER==0)THEN
    ...
    IF ((YRDYNA%NVDVAR==4 .OR. YRDYNA%NVDVAR==5) .AND. YRDYNA%ND4SYS==2) THEN
      IF (.NOT.LLCT) THEN
        DO JLEV=1,NFLEVG
