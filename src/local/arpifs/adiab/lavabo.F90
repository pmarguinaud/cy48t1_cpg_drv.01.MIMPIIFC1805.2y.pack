SUBROUTINE LAVABO(YDGEOMETRY,YGFL,YDDYN,YDPTRSLB1,KST,KPROF,LD2TLFF1,LDSLPHY,PB1)
...
USE YOMDYNA      , ONLY : NVDVAR, LSLINL, LSLINLC2
...
IF (NVLAG == 2.OR.NVLAG == 3) THEN
  ...
  IF (LSLINLC2) PB1(KST:KPROF,MSLB1C9_SI)=0.0_JPRB
  IF (LSLINLC2) PB1(KST:KPROF,MSLB1C9_SI+NFLEVG+1)=0.0_JPRB
ENDIF
...
IF( LDSLPHY ) THEN
  ...
  DO JGFL=1,NUMFLDS
    IF(YCOMP(JGFL)%LPHY) THEN
      ISLB1GFLP9=MSLB1GFLP9+(JGFL-1)*(NFLEN-NFLSA+1)
