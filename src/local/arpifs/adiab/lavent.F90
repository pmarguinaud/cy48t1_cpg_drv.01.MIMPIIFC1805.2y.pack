!OCL  NOEVAL
SUBROUTINE LAVENT(YDCST, YDGEOMETRY,YDVARS,YDCPG_SL1, YDCPG_SL2, YDRIP,YDML_DYN,YDPARAR,KST,KPROF,KIBL,KSETTLOFF,LD2TLFF1, &
...
& YDPTRSLB1=>YDML_DYN%YRPTRSLB1,YDPTRSLB2=>YDML_DYN%YRPTRSLB2,  YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL))
...
LLCT = (YRDYNA%LPC_FULL .AND. NCURRENT_ITER > 0)
...
IF (LSLDP_CURV) THEN
  DO JLEV=1,NFLEVG
    DO JROF=KST,KPROF
      ...
      YDCPG_SL1%ZR0(JROF,JLEV)=YDCPG_SL1%VR0(JROF,JLEV)*YDGSGEOM%GSQM2(JROF)
      YDCPG_SL1%UR0(JROF,JLEV)=-YDCPG_SL1%UR0(JROF,JLEV)*YDGSGEOM%GESLO(JROF)&
       & -YDCPG_SL1%VR0(JROF,JLEV)*YDGSGEOM%GECLO(JROF)*YDGSGEOM%GEMU(JROF)
      YDCPG_SL1%VR0(JROF,JLEV)= ZUCOMP*YDGSGEOM%GECLO(JROF) &
       & -YDCPG_SL1%VR0(JROF,JLEV)*YDGSGEOM%GESLO(JROF)*YDGSGEOM%GEMU(JROF)
    ENDDO
  ENDDO
  IF (LSLDP_RK) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KPROF
        ...
        YDCPG_SL1%ZR00(JROF,JLEV)= YDCPG_SL1%VR00(JROF,JLEV)*YDGSGEOM%GSQM2(JROF)
        YDCPG_SL1%UR00(JROF,JLEV)=-YDCPG_SL1%UR00(JROF,JLEV)*YDGSGEOM%GESLO(JROF)&
         & -YDCPG_SL1%VR00(JROF,JLEV)*YDGSGEOM%GECLO(JROF)*YDGSGEOM%GEMU(JROF)
        YDCPG_SL1%VR00(JROF,JLEV)= ZUCOMP*YDGSGEOM%GECLO(JROF) &
         & -YDCPG_SL1%VR00(JROF,JLEV)*YDGSGEOM%GESLO(JROF)*YDGSGEOM%GEMU(JROF)
      ENDDO
    ENDDO
  ENDIF
ENDIF
