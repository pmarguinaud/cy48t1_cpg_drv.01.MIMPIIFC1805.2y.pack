...
CALL SITNU(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,PIN,ZTAU,ZNU,KNLON)
ZTAU(:)=ZTAU(:)*YDCST%RCPD

! * Computes ZGAM = Gamma * PIN
ZSP(1:KNLON)=0.0_JPRB
CALL SIGAM(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,ZGAM,PIN,ZSP,KNLON,NFLEVG)

! * Computes ZGAMTAU = Cp_dry * Gamma * (Tau * PIN) = Gamma * ZTAU
ZSP(1:KNLON)=0.0_JPRB
CALL SIGAM(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,ZGAMTAU,ZTAU,ZSP,KNLON,NFLEVG)
...
IF (PRESENT(POU2)) THEN
  ...
  ZSP(1:KNLON)=0.0_JPRB
  CALL SITNU(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,ZGAM,ZTAUGAM,ZSP,KNLON)
