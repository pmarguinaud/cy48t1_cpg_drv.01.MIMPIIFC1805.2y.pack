SUBROUTINE SI_CCCOR(YDGEOMETRY,YDDYN,KLEV,KLON,KNLON,PIN,POU,POU2)
...
CALL SITNU(YDGEOMETRY,YDDYN,KLEV,KLON,PIN,ZTAU,ZNU,KNLON)
ZTAU(:)=ZTAU(:)*RCPD

! * Computes ZGAM = Gamma * PIN
ZSP(1:KNLON)=0.0_JPRB
CALL SIGAM(YDGEOMETRY,YDDYN,KLEV,KLON,ZGAM,PIN,ZSP,KNLON,NFLEVG)

! * Computes ZGAMTAU = Cp_dry * Gamma * (Tau * PIN) = Gamma * ZTAU
ZSP(1:KNLON)=0.0_JPRB
CALL SIGAM(YDGEOMETRY,YDDYN,KLEV,KLON,ZGAMTAU,ZTAU,ZSP,KNLON,NFLEVG)

! * Computes POU:
DO JLEV=1,NFLEVG
  DO JLON=1,KNLON
    IDT=1+(JLEV-1)*KLEV+(JLON-1)*KLON
    POU(IDT)=ZGAMTAU(IDT)/RD-SITR*ZGAM(IDT)-ZTAU(IDT)+RD*SITR*ZNU(JLON)
  ENDDO
ENDDO
...
IF (PRESENT(POU2)) THEN
  ...
  ZSP(1:KNLON)=0.0_JPRB
  CALL SITNU(YDGEOMETRY,YDDYN,KLEV,KLON,ZGAM,ZTAUGAM,ZSP,KNLON)

  ! * Computes (g**2/(N**2 C**2)) A2 * PIN:
  DO JLEV=1,NFLEVG
    DO JLON=1,KNLON
      IDT=1+(JLEV-1)*KLEV+(JLON-1)*KLON
      ZAUX(IDT)=((RCVD/RD)*ZTAUGAM(IDT)-ZTAU(IDT)-SITR*ZGAM(IDT))*RCPD/(RD*RD*SITR)
    ENDDO
  ENDDO
