...
CALL SIGAM(YDGEOMETRY,YDDYN,ILEV,ILON,PDH,PT,PSP,INLON,NFLEVG)
...
CALL SIGAM(YDGEOMETRY,YDDYN,ILEV,ILON,ZW3D,PRNH,ZW2D,INLON,NFLEVG)

!$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JLEV,JAD)
DO JLEV = 1, NFLEVG
  DO JAD=1+(JLEV-1)*ILEV,1+(JLEV-1)*ILEV+(INLON-1)*ILON,ILON
    PDH(JAD) = PDH(JAD) - SITR*ZW3D(JAD) + RD*SITR*PRNH(JAD)
  ENDDO
ENDDO
!$OMP END PARALLEL DO
