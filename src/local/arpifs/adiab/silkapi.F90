...
!$OMP END PARALLEL DO
  ...
  ELSEIF (KOPT == 2 .AND. .NOT.LLCSTSITRA) THEN
    ...
    CALL SISKAP(LVERTFE, YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,KNLON,PIN,ZZ0)
...
!$OMP END PARALLEL DO
  ...
  CALL SINHQE_SEVE(LVFE_LAPL_BC, YDCST, YDGEOMETRY,YDDYN,1,NFLEVG,ZID,ZL1,NFLEVG,LD_LSTAR=.TRUE.)

  ! * Compute ZL2 = partial_star ZID
  CALL SINHQE_VDERI(LVFE_LAPL_BC, YDCST, YDGEOMETRY,YDDYN,1,NFLEVG,ZID,ZL2,NFLEVG,LD_LSTAR=.TRUE.)
