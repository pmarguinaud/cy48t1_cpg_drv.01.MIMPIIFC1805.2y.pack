...
IF (YRDYNA%LNHQE_C2) THEN
  ...
  IF (KOPT == 0) THEN
    ...
    CALL SITNU(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,PIN,ZZ4,ZSP,KNLON)

    ! * Compute "ZZ5 = gamma ZZ4 + Rd Tstar ZSP": ZZ5 = [gamma tau + Rd Tstar nu] Z
    CALL SIGAM(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,ZZ5,ZZ4,ZSP,KNLON,NFLEVG)
...
!$OMP END PARALLEL DO
    ...
    CALL SITNU(YDCST, YDGEOMETRY,YDDYN,KLEV,KLON,ZZ2,ZZ4,ZSP,KNLON)
