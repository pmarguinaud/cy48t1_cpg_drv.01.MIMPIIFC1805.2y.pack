...
CALL SITNU(YDGEOMETRY,YDDYN,ILEV,ILON,PDH,PRNH,PSP,INLON)
...
!DEC$ IVDEP
  DO JAD=1+(JLEV-1)*ILEV,1+(JLEV-1)*ILEV+(INLON-1)*ILON,ILON
    Z3DIVC = (PDH(JAD) + PDV(JAD))/RCVD
    PT(JAD) = RD*SITR*Z3DIVC
    PRNH(JAD) = RCPD*Z3DIVC - RCPD*PRNH(JAD)/(RD*SITR)
  ENDDO
ENDDO
!$OMP END PARALLEL DO
