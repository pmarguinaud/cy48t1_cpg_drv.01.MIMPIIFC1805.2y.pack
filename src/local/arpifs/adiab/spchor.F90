...
IF (LRHDDDH) THEN
  ...
  IF (LNHDYN) THEN
    ...
    IF (YRDYNA%LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=-PSPSNHX(1:NFLEVG,KSTA:KEND)
  ENDIF
  IF (YQ%LSP) THEN
    DO JSP=KSTA,KEND
      DO JLEV=1,NFLEVG
...
!         - GMVS variables.
  ...
  IF ((YRDYNA%LSLHD_W.OR.YRDYNA%LSLHD_SVD).AND.(.NOT.YRDYNA%LSLHD_STATIC)) THEN
...
!OCL NOVREC
      ...
      IF (YRDYNA%LNHX) THEN
        ...
        ENDDO
      ENDIF
      IF (YRDYNA%LSLHD_SVD.AND.(.NOT.YRDYNA%LSLHD_STATIC)) THEN
        ! * Additional diffusion
!OCL NOVREC
        DO JLEV=1,NFLEVL
...
IF (LRHDDDH) THEN
  ...
  IF (LNHDYN) THEN
    ...
    IF (YRDYNA%LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)&
