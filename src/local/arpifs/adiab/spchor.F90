...
USE YOMDYNA      , ONLY : LSLHD_STATIC, LSLHD_W, LSLHD_SVD, LNHX
...
IF (LRHDDDH) THEN
  ...
  IF (LNHDYN) THEN
    ...
    IF (LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=-PSPSNHX(1:NFLEVG,KSTA:KEND)
  ENDIF
  IF (YQ%LSP) THEN
    DO JSP=KSTA,KEND
      DO JLEV=1,NFLEVG
...
!         - GMVS variables.
  ...
  IF ((LSLHD_W.OR.LSLHD_SVD).AND.(.NOT.LSLHD_STATIC)) THEN
    ...
    IF (LNHDYN.AND.LSLHD_SVD) IVTHS=IVTHS+1
    IF (LNHX.AND.LSLHD_SVD) IVTHS=IVTHS+1
    IL=IVTHS*NFLEVL
...
!OCL NOVREC
      ...
      IF (LNHX) THEN
        ...
        ENDDO
      ENDIF
      IF (LSLHD_SVD.AND.(.NOT.LSLHD_STATIC)) THEN
        ! * Additional diffusion
!OCL NOVREC
        DO JLEV=1,NFLEVL
...
IF (LRHDDDH) THEN
  ...
  IF (LNHDYN) THEN
    ...
    IF (LNHX) PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)=PSPTNDHD_SNHX(1:NFLEVG,KSTA:KEND)&
