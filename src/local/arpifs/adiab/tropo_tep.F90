SUBROUTINE TROPO_TEP(YDDYNA, YDCST,YDGEOMETRY,KPROMA,KSTART,KPROF,KFLEV,&
  & PSPT0,PTT0,KTROP)
...
CALL GPHPRE_EXPL (YDDYNA%LAPRXPK, LVERTFE, YDDYNA%NDLNPR, YDDYNA%RHYDR0, TOPPRES, YDCST,KPROMA,KFLEV,KSTART,KPROF,YDVAB,ZPREH,PRESF=ZPREF)

!*         Find the lowest stratospheric level
KTROP(KSTART:KPROF)=1
DO JLEV=2,KFLEV
  DO JROF=KSTART,KPROF
    IF (ZPREF(JROF,KTROP(JROF)) < RPSTRA-ZPDIFFU) KTROP(JROF)=JLEV
    IF ( (PTT0(JROF,JLEV) <= PTT0(JROF,JLEV-1)).AND. &
     &  (ZPREF(JROF,JLEV) < RPTROP+ZPDIFFL))         &
