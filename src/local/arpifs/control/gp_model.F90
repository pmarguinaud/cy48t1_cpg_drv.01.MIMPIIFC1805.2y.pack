...
IF (CDCONF(3:3) == 'A'.OR.CDCONF(3:3) == 'B'.OR.CDCONF(3:3) == '1') THEN
  ...
  ENDIF
  IF(YDMODEL%YRML_DYN%YRDYNA%LPC_FULL.AND.LSLAG.AND.(LEPHYS.OR.LMPHYS).AND..NOT.ALLOCATED(YDGFL%GFLPT)) THEN
    CALL ABOR1('GP_MODEL: GFLPT NOT ALLOCATED')
  ENDIF
  IF(YDMODEL%YRML_DYN%YRDYNA%LPC_FULL.AND..NOT.LTWOTL.AND..NOT.ALLOCATED(YDGFL%GFLPC)) THEN
...
IF(YDMODEL%YRML_DYN%YRDYNA%LPC_FULL.AND.LTWOTL.AND.LEPHYS)THEN
...
!     ------------------------------------------------------------------
  ...
  IF (LNHQE.AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LNHQE_SIHYD)) THEN
...
IF (.NOT.LR2D) THEN
  IF (LEPHYS.AND.LAGPHY.AND.LERADI.AND.(.NOT.LPHYLIN)) THEN
    ...
    ELSEIF (MOD(NSTEP,NRADFR) == 0) THEN
      IF (YDMODEL%YRML_DYN%YRDYNA%LPC_FULL) THEN
...
! cop
  ...
  IF((LEPHYS.OR.LHELDSUAREZ) .AND. LAGPHY) THEN
    IF ((YDMODEL%YRML_DYN%YRDYNA%LPC_FULL.AND.(NCURRENT_ITER == NSITER)).OR.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LPC_FULL)) THEN
