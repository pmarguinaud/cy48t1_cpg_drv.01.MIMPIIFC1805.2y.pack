...
IF (CDCONF(3:3) == 'A'.OR.CDCONF(3:3) == 'B'.OR.CDCONF(3:3) == '1') THEN
  ...
  ENDIF
  IF(YRDYNA%LPC_FULL.AND.LSLAG.AND.(LEPHYS.OR.LMPHYS).AND..NOT.ALLOCATED(YDGFL%GFLPT)) THEN
    CALL ABOR1('GP_MODEL: GFLPT NOT ALLOCATED')
  ENDIF
  IF(YRDYNA%LPC_FULL.AND..NOT.LTWOTL.AND..NOT.ALLOCATED(YDGFL%GFLPC)) THEN
...
IF(YRDYNA%LPC_FULL.AND.LTWOTL.AND.LEPHYS)THEN
