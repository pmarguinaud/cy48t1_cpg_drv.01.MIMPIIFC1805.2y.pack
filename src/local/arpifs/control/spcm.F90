...
LL_CALL_SPCIMPF=LIMPF.AND.(.NOT.(LNHEE.AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LSI_NHEE)))
...
IPART_2B2C=2
IPART_2D=3
IF (LNHDYN.AND.LNHQE.AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LNHQE_SIHYD)) THEN
  IF (NITERHELM==0) THEN
    ! Predictor:
...
IF (CDCONF == 'A'.OR.CDCONF == 'I') THEN
 ...
 IF (LR3D) THEN
  ...
  IF (LLDOSI) THEN
    ...
    IF (LNHQE.AND.(.NOT.YDMODEL%YRML_DYN%YRDYNA%LNHQE_SIHYD)) ALLOCATE(ZSPBUFG(NFLEVG,ISPEC2V,3))
    ...
    IF (LLONEM) THEN
      ...
      ELSE
        ...
        ELSE
          ...
          ENDDO
        ENDIF
        IF(LNHQE.AND.YDMODEL%YRML_DYN%YRDYNA%LNHQE_SIHYD) ZSPSPDG(:,:)=0.0_JPRB
