...
USE YOMDYNA  , ONLY : SLHDKMIN, HOISLTH, HOISLTV, LSLTVWENO, LHOISLT
...
IF (KWIS==102.OR.KWIS==202) THEN
  ZSLHDKMINH=HOISLTH
  ZSLHDKMINV=HOISLTV  ! useless for shallow-water
ENDIF
...
!CDIR NODEP
  ...
  ! Modify previous defaults for high order trajectory research
  IF (KWIS==102) LLT_SLHD(2)= (HOISLTH /= 0._JPRB)
...
!CDIR NODEP
  ...
  IF (KWIS == 202) LLT_SLHD(2)= (HOISLTH /= 0._JPRB)
