...
ELSE
  SELF%F_CTY => CREATE_TEMPORARY ('CTY', GEOM=REGISTRY%GEOM, NLEV=NLEV+1, NDIM=YYTCTY0%NDIM, PERSISTENT=PERSISTENT)
ENDIF

IF (PRESENT (PWRL)) THEN
  SELF%F_WRL => CREATE_FIELD_WRAP ('WRL', PWRL)
ELSE
  SELF%F_WRL => CREATE_TEMPORARY ('WRL', GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
SELF%RCP            => SELF%F_RCP %GET_VIEW (BLOCK_INDEX)
SELF%PREF           => SELF%F_PREF%GET_VIEW (BLOCK_INDEX)
SELF%CTY (1:,0:,1:) => SELF%F_CTY %GET_VIEW (BLOCK_INDEX)
...
CALL DELETE_TEMPORARY (SELF%F_PHIF) ! Should have a DELETE_FIELD_WRAP or a method handling both cases
CALL DELETE_TEMPORARY (SELF%F_RCP ) ! Should have a DELETE_FIELD_WRAP or a method handling both cases
CALL DELETE_TEMPORARY (SELF%F_PREF) ! Should have a DELETE_FIELD_WRAP or a method handling both cases
CALL DELETE_TEMPORARY (SELF%F_CTY ) ! Should have a DELETE_FIELD_WRAP or a method handling both cases
CALL DELETE_TEMPORARY (SELF%F_WRL ) ! Should have a DELETE_FIELD_WRAP or a method handling both cases
