...
TYPE CPG_SLB_TYPE
  ...
  REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZDATA (:,:,:) => NULL ()
CONTAINS
  PROCEDURE :: INIT => CPG_SLB_TYPE_INIT
  PROCEDURE :: UPDATE_VIEW => CPG_SLB_TYPE_UPDATE_VIEW
  PROCEDURE :: FINAL => CPG_SLB_TYPE_FINAL
END TYPE CPG_SLB_TYPE
...
IF (PRESENT (PTXYBDER)) THEN
  ...
  SELF%LOWNED = .FALSE.
ELSE
  ALLOCATE (SELF%ZDATA (REGISTRY%NPROMA, NLEV, YDTXYBDER%NDIM, REGISTRY%NBLOCKS))
  SELF%LOWNED = .TRUE.
ENDIF

 IF (YDTXYBDER%M_LNPRL > 0) THEN
   SELF%F_LNPRL => CREATE_FIELD_WRAP ('LNPRL', SELF%ZDATA (:, :, YDTXYBDER%M_LNPRL, :))
 ENDIF
 IF (YDTXYBDER%M_LNPRM > 0) THEN
   SELF%F_LNPRM => CREATE_FIELD_WRAP ('LNPRM', SELF%ZDATA (:, :, YDTXYBDER%M_LNPRM, :))
 ENDIF
 IF (YDTXYBDER%M_ALPHL > 0) THEN
   SELF%F_ALPHL => CREATE_FIELD_WRAP ('ALPHL', SELF%ZDATA (:, :, YDTXYBDER%M_ALPHL, :))
 ENDIF
 IF (YDTXYBDER%M_ALPHM > 0) THEN
   SELF%F_ALPHM => CREATE_FIELD_WRAP ('ALPHM', SELF%ZDATA (:, :, YDTXYBDER%M_ALPHM, :))
 ENDIF
 IF (YDTXYBDER%M_ALPHPLL > 0) THEN
   SELF%F_ALPHPLL => CREATE_FIELD_WRAP ('ALPHPLL', SELF%ZDATA (:, :, YDTXYBDER%M_ALPHPLL, :))
 ENDIF
 IF (YDTXYBDER%M_ALPHPLM > 0) THEN
   SELF%F_ALPHPLM => CREATE_FIELD_WRAP ('ALPHPLM', SELF%ZDATA (:, :, YDTXYBDER%M_ALPHPLM, :))
 ENDIF
 IF (YDTXYBDER%M_COEFD > 0) THEN
   SELF%F_COEFD => CREATE_FIELD_WRAP ('COEFD', SELF%ZDATA (:, :, YDTXYBDER%M_COEFD, :))
 ENDIF
 IF (YDTXYBDER%M_COEFA > 0) THEN
   SELF%F_COEFA => CREATE_FIELD_WRAP ('COEFA', SELF%ZDATA (:, :, YDTXYBDER%M_COEFA, :))
 ENDIF
 IF (YDTXYBDER%M_COEFAPL > 0) THEN
   SELF%F_COEFAPL => CREATE_FIELD_WRAP ('COEFAPL', SELF%ZDATA (:, :, YDTXYBDER%M_COEFAPL, :))
 ENDIF
...
SUBROUTINE CPG_SLB_TYPE_INIT (SELF, REGISTRY, NLEV, YDTPTRSLB1, PERSISTENT, PTPTRSLB1)

USE PTRSLB1, ONLY : TPTRSLB1

CLASS (CPG_SLB_TYPE)                          :: SELF
TYPE (FIELD_REGISTRY),              INTENT (INOUT) :: REGISTRY
INTEGER (KIND=JPIM),                INTENT (IN)    :: NLEV
TYPE (TPTRSLB1),                    INTENT (IN)    :: YDTPTRSLB1
LOGICAL, OPTIONAL,                  INTENT (IN)    :: PERSISTENT
REAL (KIND=JPRB), OPTIONAL, TARGET, INTENT (IN)    :: PTPTRSLB1 (:, :, :)

IF (PRESENT (PTPTRSLB1)) THEN
  SELF%ZDATA => PTPTRSLB1
  SELF%LOWNED = .FALSE.
ELSE
  ALLOCATE (SELF%ZDATA (REGISTRY%NPROMA, YDTPTRSLB1%NFLDSLB1, REGISTRY%NBLOCKS))
  SELF%LOWNED = .TRUE.
ENDIF

END SUBROUTINE CPG_SLB_TYPE_INIT

SUBROUTINE CPG_SLB_TYPE_UPDATE_VIEW (SELF, BLOCK_INDEX)
...
END SUBROUTINE CPG_SLB_TYPE_UPDATE_VIEW

SUBROUTINE CPG_SLB_TYPE_FINAL (SELF)
CLASS (CPG_SLB_TYPE)          :: SELF

IF (SELF%LOWNED) DEALLOCATE (SELF%ZDATA)
SELF%ZDATA => NULL ()

END SUBROUTINE CPG_SLB_TYPE_FINAL
