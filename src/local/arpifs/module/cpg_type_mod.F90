...
IF (LNHDYN) THEN
  SELF%F_GDW => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

SELF%F_GWHT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
...
IF (LNHEE) THEN
  SELF%F_DELNHPRE => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.(.NOT.LNHDYN)) THEN
  SELF%F_DPHYCTY => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

SELF%F_DVER => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)

IF (LLT0.AND.LNHDYN) THEN
  SELF%F_DVERL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHDYN) THEN
  SELF%F_DVERM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHEE) THEN
  SELF%F_DVERW => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHQE) THEN
  SELF%F_EIQCHAF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHQE) THEN
  SELF%F_EQCHAF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHQE) THEN
  SELF%F_EQCHAH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_GPHL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_GPHM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHDYN) THEN
  SELF%F_GWHL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHDYN) THEN
  SELF%F_GWHM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHQE) THEN
  SELF%F_KAPH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_LNNHPREFL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_LNNHPREFM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHEE) THEN
  SELF%F_NHPPI => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_NHPREL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_NHPREM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHQE) THEN
  SELF%F_NHXD_T => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_NHXS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHQE) THEN
  SELF%F_NHXS_T => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF ((.NOT.LNHDYN)) THEN
  SELF%F_NHXT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHEE) THEN
  SELF%F_PDEP => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LLT0) THEN
  SELF%F_PSGRTL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_PSGRTM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (.NOT.LLT0) THEN
  SELF%F_R0T => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF ((.NOT.LLT0).AND.LNHQE) THEN
  SELF%F_R9T => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF ((.NOT.LNHDYN).OR.LNHEE) THEN
  SELF%F_RDT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_RDTL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_RDTM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHEE) THEN
  SELF%F_RNHPPI => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_RPREF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHEE) THEN
  SELF%F_RRED => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_RT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_RTR => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_SGRTL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_SGRTM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LLT0.AND.LNHEE) THEN
  SELF%F_SVDINCR13 => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_TNDGWF_LAP => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_TNDGWF_OTH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_TNDGWH_LAP => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_TNDGWH_OTH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LLT0.AND.LNHEE) THEN
  SELF%F_Z3DIVG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0.AND.LNHEE) THEN
  SELF%F_WH2F => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, NDIM=2, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LLT0.AND.LNHQE) THEN
  SELF%F_ONE => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
SELF%F_PHI => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)

SELF%F_PRE => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)

IF (PRESENT (PHIF)) THEN
  SELF%F_PHIF => CREATE_FIELD_WRAP (PHIF)
ELSE
  SELF%F_PHIF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (PRESENT (PREF)) THEN
  SELF%F_PREF => CREATE_FIELD_WRAP (PREF)
ELSE
  SELF%F_PREF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (PRESENT (PWRL)) THEN
  SELF%F_WRL => CREATE_FIELD_WRAP (PWRL)
ENDIF
...
SELF%F_NHY => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)

SELF%F_GWFT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)

IF (LLT0) THEN
  SELF%F_KENE => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

SELF%F_NHX => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)

IF (LLT0) THEN
  SELF%F_PHIFL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_PHIFM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_RTL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLT0) THEN
  SELF%F_RTM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LNHDYN .AND. LLT0) THEN
  SELF%F_GWFL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN .AND. LLT0) THEN
  SELF%F_GWFM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN) THEN
  SELF%F_GWT => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN) THEN
  SELF%F_NHPREF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN) THEN
  SELF%F_NHPREH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN .AND. LLT0) THEN
  SELF%F_QCHAL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN .AND. LLT0) THEN
  SELF%F_QCHAM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LNHDYN .AND. LLT0) THEN
  SELF%F_RDPHI => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
IF (LLMF_PHYS) THEN
  SELF%F_PREHYD => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV+1, PERSISTENT=PERSISTENT)
ENDIF

IF (LLMF_PHYS) THEN
  SELF%F_PREHYDF => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLMF_PHYS) THEN
  SELF%F_WL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLMF_PHYS) THEN
  SELF%F_WM => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF

IF (LLMF_PHYS) THEN
  SELF%F_W => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=NLEV, PERSISTENT=PERSISTENT)
ENDIF
...
SELF%F_NEB => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_QLI => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_QICE => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_QRAIN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_QSNOW => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_RH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)
...
SELF%F_FTCNS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, NDIM=6, PERSISTENT=PERSISTENT)

SELF%F_KOZO => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, NDIM=YDCPG_DIM%KVCLIS, PERSISTENT=PERSISTENT)
