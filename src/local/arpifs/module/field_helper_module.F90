...
CONTAINS
  ...
  SUBROUTINE CREATE_TEMPORARY_2D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (2)
    FIELD_PTR = FIELD_2D(SHAPE=ISIZE(1:2-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
  ...
  SUBROUTINE CREATE_TEMPORARY_3D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (3)
    FIELD_PTR = FIELD_3D(SHAPE=ISIZE(1:3-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  ...
  SUBROUTINE CREATE_TEMPORARY_4D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (4)
    FIELD_PTR = FIELD_4D(SHAPE=ISIZE(1:4-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  END SUBROUTINE CREATE_TEMPORARY_4D_LU
  SUBROUTINE CREATE_TEMPORARY_5D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ! Create a single 5D field for temporary use with an implicit blocking dimension.
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (5)
    FIELD_PTR = FIELD_5D(SHAPE=ISIZE(1:5-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  ...
  SUBROUTINE CREATE_TEMPORARY_INT2D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (2)
    FIELD_PTR = FIELD_INT2D(SHAPE=ISIZE(1:2-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  ...
  SUBROUTINE CREATE_TEMPORARY_INT3D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (3)
    FIELD_PTR = FIELD_INT3D(SHAPE=ISIZE(1:3-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  ...
  SUBROUTINE CREATE_TEMPORARY_LOG2D_LU (FIELD_PTR, UBOUNDS, LBOUNDS, PERSISTENT) 
    ...
    ALLOCATE(FIELD_PTR)
    NBLOCKS = UBOUNDS (2)
    FIELD_PTR = FIELD_LOG2D(SHAPE=ISIZE(1:2-1), NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT, LBOUNDS=LBOUNDS)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
