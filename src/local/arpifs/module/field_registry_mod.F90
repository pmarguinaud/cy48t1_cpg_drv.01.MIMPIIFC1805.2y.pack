...
USE FIELD_MODULE, ONLY: FIELD_2D, FIELD_INT2D, FIELD_LOG2D, FIELD_3D, FIELD_4D, FIELD_5D, &
...
CONTAINS
  ...
  FUNCTION CREATE_FIELD_WRAP_2D(DATA, PERSISTENT) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_2D(DATA=DATA, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_FIELD_WRAP_2D

  FUNCTION CREATE_FIELD_WRAP_3D(DATA, PERSISTENT) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_3D(DATA=DATA, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_FIELD_WRAP_3D

  FUNCTION CREATE_FIELD_WRAP_4D(DATA, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 1D field with implicit blocking dimension by wrapping existing data
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_4D(DATA=DATA, PERSISTENT=PERSISTENT)
  ...
  FUNCTION CREATE_FIELD_WRAP_PACKED_4D(DATA, IDX) RESULT(FIELD_PTR)
    ! Create a single 1D field with implicit blocking dimension by wrapping existing data
    TYPE(FIELD_4D), POINTER :: FIELD_PTR
    REAL(KIND=JPRB), TARGET, INTENT(IN) :: DATA(:,:,:,:,:)
    INTEGER(KIND=JPIM), INTENT(IN) :: IDX
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_4D(DATA=DATA, IDX=IDX)
  ...
  FUNCTION CREATE_TEMPORARY_3D(GEOM, NLEV, PERSISTENT) RESULT(FIELD_PTR)
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_3D(SHAPE=[NPROMA, NLEV], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  END FUNCTION CREATE_TEMPORARY_3D

  FUNCTION CREATE_TEMPORARY_4D(GEOM, NLEV, NDIM, PERSISTENT) RESULT(FIELD_PTR)
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_4D(SHAPE=[NPROMA, NLEV, NDIM], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
  END FUNCTION CREATE_TEMPORARY_4D

  FUNCTION CREATE_TEMPORARY_5D(GEOM, NLEV, NDIM, NDIM2, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 4D field for temporary use with an implicit blocking dimension.
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_5D(SHAPE=[NPROMA, NLEV, NDIM, NDIM2], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
    ! Due to Fortran's reallocation-on-assignment behaviour, the
    ! FIELD_PTR%PTR pointer needs updating here for owned objects.
    IF (FIELD_PTR%OWNED) FIELD_PTR%PTR => FIELD_PTR%DATA
