...
USE FIELD_MODULE, ONLY: FIELD_2D, FIELD_INT2D, FIELD_LOG2D, FIELD_3D, FIELD_4D, FIELD_5D, FIELD_2D_EMPTY, FIELD_3D_EMPTY, FIELD_4D_EMPTY
...
CONTAINS
  ...
  PROCEDURE :: FIELD_REGISTRY_CREATE_VARIABLE_WRAP_2D, FIELD_REGISTRY_CREATE_VARIABLE_WRAP_3D
  PROCEDURE :: FIELD_REGISTRY_CREATE_VARIABLE_WRAP_4D
  GENERIC   :: CREATE_VARIABLE => &
   ...
   & FIELD_REGISTRY_CREATE_VARIABLE_WRAP_2D, FIELD_REGISTRY_CREATE_VARIABLE_WRAP_3D, &
   & FIELD_REGISTRY_CREATE_VARIABLE_WRAP_4D
...
INTERFACE CREATE_TEMPORARY_INT
  MODULE PROCEDURE CREATE_TEMPORARY_INT2D
...
INTERFACE DELETE_TEMPORARY
  ...
  MODULE PROCEDURE DELETE_TEMPORARY_3D, DELETE_TEMPORARY_4D, DELETE_TEMPORARY_5D
END INTERFACE DELETE_TEMPORARY

CONTAINS

  FUNCTION CREATE_FIELD_WRAP_2D(NAME, DATA, PERSISTENT) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_2D(NAME, DATA=DATA, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_FIELD_WRAP_2D

  FUNCTION CREATE_FIELD_WRAP_3D(NAME, DATA, PERSISTENT) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_3D(NAME, DATA=DATA, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_FIELD_WRAP_3D

  FUNCTION CREATE_FIELD_WRAP_4D(NAME, DATA, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 1D field with implicit blocking dimension by wrapping existing data
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_4D(NAME, DATA=DATA, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_FIELD_WRAP_4D

  FUNCTION CREATE_FIELD_EMPTY_2D(NAME, SHAPE) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_2D_EMPTY(NAME, SHAPE=SHAPE)
  END FUNCTION CREATE_FIELD_EMPTY_2D

  FUNCTION CREATE_FIELD_EMPTY_3D(NAME, SHAPE) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_3D_EMPTY(NAME, SHAPE=SHAPE)
  END FUNCTION CREATE_FIELD_EMPTY_3D

  FUNCTION CREATE_FIELD_EMPTY_4D(NAME, SHAPE) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_4D_EMPTY(NAME, SHAPE=SHAPE)
  END FUNCTION CREATE_FIELD_EMPTY_4D

  FUNCTION CREATE_TEMPORARY_2D(NAME, GEOM, PERSISTENT) RESULT(FIELD_PTR)
    ...
    FIELD_PTR = FIELD_2D(NAME, SHAPE=[NPROMA], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_TEMPORARY_2D

  FUNCTION CREATE_TEMPORARY_INT2D(NAME, GEOM, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 2D field for temporary use with an implicit blocking dimension.
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_INT2D(NAME, SHAPE=[NPROMA], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_TEMPORARY_INT2D

  FUNCTION CREATE_TEMPORARY_LOG2D(NAME, GEOM, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 2D field for temporary use with an implicit blocking dimension.
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_LOG2D(NAME, SHAPE=[NPROMA], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_TEMPORARY_LOG2D

  FUNCTION CREATE_TEMPORARY_3D(NAME, GEOM, NLEV, PERSISTENT) RESULT(FIELD_PTR)
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_3D(NAME, SHAPE=[NPROMA, NLEV], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_TEMPORARY_3D

  FUNCTION CREATE_TEMPORARY_4D(NAME, GEOM, NLEV, NDIM, PERSISTENT) RESULT(FIELD_PTR)
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_4D(NAME, SHAPE=[NPROMA, NLEV, NDIM], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  END FUNCTION CREATE_TEMPORARY_4D

  FUNCTION CREATE_TEMPORARY_5D(NAME, GEOM, NLEV, NDIM, PERSISTENT) RESULT(FIELD_PTR)
    ! Create a single 4D field for temporary use with an implicit blocking dimension.
    TYPE(FIELD_5D), POINTER :: FIELD_PTR
    CHARACTER(*), INTENT(IN) :: NAME
    TYPE(GEOMETRY), INTENT(IN) :: GEOM
    INTEGER(KIND=JPIM), INTENT(IN) :: NLEV, NDIM(2)
    LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT
    INTEGER(KIND=JPIM) :: NPROMA, NBLOCKS
    ...
    NBLOCKS = GEOM%YRDIM%NGPBLKS
    NPROMA = GEOM%YRDIM%NPROMA
    FIELD_PTR = FIELD_5D(NAME, SHAPE=[NPROMA, NLEV, NDIM(1), NDIM(2)], NBLOCKS=NBLOCKS, PERSISTENT=PERSISTENT)
  ...
  FUNCTION FIELD_REGISTRY_CREATE_VARIABLE_WRAP_2D(SELF, NAME, DATA) RESULT(VARIABLE)
    ...
    VARIABLE%FT0 => CREATE_FIELD_WRAP(NAME, DATA=DATA)
  ...
  FUNCTION FIELD_REGISTRY_CREATE_VARIABLE_2D_GMV(SELF, NAME, GMV, GMVT1, PT0, PT1, PT9, PPH9, PDL, PDM, &
   &                                             PDL9, PDM9, PERSISTENT) RESULT(VARIABLE)
    ...
    LOGICAL, OPTIONAL, INTENT(IN) :: PERSISTENT
    LOGICAL :: LT1, LT9, LPH9, LDL, LDM, LDL9, LDM9
    ...
    VARIABLE%FT0 => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PT0,:))

    IF (LT1) THEN
      VARIABLE%FT1 => CREATE_FIELD_WRAP(NAME, DATA=GMVT1(:,PT1,:))
    ELSE
      VARIABLE%FT1 => NULL()
    END IF
    IF (LT9) THEN
      VARIABLE%FT9 => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PT9,:))
    ELSE
      VARIABLE%FT9 => NULL()
    END IF
    IF (LPH9) THEN
      VARIABLE%FPH9 => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PPH9,:))
    ELSE
      VARIABLE%FPH9 => NULL()
    END IF
    IF (LDL) THEN
      VARIABLE%FDL => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PDL,:))
    ELSE
      VARIABLE%FDL => NULL()
    END IF
    IF (LDM) THEN
      VARIABLE%FDM => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PDM,:))
    ELSE
      VARIABLE%FDM => NULL()
    END IF
    IF (LDL9) THEN
      VARIABLE%FDL9 => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PDL9,:))
    ELSE
      VARIABLE%FDL9 => NULL()
    END IF
    IF (LDM9) THEN
      VARIABLE%FDM9 => CREATE_FIELD_WRAP(NAME, DATA=GMV(:,PDM9,:))
    ELSE
      VARIABLE%FDM9 => NULL()
    END IF
  ...
  FUNCTION FIELD_REGISTRY_CREATE_VARIABLE_GFL(SELF, NAME, YDCOMP, GFL, PERSISTENT) RESULT(VARIABLE)
    ...
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MP > 0) THEN
      VARIABLE%FT0 => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFL(:,:,YDCOMP%MP,:))
    ELSE
      VARIABLE%FT0 => NULL()
    END IF
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MP1 > 0) THEN
      VARIABLE%FT1 => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFLT1(:,:,YDCOMP%MP1,:))
    ELSE
      VARIABLE%FT1 => NULL()
    END IF
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MP9 > 0) THEN
      VARIABLE%FT9 => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFL(:,:,YDCOMP%MP9,:))
    ELSE
      VARIABLE%FT9 => NULL()
    END IF
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MP9_PH > 0) THEN
      VARIABLE%FPH9 => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFL(:,:,YDCOMP%MP9_PH,:))
    ELSE
      VARIABLE%FPH9 => NULL()
    END IF
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MPL > 0) THEN
      VARIABLE%FDL => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFL(:,:,YDCOMP%MPL,:))
    ELSE
      VARIABLE%FDL => NULL()
    END IF
    IF (YDCOMP%LACTIVE .AND. YDCOMP%MPM > 0) THEN
      VARIABLE%FDM => CREATE_FIELD_WRAP(NAME, DATA=GFL%GFL(:,:,YDCOMP%MPM,:))
    ELSE
      VARIABLE%FDM => NULL()
    END IF
  ...
  SUBROUTINE FIELD_REGISTRY_CREATE_SURFACE_VARIABLE_2D(SELF, NAME, VARIABLE, ARRAY, META, DESCR, ACTIVE, PERSISTENT)
    ...
    IF (.NOT. ACTIVE) THEN
      ...
      VARIABLE%FT0 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
      VARIABLE%FT1 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
      VARIABLE%FT9 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
    ELSE
      ! Create variable according to existing meta-data
      VARIABLE = VARIABLE_2D(NAME, LACTIVE=META%MP > 0, LT1=META%MP1 > 0, LT9=META%MP9 > 0)

      IF (META%MP > 0) THEN
        VARIABLE%FT0 => CREATE_FIELD_WRAP(NAME, DATA=ARRAY(:,META%MP,:))
      ELSE
        VARIABLE%FT0 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
      END IF
      IF (META%MP1 > 0) THEN
        VARIABLE%FT1 => CREATE_FIELD_WRAP(NAME, DATA=ARRAY(:,META%MP1,:))
      ELSE
        VARIABLE%FT1 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
      END IF
      IF (META%MP9 > 0) THEN
        VARIABLE%FT9 => CREATE_FIELD_WRAP(NAME, DATA=ARRAY(:,META%MP9,:))
      ELSE
        VARIABLE%FT9 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, PERSISTENT=PERSISTENT)
      END IF
    END IF
  ...
  SUBROUTINE FIELD_REGISTRY_CREATE_SURFACE_VARIABLE_3D(SELF, NAME, VARIABLE, ARRAY, META, DESCR, ACTIVE, PERSISTENT)
    ...
    IF (.NOT. ACTIVE) THEN
      ...
      VARIABLE%FT0 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, NLEV=DESCR%NLEVS, PERSISTENT=PERSISTENT)
      VARIABLE%FT1 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, NLEV=DESCR%NLEVS, PERSISTENT=PERSISTENT)
      VARIABLE%FT9 => CREATE_TEMPORARY(NAME, GEOM=SELF%GEOM, NLEV=DESCR%NLEVS, PERSISTENT=PERSISTENT)
    ELSE
      ! Create variable according to existing meta-data
  ...
  SUBROUTINE FIELD_REGISTRY_REGISTER_SURFACE_FIELDS(SELF, SURFVARS, YDSURF)
    ...
    SURFVARS%GSP_SB%F_GROUP = FIELD_4D(NAME='SP_SB', DATA=SP_SB)
    END ASSOCIATE
    ...
    SURFVARS%GSP_SG%F_GROUP = FIELD_4D(NAME='SP_SG', DATA=SP_SG)
    END ASSOCIATE
    ...
    SURFVARS%GSP_SL%F_GROUP = FIELD_3D(NAME='SP_SL', DATA=SP_SL)
    END ASSOCIATE
    ...
    SURFVARS%GSP_RR%F_GROUP = FIELD_3D(NAME='SP_RR', DATA=SP_RR)
    END ASSOCIATE
    ...
    SURFVARS%GSP_CL%F_GROUP = FIELD_3D(NAME='SP_CL', DATA=SP_CL)
    END ASSOCIATE
    ...
    SURFVARS%GSP_OM%F_GROUP = FIELD_4D(NAME='SP_OM', DATA=SP_OM)
    END ASSOCIATE

    ASSOCIATE(SP_EP=>YDSURF%SP_EP, YSP_EP=>YDSURF%YSP_EP, YSP_EPD=>YDSURF%YSP_EPD)
    SURFVARS%GSP_EP%F_GROUP = FIELD_4D(NAME='SP_EP', DATA=SP_EP)
    END ASSOCIATE

    ASSOCIATE(SP_X2=>YDSURF%SP_X2, YSP_X2=>YDSURF%YSP_X2, YSP_X2D=>YDSURF%YSP_X2D)
    SURFVARS%GSP_X2%F_GROUP = FIELD_3D(NAME='SP_X2', DATA=SP_X2)
    END ASSOCIATE

    ASSOCIATE(SP_CI=>YDSURF%SP_CI, YSP_CI=>YDSURF%YSP_CI, YSP_CID=>YDSURF%YSP_CID)
    SURFVARS%GSP_CI%F_GROUP = FIELD_3D(NAME='SP_CI', DATA=SP_CI)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VF%F_GROUP = FIELD_3D(NAME='SD_VF', DATA=SD_VF)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VH%F_GROUP = FIELD_3D(NAME='SD_VH', DATA=SD_VH)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VK%F_GROUP = FIELD_3D(NAME='SD_VK', DATA=SD_VK)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VP%F_GROUP = FIELD_3D(NAME='SD_VP', DATA=SD_VP)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VV%F_GROUP = FIELD_3D(NAME='SD_VV', DATA=SD_VV)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VA%F_GROUP = FIELD_3D(NAME='SD_VA', DATA=SD_VA)
    END ASSOCIATE
    ...
    SURFVARS%GSD_VN%F_GROUP = FIELD_3D(NAME='SD_VN', DATA=SD_VN)
    END ASSOCIATE
    ...
    SURFVARS%GSD_V2%F_GROUP = FIELD_3D(NAME='SD_V2', DATA=SD_V2)
    END ASSOCIATE
    ...
    SURFVARS%GSD_V3%F_GROUP = FIELD_4D(NAME='SD_V3', DATA=SD_V3)
    END ASSOCIATE
    ...
    END IF
    SURFVARS%GSD_VD%F_GROUP = FIELD_3D(NAME='SD_VD', DATA=SD_VD)
    END ASSOCIATE
    ...
    SURFVARS%GSD_SM%F_GROUP = FIELD_4D(NAME='SD_SM', DATA=SD_SM)
    END ASSOCIATE
    ...
    SURFVARS%GSD_WS%F_GROUP = FIELD_3D(NAME='SD_WS', DATA=SD_WS)
    END ASSOCIATE
    ...
    SURFVARS%GSD_WW%F_GROUP = FIELD_3D(NAME='SD_WW', DATA=SD_WW)
    END ASSOCIATE
    ASSOCIATE(SD_XA=>YDSURF%SD_XA, YSD_XA=>YDSURF%YSD_XA, YSD_XAD=>YDSURF%YSD_XAD)
    SURFVARS%GSD_XA%F_GROUP = FIELD_4D(NAME='SD_XA', DATA=SD_XA)
    END ASSOCIATE
    ...
    SURFVARS%GSD_DI%F_GROUP = FIELD_4D(NAME='SD_DI', DATA=SD_DI)
    END ASSOCIATE
    ...
    SURFVARS%GSD_XP%F_GROUP = FIELD_4D(NAME='SD_XP', DATA=SD_XP)
    END ASSOCIATE
