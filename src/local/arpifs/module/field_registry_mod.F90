...
CONTAINS
  ...
  FUNCTION CREATE_FIELD_WRAP_2D(NAME, DATA) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_2D(NAME, DATA=DATA)
  END FUNCTION CREATE_FIELD_WRAP_2D

  FUNCTION CREATE_FIELD_WRAP_3D(NAME, DATA) RESULT(FIELD_PTR)
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_3D(NAME, DATA=DATA)
  END FUNCTION CREATE_FIELD_WRAP_3D

  FUNCTION CREATE_FIELD_WRAP_4D(NAME, DATA) RESULT(FIELD_PTR)
    ! Create a single 1D field with implicit blocking dimension by wrapping existing data
    ...
    ALLOCATE(FIELD_PTR)
    FIELD_PTR = FIELD_4D(NAME, DATA=DATA)
  ...
  FUNCTION FIELD_REGISTRY_CREATE_VARIABLE_GFL(SELF, NAME, YDCOMP, GFL, PERSISTENT) RESULT(VARIABLE)
    ...
    VARIABLE = VARIABLE_3D(NAME, CNAME=YDCOMP%CNAME, IGRBCODE=YDCOMP%IGRBCODE, LACTIVE=YDCOMP%LACTIVE, &
     &                     LADV=YDCOMP%LADV, LT1=YDCOMP%LT1, LT9=YDCOMP%LT9, LPH9=YDCOMP%LPHY)
