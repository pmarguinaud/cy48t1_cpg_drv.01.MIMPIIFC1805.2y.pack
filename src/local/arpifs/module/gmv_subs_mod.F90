...
USE YOMDYNA   , ONLY : LGWADV, NVDVAR, LRDBBC, LNHX ,LNHXDER, LGRADSP, LRUBC,&
 &                     LPC_FULL, LNESC, LSLINL, LSLINLC2, LNHQE_SIHYD
...
CALL SETUP_T0(YDDIMF,YT5)
IF( LGRADSP ) YT5%NDIM=YT5%NDIM-2
  ...
  IF (LTWOTL) THEN
    ...
    ELSE
      ...
      IF( LPC_FULL )THEN
        ...
        ENDIF
      ENDIF
      IF(LPC_FULL.AND.(LMPHYS.OR.LEPHYS))THEN  
        IPT = IPT+1
        YDT9%MCUPT = IPT
        IPT = IPT+1
        YDT9%MCVPT = IPT
