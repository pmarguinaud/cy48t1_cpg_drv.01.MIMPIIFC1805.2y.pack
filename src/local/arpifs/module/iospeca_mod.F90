...
! Compute PD,VD
    ...
    IF (LNHDYN) THEN
      ...
      IF (LNHX) ALLOCATE (YDCTX%RSPNHX (YDGEOMETRY%YRDIMV%NFLSUR,YDGEOMETRY%YRDIM%NSPEC2))
 
! Copy to avoid overwriting
      YDCTX%RSPSPD (:,:) = YDSPEC%SPD (:,:)
      YDCTX%RSPSVD (:,:) = YDSPEC%SVD (:,:)
      IF (LNHX) YDCTX%RSPNHX (:,:) = YDSPEC%NHX (:,:)
...
! Restore NH model variables in spectral arrays
    ...
    IF (LNHDYN) THEN
      ...
      IF (LNHX) YDSPEC%NHX (:,:) = YDCTX%RSPNHX (:,:)
    
      DEALLOCATE (YDCTX%RSPSPD)
      DEALLOCATE (YDCTX%RSPSVD)
      IF (LNHX) DEALLOCATE (YDCTX%RSPNHX)
    ENDIF
