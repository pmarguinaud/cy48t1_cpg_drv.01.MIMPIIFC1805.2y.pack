...
SELF%F_DIFCQ => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFCQL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFCQN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFCS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFTQ => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFTQL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFTQN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFTS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_EDR => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_FCCQL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCCQN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCSQL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCSQN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCQLNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCQNNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCQNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLCL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLCN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLCG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLCH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLSG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPLSH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPFPSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPFPSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPFPSG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPFPCL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPFPCN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPSG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPCL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPCN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FPEVPCG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FRSO => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, NDIM=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_FRTH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, NDIM=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_STRCU => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRCV => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRDU => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRDV => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRTU => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRTV => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRMU => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_STRMV => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFCQLC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_DIFCQIC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FIMCC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FEDQLC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FEDQIC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FEDQRC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FEDQSC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCNEGQLC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCNEGQIC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCNEGQRC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCNEGQSC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FRMH => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCHOZ => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FDIS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPSG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPCL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPCN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHSCL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHSCN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHPCG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHSSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHSSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FEPFP => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCMPCQ => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCMPSN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FHSSG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCMPSL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FRSOC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=1+1, PERSISTENT=PERSISTENT)

SELF%F_FRTHC => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=1+1, PERSISTENT=PERSISTENT)

SELF%F_FCHSP => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%YRSURF%YSP_SBD%NLEVS, PERSISTENT=PERSISTENT)

SELF%F_FCLL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_FCLN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_FCS => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_FEVL => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)

SELF%F_FEVN => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KTSSG+1, PERSISTENT=PERSISTENT)
...
SELF%F_TENDU => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_TENDV => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG, PERSISTENT=PERSISTENT)

SELF%F_FCQRNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCQSNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)

SELF%F_FCQGNG => CREATE_TEMPORARY (GEOM=REGISTRY%GEOM, NLEV=YDCPG_DIM%KFLEVG+1, PERSISTENT=PERSISTENT)
...
IF (LLMF_PHYS) THEN
  IF (PRESENT (PGDEOSI)) THEN
    SELF%F_GDEOSI => CREATE_FIELD_WRAP (PGDEOSI)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGUEOSI)) THEN
    SELF%F_GUEOSI => CREATE_FIELD_WRAP (PGUEOSI)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGMU0)) THEN
    SELF%F_GMU0 => CREATE_FIELD_WRAP (PGMU0)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGMU0_MIN)) THEN
    SELF%F_GMU0_MIN => CREATE_FIELD_WRAP (PGMU0_MIN)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGMU0_MAX)) THEN
    SELF%F_GMU0_MAX => CREATE_FIELD_WRAP (PGMU0_MAX)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGDEOTI)) THEN
    SELF%F_GDEOTI => CREATE_FIELD_WRAP (PGDEOTI)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGDEOTI2)) THEN
    SELF%F_GDEOTI2 => CREATE_FIELD_WRAP (PGDEOTI2)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGUEOTI)) THEN
    SELF%F_GUEOTI => CREATE_FIELD_WRAP (PGUEOTI)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGUEOTI2)) THEN
    SELF%F_GUEOTI2 => CREATE_FIELD_WRAP (PGUEOTI2)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGEOLT)) THEN
    SELF%F_GEOLT => CREATE_FIELD_WRAP (PGEOLT)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGEOXT)) THEN
    SELF%F_GEOXT => CREATE_FIELD_WRAP (PGEOXT)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGRPROX)) THEN
    SELF%F_GRPROX => CREATE_FIELD_WRAP (PGRPROX)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGMIXP)) THEN
    SELF%F_GMIXP => CREATE_FIELD_WRAP (PGMIXP)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGFLUXC)) THEN
    SELF%F_GFLUXC => CREATE_FIELD_WRAP (PGFLUXC)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PGRSURF)) THEN
    SELF%F_GRSURF => CREATE_FIELD_WRAP (PGRSURF)
  ENDIF
ENDIF
...
IF (LLMF_PHYS) THEN
  IF (PRESENT (PU)) THEN
    SELF%F_U => CREATE_FIELD_WRAP (PU)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PV)) THEN
    SELF%F_V => CREATE_FIELD_WRAP (PV)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PT)) THEN
    SELF%F_T => CREATE_FIELD_WRAP (PT)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PQ)) THEN
    SELF%F_Q => CREATE_FIELD_WRAP (PQ)
  ENDIF
ENDIF
...
IF (LLMF_PHYS) THEN
  IF (PRESENT (PEMTD)) THEN
    SELF%F_EMTD => CREATE_FIELD_WRAP (PEMTD)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PEMTU)) THEN
    SELF%F_EMTU => CREATE_FIELD_WRAP (PEMTU)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PTRSW)) THEN
    SELF%F_TRSW => CREATE_FIELD_WRAP (PTRSW)
  ENDIF
ENDIF

IF (LLMF_PHYS) THEN
  IF (PRESENT (PRMOON)) THEN
    SELF%F_RMOON => CREATE_FIELD_WRAP (PRMOON)
  ENDIF
ENDIF
