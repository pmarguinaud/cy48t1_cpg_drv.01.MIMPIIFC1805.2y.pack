...
INTEGER(KIND=JPIM) :: ISTUV,IENUV,ISTGMV,IENGMV,IDIMGMVS
!     ------------------------------------------------------------------
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('TRANSDIR_MDL_FROM_T0_AD',0,ZHOOK_HANDLE)
...
! Normal configuration (u,v grid-point to vor,div spectral)
  ...
  ELSE
    WRITE(NULOUT,*) 'think I want to be calling this dir_transad ...'
    CALL DIR_TRANSAD(PSPVOR=YDSP%VOR,PSPDIV=YDSP%DIV,&
     & PSPSC2=ZSP(1:IST,:),PSPSC3A=YDSP%HV,&
     & KRESOL=NRESOL,KPROMA=NPROMA,KVSETUV=NBSETLEV,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3A=NBSETLEV,&
...
! Normal configuration (u,v grid-point to vor,div spectral)
  IF(NUMSPFLDS1>0) THEN
    CALL DIR_TRANSAD(PSPVOR=YDSP%VOR,PSPDIV=YDSP%DIV,PSPSC2=ZSP(1:IST,:),&
     & PSPSC3B=YDSP%GFL,&
     & KRESOL=NRESOL,KPROMA=NPROMA,KVSETUV=NBSETLEV,KVSETSC2=IVSETSC(1:1),&
     & KVSETSC3B=NBSETLEV,&
     & PGPUV=GMV(:,:,ISTUV:IENUV,:),PGP2=GMVS(:,1:YT1%NDIMS,:),&
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('TRANSDIR_MDL_FROM_T0_AD',1,ZHOOK_HANDLE)
