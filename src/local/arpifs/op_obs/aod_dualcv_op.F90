...
!*** *AOD_DUALCV_OP* Operator routine for aerosol optical depth observations.
! To be used when NAEROCV=2

!**   INTERFACE.
!     ----------
!          *AOD_DUALCV_OP* IS CALLED FROM *HOP*|*OBSOP_COMPOSITION* (oops design).
!     CALL AOD_DIALCV_OP(KFLEVG,KDLEN,ILEN,IMXCOUNT,ZQF, ZTF, ZPRESH, ZPRESF, ZGEMSPROF, ZXPP)
...
!           ZGEMSPROF = GEMS variables (INPUT)
...
! A. Geer      29 Dec 2015   Final removal of GEMSPROF and transition to PPNEW
...
! Optical properties in su_aerop.F90 available at following wavelenghts
...
USE YOM_GRIB_CODES, ONLY : NGRBAERMR01,NGRBAERMR02, NGRBAERMR03, NGRBAERMR04, NGRBAERMR05, &
 ...
 & NGRBAERMR19, NGRBAERMR20, NGRBAERMR21
...
INTEGER(KIND=JPIM) :: IACTAEROP ! Number of optically active aerosol species 
...
!-----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('AOD_DUALCV_OP',0,ZHOOK_HANDLE)
ASSOCIATE( &
...
!     IWAVLINDEX=IWAVL(JJ)
  ...
  DO JL=1,KLEN
   DO JAER=1,IACTAEROP  ! SO2 does not contribute to aerosol optical depth
...
! This indeces are defined in su_aerw.F90 
         ...
         IF(ITYP /= 0 .AND. IBIN == 0) THEN
          WRITE(NULERR,*)'AOD_DUALCV_OP:AT LEAST ONE BIN NEEDS TO BE ALLOCATED FOR TYPE ', ITYP
          WRITE(NULERR,*) 'PROGRAM WILL STOP'
          CALL ABOR1('') 
         ENDIF
         IF(ITYP /= ITYPE(JAER)) THEN
          WRITE(NULERR,*)'AOD_DUALCV_OP: AEROSOL TYPE IS NOT MATCHING', ITYP
          WRITE(NULERR,*) 'PROGRAM WILL STOP'
          CALL ABOR1('')
         ENDIF
...
! It has changed a number of times!!!!!!!!
         ZFAC=1.0_JPRB
         IF (ITYP == 1) THEN
          ZALF(JAER)=ALF_SS(IRH(JL,JK),IWAVLINDEX,IBIN)
         ...
         ELSEIF (ITYP == 6) THEN ! Nitrate
            ZALF(JAER)=ALF_NI(IRH(JL,JK),IWAVLINDEX,IBIN)
         ELSEIF (ITYP == 7) THEN ! Ammonium
            ZALF(JAER)=ALF_AM(IRH(JL,JK),IWAVLINDEX)
         ELSEIF (ITYP == 8) THEN ! Nitrate
            ZALF(JAER)=ALF_SOA(IRH(JL,JK),IWAVLINDEX,IBIN)
...
!-----------------------------------------------------------------------
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('AOD_DUALCV_OP',1,ZHOOK_HANDLE)
