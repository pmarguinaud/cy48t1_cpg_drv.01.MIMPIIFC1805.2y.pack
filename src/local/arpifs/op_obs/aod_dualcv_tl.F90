...
!*** *AOD_DUALCV_TL* Tangent Linear Operator routine for aerosol optical depth observations.

!**   INTERFACE.
!     ----------
!          *AOD_DUALCV_TL* IS CALLED FROM *HOPTL*|*OBSOP_COMPOSITION* (oops design).
!     CALL AOD_DUALCV_TL(KFLEVG,KDLEN,ILEN,IMXCOUNT,ZQF, ZTF, ZPRESH, ZPRESF, ZLS, ZGEMSPROF,
!                  ZQF5, ZTF5, ZPRESH5, ZPRESF5, ZLS5, ZGEMSPROF5, ZXPP)
...
!-----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('AOD_DUALCV_TL',0,ZHOOK_HANDLE)
ASSOCIATE(&
...
! optically active species
 ...
 IF(( JPACTAERSM + JPACTAERLG) > INACTAERO)  THEN
  CALL ABOR1('AOD_DUALCV_TL:SUM OF FINE AND COARSE EXCEEDS INACTAERO')
 ENDIF
...
! Fine mode 
    DO JAER=1,JPACTAERSM
      ...
      ENDIF
      ! Assign perturbations to single species using fractional contribution to fine mode mixing ratio
       ZAER(JLEN,JLEV,IACTAERSM(JAER))=ZAERSMMR(JLEN,JLEV) * ZFRAERSM5(JLEN,JLEV,IACTAERSM(JAER))
    ENDDO
! Coarse mode
    DO JAER=1,JPACTAERLG
      ...
      ENDIF
     ! Assign perturbations to single species using fractional contribution to coarse mode mixing ratio
       ZAER(JLEN,JLEV,IACTAERLG(JAER))=ZAERLGMR(JLEN,JLEV) * ZFRAERLG5(JLEN,JLEV,IACTAERLG(JAER))
    ENDDO
...
!      ENDIF
!        ZAER(JLEN,JLEV,JAER)=ZAERTMR(JLEN,JLEV) * ZFRACAER5(JLEN,JLEV,JAER)
!    ENDDO
!  ENDDO
!ENDDO
...
!   IBIN is the bin index: 1-3:SS, 1-3:DD, 1:OM, 1:BC, 1:SU, 1:FA
     ...
     IF(ITYP /= ITYPE(JAER)) THEN
      WRITE(NULERR,*)'AOD_DUALCV_TL: AEROSOL TYPE IS NOT MATCHING', ITYP
      WRITE(NULERR,*) 'PROGRAM WILL STOP'
      CALL ABOR1('')
     ENDIF
...
!-- use dust for 0.9-20 um bin
        ...
        ZAEREXT=ZAER(JL,JK,JAER)*ZFAC*ZALF5(JAER)
...
!-----------------------------------------------------------------------
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('AOD_DUALCV_TL',1,ZHOOK_HANDLE)
