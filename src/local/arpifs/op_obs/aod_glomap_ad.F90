...
!*** *AOD_GLOMAP_AD* Adjoint Operator routine for aerosol optical depth observations with the GLOMAP model.
!
!**   INTERFACE.
!     ----------
!
...
!           YDGP_AD   = Perturbs in variables at observation points, model levels (INPUT)
!           YDGP5     = Nonlinear trajectory values at observation points, model levels (INPUT)
!
!           PAEROD_HAT = Aerosol optical depth at different wavelengths (OUTPUT)
!
!
!**   AUTHOR.
!     -------
...
!     PURPOSE.
!     --------
!     - derive adjoint aerosol optical thickness from profiles of mode number concentration and component mass mixing ratio 
!
!----------------------------------------------------------------------- 
...
!Variables that describe the adjoint
REAL(KIND=JPRB), DIMENSION(KLEN,KFLEVG,NMODES) :: ZMODAL_MMR !Glomap: mass mixing ratio of each mode 
REAL(KIND=JPRB), DIMENSION(KLEN,KFLEVG)   :: ZTMMR ! total aer mixing ratios pert

!Variables for calcuting the AOD
REAL(KIND=JPRB), DIMENSION(KLEN,KMXCOUNT) :: ZAEROD5
REAL(KIND=JPRB), DIMENSION(KLEN,NMODES)   :: ZAERTAU5, ZAERTAU
REAL(KIND=JPRB), DIMENSION(KLEN,KFLEVG)   :: ZDP5
REAL(KIND=JPRB)    :: ZDP
REAL(KIND=JPRB)    :: ZAEREXT5, ZAEREXT
...
!-----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('AOD_GLOMAP_AD',0,ZHOOK_HANDLE)
...
!    !Check that this structure still matches ifs/module/ukca_mode_setup.F90
    DO JF=1,YDGP5%NGEMS
        ...
        IF(YDGP5%GEMS_IGRIB(JF) == NGRBAERLG) THEN ! Total aerosol mixing ratio and pert
           ZTMMR5(JLEN,:)=YDGP5%GEMS(JLEN,1:KFLEVG,IH,JF)
        ENDIF
    ENDDO
    ...
    DO JMODE=2,NMODES !Modes - exclude nucleation mode
      ...
      DO JK=1,KFLEVG !Model level
        ...
        IF(MODE(JMODE)) THEN
            ...
            ZAERTAU5(JL,JMODE)=ZAERTAU5(JL,JMODE)+ ZAEREXT5*(ZDP5(JL,JK)/RG)
        ENDIF
...
! Initialize local variables
      ZDP     = 0.0_JPRB
      ZAEREXT = 0.0_JPRB
      ZAERTAU(JL,JMODE)=0.0_JPRB

      ZAERTAU(JL,JMODE)= ZAERTAU(JL,JMODE)+ PAEROD_HAT(JL,JJ)
      ...
      DO JK=KFLEVG,1,-1
         ...
         !Extinction coefficient times mmr for full nonlinear model
         ZAEREXT5 = ZMODAL_MMR5(JL,JK,JMODE)*(ZLOC_ABS5+ZLOC_SCA5)
 
         ZAEREXT = ZAEREXT + (ZDP5(JL,JK)/RG)* ZAERTAU(JL,JMODE)
         ZDP    = ZDP + ZAEREXT5/RG * ZAERTAU(JL,JMODE)  
         ! For now set ZDP to zero here and avoid increments in surface pressure
         ZDP= 0.0_JPRB
   
         ZMODAL_MMR(JL,JK,JMODE) = ZMODAL_MMR(JL,JK,JMODE) + (ZLOC_ABS5+ZLOC_SCA5)*ZAEREXT
         ZAEREXT= 0.0_JPRB
...
! The gradient of the total mass mixing ratio are computed from the gradient of
! the various species using the mass fraction assumed constant 
ZTMMR(:,:)=0.0_JPRB
DO JLEN=1,KLEN
  DO JMODE=NMODES,2,-1
    ZTMMR(JLEN,:) = ZTMMR(JLEN,:) + ZMODAL_MMR(JLEN,:,JMODE)*ZFRACMMR5(JLEN,:,JMODE)
    ZMODAL_MMR(JLEN,:,JMODE)=0.0_JPRB
  ENDDO
ENDDO
...
DO JLEN=1,KLEN
  DO JF=1,YDGP5%NGEMS
    IF(YDGP5%GEMS_IGRIB(JF) == 210048) THEN 
      YDGP_AD%GEMS(JLEN,1:KFLEVG,IH,JF) = YDGP_AD%GEMS(JLEN,1:KFLEVG,IH,JF) + ZTMMR(JLEN,:)
      ZTMMR(JLEN,:)=0.0_JPRB
    ENDIF
  ENDDO
ENDDO
...
!-----------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('AOD_GLOMAP_AD',1,ZHOOK_HANDLE)
