!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACCDEV ( YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
 !-----------------------------------------------------------------------
 ! - INPUT  1D VERTICAL.
 & PVETAF,&
 ! - INPUT  2D .
...
! Prevent melting of existing ice below RTT.
        ...
        ZDELTA=ZNEIGE*MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-PT(JLON,JLEV)))
...
! Define the coefficients for Condensation - Evaporation calculations
     ...
     & ZNEWRAIN)
      ...
      ZMELTHEAT = -(RLSTT-RLVTT) * ZNEWRAIN * RG/RCPD/PDELP(JLON,JLEV)
...
!PART 2: Compute and define the coefficients for C-E calculations
      ...
      ZCGAMMA(JLON)=ZCALPHA(JLON)+RLVTT*ZCBETA(JLON)/RCPD
      ZCGAMAH(JLON)=ZCALPHA(JLON)+RLVTT*ZCBETAH(JLON)/RCPD
...
!First iteration
      ZTTEMP=PT(JLON,JLEV)+TSPHY*(PRKTTEND(JLON,JLEV)+(1._JPRB/RCPD)*&
     & ((RLVTT+(RLSTT-RLVTT)*ZICE)*ZCME(JLON,JLEV))) 
...
! Convert from tendencies to fluxe divergence:
      ZFCSQL(JLON,JLEV)=ZFCSQL(JLON,JLEV-1)+(1._JPRB/RG)*PDELP(JLON,JLEV)*ZCMEL
      ZFCSQN(JLON,JLEV)=ZFCSQN(JLON,JLEV-1)+(1._JPRB/RG)*PDELP(JLON,JLEV)*ZCMEI
...
IF (LSMITH_CDEV) THEN
  ...
  CALL ACNEBSM (YDML_PHY_MF%YRPHY0, &
