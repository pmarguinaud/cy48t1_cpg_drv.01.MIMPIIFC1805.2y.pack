!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACDRAGL (YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
 !-----------------------------------------------------------------------
 ! - INPUT  2D .
...
!     CORRECTION FOR SHALLOW CONVECTION (OPTIONAL).
    ...
    IF (LCVPP) THEN
      ZAUX=MIN(0.0_JPRB,PQ(JLON,JLEV)-ZQSAT(JLON,JLEV)-PQ(JLON,JLEV+1)&
       ...
       & +ZLSCPE(JLON,JLEV+1)))*MAX(0.0_JPRB,SIGN(1.0_JPRB,RD&
...
!     STABILITY COMPUTATION.
  ...
  ZRTI=2.0_JPRB/(ZR(JLON,KLEV)*PT(JLON,KLEV)+RKAPPA*ZDPHI+ZRS(JLON)*PTS(JLON))
  ZSTA=ZDPHI*(ZR(JLON,KLEV)*PT(JLON,KLEV)+RKAPPA&
