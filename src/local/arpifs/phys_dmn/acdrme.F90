...
DO JLEV=KTDIA,KLEV
  DO JLON=KIDIA,KFDIA
    ZTENU(JLON,JLEV)= - YDTOPH%RMESOU(JLEV)/&
     & (1.0_JPRB+YDTOPH%RMESOU(JLEV)*ZALPHA*TSPHY) * (PU(JLON,JLEV)-ZUREL(JLEV) )  
    ZTENV(JLON,JLEV)= - YDTOPH%RMESOU(JLEV)/&
     & (1.0_JPRB+YDTOPH%RMESOU(JLEV)*ZALPHA*TSPHY) * (PV(JLON,JLEV)-ZVREL(JLEV) )  
    ZTENT(JLON,JLEV)= - YDTOPH%RMESOT(JLEV)/&
     & (1.0_JPRB+YDTOPH%RMESOT(JLEV)*ZALPHA*TSPHY) * (PT(JLON,JLEV)-ZTREL(JLEV) )  
    ZTENQ(JLON,JLEV)= - YDTOPH%RMESOQ(JLEV)/&
     & (1.0_JPRB+YDTOPH%RMESOQ(JLEV)*ZALPHA*TSPHY) * (PQ(JLON,JLEV)-ZQREL(JLEV) )  
  ENDDO
ENDDO
