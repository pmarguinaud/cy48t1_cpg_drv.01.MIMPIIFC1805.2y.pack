!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACDROV ( YDML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,KCSS,&
 !-----------------------------------------------------------------------
 ! - INPUT  2D .
...
USE YOMCST   , ONLY : RDAY     ,RLMLT    ,RTT
...
!*******************************************************************************
  ...
  IF (LNEIGE) THEN
    ...
    IF (LSNV) THEN
      ...
      PFONTE(JLON)=MAX(0.0_JPRB,(ZTF-(PLSM(JLON)*RTT&
       & +(1.0_JPRB-PLSM(JLON))*TMERGL))&
       & /(MAX(ZEPS1,ZPCTDT)*RLMLT))  
    ...
    ELSE
      PFONTE(JLON)=PLSM(JLON)&
       ...
       & *RTT+(1.0_JPRB-PLSM(JLON))*TMERGL))&
       & /(MAX(ZEPS1,ZPCTDT)*RLMLT)))  
    ENDIF

    ZTSP = ZTSP - ZPCTDT * RLMLT * PFONTE(JLON)
