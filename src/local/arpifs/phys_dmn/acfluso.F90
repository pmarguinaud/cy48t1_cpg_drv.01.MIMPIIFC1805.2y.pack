...
DO JLON=KIDIA,KFDIA
  ...
  ZCIS (JLON) = PU(JLON,KLEV)**2 + PV(JLON,KLEV)**2&
              & + ( GEVAVF * ZDPHI(JLON) / YDCST%RG )**2
  ZDU  (JLON) = SQRT( ZCIS(JLON) )
  ZT   (JLON) = PT(JLON,KLEV) - YDCST%RTT
  ZTS  (JLON) = PTS(JLON)     - YDCST%RTT
  ZQ   (JLON) = PQ(JLON,KLEV)
  ZHT  (JLON) = ( PAPHIF(JLON,KLEV) - PAPHI(JLON,KLEV) ) / YDCST%RG
  ZDQ  (JLON) = ZQ(JLON) - PQSATS(JLON)
  ZDTH (JLON) = ZT(JLON) - ZTS(JLON) + ZHT(JLON) * YDCST%RG / YDCST%RCPD
...
DO JJ=1,NITERFL
  DO JLON=KIDIA,KFDIA
    ...
    ZLMO=YDCST%RG*VKARMN*ZHT(JLON)*(ZTSR2*(1.0_JPRB+YDCST%RETV*ZQ(JLON))+YDCST%RETV&
     & *(ZT(JLON)+YDCST%RTT)*ZQSR2)/((ZT(JLON)+YDCST%RTT)*(1.0_JPRB+YDCST%RETV*ZQ(JLON))&
     & *ZUSR2*ZUSR2)  
...
DO JLON=KIDIA,KFDIA
  ...
  IF (LDHMT) THEN
    ZLMOUCLS=VKARMN*PDPHIU(JLON)*(ZTSR(JLON)*(1.0_JPRB+YDCST%RETV*ZQ(JLON))+YDCST%RETV&
     & *(ZT(JLON)+YDCST%RTT)*ZQSR(JLON))/((ZT(JLON)+YDCST%RTT)*(1.0_JPRB+YDCST%RETV*ZQ(JLON))&
     & *ZUSR(JLON)*ZUSR(JLON))  
    ...
    PTCLS(JLON)=ZLSM0*PTCLS(JLON)&
     & +ZLSM1*(ZDTCLS+PTS(JLON)-PDPHIT(JLON)/YDCST%RCPD)  
    ...
    ELSE
      ZDELTA=0.0_JPRB
    ENDIF
    ZPCLS=PAPRS(JLON,KLEV)+PDPHIT(JLON)/(YDCST%RG*ZHT(JLON))&
     & *(PAPRSF(JLON,KLEV)-PAPRS(JLON,KLEV))  
    ZEW= FOEW (PTCLS(JLON),ZDELTA)
    PRHCLS(JLON)=ZLSM0*PRHCLS(JLON)&
     & +ZLSM1*(ZPCLS*PQCLS(JLON)*(YDCST%RETV+1.0_JPRB))&
