!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACHMT (YDPHY, YDPHY0, YDPHY1, YDPHY2, YDCST, KIDIA,KFDIA,KLON,KLEV,LDZ0H,&
 !-----------------------------------------------------------------------
 ! - INPUT  2D .
...
ASSOCIATE(GCISMIN=>YDPHY0%GCISMIN, VKARMN=>YDPHY0%VKARMN, &
 & UTILGUST=>YDPHY0%UTILGUST, VZIUSTAR0=>YDPHY0%VZIUSTAR0, &
 & USURICL=>YDPHY0%USURICL, EDB=>YDPHY0%EDB, EDC=>YDPHY0%EDC, EDD=>YDPHY0%EDD, &
 & EDK=>YDPHY0%EDK, RRSCALE=>YDPHY0%RRSCALE, RRGAMMA=>YDPHY0%RRGAMMA, &
 & USURIC=>YDPHY0%USURIC, USURID=>YDPHY0%USURID, &
 & RD1=>YDPHY1%RD1, GCZ0H=>YDPHY1%GCZ0H, WSMX=>YDPHY1%WSMX, &
 & ALRCN2=>YDPHY1%ALRCN2, ALRCN1=>YDPHY1%ALRCN1, WCRIN=>YDPHY1%WCRIN, &
 & GCONV=>YDPHY1%GCONV, RZHZ0M=>YDPHY1%RZHZ0M, &
 & TSPHY=>YDPHY2%TSPHY, GZ0RAF=>YDPHY2%GZ0RAF, LRAFTUR=>YDPHY2%LRAFTUR, &
 & XMULAF=>YDPHY2%XMULAF, FACRAF=>YDPHY2%FACRAF, &
 & LSOLV=>YDPHY%LSOLV, LNEIGE=>YDPHY%LNEIGE, LSNV=>YDPHY%LSNV, &
 & LZ0HSREL=>YDPHY%LZ0HSREL, LRRGUST=>YDPHY%LRRGUST, LFGEL=>YDPHY%LFGEL)
!-----------------------------------------------------------------------
...
!            : ONE FOR SATURATION CALCULATION FOR ICE, ZERO OTHERWISE.
  ...
  IF (LNEIGE) THEN
    ...
    ENDIF
    ZDELT(JLON)=MAX(0.0_JPRB,SIGN(1.0_JPRB,YDCST%RTT-PTS(JLON)))
  ELSE
    PNEIJ(JLON)=0.0_JPRB
    PVEG(JLON)=PVEG0(JLON)
...
!     OPTIONAL COMPUTATIONS ACCORDING TO THE CHOSEN SOIL/VEG SCHEME.
  ...
  IF (LSOLV) THEN
    ...
    IF (LSNV) THEN
      PHU(JLON)=1.0_JPRB+PLSM(JLON)*(1.0_JPRB-PNEIJG(JLON))*&
       & (0.5_JPRB*(1.0_JPRB-COS(YDCST%RPI*MIN(ZWS/ZWFC,1.0_JPRB)))-1.0_JPRB)  
  ...
  ELSE
    PHU(JLON)=1.0_JPRB+PLSM(JLON)*(1.0_JPRB-PNEIJ(JLON))&
     & *(0.5_JPRB*(1.0_JPRB-COS(YDCST%RPI*PWS(JLON)/WSMX))-1.0_JPRB)  
...
!            : APPROXIMATION OF DELTA(PHI)*DELTA(LN(THETA)).
  ...
  PRTI(JLON)=2.0_JPRB/(PR(JLON,KLEV)*PT(JLON,KLEV)+YDCST%RKAPPA*ZDPHI(JLON)&
   & +PRS(JLON)*PTS(JLON))  
...
!     ANTI-FIBRILLATION "GIRARD-DELAGE" COMPUTATION.
  ...
  IF (LLAFGD) THEN
    ...
    ELSE
      ZGAMDZ=4._JPRB*TSPHY*(YDCST%RG/ZDPHI(JLON))
...
IF (LDHMT) THEN
  ...
  CALL ACNTCLS (YDPHY,YDPHY0,YDCST,KIDIA,KFDIA,KLON,KLEV,&
  ...
  IF (LRAFTUR) THEN
    DO JLON=KIDIA,KFDIA
      ZSCRAF=(PCD(JLON)*(PU(JLON,KLEV)**2+PV(JLON,KLEV)**2)/ &
       & (PUCLS(JLON)**2+PVCLS(JLON)**2+(GCISMIN*PDPHIV(JLON)/YDCST%RG)**2))/ &
