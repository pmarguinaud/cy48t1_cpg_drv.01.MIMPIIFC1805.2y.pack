...
DO JLEV=KTDIA,KLEV
  ZVETAF(JLEV) = MAX(PVETAF(JLEV),RETAMIN)
ENDDO
                                                                                
DO JLEV=KTDIA,KLEV
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    PRHCRI(JLON,JLEV) = ZA(JLON) * ZVETAF(JLEV)**3&
     & + ZB(JLON) * ZVETAF(JLEV)**2&
     & + ZC(JLON) * ZVETAF(JLEV)&
     & + RHCRIT2
  ENDDO   
ENDDO   
...
IF (RQCRNS /= 0.0_JPRB) THEN
  DO JLEV=KTDIA,KLEV
    ZSURF(JLEV) = 1.0_JPRB-EXP(-(1.0_JPRB-PVETAF(JLEV))/RQCRNS)
  ENDDO
ELSE
  DO JLEV=KTDIA,KLEV
    ZSURF(JLEV) = 1.0_JPRB
...
DO JLEV=KTDIA,KLEV
  DO JLON=KIDIA,KFDIA
    ...
    PNEBS(JLON,JLEV) = ZSURF(JLEV) * PNEBS(JLON,JLEV) * RFACNSM
