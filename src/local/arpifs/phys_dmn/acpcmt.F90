!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACPCMT (YDGEM,YDDIM,YDEGEO,YDLDDH,YDMDDH,YDRIP,YDML_PHY_MF,  KIDIA,KFDIA,KLON,KTDIA,KLEV,KTRA,&
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
...
! Call microphysics.
LLADJCLD=.FALSE.
CALL ACPLUIZ ( YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
...
DO JLEV=KTDIA,KLEV
   DO JLON=KIDIA,KFDIA
      ...
      ! CUMULATE DEPTH OF LAYERS WHERE VERTICAL VELOCITY IS GREATER THAN A GIVEN THRESHOLD.
      ZDEPTH(JLON)=ZDEPTH(JLON)+(PAPHI(JLON,JLEV-1)-PAPHI(JLON,JLEV))/RG &
