!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACPLUIS ( YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
 !-----------------------------------------------------------------------
 ! - INPUT  2D .
...
!     EVAPORATION PROCESS.
    ...
    ZPLC=ZPLC**2
    ZCLTEND=(ZPLC-ZPLH(JLON))*RG/PDELP(JLON,JLEV)
    ZTENDCL=ZCLTEND*(1.0_JPRB-PNEBS(JLON,JLEV))
    ZTENDCL=-MIN(ABS(ZDELTAQ/TSPHY),-ZTENDCL)
    PFPEVPL(JLON,JLEV)=ZTENDCL*PDELP(JLON,JLEV)/RG
    ...
    ZPLC=MAX(0.0_JPRB,ZCLTEND*PDELP(JLON,JLEV)/RG+ZPLH(JLON))
...
!   ICE PHASE.
    ...
    IF (LNEIGE) THEN
      ...
      ZTVF=TVF*( ZINEBGY*(ZQLI*1000._JPRB)**0.2_JPRB&
       & +(1.0_JPRB-ZINEBGY) )  
      ZZB=RG/PDELP(JLON,JLEV)*ZPLH(JLON)
      ZZC=RG/PDELP(JLON,JLEV)*PAPRSF(JLON,JLEV)*ZTVF/PR(JLON,JLEV)&
       & / PT(JLON,JLEV)  
...
! - TENDANCE D'HUMIDITE DUE A LA CONDENSATION.
    ...
    ZPLS=ZNUTEND*PDELP(JLON,JLEV)/RG+ZPLH(JLON)
...
! - KESSLER'S SCHEME IF PQ(JLON,JLEV)<QSMIN.
    ...
    ZDELTAQ=PQW(JLON,JLEV)-PQ(JLON,JLEV)
    ZPLBK=MAX(0.0_JPRB,ZPLH(JLON)-ZDELTAQ*PDELP(JLON,JLEV)/(RG*TSPHY))
...
!     OPTION "NEIGE").
    ...
    IF (LNEIGE) THEN
      ZGENN=(ZNEIGE(JLON)*ZPLH(JLON)&
       & +ZH*ZTENDNU*PDELP(JLON,JLEV)/RG)&
       & /MAX(ZEPS1,(ZPLH(JLON)+ZTENDNU*PDELP(JLON,JLEV)/RG))  
      ZHE=FONICE(PT(JLON,JLEV),YDPHY0%RDTFAC)
      ZGENE=(ZRME(JLON)*ZPLH(JLON)&
       & +ZHE*ZTENDNU*PDELP(JLON,JLEV)/RG)&
       & /MAX(ZEPS1,(ZPLH(JLON)+ZTENDNU*PDELP(JLON,JLEV)/RG))  
