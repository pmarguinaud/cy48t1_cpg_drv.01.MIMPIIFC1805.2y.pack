SUBROUTINE ACTRGKF( YDECUMF,YDPHY0,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
!--------------------------------------------------------------------------
! - INPUT 2D.
...
!**** *ACTRGKF * USL TRIGGERING CALCULATION FOR ACCSU
...
! POME       : RESOLVED OMEGA (dp/dt vertical velocity)
...
!                     ACUSLTRIG10 JUNE 2014, L. GERARD
!                     ACTRGKF MARCH 2016, L. GERARD
!      MODIFICATIONS
!      -------------
!--------------------------------------------------------------------------
...
REAL(KIND=JPRB) :: ZA12(KLON,KLEV), ZA13(KLON,KLEV), &
    ...
    &ZTM(KLON,KLEV),ZOMT(KLON,KLEV), ZTHESE(KLON,KLEV)

! 1D REAL LOCALS
REAL(KIND=JPRB) :: ZCONTINUE(KLON),  ZS14(KLON), &
    &ZS6(KLON), ZDTVLCL(KLON),ZDTVFC(KLON),& 
    &ZSUM(KLON), ZSUM1(KLON), ZSDP(KLON), &
...
REAL(KIND=JPRB) :: ZB, ZBAS, ZBLUE,  ZCPM,&
    ...
    & ZESL,&
    & ZEW, ZEXP1,ZEXP2, ZHPHI, ZIALBU, ZINAC9, ZINCIN, ZIRHOG,&
    & ZLIQ,ZLM, ZNO,&
...
!---------------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('ACTRGKF',0,ZHOOK_HANDLE)
ASSOCIATE(GCVALBU=>YDPHY0%GCVALBU, GTRBRC=>YDPHY0%GTRBRC,&
 & GTRGDPMIX=>YDPHY0%GTRGDPMIX, GTRGDPHIMN=>YDPHY0%GTRGDPHIMN, &
 & GTRGPUSLMN=>YDPHY0%GTRGPUSLMN, GTRKGAIN=>YDPHY0%GTRKGAIN, &
 & GTRKTHRS=>YDPHY0%GTRKTHRS, GTRKTHCK=>YDPHY0%GTRKTHCK,&
 & GTRTHRS=>YDPHY0%GTRTHRS, RDTFAC=>YDPHY0%RDTFAC, NJKT1=>YDECUMF%NJKT1)
!------------------------------------------------------------------------
...
DO JLEV=KTDIA,KLEV
   DO JLON=KIDIA,KFDIA
      ZIRHOG=ZRDSRG*PTVE(JLON,JLEV)/(PAPRSF(JLON,JLEV))
      ZOMT(JLON,JLEV)=-POME(JLON,JLEV)*ZIRHOG
...
! ZDTVFC(KLON): KAIN-FRITSCH-CHAPPELL BUOYANCY KICK AT LCL
...
! KFC KICK AT LCL (TO KEEP IN MEMORY)
        ZZ=ZOK2*(ZOMT(JLON,JL1)-GTRKTHRS*MIN(1._JPRB,&
                & (PAPHIF(JLON,JL1)-PAPHI(JLON,KLEV))/(GTRKTHCK*RG)))
        ZDTVFC(JLON)=ZDTVFC(JLON)*(1._JPRB-ZOK2)+&
                & SIGN(1._JPRB,ZZ)*(GTRKGAIN*ABS(ZZ))**(Z3I)
!               ====================================
...
! ZZ2 is 1 as long as ZSQC too small : ZTHRLCL INCL POSSIBLE RESOL DEPENDENCE
...
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('ACTRGKF',1,ZHOOK_HANDLE)
 END SUBROUTINE ACTRGKF
