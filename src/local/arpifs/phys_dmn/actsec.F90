...
DO JLEV=KTDIA,KLEV
  DO JLON=KIDIA,KFDIA
    ...
    ZR(JLON,JLEV)=RD*(1.0_JPRB-PQ(JLON,JLEV))+RV*PQ(JLON,JLEV)
...
! ZSTA      : APPROXIMATION OF DELTA(PHI)*DELTA(LN(THETA)).
  ...
  ZRTI(JLON)=2.0_JPRB/(ZR(JLON,KLEV)*PT(JLON,KLEV)+RKAPPA*ZDPHI(JLON)&
   & +ZRS(JLON)*PTS(JLON))  
  ZSTA(JLON)=ZDPHI(JLON)*(ZR(JLON,KLEV)*PT(JLON,KLEV)+RKAPPA&
...
!     PRELIMINARY STABILITY COMPUTATION.
    ...
    ZDTETA=ZR(JLON,JLEV)*PT(JLON,JLEV)-ZR(JLON,JLEV+1)&
     & *PT(JLON,JLEV+1)+RKAPPA*ZDPHI(JLON)  
...
!     CORRECTION FOR SHALLOW CONVECTION (OPTIONAL).
    ...
    IF (LCVPP) THEN
      ZAUX=ZRCVPP*MIN(0.0_JPRB,PQ(JLON,JLEV)-ZQSAT(JLON,JLEV)&
       ...
       & *MAX(0.0_JPRB,SIGN(1.0_JPRB,RD*(PQ(JLON,JLEV+1)-PQ(JLON,JLEV))&
...
!     STABILITY COMPUTATION.
    ...
    ZRTI(JLON)=2.0_JPRB/(ZR(JLON,JLEV)*PT(JLON,JLEV)+ZR(JLON,JLEV+1)&
     & *PT(JLON,JLEV+1))  
    ZSTA(JLON)=ZDPHI(JLON)*(ZDTETA+RD*ZAUX)*ZRTI(JLON)
