...
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
DO JLON=KIDIA,KFDIA
  ZRS  (JLON) = YDCST%RD + (YDCST%RV-YDCST%RD)*PQS(JLON)
...
!         PRELIMINARY STABILITY COMPUTATION.
    ...
    ZDTETA=PR(JLON,JLEV)  *PT(JLON,JLEV)&
     & -PR(JLON,JLEV+1)*PT(JLON,JLEV+1)+YDCST%RKAPPA*ZDPHI0
...
DO JLON=KIDIA,KFDIA
   IF ((ILEVBI(JLON) > 1).AND.(ILEVBI(JLON) < KLEV)) THEN
      ZHBOT=(PAPHI(JLON,ILEVBI(JLON))-PAPHI(JLON,KLEV))/YDCST%RG
      ZHTOP=(PAPHI(JLON,ILEVBI(JLON)-1)-PAPHI(JLON,KLEV))/YDCST%RG
   ...
   ELSEIF (ILEVBI(JLON) == KLEV) THEN
      ZBLH(JLON)=(PAPHIF(JLON,KLEV)-PAPHI(JLON,KLEV))/YDCST%RG
   ELSEIF (ILEVBI(JLON) == 0) THEN
      ZBLH(JLON)=(PAPHI(JLON,KTDIAN)-PAPHI(JLON,KLEV))/YDCST%RG
   ELSE
      ZBLH(JLON)=(PAPHI(JLON,ILEVBI(JLON))-PAPHI(JLON,KLEV))/YDCST%RG
   ENDIF
ENDDO
ENDIF
...
!DEC$ IVDEP
   DO JLON=KIDIA,KFDIA
     ...
     ZTETA          = ZRTV(JLON,KLEV)+YDCST%RKAPPA*ZZ(JLON,KLEV)-ZRS(JLON)*PTS(JLON)
     ZRQZERO        = MAX(ZEPS,-PCH(JLON)*ZTETA*ZMODU)
     ZWSTAR(JLON)   = (YDCST%RG*ZBLH(JLON)*ZRQZERO/ZRTV(JLON,KLEV))**UCWSTAR
...
IF(LMUSCLFA) THEN
   CALL WRSCMR(NMUSCLFA,'ZKH',ZGKTH/YDCST%RG,KLON,KLEV+1)
   CALL WRSCMR(NMUSCLFA,'ZKM',ZGKUH/YDCST%RG,KLON,KLEV+1)
   ZLM(:,:)=PLMECT(:,:)/YDCST%RG
...
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    IF (ILEVBI(JLON) > 0) THEN
      ...
      ZBIC  = YDCST%RG*YDCST%RG*AJBUMIN/ZDPHI
...
!     ZLINV=PLMECT(JLON,ILEVT+1)/RG ! just below the inversion
      ...
      IF (AGREF < 0._JPRB) THEN
        ...
        ZA = -AGREF*AGRE1*(1._JPRB-(1._JPRB/AGREF+1._JPRB)&
         &*SIN(YDCST%RPI/2._JPRB*MAX(0._JPRB,MIN(1._JPRB,((ZBICX-ZBI)&
         &/(ZBICX-ZBIC)))))**2._JPRB)
      ...
      ZBI = MAX( ZBI, ZBIC )
      ZGKENT = MAX( ZA*ZECTBLK*SQRT(ZECTBLK)*YDCST%RG/ZLINV/ZBI ,&
     &              ZGKTH(JLON,ILEVT))
