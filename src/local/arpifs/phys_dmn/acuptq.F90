  ...
  DO JROF = KSTART, KPROF
     IF (LDPTQ) THEN
        ZJTOT(JROF,JLEV) = PFRSO(JROF,JLEV)+PFRTH(JROF,JLEV)+&
            ...
            & (YDCST%RCW-YDCST%RCPD)*PFPLCL(JROF,JLEV)*ZTI(JROF,JLEV)+&
            & (YDCST%RCS-YDCST%RCPD)*PFPLCN(JROF,JLEV)*ZTI(JROF,JLEV)-&
            & YDCST%RLVZER*(PFCCQL(JROF,JLEV)-PFECL(JROF,JLEV))-&
            & YDCST%RLSZER*(PFCCQN(JROF,JLEV)-PFECN(JROF,JLEV))
      ELSE      
        ZJTOT(JROF,JLEV) = PFRSO(JROF,JLEV)+PFRTH(JROF,JLEV)+&
            & PDIFTS(JROF,JLEV)
      ENDIF      
