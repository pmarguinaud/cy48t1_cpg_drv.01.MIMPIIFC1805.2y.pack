...
!LOP   & * RPI / (4._JPRB * (2.0_JPRB**(1.0_JPRB+ZTAU2/3._JPRB)) * ZSIGMA1) 
  ...
  ZCOEFF3 = 2.0_JPRB * ZFVENTR1 * SQRT(YDCST%RPI)
  ZCOEFF4 = 2.0_JPRB * YDCST%RPI**((3._JPRB-ZNU2)/8._JPRB) * ZFVENTR2 * SQRT(ZNU1)&
...
!DEC$ IVDEP
    DO JLON = KIDIA, KFDIA
      ...
      IF (LCOLLEC) THEN
        ...
        &     * MAX(0.0_JPRB,SIGN(1.0_JPRB,PT(JLON,JLEV)-YDCST%RTT))
