...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    CALL ARO_ADJUST_LIMA (YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,NLIMA,KSTEP+1,&
...
! specific to new data flow for diagnostics
  IF (LDDH_OMP) THEN
    ...
    CALL ARO_SUINTBUDGET_OMP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,KSTEP,ZCON1,ZCON2,ZCON3,YDDDH)
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,KSTEP,YDCPG_DIM%KFDIA,ZQDM,ZEXNREFM_,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL,YDCPG_DIM%KGPCOMP,&
       & YDCPG_DIM%KFDIA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,KSTEP,&
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    CALL VDFHGHTHL(YDMODEL%YRML_PHY_G%YRVDF,YDMODEL%YRML_PHY_SLIN%YREPHLI, &
     ...
     & KSTEP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,INDRAFT,&
...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    ENDIF
    CALL ARO_LIMA(YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,NLIMA,KSTEP+1,NSPLITR,NSPLITG, ZDT,ZDZZ_ ,&
     ...
     & YDDDH, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH )
  ELSE
    CALL ARO_RAIN_ICE (YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,KSTEP+1,NSPLITR,NGFL_EZDIAG,&
...
IF (LUSECHEM) THEN
  ...
  CALL ARO_MNHC(ZSVSIN_,&
   ...
   & YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT, NRR, KSTEP+1,NULOUT,IEZDIAG_CHEM, ZPEZDIAG_(:,:,IOFF_MFSHAL:NGFL_EZDIAG),ZSVS_ )
...
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,KSTEP,&
