...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    & NLIMA, YDCPG_OPTS%NSTEP+1, LOSUBG_COND, LOSIGMAS, LOCND2, ZDT, VSIGQSAT, ZZZ_F_, ZRHODJM__(:, 1:YDCPG_OPTS%KFLEVG), &
...
! specific to new data flow for diagnostics
  ...
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%NSTEP, YDCPG_BNDS%KFDIA, &
...
!* Compute PBL-diagnostics
   ...
   ZCAPE(:)=0._JPRB
   ZDCAPE(:)=0._JPRB
  CALL ACCLDIA(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDTOPH, YDCPG_BNDS%KIDIA,    &
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDMODEL%YRCST, YDVISI, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, &
   & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF,                  &
...
IF (LMFSHAL) THEN
  ...
  DO JDRAFT=IDRAFT,3
    ...
    & OMIXUV=LMIXUV, ONOMIXLG=.FALSE., KSV_LGBEG=0, KSV_LGEND=0, KTCOUNT=YDCPG_OPTS%NSTEP+1, PTSTEP=ZDT,                            &
...
! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
  ...
  & KRRL=NRRL, KRRI= NRRI, KSV=NLIMA, KTCOUNT=YDCPG_OPTS%NSTEP+1, KGRADIENTS=NGRADIENTS, LDHARATU=LHARATU,           &
...
IF (LMICRO) THEN
  ...
  IF (MOD(YDCPG_OPTS%NSTEP+1,NPRINTFR)==0) THEN
    ...
    WRITE(NULOUT,*)'NRR and co',NRR,YDCPG_OPTS%NSTEP+1,NSPLITR,LOSUBG_COND, LOSIGMAS, CSUBG_AUCV_RC,LOWARM  
  ENDIF
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    ENDIF
    CALL ARO_LIMA(YDCPG_OPTS%KFLEVG, IKU, IKL, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, NRR, NLIMA, YDCPG_OPTS%NSTEP+1,       &
    ...
    &              )
  ELSE
    CALL ARO_RAIN_ICE (YDCPG_OPTS%KFLEVG, IKU, IKL, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, NRR, YDCPG_OPTS%NSTEP+1,               &
...
IF(LSPSDT) THEN
  ...
  & YDCPG_OPTS%KFLEVG, 1, YDCPG_OPTS%ZDTPHY, PTSL=YDMF_PHYS_BASE_STATE%T, PQSL=YDMF_PHYS_BASE_STATE%Q, PA=YDVARS%A%T1,       &
...
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%NSTEP,     &
...
CALL PPWETPOINT(YDMODEL%YRCST, YDPHY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE(:, YDCPG_OPTS%KFLEVG), &
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_OPTS%KFLEVG
      CALL PPWETPOINT(YDMODEL%YRCST, YDPHY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF(:, JLEV), &
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDMODEL%YRCST, YDPRECIPS, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDVARS%GEOMETRY%OROG%T0, &
