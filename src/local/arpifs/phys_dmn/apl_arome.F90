...
SUBROUTINE APL_AROME(YDGEOMETRY,YDCPG_OPT,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 & KTDIA  , KLEV , KSTEP ,&
...
USE CPG_TYPE_MOD       , ONLY : CPG_MISC_TYPE, CPG_OPT_TYPE
...
!* Compute PBL-diagnostics
  ...
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,KIDIA,KFDIA,KLON,KLEV,YDMF_PHYS%OUT%UCLS,&
   & YDMF_PHYS%OUT%VCLS,PUM(:,1:KLEV),PVM(:,1:KLEV),ZCAPE,ZDCAPE,ZTKEM(:,1:KLEV),PAPHIFM(:,1:KLEV),POROG,&
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(KSTEP,NRADFR) == 0) THEN 
     ...
     & PGP2DSPP, PEZDIAG,                                &
     & PEMTD   , PEMTU , PTRSO    , YDMF_PHYS%OUT%FRTHC   , YDMF_PHYS%OUT%FRTH   , & 
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & YDCPG_OPT%GRPROX,YDCPG_OPT%GMIXP,YDCPG_OPT%GFLUXC,YDCPG_OPT%GRSURF,YDMF_PHYS_SURF%GSD_VD%PSUND,&
   & ZFRSO,ZFRTH,&
...
IF(LFORCENL.AND.(KSTEP*(TSPHY/RHOUR)>=NFORCESTART).AND.&
  ...
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%OUT%TENDU(JLON,JLEV)=YDMF_PHYS%OUT%TENDU(JLON,JLEV)+AMAGSTOPH_CASBS*PFORCEU(JLON,JLEV)
      YDMF_PHYS%OUT%TENDV(JLON,JLEV)=YDMF_PHYS%OUT%TENDV(JLON,JLEV)+AMAGSTOPH_CASBS*PFORCEV(JLON,JLEV)
...
IF (LXXDIAGH) THEN
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%OUT%DIAGH(JLON)=YDMF_PHYS%OUT%DIAGH(JLON)+ZQGM(JLON,JLEV)*PDELPM(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
ENDIF
! lightening density
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,KIDIA,KFDIA,KLON,KLEV,KSTEP,&
    ...
    &PDELPM,ZTM,PWM,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS%OUT%FLASH)
ENDIF
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,PAPRSM(:,KLEV),YDMF_PHYS%OUT%TCLS,&
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,PAPHIFM,&
