...
SUBROUTINE APL_AROME(YDGEOMETRY,YDCPG_DIM,YDMF_PHYS_STATE,YDCPG_MISC,YDMF_PHYS,&
 & YDMF_PHYS_TMP,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, &
 & KMAXDRAFT, KSGST, PDT, LDXFUMSE, &
 & POROG,&
 & PGP2DSDT, PGP2DSPP, &
 !  - INOUT A S
 & PGPAR, &
 !  - OUT A S
 & PTENDT, PTENDR, PTENDW,&
 & PTENDLIMA, &
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDR(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_PHY_MF%YRPARAR%NRR) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDW(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDLIMA(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NLIMA)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDTKE(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTENDEFB1(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTENDEFB2(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTENDEFB3(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PTENDEXT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)
...
REAL(KIND=JPRB) :: ZTENDT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) ! array to save heating profile for LHN
...
REAL(KIND=JPRB) :: ZMF_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZTHETAL_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZQT_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZTHTV_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZQC_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZQI_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZU_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
REAL(KIND=JPRB) :: ZV_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,KMAXDRAFT)
...
ASSOCIATE(MINPRR=>YDPARAR%MINPRR, MINPRS=>YDPARAR%MINPRS, MVQS=>YDPARAR%MVQS, &
 ...
 & LRSTAER=>YDPHY%LRSTAER, LRNUEXP=>YDPHY%LRNUEXP, &
...
DO JLEV=1,YDCPG_DIM%KFLEVG
  PTENDT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=0.0_JPRB
  ...
  PTENDW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=0.0_JPRB
  PTENDTKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=0.0_JPRB
ENDDO
DO JRR=1,NRR
  DO JLEV=1,YDCPG_DIM%KFLEVG
    PTENDR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,JRR)=0.0_JPRB
  ENDDO
ENDDO
DO JGFL=1,NGFL_EXT
  DO JLEV=1,YDCPG_DIM%KFLEVG
    PTENDEXT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,JGFL)=0.0_JPRB
  ENDDO
ENDDO
DO JGFL=1,NLIMA
  DO JLEV=1,YDCPG_DIM%KFLEVG
    PTENDLIMA(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,JGFL)=0.0_JPRB
  ENDDO
ENDDO
...
DO JLEV = 1,YDCPG_DIM%KFLEVG
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ZZZ_F_(JLON,JLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHIF(JLON,JLEV)*ZINVG
...
IF (LRSLDDH) THEN
  DO JLEV=1, YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ZTKEM4SLDDH(JLON,JLEV)=MAX(YDMF_PHYS_STATE%TKE(JLON,JLEV),PPTKEMIN)
    ENDDO
  ENDDO
  ZTKEM => ZTKEM4SLDDH(:,:)
ELSE
  ZTKEM => YDMF_PHYS_STATE%TKE(:,:)
  !test TKE > 0.
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
   ...
   !et convertir q en r 
    IF (NRR==7) THEN
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YDMF_PHYS_STATE%Q(JLON,JLEV)-YDMF_PHYS_STATE%L(JLON,JLEV)-YDMF_PHYS_STATE%R(JLON,JLEV)&
         & -YDMF_PHYS_STATE%I(JLON,JLEV)-YDMF_PHYS_STATE%S(JLON,JLEV)-YDMF_PHYS_STATE%G(JLON,JLEV)-YDMF_PHYS_STATE%H(JLON,JLEV)   
      ENDDO
    ELSE
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YDMF_PHYS_STATE%Q(JLON,JLEV)-YDMF_PHYS_STATE%L(JLON,JLEV)-YDMF_PHYS_STATE%R(JLON,JLEV)&
         & -YDMF_PHYS_STATE%I(JLON,JLEV)-YDMF_PHYS_STATE%S(JLON,JLEV)-YDMF_PHYS_STATE%G(JLON,JLEV)
      ENDDO
    ENDIF
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
   !initialisation de ZRHODREFM__ (=qd*zrho) 
      ZRHO=YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV)/(YDMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,JLEV)*YDMF_PHYS_STATE%T(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
      ZRHODJM__(JLON,JLEV)=YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
  !initialisation de ZEXNREFM_
      ZEXNREFM_(JLON,JLEV)=(YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV)*ZINVATM)**(ZRSCP)
 ! vent horizontal et TKE
      ZPABSM__(JLON,JLEV)=YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV)
      ZUM__(JLON,JLEV)= YDMF_PHYS_STATE%U(JLON,JLEV)
      ZVM__(JLON,JLEV)= YDMF_PHYS_STATE%V(JLON,JLEV)
      ZWM__(JLON,JLEV)= YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)
      ZTKEM__(JLON,JLEV)= ZTKEM(JLON,JLEV)
      ZZZ_(JLON,JLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHI(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
  ...
  IF (LOSIGMAS) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG 
      ZSIGM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)= YDMF_PHYS_STATE%SRC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ENDDO
  ENDIF
  ...
  IF ( LKFBCONV.AND.LOSUBG_COND.AND..NOT.LOSIGMAS) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG 
      ZMFM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YDMF_PHYS_STATE%SRC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV) 
    ENDDO
  ENDIF
!!! initialisation des variables d etat MNH �t
  ...
  DO JLEV = 1, YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZTHM__(JLON,JLEV)=YDMF_PHYS_STATE%T(JLON,JLEV)/ZEXNREFM_(JLON,JLEV)
      ZRM_(JLON,JLEV,1)=YDMF_PHYS_STATE%Q(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,2)=YDMF_PHYS_STATE%L(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,3)=YDMF_PHYS_STATE%R(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,4)=YDMF_PHYS_STATE%I(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,5)=YDMF_PHYS_STATE%S(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,6)=YDMF_PHYS_STATE%G(JLON,JLEV)/ZQDM(JLON,JLEV)  
    ENDDO
  ENDDO
  ...
  IF (NRR==7) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZRM_(JLON,JLEV,7)=YDMF_PHYS_STATE%H(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ENDDO
    ENDDO
  ENDIF
...
!!! suivant les parametrisations)   
  ...
  DO JLEV = 1, YDCPG_DIM%KFLEVG 
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZUS__(JLON,JLEV)= YDMF_PHYS_STATE%U(JLON,JLEV)*ZINVDT
      ZVS__(JLON,JLEV)= YDMF_PHYS_STATE%V(JLON,JLEV)*ZINVDT
      ZWS__(JLON,JLEV)= YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)*ZINVDT
...
!!! Initialisations temporaires d'arguments non-utilises
  ...
  IF (LLMSE.OR.LSFORCS) THEN
    ZACPRR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRR)
    ZACPRS_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRS)
    ZACPRG_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRG)
    ZINPRR_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRR)
    ZINPRS_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRS)
    ZINPRG_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRG)
  ENDIF
  ...
  DO JGFL=1,NGFL_EXT
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZSVM_(JLON,JLEV,JGFL)=YDMF_PHYS_STATE%P1EXT(JLON,JLEV,JGFL)
        ZSVS_(JLON,JLEV,JGFL)=YDMF_PHYS_STATE%P1EXT(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JGFL=1,NLIMA
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZLIMAM_(JLON,JLEV,JGFL)=YDMF_PHYS_STATE%P1LIMA(JLON,JLEV,JGFL)
        ZLIMAS_(JLON,JLEV,JGFL)=YDMF_PHYS_STATE%P1LIMA(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
...
IF (LDIAGWMAX) THEN
  ...
  ! calcul de wmax
    DO JLEV = 1 , YDCPG_DIM%KFLEVG
      ...
      Z_WMIN=0._JPRB
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF (YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)>Z_WMAX) THEN
          Z_WMAX=YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)
        ENDIF
        IF (YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)<Z_WMIN) THEN
          Z_WMIN=YDMF_PHYS_STATE%YCPG_PHY%W(JLON,JLEV)
        ENDIF
      ENDDO
    ENDDO
  ENDIF
...
IF (LFLEXDIA) THEN
  !save tendencies
  ZTENDTBAK(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=PTENDT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  DO JR=1,NRR
    ZTENDRBAK(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,JR)=PTENDR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,JR)
  ENDDO
ENDIF
...
IF (LMICRO) THEN
  ...
  IF (LMFSHAL .AND. (CMF_CLOUD=='DIRE'.OR.CMF_CLOUD=='BIGA')) THEN
    ...
    ELSE
      DO JLEV = 1, YDCPG_DIM%KFLEVG 
        ZRC_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,1)
        ZRI_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,3)
        ZCF_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,2)
      ENDDO
    ENDIF
    PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,1:3)=0._JPRB
  ENDIF
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    CALL ARO_ADJUST_LIMA (YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,NLIMA,YDCPG_DIM%KSTEP+1,&
     & LOSUBG_COND, LOSIGMAS, LOCND2, &
     & ZDT,VSIGQSAT,ZZZ_F_,&
     & ZRHODJM__(:,1:YDCPG_DIM%KFLEVG),&
     & ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),&
     & ZEXNREFM_,&
     & ZPABSM__(:,1:YDCPG_DIM%KFLEVG),&
     & ZTHM__(:,1:YDCPG_DIM%KFLEVG),&
     & ZRM_,&
     & ZLIMAM_,&
     & ZSIGM_,&
     & ZMFM_,ZRC_MF_,&
     & ZRI_MF_,ZCF_MF_,&
     & ZTHS__(:,1:YDCPG_DIM%KFLEVG),ZRS_,&
     & ZLIMAS_,&
     & ZSRCS__(:,1:YDCPG_DIM%KFLEVG),ZNEBMNH_,&
     & YDDDH, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH)
  ELSE

    CALL ARO_ADJUST (YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,&
     & NGFL_EZDIAG, &
  ...
  CALL GPRCP(YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PQ=ZQVM,PQI=ZQIM,PQL=ZQCM,PQR=ZQRM,PQS=ZQSM,PQG=ZQHGM,PCP=ZCPM,PR=ZRHM)  

  DO JLEV = 1,YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZTM(JLON,JLEV)=ZTHM__(JLON,JLEV)*ZEXNREFM_(JLON,JLEV)
  !reinitialisation de ZRHODREFM__ (=qd*zrho)
      ZRHO=YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV)/(ZRHM(JLON,JLEV)*ZTM(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
    ENDDO
  ENDDO
  ...
  ZAPHIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  CALL GPGEO(YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,ZAPHIM,ZAPHIFM,ZTM,ZRHM,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDGEOMETRY%YRVERT_GEOM)
...
ELSE
  ...
  ZTM (YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZRHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%RCP%R(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%I(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQCM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%L(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQRM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%R(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%S(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQGM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%G(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  IF (NRR==7) THEN
    ZQHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%H(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ELSE
    ZQHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=0._JPRB
  ENDIF
  ZCPM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%RCP%CP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)
  ZAPHIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,0:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,0:YDCPG_DIM%KFLEVG)
  ZAPHIFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%PHIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZZZ_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*ZINVG
...
IF (LFLEXDIA) THEN
  DO JLEV = 1, YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
      ZTMPAF(JLON,JLEV)=(PTENDT(JLON,JLEV)-ZTENDTBAK(JLON,JLEV))*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG*ZCPM(JLON,JLEV)
    ENDDO
  ENDDO
  IF (LDDH_OMP) THEN
  ...
  DO JR=1,NRR
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZTMPAF(JLON,JLEV)=(PTENDR(JLON,JLEV,JR)-ZTENDRBAK(JLON,JLEV,JR))*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZTMPAF(JLON,JLEV)=YDVARS%A%T1(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
...
! specific to new data flow for diagnostics
  IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZCON3(JLON,JLEV) = YDMF_PHYS_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)*ZEXNREFM_(JLON,JLEV)
      ENDDO
    ENDDO
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET_OMP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,ZCON1,ZCON2,ZCON3,YDDDH)
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,YDCPG_DIM%KFDIA,ZQDM,ZEXNREFM_,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
ENDDO
DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ZDZZ_F_(JLON,YDCPG_DIM%KFLEVG)=ZZZ_F_(JLON,YDCPG_DIM%KFLEVG)-POROG(JLON)*ZINVG
ENDDO
...
!     --------------------------------------------------------------------
IF (LRDUST) THEN
  PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,IOFF_MFSHAL:NGFL_EZDIAG)=0.0_JPRB
  ...
  CALL SWAP_SVM
  ! input dust scalar concentration in ppp from
  CALL ARO_MNHDUST (IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT, PDT,ZSVMIN_,ZZZ_,ZDZZ_,&
   & ZPABSM__(:,1:YDCPG_DIM%KFLEVG),ZTHM__(:,1:YDCPG_DIM%KFLEVG),ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),&
   & NSWB_MNH,YDCPG_DIM%KSTEP+1,ZSVM_,ZPIZA_DST_,ZCGA_DST_,ZTAUREL_DST_,ZAERD_,IEZDIAG_CHEM,&
   & ZPEZDIAG_(:,:,IOFF_MFSHAL:NGFL_EZDIAG)           )
  PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,IOFF_MFSHAL:NGFL_EZDIAG)=ZPEZDIAG_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,IOFF_MFSHAL:NGFL_EZDIAG)
! return to tendency
  DO JGFL=1, NGFL_EXT
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA) = YDMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  ZTN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)    = YDMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  ZQS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)    = YDMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ZRHODREFM(JLON) = YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,YDCPG_DIM%KFLEVG)/(YDMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)*YDMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG))
    ZTHETAS(JLON)   = ZTSURF(JLON)*(RATM/YDMF_PHYS_STATE%YCPG_PHY%PRE(JLON,YDCPG_DIM%KFLEVG))**RKAPPA
  ENDDO

  LLAROME=.TRUE.
  CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS, LLAROME, YDCPG_DIM%KIDIA , YDCPG_DIM%KFDIA  , YDCPG_DIM%KLON, YDMF_PHYS_STATE%YCPG_DYN%PHIF(:,YDCPG_DIM%KFLEVG), &
   & ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,ZTN,ZTSURF,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_STATE%Q(:,YDCPG_DIM%KFLEVG), YDMF_PHYS_STATE%U(:,YDCPG_DIM%KFLEVG), YDMF_PHYS_STATE%V(:,YDCPG_DIM%KFLEVG), ZTHETAS, &
...
!* Compute PBL-diagnostics
   ...
   ZCAPE(:)=0._JPRB
   ZDCAPE(:)=0._JPRB
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMF_PHYS%OUT%UCLS,&
   & YDMF_PHYS%OUT%VCLS,YDMF_PHYS_STATE%U(:,1:YDCPG_DIM%KFLEVG),YDMF_PHYS_STATE%V(:,1:YDCPG_DIM%KFLEVG),ZCAPE,ZDCAPE,ZTKEM(:,1:YDCPG_DIM%KFLEVG),YDMF_PHYS_STATE%YCPG_DYN%PHIF(:,1:YDCPG_DIM%KFLEVG),POROG,&
   & YDMF_PHYS%OUT%UGST,YDMF_PHYS%OUT%VGST,YDMF_PHYS%OUT%CLPH,ICLPH)
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  CALL ACTQSAT ( YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTQSAT,YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREF, ZCPM, ZQVM, ZTM,&
   & ZGEOSLC, ZLH, ZLSCPE, ZQSAT, ZQW, YDCPG_MISC%RH, ZTW)  
...
! Albedo dans les intervalles, direct (parallel) et diffus (diffuse).
  IF (NSW==6.OR.NSW==1) THEN
    IF (LLMSE) THEN
      DO JSW=1,NSW
        ZALBP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR-1+JSW)
        ZALBD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA-1+JSW)
        IF (LHLRADUPD) THEN
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  ! to be applied for HLRADIA, ACRANEB2 which use a single solar spectral band
    IF (LHLRADUPD) THEN
      ...
      ENDDO
    ELSE
       ZALBP1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR)
       ZALBD1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA)
    ENDIF
  ...
  IF (LLMSE) THEN
    ZEMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVEMIS)
    ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVTS)
    ...
    IF (NAER >= 1 ) THEN
      IF(YSD_VAD%NUMFLDS >= 4) THEN
        CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDPHY, YDCPG_DIM%KIDIA , YDCPG_DIM%KFDIA , YDCPG_DIM%KLON  , YDCPG_DIM%KFLEVG,&
         & YDMF_PHYS_STATE%YCPG_PHY%PRE,YDMF_PHYS_STATE%YCPG_PHY%PREF,ZTM,ZTSURF,YDMF_PHYS_SURF%GSD_VA%PSEA,YDMF_PHYS_SURF%GSD_VA%PLAN,YDMF_PHYS_SURF%GSD_VA%PSOO,YDMF_PHYS_SURF%GSD_VA%PDES,YDMF_PHYS_SURF%GSD_VA%PSUL,YDMF_PHYS_SURF%GSD_VA%PVOL,ZAER,ZAERINDS)
      ELSE
  ...
  IF (NOZOCL == 1) THEN
    ! as in IFS
    CALL RADOZC(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1,YDCPG_DIM%KLON,0,YDMF_PHYS_STATE%YCPG_PHY%PRE,YDGSGEOM%GEMU,ZROZ)
    DO JK=1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,JK)=ZROZ(JLON,JK)/YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JK)
      ENDDO
    ENDDO
  ELSEIF (NOZOCL == 2) THEN
    ! as in ARPEGE (from clim profiles)
    CALL SUOZON(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZQO3,.FALSE.,YDMF_PHYS_STATE%YCPG_PHY%PRE,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZALBD   , ZALBP , YDMF_PHYS_STATE%YCPG_PHY%PRE   , YDMF_PHYS_STATE%YCPG_PHY%PREF  ,           &
     & YDVARS%A%T1   , ZQO3  , ZAER     , YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP   , ZEMIS   , &
     & YDMF_PHYS_TMP%RDG%MU0M   , ZQV   , ZQSAT    , ZQICE    , ZQLIQ   , &
     & ZQSM    , ZQRM  , YDMF_PHYS_SURF%GSD_VF%PLSM     , ZTM      , ZTSURF  , &
     & PGP2DSPP, PEZDIAG,                                &
     & YDMF_PHYS%RAD%EMTD   , YDMF_PHYS%RAD%EMTU , YDMF_PHYS%RAD%TRSW    , YDMF_PHYS%OUT%FRTHC   , YDMF_PHYS%OUT%FRTH   , & 
     & YDMF_PHYS%OUT%FRSOC  , YDMF_PHYS%OUT%FRSO , ZZS_FSWDIR, ZZS_FSWDIF, ZFSDNN  , &
     & ZFSDNV  , ZCTRSO, ZCEMTR   , ZTRSOD   , ZTRSODIR, &
     & ZTRSODIF, ZPIZA_DST_,ZCGA_DST_,ZTAUREL_DST_,ZAERINDS,&
     & YDGSGEOM%GELAM , YDGSGEOM%GEMU ,PGPAR , &
     & YDMF_PHYS_TMP%RDG%MU0LU , ZALBD1 , ZFRSOLU)
   ELSE
     IF (LLMSE) THEN 
       DO JSW=1,NSW 
         ZTRSODIR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIR+JSW-1)
         ZTRSODIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIF+JSW-1)
       ENDDO 
     ENDIF
     ZCTRSO(:,:)=0._JPRB
   ENDIF
   ...
   CALL RADHEAT&
   & (  YDERAD,YDERDI,YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA  , YDCPG_DIM%KFDIA  , YDCPG_DIM%KLON   , YDCPG_DIM%KFLEVG,&
   & YDMF_PHYS_STATE%YCPG_PHY%PRE  , ZEMIS  , YDMF_PHYS%RAD%EMTD  , YDMF_PHYS_TMP%RDG%MU0, ZQVM,&
   & ZTENT  , YDMF_PHYS%RAD%TRSW  , ZTRSOD , ZTSURF   , PDT,&
   ...
   & YDMF_PHYS%OUT%FRSOPS, ZFRSOFS, YDMF_PHYS%OUT%FRSOPT )
...
CALL ACNPART(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTNEBU,YDCPG_DIM%KFLEVG,&
 & YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREF,ZDECRD,ZNEB0,&
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & YDMF_PHYS_STATE%YCPG_PHY%PRE,YDMF_PHYS_STATE%YCPG_PHY%PREF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZNEB0,&
   & ZQV,ZQCO2,ZQICE,ZQLIQ,ZQO3,YDMF_PHYS_STATE%T,&
   & ZALBD1,ZALBP1,ZEMIS,YDGSGEOM%GELAM,YDGSGEOM%GEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_TMP%RDG%MU0LU,ZTSURF, &
   & ZDECRD,ZCLCT_RAD,&
   ...
   & ZFRSO,ZFRTH,&
   & YDMF_PHYS%OUT%FRSOC,YDMF_PHYS%OUT%FRTHC,ZFRSODS,YDMF_PHYS%OUT%FRSOPS,ZFRSOLU,YDMF_PHYS%OUT%FRTHDS,ZAER)
...
IF(LKFBCONV) THEN
  ...
  CALL BRI2ACCONV(YDMODEL%YRML_PHY_MF,YDGEOMETRY%YREGEO,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,YDGSGEOM%GM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
   & ZPABSM__(:,1:YDCPG_DIM%KFLEVG),ZZZ_F_, ZTM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:), ZRM_(:,:,1),ZRM_(:,:,2), ZRM_(:,:,4), &
   & ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG), ZUM__(:,1:YDCPG_DIM%KFLEVG),ZVM__(:,1:YDCPG_DIM%KFLEVG), ZWM__(:,1:YDCPG_DIM%KFLEVG),ZMFS_,&
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ...
      ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZCVTENDT_(JLON,JLEV)*(RATM/YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)  
    ENDDO
  ENDDO
  DO JLON =YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
  ...
  IF (MINVAL(ZDEPTH_HEIGHT_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,IKB)) <= 0._JPRB) THEN
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      IF (ZDEPTH_HEIGHT_(JLON,IKB) <= 0._JPRB) THEN
        WRITE (NULOUT,*)'sous apl_arome pb height en', JLON,ZAPHIFM(JLON,YDCPG_DIM%KFLEVG),POROG(JLON)
      ENDIF
    ENDDO
  ENDIF
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL,YDCPG_DIM%KGPCOMP,&
       & YDCPG_DIM%KFDIA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KSTEP,&
       ...
       & ZUM__(:,IKB),&
       & ZVM__(:,IKB),&
       & ZTM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG),ZRM_(:,IKB,1),&
       & ZSVMB_,&
       & RCARDI,ZRHODREFM__(:,IKB),&
       & ZPABSM__(:,IKB),YDMF_PHYS_STATE%YCPG_PHY%PRE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG),&
       ...
       & ZINPRG_NOTINCR_,&
       & YDMF_PHYS%OUT%FRTHDS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),ZZS_FSWDIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NSW),&
       & ZZS_FSWDIR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NSW),&
       ...
       & ZEMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDMF_PHYS%OUT%FRTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,1))
...
!ZSFRV_(KIDIA:KFDIA) = 0._JPRB
    ...
    IF (LLMSE_DIAG) THEN
      ...
      CALL ARO_GROUND_DIAG( YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP,&
       & YDCPG_DIM%KFDIA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG, IKL,&
       ...
       & YDCSGEOM%RINDX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDCSGEOM%RINDY(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
       & ZQS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),ZGZ0_,ZGZ0H_,&
       & YDMF_PHYS%OUT%TCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDMF_PHYS%OUT%QCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDMF_PHYS%OUT%RHCLS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
       ...
       & ZSSO_STDEV_, YDMF_PHYS_SURF%GSP_SG%PF_T1,&
       & ZBUDTH_, ZBUDSO_,&
       & ZFCLL_, ZTOWNS_,&
       & ZCD_                         )
      CALL ARO_GROUND_DIAG_2ISBA( YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, &
       & YDCPG_DIM%KFDIA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, &
       & NDGUNG, NDGUXG, NDLUNG, NDLUXG, LSURFEX_KFROM, &
       & YDCSGEOM%RINDX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDCSGEOM%RINDY(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), &
       & YDMF_PHYS_SURF%GSD_VF%PLSM, ZDUMMY1, ZDUMMY1, ZDUMMY1, ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_SURF%GSP_SG%PF_T1, &
       & ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), &
       & ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), &
       & ZDUMMY1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_SURF%GSP_SG%PR_T1, ZDUMMY1 )
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDVISI,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREF,&
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ZZS_FTH_(JLON)=-1._JPRB*ZSFTH_(JLON)*(YDMF_PHYS_STATE%YCPG_PHY%PRE(JLON,YDCPG_DIM%KFLEVG)*ZINVATM)**(ZRSCP)
    ...
    CALL VDFHGHTHL(YDMODEL%YRML_PHY_G%YRVDF,YDMODEL%YRML_PHY_SLIN%YREPHLI, &
     ...
     & YDCPG_DIM%KSTEP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,INDRAFT,&
     & PDT, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,&
     & ZTM,ZQVM,ZQCM,ZQIM,YDVARS%A%T1,&
     & YDMF_PHYS_STATE%YCPG_PHY%PRE, YDMF_PHYS_STATE%YCPG_PHY%PREF, ZAPHIFM,ZAPHIM,&
      ...
      DO JLEV= 1, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          ...
          ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZTENDTUP(JLON,JLEV)*(RATM/&
           & YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)
  ...
  DO JDRAFT=IDRAFT,3
    ...
    CALL ARO_SHALLOW_MF (KKL=IKL, KLON=YDCPG_DIM%KFDIA,KLEV=YDCPG_DIM%KFLEVG,KRR=NRR,KRRL=NRRL,&
     ...
     & KTCOUNT=YDCPG_DIM%KSTEP+1, PTSTEP=ZDT,&
     & PZZ=ZZZ_,PZZF=ZZZ_F_,&
     & PDZZF=ZDZZ_F_,&
     & PRHODJ=ZRHODJM__(:,1:YDCPG_DIM%KFLEVG),&
     & PRHODREF=ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),&
     & PPABSM=ZPABSM__(:,1:YDCPG_DIM%KFLEVG),&
     & PEXNM=ZEXNREFM_,&
     & PSFTH=ZSFTH_,PSFRV=ZSFRV_,&
     & PTHM=ZTHM__(:,1:YDCPG_DIM%KFLEVG),PRM=ZRM_,&
     & PUM=ZUM__(:,1:YDCPG_DIM%KFLEVG),PVM=ZVM__(:,1:YDCPG_DIM%KFLEVG),&
     & PTKEM=ZTKEM__(:,1:YDCPG_DIM%KFLEVG),PSVM=ZSVM_,&
     ...
     & PFRAC_UP=ZZFRAC_UP_,PEMF=ZMF_UP__(:,1:YDCPG_DIM%KFLEVG))
    ...
    IF ((CMF_CLOUD=='DIRE'.OR.CMF_CLOUD=='BIGA').AND.JDRAFT==3) THEN
      ...
      DO JLEV = 1,YDCPG_DIM%KFLEVG
        PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,1)=ZRC_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
        PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,3)=ZRI_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
        PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV,2)=ZCF_MF_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
      ENDDO
    ENDIF
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
! KSV_TURB, ZSFTURB, ZTURBM, ZTURBS, ZTENDSV_TURB
  CALL ARO_TURB_MNH(KKA=IKA,KKU=IKU,KKL=IKL,KLON=YDCPG_DIM%KFDIA,KLEV=YDCPG_DIM%KFLEVG,&
   & KRR=NRR, KRRL=NRRL,KRRI= NRRI,&
   & KSV=NLIMA,KTCOUNT=YDCPG_DIM%KSTEP+1,KGRADIENTS=NGRADIENTS,LDHARATU=LHARATU,PTSTEP=ZDT,&
   & PZZ=ZZZ_,PZZF=ZZZ_F_,&
...
!!$  ENDIF
  ...
  IF (LFLEXDIA) THEN
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZDP__(JLON,JLEV)=ZDP__(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTP__(JLON,JLEV)=(ZTP__(JLON,JLEV)-ZTPMF__(JLON,JLEV))*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTPMF__(JLON,JLEV)=ZTPMF__(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDIFF__(JLON,JLEV)=ZTDIFF__(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDISS__(JLON,JLEV)=ZTDISS__(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
      ...
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTPMF__(:,1:YDCPG_DIM%KFLEVG),'TKEPRTHMF',YDDDH)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTDIFF__(:,1:YDCPG_DIM%KFLEVG),'TKEDIFF',YDDDH)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTDISS__(:,1:YDCPG_DIM%KFLEVG),'TKEDISS',YDDDH)
    ELSE
      ...
      CALL ADD_FIELD_3D(YLDDH,ZTPMF__(:,1:YDCPG_DIM%KFLEVG),'TKEPRTHMF','T','ARO',.TRUE.,.TRUE.)
      CALL ADD_FIELD_3D(YLDDH,ZTDIFF__(:,1:YDCPG_DIM%KFLEVG),'TKEDIFF','T','ARO',.TRUE.,.TRUE.)
      CALL ADD_FIELD_3D(YLDDH,ZTDISS__(:,1:YDCPG_DIM%KFLEVG),'TKEDISS','T','ARO',.TRUE.,.TRUE.)
    ENDIF
  ENDIF 
...
IF (LMICRO) THEN
  ...
  IF (MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=2,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,'(I2,X,4f10.3)')JLEV, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(NPTP,JLEV),&
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    ENDIF
    CALL ARO_LIMA(YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,NLIMA,YDCPG_DIM%KSTEP+1,NSPLITR,NSPLITG, ZDT,ZDZZ_ ,&
     & ZRHODJM__(:,1:YDCPG_DIM%KFLEVG),ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG), ZEXNREFM_, ZPABSM__(:,1:YDCPG_DIM%KFLEVG), ZPTRWNU_, &
     & ZTHM__(:,1:YDCPG_DIM%KFLEVG),ZRM_, ZLIMAM_,  ZTHS__(:,1:YDCPG_DIM%KFLEVG),ZRS_,  ZLIMAS_,  ZEVAP_, &
     & ZINPRR_NOTINCR_, ZINPRS_NOTINCR_, ZINPRG_NOTINCR_, ZINPRH_NOTINCR_,ZPFPR_,&
     & YDDDH, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH )
  ELSE
    CALL ARO_RAIN_ICE (YDCPG_DIM%KFLEVG,IKU,IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NRR,YDCPG_DIM%KSTEP+1,NSPLITR,NGFL_EZDIAG,&
     & LOSUBG_COND, CSUBG_AUCV_RC, LOSEDIC,CSEDIM, CMICRO, ZDT,ZDZZ_ ,&
     & ZRHODJM__(:,1:YDCPG_DIM%KFLEVG),ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG), ZEXNREFM_, ZPABSM__(:,1:YDCPG_DIM%KFLEVG),&
     & ZTHM__(:,1:YDCPG_DIM%KFLEVG),ZRM_, ZSIGS__(:,1:YDCPG_DIM%KFLEVG), ZNEBMNH_, ZTHS__(:,1:YDCPG_DIM%KFLEVG),ZRS_,&
  ...
  IF (LRDEPOS) THEN
    ...
    ! Swapp because IN and OUT will be needed simultaneously
    CALL SWAP_SVM
    CALL ARO_RAINAERO(YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT,NRR, PDT,ZSVMIN_,ZZZ_,&
     & ZPABSM__(:,1:YDCPG_DIM%KFLEVG),ZTHM__(:,1:YDCPG_DIM%KFLEVG),ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),&
     & YDCPG_DIM%KSTEP+1,ZRM_,ZEVAP_, ISPLITR, ZSVM_           )
    ! return to tendency
    DO JGFL=1,NGFL_EXT
      DO JLEV = 1,YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
IF (LNUDGLH.AND.YDCPG_DIM%KSTEP > NSTARTNUDGLH.AND.YDCPG_DIM%KSTEP <= NSTOPNUDGLH) THEN
  ...
  IF(MOD(ISTEP+1,NINTNUDGLH)==0) THEN
    CALL NUDGLHPREP(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZTENDT,YDCPG_DIM%KBL)
  ENDIF
  ! LHN step
  IF(MOD(ISTEP,NINTNUDGLH)==0) THEN
    ...
    CALL NUDGLH(NGPTOT,NPROMA,NGPBLKS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZACPRR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
       & ZACPRS_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),ZACPRG_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
       & ZTENDT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),JLHSTEP,YDCPG_DIM%KBL,&
       & ZEXNREFM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),.TRUE.,&
       & LLHN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZPABSM__(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZDT,&
       & ZTHM__(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZRM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,:),&
       & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),PTENDR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,1),NRR,LNUDGLHNREAD)
    ...
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YDMF_PHYS_STATE%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            PTENDR(JLON,JLEV,1)=PTENDR(JLON,JLEV,1)+RLVTT/RV/((YDMF_PHYS_STATE%T(JLON,JLEV))**2._JPRB)* &
  ...
  ELSEIF(MOD(ISTEP,NINTNUDGLH)<NTAUNUDGLH.AND.MOD(ISTEP,NINTNUDGLH)>0 &
    ...
    CALL NUDGLH(NGPTOT,NPROMA,NGPBLKS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZACPRR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
        & ZACPRS_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),ZACPRG_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
        & ZTENDT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),JLHSTEP,YDCPG_DIM%KBL,&
        & ZEXNREFM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),.FALSE.,&
        & LLHN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZPABSM__(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZDT,&
        & ZTHM__(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZRM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,:),&
        & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),PTENDR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,1),NRR,LNUDGLHNREAD)
    !IF(MYPROC==1) WRITE(NULOUT,*)'calling LH successful - convert TH to T and
...
IF (LUSECHEM) THEN
  ...
  CALL ARO_MNHC(ZSVSIN_,&
   & ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),PDT, ZTHM__(:,1:YDCPG_DIM%KFLEVG), ZPABSM__(:,1:YDCPG_DIM%KFLEVG), ZRM_, ZLAT_, ZLON_, ZALB_UV_, &
   & ZZS_, ZZENITH_,ZZZ_, IYEAR,IMONTH,IDAY, REAL(RHGMT,JPRB)+PDT/2._JPRB,&
   & YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT, NRR, YDCPG_DIM%KSTEP+1,NULOUT,IEZDIAG_CHEM, ZPEZDIAG_(:,:,IOFF_MFSHAL:NGFL_EZDIAG),ZSVS_ )
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP,YGFL, &
   & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1,PDT,         &  ! In: block indices, physicstimestep
   & PTSL=YDMF_PHYS_STATE%T,PQSL=YDMF_PHYS_STATE%Q, PA=YDVARS%A%T1, PAP=YDMF_PHYS_STATE%YCPG_PHY%PREF, PAPH=YDMF_PHYS_STATE%YCPG_PHY%PRE,   &  ! In: (T,Q,cloud) forsupersatcheck, Pfull, Phalf
   & PDYN_U=ZDUMMY,PDYN_V=ZDUMMY,PDYN_T=ZDUMMY,PDYN_Q=ZDUMMY,  &  ! In: dummy nonphys tendencies
   & PUNP_U=YDMF_PHYS%OUT%TENDU,PUNP_V=YDMF_PHYS%OUT%TENDV,PUNP_T=PTENDT,PUNP_Q=PTENDR(:,:,1), &  ! In: (u,v,t,qv) tendencies to perturb
   & PMULNOISE=PGP2DSDT(1,1,1),                           &  ! In: stochphy 3D random multiplicative pattern (less one)
   & PTENU=YDMF_PHYS%OUT%TENDU,PTENV=YDMF_PHYS%OUT%TENDV,PTENT=PTENDT,PTENQ=PTENDR(:,:,1) )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF(LFORCENL.AND.(YDCPG_DIM%KSTEP*(TSPHY/RHOUR)>=NFORCESTART).AND.&
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ...
      PTENDT(JLON,JLEV)=PTENDT(JLON,JLEV)+AMAGSTOPH_CASBS*YDMF_PHYS%FOR%T(JLON,JLEV)
      PTENDR(JLON,JLEV,1)=PTENDR(JLON,JLEV,1)+AMAGSTOPH_CASBS*YDMF_PHYS%FOR%Q(JLON,JLEV)
    ENDDO
  ENDDO
ENDIF
...
IF(LLMSE.OR.LSFORCS) THEN
  ...
  ENDDO
  PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVTS)=ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVEMIS)=ZEMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVQS)=ZQS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  DO JSW=1,NSW
    PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR-1+JSW)=ZALBP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)
    PGPAR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA-1+JSW)=ZALBD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)
  ENDDO
ENDIF
...
IF (LXXDIAGH) THEN
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDMF_PHYS%OUT%DIAGH(JLON)=YDMF_PHYS%OUT%DIAGH(JLON)+ZQGM(JLON,JLEV)*YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
ENDIF
! lightening density
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,&
    ...
    &YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZTM,YDMF_PHYS_STATE%YCPG_PHY%W,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS%OUT%FLASH)
ENDIF
...
IF (LINTFLEX) THEN
  ...
  CALL APL_AROME2INTFLEX(YGFL,YDPARAR,YDPHY,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG, PDT,&
   & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%T, PGPAR(1,MVTS), YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,&
   & ZFPR,&! precipitation fluxes
   & LLRAD, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO,&! radiative fluxes
   & YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, PTENDT,&! momentum and temperature tendencies
...
CALL PPWETPOINT(YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDMF_PHYS_STATE%YCPG_PHY%PRE(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%TCLS,&
  & YDMF_PHYS%OUT%QCLS,ZQCM(:,YDCPG_DIM%KFLEVG),ZQIM(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%TPWCLS)
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
      CALL PPWETPOINT(YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDMF_PHYS_STATE%YCPG_PHY%PREF(:,JLEV),ZTM(:,JLEV),&
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FPLSN(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FPLSG(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_TMP%PRC%DPRECIPS(:,NDTPRECCUR))
  ENDIF
  ...
  IF (LDPRECIPS2) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FPLSN(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FPLSG(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_TMP%PRC%DPRECIPS2(:,NDTPRECCUR2))
