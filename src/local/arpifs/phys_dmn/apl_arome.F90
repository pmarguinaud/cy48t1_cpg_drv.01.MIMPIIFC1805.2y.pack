...
ZTENDQ    => ZTENDGFLR (:,:,IRR+0) ! 1 Q
ZTENDL    => ZTENDGFLR (:,:,IRR+1) ! 2 L
ZTENDR    => ZTENDGFLR (:,:,IRR+2) ! 3 R
ZTENDI    => ZTENDGFLR (:,:,IRR+3) ! 4 I
ZTENDS    => ZTENDGFLR (:,:,IRR+4) ! 5 S
ZTENDG    => ZTENDGFLR (:,:,IRR+5) ! 6 G
...
IF(LKFBCONV) THEN
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ...
      ZTENDQ(JLON,JLEV) = ZTENDQ(JLON,JLEV) + ZCVTENDRV_(JLON,JLEV)*ZQDM(JLON,JLEV)  
      ZTENDL(JLON,JLEV) = ZTENDL(JLON,JLEV) + ZCVTENDRC_(JLON,JLEV)*ZQDM(JLON,JLEV)  
      ZTENDI(JLON,JLEV) = ZTENDI(JLON,JLEV) + ZCVTENDRI_(JLON,JLEV)*ZQDM(JLON,JLEV)  
  ...
  IF(MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,*)ZTENDQ(NPTP,JLEV),ZTENDL(NPTP,JLEV),ZTENDI(NPTP,JLEV)
    ENDDO
  ENDIF
  ...
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    DO JLEV = 2 ,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTENDQ(JLON,JLEV)=ZTENDQ(JLON,JLEV) + ZTENDQVUP(JLON,JLEV)
      ENDDO
    ENDDO
    ...
    IF(LTOTPREC)THEN
      ...
      DO JLEV= 1, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          ...
          ZTENDR(JLON,JLEV)=ZTENDR(JLON,JLEV)+ZUPGENL(JLON,JLEV)
          ZTENDS(JLON,JLEV)=ZTENDS(JLON,JLEV)+ZUPGENN(JLON,JLEV)
        ENDDO
      ENDDO 
    ENDIF
  ...
  DO JDRAFT=IDRAFT,3
    ...
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        !inversion niveaux tendances des ri et conversion en qi en multipliant par qd
        ZTENDQ(JLON,JLEV) = ZTENDQ(JLON,JLEV)+ZRTS_(JLON,JLEV)*ZQDM(JLON,JLEV)
      ENDDO
    ENDDO
...
!!$  ENDIF
  ...
  !inversion niveaux tendances des rv et conversion en qv en multipliant par qd
      ZTENDQ(JLON,JLEV)= ZTENDQ(JLON,JLEV)+ZTENDRT_TURB__(JLON,JLEV)*ZQDM(JLON,JLEV)
    ENDDO
  ENDDO
...
IF (LMICRO) THEN
  ...
  IF(MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,'(I2,X,6E11.4)')JLEV,ZTENDQ(NPTP,JLEV),ZTENDL(NPTP,JLEV),&
       & ZTENDR(NPTP,JLEV),ZTENDI(NPTP,JLEV),ZTENDS(NPTP,JLEV),ZTENDG(NPTP,JLEV)  
    ENDDO
...
IF (LNUDGLH.AND.YDCPG_DIM%KSTEP > NSTARTNUDGLH.AND.YDCPG_DIM%KSTEP <= NSTOPNUDGLH) THEN
  ...
  IF(MOD(ISTEP,NINTNUDGLH)==0) THEN
    ...
    & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG), ZTENDQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),                            &
    & NRR, LNUDGLHNREAD)
      ...
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YLMF_PHYS_BASE_STATE%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            ZTENDQ(JLON,JLEV)=ZTENDQ(JLON,JLEV)+RLVTT/RV/((YLMF_PHYS_BASE_STATE%T(JLON,JLEV))**2._JPRB)* &
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  & PUNP_V=YDMF_PHYS%OUT%TENDV, PUNP_T=ZTENDT, PUNP_Q=ZTENDQ, PMULNOISE=PGP2DSDT(1, 1, 1), PTENU=YDMF_PHYS%OUT%TENDU, &
  & PTENV=YDMF_PHYS%OUT%TENDV, PTENT=ZTENDT, PTENQ=ZTENDQ )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF(LFORCENL.AND.(YDCPG_DIM%KSTEP*(TSPHY/RHOUR)>=NFORCESTART).AND.&
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ...
      ZTENDQ(JLON,JLEV)=ZTENDQ(JLON,JLEV)+AMAGSTOPH_CASBS*YDMF_PHYS%FOR%Q(JLON,JLEV)
    ENDDO
  ENDDO
ENDIF
