...
& YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDCPG_SL1, YDVARS, YDGMV, YDSURF, YDCFU, YDXFU,   &
& YDMODEL, LDCONFX, PDTPHY, PGFL, PGP2DSDT, PGMVT1, PGFLT1, PTRAJ_PHYS, YDDDH)
...
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDQ    (:,:)  ! 1
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDL    (:,:)  ! 2
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDR    (:,:)  ! 3
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDI    (:,:)  ! 4
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDS    (:,:)  ! 5
REAL(KIND=JPRB), POINTER, CONTIGUOUS :: ZTENDG    (:,:)  ! 6
...
IF (LLDIAB) THEN
  ...
  IF (LL_SAVE_PHSURF) THEN
    CALL MF_PHYS_SAVE_PHSURF_PART1 (YDCPG_DIM, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH, ZSAV_FHPS,                             &
...
IF (.NOT. LLDONE) THEN
WRITE (0, *) __FILE__, ':', __LINE__, " IRR = ", IRR
WRITE (0, *) __FILE__, ':', __LINE__, " NRR = ", YDMODEL%YRML_PHY_MF%YRPARAR%NRR
LLDONE = .TRUE.
ENDIF

ZTENDQ    => ZTENDGFLR (:,:,IRR+0)
ZTENDL    => ZTENDGFLR (:,:,IRR+1)
ZTENDR    => ZTENDGFLR (:,:,IRR+2)
ZTENDI    => ZTENDGFLR (:,:,IRR+3)
ZTENDS    => ZTENDGFLR (:,:,IRR+4)
ZTENDG    => ZTENDGFLR (:,:,IRR+5)
...
DO JLEV = 1,YDCPG_DIM%KFLEVG
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ZZZ_F_(JLON,JLEV)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(JLON,JLEV)*ZINVG
...
IF (LRSLDDH) THEN
  DO JLEV=1, YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ZTKEM4SLDDH(JLON,JLEV)=MAX(YLMF_PHYS_BASE_STATE%TKE(JLON,JLEV),PPTKEMIN)
    ENDDO
  ENDDO
  ZTKEM => ZTKEM4SLDDH(:,:)
ELSE
  ZTKEM => YLMF_PHYS_BASE_STATE%TKE(:,:)
  !test TKE > 0.
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
   ...
   !et convertir q en r 
    IF (NRR==7) THEN
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YLMF_PHYS_BASE_STATE%Q(JLON,JLEV)-YLMF_PHYS_BASE_STATE%L(JLON,JLEV)-YLMF_PHYS_BASE_STATE%R(JLON,JLEV)&
         & -YLMF_PHYS_BASE_STATE%I(JLON,JLEV)-YLMF_PHYS_BASE_STATE%S(JLON,JLEV)-YLMF_PHYS_BASE_STATE%G(JLON,JLEV)-YLMF_PHYS_BASE_STATE%H(JLON,JLEV)   
      ENDDO
    ELSE
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YLMF_PHYS_BASE_STATE%Q(JLON,JLEV)-YLMF_PHYS_BASE_STATE%L(JLON,JLEV)-YLMF_PHYS_BASE_STATE%R(JLON,JLEV)&
         & -YLMF_PHYS_BASE_STATE%I(JLON,JLEV)-YLMF_PHYS_BASE_STATE%S(JLON,JLEV)-YLMF_PHYS_BASE_STATE%G(JLON,JLEV)
      ENDDO
    ENDIF
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
   !initialisation de ZRHODREFM__ (=qd*zrho) 
      ZRHO=YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV)/(YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R(JLON,JLEV)*YLMF_PHYS_BASE_STATE%T(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
      ZRHODJM__(JLON,JLEV)=YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
  !initialisation de ZEXNREFM_
      ZEXNREFM_(JLON,JLEV)=(YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV)*ZINVATM)**(ZRSCP)
 ! vent horizontal et TKE
      ZPABSM__(JLON,JLEV)=YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV)
      ZUM__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%U(JLON,JLEV)
      ZVM__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%V(JLON,JLEV)
      ZWM__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)
      ZTKEM__(JLON,JLEV)= ZTKEM(JLON,JLEV)
      ZZZ_(JLON,JLEV)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
  ...
  IF (LOSIGMAS) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG 
      ZSIGM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)= YLMF_PHYS_BASE_STATE%SRC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    ENDDO
  ENDIF
  ...
  IF ( LKFBCONV.AND.LOSUBG_COND.AND..NOT.LOSIGMAS) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG 
      ZMFM_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)=YLMF_PHYS_BASE_STATE%SRC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV) 
    ENDDO
  ENDIF
!!! initialisation des variables d etat MNH �t
  ...
  DO JLEV = 1, YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZTHM__(JLON,JLEV)=YLMF_PHYS_BASE_STATE%T(JLON,JLEV)/ZEXNREFM_(JLON,JLEV)
      ZRM_(JLON,JLEV,1)=YLMF_PHYS_BASE_STATE%Q(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,2)=YLMF_PHYS_BASE_STATE%L(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,3)=YLMF_PHYS_BASE_STATE%R(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,4)=YLMF_PHYS_BASE_STATE%I(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,5)=YLMF_PHYS_BASE_STATE%S(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,6)=YLMF_PHYS_BASE_STATE%G(JLON,JLEV)/ZQDM(JLON,JLEV)  
    ENDDO
  ENDDO
  ...
  IF (NRR==7) THEN
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZRM_(JLON,JLEV,7)=YLMF_PHYS_BASE_STATE%H(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ENDDO
    ENDDO
  ENDIF
...
!!! suivant les parametrisations)   
  ...
  DO JLEV = 1, YDCPG_DIM%KFLEVG 
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZUS__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%U(JLON,JLEV)*ZINVDT
      ZVS__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%V(JLON,JLEV)*ZINVDT
      ZWS__(JLON,JLEV)= YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)*ZINVDT
...
!!! Initialisations temporaires d'arguments non-utilises
  ...
  DO JGFL=1,NGFL_EXT
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZSVM_(JLON,JLEV,JGFL)=YLMF_PHYS_BASE_STATE%P1EXT(JLON,JLEV,JGFL)
        ZSVS_(JLON,JLEV,JGFL)=YLMF_PHYS_BASE_STATE%P1EXT(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JGFL=1,NLIMA
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZLIMAM_(JLON,JLEV,JGFL)=YLMF_PHYS_BASE_STATE%P1LIMA(JLON,JLEV,JGFL)
        ZLIMAS_(JLON,JLEV,JGFL)=YLMF_PHYS_BASE_STATE%P1LIMA(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
...
IF (LDIAGWMAX) THEN
  ...
  ! calcul de wmax
    DO JLEV = 1 , YDCPG_DIM%KFLEVG
      ...
      Z_WMIN=0._JPRB
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF (YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)>Z_WMAX) THEN
          Z_WMAX=YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)
        ENDIF
        IF (YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)<Z_WMIN) THEN
          Z_WMIN=YLMF_PHYS_BASE_STATE%YCPG_PHY%W(JLON,JLEV)
        ENDIF
      ENDDO
    ENDDO
  ENDIF
...
IF (LFLEXDIA) THEN
  ...
  DO JR=1,NRR
    ZTENDRBAK(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,JR)=ZTENDRA(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,JR)
  ENDDO
ENDIF
...
IF (LMICRO) THEN
  ...
  !reinitialisation de ZRHODREFM__ (=qd*zrho)
      ZRHO=YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV)/(ZRHM(JLON,JLEV)*ZTM(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
    ENDDO
  ENDDO
  ...
  ZAPHIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  CALL GPGEO(YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, ZAPHIM, ZAPHIFM,          &
  & ZTM, ZRHM, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDGEOMETRY%YRVERT_GEOM&
  &           )
...
ELSE
  ...
  ZTM (YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZRHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%I(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQCM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%L(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQRM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%R(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%S(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZQGM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%G(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  IF (NRR==7) THEN
    ZQHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%H(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ELSE
    ZQHM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=0._JPRB
  ENDIF
  ZCPM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)
  ZAPHIM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,0:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,0:YDCPG_DIM%KFLEVG)
  ZAPHIFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
  ZZZ_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*ZINVG
...
IF (LFLEXDIA) THEN
  DO JLEV = 1, YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
      ZTMPAF(JLON,JLEV)=(ZTENDT(JLON,JLEV)-ZTENDTBAK(JLON,JLEV))*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG*ZCPM(JLON,JLEV)
    ENDDO
  ENDDO
  IF (LDDH_OMP) THEN
  ...
  DO JR=1,NRR
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZTMPAF(JLON,JLEV)=(ZTENDRA(JLON,JLEV,JR)-ZTENDRA(JLON,JLEV,JR))*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZTMPAF(JLON,JLEV)=YDVARS%A%T1(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
...
! specific to new data flow for diagnostics
  IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZCON3(JLON,JLEV) = YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)*ZEXNREFM_(JLON,JLEV)
      ENDDO
    ENDDO
  ...
  ELSE
    ...
    & ZEXNREFM_, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
ENDDO
DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ZDZZ_F_(JLON,YDCPG_DIM%KFLEVG)=ZZZ_F_(JLON,YDCPG_DIM%KFLEVG)-YDVARS%GEOMETRY%OROG%T0(JLON)*ZINVG
ENDDO
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA) = YLMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  ZTN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)    = YLMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  ZQS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)    = YLMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ZRHODREFM(JLON) = YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,YDCPG_DIM%KFLEVG)/(YLMF_PHYS_BASE_STATE%T(JLON,YDCPG_DIM%KFLEVG)*YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG))
    ZTHETAS(JLON)   = ZTSURF(JLON)*(RATM/YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE(JLON,YDCPG_DIM%KFLEVG))**RKAPPA
  ENDDO
  ...
  & ZTN, ZTSURF, YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_BASE_STATE%Q(:, YDCPG_DIM%KFLEVG), YLMF_PHYS_BASE_STATE%U(:, YDCPG_DIM%KFLEVG), &
  & YLMF_PHYS_BASE_STATE%V(:, YDCPG_DIM%KFLEVG), ZTHETAS, ZSFTH_, ZSFRV_, ZSFU_, ZSFV_)
...
!* Compute PBL-diagnostics
  ...
  & YDCPG_DIM%KFLEVG, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YLMF_PHYS_BASE_STATE%U(:, 1:YDCPG_DIM%KFLEVG),                                          &
  & YLMF_PHYS_BASE_STATE%V(:, 1:YDCPG_DIM%KFLEVG), ZCAPE, ZDCAPE, ZTKEM(:, 1:YDCPG_DIM%KFLEVG), YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(:, 1:YDCPG_DIM%KFLEVG), &
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  CALL ACTQSAT ( YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTQSAT, YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, &
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  ELSE
    ...
    IF (NAER >= 1 ) THEN
      IF(YSD_VAD%NUMFLDS >= 4) THEN
        ...
        & YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF,   &
  ...
  IF (NOZOCL == 1) THEN
    ...
    & 0, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YDVARS%GEOMETRY%GEMU%T0, ZROZ)
    DO JK=1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,JK)=ZROZ(JLON,JK)/YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JK)
      ENDDO
    ENDDO
  ELSEIF (NOZOCL == 2) THEN
    ! as in ARPEGE (from clim profiles)
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, ZQO3, .FALSE., YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, &
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & YDCPG_DIM%KSW, ZALBD, ZALBP, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, YDVARS%A%T1,     &
     & ZQO3, ZAER, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZEMIS, ZRDG_MU0M, ZQV, ZQSAT, ZQICE, ZQLIQ,                 &
     & ZQSM, ZQRM, YDMF_PHYS_SURF%GSD_VF%PLSM, ZTM, ZTSURF, ZGP2DSPP, ZP1EZDIAG, YDMF_PHYS%RAD%EMTD,              &
     ...
     & YDCPG_GPAR%SWDIF, ZRDG_MU0LU, ZALBD1, ZFRSOLU)
   ELSE
     IF (LLMSE) THEN 
       DO JSW=1,NSW 
   ...
   & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, ZEMIS, YDMF_PHYS%RAD%EMTD, ZRDG_MU0, ZQVM,                   &
...
& YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, ZDECRD,         &
  ...
  ELSE
    ...
    IF (NAER >= 1) THEN
      IF (YSD_VAD%NUMFLDS >= 4) THEN
        ...
        & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, ZTM, ZTSURF,                     &
...
!+++ End of redundant input preparations for ACRANEB
  ...
  & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDCPG_DIM%KSTEP, YDCFU%NFRRC, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, &
  & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,                       &
  & ZNEB0, ZQV, ZQCO2, ZQICE, ZQLIQ, ZQO3, YLMF_PHYS_BASE_STATE%T, ZALBD1, ZALBP1, ZEMIS, YDVARS%GEOMETRY%GELAM%T0,                          &
...
IF(LKFBCONV) THEN
  ...
  & YDCPG_DIM%KFLEVG, YDVARS%GEOMETRY%GM%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZPABSM__(:, 1:YDCPG_DIM%KFLEVG),                                  &
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ...
      ZTENDRA(JLON,JLEV,1) = ZTENDRA(JLON,JLEV,1) + ZCVTENDRV_(JLON,JLEV)*ZQDM(JLON,JLEV)  
      ZTENDRA(JLON,JLEV,2) = ZTENDRA(JLON,JLEV,2) + ZCVTENDRC_(JLON,JLEV)*ZQDM(JLON,JLEV)  
      ZTENDRA(JLON,JLEV,4) = ZTENDRA(JLON,JLEV,4) + ZCVTENDRI_(JLON,JLEV)*ZQDM(JLON,JLEV)  
      ...
      ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZCVTENDT_(JLON,JLEV)*(RATM/YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)  
    ENDDO
  ENDDO
  DO JLON =YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
  ...
  IF(MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,*)ZTENDRA(NPTP,JLEV,1),ZTENDRA(NPTP,JLEV,2),ZTENDRA(NPTP,JLEV,4)
    ENDDO
  ENDIF
  ...
  IF (MINVAL(ZDEPTH_HEIGHT_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,IKB)) <= 0._JPRB) THEN
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      IF (ZDEPTH_HEIGHT_(JLON,IKB) <= 0._JPRB) THEN
        WRITE (NULOUT,*)'sous apl_arome pb height en', JLON,ZAPHIFM(JLON,YDCPG_DIM%KFLEVG),YDVARS%GEOMETRY%OROG%T0(JLON)
      ENDIF
    ENDDO
  ENDIF
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      ...
      & YDVARS%GEOMETRY%RINDY%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZUM__(:, IKB), ZVM__(:, IKB), ZTM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG),                   &
      & ZRM_(:, IKB, 1), ZSVMB_, RCARDI, ZRHODREFM__(:, IKB), ZPABSM__(:, IKB), YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG), &
...
!ZSFRV_(KIDIA:KFDIA) = 0._JPRB
    ...
    IF (LLMSE_DIAG) THEN
      ...
      & YDMF_PHYS%OUT%FRSO(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, 1), YDVARS%GEOMETRY%RINDX%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),   &
...
!* Compute PBL-diagnostics
   ...
   & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, ZTM,        &
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      ZZS_FTH_(JLON)=-1._JPRB*ZSFTH_(JLON)*(YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE(JLON,YDCPG_DIM%KFLEVG)*ZINVATM)**(ZRSCP)
    ...
    & YDCPG_DIM%KFLEVG, INDRAFT, PDTPHY, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, ZTM, ZQVM, ZQCM, ZQIM,         &
    & YDVARS%A%T1, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF, ZAPHIFM, ZAPHIM,              &
    ...
    DO JLEV = 2 ,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTENDRA(JLON,JLEV,1)=ZTENDRA(JLON,JLEV,1) + ZTENDQVUP(JLON,JLEV)
      ENDDO
    ENDDO
      ...
      DO JLEV= 1, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          ...
          ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZTENDTUP(JLON,JLEV)*(RATM/&
           & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)
          !Update rain/snow tendencies:
          ZTENDRA(JLON,JLEV,3)=ZTENDRA(JLON,JLEV,3)+ZUPGENL(JLON,JLEV)
          ZTENDRA(JLON,JLEV,5)=ZTENDRA(JLON,JLEV,5)+ZUPGENN(JLON,JLEV)
        ENDDO
      ENDDO 
    ENDIF
  ...
  DO JDRAFT=IDRAFT,3
    ...
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        !inversion niveaux tendances des ri et conversion en qi en multipliant par qd
        ZTENDRA(JLON,JLEV,1) = ZTENDRA(JLON,JLEV,1)+ZRTS_(JLON,JLEV)*ZQDM(JLON,JLEV)
      ENDDO
    ENDDO
...
!!$  ENDIF
  ...
  IF (LFLEXDIA) THEN
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZDP__(JLON,JLEV)=ZDP__(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTP__(JLON,JLEV)=(ZTP__(JLON,JLEV)-ZTPMF__(JLON,JLEV))*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTPMF__(JLON,JLEV)=ZTPMF__(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDIFF__(JLON,JLEV)=ZTDIFF__(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDISS__(JLON,JLEV)=ZTDISS__(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
  ...
  !inversion niveaux tendances des rv et conversion en qv en multipliant par qd
      ZTENDRA(JLON,JLEV,1)= ZTENDRA(JLON,JLEV,1)+ZTENDRT_TURB__(JLON,JLEV)*ZQDM(JLON,JLEV)
    ENDDO
  ENDDO
...
IF (LMICRO) THEN
  ...
  IF (MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=2,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,'(I2,X,4f10.3)')JLEV, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(NPTP,JLEV),&
  ...
  IF(MOD(YDCPG_DIM%KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      WRITE(NULOUT,'(I2,X,6E11.4)')JLEV,ZTENDRA(NPTP,JLEV,1),ZTENDRA(NPTP,JLEV,2),&
       & ZTENDRA(NPTP,JLEV,3),ZTENDRA(NPTP,JLEV,4),ZTENDRA(NPTP,JLEV,5),ZTENDRA(NPTP,JLEV,6)  
    ENDDO
...
IF (LNUDGLH.AND.YDCPG_DIM%KSTEP > NSTARTNUDGLH.AND.YDCPG_DIM%KSTEP <= NSTOPNUDGLH) THEN
  ...
  IF(MOD(ISTEP,NINTNUDGLH)==0) THEN
    ...
    & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG), ZTENDRA(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG, 1),                            &
    & NRR, LNUDGLHNREAD)
    ...
    DO JLEV = 1,YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YLMF_PHYS_BASE_STATE%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            ZTENDRA(JLON,JLEV,1)=ZTENDRA(JLON,JLEV,1)+RLVTT/RV/((YLMF_PHYS_BASE_STATE%T(JLON,JLEV))**2._JPRB)* &
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  & 1, PDTPHY, PTSL=YLMF_PHYS_BASE_STATE%T, PQSL=YLMF_PHYS_BASE_STATE%Q, PA=YDVARS%A%T1, PAP=YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF,              &
  & PAPH=YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE, PDYN_U=ZDUMMY, PDYN_V=ZDUMMY, PDYN_T=ZDUMMY, PDYN_Q=ZDUMMY, PUNP_U=YDMF_PHYS%OUT%TENDU, &
  & PUNP_V=YDMF_PHYS%OUT%TENDV, PUNP_T=ZTENDT, PUNP_Q=ZTENDRA(:, :, 1), PMULNOISE=PGP2DSDT(1, 1, 1), PTENU=YDMF_PHYS%OUT%TENDU, &
  & PTENV=YDMF_PHYS%OUT%TENDV, PTENT=ZTENDT, PTENQ=ZTENDRA(:, :, 1) )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF(LFORCENL.AND.(YDCPG_DIM%KSTEP*(TSPHY/RHOUR)>=NFORCESTART).AND.&
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ...
      ZTENDRA(JLON,JLEV,1)=ZTENDRA(JLON,JLEV,1)+AMAGSTOPH_CASBS*YDMF_PHYS%FOR%Q(JLON,JLEV)
    ENDDO
  ENDDO
ENDIF
...
IF (LXXDIAGH) THEN
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDMF_PHYS%OUT%DIAGH(JLON)=YDMF_PHYS%OUT%DIAGH(JLON)+ZQGM(JLON,JLEV)*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
ENDIF
! lightening density
IF (LFLASH) THEN
  ...
  & ZQCM, ZQIM, ZQRM, ZQSM, ZQGM, ZQHM, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZTM, YLMF_PHYS_BASE_STATE%YCPG_PHY%W,  &
...
IF (LINTFLEX) THEN
  ...
  & PDTPHY, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%T,           &
  & YDCPG_GPAR%VTS, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZFPR, LLRAD, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO,          &
...
CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YLMF_PHYS_BASE_STATE%YCPG_PHY%PRE(:, YDCPG_DIM%KFLEVG), &
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
      CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREF(:, JLEV), &
  ...
  IF (LDPRECIPS) THEN
   ...
   & YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, ZDZZ, ZTPW, ZQCM, YDMF_PHYS%OUT%FPLSL(:, YDCPG_DIM%KFLEVG), &
   & YDMF_PHYS%OUT%FPLSN(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%FPLSG(:, YDCPG_DIM%KFLEVG), ZPRC_DPRECIPS(:, NDTPRECCUR)                      &
   &                                                                     )
  ENDIF
  ...
  IF (LDPRECIPS2) THEN
   ...
   CALL DPRECIPS(YDPRECIPS, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDVARS%GEOMETRY%OROG%T0,                               &
   & YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, ZDZZ, ZTPW, ZQCM, YDMF_PHYS%OUT%FPLSL(:, YDCPG_DIM%KFLEVG), &
   & YDMF_PHYS%OUT%FPLSN(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%FPLSG(:, YDCPG_DIM%KFLEVG), ZPRC_DPRECIPS2(:, NDTPRECCUR2)                    &
   &                                                           )
...
IF ( LINTFLEX ) THEN
  ...
  & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%YGSP_RR%T, &
  ...
  & YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDD, ZTENDGFL, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,  &
...
ELSE
  ...
  & ZTENDD, YDCPG_SL1%ZVIEW, PGMVT1, PGFLT1)
ENDIF
...
IF (LLDIAB) THEN
  IF (LL_SAVE_PHSURF) THEN
    CALL MF_PHYS_SAVE_PHSURF_PART2 (YDCPG_DIM, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH, ZSAV_FHPS,                             &
    & ZSAV_GZ0F, ZSAV_GZ0HF, ZSAV_HV, ZSAV_PBLH, ZSAV_QSH, ZSAV_UDAL, ZSAV_UDGRO, ZSAV_UDOM,                            &
    & ZSAV_UNEBH, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VH%PPBLH, YDMF_PHYS_SURF%GSD_VH%PQSH,                  &
    & YDMF_PHYS_SURF%GSD_VH%PSPSH, YDMF_PHYS_SURF%GSD_VK%PUDGRO, YDMF_PHYS_SURF%GSD_VV%PHV, YDMF_PHYS_SURF%GSD_VV%PZ0H, &
    & YDVARS%DAL%T0, YDVARS%DOM%T0, YDVARS%UAL%T0, YDVARS%UEN%T0, YDVARS%UNEBH%T0, YDVARS%UOM%T0,                       &
