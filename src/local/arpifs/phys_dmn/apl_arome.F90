...
IF (LLDIAB) THEN
  ...
  & YDCPG_PHY0%XYB%RDELP, YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0, ZRDG_MU0, ZRDG_MU0LU, &
  & ZRDG_MU0M, ZRDG_MU0N, ZRDG_CVGQ)
...
IF (LLDIAB) THEN
  ...
  IF (LL_SAVE_PHSURF) THEN
    ...
    & ZSAV_FHPS, ZSAV_GZ0F, ZSAV_GZ0HF, ZSAV_HV,                    &
    & ZSAV_PBLH, ZSAV_QSH, ZSAV_UDAL, ZSAV_UDGRO,                   &
    & ZSAV_UDOM, ZSAV_UNEBH, YDMF_PHYS_SURF, YDVARS%DAL%T0, YDVARS%DOM%T0,                    &
    & YDVARS%UAL%T0, YDVARS%UEN%T0, YDVARS%UNEBH%T0, YDVARS%UOM%T0, YDSURF, YDMODEL)
  ENDIF
ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZQO3, ZAER, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZEMIS, ZRDG_MU0M, ZQV, ZQSAT,                   &
     & ZQICE, ZQLIQ, ZQSM, ZQRM, YDMF_PHYS_SURF%GSD_VF%PLSM, ZTM, ZTSURF, ZGP2DSPP, ZP1EZDIAG, YDMF_PHYS%RAD%EMTD, &
   ...
   & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PRE, ZEMIS, YDMF_PHYS%RAD%EMTD, ZRDG_MU0,                   &
...
!+++ End of redundant input preparations for ACRANEB
  ...
  & YDGSGEOM%GEMU, ZRDG_MU0, ZRDG_MU0LU, ZTSURF, ZDECRD, ZCLCT_RAD, YDMF_PHYS%OPT%GDEOSI,           &
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      ...
      & ZDTMSE, ZDEPTH_HEIGHT_(:, IKB), ZZS_, XZSEPS, ZRDG_MU0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                    &
      & ZRDG_MU0N(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDGSGEOM%GELAM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                &
