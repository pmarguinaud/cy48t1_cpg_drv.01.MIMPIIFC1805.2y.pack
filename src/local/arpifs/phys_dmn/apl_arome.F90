...
SUBROUTINE APL_AROME(YDGEOMETRY,YDCPG_DIM,YDMF_PHYS_STATE,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 & KTDIA  , KLEV , KSTEP ,&
...
DO JLEV=1,KLEV
  ...
  YDMF_PHYS%OUT%TENDU(KIDIA:KFDIA,JLEV)=0.0_JPRB
  YDMF_PHYS%OUT%TENDV(KIDIA:KFDIA,JLEV)=0.0_JPRB
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
 ...
 ! vent horizontal et TKE
  ...
  IF (LOSIGMAS) THEN
    DO JLEV = 1, KLEV 
      ZSIGM_(KIDIA:KFDIA,JLEV)= YDMF_PHYS_STATE%SRC(KIDIA:KFDIA,JLEV)
    ENDDO
  ENDIF
  ...
  IF ( LKFBCONV.AND.LOSUBG_COND.AND..NOT.LOSIGMAS) THEN
    DO JLEV = 1, KLEV 
      ZMFM_(KIDIA:KFDIA,JLEV)=YDMF_PHYS_STATE%SRC(KIDIA:KFDIA,JLEV) 
    ENDDO
  ENDIF
...
IF (LMICRO) THEN
  ...
  DO JLEV=1,KLEV
    YDVARS%A%T1(KIDIA:KFDIA,JLEV)=ZNEBMNH_(KIDIA:KFDIA,JLEV) 
  ENDDO
  ...
  ZAPHIM(KIDIA:KFDIA,KLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHI(KIDIA:KFDIA,KLEV)
  CALL GPGEO(KLON,KIDIA,KFDIA,KLEV,ZAPHIM,ZAPHIFM,ZTM,ZRHM,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDGEOMETRY%YRVERT_GEOM)
...
ELSE
  ...
  ZTM (KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%T(KIDIA:KFDIA,1:KLEV)
  ZRHM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%YCPG_DYN%RCP%R(KIDIA:KFDIA,1:KLEV)
  ZQVM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%Q(KIDIA:KFDIA,1:KLEV)
  ZQIM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%I(KIDIA:KFDIA,1:KLEV)
  ZQCM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%L(KIDIA:KFDIA,1:KLEV)
  ZQRM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%R(KIDIA:KFDIA,1:KLEV)
  ZQSM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%S(KIDIA:KFDIA,1:KLEV)
  ZQGM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%G(KIDIA:KFDIA,1:KLEV)
  IF (NRR==7) THEN
    ZQHM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%H(KIDIA:KFDIA,1:KLEV)
  ELSE
    ZQHM(KIDIA:KFDIA,1:KLEV)=0._JPRB
  ENDIF
  ZCPM(KIDIA:KFDIA,KTDIA:KLEV)=YDMF_PHYS_STATE%YCPG_DYN%RCP%CP(KIDIA:KFDIA,KTDIA:KLEV)
  ZAPHIM(KIDIA:KFDIA,0:KLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHI(KIDIA:KFDIA,0:KLEV)
  ZAPHIFM(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHIF(KIDIA:KFDIA,1:KLEV)
  ZZZ_(KIDIA:KFDIA,1:KLEV)=YDMF_PHYS_STATE%YCPG_DYN%PHI(KIDIA:KFDIA,1:KLEV)*ZINVG
  !initialisation of PCLFS outside LMICRO to be zero in case LMICRO=F
  YDVARS%A%T1(KIDIA:KFDIA,1:KLEV)=0._JPRB
...
! specific to new data flow for diagnostics
  ...
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(KLON,KLEV,KSTEP,KFDIA,ZQDM,ZEXNREFM_,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  ZTSURF(KIDIA:KFDIA) = YDMF_PHYS_STATE%T(KIDIA:KFDIA,KLEV)
  ZTN(KIDIA:KFDIA)    = YDMF_PHYS_STATE%T(KIDIA:KFDIA,KLEV)
  ZQS(KIDIA:KFDIA)    = YDMF_PHYS_STATE%Q(KIDIA:KFDIA,KLEV)
  DO JLON=KIDIA,KFDIA
    ZRHODREFM(JLON) = YDMF_PHYS_STATE%YCPG_PHY%PREF(JLON,KLEV)/(YDMF_PHYS_STATE%T(JLON,KLEV)*YDMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,KLEV))
    ZTHETAS(JLON)   = ZTSURF(JLON)*(RATM/YDMF_PHYS_STATE%YCPG_PHY%PRE(JLON,KLEV))**RKAPPA
  ENDDO

  LLAROME=.TRUE.
  CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS, LLAROME, KIDIA , KFDIA  , KLON, YDMF_PHYS_STATE%YCPG_DYN%PHIF(:,KLEV), &
   & ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,ZTN,ZTSURF,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_STATE%Q(:,KLEV), YDMF_PHYS_STATE%U(:,KLEV), YDMF_PHYS_STATE%V(:,KLEV), ZTHETAS, &
...
!* Compute PBL-diagnostics
   ...
   ZCAPE(:)=0._JPRB
   ZDCAPE(:)=0._JPRB
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,KIDIA,KFDIA,KLON,KLEV,YDMF_PHYS%OUT%UCLS,&
   & YDMF_PHYS%OUT%VCLS,YDMF_PHYS_STATE%U(:,1:KLEV),YDMF_PHYS_STATE%V(:,1:KLEV),ZCAPE,ZDCAPE,ZTKEM(:,1:KLEV),YDMF_PHYS_STATE%YCPG_DYN%PHIF(:,1:KLEV),POROG,&
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  CALL ACTQSAT ( YDPHY,KIDIA,KFDIA,KLON,NTQSAT,KLEV, YDMF_PHYS_STATE%YCPG_PHY%PREF, ZCPM, ZQVM, ZTM,&
  ...
  IF (YIRAD%LGP) YDVARS%IRAD%T1(KIDIA:KFDIA,:) = ZQICE(KIDIA:KFDIA,:)
  IF (YLRAD%LGP) YDVARS%LRAD%T1(KIDIA:KFDIA,:) = ZQLIQ(KIDIA:KFDIA,:)
  ...
  IF (NSW==6.OR.NSW==1) THEN
    IF (LLMSE) THEN
      DO JSW=1,NSW
        ...
        IF (LHLRADUPD) THEN
          DO JLON=KIDIA,KFDIA
            ZSALBCOR=0.2_JPRB/(1._JPRB+YDMF_PHYS%TMP%RDG%MU0(JLON))-0.12_JPRB
...
!              ZALBP(JLON,JSW)=RALB_FORC+ZSALBCOR
    ...
    ELSE
     !pour pouvoir tourner sans la surface
      DO JSW=1,NSW
        ZALBP(KIDIA:KFDIA,JSW)=YDMF_PHYS_SURF%GSD_VF%PALBF(KIDIA:KFDIA)
        ZALBD(KIDIA:KFDIA,JSW)=YDMF_PHYS_SURF%GSD_VF%PALBF(KIDIA:KFDIA)
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  IF (NOZOCL == 1) THEN
    ! as in IFS
    CALL RADOZC(KIDIA,KFDIA,KLON,KLEV,1,KLON,0,YDMF_PHYS_STATE%YCPG_PHY%PRE,PGEMU,ZROZ)
    DO JK=1,KLEV
      DO JLON=KIDIA,KFDIA
  ...
  ELSEIF (NOZOCL == 2) THEN
    ...
    CALL SUOZON(KIDIA,KFDIA,KLON,KLEV,ZQO3,.FALSE.,YDMF_PHYS_STATE%YCPG_PHY%PRE,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(KSTEP,NRADFR) == 0) THEN 
     ...
     & YDMF_PHYS%TMP%RDG%MU0M   , ZQV   , ZQSAT    , ZQICE    , ZQLIQ   , &
     ...
     & PGELAM , PGEMU ,PGPAR , &
     & YDMF_PHYS%TMP%RDG%MU0LU , ZALBD1 , ZFRSOLU)
   ELSE
     IF (LLMSE) THEN 
       DO JSW=1,NSW 
   ...
   & YDMF_PHYS_STATE%YCPG_PHY%PRE  , ZEMIS  , YDMF_PHYS%RAD%EMTD  , YDMF_PHYS%TMP%RDG%MU0, ZQVM,&
  ...
  DO JLON = KIDIA, KFDIA
    ...
    IF (YDMF_PHYS%TMP%RDG%MU0(JLON) > 3.0E-02_JPRB) THEN
      YDMF_PHYS%OUT%FRSDNI(JLON)=MAX(0.0_JPRB,YDMF_PHYS%OUT%FRSOPS(JLON)/YDMF_PHYS%TMP%RDG%MU0(JLON))
    ELSE
    ...
    IF (LLMSE) THEN
      DO JSW=1, NSW
        WRITE(NULOUT,*)'ZSFSWDIR ZSFSWDIF ZFSDNN ZFSDNV PFRSO',&
         & ZZS_FSWDIR(NPTP,JSW),ZZS_FSWDIF(NPTP,JSW),ZFSDNN(NPTP), ZFSDNV(NPTP),YDMF_PHYS%OUT%FRSO(NPTP,KLEV,1)
...
ELSE
  ...
  YDMF_PHYS%OUT%FRSOC(KIDIA:KFDIA,0:1)=0.0_JPRB
  YDMF_PHYS%OUT%FRTHC(KIDIA:KFDIA,0:1)=0.0_JPRB
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & ZALBD1,ZALBP1,ZEMIS,PGELAM,PGEMU,YDMF_PHYS%TMP%RDG%MU0,YDMF_PHYS%TMP%RDG%MU0LU,ZTSURF, &
   & ZDECRD,ZCLCT_RAD,&
  ...
  IF (LLMSE) THEN
    IF (LHLRADUPD) THEN
      ...
      ENDDO
    ELSE
      ZZS_FSWDIR(KIDIA:KFDIA,1)=YDMF_PHYS%OUT%FRSOPS(KIDIA:KFDIA) ! direct surface swdn flux
      ZZS_FSWDIF(KIDIA:KFDIA,1)=ZFRSODS(KIDIA:KFDIA) ! diffuse surface swdn flux
    ENDIF
  ENDIF

  ! Estimate of the direct normal irradiance, with securities
  YDMF_PHYS%OUT%FRSDNI(KIDIA:KFDIA)=YDMF_PHYS%OUT%FRSOPS(KIDIA:KFDIA)
  DO JLON = KIDIA, KFDIA
    IF (YDMF_PHYS%TMP%RDG%MU0(JLON) > 3.0E-02_JPRB) THEN
...
IF (.NOT.(LRAY.AND.NRAY == 2.AND.LRADFLEX).AND..NOT.LRAYFM) THEN
  ...
  ENDDO
  YDMF_PHYS%OUT%FRSOPS(KIDIA:KFDIA)=0._JPRB
ENDIF
...
IF(LKFBCONV) THEN
  ...
  IF (LOSUBG_COND.AND..NOT.LOSIGMAS) THEN
    YDVARS%SRC%T1(KIDIA:KFDIA,1:KLEV)=ZMFS_(KIDIA:KFDIA,1:KLEV)
  ENDIF
  IF(MOD(KSTEP+1,NPRINTFR)==0) THEN
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( KBL,KGPCOMP,&
       ...
       & ZPABSM__(:,IKB),YDMF_PHYS_STATE%YCPG_PHY%PRE(KIDIA:KFDIA,KLEV),&
       ...
       & ZINPRG_NOTINCR_,&
       & YDMF_PHYS%OUT%FRTHDS(KIDIA:KFDIA),ZZS_FSWDIF(KIDIA:KFDIA,1:NSW),&
       ...
       & ZEMIS(KIDIA:KFDIA),ZTSURF(KIDIA:KFDIA),YDMF_PHYS%OUT%FRTH(KIDIA:KFDIA,KLEV,1))
...
!ZSFRV_(KIDIA:KFDIA) = 0._JPRB
    ...
    IF (LLMSE_DIAG) THEN
      ...
      CALL ARO_GROUND_DIAG( KBL, KGPCOMP,&
       ...
       & YDMF_PHYS%OUT%TCLS(KIDIA:KFDIA),YDMF_PHYS%OUT%QCLS(KIDIA:KFDIA),YDMF_PHYS%OUT%RHCLS(KIDIA:KFDIA),&
      ...
      CALL ARO_GROUND_DIAG_2ISBA( KBL, KGPCOMP, &
       ...
       & NDGUNG, NDGUXG, NDLUNG, NDLUXG, LSURFEX_KFROM, &
       & PINDX(KIDIA:KFDIA), PINDY(KIDIA:KFDIA), &
       & YDMF_PHYS_SURF%GSD_VF%PLSM, ZDUMMY1, ZDUMMY1, ZDUMMY1, ZTSURF(KIDIA:KFDIA), YDMF_PHYS_SURF%GSP_SG%PF_T1, &
       & ZDUMMY1(KIDIA:KFDIA), ZDUMMY1(KIDIA:KFDIA), ZDUMMY1(KIDIA:KFDIA), &
       & ZDUMMY1(KIDIA:KFDIA), ZDUMMY1(KIDIA:KFDIA), ZDUMMY1(KIDIA:KFDIA), &
       & ZDUMMY1(KIDIA:KFDIA), YDMF_PHYS_SURF%GSP_SG%PR_T1, ZDUMMY1 )
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREF,&
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    DO JLON = KIDIA, KFDIA
      ZZS_FTH_(JLON)=-1._JPRB*ZSFTH_(JLON)*(YDMF_PHYS_STATE%YCPG_PHY%PRE(JLON,KLEV)*ZINVATM)**(ZRSCP)
...
!!$  ENDIF
  ...
  !   
    DO JLEV = 1,KLEV
      DO JGR=1,NGRADIENTS
        ZTURB3D__(KIDIA:KFDIA,JLEV,JGR)=YDMF_PHYS%GRA%G(KIDIA:KFDIA,JGR,JLEV)
      ENDDO
    ENDDO
...
!!$  ENDIF
  ...
  IF (LOSUBG_COND .AND. LOSIGMAS) THEN
    IF (CMF_CLOUD=='DIRE'.OR.CMF_CLOUD=='BIGA'.OR.CMF_CLOUD=='NONE') THEN
      DO JLEV = 1,KLEV
        YDVARS%SRC%T1(KIDIA:KFDIA,JLEV)=ZSIGS__(KIDIA:KFDIA,JLEV)
      ENDDO
...
IF (LMICRO) THEN
  ...
  IF (CMICRO == 'LIMA') THEN
    ...
    CALL ARO_LIMA(KLEV,IKU,IKL,KFDIA,KLEV,NRR,NLIMA,KSTEP+1,NSPLITR,NSPLITG, ZDT,ZDZZ_ ,&
     ...
     & YDDDH, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH )
  ELSE
    CALL ARO_RAIN_ICE (KLEV,IKU,IKL,KFDIA,KLEV,NRR,KSTEP+1,NSPLITR,NGFL_EZDIAG,&
...
IF (LUSECHEM) THEN
  ...
  DO JLON = KIDIA,KFDIA
    ...
    ZZENITH_(JLON) = ACOS( YDMF_PHYS%TMP%RDG%MU0(JLON) )
...
IF (LMUSCLFA) CALL ECR1D(NMUSCLFA,'PCLCT_apl',YDCPG_MISC%CLCT,1,KLON)
...
ENDDO
!Hail diagnostic
YDMF_PHYS%OUT%DIAGH(KIDIA:KFDIA)=0._JPRB
IF (LXXDIAGH) THEN
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
...
! lightening density
IF (LFLASH) THEN
  IF (KSTEP==0) YDMF_PHYS%OUT%FLASH=0._JPRB
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,YDMF_PHYS_STATE%YCPG_PHY%PRE(:,KLEV),YDMF_PHYS%OUT%TCLS,&
  & YDMF_PHYS%OUT%QCLS,ZQCM(:,KLEV),ZQIM(:,KLEV),YDMF_PHYS%OUT%TPWCLS)
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,KLEV
      CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,YDMF_PHYS_STATE%YCPG_PHY%PREF(:,JLEV),ZTM(:,JLEV),&
  ...
  IF (LDPRECIPS) THEN
   ...
   YDMF_PHYS%TMP%PRC%DPRECIPS(:,NDTPRECCUR)=0._JPRB

   !WRITE(NULOUT,*)'sous apl_arome NDTPRECCUR=',NDTPRECCUR,NDTPREC
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,KLEV),YDMF_PHYS%OUT%FPLSN(:,KLEV),YDMF_PHYS%OUT%FPLSG(:,KLEV),YDMF_PHYS%TMP%PRC%DPRECIPS(:,NDTPRECCUR))
  ENDIF
  ...
  IF (LDPRECIPS2) THEN
   ...
   YDMF_PHYS%TMP%PRC%DPRECIPS2(:,NDTPRECCUR2)=0._JPRB

   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,KLEV),YDMF_PHYS%OUT%FPLSN(:,KLEV),YDMF_PHYS%OUT%FPLSG(:,KLEV),YDMF_PHYS%TMP%PRC%DPRECIPS2(:,NDTPRECCUR2))
