...
& YDMODEL, LDCONFX, PDTPHY, PGFL, PGP2DSDT, PB1, PB2, PGMVT1, PGFLT1, PTRAJ_PHYS, YDDDH)
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  IF (NOZOCL == 1) THEN
    ...
    & 0, YLMF_PHYS_STATE%YCPG_PHY%PRE, YDVARS%GEOMETRY%GEMU%T0, ZROZ)
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & YDCPG_DIM%KSW, ZALBD, ZALBP, YLMF_PHYS_STATE%YCPG_PHY%PRE, YLMF_PHYS_STATE%YCPG_PHY%PREF, YDVARS%A%T1,     &
     & ZQO3, ZAER, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZEMIS, ZRDG_MU0M, ZQV, ZQSAT, ZQICE, ZQLIQ,                 &
   ...
   & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PRE, ZEMIS, YDMF_PHYS%RAD%EMTD, ZRDG_MU0, ZQVM,                   &
...
!+++ End of redundant input preparations for ACRANEB
  ...
  & ZNEB0, ZQV, ZQCO2, ZQICE, ZQLIQ, ZQO3, YLMF_PHYS_STATE%T, ZALBD1, ZALBP1, ZEMIS, YDVARS%GEOMETRY%GELAM%T0,                          &
...
IF ( LINTFLEX ) THEN
  ...
  & YDVARS%GEOMETRY%GNORDL%T0, YDVARS%GEOMETRY%GNORDM%T0, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,             &
