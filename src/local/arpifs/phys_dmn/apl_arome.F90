...
IF (LNUDGLH.AND.YDCPG_DIM%KSTEP > NSTARTNUDGLH.AND.YDCPG_DIM%KSTEP <= NSTOPNUDGLH) THEN
  ...
  IF(MOD(ISTEP,NINTNUDGLH)==0) THEN
    ...
    & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG), ZTEND_Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),                            &
    & NRR, LNUDGLHNREAD)
      ...
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YLMF_PHYS_BASE_STATE%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            ZTEND_Q(JLON,JLEV)=ZTEND_Q(JLON,JLEV)+RLVTT/RV/((YLMF_PHYS_BASE_STATE%T(JLON,JLEV))**2._JPRB)* &
...
ELSE
  ...
  CALL CPUTQY_AROME(YDMODEL, YDGEOMETRY%YRDIMV, YDGMV, YDCPG_DIM, &
  & PDTPHY, IPGFL, IPTR, ZTEND_Q, ZTEND_L, ZTEND_R, ZTEND_I, ZTEND_S, ZTEND_G, ZTEND_H,              &
  & ZTENDT, ZTENDGFLR(:, :, 1:), YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDD, YDCPG_SL1%ZVIEW, PGMVT1, PGFLT1)
ENDIF
