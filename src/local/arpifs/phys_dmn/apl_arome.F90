...
SUBROUTINE APL_AROME(YDGEOMETRY,YDAPLPAR,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 & KTDIA  , KLEV , KSTEP ,&
...
USE APLPAR_STATE_TYPE_MOD &
               , ONLY : APLPAR_STATE_TYPE
!     --------------------------------------------------------------------------

IMPLICIT NONE

TYPE(GEOMETRY)    ,INTENT(IN)    :: YDGEOMETRY
TYPE (APLPAR_STATE_TYPE),INTENT(INOUT):: YDAPLPAR
...
DO JLEV = 1,KLEV
  DO JLON = KIDIA,KFDIA
    ZZZ_F_(JLON,JLEV)=YDAPLPAR%YCPG_DYN%PHIF(JLON,JLEV)*ZINVG
...
IF (LRSLDDH) THEN
  DO JLEV=1, KLEV
    DO JLON = KIDIA, KFDIA
      ZTKEM4SLDDH(JLON,JLEV)=MAX(YDAPLPAR%TKE(JLON,JLEV),PPTKEMIN)
    ENDDO
  ENDDO
  ZTKEM => ZTKEM4SLDDH(:,:)
ELSE
...
IF (LMICRO.OR.LTURB.OR.LLMSE.OR.LKFBCONV) THEN
  ...
  DO JLEV = 1,KLEV
   ...
   !et convertir q en r 
    IF (NRR==7) THEN
      DO JLON = KIDIA,KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YDAPLPAR%Q(JLON,JLEV)-YDAPLPAR%L(JLON,JLEV)-YDAPLPAR%R(JLON,JLEV)&
         & -YDAPLPAR%I(JLON,JLEV)-YDAPLPAR%S(JLON,JLEV)-YDAPLPAR%G(JLON,JLEV)-YDAPLPAR%H(JLON,JLEV)   
      ENDDO
    ELSE
      DO JLON = KIDIA,KFDIA
        ZQDM(JLON,JLEV)=1._JPRB-YDAPLPAR%Q(JLON,JLEV)-YDAPLPAR%L(JLON,JLEV)-YDAPLPAR%R(JLON,JLEV)&
         & -YDAPLPAR%I(JLON,JLEV)-YDAPLPAR%S(JLON,JLEV)-YDAPLPAR%G(JLON,JLEV)
      ENDDO
    ENDIF
    DO JLON = KIDIA,KFDIA
   !initialisation de ZRHODREFM__ (=qd*zrho) 
      ZRHO=YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV)/(YDAPLPAR%YCPG_DYN%RCP%R(JLON,JLEV)*YDAPLPAR%T(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
      ZRHODJM__(JLON,JLEV)=YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
  !initialisation de ZEXNREFM_
      ZEXNREFM_(JLON,JLEV)=(YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV)*ZINVATM)**(ZRSCP)
 ! vent horizontal et TKE
      ZPABSM__(JLON,JLEV)=YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV)
      ZUM__(JLON,JLEV)= YDAPLPAR%U(JLON,JLEV)
      ZVM__(JLON,JLEV)= YDAPLPAR%V(JLON,JLEV)
      ZWM__(JLON,JLEV)= YDAPLPAR%YCPG_PHY%W(JLON,JLEV)
      ZTKEM__(JLON,JLEV)= ZTKEM(JLON,JLEV)
      ZZZ_(JLON,JLEV)=YDAPLPAR%YCPG_DYN%PHI(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
  ...
  IF (LOSIGMAS) THEN
    DO JLEV = 1, KLEV 
      ZSIGM_(KIDIA:KFDIA,JLEV)= YDAPLPAR%SRC(KIDIA:KFDIA,JLEV)
    ENDDO
  ENDIF
  ...
  IF ( LKFBCONV.AND.LOSUBG_COND.AND..NOT.LOSIGMAS) THEN
    DO JLEV = 1, KLEV 
      ZMFM_(KIDIA:KFDIA,JLEV)=YDAPLPAR%SRC(KIDIA:KFDIA,JLEV) 
    ENDDO
  ENDIF
!!! initialisation des variables d etat MNH �t
  ...
  DO JLEV = 1, KLEV
    DO JLON = KIDIA,KFDIA
      ZTHM__(JLON,JLEV)=YDAPLPAR%T(JLON,JLEV)/ZEXNREFM_(JLON,JLEV)
      ZRM_(JLON,JLEV,1)=YDAPLPAR%Q(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,2)=YDAPLPAR%L(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,3)=YDAPLPAR%R(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,4)=YDAPLPAR%I(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,5)=YDAPLPAR%S(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ZRM_(JLON,JLEV,6)=YDAPLPAR%G(JLON,JLEV)/ZQDM(JLON,JLEV)  
    ENDDO
  ENDDO
  ...
  IF (NRR==7) THEN
    DO JLEV = 1, KLEV
      DO JLON = KIDIA,KFDIA
        ZRM_(JLON,JLEV,7)=YDAPLPAR%H(JLON,JLEV)/ZQDM(JLON,JLEV)  
      ENDDO
    ENDDO
  ENDIF
...
!!! suivant les parametrisations)   
  ...
  DO JLEV = 1, KLEV 
    DO JLON=KIDIA,KFDIA
      ZUS__(JLON,JLEV)= YDAPLPAR%U(JLON,JLEV)*ZINVDT
      ZVS__(JLON,JLEV)= YDAPLPAR%V(JLON,JLEV)*ZINVDT
      ZWS__(JLON,JLEV)= YDAPLPAR%YCPG_PHY%W(JLON,JLEV)*ZINVDT
...
!!! Initialisations temporaires d'arguments non-utilises
  ...
  DO JGFL=1,NGFL_EXT
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA
        ZSVM_(JLON,JLEV,JGFL)=YDAPLPAR%P1EXT(JLON,JLEV,JGFL)
        ZSVS_(JLON,JLEV,JGFL)=YDAPLPAR%P1EXT(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
  ...
  DO JGFL=1,NLIMA
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA
        ZLIMAM_(JLON,JLEV,JGFL)=YDAPLPAR%P1LIMA(JLON,JLEV,JGFL)
        ZLIMAS_(JLON,JLEV,JGFL)=YDAPLPAR%P1LIMA(JLON,JLEV,JGFL)*ZINVDT
      ENDDO
    ENDDO
  ENDDO
...
IF (LDIAGWMAX) THEN
  ...
  ! calcul de wmax
    DO JLEV = 1 , KLEV
      ...
      Z_WMIN=0._JPRB
      DO JLON=KIDIA,KFDIA
        IF (YDAPLPAR%YCPG_PHY%W(JLON,JLEV)>Z_WMAX) THEN
          Z_WMAX=YDAPLPAR%YCPG_PHY%W(JLON,JLEV)
        ENDIF
        IF (YDAPLPAR%YCPG_PHY%W(JLON,JLEV)<Z_WMIN) THEN
          Z_WMIN=YDAPLPAR%YCPG_PHY%W(JLON,JLEV)
        ENDIF
      ENDDO
    ENDDO
  ENDIF
...
IF (LMICRO) THEN
  ...
  !reinitialisation de ZRHODREFM__ (=qd*zrho)
      ZRHO=YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV)/(ZRHM(JLON,JLEV)*ZTM(JLON,JLEV))
      ZRHODREFM__(JLON,JLEV)=ZRHO*ZQDM(JLON,JLEV)
    ENDDO
  ENDDO
  ...
  ZAPHIM(KIDIA:KFDIA,KLEV)=YDAPLPAR%YCPG_DYN%PHI(KIDIA:KFDIA,KLEV)
  CALL GPGEO(KLON,KIDIA,KFDIA,KLEV,ZAPHIM,ZAPHIFM,ZTM,ZRHM,YDAPLPAR%YCPG_PHY%XYB%LNPR,YDAPLPAR%YCPG_PHY%XYB%ALPH,YDGEOMETRY%YRVERT_GEOM)
...
ELSE
  ...
  ZTM (KIDIA:KFDIA,1:KLEV)=YDAPLPAR%T(KIDIA:KFDIA,1:KLEV)
  ZRHM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_DYN%RCP%R(KIDIA:KFDIA,1:KLEV)
  ZQVM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%Q(KIDIA:KFDIA,1:KLEV)
  ZQIM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%I(KIDIA:KFDIA,1:KLEV)
  ZQCM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%L(KIDIA:KFDIA,1:KLEV)
  ZQRM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%R(KIDIA:KFDIA,1:KLEV)
  ZQSM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%S(KIDIA:KFDIA,1:KLEV)
  ZQGM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%G(KIDIA:KFDIA,1:KLEV)
  IF (NRR==7) THEN
    ZQHM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%H(KIDIA:KFDIA,1:KLEV)
  ELSE
    ZQHM(KIDIA:KFDIA,1:KLEV)=0._JPRB
  ENDIF
  ZCPM(KIDIA:KFDIA,KTDIA:KLEV)=YDAPLPAR%YCPG_DYN%RCP%CP(KIDIA:KFDIA,KTDIA:KLEV)
  ZAPHIM(KIDIA:KFDIA,0:KLEV)=YDAPLPAR%YCPG_DYN%PHI(KIDIA:KFDIA,0:KLEV)
  ZAPHIFM(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_DYN%PHIF(KIDIA:KFDIA,1:KLEV)
  ZZZ_(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_DYN%PHI(KIDIA:KFDIA,1:KLEV)*ZINVG
...
IF (LFLEXDIA) THEN
  DO JLEV = 1, KLEV
    DO JLON=KIDIA,KFDIA 
      ZTMPAF(JLON,JLEV)=(PTENDT(JLON,JLEV)-ZTENDTBAK(JLON,JLEV))*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG*ZCPM(JLON,JLEV)
    ENDDO
  ENDDO
  IF (LDDH_OMP) THEN
  ...
  DO JR=1,NRR
    ...
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA 
        ZTMPAF(JLON,JLEV)=(PTENDR(JLON,JLEV,JR)-ZTENDRBAK(JLON,JLEV,JR))*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA 
        ZTMPAF(JLON,JLEV)=YDVARS%A%T1(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
...
! specific to new data flow for diagnostics
  IF (LDDH_OMP) THEN
    ...
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA 
        ZCON3(JLON,JLEV) = YDAPLPAR%YCPG_DYN%RCP%CP(JLON,JLEV)*ZEXNREFM_(JLON,JLEV)
      ENDDO
    ENDDO
  ...
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(KLON,KLEV,KSTEP,KFDIA,ZQDM,ZEXNREFM_,YDAPLPAR%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  ZTSURF(KIDIA:KFDIA) = YDAPLPAR%T(KIDIA:KFDIA,KLEV)
  ZTN(KIDIA:KFDIA)    = YDAPLPAR%T(KIDIA:KFDIA,KLEV)
  ZQS(KIDIA:KFDIA)    = YDAPLPAR%Q(KIDIA:KFDIA,KLEV)
  DO JLON=KIDIA,KFDIA
    ZRHODREFM(JLON) = YDAPLPAR%YCPG_PHY%PREF(JLON,KLEV)/(YDAPLPAR%T(JLON,KLEV)*YDAPLPAR%YCPG_DYN%RCP%R(JLON,KLEV))
    ZTHETAS(JLON)   = ZTSURF(JLON)*(RATM/YDAPLPAR%YCPG_PHY%PRE(JLON,KLEV))**RKAPPA
  ENDDO

  LLAROME=.TRUE.
  CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS, LLAROME, KIDIA , KFDIA  , KLON, YDAPLPAR%YCPG_DYN%PHIF(:,KLEV), &
   & ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,ZTN,ZTSURF,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR%Q(:,KLEV), YDAPLPAR%U(:,KLEV), YDAPLPAR%V(:,KLEV), ZTHETAS, &
...
!* Compute PBL-diagnostics
  ...
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,KIDIA,KFDIA,KLON,KLEV,YDMF_PHYS%OUT%UCLS,&
   & YDMF_PHYS%OUT%VCLS,YDAPLPAR%U(:,1:KLEV),YDAPLPAR%V(:,1:KLEV),ZCAPE,ZDCAPE,ZTKEM(:,1:KLEV),YDAPLPAR%YCPG_DYN%PHIF(:,1:KLEV),POROG,&
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  CALL ACTQSAT ( YDPHY,KIDIA,KFDIA,KLON,NTQSAT,KLEV, YDAPLPAR%YCPG_PHY%PREF, ZCPM, ZQVM, ZTM,&
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
    ...
    IF (NAER >= 1 ) THEN
      IF(YSD_VAD%NUMFLDS >= 4) THEN
        CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDPHY, KIDIA , KFDIA , KLON  , KLEV,&
         & YDAPLPAR%YCPG_PHY%PRE,YDAPLPAR%YCPG_PHY%PREF,ZTM,ZTSURF,YDMF_PHYS_SURF%GSD_VA%PSEA,YDMF_PHYS_SURF%GSD_VA%PLAN,YDMF_PHYS_SURF%GSD_VA%PSOO,YDMF_PHYS_SURF%GSD_VA%PDES,YDMF_PHYS_SURF%GSD_VA%PSUL,YDMF_PHYS_SURF%GSD_VA%PVOL,ZAER,ZAERINDS)
      ELSE
  ...
  IF (NOZOCL == 1) THEN
    ! as in IFS
    CALL RADOZC(KIDIA,KFDIA,KLON,KLEV,1,KLON,0,YDAPLPAR%YCPG_PHY%PRE,PGEMU,ZROZ)
    DO JK=1,KLEV
      DO JLON=KIDIA,KFDIA
        ZQO3(JLON,JK)=ZROZ(JLON,JK)/YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JK)
      ENDDO
    ENDDO
  ELSEIF (NOZOCL == 2) THEN
    ! as in ARPEGE (from clim profiles)
    CALL SUOZON(KIDIA,KFDIA,KLON,KLEV,ZQO3,.FALSE.,YDAPLPAR%YCPG_PHY%PRE,YDAPLPAR%YCPG_PHY%XYB%RDELP,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(KSTEP,NRADFR) == 0) THEN 
     ...
     & ZALBD   , ZALBP , YDAPLPAR%YCPG_PHY%PRE   , YDAPLPAR%YCPG_PHY%PREF  ,           &
     & YDVARS%A%T1   , ZQO3  , ZAER     , YDAPLPAR%YCPG_PHY%XYB%DELP   , ZEMIS   , &
   ...
   & YDAPLPAR%YCPG_PHY%PRE  , ZEMIS  , YDMF_PHYS%RAD%EMTD  , PMU0, ZQVM,&
...
CALL ACNPART(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
 & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREF,ZDECRD,ZNEB0,&
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & YDAPLPAR%YCPG_PHY%PRE,YDAPLPAR%YCPG_PHY%PREF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%DELP,ZNEB0,&
   & ZQV,ZQCO2,ZQICE,ZQLIQ,ZQO3,YDAPLPAR%T,&
...
IF(LKFBCONV) THEN
  ...
  DO JLEV = 1,KLEV
    DO JLON = KIDIA, KFDIA
      ...
      ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZCVTENDT_(JLON,JLEV)*(RATM/YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)  
    ENDDO
  ENDDO
  DO JLON =KIDIA, KFDIA
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( KBL,KGPCOMP,&
       ...
       & ZPABSM__(:,IKB),YDAPLPAR%YCPG_PHY%PRE(KIDIA:KFDIA,KLEV),&
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREF,&
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    DO JLON = KIDIA, KFDIA
      ZZS_FTH_(JLON)=-1._JPRB*ZSFTH_(JLON)*(YDAPLPAR%YCPG_PHY%PRE(JLON,KLEV)*ZINVATM)**(ZRSCP)
    ...
    CALL VDFHGHTHL(YDMODEL%YRML_PHY_G%YRVDF,YDMODEL%YRML_PHY_SLIN%YREPHLI, &
     ...
     & PDT, YDAPLPAR%U, YDAPLPAR%V,&
     & ZTM,ZQVM,ZQCM,ZQIM,YDVARS%A%T1,&
     & YDAPLPAR%YCPG_PHY%PRE, YDAPLPAR%YCPG_PHY%PREF, ZAPHIFM,ZAPHIM,&
      ...
      DO JLEV= 1, KLEV
        DO JLON = KIDIA, KFDIA
          ...
          ZTHS__(JLON,JLEV)=ZTHS__(JLON,JLEV)+ZTENDTUP(JLON,JLEV)*(RATM/&
           & YDAPLPAR%YCPG_PHY%PREF(JLON,JLEV))**(RD/RCPD)
...
!!$  ENDIF
  ...
  IF (LFLEXDIA) THEN
    DO JLEV = 1,KLEV
      DO JLON = KIDIA,KFDIA
        ZDP__(JLON,JLEV)=ZDP__(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTP__(JLON,JLEV)=(ZTP__(JLON,JLEV)-ZTPMF__(JLON,JLEV))*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTPMF__(JLON,JLEV)=ZTPMF__(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDIFF__(JLON,JLEV)=ZTDIFF__(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
        ZTDISS__(JLON,JLEV)=ZTDISS__(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
...
IF (LMICRO) THEN
  ...
  IF (MOD(KSTEP+1,NPRINTFR)==0) THEN
    ...
    DO JLEV=2,KLEV
      WRITE(NULOUT,'(I2,X,4f10.3)')JLEV, YDAPLPAR%YCPG_PHY%XYB%DELP(NPTP,JLEV),&
    ...
    DO JLEV = 1,KLEV
      DO JLON = KIDIA,KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YDAPLPAR%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            PTENDR(JLON,JLEV,1)=PTENDR(JLON,JLEV,1)+RLVTT/RV/((YDAPLPAR%T(JLON,JLEV))**2._JPRB)* &
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP,YGFL, &
   ...
   & PTSL=YDAPLPAR%T,PQSL=YDAPLPAR%Q, PA=YDVARS%A%T1, PAP=YDAPLPAR%YCPG_PHY%PREF, PAPH=YDAPLPAR%YCPG_PHY%PRE,   &  ! In: (T,Q,cloud) forsupersatcheck, Pfull, Phalf
...
IF (LXXDIAGH) THEN
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%OUT%DIAGH(JLON)=YDMF_PHYS%OUT%DIAGH(JLON)+ZQGM(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
ENDIF
! lightening density
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,KIDIA,KFDIA,KLON,KLEV,KSTEP,&
    ...
    &YDAPLPAR%YCPG_PHY%XYB%DELP,ZTM,YDAPLPAR%YCPG_PHY%W,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS%OUT%FLASH)
ENDIF
...
IF (LINTFLEX) THEN
  ...
  CALL APL_AROME2INTFLEX(YGFL,YDPARAR,YDPHY,KLON,KIDIA,KFDIA,KLEV, PDT,&
   & YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%T, PGPAR(1,MVTS), YDAPLPAR%YCPG_DYN%RCP%CP,&
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,YDAPLPAR%YCPG_PHY%PRE(:,KLEV),YDMF_PHYS%OUT%TCLS,&
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,KLEV
      CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,YDAPLPAR%YCPG_PHY%PREF(:,JLEV),ZTM(:,JLEV),&
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDAPLPAR%YCPG_DYN%PHIF,&
