...
& YDCPG_SL1, YDVARS, YDGMV, YDSURF, YDCFU, YDXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, PGP2DSDT, PGMVT1,  &
...
IF (LMICRO) THEN
  ...
  ELSE
    ...
    & NGFL_EZDIAG, CFRAC_ICE_ADJUST, LOSUBG_COND, LOSIGMAS, CMICRO, LOCND2, ZDT, VSIGQSAT, ZZZ_F_, ZRHODJM__(:, 1:YDCPG_OPTS%KFLEVG), &
    & ZEXNREFM_, ZPABSM__(:, 1:YDCPG_OPTS%KFLEVG), ZTHM__(:, 1:YDCPG_OPTS%KFLEVG), ZRM_, ZSIGM_,                                      &
...
! specific to new data flow for diagnostics
  ...
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%KSTEP, YDCPG_BNDS%KFDIA, &
    & ZQDM, ZEXNREFM_, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
IF (LRDUST) THEN
  ...
  CALL SWAP_SVM
  ! input dust scalar concentration in ppp from
  CALL ARO_MNHDUST (IKL, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, NGFL_EXT, PDTPHY, ZSVMIN_, ZZZ_, ZDZZ_,                                       &
...
!* Compute PBL-diagnostics
   ...
   ZCAPE(:)=0._JPRB
   ZDCAPE(:)=0._JPRB
  CALL ACCLDIA(YDXFU, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDTOPH, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,                                                          &
  & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_BASE_STATE%U(:, 1:YDCPG_OPTS%KFLEVG),                               &
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  ELSEIF (NOZOCL == 2) THEN
    ...
    & .FALSE., YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, LO3ABC, YDMF_PHYS_SURF%GSD_VC%PGROUP&
    & )
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_OPTS%KSTEP,NRADFR) == 0) THEN 
     ...
     & YDCPG_OPTS%KSW, ZALBD, ZALBP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF,           &
     & YDVARS%A%T1, ZQO3, ZAER, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZEMIS, ZRDG_MU0M, ZQV, ZQSAT,                 &
     ...
     & YDCPG_GPAR%SWDIR, YDCPG_GPAR%SWDIF, ZRDG_MU0LU, ZALBD1, ZFRSOLU, ZGP2DSPP, ZP1EZDIAG)
   ELSE
     IF (LLMSE) THEN 
       DO JSW=1,NSW 
...
& ZDECRD, ZNEB0, YDMF_PHYS%OUT%CLCH, YDMF_PHYS%OUT%CLCM, YDMF_PHYS%OUT%CLCL, YDCPG_MISC%CLCT,                     &
& ZCLCT_RAD)
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    & YDCPG_OPTS%KFLEVG, INDRAFT, PDTPHY, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, ZTM, ZQVM,              &
    & ZQCM, ZQIM, YDVARS%A%T1, YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF,             &
    & ZAPHIFM, ZAPHIM, ZZS_FTH_, ZZS_FRV_, ZZS_FU_, ZZS_FV_, ZMF_UP, ZTHETAL_UP, ZQT_UP, ZTHTV_UP,                &
    & ZQC_UP, ZQI_UP, ZU_UP, ZV_UP, ZGP2DSPP, NGFL_EZDIAG, ZP1EZDIAG, ZTENDQVUP, ZTENDTUP, ZSURFPREP,             &
...
IF (LUSECHEM) THEN
  ...
  ENDDO
  IEZDIAG_CHEM=NGFL_EZDIAG-IOFF_MFSHAL+1
  CALL ARO_MNHC(ZSVSIN_, ZRHODREFM__(:, 1:YDCPG_OPTS%KFLEVG), PDTPHY, ZTHM__(:, 1:YDCPG_OPTS%KFLEVG), &
  & ZPABSM__(:, 1:YDCPG_OPTS%KFLEVG), ZRM_, ZLAT_, ZLON_, ZALB_UV_, ZZS_, ZZENITH_, ZZZ_, IYEAR,      &
  & IMONTH, IDAY, REAL(RHGMT, JPRB)+PDTPHY/2._JPRB, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, NGFL_EXT,    &
...
IF(LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP, YGFL, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,            &
  & YDCPG_OPTS%KFLEVG, 1, PDTPHY, PTSL=YDMF_PHYS_BASE_STATE%T, PQSL=YDMF_PHYS_BASE_STATE%Q, PA=YDVARS%A%T1,       &
  & PAP=YDMF_PHYS_BASE_STATE%YCPG_PHY%PREF, PAPH=YDMF_PHYS_BASE_STATE%YCPG_PHY%PRE, PDYN_U=ZDUMMY, PDYN_V=ZDUMMY, &
  & PDYN_T=ZDUMMY, PDYN_Q=ZDUMMY, PUNP_U=YDMF_PHYS%OUT%TENDU, PUNP_V=YDMF_PHYS%OUT%TENDV, PUNP_T=ZTENDT,          &
  & PUNP_Q=ZTEND_Q, PMULNOISE=PGP2DSDT(1, 1, 1), PTENU=YDMF_PHYS%OUT%TENDU, PTENV=YDMF_PHYS%OUT%TENDV,            &
  & PTENT=ZTENDT, PTENQ=ZTEND_Q )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF (LINTFLEX) THEN
  ...
  & PDTPHY, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V,                   &
...
ELSE
  ...
  & YDCPG_OPTS, PDTPHY, ZTEND_Q, ZTEND_L, ZTEND_R, ZTEND_I, ZTEND_S, ZTEND_G, ZTEND_H, ZTENDTKE, &
  & ZTENDT, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDD)
  CALL CPUTQY_AROME_LOOP (YDMODEL, YDGEOMETRY%YRDIMV, YDGMV, YDCPG_BNDS, YDCPG_OPTS, PDTPHY, IPGFL, &
