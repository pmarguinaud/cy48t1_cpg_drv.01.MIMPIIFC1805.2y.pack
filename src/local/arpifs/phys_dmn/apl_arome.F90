...
IF (LLDIAB) THEN
  ...
  IF (LL_SAVE_PHSURF) THEN
    CALL MF_PHYS_SAVE_PHSURF_PART1 (YDCPG_DIM, YDMF_PHYS_TMP, YDMF_PHYS_SURF, YDVARS, YDSURF, &
    & YDMODEL)
  ENDIF
ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZALBD, ZALBP, YLMF_PHYS_STATE%YCPG_PHY%PRE, YLMF_PHYS_STATE%YCPG_PHY%PREF, YDVARS%A%T1, ZQO3,              &
     ...
     & ZTRSODIF, ZPIZA_DST_, ZCGA_DST_, ZTAUREL_DST_, ZAERINDS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, PGPAR,             &
...
IF (LINTFLEX) THEN
  ...
  & PGPAR(1, MVTS), YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZFPR, LLRAD, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO,          &
...
IF (LLDIAB) THEN
  IF (LL_SAVE_PHSURF) THEN
    CALL MF_PHYS_SAVE_PHSURF_PART2 (YDCPG_DIM, YDMF_PHYS_TMP, YDMF_PHYS_SURF, YDVARS, YDSURF, &
    & YDMODEL)
  ENDIF
ENDIF
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDGSGEOM%       GECLO, YDGSGEOM%GESLO, YDMF_PHYS_TMP  )
ENDIF
