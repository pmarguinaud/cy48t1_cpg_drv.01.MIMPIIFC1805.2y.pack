...
SUBROUTINE APL_AROME(YDGEOMETRY,YDCPG_DIM,YLMF_PHYS_STATE,YDCPG_MISC,YDMF_PHYS,&
 & YDMF_PHYS_TMP,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, &
 & IMAXDRAFT, NTSSG, PDTPHY, LLXFUMSE, &
 & PGP2DSDT, ZGP2DSPP, &
 !  - INOUT A S
 & PGPAR, &
 !  - OUT A S
 & ZTENDT, ZTENDR, ZTENDW,&
 & ZTENDLIMA, &
 & ZTENDTKE, ZTENDEFB1, ZTENDEFB2, ZTENDEFB3,&
 & ZTENDEXT,&
 !---------------------------------------------------------------------
...
INTEGER(KIND=JPIM),INTENT(IN)    :: IMAXDRAFT
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDR(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_PHY_MF%YRPARAR%NRR) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDW(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDLIMA(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NLIMA)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDTKE(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTENDEFB1(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTENDEFB2(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTENDEFB3(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDEXT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZP1EZDIAG(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EZDIAG)
TYPE(TYPE_INTPROCSET), INTENT(INOUT) :: YLPROCSET
...
REAL(KIND=JPRB) :: ZMF_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZTHETAL_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZQT_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZTHTV_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZQC_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZQI_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZU_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
REAL(KIND=JPRB) :: ZV_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,IMAXDRAFT)
...
IF (LMICRO) THEN
  ...
  CALL GPGEO(YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,ZAPHIM,ZAPHIFM,ZTM,ZRHM,YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDGEOMETRY%YRVERT_GEOM)
...
! specific to new data flow for diagnostics
  ...
  ELSE
    ! missing interface !!! REK
    CALL ARO_SUINTBUDGET(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,YDCPG_DIM%KFDIA,ZQDM,ZEXNREFM_,YLMF_PHYS_STATE%YCPG_DYN%RCP%CP)
  ENDIF
ENDIF
...
IF (LRDUST) THEN
  ...
  CALL SWAP_SVM
  ! input dust scalar concentration in ppp from
  CALL ARO_MNHDUST (IKL,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT, PDTPHY,ZSVMIN_,ZZZ_,ZDZZ_,&
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  LLAROME=.TRUE.
  CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS, LLAROME, YDCPG_DIM%KIDIA , YDCPG_DIM%KFDIA  , YDCPG_DIM%KLON, YLMF_PHYS_STATE%YCPG_DYN%PHIF(:,YDCPG_DIM%KFLEVG), &
   & ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,ZTN,ZTSURF,YDMF_PHYS_SURF%GSD_VF%PLSM,YLMF_PHYS_STATE%Q(:,YDCPG_DIM%KFLEVG), YLMF_PHYS_STATE%U(:,YDCPG_DIM%KFLEVG), YLMF_PHYS_STATE%V(:,YDCPG_DIM%KFLEVG), ZTHETAS, &
...
!* Compute PBL-diagnostics
  ...
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMF_PHYS%OUT%UCLS,&
   & YDMF_PHYS%OUT%VCLS,YLMF_PHYS_STATE%U(:,1:YDCPG_DIM%KFLEVG),YLMF_PHYS_STATE%V(:,1:YDCPG_DIM%KFLEVG),ZCAPE,ZDCAPE,ZTKEM(:,1:YDCPG_DIM%KFLEVG),YLMF_PHYS_STATE%YCPG_DYN%PHIF(:,1:YDCPG_DIM%KFLEVG),YDOROG%OROG,&
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  CALL ACTQSAT ( YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTQSAT,YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREF, ZCPM, ZQVM, ZTM,&
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
    ...
    IF (NAER >= 1 ) THEN
      IF(YSD_VAD%NUMFLDS >= 4) THEN
        CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDPHY, YDCPG_DIM%KIDIA , YDCPG_DIM%KFDIA , YDCPG_DIM%KLON  , YDCPG_DIM%KFLEVG,&
         & YLMF_PHYS_STATE%YCPG_PHY%PRE,YLMF_PHYS_STATE%YCPG_PHY%PREF,ZTM,ZTSURF,YDMF_PHYS_SURF%GSD_VA%PSEA,YDMF_PHYS_SURF%GSD_VA%PLAN,YDMF_PHYS_SURF%GSD_VA%PSOO,YDMF_PHYS_SURF%GSD_VA%PDES,YDMF_PHYS_SURF%GSD_VA%PSUL,YDMF_PHYS_SURF%GSD_VA%PVOL,ZAER,ZAERINDS)
      ELSE
  ...
  IF (NOZOCL == 1) THEN
    ! as in IFS
    CALL RADOZC(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1,YDCPG_DIM%KLON,0,YLMF_PHYS_STATE%YCPG_PHY%PRE,YDGSGEOM%GEMU,ZROZ)
  ...
  ELSEIF (NOZOCL == 2) THEN
    ...
    CALL SUOZON(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZQO3,.FALSE.,YLMF_PHYS_STATE%YCPG_PHY%PRE,YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZALBD   , ZALBP , YLMF_PHYS_STATE%YCPG_PHY%PRE   , YLMF_PHYS_STATE%YCPG_PHY%PREF  ,           &
     & YDVARS%A%T1   , ZQO3  , ZAER     , YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP   , ZEMIS   , &
   ...
   & YLMF_PHYS_STATE%YCPG_PHY%PRE  , ZEMIS  , YDMF_PHYS%RAD%EMTD  , YDMF_PHYS_TMP%RDG%MU0, ZQVM,&
   & ZTENT  , YDMF_PHYS%RAD%TRSW  , ZTRSOD , ZTSURF   , PDTPHY,&
...
CALL ACNPART(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTNEBU,YDCPG_DIM%KFLEVG,&
 & YLMF_PHYS_STATE%YCPG_DYN%PHI,YLMF_PHYS_STATE%YCPG_DYN%PHIF,YLMF_PHYS_STATE%YCPG_PHY%PREF,ZDECRD,ZNEB0,&
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & YLMF_PHYS_STATE%YCPG_PHY%PRE,YLMF_PHYS_STATE%YCPG_PHY%PREF,YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,YLMF_PHYS_STATE%YCPG_DYN%RCP%R,YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZNEB0,&
   & ZQV,ZQCO2,ZQICE,ZQLIQ,ZQO3,YLMF_PHYS_STATE%T,&
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL,YDCPG_DIM%KGPCOMP,&
       ...
       & ZPABSM__(:,IKB),YLMF_PHYS_STATE%YCPG_PHY%PRE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG),&
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDVISI,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YLMF_PHYS_STATE%YCPG_DYN%PHI,YLMF_PHYS_STATE%YCPG_DYN%PHIF,YLMF_PHYS_STATE%YCPG_PHY%PREF,&
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    CALL VDFHGHTHL(YDMODEL%YRML_PHY_G%YRVDF,YDMODEL%YRML_PHY_SLIN%YREPHLI, &
     ...
     & PDTPHY, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,&
     & ZTM,ZQVM,ZQCM,ZQIM,YDVARS%A%T1,&
     & YLMF_PHYS_STATE%YCPG_PHY%PRE, YLMF_PHYS_STATE%YCPG_PHY%PREF, ZAPHIFM,ZAPHIM,&
     & ZZS_FTH_,ZZS_FRV_,ZZS_FU_,ZZS_FV_,&
     & ZMF_UP,ZTHETAL_UP,ZQT_UP,ZTHTV_UP,ZQC_UP,ZQI_UP,&
     & ZU_UP, ZV_UP,&
     & ZGP2DSPP, NGFL_EZDIAG, ZP1EZDIAG, &
...
IF (LMICRO) THEN
  ...
  IF (LRDEPOS) THEN
    ...
    ! Swapp because IN and OUT will be needed simultaneously
    CALL SWAP_SVM
    CALL ARO_RAINAERO(YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,NGFL_EXT,NRR, PDTPHY,ZSVMIN_,ZZZ_,&
     & ZPABSM__(:,1:YDCPG_DIM%KFLEVG),ZTHM__(:,1:YDCPG_DIM%KFLEVG),ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),&
...
IF (LNUDGLH.AND.YDCPG_DIM%KSTEP > NSTARTNUDGLH.AND.YDCPG_DIM%KSTEP <= NSTOPNUDGLH) THEN
  ...
  IF(MOD(ISTEP,NINTNUDGLH)==0) THEN
    ...
    CALL NUDGLH(NGPTOT,NPROMA,NGPBLKS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,ZACPRR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
       ...
       & ZQDM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ZTENDR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG,1),NRR,LNUDGLHNREAD)
...
IF (LUSECHEM) THEN
  ...
  CALL ARO_MNHC(ZSVSIN_,&
   & ZRHODREFM__(:,1:YDCPG_DIM%KFLEVG),PDTPHY, ZTHM__(:,1:YDCPG_DIM%KFLEVG), ZPABSM__(:,1:YDCPG_DIM%KFLEVG), ZRM_, ZLAT_, ZLON_, ZALB_UV_, &
   & ZZS_, ZZENITH_,ZZZ_, IYEAR,IMONTH,IDAY, REAL(RHGMT,JPRB)+PDTPHY/2._JPRB,&
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP,YGFL, &
   & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1,PDTPHY,         &  ! In: block indices, physicstimestep
   & PTSL=YLMF_PHYS_STATE%T,PQSL=YLMF_PHYS_STATE%Q, PA=YDVARS%A%T1, PAP=YLMF_PHYS_STATE%YCPG_PHY%PREF, PAPH=YLMF_PHYS_STATE%YCPG_PHY%PRE,   &  ! In: (T,Q,cloud) forsupersatcheck, Pfull, Phalf
   & PDYN_U=ZDUMMY,PDYN_V=ZDUMMY,PDYN_T=ZDUMMY,PDYN_Q=ZDUMMY,  &  ! In: dummy nonphys tendencies
   & PUNP_U=YDMF_PHYS%OUT%TENDU,PUNP_V=YDMF_PHYS%OUT%TENDV,PUNP_T=ZTENDT,PUNP_Q=ZTENDR(:,:,1), &  ! In: (u,v,t,qv) tendencies to perturb
   & PMULNOISE=PGP2DSDT(1,1,1),                           &  ! In: stochphy 3D random multiplicative pattern (less one)
   & PTENU=YDMF_PHYS%OUT%TENDU,PTENV=YDMF_PHYS%OUT%TENDV,PTENT=ZTENDT,PTENQ=ZTENDR(:,:,1) )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF(LLMSE.OR.LSFORCS) THEN
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ...
    PGPAR(JLON,MACPRR)=PGPAR(JLON,MACPRR)+(ZINPRR_(JLON)+ZSURFPREP(JLON)/1000._JPRB)*PDTPHY
    PGPAR(JLON,MACPRS)=PGPAR(JLON,MACPRS)+(ZINPRS_(JLON)+ZSURFSNOW(JLON)/1000._JPRB)*PDTPHY
    PGPAR(JLON,MACPRG)=PGPAR(JLON,MACPRG)+(ZINPRG_(JLON)+ZINPRH_(JLON))*PDTPHY
  ENDDO
...
IF (LFLASH) THEN
  ...
  CALL DIAGFLASH(YDCFU,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,&
    ...
    &YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZTM,YLMF_PHYS_STATE%YCPG_PHY%W,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS%OUT%FLASH)
ENDIF
...
IF (LINTFLEX) THEN
  ...
  CALL APL_AROME2INTFLEX(YGFL,YDPARAR,YDPHY,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG, PDTPHY,&
   & YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%T, PGPAR(1,MVTS), YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,&
   & ZFPR,&! precipitation fluxes
   & LLRAD, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO,&! radiative fluxes
   & YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDT,&! momentum and temperature tendencies
   & ZTENDR, ZTENDTKE, ZTENDEXT,&! total gfl tendencies
   & YLPROCSET)
ENDIF
...
CALL PPWETPOINT(YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YLMF_PHYS_STATE%YCPG_PHY%PRE(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%TCLS,&
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
      CALL PPWETPOINT(YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YLMF_PHYS_STATE%YCPG_PHY%PREF(:,JLEV),ZTM(:,JLEV),&
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDOROG%OROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YLMF_PHYS_STATE%YCPG_DYN%PHIF,&
