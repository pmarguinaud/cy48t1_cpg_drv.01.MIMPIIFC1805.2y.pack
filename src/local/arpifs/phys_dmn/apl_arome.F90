...
IF (LFLEXDIA) THEN
  ...
  DO JR=1,NRR
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
        ZTMPAF(JLON,JLEV)=(ZTENDR(JLON,JLEV,JR)-ZTENDRBAK(JLON,JLEV,JR))*YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZINVG
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
      ...
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF(LLHN(JLON,JLEV))THEN
          ...
          IF(YLMF_PHYS_BASE_STATE%T(JLON,JLEV)>0.01_JPRB.AND.LNUDGLHCOMPT)THEN
            ZTENDR(JLON,JLEV,1)=ZTENDR(JLON,JLEV,1)+RLVTT/RV/((YLMF_PHYS_BASE_STATE%T(JLON,JLEV))**2._JPRB)* &
