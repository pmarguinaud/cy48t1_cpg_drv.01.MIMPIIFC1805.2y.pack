...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PCLON(YDCPG_DIM%KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PSLON(YDCPG_DIM%KLON)
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  IF (NOZOCL == 1) THEN
    ! as in IFS
    CALL RADOZC(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,1,YDCPG_DIM%KLON,0,YDMF_PHYS_STATE%YCPG_PHY%PRE,PGEMU,ZROZ)
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & ZALBD1,ZALBP1,ZEMIS,PGELAM,PGEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_TMP%RDG%MU0LU,ZTSURF, &
   & ZDECRD,ZCLCT_RAD,&
...
IF(LKFBCONV) THEN
  ...
  CALL BRI2ACCONV(YDMODEL%YRML_PHY_MF,YDGEOMETRY%YREGEO,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PGM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
