...
!!! Initialisations temporaires d'arguments non-utilises
  ...
  IF (LLMSE.OR.LSFORCS) THEN
    ZACPRR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRR)
    ZACPRS_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRS)
    ZACPRG_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MACPRG)
    ZINPRR_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRR)
    ZINPRS_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRS)
    ZINPRG_NOTINCR_(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MINPRG)
  ENDIF
...
! Albedo dans les intervalles, direct (parallel) et diffus (diffuse).
  IF (NSW==6.OR.NSW==1) THEN
    IF (LLMSE) THEN
      DO JSW=1,NSW
        ZALBP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR-1+JSW)
        ZALBD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA-1+JSW)
        IF (LHLRADUPD) THEN
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
  ...
  ! to be applied for HLRADIA, ACRANEB2 which use a single solar spectral band
    IF (LHLRADUPD) THEN
      ...
      ENDDO
    ELSE
       ZALBP1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR)
       ZALBD1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA)
    ENDIF
  ...
  IF (LLMSE) THEN
    ZEMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVEMIS)
    ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVTS)
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZTRSODIF, ZPIZA_DST_, ZCGA_DST_, ZTAUREL_DST_, ZAERINDS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDCPG_GPAR%ZVIEW,             &
     & YDMF_PHYS_TMP%RDG%MU0LU, ZALBD1, ZFRSOLU)
   ELSE
     IF (LLMSE) THEN 
       DO JSW=1,NSW 
         ZTRSODIR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIR+JSW-1)
         ZTRSODIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIF+JSW-1)
       ENDDO 
     ENDIF
     ZCTRSO(:,:)=0._JPRB
   ENDIF
...
IF(LLMSE.OR.LSFORCS) THEN
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    YDCPG_GPAR%ZVIEW(JLON,MINPRR)=ZINPRR_(JLON)+ZSURFPREP(JLON)/1000._JPRB
    YDCPG_GPAR%ZVIEW(JLON,MINPRS)=ZINPRS_(JLON)+ZSURFSNOW(JLON)/1000._JPRB
    YDCPG_GPAR%ZVIEW(JLON,MINPRG)=ZINPRG_(JLON)+ZINPRH_(JLON)
    YDCPG_GPAR%ZVIEW(JLON,MACPRR)=YDCPG_GPAR%ZVIEW(JLON,MACPRR)+(ZINPRR_(JLON)+ZSURFPREP(JLON)/1000._JPRB)*PDTPHY
    YDCPG_GPAR%ZVIEW(JLON,MACPRS)=YDCPG_GPAR%ZVIEW(JLON,MACPRS)+(ZINPRS_(JLON)+ZSURFSNOW(JLON)/1000._JPRB)*PDTPHY
    YDCPG_GPAR%ZVIEW(JLON,MACPRG)=YDCPG_GPAR%ZVIEW(JLON,MACPRG)+(ZINPRG_(JLON)+ZINPRH_(JLON))*PDTPHY
  ENDDO
  YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVTS)=ZTSURF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVEMIS)=ZEMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MVQS)=ZQS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  DO JSW=1,NSW
    YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBDIR-1+JSW)=ZALBP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)
    YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MALBSCA-1+JSW)=ZALBD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSW)
  ENDDO
ENDIF
...
IF (LINTFLEX) THEN
  ...
  & YDCPG_GPAR%ZVIEW(:, MVTS), YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZFPR, LLRAD, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSO,          &
