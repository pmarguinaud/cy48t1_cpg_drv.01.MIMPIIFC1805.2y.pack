...
SUBROUTINE APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 & KTDIA  , KLEV , KSTEP ,&
 ...
 & PFCLN,PUCLS,PVCLS,PNUCLS,PNVCLS,PTCLS,PQCLS,PHUCLS,PUGST,PVGST,PFEVL,PFEVN, PPBLH,&
 & PDIAGH,PFLASH,PTPWCLS,PDPRECIPS,PDPRECIPS2,PVISICLD,PVISIHYDRO,PMXCLWC,&
 ! daand: radflex
 & YDPROCSET ,YDDDH)
...
!    Integers
INTEGER(KIND=JPIM) :: JLEV, JLON, JRR, JGFL, JGR, ISPLITR, JDIAG
...
REAL(KIND=JPRB), POINTER :: ZEZDIAG_POS (:,:)
REAL(KIND=JPRB), TARGET :: ZEZDIAG_DUM (KLON,KLEV)
...
IF (YSPP_CONFIG%IEZDIAG_POS > 0) THEN
  ZEZDIAG_POS => YDVARS%EZDIAG(YSPP_CONFIG%IEZDIAG_POS)%T0
ELSE
  ZEZDIAG_POS => ZEZDIAG_DUM
ENDIF
...
DO JLEV=1,KLEV
  ...
  YDMF_PHYS%TENDU(KIDIA:KFDIA,JLEV)=0.0_JPRB
  YDMF_PHYS%TENDV(KIDIA:KFDIA,JLEV)=0.0_JPRB
...
! daand: radflex
ZFRSO => YDMF_PHYS%FRSO(:,:,1)
ZFRTH => YDMF_PHYS%FRTH(:,:,1)
...
IF (LMICRO) THEN
  ...
  IF (LMFSHAL .AND. (CMF_CLOUD=='DIRE'.OR.CMF_CLOUD=='BIGA')) THEN
    ...
    ELSE
      DO JLEV = 1, KLEV 
        ZRC_MF_(KIDIA:KFDIA,JLEV)=YDVARS%EZDIAG(1)%T0 (KIDIA:KFDIA,JLEV)
        ZRI_MF_(KIDIA:KFDIA,JLEV)=YDVARS%EZDIAG(2)%T0 (KIDIA:KFDIA,JLEV)
        ZCF_MF_(KIDIA:KFDIA,JLEV)=YDVARS%EZDIAG(3)%T0 (KIDIA:KFDIA,JLEV)
      ENDDO
    ENDIF
    YDVARS%EZDIAG(1)%T0 = 0._JPRB
    YDVARS%EZDIAG(2)%T0 = 0._JPRB
    YDVARS%EZDIAG(3)%T0 = 0._JPRB
  ENDIF
...
IF (LFLEXDIA) THEN
  ...
  DO JR=1,NRR
    ...
    DO JLEV = 1, KLEV
      DO JLON=KIDIA,KFDIA 
        ZTMPAF(JLON,JLEV)=YDVARS%A%T1(JLON,JLEV)*PDELPM(JLON,JLEV)
      ENDDO
    ENDDO
    IF (LDDH_OMP) THEN
...
!     --------------------------------------------------------------------
IF (LRDUST) THEN
  DO JDIAG = IOFF_MFSHAL, NGFL_EZDIAG
    YDVARS%EZDIAG(JDIAG)%T0(KIDIA:KFDIA,1:KLEV)=0.0_JPRB
  ENDDO
  ...
  DO JDIAG = IOFF_MFSHAL, NGFL_EZDIAG
    YDVARS%EZDIAG(JDIAG)%T0(KIDIA:KFDIA,1:KLEV)=ZPEZDIAG_(KIDIA:KFDIA,1:KLEV,JDIAG)
  ENDDO
! return to tendency
  DO JGFL=1, NGFL_EXT
    DO JLEV = 1,KLEV
...
IF (LSFORCS) THEN   ! <== Surface forcing for MUSC
  ...
  CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS, LLAROME, KIDIA , KFDIA  , KLON, PAPHIFM(:,KLEV), &
   & ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,ZTN,ZTSURF,YDMF_PHYS_SURF%GSD_VF%PLSM,PQVM(:,KLEV), PUM(:,KLEV), PVM(:,KLEV), ZTHETAS, &
...
!* Compute PBL-diagnostics
   ...
   ZCAPE(:)=0._JPRB
   ZDCAPE(:)=0._JPRB
  CALL ACCLDIA(YDXFU,YDPHY,YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,KIDIA,KFDIA,KLON,KLEV,YDMF_PHYS%UCLS,&
   & YDMF_PHYS%VCLS,PUM(:,1:KLEV),PVM(:,1:KLEV),ZCAPE,ZDCAPE,ZTKEM(:,1:KLEV),PAPHIFM(:,1:KLEV),POROG,&
   & YDMF_PHYS%UGST,YDMF_PHYS%VGST,YDMF_PHYS%CLPH,ICLPH)

  YDMF_PHYS%CLPH(KIDIA:KFDIA)=MIN(XMAXLM,MAX(XMINLM,YDMF_PHYS%CLPH(KIDIA:KFDIA))) 
...
IF (LRAYFM.OR.LRAY) THEN
  ...
  IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RADGR) THEN
   ...
   IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
    ...
    JKE=2*YSPP%MP_RADGR
    DO JLON=KIDIA,KFDIA
     YDVARS%EZDIAG(YSPP_CONFIG%IEZDIAG_POS)%T0(JLON,JKO) = PGP2DSPP(JLON,YSPP%MP_RADGR)
     YDVARS%EZDIAG(YSPP_CONFIG%IEZDIAG_POS)%T0(JLON,JKE) = ZRADGR(JLON)
    ENDDO
   ENDIF
  ELSE
   DO JLON=KIDIA,KFDIA
  ...
  IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RADSN) THEN
   ...
   IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
    ...
    JKE=2*YSPP%MP_RADSN
    DO JLON=KIDIA,KFDIA
     YDVARS%EZDIAG(YSPP_CONFIG%IEZDIAG_POS)%T0(JLON,JKO) = PGP2DSPP(JLON,YSPP%MP_RADSN)
     YDVARS%EZDIAG(YSPP_CONFIG%IEZDIAG_POS)%T0(JLON,JKE) = ZRADSN(JLON)
    ENDDO
   ENDIF
  ELSE
   DO JLON=KIDIA,KFDIA
...
!              ZALBP(JLON,JSW)=PALBIN(JLON)+ZSALBCOR
    ...
    IF (NAER >= 1 ) THEN
      IF(YSD_VAD%NUMFLDS >= 4) THEN
        CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDPHY, KIDIA , KFDIA , KLON  , KLEV,&
         & PAPRSM,PAPRSFM,ZTM,ZTSURF,YDMF_PHYS_SURF%GSD_VA%PSEA,YDMF_PHYS_SURF%GSD_VA%PLAN,YDMF_PHYS_SURF%GSD_VA%PSOO,YDMF_PHYS_SURF%GSD_VA%PDES,YDMF_PHYS_SURF%GSD_VA%PSUL,YDMF_PHYS_SURF%GSD_VA%PVOL,ZAER,ZAERINDS)
      ELSE
  ...
  ELSEIF (NOZOCL == 2) THEN
    ...
    CALL SUOZON(KIDIA,KFDIA,KLON,KLEV,ZQO3,.FALSE.,PAPRSM,PRDELPM,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(KSTEP,NRADFR) == 0) THEN 
     ...
     & YDVARS%A%T1   , ZQO3  , ZAER     , PDELPM   , ZEMIS   , &
     & PMU0M   , ZQV   , ZQSAT    , ZQICE    , ZQLIQ   , &
     & ZQSM    , ZQRM  , YDMF_PHYS_SURF%GSD_VF%PLSM     , ZTM      , ZTSURF  , &
     & PGP2DSPP, ZEZDIAG_POS, &
     & PEMTD   , PEMTU , PTRSO    , YDMF_PHYS%FRTHC   , YDMF_PHYS%FRTH   , & 
     & YDMF_PHYS%FRSOC  , YDMF_PHYS%FRSO , ZZS_FSWDIR, ZZS_FSWDIF, ZFSDNN  , &
   ...
   & ZFRSO  , ZFRTH  , YDMF_PHYS%FRSODS, YDMF_PHYS%FRTHDS, ZCEMTR , ZCTRSO , YDMF_PHYS%FRSOC , YDMF_PHYS%FRTHC,&
   & ZSUDU  , ZSDUR  , ZDSRP  , ZZS_FSWDIR , ZZS_FSWDIF    ,&
   & YDMF_PHYS%FRSOPS, ZFRSOFS, YDMF_PHYS%FRSOPT )

  ! daand: radflex
  IF (LRADFLEX) THEN
    ! store for further calculations and diagnostics
    ! warning : pointers. REK
    YDMF_PHYS%FRSO(:,:,1)=ZFRSO
    YDMF_PHYS%FRTH(:,:,1)=ZFRTH
  ELSE
  ...
  DO JLON = KIDIA, KFDIA
    ...
    IF (PMU0(JLON) > 3.0E-02_JPRB) THEN
      YDMF_PHYS%FRSDNI(JLON)=MAX(0.0_JPRB,YDMF_PHYS%FRSOPS(JLON)/PMU0(JLON))
    ELSE
      YDMF_PHYS%FRSDNI(JLON)=MAX(0.0_JPRB,YDMF_PHYS%FRSOPS(JLON))
    ENDIF
  ENDDO 
    ...
    IF (LLMSE) THEN
      DO JSW=1, NSW
        WRITE(NULOUT,*)'ZSFSWDIR ZSFSWDIF ZFSDNN ZFSDNV PFRSO',&
         & ZZS_FSWDIR(NPTP,JSW),ZZS_FSWDIF(NPTP,JSW),ZFSDNN(NPTP), ZFSDNV(NPTP),YDMF_PHYS%FRSO(NPTP,KLEV,1)
        WRITE(NULOUT,*)'ZALBD ZALBP',ZALBD(NPTP,JSW),ZALBP(NPTP,JSW)
      ENDDO
    ENDIF
    WRITE(NULOUT,*)ZFSDNN(NPTP),ZFSDNV(NPTP)
    WRITE (NULOUT,*)'TSURF EMIS ZFRTH',ZTSURF(NPTP),ZEMIS(NPTP),YDMF_PHYS%FRTHDS(NPTP)
  ENDIF

  IF (LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%FRSO(:,:,1),'FCTRAYSO',YDDDH)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%FRTH(:,:,1),'FCTRAYTH',YDDDH)
    ELSE
      CALL ADD_FIELD_3D(YLDDH,YDMF_PHYS%FRSO(:,:,1),'FCTRAYSO','F','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_3D(YLDDH,YDMF_PHYS%FRTH(:,:,1),'FCTRAYTH','F','ARP',.TRUE.,.TRUE.)
    ENDIF
  ENDIF

ELSE

  YDMF_PHYS%FRSOC(KIDIA:KFDIA,0:1)=0.0_JPRB
  YDMF_PHYS%FRTHC(KIDIA:KFDIA,0:1)=0.0_JPRB
...
CALL ACNPART(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
 ...
 & YDMF_PHYS%CLCH,YDMF_PHYS%CLCM,YDMF_PHYS%CLCL,PCLCT,ZCLCT_RAD)
...
!+++ End of redundant input preparations for ACRANEB
  ...
  CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
   ...
   & ZFRSO,ZFRTH,&
   & YDMF_PHYS%FRSOC,YDMF_PHYS%FRTHC,ZFRSODS,YDMF_PHYS%FRSOPS,ZFRSOLU,YDMF_PHYS%FRTHDS,ZAER)
  ...
  DO JLON=KIDIA,KFDIA
    YDMF_PHYS%FRSODS(JLON)=ZFRSODS(JLON)+YDMF_PHYS%FRSOPS(JLON)  ! downward surface sw flux
  ENDDO
  ...
  IF (LLMSE) THEN
    IF (LHLRADUPD) THEN
      DO JSW = 1,NSW
        DO JLON=KIDIA,KFDIA
          ZZS_FSWDIR(JLON,JSW) = YDMF_PHYS%FRSOPS(JLON)*RSUN2(JSW)
          ZZS_FSWDIF(JLON,JSW) = ZFRSODS(JLON)*RSUN2(JSW)
         ENDDO
      ENDDO
    ELSE
      ZZS_FSWDIR(KIDIA:KFDIA,1)=YDMF_PHYS%FRSOPS(KIDIA:KFDIA) ! direct surface swdn flux
      ZZS_FSWDIF(KIDIA:KFDIA,1)=ZFRSODS(KIDIA:KFDIA) ! diffuse surface swdn flux
    ENDIF
  ENDIF

  ! Estimate of the direct normal irradiance, with securities
  YDMF_PHYS%FRSDNI(KIDIA:KFDIA)=YDMF_PHYS%FRSOPS(KIDIA:KFDIA)
  DO JLON = KIDIA, KFDIA
    IF (PMU0(JLON) > 3.0E-02_JPRB) THEN
      YDMF_PHYS%FRSDNI(JLON)=YDMF_PHYS%FRSOPS(JLON)/PMU0(JLON)
    ENDIF
  ENDDO
  DO JLON = KIDIA, KFDIA
    YDMF_PHYS%FRSDNI(JLON)=MAX(0.0_JPRB,YDMF_PHYS%FRSDNI(JLON))
  ENDDO

  IF (LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%FRSO(:,:,1),'FCTRAYSO',YDDDH)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%FRTH(:,:,1),'FCTRAYSO',YDDDH)
    ELSE
      CALL ADD_FIELD_3D(YLDDH,YDMF_PHYS%FRSO(:,:,1),'FCTRAYSO','F','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_3D(YLDDH,YDMF_PHYS%FRTH(:,:,1),'FCTRAYTH','F','ARP',.TRUE.,.TRUE.)
    ENDIF
...
IF (.NOT.(LRAY.AND.NRAY == 2.AND.LRADFLEX).AND..NOT.LRAYFM) THEN
  ...
  ENDDO
  YDMF_PHYS%FRSOPS(KIDIA:KFDIA)=0._JPRB
ENDIF
...
!$OMP CRITICAL (ARO_GROUND_PARAM_LOCK)
    ...
    IF (LLMSE_PARAM) THEN
      CALL ARO_GROUND_PARAM( KBL,KGPCOMP,&
       ...
       & ZINPRG_NOTINCR_,&
       & YDMF_PHYS%FRTHDS(KIDIA:KFDIA),ZZS_FSWDIF(KIDIA:KFDIA,1:NSW),&
       ...
       & ZEMIS(KIDIA:KFDIA),ZTSURF(KIDIA:KFDIA),YDMF_PHYS%FRTH(KIDIA:KFDIA,KLEV,1))
...
!ZSFRV_(KIDIA:KFDIA) = 0._JPRB
    ...
    IF (LLMSE_DIAG) THEN
      ...
      CALL ARO_GROUND_DIAG( KBL, KGPCOMP,&
       ...
       & YDMF_PHYS%FRTH(KIDIA:KFDIA,KLEV,1),YDMF_PHYS%FRSO(KIDIA:KFDIA,KLEV,1),&
       & PINDX(KIDIA:KFDIA),PINDY(KIDIA:KFDIA),&
       & ZQS(KIDIA:KFDIA),ZGZ0_,ZGZ0H_,&
       & YDMF_PHYS%TCLS(KIDIA:KFDIA),YDMF_PHYS%QCLS(KIDIA:KFDIA),YDMF_PHYS%RHCLS(KIDIA:KFDIA),&
       & YDMF_PHYS%UCLS(KIDIA:KFDIA),YDMF_PHYS%VCLS(KIDIA:KFDIA),&
       & YDMF_PHYS%NUCLS(KIDIA:KFDIA),YDMF_PHYS%NVCLS(KIDIA:KFDIA),&
       & YDMF_PHYS%FCLL(KIDIA:KFDIA,1),YDMF_PHYS%FCLN(KIDIA:KFDIA,1),&
       & YDMF_PHYS%FEVL(KIDIA:KFDIA,1),YDMF_PHYS%FEVN(KIDIA:KFDIA,1),&
...
!* Compute PBL-diagnostics
   ...
   CALL ACVISIH(YDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,PAPHIM,PAPHIFM,PAPRSFM,&
    & ZTM,ZRHM,ZQCM,ZQIM,ZQRM,ZQSM,ZQGM,YDMF_PHYS%VISICLD, YDMF_PHYS%VISIHYD,YDMF_PHYS%MXCLWC)
...
IF (LMFSHAL) THEN
  IF (CMF_UPDRAFT=='DUAL') THEN
    ...
    CALL VDFHGHTHL(YDMODEL%YRML_PHY_G%YRVDF,YDMODEL%YRML_PHY_SLIN%YREPHLI, &
     ...
     & ZU_UP, ZV_UP,&
     & PGP2DSPP, NGFL_EZDIAG, ZEZDIAG_POS, &
  ...
  DO JDRAFT=IDRAFT,3
    ...
    IF ((CMF_CLOUD=='DIRE'.OR.CMF_CLOUD=='BIGA').AND.JDRAFT==3) THEN
      ...
      DO JLEV = 1,KLEV
        YDVARS%EZDIAG(1)%T0(KIDIA:KFDIA,JLEV)=ZRC_MF_(KIDIA:KFDIA,JLEV)
        YDVARS%EZDIAG(2)%T0(KIDIA:KFDIA,JLEV)=ZRI_MF_(KIDIA:KFDIA,JLEV)
        YDVARS%EZDIAG(3)%T0(KIDIA:KFDIA,JLEV)=ZCF_MF_(KIDIA:KFDIA,JLEV)
      ENDDO
    ENDIF
    DO JLEV = 1,KLEV
      DO JLON = KIDIA,KFDIA
        ...
        YDMF_PHYS%TENDU(JLON,JLEV)=YDMF_PHYS%TENDU(JLON,JLEV)+ZMFUS_(JLON,JLEV)
        YDMF_PHYS%TENDV(JLON,JLEV)=YDMF_PHYS%TENDV(JLON,JLEV)+ZMFVS_(JLON,JLEV)
...
!!$  ENDIF
  ...
  DO JLEV = 1,KLEV
    DO JLON = KIDIA,KFDIA
      YDMF_PHYS%TENDU(JLON,JLEV)=YDMF_PHYS%TENDU(JLON,JLEV)+ZTENDU_TURB__(JLON,JLEV)
      YDMF_PHYS%TENDV(JLON,JLEV)=YDMF_PHYS%TENDV(JLON,JLEV)+ZTENDV_TURB__(JLON,JLEV)
...
IF (LMICRO) THEN
  ...
  IF (LFPREC3D) THEN
    DO JR=2,NRR ! no precip for qv
      DO JLEV=1,KLEV
        DO JLON=KIDIA,KFDIA
          YDVARS%EZDIAG(4)%T0(JLON,JLEV)=YDVARS%EZDIAG(4)%T0(JLON,JLEV)+ZPFPR_(JLON,JLEV,JR)*1000._JPRB*PDT
        ENDDO
      ENDDO
    ENDDO
  ENDIF
...
IF(YSPPT_CONFIG%LSPSDT) THEN
  ...
  CALL SPPTEN (YDMODEL%YRML_PHY_EC%YRECLDP,YGFL, &
   ...
   & PTSL=PTM,PQSL=PQVM, PA=YDVARS%A%T1, PAP=PAPRSFM, PAPH=PAPRSM,   &  ! In: (T,Q,cloud) forsupersatcheck, Pfull, Phalf
   & PDYN_U=ZDUMMY,PDYN_V=ZDUMMY,PDYN_T=ZDUMMY,PDYN_Q=ZDUMMY,  &  ! In: dummy nonphys tendencies
   & PUNP_U=YDMF_PHYS%TENDU,PUNP_V=YDMF_PHYS%TENDV,PUNP_T=PTENDT,PUNP_Q=PTENDR(:,:,1), &  ! In: (u,v,t,qv) tendencies to perturb
   & PMULNOISE=PGP2DSDT(1,1,1),                           &  ! In: stochphy 3D random multiplicative pattern (less one)
   & PTENU=YDMF_PHYS%TENDU,PTENV=YDMF_PHYS%TENDV,PTENT=PTENDT,PTENQ=PTENDR(:,:,1) )    ! Out: (u,v,t,qv) total perturbed tendencies
ENDIF
...
IF(LFORCENL.AND.(KSTEP*(TSPHY/RHOUR)>=NFORCESTART).AND.&
  ...
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%TENDU(JLON,JLEV)=YDMF_PHYS%TENDU(JLON,JLEV)+AMAGSTOPH_CASBS*PFORCEU(JLON,JLEV)
      YDMF_PHYS%TENDV(JLON,JLEV)=YDMF_PHYS%TENDV(JLON,JLEV)+AMAGSTOPH_CASBS*PFORCEV(JLON,JLEV)
...
DO JLEV = 0,KLEV
  DO JLON = KIDIA,KFDIA
    ...
    YDMF_PHYS%FPLSL(JLON,JLEV)= ZINPRR_(JLON)*1000._JPRB+ZSURFPREP(JLON)
    YDMF_PHYS%FPLSN(JLON,JLEV)= ZINPRS_(JLON)*1000._JPRB+ZSURFSNOW(JLON)
    YDMF_PHYS%FPLSG(JLON,JLEV)= ZINPRG_(JLON)*1000._JPRB
    YDMF_PHYS%FPLSH(JLON,JLEV)= ZINPRH_(JLON)*1000._JPRB
    ! conversion in correct Unit for BADP (same as ALADIN)
    YDMF_PHYS%STRTU(JLON,JLEV)= ZSFU_(JLON)*ZRHODREFM__(JLON,IKB) 
    YDMF_PHYS%STRTV(JLON,JLEV)= ZSFV_(JLON)*ZRHODREFM__(JLON,IKB) 
  ENDDO
ENDDO
!Hail diagnostic
YDMF_PHYS%DIAGH(KIDIA:KFDIA)=0._JPRB
IF (LXXDIAGH) THEN
  DO JLEV=1,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%DIAGH(JLON)=YDMF_PHYS%DIAGH(JLON)+ZQGM(JLON,JLEV)*PDELPM(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
ENDIF
! lightening density
IF (LFLASH) THEN
  IF (KSTEP==0) YDMF_PHYS%FLASH=0._JPRB

  CALL DIAGFLASH(YDCFU,KIDIA,KFDIA,KLON,KLEV,KSTEP,&
    &ZQCM,ZQIM,ZQRM,ZQSM,ZQGM,ZQHM,&
    &PDELPM,ZTM,PWM,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS%FLASH)
ENDIF
!!! modif pour LMSE non activee
IF (LLMSE) THEN
  DO JLEV=1,KSGST+1
    DO JLON = KIDIA,KFDIA
      YDMF_PHYS%FCS(JLON,JLEV)=-ZSFTH_(JLON)*ZRHODREFM__(JLON,IKB)*RCPD
      YDMF_PHYS%FCLL(JLON,JLEV) = YDMF_PHYS%FCLL(JLON,JLEV)*ZRHODREFM__(JLON,IKB)
      YDMF_PHYS%FCLN(JLON,JLEV) = YDMF_PHYS%FCLN(JLON,JLEV)*ZRHODREFM__(JLON,IKB)
      YDMF_PHYS%FEVL(JLON,JLEV) = YDMF_PHYS%FEVL(JLON,JLEV)*ZRHODREFM__(JLON,IKB)
      YDMF_PHYS%FEVN(JLON,JLEV) = YDMF_PHYS%FEVN(JLON,JLEV)*ZRHODREFM__(JLON,IKB)
    ENDDO
  ENDDO
ENDIF
IF (LSFORCS) THEN
  DO JLEV=1,KSGST+1
    DO JLON = KIDIA,KFDIA
      ...
      YDMF_PHYS%FCLL(JLON,JLEV)=-ZSFRV_(JLON)*ZRHODREFM__(JLON,IKB)* FOLH (ZTSURF(JLON),0._JPRB)*(1.0_JPRB-ZDELTA)
      YDMF_PHYS%FCLN(JLON,JLEV)=-ZSFRV_(JLON)*ZRHODREFM__(JLON,IKB)* FOLH (ZTSURF(JLON),0._JPRB)*ZDELTA
    ENDDO
  ENDDO
ENDIF  
...
DO JSG  = 1, KSGST+1
  DO JLEV = 0, KLEV
    DO JLON = KIDIA, KFDIA
      YDMF_PHYS%FRTH(JLON,JLEV,JSG)=YDMF_PHYS%FRTH(JLON,JLEV,JSG)+ZBUDTH_(JLON)
    ENDDO
  ENDDO
ENDDO
...
IF (LINTFLEX) THEN
  ...
  CALL APL_AROME2INTFLEX(YGFL,YDPARAR,YDPHY,KLON,KIDIA,KFDIA,KLEV, PDT,&
   ...
   & LLRAD, YDMF_PHYS%FRTH, YDMF_PHYS%FRSO,&! radiative fluxes
   & YDMF_PHYS%TENDU, YDMF_PHYS%TENDV, PTENDT,&! momentum and temperature tendencies
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,PAPRSM(:,KLEV),YDMF_PHYS%TCLS,&
  & YDMF_PHYS%QCLS,ZQCM(:,KLEV),ZQIM(:,KLEV),YDMF_PHYS%TPWCLS)
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%TPWCLS,YDMF_PHYS%DIAGH,PAPHIFM,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%FPLSL(:,KLEV),YDMF_PHYS%FPLSN(:,KLEV),YDMF_PHYS%FPLSG(:,KLEV),PDPRECIPS(:,NDTPRECCUR))
  ENDIF
  ...
  IF (LDPRECIPS2) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%TPWCLS,YDMF_PHYS%DIAGH,PAPHIFM,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%FPLSL(:,KLEV),YDMF_PHYS%FPLSN(:,KLEV),YDMF_PHYS%FPLSG(:,KLEV),PDPRECIPS2(:,NDTPRECCUR2))
