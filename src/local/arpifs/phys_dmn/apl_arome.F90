...
SUBROUTINE APL_AROME(YDGEOMETRY,YDMF_PHYS_STATE,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KIDIA , KFDIA , KLON ,&
 ...
 & PGEMU,PGELAM,POROG,PGM,PMU0,PMU0LU,PMU0M,PMU0N,PCLON, PSLON,&
 & PGP2DSDT, PGP2DSPP, &
 !---------------------------------------------------------------------
 ! - INPUT A M
 & PEDR,&
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   & YDMF_PHYS_STATE%YCPG_PHY%PRE  , ZEMIS  , YDMF_PHYS%RAD%EMTD  , PMU0, ZQVM,&
  ...
  DO JLON = KIDIA, KFDIA
    ...
    IF (PMU0(JLON) > 3.0E-02_JPRB) THEN
      YDMF_PHYS%OUT%FRSDNI(JLON)=MAX(0.0_JPRB,YDMF_PHYS%OUT%FRSOPS(JLON)/PMU0(JLON))
    ELSE
...
IF (LDPRECIPS.OR.LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,KLEV),YDMF_PHYS%OUT%FPLSN(:,KLEV),YDMF_PHYS%OUT%FPLSG(:,KLEV),PDPRECIPS(:,NDTPRECCUR))
  ENDIF
  ...
  IF (LDPRECIPS2) THEN
   ...
   CALL DPRECIPS(YDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTPW,ZQCM,YDMF_PHYS%OUT%FPLSL(:,KLEV),YDMF_PHYS%OUT%FPLSN(:,KLEV),YDMF_PHYS%OUT%FPLSG(:,KLEV),PDPRECIPS2(:,NDTPRECCUR2))
