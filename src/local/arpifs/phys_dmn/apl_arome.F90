...
IF (LLDIAB) THEN
  ...
  IF (LL_SAVE_PHSURF) THEN
    ...
    & YDMF_PHYS_TMP%SAV%FHPS, YDMF_PHYS_TMP%SAV%GZ0F, YDMF_PHYS_TMP%SAV%GZ0HF, YDMF_PHYS_TMP%SAV%HV,                    &
    & YDMF_PHYS_TMP%SAV%PBLH, YDMF_PHYS_TMP%SAV%QSH, YDMF_PHYS_TMP%SAV%UDAL, YDMF_PHYS_TMP%SAV%UDGRO,                   &
    & YDMF_PHYS_TMP%SAV%UDOM, YDMF_PHYS_TMP%SAV%UNEBH, YDMF_PHYS_SURF, YDVARS%DAL%T0, YDVARS%DOM%T0,                    &
...
ENDIF
 ...
 IF (LRAYFM) THEN
   ...
   IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN 
     ...
     & ZQO3, ZAER, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZEMIS, YDMF_PHYS_TMP%RDG%MU0M, ZQV, ZQSAT,                   &
...
IF (LLDIAB) THEN
  IF (LL_SAVE_PHSURF) THEN
    ...
    & YDMF_PHYS_TMP%SAV%FHPS, YDMF_PHYS_TMP%SAV%GZ0F, YDMF_PHYS_TMP%SAV%GZ0HF, YDMF_PHYS_TMP%SAV%HV,                    &
    & YDMF_PHYS_TMP%SAV%PBLH, YDMF_PHYS_TMP%SAV%QSH, YDMF_PHYS_TMP%SAV%UDAL, YDMF_PHYS_TMP%SAV%UDGRO,                   &
    & YDMF_PHYS_TMP%SAV%UDOM, YDMF_PHYS_TMP%SAV%UNEBH, YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL)
  ENDIF
ENDIF
