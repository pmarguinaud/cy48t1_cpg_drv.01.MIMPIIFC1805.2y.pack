...
REAL(KIND=JPRB)  :: ZSVM (YDCPG_DIM%KLON,1,1), ZPSV (YDCPG_DIM%KLON,1,1)
...
& YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL, YDRIP=>YDMODEL%YRML_GCONF%YRRIP, YDRCOEF=>YDMODEL%YRML_PHY_RAD%YRRCOEF,                   &
...
& YDPARAR=>YDMODEL%YRML_PHY_MF%YRPARAR, YDPRECIPS=>YDMODEL%YRML_PHY_MF%YRPHY%YRDPRECIPS, YDGEM=>YDGEOMETRY%YRGEM,                   &
& YDSTA=>YDGEOMETRY%YRSTA, YDERDI=>YDMODEL%YRML_PHY_RAD%YRERDI, YDMCC=>YDMODEL%YRML_AOC%YRMCC, YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD, &
& YDPHY3=>YDMODEL%YRML_PHY_MF%YRPHY3, YDPHY1=>YDMODEL%YRML_PHY_MF%YRPHY1, YDPHY0=>YDMODEL%YRML_PHY_MF%YRPHY0,                       &
& YDNORGWD=>YDMODEL%YRML_PHY_MF%YRNORGWD, YDMSE=>YDMODEL%YRML_PHY_MF%YRMSE)


ASSOCIATE(TSPHY=>YDPHY2%TSPHY, NTSSG=>YDDPHY%NTSSG, LMSE=>YDARPHY%LMSE, YI=>YGFL%YI, YL     =>YGFL%YL,                      &
& YG=>YGFL%YG, YQ=>YGFL%YQ, YR=>YGFL%YR, YSCONV=>YGFL%YSCONV, YS=>YGFL%YS, YEFB3=>YGFL%YEFB3, YEFB2=>YGFL%YEFB2,            &
& YEFB1=>YGFL%YEFB1, NGFL_EXT=>YGFL%NGFL_EXT, YTKE=>YGFL%YTKE, YLCONV=>YGFL%YLCONV, YRCONV=>YGFL%YRCONV,                    &
& YICONV=>YGFL%YICONV, YSP_SBD=>YDSURF%YSP_SBD, LNEBN=>YDPHY%LNEBN, NDPSFI=>YDPHY%NDPSFI, LRRGUST=>YDPHY%LRRGUST,           &
& LEDR=>YDPHY%LEDR, NTPLUI=>YDTOPH%NTPLUI, NDTPRECCUR=>YDPRECIPS%NDTPRECCUR, NDTPRECCUR2=>YDPRECIPS%NDTPRECCUR2,            &
& XMINLM=>YDPHY0%XMINLM, GRSO=>YDPHY0%GRSO, GAEPS=>YDPHY0%GAEPS, AERCS1=>YDPHY0%AERCS1, AERCS3=>YDPHY0%AERCS3,              &
& AERCS5=>YDPHY0%AERCS5, HUTIL2=>YDPHY0%HUTIL2, HUTIL1=>YDPHY0%HUTIL1, XMAXLM=>YDPHY0%XMAXLM, HUCOE=>YDPHY0%HUCOE,          &
& HUTIL=>YDPHY0%HUTIL, NPCLO1=>YDPHY0%NPCLO1, NPCLO2=>YDPHY0%NPCLO2, RDECRD=>YDPHY0%RDECRD, RDECRD1=>YDPHY0%RDECRD1,        &
& RDECRD2=>YDPHY0%RDECRD2, RDECRD3=>YDPHY0%RDECRD3, RDECRD4=>YDPHY0%RDECRD4, HSOLIWR=>YDPHY1%HSOLIWR,                       &
& ALCRIN=>YDPHY1%ALCRIN, ALBMED=>YDPHY1%ALBMED, WSMX=>YDPHY1%WSMX, HSOLIT0=>YDPHY1%HSOLIT0, HSOL=>YDPHY1%HSOL,              &
& WPMX=>YDPHY1%WPMX, EMCRIN=>YDPHY1%EMCRIN, EMMMER=>YDPHY1%EMMMER, TMERGL=>YDPHY1%TMERGL, EMMGLA=>YDPHY1%EMMGLA,            &
& LRAFTKE=>YDPHY2%LRAFTKE, HVCLS=>YDPHY2%HVCLS, HTCLS=>YDPHY2%HTCLS, FSM_HH=>YDPHY3%FSM_HH, FSM_GG=>YDPHY3%FSM_GG,          &
& FSM_FF=>YDPHY3%FSM_FF, FSM_EE=>YDPHY3%FSM_EE, FSM_II=>YDPHY3%FSM_II, FSM_CC=>YDPHY3%FSM_CC, FSM_DD=>YDPHY3%FSM_DD,        &
& RII0=>YDPHY3%RII0, QCO2=>YDPHY3%QCO2, NDLUNG=>YDDIM%NDLUNG, NDGUNG=>YDDIM%NDGUNG, NDLUXG=>YDDIM%NDLUXG,                   &
& NDGUXG=>YDDIM%NDGUXG, LMPA=>YDARPHY%LMPA, CCOUPLING=>YDARPHY%CCOUPLING, LSURFEX_KFROM=>YDARPHY%LSURFEX_KFROM,             &
& YA=>YGFL%YA, YIRAD=>YGFL%YIRAD, YLRAD=>YGFL%YLRAD, XZSEPS=>YDMSE%XZSEPS, LAEROSOO=>YDPHY%LAEROSOO,                        &
& LRKCDEV=>YDPHY%LRKCDEV, LHUCN=>YDPHY%LHUCN, LAEROLAN=>YDPHY%LAEROLAN, LAERODES=>YDPHY%LAERODES, LNODIFQC=>YDPHY%LNODIFQC, &
& NCALLRAD=>YDPHY%NCALLRAD, LO3ABC=>YDPHY%LO3ABC, LSTRAS=>YDPHY%LSTRAS, LAEROSEA=>YDPHY%LAEROSEA, NDIFFNEB=>YDPHY%NDIFFNEB, &
& LSOLV=>YDPHY%LSOLV, LRNUEXP=>YDPHY%LRNUEXP, LDAYD=>YDPHY%LDAYD, RDECLI=>YDRIP%RDECLI, XSW_BANDS=>YDPARAR%XSW_BANDS,       &
& NSWB_MNH=>YDPARAR%NSWB_MNH, NTNEBU=>YDTOPH%NTNEBU, NTDRME=>YDTOPH%NTDRME, NTCVIM=>YDTOPH%NTCVIM, NTCOET=>YDTOPH%NTCOET,   &
& NTDRAG=>YDTOPH%NTDRAG, RMESOQ=>YDTOPH%RMESOQ, RMESOT=>YDTOPH%RMESOT, RMESOU=>YDTOPH%RMESOU, NTQSAT=>YDTOPH%NTQSAT,        &
& NTCOEF=>YDTOPH%NTCOEF, NAER=>YDERAD%NAER, NOZOCL=>YDERAD%NOZOCL, NRADFR=>YDERAD%NRADFR, NSW=>YDERAD%NSW,                  &
& RCARDI=>YDERDI%RCARDI, RSUNDUR=>YDERDI%RSUNDUR, LMCC03=>YDMCC%LMCC03, NSTOP=>YDRIP%NSTOP, RCODEC=>YDRIP%RCODEC,           &
& RHGMT=>YDRIP%RHGMT, RSIDEC=>YDRIP%RSIDEC, RSOVR=>YDRIP%RSOVR, RSTATI=>YDRIP%RSTATI, TSTEP=>YDRIP%TSTEP,                   &
& STPRE=>YDSTA%STPRE, STPREH=>YDSTA%STPREH, STTEM=>YDSTA%STTEM, LGCHECKMV=>YDPHY%LGCHECKMV, NAERO=>YGFL%NAERO,              &
& RDELXN=>YDGEM%RDELXN, LFLASH =>YDCFU%LFLASH)
...
!       Computation of ZRTI
  ...
  IF (LRAFTKE) THEN
    ...
    & ZDCAPE, YDMF_PHYS_BASE_STATE%TKE, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDVARS%GEOMETRY%OROG%T0,               &
...
!     ------------------------------------------------------------------
    ...
    & ZDIFCVPPQ, ZDIFCVPPS, ZCONDCVPPL, ZCONDCVPPI, ZPRODTH_CVPP, INLAB_CVPP, ZQLI_CVPP, ZNEB_CVPP, INND)
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZFSDNV, ZCTRSO, ZCEMTR, ZTRSOD, ZTRSODIR, ZTRSODIF, ZPIZA_DST, ZCGA_DST, ZTAUREL_DST,                            &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        & ZFEV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZSFSV,                                                                                                                  &
...
! -----------------------------------------------------
  ...
  ENDIF
    ...
    & ZNEB_CVPP, ZQLI_CVPP, ZQC_DET_PCMT, ZTENHA, ZTENQVA, .TRUE., YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI,                    &
    & YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDVARS%GEOMETRY%GM%T0,                      &
    & ZVETAF, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL, &
...
!     -----------------------------------------------------------------------
  ...
  IF (LRAFTKE) THEN
     ...
     CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
     & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%CAPE,  &
     & ZDCAPE, ZTKE1, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDVARS%GEOMETRY%OROG%T0, YDMF_PHYS%OUT%UGST,              &
...
! eventuelles de W et de OMEGA/P
  ...
  & YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDH, ZTENDQ, ZTENDI, ZTENDL, ZTENDLCONV,                                        &
  & ZTENDICONV, ZTENDRCONV, ZTENDSCONV, ZTENDR, ZTENDS, ZTENDG, ZTENDTKE, ZTENDEFB1,                                        &
  & ZTENDEFB2, ZTENDEFB3, ZTENDEXT, YDDDH)
...
& ZTENDEFB2, ZTENDEFB3, ZTENDG, ZTENDICONV, ZTENDI, ZTENDLCONV, ZTENDL, ZTENDQ,       &
& ZTENDRCONV, ZTENDR, ZTENDSCONV, ZTENDS, ZTENDTKE, YDMF_PHYS%OUT%FDIS)
