...
REAL(KIND=JPRB) :: ZDIFEXT(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)     ! Extra-GFL fluxes.
    ...
    ! U tendency without deep convection contribution
    ! V tendency without deep convection contribution
...
!DISSIPATION TIME SCALE TAU  -FOR TOM's CALCULATION
   !  Conversion function lm-L
   !  Function in computation of tte_tilde
...
REAL(KIND=JPRB) :: ZUNEBH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)   !Nebulosite convective histo
   !fraction of instantaneous detrained air
...
& YDPRECIPS=>YDMODEL%YRML_PHY_MF%YRPHY%YRDPRECIPS, YDGEM=>YDGEOMETRY%YRGEM, YDSTA=>YDGEOMETRY%YRSTA,                              &
& YDERDI=>YDMODEL%YRML_PHY_RAD%YRERDI, YDMCC=>YDMODEL%YRML_AOC%YRMCC, YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD,                        &
& YDPHY3=>YDMODEL%YRML_PHY_MF%YRPHY3, YDPHY1=>YDMODEL%YRML_PHY_MF%YRPHY1, YDPHY0=>YDMODEL%YRML_PHY_MF%YRPHY0,                     &
& YDMSE=>YDMODEL%YRML_PHY_MF%YRMSE)


ASSOCIATE(TSPHY=>YDPHY2%TSPHY, NTSSG=>YDDPHY%NTSSG, LMSE=>YDARPHY%LMSE, YSP_SBD=>YDSURF%YSP_SBD, LNEBN=>YDPHY%LNEBN,            &
...
& LRAFTKE=>YDPHY2%LRAFTKE, HVCLS=>YDPHY2%HVCLS, HTCLS=>YDPHY2%HTCLS, RII0=>YDPHY3%RII0, QCO2=>YDPHY3%QCO2,                      &
& NDLUNG=>YDDIM%NDLUNG, NDGUNG=>YDDIM%NDGUNG, NDLUXG=>YDDIM%NDLUXG, NDGUXG=>YDDIM%NDGUXG, LSURFEX_KFROM=>YDARPHY%LSURFEX_KFROM, &
& YA=>YGFL%YA, YIRAD=>YGFL%YIRAD, YLRAD=>YGFL%YLRAD, LHUCN=>YDPHY%LHUCN, LO3ABC=>YDPHY%LO3ABC, LSTRAS=>YDPHY%LSTRAS,            &
& LSOLV=>YDPHY%LSOLV, LRNUEXP=>YDPHY%LRNUEXP, RDECLI=>YDRIP%RDECLI, NSWB_MNH=>YDPARAR%NSWB_MNH, NTDRME=>YDTOPH%NTDRME,          &
& NTDRAG=>YDTOPH%NTDRAG, RMESOQ=>YDTOPH%RMESOQ, RMESOT=>YDTOPH%RMESOT, RMESOU=>YDTOPH%RMESOU, NTQSAT=>YDTOPH%NTQSAT,            &
& NOZOCL=>YDERAD%NOZOCL, NRADFR=>YDERAD%NRADFR, NSW=>YDERAD%NSW, LMCC03=>YDMCC%LMCC03, RHGMT=>YDRIP%RHGMT,                      &
& RSTATI=>YDRIP%RSTATI, STPRE=>YDSTA%STPRE, STPREH=>YDSTA%STPREH, STTEM=>YDSTA%STTEM, LGCHECKMV=>YDPHY%LGCHECKMV)
...
CALL APL_ARPEGE_INIT (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, IMOC_CLPH, &
& INLAB_CVPP, ZAER, ZAERINDS, ZAIPCMT, ZALBD, ZALBP, ZALPHA1, ZCEMTR, ZCFATH, ZCFAU, ZCFBTH, ZCFBU, ZCFBV, ZCOEFA, &
& ZCONDCVPPI, ZCONDCVPPL, ZCTRSO, ZDECRD, ZDIFCVPPQ, ZDIFCVPPS, ZDIFCVPPU, ZDIFCVPPV, ZDIFWQ, ZDIFWS, ZEDMFQ, ZEDMFS, &
& ZEDMFU, ZEDMFV, ZEPS0, ZEPSNEB, ZFPCOR, ZKQLROV, ZKQROV, ZKTROV, ZKUROV, ZLVT, ZMF_UP, ZMRIPP, ZNEBC0, ZNEBCH, &
& ZNEBDIFF, ZNEBS, ZNEBS0, ZNEB_CVPP, ZPFL_FPLCH, ZPFL_FPLSH, ZPOID, ZPRODTH_CVPP, ZQC_DET_PCMT, ZQI, ZQIC, ZQICE, &
...
CALL APL_ARPEGE_OCEANIC_FLUXES (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, &
...
CALL APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_MISC,      &
...
CALL APL_ARPEGE_ALBEDO_COMPUTATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, &
& YDMODEL, ZALBD, ZALBP, ZEPS0, ZFLU_EMIS, ZFLU_NEIJ, ZRDG_MU0)

CALL APL_ARPEGE_AEROSOLS_FOR_RADIATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDMF_PHYS_SURF, YDMODEL, &
& ZAER, ZAERINDS)

CALL APL_ARPEGE_CLOUDINESS (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_MISC, YDMF_PHYS, YDVARS, YDMODEL,  &
...
CALL APL_ARPEGE_SOIL_HYDRO (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, &
...
CALL APL_ARPEGE_SURFACE (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_GPAR,         &
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL, LDCONFX, ZALBD, ZALBP, ZALPHA1, ZCDROV, ZCEROV, &
...
CALL APL_ARPEGE_PRECIPITATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, &
...
CALL APL_ARPEGE_HYDRO_BUDGET (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, &
& YDSURF, YDMODEL, ZC3, ZCN, ZDSA_C1, ZDSA_C2, ZFLU_FEVI, ZFLU_NEIJ, ZFLU_VEG, ZFPLSL, ZFPLSN, ZWFC,  &
...
CALL APL_ARPEGE_ATMOSPHERE_UPDATE (YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, &
& YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL, LDCONFX, PDTPHY, YDDDH, ZDIFEXT, ZDSA_CPS, ZMSC_FRMQ, ZPFL_FEFB1, ZPFL_FEFB2, ZPFL_FEFB3, &
& ZPFL_FTKE, ZPFL_FTKEI, ZRDG_CVGQ, ZRDG_LCVQ, ZRDG_MU0)
...
CALL APL_ARPEGE_SURFACE_UPDATE (YDCPG_DIM, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDSURF, YDMODEL, &
