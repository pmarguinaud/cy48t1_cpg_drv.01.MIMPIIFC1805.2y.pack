...
INTEGER(KIND=JPIM) :: INLAB_CVPP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB) :: ZMRIPP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB) :: ZCDROV(YDCPG_DIM%KLON),ZCHROV(YDCPG_DIM%KLON),ZDQSTS(YDCPG_DIM%KLON),ZGWDCS(YDCPG_DIM%KLON),&
 & ZHQ(YDCPG_DIM%KLON),ZHU(YDCPG_DIM%KLON),ZHTR(YDCPG_DIM%KLON),&
 & ZRTI(YDCPG_DIM%KLON),ZDPHI(YDCPG_DIM%KLON),ZPRS(YDCPG_DIM%KLON),ZSTAB(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB) :: ZHUC(YDCPG_DIM%KFLEVG), ZBLH(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB) :: ZSUDU(YDCPG_DIM%KLON) 
...
REAL(KIND=JPRB)    :: ZQIC(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG),ZQLC(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
CALL APLPAR_INIT (YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, NTSSG, YDCPG_DIM%YRSURF%YSP_SBD%NLEVS, &
...
!       Computation of ZRTI
  ...
  ENDDO      
ELSE      
  CALL ACHMT ( YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDCPG_DIM%YRSURF%YSD_VVD%NUMFLDS>=8.AND.LSOLV,&
