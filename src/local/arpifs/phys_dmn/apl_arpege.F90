...
ASSOCIATE( &
 PAPRSF=> YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, &
 PAPRS => YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
 PAPHIF=> YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
 PAPHI => YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
 PDELP => YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
 PR    => YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
 PT    => YDMF_PHYS_BASE_STATE%T, &
 PU    => YDMF_PHYS_BASE_STATE%U, &
 PV    => YDMF_PHYS_BASE_STATE%V, &
 PCP   => YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, &
 PTKE  => YDMF_PHYS_BASE_STATE%TKE, &
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,  &
              & YDCPG_DIM%KSTEP, PAPHI, PAPHIF,                &
              & PAPRS, PAPRSF, YDVARS%GEOMETRY%GELAM%T0, &
              & YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, PT,                 &
...
!*        1.1 INITIALISATION DE L'OZONE
  ...
  ELSEIF (NOZOCL == 1) THEN
    IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, PAPRS, &
  ...
  ELSE
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, ZQO3, .FALSE., PAPRS, &
...
IF (LMSE) THEN      
  ...
  & PAPHI, PAPHIF, PAPRS, &
  & PAPRSF, PR, PT,          &
  & PU, PV, YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDCPG_MISC%QS,               &
...
& YDCPG_DIM%KFLEVG, ZTHETAV, PAPHI, PAPHIF,            &
& PU, PV, ZTHETAVS, IMOC_CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%VEIN, &
& ZUGST, ZVGST)
ZBLH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS%OUT%CLPH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      
    

! BEGIN OF OCEANIC FLUXES COMPUTATION (IF NOT SURFEX)  
IF ((.NOT.LMSE)) THEN
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  & PAPHI, PAPHIF, PAPRS,   &
  & PAPRSF, YDMF_PHYS_BASE_STATE%Q, PT, PU, &
  & PV, ZDPHIT, ZDPHIV, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_SURF%GSD_VF%PLSM,                         &
...
ELSE
  ...
  ENDDO
ENDIF
! END OF OCEANIC FLUXES COMPUTATION (IF NOT SURFEX)  
...
IF (LRAFTKE) THEN
  ...
  & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, PU, PV, YDMF_PHYS%OUT%CAPE,  &
  & ZDCAPE, PTKE, PAPHIF, YDVARS%GEOMETRY%OROG%T0,               &
...
& PAPRSF, PAPHIF, PDELP,  &
& PR, PT, ZQV, ZQL, ZQI, PU,                 &
& PV, PVERVEL (:, 1:), PCP, PTKE, &
...
& YDCPG_DIM%KLON, NTCOEF, NTCOET, YDCPG_DIM%KFLEVG, PAPHI, PAPHIF, &
& PAPRS, PAPRSF, PDELP,     &
& PR, PT, PU, PV,             &
& ZQV, ZQIC, ZQLC, ZMSC_LSCPE, ZFLU_CD, ZFLU_CH, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_BASE_STATE%YGSP_RR%T,                        &
& YDCPG_MISC%QS, YDCPG_MISC%QICE, YDCPG_MISC%QLI, PTKE, ZPRODTH_CVPP, ZNLAB,                           &
...
IF (((MOD(YDCPG_DIM%KSTEP,NRADFR) == 0))) THEN
  ...
  IF ( ( (NAER /= 0)).AND.LLAERO )  THEN
    ...
    & YDCPG_DIM%KFLEVG, PAPRS, PAPRSF,             &
    & PT, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZAESEA, ZAELAN, ZAESOO, ZAEDES,                    &
...
& NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, PAPHI, PAPHIF, &
& PAPRSF, PCP, PR,       &
& PDELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, ZMSC_QW, PT,            &
...
IF(LNEBN) THEN
  ...
  & PAPHIF, PCP, ZQV, ZQL, ZQI,                                        &
  & ZFLU_QSAT, PT, ZPFL_FPLCH, PDELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
  & PAPRSF, ZUNEBH, ZNEBS0, ZQLIS0, ZQLI_CVP, ZNEB_CVPP, ZQLI_CVPP,                                  &
...
IF (LLCALLRAD) THEN
  ...
  & YDCPG_DIM%KSW, ZALBD, ZALBP, PAPRS, PAPRSF,        &
  & YDCPG_MISC%NEB, ZQO3, ZAER, PDELP, ZFLU_EMIS, ZRDG_MU0M,                        &
  & ZQV, ZFLU_QSAT, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQS, ZQR, YDMF_PHYS_SURF%GSD_VF%PLSM, PT,   &
...
& YDCPG_DIM%KFLEVG, PAPRS, ZFLU_EMIS, YDMF_PHYS%RAD%EMTD,              &
...
IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,           &
          & YDCPG_DIM%KTDIA, NTSSG, YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, PAPHIF, &
          & PDELP, YDMF_PHYS%OUT%FRSO)
...
IF (LSOLV.AND.(.NOT.LMSE)) THEN
  ...
  & ZQV, ZFLU_QSAT, PT, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VF%PLSM,                    &
...
CALL ACDIFV1 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA,                   &
  & YDCPG_DIM%KFLEVG, PAPHIF, PAPRSF,                                 &
  & ZCP, PDELP, ZKTROV, ZKQROV, ZKUROV, ZQV, ZQL, ZQI, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
  & PT, PU, PV, ZSVM, ZXTROV, ZXUROV,                                &
...
IF ( LMSE.AND.LCALLSFX ) THEN
  ...
  CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1, YDCPG_DIM%KIDIA,                                                           &
      ...
      & PV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                          &
      & PT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                          &
      & YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                          &
      & ZSVM, ZCARDI, ZRHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), PAPRSF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), &
      & PAPRS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                            &
      ...
      & ZFEV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZSFSV, ZSFCO2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZFMDU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                       &
      & ZFMDV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZALBP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:NSW), ZALBD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:NSW),                                &
      & ZFLU_EMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZTSN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS%OUT%FRTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, 1))               !orographic shadowing
...
! calculation of variables for the old "ISBA" atmosphere scheme
  ...
  IF (.NOT.LNODIFQC) THEN
    ...
    & YDCPG_DIM%KFLEVG, PAPRS, ZCOEFN, PCP, &
    & ZQL, ZQI, PT, ZALPHA1, ZCOEFA, ZLVT, ZQICE)
  ENDIF
...
ELSEIF (.NOT.LMSE) THEN
  ...
  CALL ARP_GROUND_PARAM ( YDMCC, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,        &
    & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, PAPHI, ZCP,                  &
    & YDMF_PHYS%OUT%FRSO, ZQV, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, PT,                  &
    & PU, PV, ZXURO, ZXQRO, ZXTRO, PAPRS, &
...
CALL ACDIFV2 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, &
  & YDCPG_DIM%KFLEVG, PAPRS, ZCFAQ, ZCFAS, ZCFAU, ZCFASV, ZCFBQ,                &
  & ZCFBS, ZCFBU, ZCFBV, ZCFBSV, ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZDSE, ZQT, PU,           &
  & PV, ZPOID, PT, ZQL, ZQI, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,  &
...
IF ( LSTRAS ) THEN
  ...
  & PAPRSF, PDELP, ZNEBS, ZQV,                    &
  & ZQLIS, ZMSC_QW, PR, PT, YDMF_PHYS%OUT%FCSQL,             &
...
& PT, YDMF_PHYS_BASE_STATE%Q, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZTENHA,                     &
& ZTENQVA )
...
& LLDSLPHY, TSPHY, ZVDIFTS, PT, ZQV, PU, PV,              &
& ZLISUM, ZVERVEL, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS, PAPRS,                    &
& PAPRSF, PAPRS, ZGEOM1, ZGEOMH, YDVARS%GEOMETRY%GM%T0, &
...
IF(LFLASH) THEN
  ...
  & ZGAW, ZGAW, PAPRSF, PAPRS, PAPHI, &
  & PAPHIF, LLLAND, PT, ZLU, ZMFU, YDMF_PHYS%OUT%CAPE,                          &
...
& YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, PT,    &
...
& ZTMIC, ZQMIC, ZQL, ZQI, ZQR, ZQS, PDELP, PAPRSF,   &
& PCP, PR, ZNEBS, ZQLIS,                           &
& ZNEB_CVPP, ZQLI_CVPP, ZQC_DET_PCMT, ZTENHA, ZTENQVA, .TRUE., PAPHI,                    &
...
& PCP, PDELP, PT,    &
& ZQV, PU, PV, YDMF_PHYS%OUT%FRMH, ZMSC_FRMQ, YDMF_PHYS%OUT%STRMU, &
...
IF (LRAFTKE) THEN
  ...
  & YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, PAPRS,   &
  & PAPRSF, PT, PCP,   &
  & PAPHIF, PAPHI, ZDCAPE)
     ! Gusts.
  ZQGM=ZEPSNEB
  CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
  & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, PU, PV, YDMF_PHYS%OUT%CAPE,  &
  & ZDCAPE, ZTKE1, PAPHIF, YDVARS%GEOMETRY%OROG%T0, YDMF_PHYS%OUT%UGST,              &
...
& YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, PAPHI, PAPHIF, &
& PAPRSF, PT, PR,       &
...
DO JLEV=1,YDCPG_DIM%KFLEVG
  CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, PAPRSF(:, JLEV), &
  & PT(:, JLEV), YDMF_PHYS_BASE_STATE%Q(:, JLEV), YDMF_PHYS_BASE_STATE%L(:, JLEV),                     &
...
& YDVARS%GEOMETRY%OROG%T0, YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, PAPHIF,                 &
...
& ZPFL_FEFB1, ZPFL_FEFB2, ZPFL_FEFB3, PDELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,    &
& PAPHIF, PCP, PU,                       &
& PV, PT, YDMF_PHYS_BASE_STATE%Q, YDMF_PHYS_BASE_STATE%I, YDMF_PHYS_BASE_STATE%L, &
...
IF (NDPSFI == 1) THEN
  ...
  & ZPFL_FP, PAPRS(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, &
