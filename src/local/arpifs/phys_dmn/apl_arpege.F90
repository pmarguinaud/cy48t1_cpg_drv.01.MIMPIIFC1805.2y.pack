SUBROUTINE APL_ARPEGE(YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, &
...
USE YOMCT0             , ONLY : LAROME, LCORWAT
...
ASSOCIATE(TSPHY=>YDPHY2%TSPHY, NTSSG=>YDDPHY%NTSSG, LMSE=>YDARPHY%LMSE, LNEBN=>YDPHY%LNEBN,       &
& NDPSFI=>YDPHY%NDPSFI, LRRGUST=>YDPHY%LRRGUST, LEDR=>YDPHY%LEDR, NTPLUI=>YDTOPH%NTPLUI, NDTPRECCUR=>YDPRECIPS%NDTPRECCUR, &
& NDTPRECCUR2=>YDPRECIPS%NDTPRECCUR2, XMINLM=>YDPHY0%XMINLM, HUTIL2=>YDPHY0%HUTIL2, HUTIL1=>YDPHY0%HUTIL1,                 &
& XMAXLM=>YDPHY0%XMAXLM, HUCOE=>YDPHY0%HUCOE, HUTIL=>YDPHY0%HUTIL, NPCLO1=>YDPHY0%NPCLO1, NPCLO2=>YDPHY0%NPCLO2,           &
...
& YIRAD=>YGFL%YIRAD, YLRAD=>YGFL%YLRAD, LHUCN=>YDPHY%LHUCN, LSTRAS=>YDPHY%LSTRAS, LSOLV=>YDPHY%LSOLV,                      &
...
CALL APLPAR_INIT (YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, NTSSG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, &
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
              & YDCPG_OPTS%KSTEP, PAPHI, PAPHIF, PAPRS, PAPRSF, YDVARS%GEOMETRY%GELAM%T0, YDVARS%GEOMETRY%GEMU%T0,     &
...
CALL APL_ARPEGE_INIT (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, &
& YDMODEL, IMOC_CLPH, INLAB_CVPP, ZAER, ZAERINDS, ZAIPCMT, ZALBD, ZALBP, ZALPHA1, ZCEMTR, ZCFATH,     &
& ZCFAU, ZCFBTH, ZCFBU, ZCFBV, ZCOEFA, ZCONDCVPPI, ZCONDCVPPL, ZCTRSO, ZDECRD, ZDIFCVPPQ, ZDIFCVPPS,  &
& ZDIFCVPPU, ZDIFCVPPV, ZDIFWQ, ZDIFWS, ZEDMFQ, ZEDMFS, ZEDMFU, ZEDMFV, ZEPS0, ZEPSNEB, ZFPCOR,       &
& ZKQLROV, ZKQROV, ZKTROV, ZKUROV, ZLVT, ZMF_UP, ZMRIPP, ZNEBC0, ZNEBCH, ZNEBDIFF, ZNEBS, ZNEBS0,     &
& ZNEB_CVPP, ZPFL_FPLCH, ZPFL_FPLSH, ZPOID, ZPRODTH_CVPP, ZQC_DET_PCMT, ZQI, ZQIC, ZQICE, ZQL, ZQLC,  &
& ZQLIS, ZQLIS0, ZQLI_CVP, ZQLI_CVPP, ZQO3, ZQR, ZQS, ZQV, ZSC_FCLL, ZSC_FCLN, ZSC_FEVI, ZSC_FEVN,    &
...
CALL APL_ARPEGE_INIT_SURFEX (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR, &
...
!       Computation of ZRTI
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ...
    ZRTI(JLON)=2.0_JPRB/(PR(JLON,YDCPG_OPTS%KFLEVG)*PT(JLON,YDCPG_OPTS%KFLEVG)+RKAPPA*ZDPHI(JLON)&
    & +ZPRS(JLON)*YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON))
  ENDDO      
ELSE      
  CALL ACHMT ( YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%YRSURF_DIMS%YSD_VVD%NUMFLDS>=8.AND.LSOLV,&
...
ZCP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG) = PCP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)
...
DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  & *(RATM/PAPRS(JLON,YDCPG_OPTS%KFLEVG))**RKAPPA  
ENDDO

CALL ACCLPH ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, &
...
CALL APL_ARPEGE_OCEANIC_FLUXES (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, &
& LLHMT, ZCDROV, ZCEROV, ZCHROV, ZDPHIT, ZDPHIV, ZDSA_RS, ZFLU_CD, ZFLU_CDN, ZFLU_CH, ZFLU_QSATS)


CALL APL_WIND_GUST (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDVARS, YDXFU, YDMODEL, IMOC_CLPH, &
& ZBLH, ZDCAPE)


CALL APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC,      &
& YDMF_PHYS, YDCPG_DYN0, YDMODEL, YDDDH, INLAB_CVPP, ZCDROV, ZCHROV, ZCOEFN, ZCONDCVPPI, ZCONDCVPPL, &
& ZDIFCVPPQ, ZDIFCVPPS, ZEPS, ZFLU_CD, ZFLU_CH, ZKQLROV, ZKQROV, ZKTROV, ZKUROV, ZMSC_LSCPE, ZNBVNO, &
& ZNEBS, ZNEBS0, ZNEB_CVPP, ZPFL_FPLCH, ZPFL_FTKE, ZPFL_FTKEI, ZPRODTH_CVPP, ZQI, ZQIC, ZQL, ZQLC,   &
& ZQLIS, ZQLIS0, ZQLI_CVPP, ZQV, ZTKE1, ZXTROV, ZXUROV)
...
CALL APL_ARPEGE_ALBEDO_COMPUTATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, &
& YDMODEL, ZALBD, ZALBP, ZEPS0, ZFLU_EMIS, ZFLU_NEIJ, ZRDG_MU0)

CALL APL_ARPEGE_AEROSOLS_FOR_RADIATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS_SURF, YDMODEL, &
& ZAER, ZAERINDS)

CALL APL_ARPEGE_CLOUDINESS (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDVARS, YDMODEL,  &
& LLREDPR, ZAIPCMT, ZBLH, ZDECRD, ZFLU_QSAT, ZHUC, ZMSC_QW, ZNEBC0, ZNEBCH, ZNEBS, ZNEBS0, ZNEB_CVPP, &
& ZPFL_FPLCH, ZQI, ZQL, ZQLIS, ZQLIS0, ZQLI_CVP, ZQLI_CVPP, ZQV, ZUNEBH, ZVETAF)
  
CALL APL_ARPEGE_RADIATION (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR,   &
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, ZAER, ZAERINDS, ZALBD, ZALBP, ZCEMTR, ZCTRSO,       &
& ZFLU_EMIS, ZFLU_QSAT, ZQO3, ZQR, ZQS, ZQV, ZRDG_MU0, ZRDG_MU0LU, ZRDG_MU0M, ZSFSWDIF, ZSFSWDIR, &
& ZSUDU, ZTENT, ZTRSOD, ZTRSODIF, ZTRSODIR)
...
CALL APL_ARPEGE_SOIL_HYDRO (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, &
& ZCHROV, ZFLU_NEIJ, ZFLU_QSAT, ZFLU_QSATS, ZFLU_VEG, ZGWDCS, ZHQ, ZHTR, ZHU, ZQV, ZWFC, ZWLMX,  &
& ZWWILT)
...
CALL APL_ARPEGE_SURFACE (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR,         &
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, LDCONFX, ZALBD, ZALBP, ZALPHA1, ZCDROV, ZCEROV, &
...
CALL APL_ARPEGE_DEEP_CONVECTION (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDCPG_DYN0, &
& YDMF_PHYS_SURF, YDVARS, YDCFU, YDMODEL, ZFPCOR, ZQV, ZTENHA, ZTENQVA, ZTENT)

CALL APL_ARPEGE_PRECIPITATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, &
& YDMODEL, ZFLU_NEIJ, ZNEBS, ZNEB_CVPP, ZQC_DET_PCMT, ZQI, ZQL, ZQLIS, ZQLI_CVPP, ZQR, ZQS, ZQV,   &
& ZSEDIQI, ZSEDIQL, ZTENHA, ZTENQVA, ZVETAF)
...
CALL QNGCOR (YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG, ZQV,                 &
...
CALL APL_ARPEGE_HYDRO_BUDGET (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, &
& YDMODEL, ZC3, ZCN, ZDSA_C1, ZDSA_C2, ZFLU_FEVI, ZFLU_NEIJ, ZFLU_VEG, ZFPLSL, ZFPLSN, ZWFC,  &
& ZWLMX, ZWPMX, ZWSEQ, ZWSMX)
...
! 
! MESOSPHERIC DRAG  
CALL ACDRME ( YDMODEL%YRCST, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,               &
...
CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,         &
& YDCPG_OPTS%KFLEVG, PAPHI, PAPHIF, PAPRSF, PT, PR, YDCPG_MISC%QLI, YDCPG_MISC%QICE, ZQR, ZQS, ZQGM, YDMF_PHYS%OUT%VISICLD, &
...
DO JLEV=1,YDCPG_OPTS%KFLEVG
  CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, PAPRSF(:, JLEV), PT(:, JLEV), &
...
& ZFPLC(:, YDCPG_OPTS%KFLEVG), ZFPLS(:, YDCPG_OPTS%KFLEVG), ZFPLSG(:, YDCPG_OPTS%KFLEVG), ZPRC_DPRECIPS(:, NDTPRECCUR)&
&           )
  
 !Idem for an other time step and an other period

CALL DPRECIPS(YDPHY%YRDPRECIPS, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDVARS%GEOMETRY%OROG%T0, &
& YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, PAPHIF, ZDZZ, ZTW, YDMF_PHYS_BASE_STATE%L, ZFPLC(:, YDCPG_OPTS%KFLEVG),          &
& ZFPLS(:, YDCPG_OPTS%KFLEVG), ZFPLSG(:, YDCPG_OPTS%KFLEVG), ZPRC_DPRECIPS2(:, NDTPRECCUR2))
...
CALL APL_ARPEGE_ATMOSPHERE_UPDATE (YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, &
& YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL, LDCONFX, PDTPHY, YDDDH, &
...
CALL APL_ARPEGE_SURFACE_UPDATE (YDCPG_DIM, YDCPG_OPTS, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDSURF, YDMODEL, &
& LDCONFX, PDTPHY, ZDSA_C1, ZDSA_C2, ZFLU_FEVI, ZFLU_VEG)
...
IF(LNUDG)THEN
  ...
  & YDMF_PHYS_SURF%GSP_SG%PF_T1, YDMF_PHYS_NEXT_STATE%T (:, 1:YDCPG_OPTS%KFLEVG), YDMF_PHYS_NEXT_STATE%Q (:, 1:YDCPG_OPTS%KFLEVG), &
  & YDMF_PHYS_NEXT_STATE%U (:, 1:YDCPG_OPTS%KFLEVG), YDMF_PHYS_NEXT_STATE%V (:, 1:YDCPG_OPTS%KFLEVG), YDMF_PHYS_NEXT_STATE%SP,     &
  & YDVARS%T%T0, YDVARS%Q%T0, YDVARS%U%T0, YDVARS%V%T0, YDCPG_PHY0%PREHYD(:, YDCPG_OPTS%KFLEVG), YDVARS%GEOMETRY%GM%T0,           &
...
CALL MF_PHYS_PRECIPS (YDCPG_DIM, YDCPG_OPTS, ZPRC_DPRECIPS, ZPRC_DPRECIPS2, YDMF_PHYS_SURF%GSD_XP%PPRECIP, YDMF_PHYS_SURF%GSD_XP2%PPRECIP2, &
& YDMODEL)
