...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM, YDVAB=>YDGEOMETRY%YRVAB, YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY,              &
& YDTOPH=>YDMODEL%YRML_PHY_MF%YRTOPH,                   &
& YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL, YDRIP=>YDMODEL%YRML_GCONF%YRRIP, &
...
ASSOCIATE(TSPHY=>YDPHY2%TSPHY, NTSSG=>YDDPHY%NTSSG, &
& LMSE=>YDARPHY%LMSE, YI=>YGFL%YI, YL     =>YGFL%YL, YG=>YGFL%YG,                      &
& YQ=>YGFL%YQ, YR=>YGFL%YR, YSCONV=>YGFL%YSCONV, YS=>YGFL%YS, YEFB3=>YGFL%YEFB3, YEFB2=>YGFL%YEFB2, YEFB1=>YGFL%YEFB1,         &
& NGFL_EXT=>YGFL%NGFL_EXT, YTKE=>YGFL%YTKE, YLCONV=>YGFL%YLCONV,        &
& YRCONV=>YGFL%YRCONV, YICONV=>YGFL%YICONV, YSP_SBD=>YDSURF%YSP_SBD, &
& LNEBN=>YDPHY%LNEBN, NDPSFI=>YDPHY%NDPSFI, &
& LRRGUST=>YDPHY%LRRGUST,                  &
& LEDR=>YDPHY%LEDR, NTPLUI=>YDTOPH%NTPLUI, &
& NDTPRECCUR=>YDPRECIPS%NDTPRECCUR, NDTPRECCUR2=>YDPRECIPS%NDTPRECCUR2, &
& XMINLM=>YDPHY0%XMINLM, GRSO=>YDPHY0%GRSO, &
& GAEPS=>YDPHY0%GAEPS, AERCS1=>YDPHY0%AERCS1, AERCS3=>YDPHY0%AERCS3, AERCS5=>YDPHY0%AERCS5, HUTIL2=>YDPHY0%HUTIL2,             &
& HUTIL1=>YDPHY0%HUTIL1, XMAXLM=>YDPHY0%XMAXLM, HUCOE=>YDPHY0%HUCOE, &
...
& HSOLIWR=>YDPHY1%HSOLIWR, ALCRIN=>YDPHY1%ALCRIN, ALBMED=>YDPHY1%ALBMED, WSMX=>YDPHY1%WSMX,         &
& HSOLIT0=>YDPHY1%HSOLIT0, HSOL=>YDPHY1%HSOL, WPMX=>YDPHY1%WPMX, EMCRIN=>YDPHY1%EMCRIN,       &
& EMMMER=>YDPHY1%EMMMER, TMERGL=>YDPHY1%TMERGL, EMMGLA=>YDPHY1%EMMGLA, LRAFTKE=>YDPHY2%LRAFTKE, &
& HVCLS=>YDPHY2%HVCLS, HTCLS=>YDPHY2%HTCLS, FSM_HH=>YDPHY3%FSM_HH, FSM_GG=>YDPHY3%FSM_GG, FSM_FF=>YDPHY3%FSM_FF,               &
& FSM_EE=>YDPHY3%FSM_EE, FSM_II=>YDPHY3%FSM_II, FSM_CC=>YDPHY3%FSM_CC, FSM_DD=>YDPHY3%FSM_DD, &
& RII0=>YDPHY3%RII0, QCO2=>YDPHY3%QCO2, NDLUNG=>YDDIM%NDLUNG, NDGUNG=>YDDIM%NDGUNG,           &
& NDLUXG=>YDDIM%NDLUXG, NDGUXG=>YDDIM%NDGUXG, LMPA=>YDARPHY%LMPA, CCOUPLING=>YDARPHY%CCOUPLING,      &
& LSURFEX_KFROM=>YDARPHY%LSURFEX_KFROM, YA=>YGFL%YA, &
& YIRAD=>YGFL%YIRAD, YLRAD=>YGFL%YLRAD, XZSEPS=>YDMSE%XZSEPS, &
& LAEROSOO=>YDPHY%LAEROSOO,                          &
& LRKCDEV=>YDPHY%LRKCDEV, LHUCN=>YDPHY%LHUCN, &
& LAEROLAN=>YDPHY%LAEROLAN, LAERODES=>YDPHY%LAERODES, &
& LNODIFQC=>YDPHY%LNODIFQC, &
& NCALLRAD=>YDPHY%NCALLRAD, LO3ABC=>YDPHY%LO3ABC,      &
& LSTRAS=>YDPHY%LSTRAS, LAEROSEA=>YDPHY%LAEROSEA,                        &
& NDIFFNEB=>YDPHY%NDIFFNEB, &
& LSOLV=>YDPHY%LSOLV, LRNUEXP=>YDPHY%LRNUEXP,        &
& LDAYD=>YDPHY%LDAYD, &
& RDECLI=>YDRIP%RDECLI, XSW_BANDS=>YDPARAR%XSW_BANDS, NSWB_MNH=>YDPARAR%NSWB_MNH,                &
& NTNEBU=>YDTOPH%NTNEBU, &
...
& LMCC03=>YDMCC%LMCC03, NSTOP=>YDRIP%NSTOP, RCODEC=>YDRIP%RCODEC,                &
& RHGMT=>YDRIP%RHGMT, RSIDEC=>YDRIP%RSIDEC, RSOVR=>YDRIP%RSOVR, RSTATI=>YDRIP%RSTATI, TSTEP=>YDRIP%TSTEP,                      &
& STPRE=>YDSTA%STPRE, STPREH=>YDSTA%STPREH,            &
& STTEM=>YDSTA%STTEM, LGCHECKMV=>YDPHY%LGCHECKMV, NAERO=>YGFL%NAERO,              &
& RDELXN=>YDGEM%RDELXN,                     &
& LFLASH =>YDCFU%LFLASH)
