...
IF (.NOT.LDCONFX) THEN
  IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
    CALL CPCHET (YDMF_PHYS, YDMF_PHYS_BASE_STATE, YDCPG_MISC, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF%YRPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, &
    & YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, PMSC_FRMQ, PDSA_CPS, ZTENDH, ZTENDQ,               &
  ...
  IF (GCHETN%LPROFV)&
   & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, PRDG_CVGQ, PRDG_LCVQ, PRDG_MU0, YDCPG_DYN0,                  &
     & YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL%YRML_PHY_G%YRDPHY, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KFDIA, YDVARS%GEOMETRY%GELAM%T0, &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%NDPSFI == 1) THEN
  CALL CPMVVPS(YDGEOMETRY%YRVAB, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, PDTPHY,     &
  & ZPFL_FP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDCPG_DYN0%CTY%EVEL, &
...
CALL CPUTQY_APLPAR_EXPL(YDCPG_DIM, YDMF_PHYS_NEXT_STATE, YDMF_PHYS_BASE_STATE, YDVARS, YDMODEL%YRML_PHY_MF%YRPHY, PDTPHY, &
