...
& YDDDH, PDIFEXT, PDSA_CPS, PMSC_FRMQ, PPFL_FEFB1, PPFL_FEFB2, PPFL_FEFB3, PPFL_FTKE, PPFL_FTKEI)

!**** *APL_ARPEGE_ATMOSPHERE_UPDATE*
...
! Calcul des tendances de T , U et de Q et modifications eventuelles de W et de OMEGA/P

CALL CPTEND_NEW(YDMODEL%YRCST, YDMODEL, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG,           &
& YDVARS%GEOMETRY%GNORDL%T0, YDVARS%GEOMETRY%GNORDM%T0, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL,  &
& YDMF_PHYS%OUT%DIFCS, PDIFEXT, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTQN, YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTS,     &
& YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL,                &
& YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLSG, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%OUT%FPLCG,                &
& YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN, YDMF_PHYS%OUT%FPEVPSG, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN,      &
& YDMF_PHYS%OUT%FPEVPCG, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN, YDMF_PHYS%OUT%FPFPSG, YDMF_PHYS%OUT%FPFPCL,          &
& YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS%OUT%FCQLNG, YDMF_PHYS%OUT%FCQNNG, YDMF_PHYS%OUT%FCQRNG, YDMF_PHYS%OUT%FCQSNG,           &
& YDMF_PHYS%OUT%FCQGNG, YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%FRMH, PMSC_FRMQ, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,       &
& YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV, YDMF_PHYS%OUT%STRDU, YDMF_PHYS%OUT%STRDV, YDMF_PHYS%OUT%STRTU,                &
& YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV, YDMF_PHYS%OUT%DIFCQLC, YDMF_PHYS%OUT%DIFCQIC,            &
& YDMF_PHYS%OUT%FIMCC, YDMF_PHYS%OUT%FEDQLC, YDMF_PHYS%OUT%FEDQIC, YDMF_PHYS%OUT%FEDQRC, YDMF_PHYS%OUT%FEDQSC,            &
& YDMF_PHYS%OUT%FCNEGQLC, YDMF_PHYS%OUT%FCNEGQIC, YDMF_PHYS%OUT%FCNEGQRC, YDMF_PHYS%OUT%FCNEGQSC,                         &
& PPFL_FTKE, PPFL_FTKEI, PPFL_FEFB1, PPFL_FEFB2, PPFL_FEFB3, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,                      &
& YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,      &
& YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%Q, YDMF_PHYS_BASE_STATE%I, &
& YDMF_PHYS_BASE_STATE%L, YDVARS%LCONV%T0, YDVARS%ICONV%T0, YDVARS%RCONV%T0, YDVARS%SCONV%T0, YDMF_PHYS_BASE_STATE%R,     &
& YDMF_PHYS_BASE_STATE%S, YDMF_PHYS_BASE_STATE%G, PDSA_CPS, YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FHSCL,          &
& YDMF_PHYS%OUT%FHSCN, YDMF_PHYS%OUT%FHSSL, YDMF_PHYS%OUT%FHSSN, YDMF_PHYS%OUT%FHSSG, YDMF_PHYS%OUT%FHPCL,                &
& YDMF_PHYS%OUT%FHPCN, YDMF_PHYS%OUT%FHPCG, YDMF_PHYS%OUT%FHPSL, YDMF_PHYS%OUT%FHPSN, YDMF_PHYS%OUT%FHPSG,                &
& ZMSC_FHP, ZPFL_FP, YDMF_PHYS%OUT%FEPFP, YDMF_PHYS%OUT%FCMPCQ, YDMF_PHYS%OUT%FCMPSN, YDMF_PHYS%OUT%FCMPSL,               &
& YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDH, ZTENDQ, ZTENDI, ZTENDL,                               &
& ZTENDLCONV, ZTENDICONV, ZTENDRCONV, ZTENDSCONV, ZTENDR, ZTENDS, ZTENDG, ZTENDTKE, ZTENDEFB1,                            &
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%NDPSFI == 1) THEN
  CALL CPMVVPS(YDMODEL%YRCST, YDGEOMETRY%YRVAB, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,           &
  & YDCPG_OPTS%KFLEVG, YDCPG_OPTS%ZDTPHY, ZPFL_FP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(:, YDCPG_OPTS%KFLEVG), &
  & YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDCPG_DYN0%CTY%EVEL, YDCPG_DYN0%CTY%PSDVBC, YDMF_PHYS_NEXT_STATE%SP&
  & )
ENDIF
...
CALL CPUTQY_APLPAR_EXPL(YDMODEL%YRCST, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS_NEXT_STATE,                       &
& YDMF_PHYS_BASE_STATE, YDVARS, YDMODEL%YRML_PHY_MF%YRPHY, YDCPG_OPTS%ZDTPHY, ZTENDH, YDMF_PHYS%OUT%TENDU, &
& YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDD, ZTENDEFB1, ZTENDEFB2, ZTENDEFB3, ZTENDG, ZTENDICONV,        &
& ZTENDI, ZTENDLCONV, ZTENDL, ZTENDQ, ZTENDRCONV, ZTENDR, ZTENDSCONV, ZTENDS, ZTENDTKE, YDMF_PHYS%OUT%FDIS &
& )
