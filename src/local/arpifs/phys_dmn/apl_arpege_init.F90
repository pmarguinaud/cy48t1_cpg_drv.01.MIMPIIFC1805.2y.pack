SUBROUTINE APL_ARPEGE_INIT (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, &
& YDVARS, YDMODEL, KMOC_CLPH, KNLAB_CVPP, PAER, PAERINDS, PAIPCMT, PALBD, PALBP, PALPHA1, PCEMTR, &
& PCFATH, PCFAU, PCFBTH, PCFBU, PCFBV, PCOEFA, PCONDCVPPI, PCONDCVPPL, PCTRSO, PDECRD, PDIFCVPPQ, &
& PDIFCVPPS, PDIFCVPPU, PDIFCVPPV, PDIFWQ, PDIFWS, PEDMFQ, PEDMFS, PEDMFU, PEDMFV, PEPS0, PEPSNEB, &
& PFPCOR, PKQLROV, PKQROV, PKTROV, PKUROV, PLVT, PMF_UP, PMRIPP, PNEBC0, PNEBCH, PNEBDIFF, PNEBS, &
& PNEBS0, PNEB_CVPP, PPFL_FPLCH, PPFL_FPLSH, PPOID, PPRODTH_CVPP, PQC_DET_PCMT, PQI, PQIC, PQICE, &
& PQL, PQLC, PQLIS, PQLIS0, PQLI_CVP, PQLI_CVPP, PQO3, PQR, PQS, PQV, PSC_FCLL, PSC_FCLN, PSC_FEVI, &
& PSC_FEVN, PSEDIQI, PSEDIQL, PSFSWDIF, PSFSWDIR, PSUDU, PTENHA, PTENQVA, PTENT, PTRSOD, PTRSODIF, &
& PTRSODIR, PUNEBH, PXDROV, PXHROV, PXQRO, PXTRO, PXTROV, PXURO, PXUROV)
...
INTEGER(KIND=JPIM),             INTENT(OUT)   :: KNLAB_CVPP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PAIPCMT(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PCONDCVPPI(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PCONDCVPPL(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PCTRSO(YDCPG_DIM%KLON,0:1)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDECRD   (YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFCVPPQ(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFCVPPS(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFCVPPU(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFCVPPV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFWQ (YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PDIFWS (YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PEDMFQ(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PEDMFS(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PEDMFU(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PEDMFV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PEPS0
REAL(KIND=JPRB),                INTENT(OUT)   :: PEPSNEB
REAL(KIND=JPRB),                INTENT(OUT)   :: PFPCOR(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PKQLROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PKQROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PKTROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PKUROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PLVT   (YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PMF_UP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PMRIPP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PNEBC0(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PNEBCH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PNEBDIFF(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PNEB_CVPP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PPOID(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PPRODTH_CVPP(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQC_DET_PCMT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQI(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQIC(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQICE  (YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQL(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQLC(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PQLI_CVP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQLI_CVPP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQO3(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQR(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQS(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PQV(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PSEDIQI(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PSEDIQL(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PSUDU(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PTENT(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PUNEBH(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PXDROV(YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PXHROV(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PXTROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PXURO(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PXUROV(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG)
...
REAL(KIND=JPRB) :: ZIPOI(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB) :: ZQV0
...
ELSEIF (YDMODEL%YRML_PHY_RAD%YRERAD%NOZOCL == 1) THEN
  IF (MOD(YDCPG_DIM%KSTEP,YDMODEL%YRML_PHY_RAD%YRERAD%NRADFR) == 0) THEN
    CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDVARS%GEOMETRY%GEMU%T0, &
    & ZROZ)
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      PQO3(JLON,0)=1.E-9_JPRB
    ENDDO
...
ELSE
  CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, PQO3, .FALSE., &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMODEL%YRML_PHY_MF%YRPHY%LO3ABC, YDMF_PHYS_SURF%GSD_VC%PGROUP)
ENDIF
...
ZGDT=RG*YDMODEL%YRML_PHY_MF%YRPHY2%TSPHY
ZGDTI=1.0_JPRB/ZGDT
...
DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  KMOC_CLPH   (JLON)   = YDCPG_DIM%KFLEVG
ENDDO
...
IF((YDMODEL%YRML_PHY_MF%YRPHY%LNEBN.OR.YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST).AND.YDCPG_DIM%KSTEP == 0) THEN
  ...
  ENDDO
ENDIF
IF(YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST.AND.YDCPG_DIM%KSTEP == 0) THEN
  DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
