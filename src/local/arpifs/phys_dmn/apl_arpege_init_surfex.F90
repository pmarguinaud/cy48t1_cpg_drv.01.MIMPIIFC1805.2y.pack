SUBROUTINE APL_ARPEGE_INIT_SURFEX (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, &
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
  ...
  IF ( (YDMODEL%YRML_PHY_MF%YRPARAR%NSWB_MNH /= YDMODEL%YRML_PHY_RAD%YRERAD%NSW) ) CALL ABOR1('APLPAR: NSWB_MNH not = NSW')
...
IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMSE.AND.YDCPG_DIM%KSTEP == 0) THEN
  ...
  & YDCPG_DIM%KFDIA, YDGEOMETRY%YRDIM%NDGUNG, YDGEOMETRY%YRDIM%NDGUXG, YDGEOMETRY%YRDIM%NDLUNG, YDGEOMETRY%YRDIM%NDLUXG, YDVARS%GEOMETRY%RINDX%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                   &
  & YDVARS%GEOMETRY%RINDY%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMODEL%YRML_PHY_MF%YRARPHY%LSURFEX_KFROM, YDMF_PHYS%OUT%GZ0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), &
