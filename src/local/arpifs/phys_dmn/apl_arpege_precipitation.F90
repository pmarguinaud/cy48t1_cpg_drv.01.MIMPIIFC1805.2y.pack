SUBROUTINE APL_ARPEGE_PRECIPITATION (YDMF_PHYS_BASE_STATE, YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF,    &
& YDVARS, YDMODEL, PFLU_NEIJ, PNEBS, PNEB_CVPP, PQC_DET_PCMT, PQI, PQL, PQLIS, PQLI_CVPP, PQR, PQS, &
& PQV, PSEDIQI, PSEDIQL, PTENHA, PTENQVA, PVETAF)
...
REAL(KIND=JPRB),                INTENT(IN)    :: PVETAF(YDCPG_OPTS%KFLEVG)
...
ZQMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=PQV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)

CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI,             &
...
& PFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDVARS%GEOMETRY%GM%T0, PVETAF, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN,                    &
