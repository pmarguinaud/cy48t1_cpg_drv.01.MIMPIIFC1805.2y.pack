...
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, PAER, PAERINDS, PALB, PALBD, PALBP, PCEMTR, PCTRSO, &
& PDSRP, PFLU_EMIS, PFLU_QSAT, PFRSODS, PFSDNN, PFSDNV, PQO3, PQR, PQS, PQV, PRDG_MU0, PRDG_MU0LU, &
& PRDG_MU0M, PSDUR, PSFSWDIF, PSFSWDIR, PSUDU, PTENT, PTRSOD, PTRSODIF, PTRSODIR)
...
REAL(KIND=JPRB),                INTENT(INOUT) :: PALB(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PDSRP(YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(IN)    :: PFLU_EMIS (YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(IN)    :: PFLU_QSAT (YDCPG_DIM%KLON, 1:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB),                INTENT(OUT)   :: PFRSODS(YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PFSDNN(YDCPG_DIM%KLON)
REAL(KIND=JPRB),                INTENT(OUT)   :: PFSDNV(YDCPG_DIM%KLON)
...
REAL(KIND=JPRB),                INTENT(OUT)   :: PSDUR(YDCPG_DIM%KLON)
...
IF (LLCALLRAD) THEN
  ...
  & YDCPG_DIM%KSW, PALBD, PALBP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDCPG_MISC%NEB, PQO3, &
  & PAER, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, PFLU_EMIS,                            &
  & PRDG_MU0M, PQV, PFLU_QSAT, YDCPG_MISC%QICE, YDCPG_MISC%QLI, PQS, PQR, YDMF_PHYS_SURF%GSD_VF%PLSM,                    &
  & YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%RAD%EMTD, YDMF_PHYS%RAD%EMTU, YDMF_PHYS%RAD%TRSW, YDMF_PHYS%OUT%FRTHC, &
  & YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSOC, YDMF_PHYS%OUT%FRSO, PSFSWDIR, PSFSWDIF, PFSDNN, PFSDNV,                     &
...
& YDMODEL%YRML_PHY_MF%YRPHY2%TSPHY, PTRSODIR, PTRSODIF, PALBD, PALBP, YDMF_PHYS%OUT%FRSO,             &
& YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSODS, YDMF_PHYS%OUT%FRTHDS, PCEMTR, PCTRSO, YDMF_PHYS%OUT%FRSOC,             &
& YDMF_PHYS%OUT%FRTHC, PSUDU, PSDUR, PDSRP, PSFSWDIR, PSFSWDIF, YDMF_PHYS%OUT%FRSOPS, PFRSODS, YDMF_PHYS%OUT%FRSOPT&
&  )


    ! ---- Take into account day duration depending on altitude.
IF(YDMODEL%YRML_PHY_MF%YRPHY%LDAYD) CALL ACDAYD(YDMODEL%YRML_GCONF%YRRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDCPG_DIM%KTDIA, &
          & YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDVARS%GEOMETRY%GEMU%T0, PRDG_MU0, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
IF(YDMODEL%YRML_PHY_MF%YRPHY0%GRSO < 1._JPRB) CALL ACRSO(YDMODEL%YRML_PHY_MF%YRPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,        &
                   & YDCPG_DIM%KTDIA, YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDVARS%GEOMETRY%GEMU%T0, PRDG_MU0, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO&
                   & )
...
IF(.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    PALB(JLON)=0.0_JPRB
    DO JSG=1,YDMODEL%YRML_PHY_RAD%YRERAD%NSW
      PALB(JLON)=PALB(JLON)+0.5_JPRB*(PALBD(JLON,JSG)+PALBP(JLON,JSG))
    ENDDO
    PALB(JLON)=PALB(JLON)/REAL(YDMODEL%YRML_PHY_RAD%YRERAD%NSW, JPRB)
    YDMF_PHYS%OUT%FRSODS(JLON)=YDMF_PHYS%OUT%FRSO(JLON,YDCPG_DIM%KFLEVG,1)/(1.0_JPRB-PALB(JLON))
  ENDDO
ENDIF
