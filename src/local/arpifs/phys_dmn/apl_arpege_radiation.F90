SUBROUTINE APL_ARPEGE_RADIATION (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR, &
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, PAER, PAERINDS, PALBD, PALBP, PCEMTR, PCTRSO,           &
& PFLU_EMIS, PFLU_QSAT, PQO3, PQR, PQS, PQV, PRDG_MU0, PRDG_MU0LU, PRDG_MU0M, PSFSWDIF, PSFSWDIR,     &
& PSUDU, PTENT, PTRSOD, PTRSODIF, PTRSODIR)
...
ELSE
  IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
    DO JSG=1,YDMODEL%YRML_PHY_RAD%YRERAD%NSW
      PTRSODIR(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JSG)=YDCPG_GPAR%SWDIR(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JSG)
      PTRSODIF(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JSG)=YDCPG_GPAR%SWDIF(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA,JSG)
    ENDDO
  ENDIF
ENDIF

YDMF_PHYS%OUT%FRSOLU(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)=YDMF_PHYS%RAD%RMOON(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)
...
& YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, PFLU_EMIS,          &
...
IF(YDMODEL%YRML_PHY_MF%YRPHY%LDAYD) CALL ACDAYD(YDMODEL%YRML_GCONF%YRRIP, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,               &
...
IF(YDMODEL%YRML_PHY_MF%YRPHY0%GRSO < 1._JPRB) CALL ACRSO(YDMODEL%YRML_PHY_MF%YRPHY0, YDCPG_BNDS%KIDIA,                                 &
                                              & YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%KTDIA,                   &
