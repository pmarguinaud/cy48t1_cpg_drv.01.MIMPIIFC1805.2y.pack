SUBROUTINE APL_ARPEGE_SURFACE (YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR,   &
& YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, LDCONFX, PALBD, PALBP, PALPHA1, PCDROV, PCEROV, &
& PCFATH, PCFAU, PCFBTH, PCFBU, PCFBV, PCHROV, PCOEFA, PCOEFN, PCP, PDIFEXT, PDIFWQ, PDIFWS, PDQSTS,  &
& PDSA_CPS, PDSA_LHS, PDTMSE, PEDMFQ, PEDMFS, PEDMFU, PEDMFV, PFLU_CD, PFLU_CDN, PFLU_EMIS,           &
...
CALL ACDIFV1 ( YDMODEL%YRML_GCONF%YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                 &
...
IF ( YDMODEL%YRML_PHY_MF%YRARPHY%LMSE.AND.LCALLSFX ) THEN
  ...
  & YDCPG_DIM%KFDIA, YDCPG_OPTS%KSTEP, IRR, YDMODEL%YRML_PHY_RAD%YRERAD%NSW, YDMODEL%YRML_GCONF%YGFL%NGFL_EXT,                                                                               &
  ...
  & LDCONFX, NINDAT, PRHGMT, PSTATI, YDMODEL%YRML_GCONF%YRRIP%RSOVR, YDMODEL%YRML_GCONF%YRRIP%RCODEC,                                                                                       &
  & YDMODEL%YRML_GCONF%YRRIP%RSIDEC, YDVARS%GEOMETRY%RINDX%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDVARS%GEOMETRY%RINDY%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                  &
  & YDMF_PHYS_BASE_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), YDMF_PHYS_BASE_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), &
  & YDMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), &
  & ZSVM, ZCARDI, ZRHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                    &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                                                               &
  & PDTMSE, ZDEPTH_HEIGHT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG), ZZS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                         &
  & YDMODEL%YRML_PHY_MF%YRMSE%XZSEPS, PRDG_MU0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), PRDG_MU0N(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                &
  & YDVARS%GEOMETRY%GELAM%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDVARS%GEOMETRY%GEMU%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                    &
  & YDMODEL%YRML_PHY_MF%YRPARAR%XSW_BANDS, PSRAIN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), PSSNOW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                &
  & PSGROUPEL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS%OUT%FRTHDS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                                      &
  & PSFSWDIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDMODEL%YRML_PHY_RAD%YRERAD%NSW), PSFSWDIR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDMODEL%YRML_PHY_RAD%YRERAD%NSW),                             &
  & ZCFAQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), PCFATH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                  &
  & PCFAU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), ZCFBQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                   &
  & PCFBTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), PCFBU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                  &
  & PCFBV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                                                       &
  ...
  & PTSN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS%OUT%FRTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, 1)                                                                         &
...
! Opposite water vapor flux
  ...
  & YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, -1, YDGEOMETRY%YRDIM%NDGUNG, YDGEOMETRY%YRDIM%NDGUXG, YDGEOMETRY%YRDIM%NDLUNG,                                   &
  & YDGEOMETRY%YRDIM%NDLUXG, YDMODEL%YRML_PHY_MF%YRARPHY%LSURFEX_KFROM, ZZS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                           &
...
! calculation of variables for the old "ISBA" atmosphere scheme
  ...
  CALL ARO_GROUND_DIAG_2ISBA( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1, YDCPG_DIM%KIDIA,                        &
  & YDCPG_DIM%KFDIA, YDGEOMETRY%YRDIM%NDGUNG, YDGEOMETRY%YRDIM%NDGUXG, YDGEOMETRY%YRDIM%NDLUNG, YDGEOMETRY%YRDIM%NDLUXG,                   &
  & YDMODEL%YRML_PHY_MF%YRARPHY%LSURFEX_KFROM, YDVARS%GEOMETRY%RINDX%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                  &
...
ELSEIF (.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
  ...
  CALL ARP_GROUND_PARAM ( YDMODEL%YRML_AOC%YRMCC, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA,                       &
...
CALL ACDIFV2 ( YDMODEL%YRML_GCONF%YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,    &
& YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZCFAQ, ZCFAS, PCFAU,                   &
