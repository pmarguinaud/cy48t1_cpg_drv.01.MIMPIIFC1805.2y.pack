...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        YDAPLPAR_TMP%DSA%LHS(JLON)=FOLH(YLMF_PHYS_BASE_STATE%YGSP_RR%T(JLON),0.0_JPRB)
     ENDDO
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YGSP_RR%IC, LLHMT, YDAPLPAR_TMP%DSA%C1, YDAPLPAR_TMP%DSA%C2, ZC3, ZCG, ZCN, YDMF_PHYS%OUT%CT,    &
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ...
      YLMF_PHYS_BASE_STATE%YGSP_RR%T      (JLON) = PGPAR(JLON,MVTS)
    ...
    IF (LMSE) THEN
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (YDCPG_DIM%KSTEP == 0) THEN 
        ...
        & YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDCPG_MISC%QS, &
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        ...
        & YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH,               &
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                    &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,         &
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDAPLPAR_TMP%MSC%LSCPE, &
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      & YLMF_PHYS_BASE_STATE%V, ZTHETAVS, YDAPLPAR_TMP%MOC%CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%VEIN,                &
      & ZUGST, ZVGST)
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%T, ZQV,                    &
        & ZQL, ZQI, YLMF_PHYS_BASE_STATE%R, YLMF_PHYS_BASE_STATE%S, YLMF_PHYS_BASE_STATE%TKE, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,          &
        ...
        & ZMN2PP, ZFMGST, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH, YDMF_PHYS%OUT%GZ0, YLMF_PHYS_BASE_STATE%YGSP_RR%T, &
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        ...
        & YDAPLPAR_TMP%MSC%LSCPE, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,                   &
        & YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0,                         &
        & ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTCOEF, YDCPG_DIM%KFLEVG,                  &
      & YDCPG_DIM%KSTEP, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,              &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, ZQV, ZQL, ZQI, ZCP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, &
      & YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,                            &
      & YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0,                 &
      & YLMF_PHYS_BASE_STATE%TKE, YDVARS%TTE%T0, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PQSH, YDAPLPAR_TMP%DSA%RS,             &
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZCRTI(JLON) = 1.0_JPRB/(YLMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)*YDAPLPAR_TMP%DSA%RS(JLON))
      ENDDO
      ...
      & YLMF_PHYS_BASE_STATE%YGSP_RR%T, LLHMT, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCE,                     &
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,               &
      & ZFMTKE, ZFTTKE, ZAUTKE, ZATTKE, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U,       &
      & YLMF_PHYS_BASE_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, ZKTROV, ZKUROV, ZKNROV, ZXTROV, ZXUROV,                   &
      & ZXPTKEROV)
    ELSE
        ...
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-YDAPLPAR_TMP%FLU%VEG(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & YDAPLPAR_TMP%FLU%VEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+YDAPLPAR_TMP%FLU%NEIJ(JLON)*YLMF_PHYS_BASE_STATE%YGSP_SG%A(JLON,1)
          ELSE
            ...
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & YLMF_PHYS_BASE_STATE%YGSP_SG%A(JLON,1))+(YDAPLPAR_TMP%FLU%NEIJ(JLON)-ZNEIJV(JLON))*     &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0,     &
      & YDGSGEOM%GEMU, YDGSGEOM%GELAM, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO, &
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,                    &
    & YDAPLPAR_TMP%MSC%LH, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQCI, ZQCL, ZQLIS, YDAPLPAR_TMP%FLU%QSAT,                   &
    & YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%T,              &
    & YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDAPLPAR_TMP%DSA%CPS, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, &
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,             &
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%T, &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDAPLPAR_TMP%MSC%LH, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, YLMF_PHYS_BASE_STATE%Q, ZQI, ZQL, YLMF_PHYS_BASE_STATE%R, YLMF_PHYS_BASE_STATE%S,                    &
      & ZQLIS, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_TMP%RDG%CVGQ,           &
      & YLMF_PHYS_BASE_STATE%T, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T,      &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,             &
    & YLMF_PHYS_BASE_STATE%T, YDAPLPAR_TMP%PFL%FPLCH, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDAPLPAR_TMP%MSC%LSCPE, ZQV,                   &
    & YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V,   &
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%GZ0, YDAPLPAR_TMP%PFL%FPLCH, &
    & YDMF_PHYS_SURF%GSD_VH%PPBLH, YDCPG_MISC%QS, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZKTROV, ZKUROV, ZNBVNO,                    &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
    & ZZNEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS_TMP%RDG%MMU0,                    &
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,          &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,             &
        & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDGSGEOM%GELAM,                 &
        & YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T,                   &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZALBD, ZALBP, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDCPG_MISC%NEB,          &
      & ZQO3, ZAER, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0M,             &
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS%RAD%EMTD, YDMF_PHYS_TMP%RDG%MU0, &
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    ...
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VF%PLSM,              &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      & ZKTROV, ZKUROV, ZKNROV, ZQV, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U,            &
      & YLMF_PHYS_BASE_STATE%V, ZXTROV, ZXUROV, ZXPTKEROV, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZQL, ZQI, ZNEBDIFF, YLMF_PHYS_BASE_STATE%TKE, ZLMU, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, &
      ...
      & YDAPLPAR_TMP%FLU%QSATS, YLMF_PHYS_BASE_STATE%YGSP_SB%T, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,               &
      & ZXDROV, ZXHROV, YLMF_PHYS_BASE_STATE%YGSP_RR%W, YLMF_PHYS_BASE_STATE%YGSP_RR%IC, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS,  &
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        & ZLMU, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZF_EPS, ZFUN_TTE, ZMRIPP, ZMRIFPP, ZRHS, ZMN2_ES,                       &
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          & ZXURO, ZXQRO, ZXTRO, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZQL, ZQI, ZNEBDIFF, YDAPLPAR_TMP%FLU%CD,              &
      ...
      IF (L3DTURB) THEN
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, &
        & YLMF_PHYS_BASE_STATE%DIV, YLMF_PHYS_BASE_STATE%VOR, YDVARS%U%DL, YDVARS%V%DL, YLMF_PHYS_BASE_STATE%YCPG_PHY%W,                            &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%WL, YLMF_PHYS_BASE_STATE%YCPG_PHY%WM, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                                 &
        & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZLML, ZFHORM, ZFHORH,                                   &
      ...
      IF (LCOEFKTKE) THEN
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, ZCP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZKTROV, ZXTROV, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, ZCHROV, ZXHROV, ZCOEFA, ZQV, ZQL, ZQI, YLMF_PHYS_BASE_STATE%U,                          &
        & YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,            &
        & ZLML, ZTH_FUN, ZWW_FUN, ZF_EPS, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS%OUT%DIFTQ, YLMF_PHYS_BASE_STATE%TKE,                         &
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, &
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      & YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, ZPREN, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDMF_PHYS%OUT%FCS, &
...
!DEC$ IVDEP
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,YDCPG_DIM%KFLEVG,1)/YDAPLPAR_TMP%FLU%EMIS(JLON)+RSIGMA*YLMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)**4
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    ...
    & YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, ZQV, YDCPG_MISC%QS, YLMF_PHYS_BASE_STATE%YGSP_RR%T, &
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%T, &
  ...
  IF ( LSTRAS ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZNEBS, ZQV, ZQLIS, YDAPLPAR_TMP%MSC%QW,      &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, ZT, ZGEOSLC, ZLH,                &
      & ZLSCPE, ZQSAT, ZQW, ZRH, ZTW)
      CALL ACNEBCOND ( YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                  &
      & NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,            &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, ZQW, ZT, ZNEBCH, YDGSGEOM%GM,          &
      & YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1, ZQLIS0, ZNEBS0)
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                 &
      & ZQG, ZQW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZT, YDAPLPAR_TMP%MSC%TW, ZNEBS, ZRHCRI, ZICEFR1,                   &
      & ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP, ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM,   &
      & YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL,              &
    ...
    ELSE
      ...
      & ZVETAF, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,       &
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                   &
      & ZQG, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YDAPLPAR_TMP%MSC%TW,              &
      & ZNEBS, ZRHCRI, ZICEFR1, ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP,                    &
      & ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T,          &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQSAT, ZQW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,                           &
      & ZT, ZTW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%VOR, YDVARS%DAL%T0, YLMF_PHYS_BASE_STATE%YGSP_RR%T,               &
...
! PASS T9 CONDENSATES TO ACCSU
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                           &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,                      &
      & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,                           &
      ...
      & YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,                             &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, &
    & ZQV, ZQI, ZQL, ZQS, ZQR, ZQG, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZT, ZIPOI, ZDQ, ZDQM, ZLHS, ZLHV,                       &
    & ZNEBS, ZPOID, ZRCVOTT, ZTCORR, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YLMF_PHYS_BASE_STATE%YGSP_RR%T,          &
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,  &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,               &
      & ZQV, ZQI, ZQL, ZQR, ZQS, ZQW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, ZSIGP, ZT, ZTW, ZU, ZV, YDCPG_DYN0%CTY%VVEL(:, 1:), &
    ...
    ELSE
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,       &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,                      &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
  ...
  !  ---------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZLHS,                         &
    & ZLHV, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZIPOI, ZPOID, ZFALLR, ZFALLS, ZFALLG, YDMF_PHYS%OUT%FPEVPSL,                 &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,                        &
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
    & YLMF_PHYS_BASE_STATE%T, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%VOR,                   &
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI, YDMF_PHYS%OUT%DIFCQ,      &
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,                &
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,                      &
    & ZQV, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%T,   &
    & YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PPBLH, &
  ...
  IF ( LPROCLD ) THEN
    ...
    & ZQLI_CVPP, ZQC_DET_PCMT, ZTENHA, ZTENQVA, LADJCLD, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YGSP_RR%T,       &
  ...
  IF ( LGWDC ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH, &
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%FRMH, YDAPLPAR_TMP%MSC%FRMQ, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV,        &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    ...
    & YDMODEL%YRML_PHY_G%YRDPHY%NVCLIS, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
    & PKOZO, ZQO3(1, 1), YLMF_PHYS_BASE_STATE%T, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS%OUT%FCHOZ)
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    & YLMF_PHYS_BASE_STATE%T, ZXTROV, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GEMU, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS%OUT%FCHOZ     &
    & )
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%QICE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAF, 'VIR', YDDDH)
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%QLI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAF, 'VLR', YDDDH)
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%NEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      ...
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_BASE_STATE%YGSP_RR%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDMF_PHYS%OUT%CT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSTS', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*RTINER*YLMF_PHYS_BASE_STATE%YGSP_SB%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YSP_SBD%NLEVS)/YDMF_PHYS%OUT%CT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSTP', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_BASE_STATE%YGSP_RR%W(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSWS', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_BASE_STATE%YGSP_SB%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSWP', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_BASE_STATE%YGSP_SG%F(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1)
      ...
      & )
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
...
IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
  ...
  & YDGSGEOM%GNORDM, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,       &
  & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%T,     &
  & YLMF_PHYS_BASE_STATE%YGSP_RR%T, PGFL, YLPROCSET, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDH, &
...
ELSE
  CALL CPTEND_NEW( YDMODEL, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
   ...
   & YDMF_PHYS%OUT%DIFCQLC,YDMF_PHYS%OUT%DIFCQIC,YDMF_PHYS%OUT%FIMCC,&
   & YDMF_PHYS%OUT%FEDQLC, YDMF_PHYS%OUT%FEDQIC, YDMF_PHYS%OUT%FEDQRC, YDMF_PHYS%OUT%FEDQSC, YDMF_PHYS%OUT%FCNEGQLC,YDMF_PHYS%OUT%FCNEGQIC,YDMF_PHYS%OUT%FCNEGQRC,YDMF_PHYS%OUT%FCNEGQSC,&
   & YDAPLPAR_TMP%PFL%FTKE, YDAPLPAR_TMP%PFL%FTKEI,  YDAPLPAR_TMP%PFL%FEFB1 , YDAPLPAR_TMP%PFL%FEFB2, YDAPLPAR_TMP%PFL%FEFB3 ,YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP ,&
   ...
   & YDAPLPAR_TMP%DSA%CPS, YLMF_PHYS_BASE_STATE%YGSP_RR%T  ,&
...
IF (LSLAG) CALL CP_PTRSLB1(YDMODEL%YRML_DYN%YRDYN,YDPTRSLB1,ISLB1U9,ISLB1V9,ISLB1T9,ISLB1VD9,ISLB1GFL9)
CALL CPUTQY_APLPAR_EXPL(YDCPG_DIM, YLMF_PHYS_NEXT_STATE, YDVARS, YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, &
& YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, PDTPHY,   &
& IPGFL, ISLB1GFL9, ZTENDH, ZTENDT, YDMF_PHYS%OUT%TENDU,      &
...
& YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,          &
& YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDCPG_SL1%ZVIEW, &
& PGMVT1, PGFLT1, YDMF_PHYS%OUT%FDIS)

CALL CPUTQY_APLPAR_LOOP(YDCPG_DIM, YLMF_PHYS_NEXT_STATE, YDVARS, YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, &
& YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, PDTPHY,   &
& IPGFL, ISLB1GFL9, ZTENDH, ZTENDT, YDMF_PHYS%OUT%TENDU,      &
& YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDD, ZTENDGFL, &
& ZPTENDEFB11, ZPTENDEFB21, ZPTENDEFB31, ZPTENDG1, ZPTENDICONV1, ZPTENDI1, ZPTENDLCONV1, &
