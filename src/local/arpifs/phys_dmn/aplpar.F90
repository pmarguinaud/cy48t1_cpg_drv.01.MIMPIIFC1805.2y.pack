...
!       Computation of ZRTI
  ...
  IF (LRAFTKE) THEN
    ...
    ZDCAPE(:)=0._JPRB
    CALL ACCLDIA(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST, YDPHY, YDPHY2, YDTOPH, YDCPG_BNDS%KIDIA,      &
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & NTPLUI, YDCPG_OPTS%KFLEVG, LLREDPR, YDMODEL%YRML_PHY_MF%YRPHY0%RHUC, ZVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI,          &
    & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,                                                                                      &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,        &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    ...
    & ZAIPCMT, YDCPG_MISC%NEB, ZNEBC0, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDMODEL%YRML_PHY_MF%YRPHY0%RHUC, ZVETAF)
    DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
!DEC$ IVDEP
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST, YDPHY, YDPHY2, YDTOPH,        &
