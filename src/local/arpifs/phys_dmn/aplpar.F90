!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDAPLPAR,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
 ...
 & PMU0   , PMU0LU ,&
 & PMU0M  , PMU0N   ,PGELAM ,&
 & PGEMU  , PGM    , POMPAC , PAC    , PCOR   , PMMU0  , &
...
!     ------------------------------------------------------------------
  ...
  IF (LCVPPKF) THEN
    CALL ACVPPKF( YDMODEL%YRML_PHY_MF,KIDIA, KFDIA, KLON, NTCVIM, KLEV,&
     ...
     & ZQL, ZQI, YDAPLPAR%U, YDAPLPAR%V, PVERVEL, YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%TKE,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      ! - INPUT  2D .
      & YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,PVERVEL,YDAPLPAR%YCPG_DYN%RCP%CP,PLH,YDAPLPAR%YCPG_DYN%RCP%R,&
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      IF (L3DTURB) THEN
        CALL ACTKECOEFKH(YDRIP,YDMODEL%YRML_PHY_MF,YDGEOMETRY%YREGEO,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
          ...
          & YDAPLPAR%DIV, YDAPLPAR%VOR, PUL, PVL, YDAPLPAR%YCPG_PHY%W, YDAPLPAR%YCPG_PHY%WL, YDAPLPAR%YCPG_PHY%WM, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%YCPG_PHY%XYB%LNPR,&
  ...
  !  -------------------------
    ...
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%RDELP,ZSIGP,ZT,ZTW,ZU,ZV,PEVEL0,ZOME,&
       & ZATSLC,ZGEOSLC,ZFHP,&
       & YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLSG,&
       & ZDIFCQD,ZDIFCQLD,ZDIFCQID,ZDIFCSD,&
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     ...
     & YDAPLPAR%YCPG_PHY%XYB%LNPR, ZQV, PQSAT, PQW, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%T, PTW, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%VOR, PVERVEL,&
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    DO JLEV=1,KLEV
      DO JLON=KIDIA,KFDIA
        ...
        ZVERVEL(JLON,JLEV)    = PVERVEL(JLON,JLEV)*YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV)
