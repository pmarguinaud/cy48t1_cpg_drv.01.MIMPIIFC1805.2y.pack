...
!-------------------------------------------------
!
IF(LGCHECKMV) CALL CHECKMV(YDMODEL%YRCST, YDRIP, YDPHY0, YDPHY2, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,       &
...
!     -------------------------------------------
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      LLHMT=.FALSE.
      CALL ACSOL (YDMODEL%YRCST, YDPHY, YDPHY1, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_SURF%GSD_VV%PARG,    &
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        ...
        &                                                                                                                                                  )
      ELSE
        CALL ACHMT (YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY1, YDMODEL%YRML_PHY_MF%YRPHY2,    &
        & YDMODEL%YRCST, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDSURF%YSD_VVD%NUMFLDS>=8.AND.LSOLV, &
        & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,      &
        & YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, ZPFL_FPLSH,                                         &
        & ZPFL_FPLCH, ZDPHIT, ZDPHIV, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,           &
  ...
  IF (LRAFTKE) THEN
    ...
    ZDCAPE(:)=0._JPRB
    CALL ACCLDIA(YDMODEL%YRCST, YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
...
!     ------------------------------------------------------------------
  ...
  IF (LCVPPKF) THEN
    CALL ACVPPKF(YDMODEL%YRCST,YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,  &
...
! Computation of the 2 znlab used in acbl89
    ...
    CALL ACTKE (YDMODEL%YRCST, YDLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA,        &
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND (YDMODEL%YRCST, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,            &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      CALL ACPCMT(YDMODEL%YRCST, YDGEM, YDGEOMETRY%YRDIM, YDGEOMETRY%YREGEO, YDLDDH, YDMODEL%YRML_DIAG%YRMDDH,                      &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    CALL ACNEBN (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTNEBU, YDCPG_OPTS%KFLEVG,  &
...
!         Diagnostique de nebulosite partielle.
  CALL ACNPART(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTNEBU, YDCPG_OPTS%KFLEVG, &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF(LDAYD) CALL ACDAYD(YDMODEL%YRCST, YDRIP, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,    &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    ...
    ELSE
      ...
      CALL ACDIFV1 (YDMODEL%YRCST, YGFL, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,                   &
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          IF (LEDMFI) THEN
            ...
            ZCFBV_G(:,:) = ZCFBV(:,:)
          ENDIF   
          CALL ARP_GROUND_PARAM (YDMODEL%YRCST,  YDMCC, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, &
      ...
      CALL ACDIFV2 (YDMODEL%YRCST,  YGFL, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,&
...
!     ---------------------------------
  ...
  IF ( LSTRAS ) THEN
    CALL ACPLUIS (YDMODEL%YRCST,  YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI, &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI,          &
...
! -----------------------------------------------------
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    CALL CUCALLN_MF   (YDCPG_OPTS%KSTEP, YDMODEL%YRML_PHY_EC%YRTHF, YDMODEL%YRCST, YDMODEL%YRML_PHY_RAD%YRERAD,           &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    ZGAW(:)=0._JPRB
    CALL CULIGHT   (YDMODEL%YRML_PHY_EC%YRTHF, YDMODEL%YRCST, YDEPHY, YGFL,                                                       &
  ...
  IF ( LADJCLD ) THEN
    ...
    ZFRTH(:,:) = 0.0_JPRB
    LLPTQ = .TRUE.
    CALL ACUPTQ (YDMODEL%YRCST,  YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, LLPTQ,   &
  ...
  IF ( LPROCLD ) THEN
    ...
    CALL ACPLUIZ (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI,                        &
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDMODEL%YRCST, YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, &
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_OPTS%KFLEVG
    CALL PPWETPOINT(YDMODEL%YRCST, YDPHY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(:, JLEV), &
...
ELSE
  CALL CPTEND_NEW(YDMODEL%YRCST,  YDMODEL, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, YDVARS%GEOMETRY%GNORDL%T0, &
...
IF (NDPSFI == 1) THEN
  CALL CPMVVPS(YDMODEL%YRCST, YDVAB, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%ZDTPHY,  &
...
CALL CPUTQY_APLPAR_EXPL(YDMODEL%YRCST, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS_NEXT_STATE, YDMF_PHYS_BASE_STATE, YDVARS, &
...
IF ((.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,YDCPG_OPTS%KFLEVG
      ...
      ENDDO
    ENDDO
    CALL CPTENDS(YDMODEL%YRCST,  YDMODEL%YRML_PHY_MF, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, &
    ...
    CALL CPWTS(YDMODEL%YRCST, YDCPG_OPTS, YDMODEL%YRML_AOC%YRMCC, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, &
