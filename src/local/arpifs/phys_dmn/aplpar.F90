...
IF (.NOT.LDCONFX) THEN
  ...
  IF (GCHETN%LPROFV)&
   & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_TMP%RDG%LCVQ, YDMF_PHYS_TMP%RDG%MU0, YDAPLPAR_TMP, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS,     &
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_TMP%RDG%LCVQ, YDMF_PHYS_TMP%RDG%MU0, YDAPLPAR_TMP  )
ENDIF
