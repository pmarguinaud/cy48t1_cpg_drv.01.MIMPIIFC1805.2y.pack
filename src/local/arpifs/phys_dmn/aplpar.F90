...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
              ...
              & YDGSGEOM%GEMU, ZRDG_MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%Q,  &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & YDGSGEOM%GELAM, ZRDG_MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,               &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI, YDGSGEOM%GM, ZDECRD_MF, YDMF_PHYS%OUT%DIFCQ,                           &
      & YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN,                    &
...
IF (.NOT.LDCONFX) THEN
  ...
  IF (GCHETN%LPROFV)&
   & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, ZRDG_CVGQ, ZRDG_LCVQ, ZRDG_MU0, YDCPG_DYN0,        &
     & YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KFDIA, YDGSGEOM%GELAM, &
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDGSGEOM%GEMU, YDGSGEOM%GM, YDOROG%      OROG, YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX,         &
  & YDGSGEOM%GECLO, YDGSGEOM%GESLO, ZRDG_CVGQ, ZRDG_LCVQ, ZRDG_MU0, ZDSA_C1, ZDSA_C2, ZDSA_CPS,              &
