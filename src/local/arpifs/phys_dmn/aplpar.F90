...
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART1 (YDCPG_DIM, YDMF_PHYS_TMP%SAV%DDAL, YDMF_PHYS_TMP%SAV%DDOM, YDMF_PHYS_TMP%SAV%ENTCH,        &
  & YDMF_PHYS_TMP%SAV%FHPS, YDMF_PHYS_TMP%SAV%GZ0F, YDMF_PHYS_TMP%SAV%GZ0HF, YDMF_PHYS_TMP%SAV%HV, YDMF_PHYS_TMP%SAV%PBLH,   &
  & YDMF_PHYS_TMP%SAV%QSH, YDMF_PHYS_TMP%SAV%UDAL, YDMF_PHYS_TMP%SAV%UDGRO, YDMF_PHYS_TMP%SAV%UDOM, YDMF_PHYS_TMP%SAV%UNEBH, &
  & YDMF_PHYS_SURF, YDVARS%DAL%T0, YDVARS%DOM%T0, YDVARS%UAL%T0, YDVARS%UEN%T0, YDVARS%UNEBH%T0, YDVARS%UOM%T0,              &
  & YDSURF, YDMODEL)
ENDIF
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
              ...
              & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDGSGEOM%GELAM,          &
              & YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_BASE_STATE%T,             &
              & YLMF_PHYS_BASE_STATE%Q, YLMF_PHYS_BASE_STATE%YGSP_RR%T                      )
!     ------------------------------------------------------------------
...
!       Computation of ZRTI
  ...
  IF (LRAFTKE) THEN
    ...
    & ZDCAPE, YLMF_PHYS_BASE_STATE%TKE, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST,       &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, YDMF_PHYS_TMP%RDG%MU0, YDGSGEOM%GEMU,        &
      & YDGSGEOM%GELAM, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO,                      &
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    & YLMF_PHYS_BASE_STATE%V, ZDSA_CPS, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%DIFCQ,                               &
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & ZNEBCH, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS,                                 &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YLMF_PHYS_BASE_STATE%S, ZQLIS, ZFLU_QSAT, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_TMP%RDG%CVGQ,         &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS_TMP%RDG%MMU0, YDCPG_MISC%DHSF, ZFLU_EMIS, ZAER, ZAC,       &
    & ZAC_HC, ZRDT_COR, ZRDT_RAB3C, ZRDT_RAB3N, ZRDT_RAB4C, ZRDT_RAB4N, ZRDT_RAB6C, ZRDT_RAB6N,          &
    & ZRDT_RAT1C, ZRDT_RAT1N, ZRDT_RAT2C, ZRDT_RAT2N, ZRDT_RAT3C, ZRDT_RAT3N, ZRDT_RAT4C, ZRDT_RAT4N,    &
    & ZRDT_RAT5C, ZRDT_RAT5N)
  ENDIF
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,              &
        & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, YDGSGEOM%GELAM, YDGSGEOM%GEMU,               &
        & YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD,                           &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & YDCPG_MISC%NEB, ZQO3, ZAER, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZFLU_EMIS, YDMF_PHYS_TMP%RDG%MU0M,         &
    ...
    & ZQV, ZTENT, YDMF_PHYS%RAD%TRSW, ZTRSOD, YLMF_PHYS_BASE_STATE%YGSP_RR%T, TSPHY, ZTRSODIR,                      &
    & ZTRSODIF, ZALBD, ZALBP, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRSODS, YDMF_PHYS%OUT%FRTHDS,   &
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    ...
    & YDGSGEOM%RCORI, YDMF_PHYS_SURF%GSD_VF%PGETRL, ZGWDCS, YDMF_PHYS_SURF%GSD_VF%PVRLAN, YDMF_PHYS_SURF%GSD_VF%PVRLDI,    &
...
!     ---------------------------------
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1,                                &
      & ZQLIS0, ZNEBS0)
      ...
      & ZFLU_NEIJ, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN,          &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_TMP%RDG%CVGQ,                            &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI,                                     &
      & ZQL, ZQSAT, ZQW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, ZT, ZTW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V,            &
      & YLMF_PHYS_BASE_STATE%VOR, YDVARS%DAL%T0, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,                               &
      & YDGSGEOM%GM, ZDECRD_MF, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCS,                 &
      & YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV, ZATSLC,                                   &
      & INLAB, YDVARS%UNEBH%T0, ZDETFI, ZTU, ZQU, ZUU, ZVU, INND, YDMF_PHYS%OUT%CAPE, YDMF_PHYS%OUT%CUCONVCA,                         &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,                    &
    ...
    & YDMF_PHYS%OUT%DIFTS, ZGEOSLC, YDGSGEOM%GEMU)
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZGEOM1, ZGEOMH, YDGSGEOM%GM,          &
  ...
  IF ( LPROCLD ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDGSGEOM%GM, ZVETAF, YDMF_PHYS%OUT%FCSQL, &
    & YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN,     &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%KOZO, ZQO3(1, 1), YLMF_PHYS_BASE_STATE%T, YDMF_PHYS_TMP%RDG%MU0, &
    & YDMF_PHYS%OUT%FCHOZ)
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   & YDOROG%OROG, YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,                             &
...
IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
  ...
  & YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
...
ELSE
  CALL CPTEND_NEW( YDMODEL, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, YDGSGEOM%GNORDL,            &
  & YDGSGEOM%GNORDM, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCS,                  &
...
IF (.NOT.LDCONFX) THEN
  ...
  IF (GCHETN%LPROFV)&
   & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_TMP%RDG%LCVQ,          &
     & YDMF_PHYS_TMP%RDG%MU0, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF,     &
     & YDCPG_DIM%KFDIA, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, YDOROG%OROG, YDGSGEOM%RCORI, YDCSGEOM%RATATH, &
     & YDCSGEOM%RATATX)
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART2 (YDCPG_DIM, YDMF_PHYS_TMP%SAV%DDAL, YDMF_PHYS_TMP%SAV%DDOM, YDMF_PHYS_TMP%SAV%ENTCH,        &
  & YDMF_PHYS_TMP%SAV%FHPS, YDMF_PHYS_TMP%SAV%GZ0F, YDMF_PHYS_TMP%SAV%GZ0HF, YDMF_PHYS_TMP%SAV%HV, YDMF_PHYS_TMP%SAV%PBLH,   &
  & YDMF_PHYS_TMP%SAV%QSH, YDMF_PHYS_TMP%SAV%UDAL, YDMF_PHYS_TMP%SAV%UDGRO, YDMF_PHYS_TMP%SAV%UDOM, YDMF_PHYS_TMP%SAV%UNEBH, &
  & YDMF_PHYS_SURF, YDVARS%DAL%T0, YDVARS%DOM%T0, YDVARS%UAL%T0, YDVARS%UEN%T0, YDVARS%UNEBH%T0, YDVARS%UOM%T0, YDSURF, YDMODEL)
ENDIF
...
CALL MF_PHYS_BAYRAD (YDCPG_DIM, ZBAY_QRCONV, ZBAY_QSCONV, YDVARS%RCONV%T1, YDVARS%SCONV%T1, YDMODEL, LAROME)
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & ZDSA_C1, ZDSA_C2, ZDSA_CPS, ZDSA_LHS, ZDSA_RS, ZFLU_CD, ZFLU_CDN, ZFLU_CH, ZFLU_EMIS, ZFLU_FEVI,         &
  & ZFLU_NEIJ, ZFLU_QSAT, ZFLU_QSATS, ZFLU_VEG, IMOC_CLPH, ZMSC_FRMQ, ZMSC_LH, ZMSC_LSCPE, ZMSC_QW,          &
  & ZMSC_TW, ZPFL_FEFB1, ZPFL_FEFB2, ZPFL_FEFB3, ZPFL_FPLCH, ZPFL_FPLSH, ZPFL_FTKE  )
ENDIF
...
CALL MF_PHYS_PRECIPS (YDCPG_DIM, YDMF_PHYS_TMP%PRC%DPRECIPS, YDMF_PHYS_TMP%PRC%DPRECIPS2, &
& YDMF_PHYS_SURF, YDMODEL)
