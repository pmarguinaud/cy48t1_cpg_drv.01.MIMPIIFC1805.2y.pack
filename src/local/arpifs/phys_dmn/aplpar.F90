!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDCPG_DIM,YDMF_PHYS_STATE,YDCPG_DYN0,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,&
 & KVCLIS , KVCLIV , &
 & KSGST  , KCSS   ,&
 & KNFRRC , PDT,&
 !---------------------------------------------------------------------
...
DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  ZTAUX   (JLON)   = ZEPS0
  YDMF_PHYS%TMP%APLPAR%MOC%CLPH   (JLON)   = YDCPG_DIM%KFLEVG
ENDDO
DO JSG = 1, NSW
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        YDMF_PHYS%OUT%FCLL(JLON,1)=YDMF_PHYS%OUT%FEVL(JLON,1)*YDMF_PHYS%TMP%APLPAR%MSC%LH(JLON,YDCPG_DIM%KFLEVG)
        YDMF_PHYS%OUT%FCLN(JLON,1)=YDMF_PHYS%OUT%FEVN(JLON,1)*YDMF_PHYS%TMP%APLPAR%MSC%LH(JLON,YDCPG_DIM%KFLEVG)
...
!       Computation of ZRTI
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      CALL ACCLPH ( YDPHY0,YDPHY2,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
       ...
       & YDMF_PHYS%TMP%APLPAR%MOC%CLPH,YDMF_PHYS%OUT%CLPH,YDMF_PHYS%OUT%VEIN,ZUGST,ZVGST)
  ...
  IF (LRAFTKE) THEN
    ...
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,ZBLH,YDMF_PHYS%TMP%APLPAR%MOC%CLPH)
  ENDIF
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,PGEMU ,YDMF_PHYS%TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,PGEMU  ,YDMF_PHYS%TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,                             &
       & YDMF_PHYS%TMP%RDG%MU0,YDMF_PHYS%OUT%FRSO(:,YDCPG_DIM%KFLEVG,1),YDMF_PHYS%OUT%FRSODS,YDMF_PHYS%OUT%FRSOPS,YDMF_PHYS%OUT%FRTH(:,YDCPG_DIM%KFLEVG,1),YDMF_PHYS%OUT%FRTHDS, &
       & YDMF_PHYS%OUT%MRT) 
    ENDIF
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDIFU,YDCPG_DIM%KFLEVG,&
       ...
       & YDMF_PHYS%TMP%APLPAR%MOC%CLPH,ZBLH,ZUGST,ZVGST)
  ...
  IF ( LPROCLD ) THEN
    IF(LGPCMT) THEN
      ...
      ZQMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)&
        & *YDMF_PHYS%TMP%APLPAR%FLU%QSAT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)&
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, YDMF_PHYS_STATE%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, YDMF_PHYS%TMP%APLPAR%MOC%CLPH)
