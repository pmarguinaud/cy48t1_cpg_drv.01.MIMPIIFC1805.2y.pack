...
& YDXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, PKOZO, PGP2DSDT, PGMVT1, PGFLT1, &
...
IF (.NOT. LLDONE) THEN
  WRITE (0, *) __FILE__, ':', __LINE__, " LOC (ZP1EZDIAG (1,1,1) = ",  LOC (ZP1EZDIAG (1,1,1)), &
& " LOC (YDVARS%EZDIAG(1)%T0 (1, 1)) = ", LOC (YDVARS%EZDIAG(1)%T0 (1, 1))
  WRITE (0, *) __FILE__, ':', __LINE__, " LOC (ZP1EZDIAG (1,1,2) = ",  LOC (ZP1EZDIAG (1,1,2)), &
& " LOC (YDVARS%EZDIAG(2)%T0 (1, 1)) = ", LOC (YDVARS%EZDIAG(2)%T0 (1, 1))
  LLDONE = .TRUE.
ENDIF
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDMF_PHYS%OUT%GZ0     (JLON) = YDCPG_GPAR%ZVIEW(JLON,MGZ0)
      YDMF_PHYS%OUT%GZ0H    (JLON) = YDCPG_GPAR%ZVIEW(JLON,MGZ0H)
      ZFLU_EMIS    (JLON) = YDCPG_GPAR%ZVIEW(JLON,MVEMIS)
      YDCPG_MISC%QS      (JLON) = YDCPG_GPAR%ZVIEW(JLON,MVQS)
      YLMF_PHYS_BASE_STATE%YGSP_RR%T      (JLON) = YDCPG_GPAR%ZVIEW(JLON,MVTS)
      ZSRAIN   (JLON) = YDCPG_GPAR%ZVIEW(JLON,MRAIN)
      ZSSNOW   (JLON) = YDCPG_GPAR%ZVIEW(JLON,MSNOW)
    ...
    IF ( LRAY ) THEN
      ...
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDMF_PHYS%OUT%ALB   (JLON) = YDCPG_GPAR%ZVIEW(JLON,MALBSCA)
        ZALBDIR(JLON) = YDCPG_GPAR%ZVIEW(JLON,MALBDIR)
      ENDDO
    ELSE
      ! FMR  radiation => NSW solar bands
      DO JSG=1,NSW
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZALBP(JLON,JSG) = YDCPG_GPAR%ZVIEW(JLON,MALBDIR-1+JSG)
          ZALBD(JLON,JSG) = YDCPG_GPAR%ZVIEW(JLON,MALBSCA-1+JSG)
        ENDDO
      ENDDO
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ...
    IF (LMSE) THEN
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (YDCPG_DIM%KSTEP == 0) THEN 
        ...
        & ZFLU_CDN, ZCDNH, ZDSA_CPS, ZRTI, ZDSA_RS)
...
!       Computation of ZRTI
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                    &
      & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, ZFLU_QSAT,         &
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, ZMSC_LSCPE, &
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        ...
        & ZMSC_LSCPE, ZQV, ZQL, ZQI, ZFLU_QSAT, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,                   &
      ...
      & ZFLU_QSAT, ZMSC_QW, ZMSC_TW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,                            &
      & YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0,                 &
      & YLMF_PHYS_BASE_STATE%TKE, YDVARS%TTE%T0, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PQSH, ZDSA_RS,             &
      & ZDSA_CPS, ZRTI, YDMF_PHYS%OUT%GZ0, LLCLS, ZMRIPP, ZMRIFPP, ZBNEBCVPP, ZBNEBQ,                                   &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, YDMF_PHYS_TMP%RDG%MU0,     &
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,                    &
    & ZMSC_LH, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQCI, ZQCL, ZQLIS, ZFLU_QSAT,                   &
    & ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%T,              &
    & ZMSC_TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, ZDSA_CPS, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & ZQLIS, ZFLU_QSAT, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_TMP%RDG%CVGQ,           &
      & YLMF_PHYS_BASE_STATE%T, ZMSC_TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T,      &
      & YDCPG_MISC%QS, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, ZEDMFQ, YDMF_PHYS%OUT%DIFCQL,                  &
    ...
    ELSE
      ...
      & YLMF_PHYS_BASE_STATE%T, ZMSC_TW, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V, YLMF_PHYS_BASE_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T,      &
      & YDCPG_MISC%QS, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, YDMF_PHYS%OUT%DIFCQ,                           &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    ...
    & ZFLU_QSAT, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, YLMF_PHYS_BASE_STATE%U, YLMF_PHYS_BASE_STATE%V,   &
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%GZ0, ZPFL_FPLCH, &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & YDCPG_MISC%DHSF, ZFLU_EMIS, ZAER, ZAC, ZAC_HC, ZRDT_COR, ZRDT_RAB3C,                &
    & ZRDT_RAB3N, ZRDT_RAB4C, ZRDT_RAB4N, ZRDT_RAB6C,                         &
    & ZRDT_RAB6N, ZRDT_RAT1C, ZRDT_RAT1N, ZRDT_RAT2C,                         &
    & ZRDT_RAT2N, ZRDT_RAT3C, ZRDT_RAT3N, ZRDT_RAT4C,                         &
    & ZRDT_RAT4N, ZRDT_RAT5C, ZRDT_RAT5N)
  ENDIF
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,          &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,             &
        & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, YDGSGEOM%GELAM,                 &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZCGA_DST, ZTAUREL_DST, ZAERINDS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDCPG_GPAR%ZVIEW, YDMF_PHYS_TMP%RDG%MU0LU,           &
      & YDMF_PHYS%OUT%ALB, YDMF_PHYS%RAD%RMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
        ZTRSODIR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSG)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIR+JSG-1)
        ZTRSODIF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JSG)=YDCPG_GPAR%ZVIEW(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSWDIF+JSG-1)
      ENDDO
      ENDIF
    ENDIF
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      & ZFLU_CD, ZFLU_CDN, ZCDROV, ZCHROV, ZCEROV, ZDSA_CPS, YDMF_PHYS%OUT%CT,        &
      ...
      & ZTENDPTKE, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, ZFLU_FEVI, &
      & YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR, ZDSA_LHS,              &
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        IF ( LMSE.AND.LCALLSFX ) THEN       
          ...
          ELSE
            ZFLU_CD(:)=MAX(YDCPG_GPAR%ZVIEW(:,MCD),ZEPS0)
          ENDIF
        ENDIF
        ...
        & ZMN2_EQ, ZRRCOR, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0, ZFLU_CD, YDMF_PHYS%OUT%GZ0,      &
...
! Opposite water vapor flux
        ...
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          YDCPG_GPAR%ZVIEW(JLON,MGZ0)   = YDMF_PHYS%OUT%GZ0 (JLON)
          YDCPG_GPAR%ZVIEW(JLON,MGZ0H)  = YDMF_PHYS%OUT%GZ0H(JLON)
          YDCPG_GPAR%ZVIEW(JLON,MVEMIS) = ZFLU_EMIS(JLON)
          YDCPG_GPAR%ZVIEW(JLON,MVQS)   = YDCPG_MISC%QS  (JLON)
          YDCPG_GPAR%ZVIEW(JLON,MCD)    = ZCD  (JLON)
        ENDDO

        DO JSG=1,NSW
          YDCPG_GPAR%ZVIEW(:,MALBSCA-1+JSG) = ZALBD(:,JSG)
          YDCPG_GPAR%ZVIEW(:,MALBDIR-1+JSG) = ZALBP(:,JSG)
        ENDDO
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      IF ( LMSE ) THEN
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          YDCPG_GPAR%ZVIEW(JLON,MVTS) = ZTSN (JLON)
      ...
      IF (L3DTURB) THEN
        ...
        & ZKUROV, ZRTI, ZFLU_CD, ZCDROV, LPTKE, ZKUR_KUROV_H, ZKUR_KTROV_H,                      &
        & ZRHS)
      ENDIF
      ...
      IF (LCOEFKTKE) THEN
        ...
        & ZDIFWQ, ZDIFWS, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, ZFLU_FEVI,                      &
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      & ZDSA_LHS, YDMF_PHYS%OUT%GZ0, IMOC_CLPH, ZBLH, ZUGST, ZVGST)
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      & YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, ZDSA_LHS, ZRTI, YDMF_PHYS_SURF%GSD_VH%PPBLH               &
      & )
    ENDIF
    ! ------------------------------------------------------------------
...
!     ---------------------------------
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & ZQG, ZQW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZT, ZMSC_TW, ZNEBS, ZRHCRI, ZICEFR1,                   &
      & ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP, ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM,   &
      & ZFLU_NEIJ, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL,              &
    ...
    ELSE
      ...
      & ZVETAF, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,       &
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                   &
      & ZQG, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%T, ZMSC_TW,              &
      & ZNEBS, ZRHCRI, ZICEFR1, ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP,                    &
      & ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM, ZFLU_NEIJ, YDGSGEOM%GM, YLMF_PHYS_BASE_STATE%YGSP_RR%T,          &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    & ZNEBS, ZPOID, ZRCVOTT, ZTCORR, YDMF_PHYS_SURF%GSD_VF%PLSM, ZFLU_NEIJ, YLMF_PHYS_BASE_STATE%YGSP_RR%T,          &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,                        &
    & ZQV, ZFLU_QSAT, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,                &
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR,                      &
    & ZQV, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_BASE_STATE%T,   &
...
!     ---------------------------
  IF ( LMSE ) THEN
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDCPG_GPAR%ZVIEW(JLON,MRAIN)=ZFPLSL(JLON,YDCPG_DIM%KFLEVG)+YDMF_PHYS%OUT%FPLCL(JLON,YDCPG_DIM%KFLEVG)
      YDCPG_GPAR%ZVIEW(JLON,MSNOW)=ZFPLSN(JLON,YDCPG_DIM%KFLEVG)+YDMF_PHYS%OUT%FPLCN(JLON,YDCPG_DIM%KFLEVG)
    ENDDO
...
IF (LINTFLEX) THEN
  ...
  & YDMF_PHYS%OUT%FCQSNG, YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%FRMH, ZMSC_FRMQ,   &
  ...
  & ZPFL_FTKE, ZTENDPTKE, ZTENDEXT, ZTENDEXT_DEP, YLPROCSET )
ENDIF
...
IF (LRCOEF.AND.(YDCPG_DIM%KSTEP == 1)) THEN
  ...
  & ZRDT_COR  , ZRDT_RAB3C, ZRDT_RAB3N, ZRDT_RAB4C, &
  & ZRDT_RAB4N, ZRDT_RAB6C, ZRDT_RAB6N, ZRDT_RAT1C, &
  & ZRDT_RAT1N, ZRDT_RAT2C, ZRDT_RAT2N, ZRDT_RAT3C, &
  & ZRDT_RAT3N, ZRDT_RAT4C, ZRDT_RAT4N, ZRDT_RAT5C, &
  & ZRDT_RAT5N, ZAC_HC)
ENDIF
...
IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
  ...
  & PFHP=ZMSC_FHP, PFP=ZPFL_FP, PFEPFP=YDMF_PHYS%OUT%FEPFP,&
...
IF ((.NOT.LSFORCS)) THEN
  ...
  ELSE
    IF (LLXFUMSE) THEN
      DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDMF_PHYS_SURF%GSP_RR%PT_T0(JROF)=YDCPG_GPAR%ZVIEW(JROF,MVTS)
      ENDDO
    ELSE
      DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDMF_PHYS_SURF%GSP_RR%PT_T1(JROF)=YDCPG_GPAR%ZVIEW(JROF,MVTS)
      ENDDO
    ENDIF
  ENDIF
  IF(LNUDG)THEN
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_TMP%RDG%LCVQ, YDMF_PHYS_TMP%RDG%MU0, ZDSA_C1, ZDSA_C2, ZDSA_CPS, &
  & ZDSA_LHS, ZDSA_RS, ZFLU_CD, ZFLU_CDN, ZFLU_CH, ZFLU_EMIS, &
  & ZFLU_FEVI, ZFLU_NEIJ, ZFLU_QSAT, ZFLU_QSATS, ZFLU_VEG, IMOC_CLPH, &
  & ZMSC_FRMQ, ZMSC_LH, ZMSC_LSCPE, ZMSC_QW, ZMSC_TW, ZPFL_FEFB1, &
