...
& YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0, ZRDG_MU0, ZRDG_MU0LU, ZRDG_MU0M,  &
& ZRDG_MU0N, ZRDG_CVGQ)
...
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART1 (YDCPG_DIM, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH,        &
  & ZSAV_FHPS, ZSAV_GZ0F, ZSAV_GZ0HF, ZSAV_HV, ZSAV_PBLH,   &
  & ZSAV_QSH, ZSAV_UDAL, ZSAV_UDGRO, ZSAV_UDOM, ZSAV_UNEBH, &
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
              ...
              & YDGSGEOM%GEMU, ZRDG_MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_BASE_STATE%T,             &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, ZRDG_MU0, YDGSGEOM%GEMU,        &
      & YDGSGEOM%GELAM, ZRDG_MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO,                      &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YLMF_PHYS_BASE_STATE%S, ZQLIS, ZFLU_QSAT, ZMSC_QW, YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, ZRDG_CVGQ,         &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & ZQO3, YLMF_PHYS_BASE_STATE%T, ZRDG_MMU0, YDCPG_MISC%DHSF, ZFLU_EMIS, ZAER, ZAC,       &
  ...
  IF ( LRAY ) THEN
    SELECT CASE (NRAY)
      CASE(1)
        ...
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
        & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,              &
        & ZQO3, YLMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, ZRDG_MU0,                       &
        & YDGSGEOM%GEMU, YDGSGEOM%GELAM, ZRDG_MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO,       &
        & YDMF_PHYS%OUT%FRTH, ZFRSODS, YDMF_PHYS%OUT%FRSOPS, YDMF_PHYS%OUT%FRSOLU, YDMF_PHYS%OUT%FRTHDS,                    &
        & ZAER, ZMAK, ZMAN)
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & ZRDG_MU0, ZRDG_MU0LU, YLMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD,                           &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZFLU_EMIS, YDMF_PHYS%RAD%EMTD, ZRDG_MU0, &
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,              &
              & YDCPG_DIM%KTDIA, NTSSG, YDGSGEOM%GEMU, ZRDG_MU0, YLMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZRDG_CVGQ,                            &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    ...
    & YLMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%KOZO, ZQO3(1, 1), YLMF_PHYS_BASE_STATE%T, ZRDG_MU0, &
    & YDMF_PHYS%OUT%FCHOZ)
...
IF (LCORWAT) THEN
  CALL MF_PHYS_CORWAT (YDCPG_DIM, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS_SURF)
ENDIF
...
IF (.NOT.LDCONFX) THEN
  ...
  IF (GCHETN%LPROFV)&
   & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, ZRDG_CVGQ, ZRDG_LCVQ,          &
     & ZRDG_MU0, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF,     &
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART2 (YDCPG_DIM, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH,        &
  & ZSAV_FHPS, ZSAV_GZ0F, ZSAV_GZ0HF, ZSAV_HV, ZSAV_PBLH,   &
  & ZSAV_QSH, ZSAV_UDAL, ZSAV_UDGRO, ZSAV_UDOM, ZSAV_UNEBH, &
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDGSGEOM%GECLO, YDGSGEOM%GESLO, ZRDG_CVGQ, ZRDG_LCVQ, ZRDG_MU0,   &
...
CALL MF_PHYS_PRECIPS (YDCPG_DIM, ZPRC_DPRECIPS, ZPRC_DPRECIPS2, &
