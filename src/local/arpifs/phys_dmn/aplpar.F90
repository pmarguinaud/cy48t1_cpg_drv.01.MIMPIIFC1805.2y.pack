!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDCPG_DIM,YLMF_PHYS_STATE,YDCPG_DYN0,YDCPG_MISC,YDMF_PHYS,&
 & YDMF_PHYS_TMP,YDAPLPAR_TMP,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,&
 & NTSSG  , &
 & PDTPHY,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
TYPE (MF_PHYS_STATE_TYPE),INTENT(INOUT):: YLMF_PHYS_STATE
...
INTEGER(KIND=JPIM),INTENT(IN)    :: NTSSG
REAL(KIND=JPRB)   ,INTENT(IN)    :: PDTPHY
LOGICAL           ,INTENT(IN)    :: LLXFUMSE



REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGFL(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NDIM)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PKOZO(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_PHY_G%YRDPHY%NVCLIS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PGPAR(YDCPG_DIM%KLON,YDMODEL%YRML_PHY_MF%YRPARAR%NGPAR+1)




REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZAC(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,0:YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZAC_HC(YDCPG_DIM%KFLEVG+1,YDCPG_DIM%KFLEVG+1)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZDIFEXT(YDCPG_DIM%KLON,0:YDCPG_DIM%KFLEVG,1:YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZP1EZDIAG(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EZDIAG)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTENDPTKE(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTENDEXT_DEP(YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMODEL%YRML_GCONF%YGFL%NGFL_EXT)
TYPE (TRAJ_PHYS_TYPE), INTENT(INOUT) :: PTRAJ_PHYS
TYPE(TYP_DDH), INTENT(INOUT)     :: YDDDH
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZGP2DSPP(YDCPG_DIM%KLON,YSPP%N2D)
...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM, YDGEM=>YDGEOMETRY%YRGEM, YDSTA=>YDGEOMETRY%YRSTA, YDCSGEOM => YDGEOMETRY%YRCSGEOM(YDCPG_DIM%KBL), &
& YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_DIM%KBL), YDOROG=>YDGEOMETRY%YROROG(YDCPG_DIM%KBL), YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY,          &
& YDLDDH=>YDMODEL%YRML_DIAG%YRLDDH, YDTOPH=>YDMODEL%YRML_PHY_MF%YRTOPH, YDRCOEF=>YDMODEL%YRML_PHY_RAD%YRRCOEF,                       &
& YDERDI=>YDMODEL%YRML_PHY_RAD%YRERDI, YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL, YDMCC=>YDMODEL%YRML_AOC%YRMCC,                        &
& YDRIP=>YDMODEL%YRML_GCONF%YRRIP, YDARPHY=>YDMODEL%YRML_PHY_MF%YRARPHY, YDERAD=>YDMODEL%YRML_PHY_RAD%YRERAD,                        &
& YDPHY3=>YDMODEL%YRML_PHY_MF%YRPHY3, YDPHY2=>YDMODEL%YRML_PHY_MF%YRPHY2, YDPHY1=>YDMODEL%YRML_PHY_MF%YRPHY1,                        &
& YDPHY0=>YDMODEL%YRML_PHY_MF%YRPHY0, YGFL=>YDMODEL%YRML_GCONF%YGFL, YDNORGWD=>YDMODEL%YRML_PHY_MF%YRNORGWD,                         &
& YDPARAR=>YDMODEL%YRML_PHY_MF%YRPARAR, YDMSE=>YDMODEL%YRML_PHY_MF%YRMSE, YDPHYDS=>YDMODEL%YRML_PHY_MF%YRPHYDS,                      &
& YDEPHY=>YDMODEL%YRML_PHY_EC%YREPHY)

ASSOCIATE(XMINLM=>YDPHY0%XMINLM, RTCAPE=>YDPHY0%RTCAPE, GRSO=>YDPHY0%GRSO, GCVTSMO=>YDPHY0%GCVTSMO, YSP_SBD=>YDSURF%YSP_SBD,      &
& XKLM=>YDPHY0%XKLM, GAEPS=>YDPHY0%GAEPS, AERCS1=>YDPHY0%AERCS1, AERCS3=>YDPHY0%AERCS3, AERCS5=>YDPHY0%AERCS5,                    &
& HUTIL2=>YDPHY0%HUTIL2, HUTIL1=>YDPHY0%HUTIL1, XMAXLM=>YDPHY0%XMAXLM, TEQK=>YDPHY0%TEQK, HUCOE=>YDPHY0%HUCOE,                    &
& LCVNHD=>YDPHY0%LCVNHD, TEQC=>YDPHY0%TEQC, UHDIFV=>YDPHY0%UHDIFV, HUTIL=>YDPHY0%HUTIL, NPCLO1=>YDPHY0%NPCLO1,                    &
& NPCLO2=>YDPHY0%NPCLO2, RDECRD=>YDPHY0%RDECRD, RDECRD1=>YDPHY0%RDECRD1, RDECRD2=>YDPHY0%RDECRD2, RDECRD3=>YDPHY0%RDECRD3,        &
& RDECRD4=>YDPHY0%RDECRD4, ETKE_MIN=>YDPHY0%ETKE_MIN, HSOLIWR=>YDPHY1%HSOLIWR, ALCRIN=>YDPHY1%ALCRIN,                             &
& ALBMED=>YDPHY1%ALBMED, WSMX=>YDPHY1%WSMX, LALBMERCLIM=>YDPHY1%LALBMERCLIM, HSOLIT0=>YDPHY1%HSOLIT0,                             &
& HSOL=>YDPHY1%HSOL, WPMX=>YDPHY1%WPMX, EMCRIN=>YDPHY1%EMCRIN, EMMMER=>YDPHY1%EMMMER, TMERGL=>YDPHY1%TMERGL,                      &
& RTINER=>YDPHY1%RTINER, EMMGLA=>YDPHY1%EMMGLA, TSPHY=>YDPHY2%TSPHY, LRAFTKE=>YDPHY2%LRAFTKE, LRAFTUR=>YDPHY2%LRAFTUR,            &
...
& LMDUST=>YDARPHY%LMDUST, LMSE=>YDARPHY%LMSE, LSURFEX_KFROM=>YDARPHY%LSURFEX_KFROM, YA=>YGFL%YA, NGFL_EZDIAG=>YGFL%NGFL_EZDIAG,   &
& YFQTUR=>YGFL%YFQTUR, YFSTUR=>YGFL%YFSTUR, YIRAD=>YGFL%YIRAD, YLRAD=>YGFL%YLRAD, XZSEPS=>YDMSE%XZSEPS,                           &
& LTRAJPS=>YDSIMPHL%LTRAJPS, LVDIFSPNL=>YDSIMPHL%LVDIFSPNL, LGWDSPNL=>YDSIMPHL%LGWDSPNL, LRAYSP=>YDSIMPHL%LRAYSP,                 &
...
& LPTKE=>YDPHY%LPTKE, LCOEFK_RIS=>YDPHY%LCOEFK_RIS, LAEROLAN=>YDPHY%LAEROLAN, LNEBECT=>YDPHY%LNEBECT,                             &
& LCVPGY=>YDPHY%LCVPGY, LAERODES=>YDPHY%LAERODES, LNEWSTAT=>YDPHY%LNEWSTAT, LTHERMO=>YDPHY%LTHERMO, LO3FL=>YDPHY%LO3FL,           &
& LRRGUST=>YDPHY%LRRGUST, LPHSPSH=>YDPHY%LPHSPSH, LSNV=>YDPHY%LSNV, LECSHAL=>YDPHY%LECSHAL, LECT=>YDPHY%LECT,                     &
& LSTRAPRO=>YDPHY%LSTRAPRO, LDIFCONS=>YDPHY%LDIFCONS, LNODIFQC=>YDPHY%LNODIFQC, LAEROVOL=>YDPHY%LAEROVOL,                         &
& LRSTAER=>YDPHY%LRSTAER, LOZONE=>YDPHY%LOZONE, NCALLRAD=>YDPHY%NCALLRAD, LNCVPGY=>YDPHY%LNCVPGY, LRAYLU=>YDPHY%LRAYLU,           &
& LAEROSUL=>YDPHY%LAEROSUL, LO3ABC=>YDPHY%LO3ABC, LSTRAS=>YDPHY%LSTRAS, LCOEFK_PTTE=>YDPHY%LCOEFK_PTTE,                           &
& LSFHYD=>YDPHY%LSFHYD, LAEROSEA=>YDPHY%LAEROSEA, NDIFFNEB=>YDPHY%NDIFFNEB, LEDKF=>YDPHY%LEDKF, LRCVOTT=>YDPHY%LRCVOTT,           &
& LNEBR=>YDPHY%LNEBR, LNEBN=>YDPHY%LNEBN, LMPHYS=>YDPHY%LMPHYS, LZ0HSREL=>YDPHY%LZ0HSREL, LCAMOD=>YDPHY%LCAMOD,                   &
& LCOMOD=>YDPHY%LCOMOD, LCONDWT=>YDPHY%LCONDWT, LCVCSD=>YDPHY%LCVCSD, LNSDO=>YDPHY%LNSDO, LUDEVOL=>YDPHY%LUDEVOL,                 &
& LRAY=>YDPHY%LRAY, LGWD=>YDPHY%LGWD, LCOEFKTKE=>YDPHY%LCOEFKTKE, L3MT=>YDPHY%L3MT, LRAYFM=>YDPHY%LRAYFM,                         &
...
& NRAY=>YDPHY%NRAY, LDAYD=>YDPHY%LDAYD, LEDMFI=>YDPHY%LEDMFI, LGPCMT=>YDPHY%LGPCMT, LFPCOR=>YDPHY%LFPCOR,                         &
& LRPROX=>YDPHY%LRPROX, LPROCLD=>YDPHY%LPROCLD, LACDIFUS=>YDPHY%LACDIFUS, LCAPE=>YDPHY%LCAPE, LCVRAV3=>YDPHY%LCVRAV3,             &
& LHMTO=>YDPHY%LHMTO, LVGSN=>YDPHY%LVGSN, LCVPPKF=>YDPHY%LCVPPKF, LCVPRO=>YDPHY%LCVPRO, LADJCLD=>YDPHY%LADJCLD,                   &
& LGRAPRO=>YDPHY%LGRAPRO, RDECLI=>YDRIP%RDECLI, MVTS=>YDPARAR%MVTS, MALBSCA=>YDPARAR%MALBSCA, MVQS=>YDPARAR%MVQS,                 &
& MRAIN=>YDPARAR%MRAIN, CMF_CLOUD=>YDPARAR%CMF_CLOUD, MALBDIR=>YDPARAR%MALBDIR, XSW_BANDS=>YDPARAR%XSW_BANDS,                     &
& MVEMIS=>YDPARAR%MVEMIS, MGZ0H=>YDPARAR%MGZ0H, MGZ0=>YDPARAR%MGZ0, MCD=>YDPARAR%MCD, NSWB_MNH=>YDPARAR%NSWB_MNH,                 &
& MSWDIR=>YDPARAR%MSWDIR, LMIXUV=>YDPARAR%LMIXUV, MSWDIF=>YDPARAR%MSWDIF, MSNOW=>YDPARAR%MSNOW, CMF_UPDRAFT=>YDPARAR%CMF_UPDRAFT, &
& NTRADI=>YDTOPH%NTRADI, NTPLUI=>YDTOPH%NTPLUI, NTNEBU=>YDTOPH%NTNEBU, NTDIFU=>YDTOPH%NTDIFU, NTOZON=>YDTOPH%NTOZON,              &
...
& LXVISI=>YDXFU%LXVISI, LFLEXDIA=>YDLDDH%LFLEXDIA, LXVISI2=>YDXFU%LXVISI2, LDDH_OMP=>YDLDDH%LDDH_OMP,                             &
& LMCC03=>YDMCC%LMCC03, LRCOEF=>YDRCOEF%LRCOEF, NSTOP=>YDRIP%NSTOP, RCODEC=>YDRIP%RCODEC, RHGMT=>YDRIP%RHGMT,                     &
& RSIDEC=>YDRIP%RSIDEC, RSOVR=>YDRIP%RSOVR, RSTATI=>YDRIP%RSTATI, TSTEP=>YDRIP%TSTEP, LDPRECIPS=>YDPHY%LDPRECIPS,                 &
& LDPRECIPS2=>YDPHY%LDPRECIPS2, NDTPREC=>YDPHY%YRDPRECIPS%NDTPREC, NDTPREC2=>YDPHY%YRDPRECIPS%NDTPREC2,                           &
& NDTPRECCUR=>YDPHY%YRDPRECIPS%NDTPRECCUR, NDTPRECCUR2=>YDPHY%YRDPRECIPS%NDTPRECCUR2, STPRE=>YDSTA%STPRE,                         &
& STPREH=>YDSTA%STPREH, STTEM=>YDSTA%STTEM, NORGWD_NNOVERDIF=>YDNORGWD%NORGWD_NNOVERDIF, LGCHECKMV=>YDPHY%LGCHECKMV,              &
& NAERO=>YGFL%NAERO, NGFL_EXT=>YGFL%NGFL_EXT, LCHEM_ARPCLIM=>YDMODEL%YRML_CHEM%YRCHEM%LCHEM_ARPCLIM,                              &
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,          &
              & YDCPG_DIM%KSTEP, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, &
              & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0,                        &
              & YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%Q, YLMF_PHYS_STATE%YGSP_RR%T                    &
              & )
!     ------------------------------------------------------------------
...
IF (LMPHYS) THEN
  IF (LOZONE) THEN
    ...
    DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,JLEV) = MAX(ZEPSO3,YLMF_PHYS_STATE%O3(JLON,JLEV))
      ENDDO
    ENDDO
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, &
      & YDGSGEOM%GEMU, ZROZ)
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,0)=1.E-9_JPRB
      ENDDO
      DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZQO3(JLON,JLEV)=ZROZ(JLON,JLEV)*YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF

  ELSE
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, ZQO3, .FALSE., YLMF_PHYS_STATE%YCPG_PHY%PREHYD, &
...
!     LAYER WEIGHTS
      ...
      ZPOID(JLON,JLEV)=YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZGDTI
      ZIPOI(JLON,JLEV)=YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP(JLON,JLEV)*ZGDT

!     CALCULATION OF LATENT HEATS

      ZLHV(JLON,JLEV)=FOLH(YLMF_PHYS_STATE%T(JLON,JLEV),0.0_JPRB)
      ZLHS(JLON,JLEV)=FOLH(YLMF_PHYS_STATE%T(JLON,JLEV),1.0_JPRB)
...
!  SIZE OF ARRAY FOR MSE
  ...
  ZINVDT=1/PDTPHY
  ZINVG=1/RG
  ZZI_APHI=YLMF_PHYS_STATE%YCPG_DYN%PHI
  ZTM=YLMF_PHYS_STATE%T
!SETUP
  ZZI_SVM=0.0_JPRB
...
!Initialisation des scalaires passifs pour aro_ground_param
  DO JGFL=1,NGFL_EXT
    DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
       ZSVM(JLON,JLEV,JGFL)=MAX(YLMF_PHYS_STATE%P1EXT(JLON,JLEV,JGFL),0.0_JPRB)
      ENDDO
    ENDDO
  ENDDO
...
!Initialisation de ZZI_RHODREFM
  DO JLEV = 1 , YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZZI_RHODREFM(JLON,1,JLEV)=YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,JLEV)/&
       & (YLMF_PHYS_STATE%T(JLON,JLEV)*YLMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,JLEV))
     ENDDO
  ENDDO
...
!Initialisation de ZZI_PABSM
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    DO JLEV = 1 , YDCPG_DIM%KFLEVG
      ZZI_PABSM(JLON,1,JLEV)=YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,JLEV)
    ENDDO
  ENDDO
...
!Initialisation de ZZI_EXNREFM
  ...
  DO JLEV = 1 , YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZZI_EXNREFM(JLON,1,JLEV)=(YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,JLEV)*ZINVATM)**(ZRSCP)
    ENDDO
  ENDDO
...
!cdir unroll=8
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZQI(JLON,JLEV)=MAX(0.0_JPRB,YLMF_PHYS_STATE%I(JLON,JLEV))
          ZQL(JLON,JLEV)=MAX(0.0_JPRB,YLMF_PHYS_STATE%L(JLON,JLEV))
          ZQR(JLON,JLEV)=MAX(0.0_JPRB,YLMF_PHYS_STATE%R(JLON,JLEV))
          ZQS(JLON,JLEV)=MAX(0.0_JPRB,YLMF_PHYS_STATE%S(JLON,JLEV))
          ZQG(JLON,JLEV)=MAX(0.0_JPRB,YLMF_PHYS_STATE%G(JLON,JLEV))
  ...
  !                         FOR RAIN/SNOW/GRAUPEL PUT IN PFCQR/S/GNG
          ...
          ZDQI=ZQI(JLON,JLEV)-YLMF_PHYS_STATE%I(JLON,JLEV)
          ZDQL=ZQL(JLON,JLEV)-YLMF_PHYS_STATE%L(JLON,JLEV)
          ZDQR=ZQR(JLON,JLEV)-YLMF_PHYS_STATE%R(JLON,JLEV)
          ZDQS=ZQS(JLON,JLEV)-YLMF_PHYS_STATE%S(JLON,JLEV)
          ZDQG=ZQG(JLON,JLEV)-YLMF_PHYS_STATE%G(JLON,JLEV)
          ZDQC=ZDQI+ZDQL+ZDQR+ZDQS+ZDQG

          ZQV0=YLMF_PHYS_STATE%Q(JLON,JLEV)-ZIPOI(JLON,JLEV)*(0.0_JPRB- YDMF_PHYS%OUT%FCQNG(JLON,JLEV-1) &
          ...
          ZDQV=MAX(0.0_JPRB,ZQV0-ZDQC)-YLMF_PHYS_STATE%Q(JLON,JLEV)
  ...
  ELSE
    DO JLEV = 1, YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQI(JLON,JLEV)=YLMF_PHYS_STATE%I(JLON,JLEV)
        ZQL(JLON,JLEV)=YLMF_PHYS_STATE%L(JLON,JLEV)
        ZQR(JLON,JLEV)=YLMF_PHYS_STATE%R(JLON,JLEV)
        ZQS(JLON,JLEV)=YLMF_PHYS_STATE%S(JLON,JLEV)
        IF (LGRAPRO) THEN
          ZQG(JLON,JLEV)=YLMF_PHYS_STATE%G(JLON,JLEV)
        ENDIF
        ZQV(JLON,JLEV)=YLMF_PHYS_STATE%Q(JLON,JLEV)
      ENDDO
    ENDDO
  ENDIF
ELSE
...
DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  ZTAUX   (JLON)   = ZEPS0
  YDAPLPAR_TMP%MOC%CLPH   (JLON)   = YDCPG_DIM%KFLEVG
ENDDO
DO JSG = 1, NSW
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
IF((LNEBN.OR.LNEBR.OR.LRRGUST).AND.YDCPG_DIM%KSTEP == 0) THEN
  DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDAPLPAR_TMP%PFL%FPLCH(JLON,JLEV)=0.0_JPRB
    ENDDO
  ENDDO
ENDIF
IF(LRRGUST.AND.YDCPG_DIM%KSTEP == 0) THEN
  DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YDAPLPAR_TMP%PFL%FPLSH(JLON,JLEV)=0.0_JPRB
    ENDDO
  ENDDO
ENDIF
IF(LCVPGY.AND.YDCPG_DIM%KSTEP == 0) THEN
  DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      YLMF_PHYS_STATE%CVV(JLON,JLEV)=0.0_JPRB
    ENDDO
  ENDDO
ENDIF
...
IF(LCOEFKTKE.AND.YDCPG_DIM%KSTEP == 0) THEN
  YDMF_PHYS_SURF%GSD_VH%PQSH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YLMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
ENDIF
...
!     ----------------------------
  IF ( LTHERMO ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, YLMF_PHYS_STATE%T,        &
    & ZGEOSLC, YDAPLPAR_TMP%MSC%LH, YDAPLPAR_TMP%MSC%LSCPE, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, &
    & YDCPG_MISC%RH, YDAPLPAR_TMP%MSC%TW)
  ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZTSN(JLON)=YLMF_PHYS_STATE%YGSP_RR%T(JLON)
        ZTN(JLON) =YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)
     ENDDO
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZRHODREFM(JLON) = YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,YDCPG_DIM%KFLEVG)/(YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)*YLMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG))
        ZTHETAS(JLON)   = ZTSN(JLON)*(RATM/YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG))**RKAPPA
     ENDDO
     LLAROME = .FALSE.
     ...
     & ZTN, ZTSN, YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_STATE%Q(:, YDCPG_DIM%KFLEVG), YLMF_PHYS_STATE%U(:, YDCPG_DIM%KFLEVG), &
     & YLMF_PHYS_STATE%V(:, YDCPG_DIM%KFLEVG), ZTHETAS, YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                &
     & ZFEV, ZFMDU, ZFMDV)
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YLMF_PHYS_STATE%YGSP_RR%T(JLON)=ZTSN(JLON)
     ENDDO
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-YLMF_PHYS_STATE%YGSP_RR%T(JLON)))
        ...
        YDMF_PHYS%OUT%FCLL(JLON,1)=YDMF_PHYS%OUT%FEVL(JLON,1)*YDAPLPAR_TMP%MSC%LH(JLON,YDCPG_DIM%KFLEVG)
        YDMF_PHYS%OUT%FCLN(JLON,1)=YDMF_PHYS%OUT%FEVN(JLON,1)*YDAPLPAR_TMP%MSC%LH(JLON,YDCPG_DIM%KFLEVG)
        YDAPLPAR_TMP%DSA%LHS(JLON)=FOLH(YLMF_PHYS_STATE%YGSP_RR%T(JLON),0.0_JPRB)
     ENDDO
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS_SURF%GSD_VV%PLAI, YLMF_PHYS_STATE%YGSP_SG%A,  &
      & YLMF_PHYS_STATE%YGSP_SG%R, YDMF_PHYS_SURF%GSD_VV%PSAB, YLMF_PHYS_STATE%YGSP_SG%F, YLMF_PHYS_STATE%YGSP_RR%T,     &
      & YDMF_PHYS_SURF%GSD_VF%PVEG, YLMF_PHYS_STATE%YGSP_SB%Q, YLMF_PHYS_STATE%YGSP_SB%TL, YLMF_PHYS_STATE%YGSP_RR%W,    &
      & YLMF_PHYS_STATE%YGSP_RR%IC, LLHMT, YDAPLPAR_TMP%DSA%C1, YDAPLPAR_TMP%DSA%C2, ZC3, ZCG, ZCN, YDMF_PHYS%OUT%CT,    &
...
!DEC$ IVDEP
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDMF_PHYS%OUT%CT(JLON)=HSOL /(&
         & 1.0_JPRB+HSOLIWR*(YLMF_PHYS_STATE%YGSP_RR%W(JLON)+YLMF_PHYS_STATE%YGSP_SB%Q(JLON,1))/(WSMX+WPMX)&
         & *EXP(-0.5_JPRB*(HSOLIT0*(YLMF_PHYS_STATE%YGSP_RR%T(JLON)-RTT))**2))
      ENDDO
    ENDIF
  ENDIF
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ...
      YDAPLPAR_TMP%FLU%EMIS    (JLON) = PGPAR(JLON,MVEMIS)
      YDCPG_MISC%QS      (JLON) = PGPAR(JLON,MVQS)
      YLMF_PHYS_STATE%YGSP_RR%T      (JLON) = PGPAR(JLON,MVTS)
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      IF (YDAPLPAR_TMP%FLU%EMIS(JLON)==0._JPRB) THEN
        YDAPLPAR_TMP%FLU%EMIS(JLON) = 0.99_JPRB
        YLMF_PHYS_STATE%YGSP_RR%T  (JLON) = 288.0_JPRB
    ...
    IF (LMSE) THEN
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (YDCPG_DIM%KSTEP == 0) THEN 
         ...
         & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NDGUNG, NDGUXG, NDLUNG, NDLUXG, YDCSGEOM%RINDX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),  &
         & YDCSGEOM%RINDY(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), LSURFEX_KFROM, YDMF_PHYS%OUT%GZ0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), &
         & YDMF_PHYS%OUT%GZ0H(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA))
        ENDIF
        ...
        & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,        &
        & YLMF_PHYS_STATE%T, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, YLMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, &
        & YDAPLPAR_TMP%FLU%CDN, ZCDNH, YDAPLPAR_TMP%DSA%CPS, ZRTI, YDAPLPAR_TMP%DSA%RS)
         
      ELSE
        CALL ACHMTLS ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,       &
        & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,               &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U,     &
        & YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, ZDPHIT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, &
        & LLCLS, ZNBVNO, ZMRIPP, YDAPLPAR_TMP%DSA%CPS, ZGWDCS, YDAPLPAR_TMP%DSA%LHS, ZPCLS, YDAPLPAR_TMP%FLU%CD,      &
        & YDAPLPAR_TMP%FLU%CDN)
!       Computation of ZRTI
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZDPHI(JLON)=YLMF_PHYS_STATE%YCPG_DYN%PHIF(JLON,YDCPG_DIM%KFLEVG)-YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
          ZPRS(JLON)=RD+ZRVMD*YDCPG_MISC%QS(JLON)
          ZRTI(JLON)=2.0_JPRB/(YLMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG)*YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)+RKAPPA*ZDPHI(JLON)&
           & +ZPRS(JLON)*YLMF_PHYS_STATE%YGSP_RR%T(JLON))
        ENDDO
      ENDIF
    ELSE
      IF (LCOEFKSURF) THEN
        ...
        & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                          &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,                &
        & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH,               &
        & ZDPHIT, ZDPHIV, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,                  &
        & YDMF_PHYS_SURF%GSD_VV%PHV, YDMF_PHYS_SURF%GSD_VF%PLSM, ZNEIJG, ZNEIJV, YLMF_PHYS_STATE%YGSP_SG%F,                      &
        & YLMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VF%PVEG, ZWFC, YLMF_PHYS_STATE%YGSP_RR%W, YLMF_PHYS_STATE%YGSP_RR%IC,    &
        & LLCLS, LLHMT, ZNBVNO, ZMRIPP, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, YDAPLPAR_TMP%FLU%CH,                  &
        & ZCHROV, YDAPLPAR_TMP%DSA%CPS, ZDQSTS, ZGWDCS, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZHQ,                              &
        & ZHU, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS%OUT%QCLS, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%QSATS, YDMF_PHYS%OUT%RHCLS,            &
        & YDAPLPAR_TMP%DSA%RS, ZRTI, ZSTAB, YDMF_PHYS%OUT%TCLS, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS,                          &
        & YDMF_PHYS%OUT%NUCLS, YDMF_PHYS%OUT%NVCLS, ZPCLS, YDAPLPAR_TMP%FLU%VEG, ZXDROV, ZXHROV, YDMF_PHYS%OUT%UGST,             &
        & YDMF_PHYS%OUT%VGST)
      ELSE
    ...
    IF (LPTKE) THEN
      YLMF_PHYS_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG) = MAX(YLMF_PHYS_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ETKE_MIN)
    ENDIF
    IF (LCOEFK_PTTE) THEN
    ...
    ELSE
      ZCP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG) = YLMF_PHYS_STATE%YCPG_DYN%RCP%CP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
    ENDIF
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                    &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,         &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDAPLPAR_TMP%MSC%LSCPE, &
      & YDMF_PHYS%OUT%GZ0, ZMN2PP, ZMRIPP)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZTHETAV(JLON,JLEV)=YLMF_PHYS_STATE%T(JLON,JLEV)*(1.0_JPRB+RETV*ZQV(JLON,JLEV))&
           & *(RATM/YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,JLEV))**RKAPPA  
        ENDDO
      ENDDO
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZTHETAVS(JLON)=YLMF_PHYS_STATE%YGSP_RR%T(JLON)*(1.0_JPRB+RETV*YDCPG_MISC%QS(JLON))&
         & *(RATM/YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG))**RKAPPA  
      ENDDO
      CALL ACCLPH ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA,             &
      & YDCPG_DIM%KFLEVG, ZTHETAV, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%U, &
      & YLMF_PHYS_STATE%V, ZTHETAVS, YDAPLPAR_TMP%MOC%CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%VEIN,                &
      & ZUGST, ZVGST)
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      & .FALSE., YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, ZBLH, YDMF_PHYS%OUT%GZ0,       &
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, &
      & YLMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZLMU, ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, &
      & YLMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZZLMT, ZLMT)
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      CALL ACMIXLENZ(YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, 1, YDCPG_DIM%KFLEVG, &
      & .TRUE., YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, ZBLH, YDMF_PHYS%OUT%GZ0,      &
      & YDMF_PHYS%OUT%GZ0H, ZLMU, ZLMT)
      ZLMU2(:,:)=ZLMU(:,:)
      ZLMT2(:,:)=ZLMT(:,:)
      ...
      IF( LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, IMLTYPE, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,      &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%T, ZQV,                    &
        & ZQL, ZQI, YLMF_PHYS_STATE%R, YLMF_PHYS_STATE%S, YLMF_PHYS_STATE%TKE, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,          &
        & YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,   &
        & ZMN2PP, ZFMGST, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH, YDMF_PHYS%OUT%GZ0, YLMF_PHYS_STATE%YGSP_RR%T, &
        & YDAPLPAR_TMP%FLU%CDN, ZBLH, ZLMU, ZLMT, YDVARS%MXL%T0, ZLML, ZLMLTILD, ZRRCOR, LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,           &
        & YDAPLPAR_TMP%MSC%LSCPE, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,                   &
        & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0,                         &
        & ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTCOEF, YDCPG_DIM%KFLEVG,                  &
      & YDCPG_DIM%KSTEP, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,              &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZQV, ZQL, ZQI, ZCP, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, &
      & YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,                            &
      & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0,                 &
      & YLMF_PHYS_STATE%TKE, YDVARS%TTE%T0, YLMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PQSH, YDAPLPAR_TMP%DSA%RS,             &
      & YDAPLPAR_TMP%DSA%CPS, ZRTI, YDMF_PHYS%OUT%GZ0, LLCLS, ZMRIPP, ZMRIFPP, ZBNEBCVPP, ZBNEBQ,                                   &
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZCRTI(JLON) = 1.0_JPRB/(YLMF_PHYS_STATE%YGSP_RR%T(JLON)*YDAPLPAR_TMP%DSA%RS(JLON))
      ENDDO
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,         &
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                 &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%Q, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, &
      & ZDPHIT, ZDPHIV, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%QSATS, ZCRTI,                 &
      & YLMF_PHYS_STATE%YGSP_RR%T, LLHMT, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCE,                     &
      & ZCEROV, YDAPLPAR_TMP%FLU%CH, ZCHROV, YDMF_PHYS%OUT%QCLS, YDMF_PHYS%OUT%RHCLS, YDMF_PHYS%OUT%TCLS,             &
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST)
    ELSE
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZCE   (JLON) = YDAPLPAR_TMP%FLU%CH   (JLON)
  ...
  IF (LRAFTKE) THEN
    ...
    & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE,                &
    & ZDCAPE, YLMF_PHYS_STATE%TKE, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, &
    & ZBLH, YDAPLPAR_TMP%MOC%CLPH)
  ENDIF
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      ...
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,               &
      & ZFMTKE, ZFTTKE, ZAUTKE, ZATTKE, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U,       &
      & YLMF_PHYS_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, ZKTROV, ZKUROV, ZKNROV, ZXTROV, ZXUROV,                   &
      & ZXPTKEROV)
    ELSE
      ...
      & YDAPLPAR_TMP%MSC%LSCPE, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,                 &
      & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH,      &
      & ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZKTROV, ZKUROV, ZKNROV, ZNBVNO,                      &
      & ZXTROV, ZXUROV, ZXPTKEROV)
    ENDIF
  ENDIF

  IF (LCVPPKF) THEN
    CALL ACVPPKF( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTCVIM, YDCPG_DIM%KFLEVG, &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,          &
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, ZQV, ZQL, ZQI, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,      &
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%TKE, ZDIFCVPPQ,                 &
  ...
  IF (LEDKF) THEN
    ...
    & ZIMPL, TSPHY, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, &
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, CMF_UPDRAFT, CMF_CLOUD, LMIXUV, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,     &
    & YLMF_PHYS_STATE%T, ZQV, ZQL, ZQI, ZQR, ZQS, YLMF_PHYS_STATE%TKE, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,         &
    & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZEDMFQ, ZEDMFS, ZEDMFU, ZEDMFV, YDMF_PHYS_SURF%GSD_SFL%PGROUP(:, 1),    &
...
! Computation of the 2 znlab used in acbl89
    ...
    IF(LNEBN.OR.LNEBR.OR.LRRGUST) THEN
      DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
         ...
         & *MAX(0.0_JPRB,SIGN(1.0_JPRB,YDAPLPAR_TMP%PFL%FPLCH(JLON,JLEV)-YDAPLPAR_TMP%PFL%FPLCH(JLON,JLEV-1)-ZEPS))
    ...
    & YDCPG_DIM%KLON, NTCOEF, NTCOET, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,  &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,           &
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, ZQV,                   &
    & ZQIC, ZQLC, YDAPLPAR_TMP%MSC%LSCPE, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CH, YDMF_PHYS%OUT%GZ0,                &
    & YLMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YLMF_PHYS_STATE%TKE,                 &
    & ZPRODTH_CVPP, ZNLAB, ZNLABCVP, ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZXTROV, ZXUROV, ZNBVNO, ZNEBS,                  &
    & ZQLIS, ZNEBS0, ZQLIS0, ZCOEFN, YDAPLPAR_TMP%PFL%FTKE, YDAPLPAR_TMP%PFL%FTKEI, ZTKE1, ZTPRDY, YDMF_PHYS%OUT%EDR, &
    & YDDDH)
    ...
    DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      IF (LSNV) THEN
        ...
        YDMF_PHYS%OUT%ALB(JLON)= (1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PVEG(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
         ...
         & MAX(YDMF_PHYS_SURF%GSD_VF%PALBSF(JLON),YLMF_PHYS_STATE%YGSP_SG%A(JLON,1))&
         & + YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*ZNEIJV(JLON) *&
         & MAX(ZALBV,YLMF_PHYS_STATE%YGSP_SG%A(JLON,1))&
         & + YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*(1.0_JPRB-ZNEIJV(JLON)) * ZALBV
        YDAPLPAR_TMP%FLU%EMIS(JLON)= (1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PVEG(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
      ...
      ELSE
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-YDAPLPAR_TMP%FLU%VEG(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & YDAPLPAR_TMP%FLU%VEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+YDAPLPAR_TMP%FLU%NEIJ(JLON)*YLMF_PHYS_STATE%YGSP_SG%A(JLON,1)
          ELSE
            ! old treatment, PNEIJ is snow fraction for bare ground
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & YLMF_PHYS_STATE%YGSP_SG%A(JLON,1))+(YDAPLPAR_TMP%FLU%NEIJ(JLON)-ZNEIJV(JLON))*     &
             & YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*(YDMF_PHYS_SURF%GSD_VV%PALV(JLON)-YLMF_PHYS_STATE%YGSP_SG%A(JLON,1))
          ENDIF
          ...
          YDAPLPAR_TMP%FLU%EMIS(JLON)=YDMF_PHYS_SURF%GSD_VF%PEMISF(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PEMISF(JLON)-EMCRIN)
      ...
      IF (LALBMERCLIM) THEN
        DO JSG=1,NSW
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            ...
            ZALBPMER=(1.0_JPRB+&
             & 0.5_JPRB*YDMF_PHYS_TMP%RDG%MU0M(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))/   (&
             & 1.0_JPRB+YDMF_PHYS_TMP%RDG%MU0M(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))**2
...
!DEC$ IVDEP
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZMERL(JLON)=(1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PLSM(JLON))*(1.0_JPRB&
            & -MAX(0.0_JPRB,SIGN(1.0_JPRB,TMERGL-YLMF_PHYS_STATE%YGSP_RR%T(JLON))))
          YDAPLPAR_TMP%FLU%EMIS(JLON)=YDAPLPAR_TMP%FLU%EMIS(JLON)*YDMF_PHYS_SURF%GSD_VF%PLSM(JLON)+ZMERL(JLON)*EMMMER&
    ...
    ELSEIF (LRAY) THEN
      ...
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        IF ( YDMF_PHYS_SURF%GSD_VF%PLSM(JLON) < 0.5_JPRB .AND. YLMF_PHYS_STATE%YGSP_RR%T(JLON) >= TMERGL ) THEN
        ...
        ZALBDIR(JLON)=ZLAMB*YDMF_PHYS%OUT%ALB(JLON)+(1._JPRB-ZLAMB)*(1._JPRB+&
         & 0.5_JPRB*YDMF_PHYS_TMP%RDG%MU0(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))/   (&
         & 1.0_JPRB+YDMF_PHYS_TMP%RDG%MU0(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))**2
      ENDDO
    ENDIF
  ...
  & .OR.  ( (LRAY.OR.LRAYSP).AND.(.NOT.LRSTAER)) ) THEN
    ...
    IF ( ( (LRAYFM.AND.NAER /= 0) .OR.LRAY.OR.LRAYSP).AND.LLAERO )  THEN
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%T,    &
      & YLMF_PHYS_STATE%YGSP_RR%T, ZAESEA, ZAELAN, ZAESOO, ZAEDES, ZAESUL, ZAEVOL, ZAER, ZAERINDS                  &
      &                                  )
    ENDIF
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & NTRADI, YDCPG_DIM%KFLEVG, IJN, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,         &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,     &
      & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,        &
      & ZQO3, YLMF_PHYS_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0,     &
      & YDGSGEOM%GEMU, YDGSGEOM%GELAM, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO, &
      & YDMF_PHYS%OUT%FRTH, ZFRSODS, YDMF_PHYS%OUT%FRSOPS, YDMF_PHYS%OUT%FRSOLU, YDMF_PHYS%OUT%FRTHDS,         &
      & ZAER, ZMAK, ZMAN)
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                       &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,                    &
    & YDAPLPAR_TMP%MSC%LH, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQCI, ZQCL, ZQLIS, YDAPLPAR_TMP%FLU%QSAT,                   &
    & YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T,              &
    & YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDAPLPAR_TMP%DSA%CPS, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T, &
    & YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL,               &
    & YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN,           &
    & ZFHMLTSC, ZFHEVPPC, ZFPEVPPC, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, ZNEBC, ZQLIC, YDMF_PHYS%OUT%STRCU,               &
    & YDMF_PHYS%OUT%STRCV, ICIS, INLAB, INND, YLMF_PHYS_STATE%CVV)
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,  &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,             &
    & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%T, &
    & ZNEBCH, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1,                    &
    & ZQLIS0, ZNEBS0)
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & INBTRA, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,                  &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDCPG_DYN0%CTY%VVEL(:, 1:),                           &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDAPLPAR_TMP%MSC%LH, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, YLMF_PHYS_STATE%Q, ZQI, ZQL, YLMF_PHYS_STATE%R, YLMF_PHYS_STATE%S,                    &
      & ZQLIS, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_TMP%RDG%CVGQ,           &
      & YLMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T,      &
      & YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, ZEDMFQ, YDMF_PHYS%OUT%DIFCQL,                  &
      & YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQLC, YDMF_PHYS%OUT%DIFCQIC, ZEDMFS, ZDIFCVTH,                                    &
      & ZTMPPRODTH, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, ZEDMFU, ZEDMFV, ZSTRCTRA(:, :, INBTRA_DEP:INBTRA),                 &
    ...
    ELSE
      ...
      & YLMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T,      &
      & YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, YDMF_PHYS%OUT%DIFCQ,                           &
      & YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQLC, YDMF_PHYS%OUT%DIFCQIC, YDMF_PHYS%OUT%DIFCS,           &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,             &
    & YLMF_PHYS_STATE%T, YDAPLPAR_TMP%PFL%FPLCH, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZUNEBH, ZNEBS0, ZQLIS0, ZQLI_CVP, ZNEB_CVPP, ZQLI_CVPP,                         &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    ...
    & NTCOEF, NTNEBU, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,                    &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDAPLPAR_TMP%MSC%LSCPE, ZQV,                   &
    & YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,   &
    & YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%GZ0, YDAPLPAR_TMP%PFL%FPLCH, &
    & YDMF_PHYS_SURF%GSD_VH%PPBLH, YDCPG_MISC%QS, YLMF_PHYS_STATE%YGSP_RR%T, ZKTROV, ZKUROV, ZNBVNO,                    &
    & YDCPG_MISC%NEB, ZNEBS, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQLIS)
  ENDIF
...
!         Diagnostique de nebulosite partielle.
  ...
  & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,              &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
    & ZZNEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQO3, YLMF_PHYS_STATE%T, YDMF_PHYS_TMP%RDG%MMU0,                    &
    & YDCPG_MISC%DHSF, YDAPLPAR_TMP%FLU%EMIS, ZAER, ZAC, ZAC_HC, YDAPLPAR_TMP%RDT%COR, YDAPLPAR_TMP%RDT%RAB3C,                &
    & YDAPLPAR_TMP%RDT%RAB3N, YDAPLPAR_TMP%RDT%RAB4C, YDAPLPAR_TMP%RDT%RAB4N, YDAPLPAR_TMP%RDT%RAB6C,                         &
    & YDAPLPAR_TMP%RDT%RAB6N, YDAPLPAR_TMP%RDT%RAT1C, YDAPLPAR_TMP%RDT%RAT1N, YDAPLPAR_TMP%RDT%RAT2C,                         &
    & YDAPLPAR_TMP%RDT%RAT2N, YDAPLPAR_TMP%RDT%RAT3C, YDAPLPAR_TMP%RDT%RAT3N, YDAPLPAR_TMP%RDT%RAT4C,                         &
    & YDAPLPAR_TMP%RDT%RAT4N, YDAPLPAR_TMP%RDT%RAT5C, YDAPLPAR_TMP%RDT%RAT5N)
  ENDIF
...
!     7.3.6 Module chimique - Chemistry module
  ...
  IF (LCHEM_ARPCLIM) THEN ! at this stage call when ARPEGE-Climat chemistry only
     ...
     DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
       DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
         ZDELP(JLON,JLEV) = YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,JLEV) - YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,JLEV-1)
       ENDDO
     ENDDO
     ZWND  (:)      = 0._JPRB  ! not used in  ARPEGE-Climat chemistry
     ZDUMMY1 (:,:)  = 1.0E-18_JPRB
     ZGELAT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA) = ASIN(YDGSGEOM%GEMU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA))
     ZCFLX (:,:)        = 0._JPRB
     ZCFLXO (:,:)       = 0._JPRB
     ZCHEMDV (:,:)      = 0._JPRB
...
!DEC$ IVDEP
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          IF ( YDMF_PHYS%OUT%FRSOPS(JLON) > RSUNDUR*YDMF_PHYS_TMP%RDG%MU0(JLON) ) THEN
      ...
      CASE(2)
        ...
        & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDCPG_DIM%KSTEP, YDCFU%NFRRC, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,               &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,          &
        & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,             &
        & ZQO3, YLMF_PHYS_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDGSGEOM%GELAM,                 &
        & YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_TMP%RDG%MU0LU, YLMF_PHYS_STATE%YGSP_RR%T,                   &
        & ZDECRD, ZCLCT_RAD, YDMF_PHYS%OPT%GDEOSI, YDMF_PHYS%OPT%GUEOSI, YDMF_PHYS%OPT%GMU0, YDMF_PHYS%OPT%GMU0_MIN,  &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZALBD, ZALBP, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDCPG_MISC%NEB,          &
      & ZQO3, ZAER, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0M,             &
      & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQS, ZQR, YDMF_PHYS_SURF%GSD_VF%PLSM,        &
      & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%YGSP_RR%T, ZGP2DSPP, ZP1EZDIAG, YDMF_PHYS%RAD%EMTD, YDMF_PHYS%RAD%EMTU,  &
      ...
      & ZCGA_DST, ZTAUREL_DST, ZAERINDS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, PGPAR, YDMF_PHYS_TMP%RDG%MU0LU,           &
      & YDMF_PHYS%OUT%ALB, YDMF_PHYS%RAD%RMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS%RAD%EMTD, YDMF_PHYS_TMP%RDG%MU0, &
    & ZQV, ZTENT, YDMF_PHYS%RAD%TRSW, ZTRSOD, YLMF_PHYS_STATE%YGSP_RR%T, TSPHY, ZTRSODIR, ZTRSODIF,                        &
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,         &
              & YDCPG_DIM%KTDIA, NTSSG, YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YLMF_PHYS_STATE%YCPG_DYN%PHIF, &
              & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO)
...
!DEC$ IVDEP
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS%OUT%FRSO(:, YDCPG_DIM%KFLEVG, 1), &
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    ...
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YLMF_PHYS_STATE%T, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VF%PLSM,              &
    & YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS_SURF%GSD_VV%PLAI, YDAPLPAR_TMP%FLU%NEIJ, YDAPLPAR_TMP%FLU%VEG,              &
    & YDMF_PHYS_SURF%GSD_VV%PRSMIN, ZCHROV, ZGWDCS, ZWFC, YLMF_PHYS_STATE%YGSP_RR%FC, ZWWILT, YLMF_PHYS_STATE%YGSP_SB%Q, &
    & YDAPLPAR_TMP%FLU%QSATS, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV, ZWLMX)
  ENDIF
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      & NTDIFU, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,             &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO,                           &
      & ZKTROV, ZKUROV, ZKNROV, ZQV, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U,            &
      & YLMF_PHYS_STATE%V, ZXTROV, ZXUROV, ZXPTKEROV, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, ZQL, ZQI, ZNEBDIFF, YLMF_PHYS_STATE%TKE, ZLMU, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, &
      & YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCHROV, ZCEROV, YDAPLPAR_TMP%DSA%CPS, YDMF_PHYS%OUT%CT,        &
      & ZDQSTS, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VH%PSPSH, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV,            &
      & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDAPLPAR_TMP%FLU%NEIJ, YDCPG_MISC%QS,                    &
      & YDAPLPAR_TMP%FLU%QSATS, YLMF_PHYS_STATE%YGSP_SB%T, YLMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,               &
      & ZXDROV, ZXHROV, YLMF_PHYS_STATE%YGSP_RR%W, YLMF_PHYS_STATE%YGSP_RR%IC, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS,  &
      & YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTQN,        &
      & ZTENDPTKE, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, &
      & YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR, YDAPLPAR_TMP%DSA%LHS,              &
      & YDMF_PHYS%OUT%FRTH, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H)
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        IF ( LMSE.AND.LCALLSFX ) THEN       
          IF (YDCPG_DIM%KSTEP == 0) THEN
            YDAPLPAR_TMP%FLU%CD(:)=YDAPLPAR_TMP%FLU%CDN(:)   ! very first approximation
          ELSE
            YDAPLPAR_TMP%FLU%CD(:)=MAX(PGPAR(:,MCD),ZEPS0)
          ENDIF
        ENDIF
        CALL ACPTKE(YGFL, YDLDDH, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                   &
        & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, &
        & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZKNROV, YLMF_PHYS_STATE%T,                &
        & YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,        &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%TKE,                           &
        & ZLMU, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZF_EPS, ZFUN_TTE, ZMRIPP, ZMRIFPP, ZRHS, ZMN2_ES,                       &
        & ZMN2_EQ, ZRRCOR, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0, YDAPLPAR_TMP%FLU%CD, YDMF_PHYS%OUT%GZ0,      &
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZCP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & ZKTROV, ZKQROV, ZKUROV, ZQV, ZQL, ZQI, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T,                              &
      & YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, ZSVM, ZXTROV, ZXUROV, ZXDROV, ZXHROV, ZEDMFS, ZEDMFQ,                                &
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZRHODREFM(JLON)=YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,YDCPG_DIM%KFLEVG)/(YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)*YLMF_PHYS_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG))
          ZDEPTH_HEIGHT(JLON,:)=(YLMF_PHYS_STATE%YCPG_DYN%PHIF(JLON,:)-YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG))/RG
          ZZS(JLON)=YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)/RG
        ENDDO
        ...
        & YDCSGEOM%RINDY(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YLMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                       &
        & YLMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), YLMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), &
        & YLMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), ZSVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG, 1:NGFL_EXT),  &
        & ZCARDI, ZRHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                     &
        & YLMF_PHYS_STATE%YCPG_PHY%PREHYD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                          &
        ...
        & YDAPLPAR_TMP%FLU%EMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZTSN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                                                &
...
! Opposite water vapor flux
        ...
        & ZFEVS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YLMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),                                     &
        & YLMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG), ZDEPTH_HEIGHT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),         &
        ...
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ...
          PGPAR(JLON,MVEMIS) = YDAPLPAR_TMP%FLU%EMIS(JLON)
...
! calculation of variables for the old "ISBA" atmosphere scheme
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          ...
          & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, ZCOEFN, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,        &
          & ZQL, ZQI, YLMF_PHYS_STATE%T, ZALPHA1, ZCOEFA, ZLVT, ZQICE)
        ENDIF
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, YLMF_PHYS_STATE%YCPG_DYN%PHI, ZCP, YDMF_PHYS%OUT%FRSO,    &
          & ZQV, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,           &
          & ZXURO, ZXQRO, ZXTRO, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, ZQL, ZQI, ZNEBDIFF, YDAPLPAR_TMP%FLU%CD,              &
          & YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCHROV, ZCEROV, YDAPLPAR_TMP%DSA%CPS, YDMF_PHYS%OUT%CT,                       &
          & ZDQSTS, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VH%PSPSH, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV,      &
          & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDAPLPAR_TMP%FLU%NEIJ, YDCPG_MISC%QS,              &
          & YDAPLPAR_TMP%FLU%QSATS, YLMF_PHYS_STATE%YGSP_SB%T, YLMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,         &
          & ZXDROV, ZXHROV, YLMF_PHYS_STATE%YGSP_RR%W, YLMF_PHYS_STATE%YGSP_RR%IC, ZDSE, ZCFAS,                         &
          ...
          & YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV,       &
          & YDMF_PHYS%OUT%FTR, YDAPLPAR_TMP%DSA%LHS, YDMF_PHYS_SURF%GSD_VH%PQSH, YDMF_PHYS%OUT%FRTH,                    &
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, ZCFAQ, ZCFAS, ZCFAU, ZCFASV, ZCFBQ, ZCFBS,              &
      & ZCFBU, ZCFBV, ZCFBSV, ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZDSE, ZQT, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,    &
      & ZPOID, YLMF_PHYS_STATE%T, ZQL, ZQI, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZCOEFA, ZALPHA1,                   &
      ...
      IF (L3DTURB) THEN
        ...
        & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%TKE, ZTENDPTKE, YLMF_PHYS_STATE%YCPG_DYN%PHI,           &
        & YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, &
        & YLMF_PHYS_STATE%DIV, YLMF_PHYS_STATE%VOR, YDVARS%U%DL, YDVARS%V%DL, YLMF_PHYS_STATE%YCPG_PHY%W,                            &
        & YLMF_PHYS_STATE%YCPG_PHY%WL, YLMF_PHYS_STATE%YCPG_PHY%WM, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                                 &
        & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZLML, ZFHORM, ZFHORH,                                   &
        & ZKUROV, ZRTI, YDAPLPAR_TMP%FLU%CD, ZCDROV, LPTKE, YDAPLPAR_TMP%KUR%KUROV_H, YDAPLPAR_TMP%KUR%KTROV_H,                      &
        & ZRHS)
      ENDIF
      ...
      IF (LCOEFKTKE) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_DYN%PHI,                      &
        & YLMF_PHYS_STATE%YCPG_DYN%PHIF, ZCP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZKTROV, ZXTROV, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, &
        & YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZCHROV, ZXHROV, ZCOEFA, ZQV, ZQL, ZQI, YLMF_PHYS_STATE%U,                          &
        & YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,            &
        & ZLML, ZTH_FUN, ZWW_FUN, ZF_EPS, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS%OUT%DIFTQ, YLMF_PHYS_STATE%TKE,                         &
        & ZTENDPTKE, ZMN2_ES, ZMN2_EQ, ZMN2_DS, ZMN2_DQ, YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTQN,                             &
        & ZDIFWQ, ZDIFWS, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI,                      &
        & YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%GZ0, ZRTI, ZSC_FEVI, ZSC_FEVN,                                   &
        & ZSC_FCLL, ZSC_FCLN, ZTSTAR, ZTSTAR2, ZTSTARQ, ZTSTAR2Q)
...
!DEC$ IVDEP
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZT(JLON,JLEV)=YLMF_PHYS_STATE%T(JLON,JLEV)-ZIPOI(JLON,JLEV)/YLMF_PHYS_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)&
        ...
        YDMF_PHYS%OUT%FCQNG(JLON,JLEV)=YDMF_PHYS%OUT%FCQNG(JLON,JLEV)+ZFCQVNG(JLON,JLEV)
        IF(LRKCDEV.AND.YDCPG_DIM%KSTEP>0) THEN
          ZDTRAD(JLON,JLEV)=ZIPOI(JLON,JLEV)/YLMF_PHYS_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)* (&
    ...
    DO JLEV = YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZT (JLON,JLEV)= YLMF_PHYS_STATE%T(JLON,JLEV)
        ZQX0=ZQI(JLON,JLEV)
        ZQX1=ZQX0-ZIPOI(JLON,JLEV)*(YDMF_PHYS%OUT%DIFTQN(JLON,JLEV)&
          & -YDMF_PHYS%OUT%DIFTQN(JLON,JLEV-1))
        ZDQI= MAX(0.0_JPRB,ZQX1)-ZQX1
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,             &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V,              &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, &
      & ZPCLS, YDMF_PHYS%OUT%TCLS, YDMF_PHYS%OUT%QCLS, YDMF_PHYS%OUT%FCS, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN,   &
      & YDAPLPAR_TMP%DSA%LHS, YDMF_PHYS%OUT%GZ0, YDAPLPAR_TMP%MOC%CLPH, ZBLH, ZUGST, ZVGST)
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZPREN(JLON)=YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG)
      ENDDO
      CALL ACPBLH ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTDIFU, YDCPG_DIM%KFLEVG,               &
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,                 &
      & ZQV, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,               &
      & YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, ZPREN, YLMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDMF_PHYS%OUT%FCS, &
      & YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDAPLPAR_TMP%DSA%LHS, ZRTI, YDMF_PHYS_SURF%GSD_VH%PPBLH               &
      & )
    ENDIF
    ! ------------------------------------------------------------------
...
!DEC$ IVDEP
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,YDCPG_DIM%KFLEVG,1)/YDAPLPAR_TMP%FLU%EMIS(JLON)+RSIGMA*YLMF_PHYS_STATE%YGSP_RR%T(JLON)**4
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    ...
    & YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, ZQV, YDCPG_MISC%QS, YLMF_PHYS_STATE%YGSP_RR%T, &
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,        &
    & ZNBVNO, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDGSGEOM%RCORI,            &
 ...
 ! SAVE FOR TL/NL COEFS FROM VERT. DIFF AND GWD
  ...
  IF (LTRAJPS.AND.(LVDIFSPNL.OR.LGWDSPNL)) THEN
     ...
     & ZKTROV_SAVE, ZKUROV_SAVE, ZCDROV_SAVE, ZCHROV_SAVE, ZTRAJGWD, YLMF_PHYS_STATE%L, YLMF_PHYS_STATE%I, &
     & YLMF_PHYS_STATE%R, YLMF_PHYS_STATE%S, ZQLIS, ZNEBS)
  ENDIF
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%T, &
  ...
  IF ( LSTRAS ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZNEBS, ZQV, ZQLIS, YDAPLPAR_TMP%MSC%QW,      &
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZT, ZGEOSLC, ZLH,                &
      & ZLSCPE, ZQSAT, ZQW, ZRH, ZTW)
      CALL ACNEBCOND ( YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                  &
      & NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,            &
      & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, ZQW, ZT, ZNEBCH, YDGSGEOM%GM,          &
      & YLMF_PHYS_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1, ZQLIS0, ZNEBS0)
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTPLUI, YDCPG_DIM%KFLEVG, &
      & ZVETAF, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,     &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                 &
      & ZQG, ZQW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, ZT, YDAPLPAR_TMP%MSC%TW, ZNEBS, ZRHCRI, ZICEFR1,                   &
      & ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP, ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM,   &
      & YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL,              &
      & YDMF_PHYS%OUT%FPFPSN, YDMF_PHYS%OUT%FPFPSG, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPEVPSL, &
      & YDMF_PHYS%OUT%FPEVPSN, YDMF_PHYS%OUT%FPEVPSG, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLSG, &
      & ZSEDIQL, ZSEDIQI, YDMF_PHYS%OUT%DIAGH)
    ELSE
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTPLUI, YDCPG_DIM%KFLEVG,   &
      & ZVETAF, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,       &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                   &
      & ZQG, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW,              &
      & ZNEBS, ZRHCRI, ZICEFR1, ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP,                    &
      & ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T,          &
    ...
    DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZT(JLON,JLEV)=ZT(JLON,JLEV)+(ZLHV(JLON,JLEV)*ZDQL&
                   & +ZLHS(JLON,JLEV)*ZDQI)/YLMF_PHYS_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)
      ENDDO
    ENDDO
...
!           DE L EVAPORATION DU SOL.
    IF(LCVCSD.OR..NOT.LCVGQD) THEN
      IF (LCOMOD) THEN
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            ZMOD(JLON)=1._JPRB/(1.0_JPRB+YDGSGEOM%GM(JLON)*TEQK)
         ENDDO
      ELSE
...
!cdir unroll=8
      DO JLEV = YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          YDMF_PHYS_TMP%RDG%CVGQ(JLON,JLEV) = YDMF_PHYS_TMP%RDG%CVGQ(JLON,JLEV)*ZMOD(JLON) &
           & -RG*YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP(JLON,JLEV)&
    ...
    IF (LCAPE) THEN
      IF (LCAMOD) THEN
         DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            ZTAUX(JLON)=RTCAPE*(YDGSGEOM%GM(JLON)*TEQC)
         ENDDO
      ELSE
         ZTAUX=RTCAPE
      ENDIF ! lcamod
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHI,         &
      & YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,                           &
      & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQSAT, ZQW, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,                           &
      & ZT, ZTW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%VOR, YDVARS%DAL%T0, YLMF_PHYS_STATE%YGSP_RR%T,               &
      & ZTAUX, YDGSGEOM%RCORI, YDGSGEOM%GM, ZDECRD_MF, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN,             &
      & YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV,                    &
      & ZATSLC, INLAB, YDVARS%UNEBH%T0, ZDETFI, ZTU, ZQU, ZUU, ZVU, INND, YDMF_PHYS%OUT%CAPE, YDMF_PHYS%OUT%CUCONVCA,               &
      & YDMF_PHYS%OUT%NLCONVCA, ZGEOSLC, YDVARS%UEN%T0, YDVARS%UAL%T0, YDVARS%UOM%T0)
    ...
    ! Initialize Environment horizontal velocity
      ZU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YLMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
      ZV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YLMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
    ELSE
! PUT IN ZTCORR THE CONDENSATE USED IN TRIGGERING
      DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
       DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZTCORR(JLON,JLEV)=MAX(0._JPRB,ZQI(JLON,JLEV)+ZQL(JLON,JLEV) -&
          &  MAX(0._JPRB,YLMF_PHYS_STATE%I(JLON,JLEV)+YLMF_PHYS_STATE%L(JLON,JLEV)) )
       ENDDO
      ENDDO
! PASS T9 CONDENSATES TO ACCSU
      CALL ACCSU ( YDMODEL%YRML_PHY_EC%YRECUCONVCA, YDMODEL%YRML_PHY_EC%YRECUMF, YDMODEL%YRML_PHY_MF,                           &
      & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, &
      & YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                           &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R,                      &
      & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                           &
      & ZQV, ZTCORR, YLMF_PHYS_STATE%I, YLMF_PHYS_STATE%L, ZQSAT, ZQW, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,                      &
      & ZT, ZTW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%VOR, YDCPG_DYN0%CTY%VVEL(:, 1:),                         &
      & YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YLMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,                             &
    ...
    & YDVARS%UAL%T0, YDVARS%UNEBH%T0, ZDETFI, ZPOID, ZIPOI, ZLHV, ZLHS, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,           &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, &
    & ZQV, ZQI, ZQL, ZQS, ZQR, ZQG, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, ZT, ZIPOI, ZDQ, ZDQM, ZLHS, ZLHV,                       &
    & ZNEBS, ZPOID, ZRCVOTT, ZTCORR, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YLMF_PHYS_STATE%YGSP_RR%T,          &
    & ZDECRD_MF, ZFCQL, ZFCQI, ZFALLR, ZFALLS, ZFALLG, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN, YDMF_PHYS%OUT%FPFPSG,    &
    & YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN, YDMF_PHYS%OUT%FPEVPSG, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN,        &
    & YDMF_PHYS%OUT%FPLSG, ZSEDIQL, ZSEDIQI, ZMELNET, ZMELGET, YDMF_PHYS%OUT%DIAGH)
   ...
   & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZLHS, ZLHV, YDCPG_DYN0%CTY%EVEL, ZIPOI,                        &
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      ...
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,  &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,               &
      & ZQV, ZQI, ZQL, ZQR, ZQS, ZQW, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZSIGP, ZT, ZTW, ZU, ZV, YDCPG_DYN0%CTY%VVEL(:, 1:), &
      & ZATSLC, ZGEOSLC, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN,              &
      & ZDIFCQD, ZDIFCQLD, ZDIFCQID, ZDIFCSD, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN, ZSTRCUD,                          &
      & ZSTRCVD, YDVARS%DAL%T0, YDVARS%DOM%T0)
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA,                          &
      & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,       &
      & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,                      &
      & YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
  ...
  !  ---------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZLHS,                         &
    & ZLHV, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, ZIPOI, ZPOID, ZFALLR, ZFALLS, ZFALLG, YDMF_PHYS%OUT%FPEVPSL,                 &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,                      &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,                  &
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                        &
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YLMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%VOR,                   &
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YDGSGEOM%GM, YLMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI, YDMF_PHYS%OUT%DIFCQ,      &
    & YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN,             &
    & YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN, YDMF_PHYS%OUT%STRCU,       &
    & YDMF_PHYS%OUT%STRCV, ZFPCOR, INLAB, YDMF_PHYS%OUT%CAPE, INND, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS,              &
    & ZGEOSLC, YDGSGEOM%GEMU)
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,                &
    & YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                      &
    & ZQV, YDAPLPAR_TMP%MSC%QW, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T,   &
    & YDAPLPAR_TMP%MSC%TW, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PPBLH, &
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZGEOM1(JLON,JLEV)     = YLMF_PHYS_STATE%YCPG_DYN%PHIF(JLON,JLEV)-YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
        ZVERVEL(JLON,JLEV)    = YDCPG_DYN0%CTY%VVEL(JLON,JLEV)*YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON,JLEV)
    ...
    DO JLEV=0,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZGEOMH(JLON,JLEV)=YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,JLEV)-YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
    ...
    & YDMF_PHYS%OUT%FPEVPCN, YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,     &
    & YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%Q, YLMF_PHYS_STATE%YGSP_RR%T, ZTENHA, ZTENQVA )
    
    DO JLEV=1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA 
         ZTENT(JLON,JLEV) = ZTENHA(JLON,JLEV)/YLMF_PHYS_STATE%YCPG_DYN%RCP%CP(JLON,JLEV)
    ...
    & LLDSLPHY, TSPHY, ZVDIFTS, YLMF_PHYS_STATE%T, ZQV, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, ZLISUM,                       &
    & ZVERVEL, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, &
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, ZGEOM1, ZGEOMH, YDGSGEOM%GM, ZCUCONVCA, ZGP2DSPPA, ZTENT,                             &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    & ZGAW, ZGAW, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_DYN%PHI,  &
    & YLMF_PHYS_STATE%YCPG_DYN%PHIF, LLLAND, YLMF_PHYS_STATE%T, ZLU, ZMFU, YDMF_PHYS%OUT%CAPE, YDMF_PHYS%OUT%FPLCL, &
  ...
  IF ( LADJCLD ) THEN
    ...
    & YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%T,              &
    & YLMF_PHYS_STATE%Q, YLMF_PHYS_STATE%YGSP_RR%T, ZTENHA, ZTENQVA )
  ENDIF
  ...
  IF ( LPROCLD ) THEN
    IF(LGPCMT) THEN
      ...
      ZTMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=YLMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)
      ZQMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)&
        & *YDAPLPAR_TMP%FLU%QSAT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)&
    ...
    & ZTMIC, ZQMIC, ZQL, ZQI, ZQR, ZQS, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF,            &
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, ZNEBS, ZQLIS, ZNEB_CVPP,                         &
    & ZQLI_CVPP, ZQC_DET_PCMT, ZTENHA, ZTENQVA, LADJCLD, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YGSP_RR%T,       &
    & YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDGSGEOM%GM, ZVETAF, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, &
  ...
  IF ( LGWDC ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_PHY%PREHYD, ZNBVNO, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%U,                        &
    & YLMF_PHYS_STATE%V, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH, &
  ...
  IF ( LNORGWD ) THEN
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        Z_PP(JLON, JLEV) = YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_UU(JLON, JLEV) = YLMF_PHYS_STATE%U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VV(JLON, JLEV) = YLMF_PHYS_STATE%V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_TT(JLON, JLEV) = YLMF_PHYS_STATE%T(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VO(JLON, JLEV) = YLMF_PHYS_STATE%VOR(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_U(JLON, JLEV) = TSPHY * YLMF_PHYS_STATE%P1NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_V(JLON, JLEV) = TSPHY * YLMF_PHYS_STATE%P2NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
      ENDDO
    ENDDO
    ...
    & TSPHY, Z_PP, YDGSGEOM%GEMU, Z_TT, Z_UU, Z_VV, Z_VO, ZPRECGWD, ZD_U, ZD_V)
    
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        YLMF_PHYS_STATE%P1NOGW(JLON, JLEV) = ZD_U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
        YLMF_PHYS_STATE%P2NOGW(JLON, JLEV) = ZD_V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
      ENDDO
    ENDDO
    ...
    DO JLEV = 1, YDCPG_DIM%KFLEVG
       DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          ZFLX_LOTT_GWU(JLON, JLEV) = ZFLX_LOTT_GWU(JLON, JLEV - 1) - YLMF_PHYS_STATE%P1NOGW(JLON, JLEV)*YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON, JLEV)/RG
          ZFLX_LOTT_GWV(JLON, JLEV) = ZFLX_LOTT_GWV(JLON, JLEV - 1) - YLMF_PHYS_STATE%P2NOGW(JLON, JLEV)*YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JLON, JLEV)/RG
...
!         SAUVEGARDE DES FLUX DE PRECIPITATION CONVECTIVE ET STRATIFORME.
  ...
  IF(LNEBN.OR.LNEBR.OR.LRRGUST) THEN
    IF (LFPCOR) THEN
      IF (LCDDPRO) THEN
        DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            YDAPLPAR_TMP%PFL%FPLCH(JLON,JLEV)=YDMF_PHYS%OUT%FPLCL(JLON,JLEV)+YDMF_PHYS%OUT%FPLCN(JLON,JLEV)
          ENDDO
        ENDDO
      ELSE
        DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
          DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            YDAPLPAR_TMP%PFL%FPLCH(JLON,JLEV)=ZFPCOR(JLON,JLEV)
          ENDDO
        ENDDO
      ENDIF
    ELSE
  ...
  IF(LRRGUST) THEN
    DO JLEV=YDCPG_DIM%KTDIA-1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDAPLPAR_TMP%PFL%FPLSH(JLON,JLEV)=YDMF_PHYS%OUT%FPLSL(JLON,JLEV)+YDMF_PHYS%OUT%FPLSN(JLON,JLEV)+YDMF_PHYS%OUT%FPLSG(JLON,JLEV)
      ENDDO
    ENDDO
  ENDIF
  ...
  IF ( LCONDWT.AND.LPROCLD.AND..NOT.LGPCMT ) THEN
    ...
    & ZQV, ZQL, ZQI, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL, &
...
!     ---------------------------
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      ...
      & YDMF_PHYS%OUT%FRTH, YDAPLPAR_TMP%DSA%C1, YDAPLPAR_TMP%DSA%C2, ZC3, ZCN, YDMF_PHYS%OUT%CT, YDMF_PHYS_SURF%GSD_VV%PD2, &
      & YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR, YDMF_PHYS_SURF%GSD_VV%PLAI, YDAPLPAR_TMP%FLU%NEIJ, YDAPLPAR_TMP%FLU%VEG,      &
      & ZWFC, ZWPMX, YLMF_PHYS_STATE%YGSP_RR%FC, ZWLMX, ZWSEQ, ZWSMX, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL,               &
      & YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN,                &
      & YDMF_PHYS_SURF%GSD_VF%PLSM, YLMF_PHYS_STATE%YGSP_SG%F, YLMF_PHYS_STATE%YGSP_SB%T, YLMF_PHYS_STATE%YGSP_RR%T,         &
      & YLMF_PHYS_STATE%YGSP_SB%Q, YLMF_PHYS_STATE%YGSP_SB%TL, YLMF_PHYS_STATE%YGSP_RR%W, YLMF_PHYS_STATE%YGSP_RR%IC,        &
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    ...
    & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YLMF_PHYS_STATE%T, ZQV, YLMF_PHYS_STATE%U, &
    & YLMF_PHYS_STATE%V, YDMF_PHYS%OUT%FRMH, YDAPLPAR_TMP%MSC%FRMQ, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV,        &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    ...
    & YDMODEL%YRML_PHY_G%YRDPHY%NVCLIS, YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
    & PKOZO, ZQO3(1, 1), YLMF_PHYS_STATE%T, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS%OUT%FCHOZ)
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,               &
    & YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FRSO, ZKTROV, ZQO3(1, 1), YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YLMF_PHYS_STATE%T, ZXTROV, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GEMU, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS%OUT%FCHOZ     &
    & )
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     ------------------------------------------------------------------
  ...
  IF (LPHSPSH) THEN
    DO JLON=YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
       YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=(YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YLMF_PHYS_STATE%YGSP_RR%T(JLON)) * ((&
    ...
    ENDDO
    IF (LGRAPRO) THEN
      YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)+(YLMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YLMF_PHYS_STATE%YGSP_RR%T(JLON)) * (&
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%QICE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAF, 'VIR', YDDDH)
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%QLI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAF, 'VLR', YDDDH)
      ZTMPAF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)*YDCPG_MISC%NEB(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
      ...
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_STATE%YGSP_RR%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)/YDMF_PHYS%OUT%CT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSTS', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*RTINER*YLMF_PHYS_STATE%YGSP_SB%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YSP_SBD%NLEVS)/YDMF_PHYS%OUT%CT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSTP', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_STATE%YGSP_RR%W(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSWS', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_STATE%YGSP_SB%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH, ZTMPAS, 'VSWP', YDDDH)
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YLMF_PHYS_STATE%YGSP_SG%F(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1)
      ...
      & )
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YLMF_PHYS_STATE%YCPG_DYN%PHI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
      ...
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*YDMF_PHYS%OUT%FCHSP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YSP_SBD%NLEVS)
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     & YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,                      &
     & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YLMF_PHYS_STATE%YCPG_DYN%PHIF, &
     & YLMF_PHYS_STATE%YCPG_DYN%PHI, ZDCAPE)
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
     & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE,            &
     & ZDCAPE, ZTKE1, YLMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,           &
     & ZBLH, YDAPLPAR_TMP%MOC%CLPH)
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     ...
     & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_DYN%PHIF,      &
     & YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%YCPG_DYN%RCP%R, YDCPG_MISC%QLI, &
...
IF ( (.NOT. LGPCMT) ) THEN
   ...
   YDAPLPAR_TMP%BAY%QRCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) =  0.0_JPRB
   YDAPLPAR_TMP%BAY%QSCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) =  0.0_JPRB


   DO JLEV=1,YDCPG_DIM%KFLEVG
     DO JLON= YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
       YDAPLPAR_TMP%BAY%QRCONV(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%OUT%FPLCL(JLON,JLEV)) * ZCOEFRAIN(1)) ** ZCOEFRAIN(2) )
       YDAPLPAR_TMP%BAY%QSCONV(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%OUT%FPLCN(JLON,JLEV)) * ZCOEFSNOW(1)) ** ZCOEFSNOW(2) )    
     ENDDO
   ENDDO
   ...
   ZDE2MR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)  = RD * YLMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) / YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
   YDAPLPAR_TMP%BAY%QRCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) = YDAPLPAR_TMP%BAY%QRCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) * ZDE2MR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
   YDAPLPAR_TMP%BAY%QSCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) = YDAPLPAR_TMP%BAY%QSCONV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) * ZDE2MR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
...
CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, ZPCLS, YDMF_PHYS%OUT%TCLS, YLMF_PHYS_STATE%Q(:, YDCPG_DIM%KFLEVG), &
& YLMF_PHYS_STATE%L(:, YDCPG_DIM%KFLEVG), YLMF_PHYS_STATE%I(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%TPWCLS                                      &
& )
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF(:, JLEV), &
    & YLMF_PHYS_STATE%T(:, JLEV), YLMF_PHYS_STATE%Q(:, JLEV), YLMF_PHYS_STATE%L(:, JLEV), YLMF_PHYS_STATE%I(:, JLEV),   &
    & ZTW(:, JLEV))
  ENDDO
  ...
  DO JLEV = 1,YDCPG_DIM%KFLEVG
    DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZZZ(JLON,1,JLEV)=YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
  ...
  ENDDO
  DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
      ZDZZ(JLON,1,1)=YLMF_PHYS_STATE%YCPG_DYN%PHI(JLON,0)*ZINVG-ZZZ(JLON,1,1)
  ENDDO
  ...
  IF (LDPRECIPS) THEN
   ...
   & YDOROG%OROG, YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, YLMF_PHYS_STATE%YCPG_DYN%PHIF, ZDZZ,                 &
   & ZTW, YLMF_PHYS_STATE%L, ZFPLC(:, YDCPG_DIM%KFLEVG), ZFPLS(:, YDCPG_DIM%KFLEVG), ZFPLSG(:, YDCPG_DIM%KFLEVG), &
