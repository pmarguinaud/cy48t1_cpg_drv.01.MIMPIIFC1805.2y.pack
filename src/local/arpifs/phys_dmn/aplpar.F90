!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
 ...
 ! - INPUT .
 & PINDX  , PINDY  ,&
 & LDXFUMSE,PAPHI  , PAPRS  , PAPHIF , PAPRSF , PALPH  , PARG   , PD2    ,&
 & PDELP  , PIVEG  , PLAI   , PLNPR  , PRDELP , PRSMIN , PSAB   ,&
 & PGZ0HF , PAESEA , PAELAN , PAESOO , PAEDES , PAESUL , PAEVOL , PRCORI ,&
 ...
 & PCP    , PCVGQ  , PR     , PKOZO  ,&
 & PFPLCH , PFPLSH ,&
 & PDDAL  , PDDOM  , PENTCH , PUDAL  , PUDOM  , PUNEBH, PEVEL0,&
 & PRKTH  , PRKTQV , PRKTQC , PTTE   , PMXL   , PSHTUR, PFQTUR,&
 & PFSTUR , PTCCH  , PSCCH  , PBCCH  , PFHPS  ,&
 & PPBLH  , PQSH   , PUDGRO,&
 ...
 & PHV    , PVERVEL,&
 & PEMTD  , PEMTU  , PTRSO  ,&
 & PALV   , PALBF  , PALBSF , PEMISF , PGETRL ,&
 & PLSM   , PVEG0  ,&
 & PGZ0F  , PGZ0RLF, PVRLAN ,&
 & PVRLDI , PVO3ABC, PSFLUX, PSFORC, PVRMOON, PMU0   , PMU0LU ,&
 & PMU0M  , PMU0N   ,PGELAM ,&
 ...
 & PCAPE  , PCTOP  , KCLPH  , PCLPH  , PVEIN  , PUGST  , PVGST  , PDIAGH , PEZDIAG,&
 & PTPWCLS,PDPRECIPS,PDPRECIPS2,PVISICLD,PVISIHYDRO,PMXCLWC,&
 & PSPSG  , PTENDPTKE,PKUROV_H,PKTROV_H,&
 & PDERNSHF, PTENDEXT_DEP, PSDUR,PTRAJ_PHYS,PTDISS,YDDDH,&
 & PTP2, PWS2, PWP2, PWSI2, PWPI2, PWR2, PSNA2, PSND2, PFTCNS, &
 & PGP2DSPP )
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTCCH(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSCCH(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PBCCH(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PPBLH(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PQSH(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PUDGRO(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PFHPS(KLON)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: PALV(KLON)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PALBF(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PALBSF(KLON)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSFLUX(KLON,YDMODEL%YRML_PHY_MF%YRPHYDS%NSFLUX)
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PQICE1(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PQLI1(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PCLF1(KLON,KLEV)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PUNEBH(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PRKTH(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PRKTQV(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PRKTQC(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTTE(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PMXL(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSHTUR(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PFQTUR(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PFSTUR(KLON,KLEV)
!---------------------------------------------------
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PALBH(KLON,YDSURF%YSP_SGD%NLEVS)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSPSG(KLON,YDSURF%YSP_SGD%NLEVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PTP2(KLON,YDSURF%YSP_SBD%NLEVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWP2(KLON,YDSURF%YSP_SBD%NLEVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWS2(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWPI2(KLON,YDSURF%YSP_SBD%NLEVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWSI2(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PWR2(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSNA2(KLON,YDSURF%YSP_SGD%NLEVS)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSND2(KLON,YDSURF%YSP_SGD%NLEVS)
...
INTEGER(KIND=JPIM) :: IJN, IMLTYPE, JCHA, JLEV, JLON, JSG, JGFL
...
IF (LMPHYS) THEN
  ...
  ELSE
    CALL SUOZON(KIDIA,KFDIA,KLON,KLEV,ZQO3,.FALSE.,PAPRS,PRDELP,LO3ABC,PVO3ABC)
  ENDIF
...
!cdir unroll=8
  ...
  !                         FOR RAIN/SNOW/GRAUPEL PUT IN PFCQR/S/GNG
    ...
    IF (LGPCMT) THEN
      DO JLEV = 1, KLEV
        DO JLON = KIDIA,KFDIA
          ZTZER=MAX(0.0_JPRB,-SIGN(1.0_JPRB,-(PQICONV(JLON,JLEV)-ZEPS0)+0.0_JPRB))
          ZQIC(JLON,JLEV)=ZTZER*PQICONV(JLON,JLEV)
          ZDQI=ZQIC(JLON,JLEV)-PQICONV(JLON,JLEV)

          ZTZER=MAX(0.0_JPRB,-SIGN(1.0_JPRB,-(PQLCONV(JLON,JLEV)-ZEPS0)+0.0_JPRB))
          ZQLC(JLON,JLEV)=ZTZER*PQLCONV(JLON,JLEV)
          ZDQL=ZQLC(JLON,JLEV)-PQLCONV(JLON,JLEV)

          ZTZER=MAX(0.0_JPRB,-SIGN(1.0_JPRB,-(PQRCONV(JLON,JLEV)-ZEPS0)+0.0_JPRB))
          ZQRC(JLON,JLEV)=ZTZER*PQRCONV(JLON,JLEV)
          ZDQR=ZQRC(JLON,JLEV)-PQRCONV(JLON,JLEV)

          ZTZER=MAX(0.0_JPRB,-SIGN(1.0_JPRB,-(PQSCONV(JLON,JLEV)-ZEPS0)+0.0_JPRB))
          ZQSC(JLON,JLEV)=ZTZER*PQSCONV(JLON,JLEV)
          ZDQS=ZQSC(JLON,JLEV)-PQSCONV(JLON,JLEV)
...
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    PPBLH(JLON)=(XMINLM+XMAXLM)*0.5_JPRB
  ENDDO
ENDIF
...
IF(LPHSPSH.AND.KSTEP == 0) THEN
  PFHPS(KIDIA:KFDIA)=0._JPRB
ENDIF
IF(LCOEFKTKE.AND.KSTEP == 0) THEN
  PQSH(KIDIA:KFDIA)=PQ(KIDIA:KFDIA,KLEV)
ENDIF
IF(LCVCSD.AND.LUDEVOL.AND.KSTEP==0) THEN
  PUDGRO(KIDIA:KFDIA)=0._JPRB
ENDIF
IF(LRKCDEV.AND.KSTEP == 0) THEN
  DO JLEV=KTDIA,KLEV
     DO JLON=KIDIA,KFDIA
        ...
        PRKTH(JLON,JLEV) = 0.0_JPRB
        PRKTQV(JLON,JLEV)= 0.0_JPRB
        PRKTQC(JLON,JLEV)= 0.0_JPRB
     ENDDO
  ENDDO
ENDIF
IF (L3MT) THEN
  IF (LCVPRO) THEN
    PUNEBH(KIDIA:KFDIA,:)=MAX(0._JPRB,PUNEBH(KIDIA:KFDIA,:))
    ZUNEBH(:,:)=MIN(1.0_JPRB-ZEPS0,PUNEBH(:,:)+MAX(0._JPRB,PUDAL(:,:)))
  ELSE
    ZUNEBH(:,:)=PUNEBH(:,:)
  ENDIF
ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS,LLAROME,KIDIA,KFDIA,KLON,PAPHIF(:,KLEV),ZRHODREFM,PSFORC,&
        & ZTN,ZTSN,PLSM,PQ(:,KLEV),PU(:,KLEV),PV(:,KLEV),ZTHETAS,PFCS(KIDIA:KFDIA,1),ZFEV,ZFMDU,ZFMDV)
     DO JLON=KIDIA,KFDIA
        PTS(JLON)=ZTSN(JLON)
     ENDDO
     DO JLON=KIDIA,KFDIA
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        CALL ACTKEHMT ( KIDIA,KFDIA,KLON,KLEV,KVCLIV>=8.AND.LSOLV,&
         ...
         & PFPLSH,PFPLCH,&
         & ZDPHIT, ZDPHIV, PGZ0F, PGZ0HF, PGZ0RLF, PHV, PLSM,&
    ...
    ENDIF
    IF (LCOEFK_PTTE) THEN
      PTTE(KIDIA:KFDIA,1:KLEV) = MAX(PTTE(KIDIA:KFDIA,1:KLEV),ETKE_MIN)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      ENDIF
    ELSE
      ZBLH(KIDIA:KFDIA)=PPBLH(KIDIA:KFDIA)
    ENDIF
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         ...
         & PGZ0,PTS,PCDN,ZBLH,ZLMU,ZLMT,PMXL,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      CALL ACMRIP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,KSTEP,&
       ...
       & PR,PT,PU,PV,&
       & PFQTUR,PFSTUR,PSHTUR,&
       & PTKE,PTTE,&
       & PTS,PQSH,PRS,PCPS,ZRTI,PGZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      ...
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
       ...
       & ZDPHIT,ZDPHIV,PGZ0,PLSM,PQSATS,ZCRTI,PTS,&
...
!     ------------------------------------------------------------------
  ...
  IF (LEDKF) THEN
    ...
    CALL ARP_SHALLOW_MF( KIDIA,KFDIA,KLON,KTDIA,KLEV,ZIMPL,TSPHY,PAPHI,PAPHIF,PR,PCP, &
                      ...
                      & PSFLUX(:,1),PSFLUX(:,2),ZPRODTH_CVPP,ZQLI_CVPP,ZNEB_CVPP,INLAB_CVPP,ZMF_UP)
...
!DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      IF (LSNV) THEN
        IF ((PVEG0(JLON) < 0.01_JPRB).OR.(PALBF(JLON) >= 0.60_JPRB)) THEN
          ZALBV=0.0_JPRB
          PALBSF(JLON)=PALBF(JLON)
        ELSE
          ZALBV=(PALBF(JLON)-(1.0_JPRB-PVEG0(JLON))*PALBSF(JLON))/PVEG0(JLON)
        ENDIF
        PALB(JLON)= (1.0_JPRB-PVEG0(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
         & PALBSF(JLON)&
         & + (1.0_JPRB-PVEG0(JLON))*ZNEIJG(JLON) *&
         & MAX(PALBSF(JLON),PALBNS(JLON))&
         & + PVEG0(JLON)*ZNEIJV(JLON) *&
         & MAX(ZALBV,PALBNS(JLON))&
         & + PVEG0(JLON)*(1.0_JPRB-ZNEIJV(JLON)) * ZALBV
        PEMIS(JLON)= (1.0_JPRB-PVEG0(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
         & PEMISF(JLON)&
         & + (1.0_JPRB-PVEG0(JLON))*ZNEIJG(JLON) * EMCRIN&
         & + PVEG0(JLON)*ZNEIJV(JLON) * EMCRIN&
         & + PVEG0(JLON)*(1.0_JPRB-ZNEIJV(JLON)) * PEMISF(JLON)
      ELSE
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ! new treatment, PNEIJ is gridbox snow fraction
            PALB(JLON)=(1.0_JPRB-PVEG(JLON)-PNEIJ(JLON))*PALBF(JLON)+ &
             & PVEG(JLON)*PALV(JLON)+PNEIJ(JLON)*PALBNS(JLON)
          ELSE
            ! old treatment, PNEIJ is snow fraction for bare ground
            PALB(JLON)=PALBF(JLON)-PNEIJ(JLON)*(PALBF(JLON)- &
             & PALBNS(JLON))+(PNEIJ(JLON)-ZNEIJV(JLON))*     &
             & PVEG0(JLON)*(PALV(JLON)-PALBNS(JLON))
          ENDIF

          PALB(JLON)=MIN(ABS(PIVEG(JLON)-2._JPRB),1.0_JPRB) * PALB(JLON) +(&
           & 1.0_JPRB-MIN(ABS(PIVEG(JLON)-2._JPRB),1.0_JPRB))&
           & * MAX(ALCRIN,PALB(JLON))
          PALBH(JLON,1)=PALB(JLON)

          PEMIS(JLON)=PEMISF(JLON)-PNEIJ(JLON)*(PEMISF(JLON)-EMCRIN)

        ELSE
          PALB(JLON)=PALBF(JLON)-PNEIJ(JLON)*(PALBF(JLON)&
           & -MAX(PALBF(JLON),ALCRIN))
          PEMIS(JLON)=PEMISF(JLON)-PNEIJ(JLON)*(PEMISF(JLON)-EMCRIN)
        ENDIF
      ENDIF
    ENDDO
      ...
      IF (LALBMERCLIM) THEN
        DO JSG=1,NSW
          DO JLON=KIDIA,KFDIA
            ...
            ZALBP(JLON,JSG)=PALB(JLON)*          PLSM(JLON)+&
                          & ZALBPMER  *(1.0_JPRB-PLSM(JLON))
          ENDDO
        ENDDO
      ELSE
!DEC$ IVDEP
        DO JLON=KIDIA,KFDIA
          ZMERL(JLON)=(1.0_JPRB-PLSM(JLON))*(1.0_JPRB&
            & -MAX(0.0_JPRB,SIGN(1.0_JPRB,TMERGL-PTS(JLON))))
          PEMIS(JLON)=PEMIS(JLON)*PLSM(JLON)+ZMERL(JLON)*EMMMER&
            & +(1.0_JPRB-PLSM(JLON))*(1.0_JPRB-ZMERL(JLON))*EMMGLA
        ENDDO
        DO JSG=1,NSW
          DO JLON=KIDIA,KFDIA
    ...
    ELSEIF (LRAY) THEN
      ...
      DO JLON=KIDIA,KFDIA
        IF ( PLSM(JLON) < 0.5_JPRB .AND. PTS(JLON) >= TMERGL ) THEN
...
!DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      IF(PFRSODS(JLON) >= RSUNDUR) THEN
        PSDUR(JLON)=PSDUR(JLON)+1.0_JPRB*TSTEP
      ENDIF
    ENDDO
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDIFU,KLEV,KCSS,&
       ...
       & PEMIS, PFHPS, ZHQ, ZHTR, ZHU, PHV,&
       & PLSM, PIVEG, PNEIJ, PQS, PQSATS, PTP, PTS, PVEG,&
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        CALL ACPTKE(YGFL,YDLDDH,YDMODEL%YRML_PHY_MF,&
          ...
          & ZMRIPP,ZMRIFPP,ZRHS,&
          & ZMN2_ES,ZMN2_EQ,ZRRCOR,PFQTUR,PFSTUR,PSHTUR,&
          & PCD,PGZ0,&
...
! calculation of variables for the old "ISBA" atmosphere scheme
        IF (.NOT. LELAM) THEN
          CALL ARO_GROUND_DIAG_2ISBA( KBL, KGPCOMP, &
            ...
            & NDGUNG, NDGUXG, NDLUNG, NDLUXG, LSURFEX_KFROM, &
            & PINDX(KIDIA:KFDIA), PINDY(KIDIA:KFDIA), &
            & PLSM, PARG, PSAB, PD2, ZTSN, ZTWSNOW, &
            & PTP2(KIDIA:KFDIA,1), PWS2(KIDIA:KFDIA), PWP2(KIDIA:KFDIA,1), &
            & PWSI2(KIDIA:KFDIA), PWPI2(KIDIA:KFDIA,1), PWR2(KIDIA:KFDIA), &
            & PSNA2(KIDIA:KFDIA,1), PSND2(KIDIA:KFDIA,1), ZHV2 )
          DO JLON=KIDIA,KFDIA
            PHV(JLON) = ZHV2(JLON)
          ENDDO
        ENDIF
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          CALL ARP_GROUND_PARAM ( YDMCC,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KCSS,&
           ...
           & PCD,PCDN,ZCDROV,ZCHROV,ZCEROV,PCPS,PCT,ZDQSTS,PEMIS,PFHPS,&
           & ZHQ,ZHTR,ZHU,PHV,PLSM,PIVEG,PNEIJ,PQS,PQSATS,&
      ...
      CALL ACDIFV2 ( YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
        ...
        & PDIFSV,PDIFTQ,PDIFTS,PSTRTU,PSTRTV,PDIFTQL,PDIFTQI,PDIFCQ,PDIFCS,PSTRCU,PSTRCV,PSHTUR)

      IF (LEDKF) THEN
        IF (LSFORCS) THEN
        DO JLON=KIDIA,KFDIA
           PSFLUX(JLON,1) = PFCS(JLON,1)
           PSFLUX(JLON,2) = ZFEV(JLON)
        ENDDO
        ELSE
          DO JLON=KIDIA,KFDIA             
            PSFLUX(JLON,1) = PDIFTS(JLON,KLEV)
            PSFLUX(JLON,2) = PDIFTQ(JLON,KLEV)
          ENDDO
        ENDIF   
      ENDIF
      ...
      IF (LCOEFKTKE) THEN
        ...
        IF (YFQTUR%LGP.AND.YFSTUR%LGP) THEN
          DO JLEV=KTDIA,KLEV
            DO JLON=KIDIA,KFDIA
              PFQTUR(JLON,JLEV)=PDIFTQ(JLON,JLEV)+PDIFTQL(JLON,JLEV)&
               &                                 +PDIFTQI(JLON,JLEV)
              PFSTUR(JLON,JLEV)=PDIFTS(JLON,JLEV)
            ENDDO
          ENDDO
        ENDIF
      ENDIF ! LCOEFKTKE
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ZBLH(KIDIA:KFDIA)=MIN(XMAXLM,MAX(XMINLM,PPBLH(KIDIA:KFDIA)))
      ...
      PPBLH(KIDIA:KFDIA)=ZBLH(KIDIA:KFDIA)
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      CALL ACPBLH ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       ...
       & PSTRTU, PSTRTV,&
       & ZPREN, PTS, PQS,&
       & PFCS,PFCLL,PFCLN,PLHS,ZRTI,PPBLH)
    ENDIF
    ! ------------------------------------------------------------------
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY,YDPHY1,KIDIA, KFDIA, KLON, KTDIA, KLEV,&
     & PEMIS, PLSM, PNEIJ, ZQV, PQS, PTS, ZCHROV,  ZDQSTS,&
     & PDERNSHF)
  ENDIF
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       ...
       & ZQLI_CVPP,ZNEB_CVPP,PLSM,PNEIJ,PGM,PTS,ZDECRD_MF,&
...
!rb : Store values for next time-step: all cases
    IF(LRKCDEV) THEN
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          PRKTH(JLON,JLEV)= ZT(JLON,JLEV)+ZDTRAD(JLON,JLEV)
          PRKTQV(JLON,JLEV)=ZQV(JLON,JLEV)+ZDQVDIFF(JLON,JLEV)*ZDTURDIFF
          PRKTQC(JLON,JLEV)= ZQI(JLON,JLEV)+ZQL(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      CALL ACCVUD ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & INLAB, PUNEBH, ZDETFI, ZTU, ZQU, ZUU, ZVU,&
       & INND,PCAPE,PCUCONVCA,PNLCONVCA,&
       & ZGEOSLC, PENTCH, PUDAL, PUDOM)
...
!rb store "-B" state: no protection of Ncv
    IF(LRKCDEV.AND.(.NOT.LNEBCV)) THEN
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          PRKTH(JLON,JLEV)=PRKTH(JLON,JLEV)-ZT(JLON,JLEV)
          PRKTQV(JLON,JLEV)=PRKTQV(JLON,JLEV)-ZQV(JLON,JLEV)
          PRKTQC(JLON,JLEV)=PRKTQC(JLON,JLEV)-ZQI(JLON,JLEV)&
        & -ZQL(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF
  ...
  !  ---------------------------------------------------------
    ...
    CALL APLMPHYS( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     ...
     & ZTCORR,PLSM, PNEIJ, PTS, ZDECRD_MF,&
     & ZFCQL, ZFCQI,&
   ...
   CALL ACUPM( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
              & PCP, ZLHS, ZLHV,&
              & PEVEL0, ZIPOI, ZPOID, PUDAL, PUDOM,&
...
! input/output  2d .
     & ZQV,ZQL,ZQI,ZQR,ZQS,ZQG,ZT,PENTCH,&
    ...
    IF((LRKCDEV).AND.(.NOT.LNEBCV)) THEN
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          PRKTH(JLON,JLEV)=PRKTH(JLON,JLEV)+ZT(JLON,JLEV)
          PRKTQV(JLON,JLEV)=PRKTQV(JLON,JLEV)+ZQV(JLON,JLEV)
          PRKTQC(JLON,JLEV)=PRKTQC(JLON,JLEV)&
...
! -----------------------------------------------------
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    ENDDO
    DO JLON=KIDIA,KFDIA
       LLLAND(JLON)    = (PLSM(JLON)>0.5_JPRB)
       LLSHCV(JLON)    = .FALSE.
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,KCSS,&
       ...
       & PC1, PC2, ZC3, ZCN, PCT, PD2, PFEVV, PFTR, PLAI, PNEIJ,&
       & PVEG, ZWFC, ZWPMX, PWL, ZWLMX, ZWSEQ, ZWSMX,&
       & PFCHSP, PFCLL, PFCLN, PFCS, PFEVI, PFEVL, PFEVN, PLSM, PSNS,&
...
! Store radiative cloudiness in GFL structure for ISP, Historical files or PostProcessing
  IF (YIRAD%LGP) PQICE1(KIDIA:KFDIA,:) = PQICE(KIDIA:KFDIA,:)
  IF (YLRAD%LGP) PQLI1 (KIDIA:KFDIA,:) = PQLI (KIDIA:KFDIA,:)
  IF (YA%LGP)    PCLF1 (KIDIA:KFDIA,:) = PNEB (KIDIA:KFDIA,:)
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ! surface variables
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,PLSM,'VLSM',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PTS(KIDIA:KFDIA)/PCT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*RTINER*PTP(KIDIA:KFDIA,KCSS)/PCT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PWS(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSWS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PWP(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSWP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PSNS(KIDIA:KFDIA)
      ...
      ZTMPAS(KIDIA:KFDIA)=-PLSM(KIDIA:KFDIA)*RLMLT*PFONTE(KIDIA:KFDIA)
    ...
    ELSE  
      ...
      ! surface variables
      CALL ADD_FIELD_2D(YDLDDH,PLSM,'VLSM','V','ARP',.TRUE.,.TRUE.)
      ZTMPAS(KIDIA:KFDIA)=PLSM(KIDIA:KFDIA)*PTS(KIDIA:KFDIA)/PCT(KIDIA:KFDIA)
