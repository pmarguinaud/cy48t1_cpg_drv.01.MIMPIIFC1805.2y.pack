...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDAPLPAR_TMP%MSC%CVGQ,YDMF_PHYS_STATE%T,YDAPLPAR_TMP%MSC%TW,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%TKE,ZTRA(:,:,INBTRA_DEP:INBTRA),ZSMOOTRAC(1:INBTRA),&
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
...
!cdir unroll=8
      DO JLEV = YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG
        DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
          YDAPLPAR_TMP%MSC%CVGQ(JLON,JLEV) = YDAPLPAR_TMP%MSC%CVGQ(JLON,JLEV)*ZMOD(JLON) &
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      CALL ACCVUD ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
       ...
       & YDAPLPAR_TMP%MSC%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,&
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDAPLPAR_TMP%MSC%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
