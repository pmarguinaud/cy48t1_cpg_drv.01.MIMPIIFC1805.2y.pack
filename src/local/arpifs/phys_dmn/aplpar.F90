...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
              & YDCPG_DIM%KSTEP, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,               &
...
IF (LMPHYS) THEN
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
  ...
  ELSE
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, ZQO3, .FALSE., YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
...
IF(LCOEFKTKE.AND.YDCPG_DIM%KSTEP == 0) THEN
  YDMF_PHYS_SURF%GSD_VH%PQSH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTN(JLON) =YDMF_PHYS_BASE_STATE%T(JLON,YDCPG_DIM%KFLEVG)
     ENDDO
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTHETAS(JLON)   = ZTSN(JLON)*(RATM/YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG))**RKAPPA
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP, YDPHY0, YDPHYDS, LLAROME, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                            &
     & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(:, YDCPG_DIM%KFLEVG), ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,                               &
     & ZTN, ZTSN, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_BASE_STATE%Q(:, YDCPG_DIM%KFLEVG), YDMF_PHYS_BASE_STATE%U(:, YDCPG_DIM%KFLEVG), &
     & YDMF_PHYS_BASE_STATE%V(:, YDCPG_DIM%KFLEVG), ZTHETAS, YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                     &
     & ZFEV, ZFMDU, ZFMDV)
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
...
!       Computation of ZRTI
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
          ZDPHI(JLON)=YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(JLON,YDCPG_DIM%KFLEVG)-YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
          ZPRS(JLON)=RD+ZRVMD*YDCPG_MISC%QS(JLON)
          ZRTI(JLON)=2.0_JPRB/(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R(JLON,YDCPG_DIM%KFLEVG)*YDMF_PHYS_BASE_STATE%T(JLON,YDCPG_DIM%KFLEVG)+RKAPPA*ZDPHI(JLON)&
    ...
    IF (LPTKE) THEN
      YDMF_PHYS_BASE_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG) = MAX(YDMF_PHYS_BASE_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ETKE_MIN)
    ENDIF
    IF (LCOEFK_PTTE) THEN
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZTHETAVS(JLON)=YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)*(1.0_JPRB+RETV*YDCPG_MISC%QS(JLON))&
         & *(RATM/YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG))**RKAPPA  
      ENDDO
      CALL ACCLPH ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA,               &
      & YDCPG_DIM%KFLEVG, ZTHETAV, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,            &
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, IMLTYPE, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,              &
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
      ...
      & YDCPG_DIM%KSTEP, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
...
! Computation of the 2 znlab used in acbl89
    ...
    & YDCPG_DIM%KLON, NTCOEF, NTCOET, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
...
!DEC$ IVDEP
  ...
  & .OR.  ( (LRAY.OR.LRAYSP).AND.(.NOT.LRSTAER)) ) THEN
    ...
    IF ( ( (LRAYFM.AND.NAER /= 0) .OR.LRAY.OR.LRAYSP).AND.LLAERO )  THEN
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,             &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,          &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    ...
    & NTCOEF, NTNEBU, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,   &
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDCPG_DIM%KSTEP, YDCFU%NFRRC, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                  &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,   &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,                &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & YDCPG_DIM%KSW, ZALBD, ZALBP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,      &
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZFLU_EMIS, YDMF_PHYS%RAD%EMTD,              &
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
              & YDCPG_DIM%KTDIA, NTSSG, YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      & NTDIFU, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,    &
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,    &
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        & YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                    &
        ...
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                      &
...
! calculation of variables for the old "ISBA" atmosphere scheme
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          ...
          & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZCOEFN, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, &
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZCFAQ, ZCFAS, ZCFAU, ZCFASV, ZCFBQ,                &
      ...
      IF (L3DTURB) THEN
        ...
        & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%TKE, ZTENDPTKE, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
      ...
      IF (LCOEFKTKE) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI,    &
...
!DEC$ IVDEP
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZPREN(JLON)=YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG)
      ENDDO
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
...
! PASS T9 CONDENSATES TO ACCSU
      ...
      & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                 &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,            &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
   ...
   & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZLHS, ZLHV, YDCPG_DYN0%CTY%EVEL, ZIPOI,                   &
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,                        &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
  ...
  !  ---------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,                     &
...
! -----------------------------------------------------
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    DO JLEV=1,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZGEOM1(JLON,JLEV)     = YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF(JLON,JLEV)-YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
    ...
    DO JLEV=0,YDCPG_DIM%KFLEVG
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZGEOMH(JLON,JLEV)=YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI(JLON,JLEV)-YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI(JLON,YDCPG_DIM%KFLEVG)
  ...
  IF ( LNORGWD ) THEN
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        Z_PP(JLON, JLEV) = YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_UU(JLON, JLEV) = YDMF_PHYS_BASE_STATE%U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VV(JLON, JLEV) = YDMF_PHYS_BASE_STATE%V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_TT(JLON, JLEV) = YDMF_PHYS_BASE_STATE%T(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VO(JLON, JLEV) = YDMF_PHYS_BASE_STATE%VOR(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_U(JLON, JLEV) = TSPHY * YDMF_PHYS_BASE_STATE%P1NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_V(JLON, JLEV) = TSPHY * YDMF_PHYS_BASE_STATE%P2NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
      ENDDO
    ENDDO
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        YDMF_PHYS_BASE_STATE%P1NOGW(JLON, JLEV) = ZD_U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
        YDMF_PHYS_BASE_STATE%P2NOGW(JLON, JLEV) = ZD_V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
      ENDDO
    ENDDO
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME (YDMODEL%YRCST, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTDRME, YDCPG_DIM%KFLEVG,  &
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,          &
...
!     ------------------------------------------------------------------
  ...
  IF (LPHSPSH) THEN
    DO JLON=YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
       YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=(YDMF_PHYS_BASE_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)) * ((&
    ...
    ENDDO
    IF (LGRAPRO) THEN
      YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)+(YDMF_PHYS_BASE_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)) * (&
...
!     -----------------------------------------------------------------------
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     ...
     & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(:, JLEV), &
...
IF (LOZONE) THEN
  ...
  & YDMF_PHYS_NEXT_STATE%O3 (:, 1:YDCPG_DIM%KFLEVG), YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP)
ENDIF
...
IF (NDPSFI == 1) THEN
  ...
  & ZPFL_FP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, &
...
IF ((.NOT.LSFORCS)) THEN
  ...
  IF(LNUDG)THEN
    ...
    & YDMF_PHYS_SURF%GSP_SB%PQ_T1, YDMF_PHYS_SURF%GSP_SG%PF_T1, YDMF_PHYS_NEXT_STATE%T (:, 1:YDCPG_DIM%KFLEVG), &
    & YDMF_PHYS_NEXT_STATE%Q (:, 1:YDCPG_DIM%KFLEVG), YDMF_PHYS_NEXT_STATE%U (:, 1:YDCPG_DIM%KFLEVG),           &
    & YDMF_PHYS_NEXT_STATE%V (:, 1:YDCPG_DIM%KFLEVG), YDMF_PHYS_NEXT_STATE%SP, YDVARS%T%T0, YDVARS%Q%T0,        &
