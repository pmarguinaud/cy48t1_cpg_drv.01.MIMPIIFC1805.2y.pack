!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDMF_PHYS_SURF,YDSURFVARS,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PALBH(KLON)
...
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSNA2(KLON)
REAL(KIND=JPRB)   ,INTENT(INOUT) :: PSND2(KLON)
...
! calculation of variables for the old "ISBA" atmosphere scheme
        IF (.NOT. LELAM) THEN
          CALL ARO_GROUND_DIAG_2ISBA( KBL, KGPCOMP, &
            ...
            & PSNA2(KIDIA:KFDIA), PSND2(KIDIA:KFDIA), ZHV2 )
          DO JLON=KIDIA,KFDIA
            PHV(JLON) = ZHV2(JLON)
          ENDDO
        ENDIF
