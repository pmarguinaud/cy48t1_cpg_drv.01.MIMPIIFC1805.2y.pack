!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDAPLPAR,YDCPG_DYN0,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
...
!-------------------------------------------------
!
IF(LGCHECKMV) CALL CHECKMV(YDRIP,YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,KLEV,KSTEP,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF &
& ,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,PGELAM,PGEMU,PMU0,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR%T,YDAPLPAR%Q,YDAPLPAR%YGSP_RR%T)
!     ------------------------------------------------------------------
...
IF (LMPHYS) THEN
  IF (LOZONE) THEN
    ...
    DO JLEV=KTDIA,KLEV
      DO JLON=KIDIA,KFDIA
        ZQO3(JLON,JLEV) = MAX(ZEPSO3,YDAPLPAR%O3(JLON,JLEV))
      ENDDO
    ENDDO
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(KIDIA,KFDIA,KLON,KLEV,YDAPLPAR%YCPG_PHY%PREHYD,PGEMU,ZROZ)
      DO JLON=KIDIA,KFDIA
        ZQO3(JLON,0)=1.E-9_JPRB
      ENDDO
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          ZQO3(JLON,JLEV)=ZROZ(JLON,JLEV)*YDAPLPAR%YCPG_PHY%XYB%RDELP(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF

  ELSE
    CALL SUOZON(KIDIA,KFDIA,KLON,KLEV,ZQO3,.FALSE.,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%XYB%RDELP,LO3ABC,YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
!     LAYER WEIGHTS
      ...
      ZPOID(JLON,JLEV)=YDAPLPAR%YCPG_PHY%XYB%DELP(JLON,JLEV)*ZGDTI
      ZIPOI(JLON,JLEV)=YDAPLPAR%YCPG_PHY%XYB%RDELP(JLON,JLEV)*ZGDT

!     CALCULATION OF LATENT HEATS

      ZLHV(JLON,JLEV)=FOLH(YDAPLPAR%T(JLON,JLEV),0.0_JPRB)
      ZLHS(JLON,JLEV)=FOLH(YDAPLPAR%T(JLON,JLEV),1.0_JPRB)
...
!  SIZE OF ARRAY FOR MSE
  ...
  ZINVDT=1/PDT
  ZINVG=1/RG
  ZZI_APHI=YDAPLPAR%YCPG_DYN%PHI
  ZTM=YDAPLPAR%T
!SETUP
  ZZI_SVM=0.0_JPRB
...
!Initialisation des scalaires passifs pour aro_ground_param
  DO JGFL=1,NGFL_EXT
    DO JLEV=KTDIA,KLEV
      DO JLON=KIDIA,KFDIA
       ZSVM(JLON,JLEV,JGFL)=MAX(YDAPLPAR%P1EXT(JLON,JLEV,JGFL),0.0_JPRB)
      ENDDO
    ENDDO
  ENDDO
...
!Initialisation de ZZI_RHODREFM
  DO JLEV = 1 , KLEV
    DO JLON=KIDIA,KFDIA
      ZZI_RHODREFM(JLON,1,JLEV)=YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV)/&
       & (YDAPLPAR%T(JLON,JLEV)*YDAPLPAR%YCPG_DYN%RCP%R(JLON,JLEV))
     ENDDO
  ENDDO
...
!Initialisation de ZZI_PABSM
  DO JLON = KIDIA,KFDIA
    DO JLEV = 1 , KLEV
      ZZI_PABSM(JLON,1,JLEV)=YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV)
    ENDDO
  ENDDO
...
!Initialisation de ZZI_EXNREFM
  ...
  DO JLEV = 1 , KLEV
    DO JLON = KIDIA,KFDIA
      ZZI_EXNREFM(JLON,1,JLEV)=(YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV)*ZINVATM)**(ZRSCP)
    ENDDO
  ENDDO
...
!cdir unroll=8
      DO JLEV = 1, KLEV
        DO JLON = KIDIA,KFDIA
          ZQI(JLON,JLEV)=MAX(0.0_JPRB,YDAPLPAR%I(JLON,JLEV))
          ZQL(JLON,JLEV)=MAX(0.0_JPRB,YDAPLPAR%L(JLON,JLEV))
          ZQR(JLON,JLEV)=MAX(0.0_JPRB,YDAPLPAR%R(JLON,JLEV))
          ZQS(JLON,JLEV)=MAX(0.0_JPRB,YDAPLPAR%S(JLON,JLEV))
          ZQG(JLON,JLEV)=MAX(0.0_JPRB,YDAPLPAR%G(JLON,JLEV))
  ...
  !                         FOR RAIN/SNOW/GRAUPEL PUT IN PFCQR/S/GNG
          ...
          ZDQI=ZQI(JLON,JLEV)-YDAPLPAR%I(JLON,JLEV)
          ZDQL=ZQL(JLON,JLEV)-YDAPLPAR%L(JLON,JLEV)
          ZDQR=ZQR(JLON,JLEV)-YDAPLPAR%R(JLON,JLEV)
          ZDQS=ZQS(JLON,JLEV)-YDAPLPAR%S(JLON,JLEV)
          ZDQG=ZQG(JLON,JLEV)-YDAPLPAR%G(JLON,JLEV)
          ZDQC=ZDQI+ZDQL+ZDQR+ZDQS+ZDQG

          ZQV0=YDAPLPAR%Q(JLON,JLEV)-ZIPOI(JLON,JLEV)*(0.0_JPRB- YDMF_PHYS%OUT%FCQNG(JLON,JLEV-1) &
          ...
          ZDQV=MAX(0.0_JPRB,ZQV0-ZDQC)-YDAPLPAR%Q(JLON,JLEV)
  ...
  ELSE
    DO JLEV = 1, KLEV
      DO JLON = KIDIA,KFDIA
        ZQI(JLON,JLEV)=YDAPLPAR%I(JLON,JLEV)
        ZQL(JLON,JLEV)=YDAPLPAR%L(JLON,JLEV)
        ZQR(JLON,JLEV)=YDAPLPAR%R(JLON,JLEV)
        ZQS(JLON,JLEV)=YDAPLPAR%S(JLON,JLEV)
        IF (LGRAPRO) THEN
          ZQG(JLON,JLEV)=YDAPLPAR%G(JLON,JLEV)
        ENDIF
        ZQV(JLON,JLEV)=YDAPLPAR%Q(JLON,JLEV)
      ENDDO
    ENDDO
  ENDIF
ELSE
...
IF(LCVPGY.AND.KSTEP == 0) THEN
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      YDAPLPAR%CVV(JLON,JLEV)=0.0_JPRB
    ENDDO
  ENDDO
ENDIF
...
IF(LCOEFKTKE.AND.KSTEP == 0) THEN
  YDMF_PHYS_SURF%GSD_VH%PQSH(KIDIA:KFDIA)=YDAPLPAR%Q(KIDIA:KFDIA,KLEV)
ENDIF
...
!     ----------------------------
  IF ( LTHERMO ) THEN
    CALL ACTQSAT ( YDPHY,KIDIA,KFDIA,KLON,NTQSAT,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, ZQV, YDAPLPAR%T,&
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     DO JLON=KIDIA,KFDIA
        ZTSN(JLON)=YDAPLPAR%YGSP_RR%T(JLON)
        ZTN(JLON) =YDAPLPAR%T(JLON,KLEV)
     ENDDO
     DO JLON=KIDIA,KFDIA
        ZRHODREFM(JLON) = YDAPLPAR%YCPG_PHY%PREHYDF(JLON,KLEV)/(YDAPLPAR%T(JLON,KLEV)*YDAPLPAR%YCPG_DYN%RCP%R(JLON,KLEV))
        ZTHETAS(JLON)   = ZTSN(JLON)*(RATM/YDAPLPAR%YCPG_PHY%PREHYD(JLON,KLEV))**RKAPPA
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS,LLAROME,KIDIA,KFDIA,KLON,YDAPLPAR%YCPG_DYN%PHIF(:,KLEV),ZRHODREFM,YDMF_PHYS_SURF%GSD_SFO%PGROUP,&
        & ZTN,ZTSN,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR%Q(:,KLEV),YDAPLPAR%U(:,KLEV),YDAPLPAR%V(:,KLEV),ZTHETAS,YDMF_PHYS%OUT%FCS(KIDIA:KFDIA,1),ZFEV,ZFMDU,ZFMDV)
     DO JLON=KIDIA,KFDIA
        YDAPLPAR%YGSP_RR%T(JLON)=ZTSN(JLON)
     ENDDO
     DO JLON=KIDIA,KFDIA
        ZDELTA=MAX(0.0_JPRB,SIGN(1.0_JPRB,RTT-YDAPLPAR%YGSP_RR%T(JLON)))
        ...
        PLHS(JLON)=FOLH(YDAPLPAR%YGSP_RR%T(JLON),0.0_JPRB)
     ENDDO
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      CALL ACSOL ( YDPHY,YDPHY1,KIDIA,KFDIA,KLON,&
       & YDMF_PHYS_SURF%GSD_VV%PARG,YDMF_PHYS_SURF%GSD_VV%PD2,YDMF_PHYS_SURF%GSD_VF%PZ0F,YDMF_PHYS_SURF%GSD_VV%PZ0H,YDMF_PHYS_SURF%GSD_VF%PZ0RLF,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_SURF%GSD_VV%PIVEG,YDMF_PHYS_SURF%GSD_VV%PLAI,YDAPLPAR%YGSP_SG%A,&
       & YDAPLPAR%YGSP_SG%R,YDMF_PHYS_SURF%GSD_VV%PSAB,YDAPLPAR%YGSP_SG%F,YDAPLPAR%YGSP_RR%T,YDMF_PHYS_SURF%GSD_VF%PVEG,YDAPLPAR%YGSP_SB%Q,YDAPLPAR%YGSP_SB%TL,YDAPLPAR%YGSP_RR%W,YDAPLPAR%YGSP_RR%IC,&
       & LLHMT,&
...
!DEC$ IVDEP
      DO JLON = KIDIA,KFDIA
        YDMF_PHYS%OUT%CT(JLON)=HSOL /(&
         & 1.0_JPRB+HSOLIWR*(YDAPLPAR%YGSP_RR%W(JLON)+YDAPLPAR%YGSP_SB%Q(JLON,1))/(WSMX+WPMX)&
         & *EXP(-0.5_JPRB*(HSOLIT0*(YDAPLPAR%YGSP_RR%T(JLON)-RTT))**2))
      ENDDO
    ENDIF
  ENDIF
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
    ...
    DO JLON=KIDIA,KFDIA
      ...
      YDAPLPAR%YGSP_RR%T      (JLON) = PGPAR(JLON,MVTS)
    ...
    DO JLON=KIDIA,KFDIA
      IF (PEMIS(JLON)==0._JPRB) THEN
        ...
        YDAPLPAR%YGSP_RR%T  (JLON) = 288.0_JPRB
      ...
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (KSTEP == 0) THEN 
        ...
        CALL ACTKEZOTLS ( YDPHY0,KIDIA,KFDIA,KLON,KLEV,&
         & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T,&
         & YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, YDAPLPAR%YGSP_RR%T, YDCPG_MISC%QS,&
         & PCDN, ZCDNH, PCPS, ZRTI, PRS)
         
      ELSE
        CALL ACHMTLS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
          & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,&
          & YDAPLPAR%YGSP_RR%T,YDCPG_MISC%QS,ZDPHIT,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,LLCLS,&
          & ZNBVNO,ZMRIPP,PCPS,ZGWDCS,PLHS,ZPCLS,PCD,PCDN)
!       Computation of ZRTI
        DO JLON=KIDIA,KFDIA
          ZDPHI(JLON)=YDAPLPAR%YCPG_DYN%PHIF(JLON,KLEV)-YDAPLPAR%YCPG_DYN%PHI(JLON,KLEV)
          ZPRS(JLON)=RD+ZRVMD*YDCPG_MISC%QS(JLON)
          ZRTI(JLON)=2.0_JPRB/(YDAPLPAR%YCPG_DYN%RCP%R(JLON,KLEV)*YDAPLPAR%T(JLON,KLEV)+RKAPPA*ZDPHI(JLON)&
           & +ZPRS(JLON)*YDAPLPAR%YGSP_RR%T(JLON))
        ENDDO
      ENDIF
    ELSE
      IF (LCOEFKSURF) THEN
        CALL ACTKEHMT ( KIDIA,KFDIA,KLON,KLEV,KVCLIV>=8.AND.LSOLV,&
         & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, ZQV, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V,&
         & PFPLSH,PFPLCH,&
         ...
         & ZNEIJG, ZNEIJV, YDAPLPAR%YGSP_SG%F, YDAPLPAR%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VF%PVEG, ZWFC, YDAPLPAR%YGSP_RR%W, YDAPLPAR%YGSP_RR%IC,&
         & LLCLS, LLHMT,&
         & ZNBVNO,ZMRIPP,&
    ...
    IF (LPTKE) THEN
      YDAPLPAR%TKE(KIDIA:KFDIA,1:KLEV) = MAX(YDAPLPAR%TKE(KIDIA:KFDIA,1:KLEV),ETKE_MIN)
    ENDIF
    IF (LCOEFK_PTTE) THEN
    ...
    ELSE
      ZCP(KIDIA:KFDIA,1:KLEV) = YDAPLPAR%YCPG_DYN%RCP%CP(KIDIA:KFDIA,1:KLEV)
    ENDIF
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, PQSAT,&
       & YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, PLSCPE,YDMF_PHYS%OUT%GZ0,&
       & ZMN2PP,ZMRIPP)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          ZTHETAV(JLON,JLEV)=YDAPLPAR%T(JLON,JLEV)*(1.0_JPRB+RETV*ZQV(JLON,JLEV))&
           & *(RATM/YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV))**RKAPPA  
        ENDDO
      ENDDO
      DO JLON=KIDIA,KFDIA
        ZTHETAVS(JLON)=YDAPLPAR%YGSP_RR%T(JLON)*(1.0_JPRB+RETV*YDCPG_MISC%QS(JLON))&
         & *(RATM/YDAPLPAR%YCPG_PHY%PREHYD(JLON,KLEV))**RKAPPA  
      ENDDO
      CALL ACCLPH ( YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       & ZTHETAV,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%U,YDAPLPAR%V,ZTHETAVS,&
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      CALL ACMIXLENZ ( YDPHY,YDPHY0,KIDIA,KFDIA,KLON,1,KLEV,.FALSE.,&
       & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,ZBLH,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZLMU,ZLMT)
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV, &
       & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZBLH,ZLMU,ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZBLH,ZZLMT,ZLMT)
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         ...
         & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%T,ZQV,ZQL,ZQI,YDAPLPAR%R,YDAPLPAR%S,YDAPLPAR%TKE,&
         & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_PHY%XYB%LNPR,ZMN2PP,ZFMGST,PFPLSH,PFPLCH,&
         & YDMF_PHYS%OUT%GZ0,YDAPLPAR%YGSP_RR%T,PCDN,ZBLH,ZLMU,ZLMT,YDVARS%MXL%T0,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
         & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%YCPG_PHY%XYB%DELP, PLSCPE,&
         & ZQV, ZQL, ZQI, PQSAT,&
         & YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, ZLMU, ZLMT,YDMF_PHYS%OUT%GZ0,&
         & ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,KSTEP,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF,&
       & ZQV, ZQL, ZQI,ZCP,&
       & YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_PHY%XYB%LNPR,PQSAT,PQW,PTW,&
       & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,&
       & YDVARS%FQTUR%T0,YDVARS%FSTUR%T0,YDVARS%SHTUR%T0,&
       & YDAPLPAR%TKE,YDVARS%TTE%T0,&
       & YDAPLPAR%YGSP_RR%T,YDMF_PHYS_SURF%GSD_VH%PQSH,PRS,PCPS,ZRTI,YDMF_PHYS%OUT%GZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      DO JLON=KIDIA,KFDIA
        ZCRTI(JLON) = 1.0_JPRB/(YDAPLPAR%YGSP_RR%T(JLON)*PRS(JLON))
      ENDDO
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%Q,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,&
       & ZDPHIT,ZDPHIV,YDMF_PHYS%OUT%GZ0,YDMF_PHYS_SURF%GSD_VF%PLSM,PQSATS,ZCRTI,YDAPLPAR%YGSP_RR%T,&
  ...
  IF (LRAFTKE) THEN
    ...
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDAPLPAR%U, YDAPLPAR%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, YDAPLPAR%TKE, YDAPLPAR%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,ZBLH,KCLPH)
  ENDIF
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      CALL ACTKECOEFK ( YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD,&
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%YCPG_PHY%XYB%DELP, PLSCPE, ZQV, ZQL, ZQI, PQSAT,&
       & YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, PFPLSH, PFPLCH, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH,&
  ...
  IF (LCVPPKF) THEN
    CALL ACVPPKF( YDMODEL%YRML_PHY_MF,KIDIA, KFDIA, KLON, NTCVIM, KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%XYB%DELP, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, ZQV,&
     & ZQL, ZQI, YDAPLPAR%U, YDAPLPAR%V, YDCPG_DYN0%CTY%VVEL(:,1:), YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%TKE,&
  ...
  IF (LEDKF) THEN
    ...
    CALL ARP_SHALLOW_MF( KIDIA,KFDIA,KLON,KTDIA,KLEV,ZIMPL,TSPHY,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_DYN%RCP%CP, &
                      & CMF_UPDRAFT,CMF_CLOUD,LMIXUV, &
                      & YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%T,ZQV,ZQL,ZQI,ZQR,ZQS,YDAPLPAR%TKE,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%XYB%DELP,          &
...
! Computation of the 2 znlab used in acbl89
    ...
    CALL ACTKE ( YDLDDH,YDMODEL%YRML_DIAG%YRMDDH,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTCOET,KLEV,&
     & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_PHY%XYB%DELP,&
     & YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, ZQV, ZQIC, ZQLC, PLSCPE,&
     & PCD, PCH, YDMF_PHYS%OUT%GZ0, YDAPLPAR%YGSP_RR%T, YDCPG_MISC%QS,&
     & YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDAPLPAR%TKE, ZPRODTH_CVPP, ZNLAB, ZNLABCVP,&
    ...
    DO JLON=KIDIA,KFDIA
      IF (LSNV) THEN
        ...
        YDMF_PHYS%OUT%ALB(JLON)= (1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PVEG(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
         ...
         & MAX(YDMF_PHYS_SURF%GSD_VF%PALBSF(JLON),YDAPLPAR%YGSP_SG%A(JLON,1))&
         & + YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*ZNEIJV(JLON) *&
         & MAX(ZALBV,YDAPLPAR%YGSP_SG%A(JLON,1))&
        ...
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-PVEG(JLON)-PNEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & PVEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+PNEIJ(JLON)*YDAPLPAR%YGSP_SG%A(JLON,1)
          ELSE
            ...
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-PNEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & YDAPLPAR%YGSP_SG%A(JLON,1))+(PNEIJ(JLON)-ZNEIJV(JLON))*     &
             & YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*(YDMF_PHYS_SURF%GSD_VV%PALV(JLON)-YDAPLPAR%YGSP_SG%A(JLON,1))
          ENDIF
...
!DEC$ IVDEP
        DO JLON=KIDIA,KFDIA
          ZMERL(JLON)=(1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PLSM(JLON))*(1.0_JPRB&
            & -MAX(0.0_JPRB,SIGN(1.0_JPRB,TMERGL-YDAPLPAR%YGSP_RR%T(JLON))))
    ...
    ELSEIF (LRAY) THEN
      ...
      DO JLON=KIDIA,KFDIA
        IF ( YDMF_PHYS_SURF%GSD_VF%PLSM(JLON) < 0.5_JPRB .AND. YDAPLPAR%YGSP_RR%T(JLON) >= TMERGL ) THEN
  ...
  & .OR.  ( (LRAY.OR.LRAYSP).AND.(.NOT.LRSTAER)) ) THEN
    ...
    IF ( ( (LRAYFM.AND.NAER /= 0) .OR.LRAY.OR.LRAYSP).AND.LLAERO )  THEN
      CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD,YDERAD,YDPHY, KIDIA , KFDIA , KLON  , KLEV,&
       & YDAPLPAR%YCPG_PHY%PREHYD , YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%T    , YDAPLPAR%YGSP_RR%T,&
  ...
  IF (LRAY.AND.NRAY == 1.AND.KNFRRC /= 0) THEN
    IF (MOD(KSTEP,KNFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       ...
       & YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%DELP,YDCPG_MISC%NEB,&
       & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,YDAPLPAR%T,&
       & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PMU0,PGEMU,PGELAM,PMU0LU,YDAPLPAR%YGSP_RR%T,&
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    CALL ACCVIMPGY ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     & YDAPLPAR%YCPG_PHY%XYB%ALPH, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP,&
     & YDAPLPAR%YCPG_PHY%XYB%DELP,PLH,YDAPLPAR%YCPG_PHY%XYB%LNPR,ZQV,ZQCI,ZQCL,ZQLIS,PQSAT,PQW,&
     & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%T,PTW, YDAPLPAR%U, YDAPLPAR%V,&
     & PCPS,PGM,YDAPLPAR%YGSP_RR%T,&
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,LLREDPR,&
     & ZHUC,ZVETAF,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%DELP,YDCPG_MISC%RH,ZBLH,ZQV,ZQI,ZQL,&
     & PQW,YDAPLPAR%T,ZNEBCH,PGM,YDAPLPAR%YGSP_RR%T,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      ! - INPUT  2D .
      & YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDCPG_DYN0%CTY%VVEL(:,1:),YDAPLPAR%YCPG_DYN%RCP%CP,PLH,YDAPLPAR%YCPG_DYN%RCP%R,&
      & YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%YCPG_PHY%XYB%LNPR,&
      & YDAPLPAR%Q,ZQI,ZQL,YDAPLPAR%R,YDAPLPAR%S,ZQLIS,PQSAT,PQW,&
      & YDAPLPAR%YCPG_PHY%XYB%RDELP,PCVGQ,YDAPLPAR%T,PTW,YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%TKE,ZTRA(:,:,INBTRA_DEP:INBTRA),ZSMOOTRAC(1:INBTRA),&
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
      & PGM, YDAPLPAR%YGSP_RR%T, YDCPG_MISC%QS, PNEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    CALL ACNEBN ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
     & YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_DYN%RCP%CP,ZQV,ZQL,ZQI,PQSAT,YDAPLPAR%T,PFPLCH,YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%YCPG_PHY%PREHYDF,&
     & ZUNEBH,&
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTNEBU,KLEV,&
     & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,PLSCPE,ZQV,&
     & PQSAT,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS%OUT%GZ0,PFPLCH,YDMF_PHYS_SURF%GSD_VH%PPBLH,YDCPG_MISC%QS,YDAPLPAR%YGSP_RR%T,&
...
!         Diagnostique de nebulosite partielle.
  CALL ACNPART(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
   & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYDF,ZDECRD,YDCPG_MISC%NEB,&
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(KSTEP == 1).AND.LRCOEF ) THEN
    ...
    CALL ACRADCOEF ( YDRCOEF,YDPHY3,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%XYB%DELP,ZZNEB,ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,&
     & ZQO3,YDAPLPAR%T,&
     & PMMU0,YDCPG_MISC%DHSF,PEMIS,&
     & ZAER,&
     & POMPAC,PAC,YDMF_PHYS%TMP%RDT%COR,&
     & YDMF_PHYS%TMP%RDT%RAB3C,YDMF_PHYS%TMP%RDT%RAB3N,YDMF_PHYS%TMP%RDT%RAB4C,YDMF_PHYS%TMP%RDT%RAB4N,YDMF_PHYS%TMP%RDT%RAB6C,YDMF_PHYS%TMP%RDT%RAB6N,&
     & YDMF_PHYS%TMP%RDT%RAT1C,YDMF_PHYS%TMP%RDT%RAT1N,YDMF_PHYS%TMP%RDT%RAT2C,YDMF_PHYS%TMP%RDT%RAT2N,YDMF_PHYS%TMP%RDT%RAT3C,YDMF_PHYS%TMP%RDT%RAT3N,&
     & YDMF_PHYS%TMP%RDT%RAT4C,YDMF_PHYS%TMP%RDT%RAT4N,YDMF_PHYS%TMP%RDT%RAT5C,YDMF_PHYS%TMP%RDT%RAT5N)
  ENDIF
...
!     7.3.6 Module chimique - Chemistry module
  ...
  IF (LCHEM_ARPCLIM) THEN ! at this stage call when ARPEGE-Climat chemistry only
     ...
     DO JLEV=KTDIA,KLEV
       DO JLON=KIDIA,KFDIA
         ZDELP(JLON,JLEV) = YDAPLPAR%YCPG_PHY%PREHYD(JLON,JLEV) - YDAPLPAR%YCPG_PHY%PREHYD(JLON,JLEV-1)
       ENDDO
     ENDDO
...
!DEC$ IVDEP
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         ...
         & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%DELP,YDCPG_MISC%NEB,&
         & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,YDAPLPAR%T,&
         & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PGELAM,PGEMU,PMU0,PMU0LU,YDAPLPAR%YGSP_RR%T,ZDECRD,ZCLCT_RAD,&
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV,YDMODEL,     &
       ...
       &  ZALBD , ZALBP, YDAPLPAR%YCPG_PHY%PREHYD , YDAPLPAR%YCPG_PHY%PREHYDF ,         &
       &  YDCPG_MISC%NEB  , ZQO3 , ZAER  , YDAPLPAR%YCPG_PHY%XYB%DELP  , PEMIS , &
       &  PMU0M , ZQV  , PQSAT , YDCPG_MISC%QICE  , YDCPG_MISC%QLI  , & 
       &  ZQS   , ZQR  , YDMF_PHYS_SURF%GSD_VF%PLSM  , YDAPLPAR%T     , YDAPLPAR%YGSP_RR%T   , &
       &  PGP2DSPP, PEZDIAG, &
    ...
    &   KIDIA  , KFDIA  , KLON    , KLEV,&
    &   YDAPLPAR%YCPG_PHY%PREHYD  , PEMIS  , YDMF_PHYS%RAD%EMTD   , PMU0, ZQV  ,&
    &   ZTENT  , YDMF_PHYS%RAD%TRSW   , ZTRSOD , YDAPLPAR%YGSP_RR%T , TSPHY,&
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU ,PMU0,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU  ,PMU0,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    CALL ACVEG ( YDPHY,YDPHY1,KIDIA,KFDIA,KLON,KLEV,&
     & YDMF_PHYS%OUT%FRSO,ZQV,PQSAT,YDAPLPAR%T,&
     ...
     & ZCHROV,ZGWDCS,ZWFC,YDAPLPAR%YGSP_RR%FC,ZWWILT,YDAPLPAR%YGSP_SB%Q,PQSATS,&
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDIFU,KLEV,KCSS,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO, ZKTROV, ZKUROV, ZKNROV,&
       & ZQV, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, ZXTROV, ZXUROV, ZXPTKEROV, YDAPLPAR%YCPG_PHY%XYB%ALPH,&
       & YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_DYN%RCP%R, ZQL, ZQI, ZNEBDIFF, YDAPLPAR%TKE, ZLMU, YDAPLPAR%YCPG_PHY%XYB%LNPR,&
       ...
       & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, PNEIJ, YDCPG_MISC%QS, PQSATS, YDAPLPAR%YGSP_SB%T, YDAPLPAR%YGSP_RR%T, PVEG,&
       & ZXDROV, ZXHROV, YDAPLPAR%YGSP_RR%W, YDAPLPAR%YGSP_RR%IC,&
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        CALL ACPTKE(YGFL,YDLDDH,YDMODEL%YRML_PHY_MF,&
          ...
          & YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%YCPG_PHY%XYB%RDELP,ZKNROV,&
          & YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%YCPG_PHY%XYB%ALPH,&
          & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%TKE,ZLMU,YDAPLPAR%YCPG_PHY%XYB%LNPR,ZF_EPS,ZFUN_TTE,&
          & ZMRIPP,ZMRIFPP,ZRHS,&
      ...
      CALL ACDIFV1 ( YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
        & YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYDF,ZCP,YDAPLPAR%YCPG_PHY%XYB%DELP,ZKTROV,ZKQROV,ZKUROV,&
        & ZQV,ZQL,ZQI,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,ZSVM,ZXTROV,ZXUROV,&
        & ZXDROV,ZXHROV,&
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        DO JLON=KIDIA,KFDIA
          ZRHODREFM(JLON)=YDAPLPAR%YCPG_PHY%PREHYDF(JLON,KLEV)/(YDAPLPAR%T(JLON,KLEV)*YDAPLPAR%YCPG_DYN%RCP%R(JLON,KLEV))
          ZDEPTH_HEIGHT(JLON,:)=(YDAPLPAR%YCPG_DYN%PHIF(JLON,:)-YDAPLPAR%YCPG_DYN%PHI(JLON,KLEV))/RG
          ZZS(JLON)=YDAPLPAR%YCPG_DYN%PHI(JLON,KLEV)/RG
        ENDDO
        ...
        CALL ARO_GROUND_PARAM( KBL, KGPCOMP,&
          ...
          & PINDX(KIDIA:KFDIA), PINDY(KIDIA:KFDIA), &
          & YDAPLPAR%U(KIDIA:KFDIA,KLEV:KLEV),YDAPLPAR%V(KIDIA:KFDIA,KLEV:KLEV), &
          & YDAPLPAR%T(KIDIA:KFDIA,KLEV:KLEV),YDAPLPAR%Q(KIDIA:KFDIA,KLEV:KLEV),&
          & ZSVM(KIDIA:KFDIA,KLEV:KLEV,1:NGFL_EXT),&
          & ZCARDI,&
          & ZRHODREFM(KIDIA:KFDIA),&
          & YDAPLPAR%YCPG_PHY%PREHYDF(KIDIA:KFDIA,KLEV:KLEV),YDAPLPAR%YCPG_PHY%PREHYD(KIDIA:KFDIA,KLEV:KLEV),&
...
! Opposite water vapor flux
        ...
        CALL ARO_GROUND_DIAG( KBL, KGPCOMP,&
          ...
          & YDAPLPAR%U(KIDIA:KFDIA,1:KLEV), YDAPLPAR%V(KIDIA:KFDIA,1:KLEV),&
...
! calculation of variables for the old "ISBA" atmosphere scheme
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          CALL ACAA1 ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
            & YDAPLPAR%YCPG_PHY%PREHYD,ZCOEFN,YDAPLPAR%YCPG_DYN%RCP%CP,ZQL,ZQI,YDAPLPAR%T,&
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          CALL ARP_GROUND_PARAM ( YDMCC,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KCSS,&
           & YDAPLPAR%YCPG_DYN%PHI,ZCP,YDMF_PHYS%OUT%FRSO,&
           & ZQV,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%T,YDAPLPAR%U,YDAPLPAR%V,ZXURO,ZXQRO,ZXTRO,&
           & YDAPLPAR%YCPG_PHY%PREHYD,ZQL,ZQI,ZNEBDIFF,&
           ...
           & YDAPLPAR%YGSP_SB%T,YDAPLPAR%YGSP_RR%T,PVEG,ZXDROV,ZXHROV,YDAPLPAR%YGSP_RR%W,YDAPLPAR%YGSP_RR%IC,&
           & ZDSE,&
           & ZCFAS,ZCFAU,&
           & ZCFBS,ZCFBU,ZCFBV,&
      ...
      CALL ACDIFV2 ( YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
        & YDAPLPAR%YCPG_PHY%PREHYD,ZCFAQ,ZCFAS,ZCFAU,ZCFASV,ZCFBQ,ZCFBS,ZCFBU,ZCFBV,ZCFBSV,&
        & ZKTROV,ZKQROV,ZKQLROV,ZKUROV,ZDSE,ZQT,YDAPLPAR%U,YDAPLPAR%V,ZPOID,&
        & YDAPLPAR%T,ZQL,ZQI,YDAPLPAR%YCPG_PHY%XYB%RDELP,&
      ...
      IF (L3DTURB) THEN
        CALL ACTKECOEFKH(YDRIP,YDMODEL%YRML_PHY_MF,YDGEOMETRY%YREGEO,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
          & YDAPLPAR%TKE, PTENDPTKE, YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%YCPG_PHY%XYB%ALPH,&
          & YDAPLPAR%DIV, YDAPLPAR%VOR, YDVARS%U%DL, YDVARS%V%DL, YDAPLPAR%YCPG_PHY%W, YDAPLPAR%YCPG_PHY%WL, YDAPLPAR%YCPG_PHY%WM, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%YCPG_PHY%XYB%LNPR,&
      ...
      IF (LCOEFKTKE) THEN
        ...
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KSTEP,&
          & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,ZCP,YDAPLPAR%YCPG_PHY%XYB%DELP,ZKTROV,ZXTROV,&
          & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%RDELP,ZCHROV,ZXHROV,&
          & ZCOEFA,ZQV,ZQL,ZQI,YDAPLPAR%U,YDAPLPAR%V,YDAPLPAR%T,YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_PHY%XYB%LNPR,&
          & ZLML, ZTH_FUN,ZWW_FUN, ZF_EPS,&
          & YDMF_PHYS%OUT%DIFTS,YDMF_PHYS%OUT%DIFTQ,YDAPLPAR%TKE,PTENDPTKE,&
...
!DEC$ IVDEP
      DO JLON = KIDIA,KFDIA
        ...
        ZT(JLON,JLEV)=YDAPLPAR%T(JLON,JLEV)-ZIPOI(JLON,JLEV)/YDAPLPAR%YCPG_DYN%RCP%CP(JLON,JLEV)&
        ...
        YDMF_PHYS%OUT%FCQNG(JLON,JLEV)=YDMF_PHYS%OUT%FCQNG(JLON,JLEV)+ZFCQVNG(JLON,JLEV)
        IF(LRKCDEV.AND.KSTEP>0) THEN
          ZDTRAD(JLON,JLEV)=ZIPOI(JLON,JLEV)/YDAPLPAR%YCPG_DYN%RCP%CP(JLON,JLEV)* (&
    ...
    DO JLEV = KTDIA, KLEV
      DO JLON = KIDIA,KFDIA
        ...
        ZT (JLON,JLEV)= YDAPLPAR%T(JLON,JLEV)
        ZQX0=ZQI(JLON,JLEV)
        ZQX1=ZQX0-ZIPOI(JLON,JLEV)*(YDMF_PHYS%OUT%DIFTQN(JLON,JLEV)&
          & -YDMF_PHYS%OUT%DIFTQN(JLON,JLEV-1))
        ZDQI= MAX(0.0_JPRB,ZQX1)-ZQX1
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%XYB%DELP, YDAPLPAR%YCPG_DYN%RCP%CP,&
       & ZQV, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%R,&
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      DO JLON = KIDIA,KFDIA
        ZPREN(JLON)=YDAPLPAR%YCPG_PHY%PREHYD(JLON,KLEV)
      ENDDO
      CALL ACPBLH ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       & YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_DYN%RCP%CP,&
       & ZQV, YDAPLPAR%T, YDAPLPAR%U, YDAPLPAR%V,YDAPLPAR%YCPG_PHY%PREHYDF,&
       & YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV,&
       & ZPREN, YDAPLPAR%YGSP_RR%T, YDCPG_MISC%QS,&
...
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,KLEV,1)/PEMIS(JLON)+RSIGMA*YDAPLPAR%YGSP_RR%T(JLON)**4
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY,YDPHY1,KIDIA, KFDIA, KLON, KTDIA, KLEV,&
     & PEMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, PNEIJ, ZQV, YDCPG_MISC%QS, YDAPLPAR%YGSP_RR%T, ZCHROV,  ZDQSTS,&
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    CALL ACDRAG ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDRAG,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF,&
     & YDAPLPAR%YCPG_PHY%XYB%DELP, ZNBVNO, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%U, YDAPLPAR%V,&
 ...
 ! SAVE FOR TL/NL COEFS FROM VERT. DIFF AND GWD
  ...
  IF (LTRAJPS.AND.(LVDIFSPNL.OR.LGWDSPNL)) THEN
     ...
     CALL WRPHTRAJTM_NL(YDGEOMETRY,YDSIMPHL,KIDIA,KFDIA,PTRAJ_PHYS,&
                ...
                & YDAPLPAR%L,YDAPLPAR%I,YDAPLPAR%R,YDAPLPAR%S,ZQLIS,ZNEBS)
  ENDIF
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    CALL ACPLUIE ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%XYB%DELP,&
     & ZQV, PQW, YDAPLPAR%T,&
  ...
  IF ( LSTRAS ) THEN
    CALL ACPLUIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%XYB%DELP,ZNEBS,ZQV,ZQLIS,PQW,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%T,&
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      CALL ACTQSATS ( YDPHY,YDPHY0,KIDIA,KFDIA,KLON,NTQSAT,KLEV,&
       & YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, ZQV, ZT,&
       & ZGEOSLC, ZLH, ZLSCPE, ZQSAT, ZQW, ZRH, ZTW)
      CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,LLREDPR,&
       & ZHUC,ZVETAF,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%DELP,YDCPG_MISC%RH,ZBLH,ZQV,ZQI,ZQL,&
       & ZQW,ZT,ZNEBCH,PGM,YDAPLPAR%YGSP_RR%T,&
       & ZQLIS,ZNEBS,ZRHCRI,ZRH,ZQSATS,ZICEFR1,ZQLIS0,ZNEBS0)
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       & ZVETAF,YDAPLPAR%YCPG_DYN%PHI,&
       & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,ZQW,YDAPLPAR%YCPG_DYN%RCP%R,ZT,PTW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,PNEIJ,PGM,YDAPLPAR%YGSP_RR%T,ZDECRD_MF,&
    ...
    ELSE
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       & ZVETAF,YDAPLPAR%YCPG_DYN%PHI,&
       & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,PQW,YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%T,PTW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,PNEIJ,PGM,YDAPLPAR%YGSP_RR%T,ZDECRD_MF,&
       & YDMF_PHYS%OUT%FPFPSL,YDMF_PHYS%OUT%FPFPSN,YDMF_PHYS%OUT%FPFPSG,YDMF_PHYS%OUT%FCSQL,YDMF_PHYS%OUT%FCSQN,YDMF_PHYS%OUT%FPEVPSL,YDMF_PHYS%OUT%FPEVPSN,YDMF_PHYS%OUT%FPEVPSG,YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLSG,&
       & ZSEDIQL,ZSEDIQI,YDMF_PHYS%OUT%DIAGH)
    ...
    DO JLEV=KTDIA,KLEV
      DO JLON=KIDIA,KFDIA
        ...
        ZT(JLON,JLEV)=ZT(JLON,JLEV)+(ZLHV(JLON,JLEV)*ZDQL&
                   & +ZLHS(JLON,JLEV)*ZDQI)/YDAPLPAR%YCPG_DYN%RCP%CP(JLON,JLEV)
      ENDDO
    ENDDO
      ...
      DO JLEV = KTDIA, KLEV
        DO JLON = KIDIA, KFDIA
          PCVGQ(JLON,JLEV) = PCVGQ(JLON,JLEV)*ZMOD(JLON) &
           & -RG*YDAPLPAR%YCPG_PHY%XYB%RDELP(JLON,JLEV)&
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      CALL ACCVUD ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       & YDAPLPAR%YCPG_PHY%XYB%ALPH, YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,&
       & PCVGQ, YDAPLPAR%YCPG_PHY%XYB%DELP, YDAPLPAR%YCPG_PHY%XYB%LNPR,&
       & ZQV, ZQI, ZQL, ZQSAT, ZQW,&
       & YDAPLPAR%YCPG_PHY%XYB%RDELP, ZT, ZTW, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%VOR,&
       & YDVARS%DAL%T0,&
       & YDAPLPAR%YGSP_RR%T, ZTAUX, PRCORI,PGM, ZDECRD_MF,&
    ...
    ! Initialize Environment horizontal velocity
      ZU(KIDIA:KFDIA,:)=YDAPLPAR%U(KIDIA:KFDIA,:)
      ZV(KIDIA:KFDIA,:)=YDAPLPAR%V(KIDIA:KFDIA,:)
    ELSE
! PUT IN ZTCORR THE CONDENSATE USED IN TRIGGERING
      DO JLEV=KTDIA,KLEV
       DO JLON=KIDIA,KFDIA
          ZTCORR(JLON,JLEV)=MAX(0._JPRB,ZQI(JLON,JLEV)+ZQL(JLON,JLEV) -&
          &  MAX(0._JPRB,YDAPLPAR%I(JLON,JLEV)+YDAPLPAR%L(JLON,JLEV)) )
       ENDDO
      ENDDO
! PASS T9 CONDENSATES TO ACCSU
      CALL ACCSU ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_EC%YRECUMF,YDMODEL%YRML_PHY_MF, &
        ...
        & YDAPLPAR%YCPG_PHY%XYB%ALPH, YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%RCP%R,&
        & PCVGQ, YDAPLPAR%YCPG_PHY%XYB%DELP, YDAPLPAR%YCPG_PHY%XYB%LNPR,&
        & ZQV, ZTCORR, YDAPLPAR%I, YDAPLPAR%L, ZQSAT, ZQW,&
        & YDAPLPAR%YCPG_PHY%XYB%RDELP, ZT, ZTW, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%VOR,&
        & YDCPG_DYN0%CTY%VVEL(:,1:), &
        & YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, &
        & YDAPLPAR%YGSP_RR%T, ZTAUX, PRCORI,ZDECRD_MF,&
    ...
    CALL ACUPU(YDPHY,YDPHY0,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
               ...
               & ZPOID,ZIPOI,ZLHV,ZLHS,YDAPLPAR%YCPG_DYN%RCP%CP,&
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    CALL APLMPHYS( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, ZQV, ZQI, ZQL, ZQS, ZQR, ZQG, YDAPLPAR%YCPG_DYN%RCP%R, ZT,&
     & ZIPOI, ZDQ, ZDQM, ZLHS, ZLHV, ZNEBS, ZPOID, ZRCVOTT,&
     & ZTCORR,YDMF_PHYS_SURF%GSD_VF%PLSM, PNEIJ, YDAPLPAR%YGSP_RR%T, ZDECRD_MF,&
     & ZFCQL, ZFCQI,&
   ...
   CALL ACUPM( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
              & YDAPLPAR%YCPG_DYN%RCP%CP, ZLHS, ZLHV,&
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      CALL ACNSDO(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       & YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,&
       & YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%YCPG_PHY%XYB%LNPR,ZQV,ZQI,ZQL,ZQR,ZQS,ZQW,&
       & YDAPLPAR%YCPG_PHY%XYB%RDELP, ZSIGP,&
    ...
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       & YDAPLPAR%YCPG_PHY%XYB%ALPH,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,&
       & YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%YCPG_PHY%XYB%LNPR,ZQV,ZQI,ZQL,ZQW,&
       & YDAPLPAR%YCPG_DYN%RCP%R,YDAPLPAR%YCPG_PHY%XYB%RDELP,ZSIGP,ZT,ZTW,ZU,ZV,YDCPG_DYN0%CTY%EVEL,ZOME,&
       & ZATSLC,ZGEOSLC,ZFHP,&
       & YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLSG,&
       & ZDIFCQD,ZDIFCQLD,ZDIFCQID,ZDIFCSD,&
...
! input  2d .
     & YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_DYN%RCP%CP,ZLHS,ZLHV,YDAPLPAR%YCPG_DYN%RCP%R,&
     & ZIPOI,ZPOID,&
     & ZFALLR,ZFALLS,ZFALLG,&
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     & YDAPLPAR%YCPG_PHY%XYB%ALPH, YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, PCVGQ, YDAPLPAR%YCPG_PHY%XYB%DELP,&
     & YDAPLPAR%YCPG_PHY%XYB%LNPR, ZQV, PQSAT, PQW, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%T, PTW, YDAPLPAR%U, YDAPLPAR%V, YDAPLPAR%VOR, YDCPG_DYN0%CTY%VVEL(:,1:),&
     & PGM, YDAPLPAR%YGSP_RR%T, ZTAUX, PRCORI,&
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    CALL ACCVIMP_V3 (YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     & YDAPLPAR%YCPG_PHY%XYB%ALPH, YDAPLPAR%YCPG_DYN%PHI, YDAPLPAR%YCPG_DYN%PHIF, YDAPLPAR%YCPG_PHY%PREHYD, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, PCVGQ, YDAPLPAR%YCPG_PHY%XYB%DELP,&
     & YDAPLPAR%YCPG_PHY%XYB%LNPR, ZQV, PQW, YDAPLPAR%YCPG_DYN%RCP%R, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%T, PTW, YDAPLPAR%U, YDAPLPAR%V,&
     & YDAPLPAR%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PPBLH,&
     & YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN,&
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    DO JLEV=1,KLEV
      DO JLON=KIDIA,KFDIA
        ZGEOM1(JLON,JLEV)     = YDAPLPAR%YCPG_DYN%PHIF(JLON,JLEV)-YDAPLPAR%YCPG_DYN%PHI(JLON,KLEV)
        ZVERVEL(JLON,JLEV)    = YDCPG_DYN0%CTY%VVEL(JLON,JLEV)*YDAPLPAR%YCPG_PHY%PREHYDF(JLON,JLEV)
    ...
    DO JLEV=0,KLEV
      DO JLON=KIDIA,KFDIA
        ZGEOMH(JLON,JLEV)=YDAPLPAR%YCPG_DYN%PHI(JLON,JLEV)-YDAPLPAR%YCPG_DYN%PHI(JLON,KLEV)
    ...
    CALL ACUPTQ ( KLON,KIDIA,KFDIA,KLEV,LLPTQ,YDMF_PHYS%OUT%FRSO,YDMF_PHYS%OUT%FRTH,&
     ...
     & YDMF_PHYS%OUT%FPEVPCL,YDMF_PHYS%OUT%FPEVPCN,YDMF_PHYS%OUT%FPFPCL,YDMF_PHYS%OUT%FPFPCN,YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%T,YDAPLPAR%Q,YDAPLPAR%YGSP_RR%T,&
     & ZTENHA,ZTENQVA )
    
    DO JLEV=1,KLEV
      DO JLON=KIDIA,KFDIA 
         ZTENT(JLON,JLEV) = ZTENHA(JLON,JLEV)/YDAPLPAR%YCPG_DYN%RCP%CP(JLON,JLEV)
    ...
    CALL CUCALLN_MF &
     ...
     & YDAPLPAR%T,ZQV,YDAPLPAR%U,YDAPLPAR%V,ZLISUM,ZVERVEL,YDMF_PHYS%OUT%DIFTQ,YDMF_PHYS%OUT%DIFTS,&
     & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%PREHYD,ZGEOM1,ZGEOMH,PGM, &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    CALL CULIGHT &
      ...
      &   YDAPLPAR%YCPG_PHY%PREHYDF,     YDAPLPAR%YCPG_PHY%PREHYD  ,  YDAPLPAR%YCPG_DYN%PHI,   YDAPLPAR%YCPG_DYN%PHIF,  LLLAND,&
      &   YDAPLPAR%T    ,  ZLU   ,  ZMFU,    YDMF_PHYS%OUT%CAPE, &
  ...
  IF ( LPROCLD ) THEN
    IF(LGPCMT) THEN
      ...
      ZTMIC(KIDIA:KFDIA,KTDIA:KLEV)=YDAPLPAR%T(KIDIA:KFDIA,KTDIA:KLEV)
    ...
    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     ...
     & YDAPLPAR%YCPG_PHY%XYB%DELP, YDAPLPAR%YCPG_PHY%PREHYDF, YDAPLPAR%YCPG_DYN%RCP%CP, YDAPLPAR%YCPG_DYN%RCP%R,&
     & ZNEBS, ZQLIS, ZNEB_CVPP, ZQLI_CVPP,ZQC_DET_PCMT,&
     & ZTENHA, ZTENQVA, LADJCLD, YDAPLPAR%YCPG_DYN%PHI,&
     & YDAPLPAR%YGSP_RR%T, PNEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, PGM, ZVETAF,&
  ...
  IF ( LGWDC ) THEN
    CALL ACDRAC ( YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,NTDRAG,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYD, ZNBVNO, YDAPLPAR%YCPG_PHY%XYB%RDELP, YDAPLPAR%U, YDAPLPAR%V,&
  ...
  IF ( LNORGWD ) THEN
    ...
    DO JLON = KIDIA, KFDIA
      DO JLEV = 1, KLEV
        Z_PP(JLON, JLEV) = YDAPLPAR%YCPG_PHY%PREHYDF(JLON, KLEV - JLEV + 1)
        Z_UU(JLON, JLEV) = YDAPLPAR%U(JLON, KLEV - JLEV + 1)
        Z_VV(JLON, JLEV) = YDAPLPAR%V(JLON, KLEV - JLEV + 1)
        Z_TT(JLON, JLEV) = YDAPLPAR%T(JLON, KLEV - JLEV + 1)
        Z_VO(JLON, JLEV) = YDAPLPAR%VOR(JLON, KLEV - JLEV + 1)
        ZD_U(JLON, JLEV) = TSPHY * YDAPLPAR%P1NOGW(JLON, KLEV - JLEV + 1)
        ZD_V(JLON, JLEV) = TSPHY * YDAPLPAR%P2NOGW(JLON, KLEV - JLEV + 1)
      ENDDO
    ENDDO
    ...
    DO JLON = KIDIA, KFDIA
      DO JLEV = 1, KLEV
        YDAPLPAR%P1NOGW(JLON, JLEV) = ZD_U(JLON, KLEV - JLEV + 1) / TSPHY
        YDAPLPAR%P2NOGW(JLON, JLEV) = ZD_V(JLON, KLEV - JLEV + 1) / TSPHY
      ENDDO
    ENDDO
    ...
    DO JLEV = 1, KLEV
       DO JLON = KIDIA, KFDIA
          ZFLX_LOTT_GWU(JLON, JLEV) = ZFLX_LOTT_GWU(JLON, JLEV - 1) - YDAPLPAR%P1NOGW(JLON, JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON, JLEV)/RG
          ZFLX_LOTT_GWV(JLON, JLEV) = ZFLX_LOTT_GWV(JLON, JLEV - 1) - YDAPLPAR%P2NOGW(JLON, JLEV)*YDAPLPAR%YCPG_PHY%XYB%DELP(JLON, JLEV)/RG
...
!         SAUVEGARDE DES FLUX DE PRECIPITATION CONVECTIVE ET STRATIFORME.
  ...
  IF ( LCONDWT.AND.LPROCLD.AND..NOT.LGPCMT ) THEN
    CALL QNGCOR (YDPHY2,KIDIA, KFDIA, KLON, NTPLUI, KLEV,&
     & ZQV, ZQL, ZQI, YDAPLPAR%YCPG_PHY%XYB%RDELP,&
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,KCSS,&
       ...
       & PVEG, ZWFC, ZWPMX, YDAPLPAR%YGSP_RR%FC, ZWLMX, ZWSEQ, ZWSMX,&
       & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, PFEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR%YGSP_SG%F,&
       & YDAPLPAR%YGSP_SB%T, YDAPLPAR%YGSP_RR%T, YDAPLPAR%YGSP_SB%Q, YDAPLPAR%YGSP_SB%TL, YDAPLPAR%YGSP_RR%W, YDAPLPAR%YGSP_RR%IC,&
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME ( YDPHY2,YDTOPH,KIDIA,KFDIA,KLON,NTDRME,KLEV,&
     & YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_PHY%XYB%DELP,YDAPLPAR%T,ZQV,YDAPLPAR%U,YDAPLPAR%V,&
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    CALL ACOZONE ( YDPHY2,YDTOPH,KIDIA,KFDIA,KLON,NTOZON,KLEV,KVCLIS,&
     & YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%XYB%DELP,PKOZO,ZQO3(1,1),YDAPLPAR%T,PMU0,&
     & YDMF_PHYS%OUT%FCHOZ)
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    CALL ACDIFOZ ( YDRIP,YDPHY2,YDMODEL%YRML_PHY_MF%YRVDOZ,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FPLCL,YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FRSO,&
     & ZKTROV,ZQO3(1,1),YDAPLPAR%YCPG_PHY%XYB%RDELP,YDAPLPAR%T,ZXTROV,&
...
!     ------------------------------------------------------------------
  ...
  IF (LPHSPSH) THEN
    DO JLON=KIDIA, KFDIA
       YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=(YDAPLPAR%T(JLON,KLEV)-YDAPLPAR%YGSP_RR%T(JLON)) * ((&
    ...
    ENDDO
    IF (LGRAPRO) THEN
      YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)+(YDAPLPAR%T(JLON,KLEV)-YDAPLPAR%YGSP_RR%T(JLON)) * (&
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_PHY%XYB%DELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%QICE(KIDIA:KFDIA,1:KLEV)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAF,'VIR',YDDDH)
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_PHY%XYB%DELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%QLI(KIDIA:KFDIA,1:KLEV)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAF,'VLR',YDDDH)
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=YDAPLPAR%YCPG_PHY%XYB%DELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%NEB(KIDIA:KFDIA,1:KLEV)
      ...
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDAPLPAR%YGSP_RR%T(KIDIA:KFDIA)/YDMF_PHYS%OUT%CT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*RTINER*YDAPLPAR%YGSP_SB%T(KIDIA:KFDIA,KCSS)/YDMF_PHYS%OUT%CT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDAPLPAR%YGSP_RR%W(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSWS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDAPLPAR%YGSP_SB%Q(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSWP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDAPLPAR%YGSP_SG%F(KIDIA:KFDIA,1)
      ...
      ZTMPAS(KIDIA:KFDIA)=YDAPLPAR%YCPG_DYN%PHI(KIDIA:KFDIA,KLEV)
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     CALL ACEVADCAPE(KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLCL,YDMF_PHYS%OUT%FPLCN,YDAPLPAR%YCPG_PHY%PREHYD,YDAPLPAR%YCPG_PHY%PREHYDF,YDAPLPAR%T,YDAPLPAR%YCPG_DYN%RCP%CP,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_DYN%PHI,ZDCAPE)
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDAPLPAR%U, YDAPLPAR%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, YDAPLPAR%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, KCLPH)
     YDMF_PHYS%OUT%CLPH(KIDIA:KFDIA)=MIN(XMAXLM,MAX(XMINLM,ZBLH(KIDIA:KFDIA)))
  ENDIF

  IF (LXVISI.OR.LXVISI2) THEN
     CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,YDAPLPAR%YCPG_DYN%PHI,YDAPLPAR%YCPG_DYN%PHIF,YDAPLPAR%YCPG_PHY%PREHYDF,&
      & YDAPLPAR%T,YDAPLPAR%YCPG_DYN%RCP%R,YDCPG_MISC%QLI,YDCPG_MISC%QICE,ZQR,ZQS,ZQGM,YDMF_PHYS%OUT%VISICLD, YDMF_PHYS%OUT%VISIHYD,YDMF_PHYS%OUT%MXCLWC)
  ENDIF
...
IF ( (.NOT. LGPCMT) ) THEN
   ...
   ZDE2MR(KIDIA:KFDIA,:)  = RD * YDAPLPAR%T(KIDIA:KFDIA,:) / YDAPLPAR%YCPG_PHY%PREHYDF(KIDIA:KFDIA,:)
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,ZPCLS,YDMF_PHYS%OUT%TCLS,&
  & YDAPLPAR%Q(:,KLEV),YDAPLPAR%L(:,KLEV),YDAPLPAR%I(:,KLEV),YDMF_PHYS%OUT%TPWCLS)
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,KLEV
    CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,YDAPLPAR%YCPG_PHY%PREHYDF(:,JLEV),YDAPLPAR%T(:,JLEV),&
     & YDAPLPAR%Q(:,JLEV),YDAPLPAR%L(:,JLEV),YDAPLPAR%I(:,JLEV),ZTW(:,JLEV))
  ENDDO
  ...
  DO JLEV = 1,KLEV
    DO JLON = KIDIA,KFDIA
      ZZZ(JLON,1,JLEV)=YDAPLPAR%YCPG_DYN%PHI(JLON,JLEV)*ZINVG
    ENDDO
  ENDDO
  ...
  ENDDO
  DO JLON = KIDIA,KFDIA
      ZDZZ(JLON,1,1)=YDAPLPAR%YCPG_DYN%PHI(JLON,0)*ZINVG-ZZZ(JLON,1,1)
  ENDDO
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS (YDPHY%YRDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDAPLPAR%YCPG_DYN%PHIF,&
      & ZDZZ,ZTW,YDAPLPAR%L,ZFPLC(:,KLEV),ZFPLS(:,KLEV),ZFPLSG(:,KLEV),PDPRECIPS(:,NDTPRECCUR))
 ...
 !Idem for an other time step and an other period
   ...
   CALL DPRECIPS(YDPHY%YRDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDAPLPAR%YCPG_DYN%PHIF,&
      & ZDZZ,ZTW,YDAPLPAR%L,ZFPLC(:,KLEV),ZFPLS(:,KLEV),ZFPLSG(:,KLEV),PDPRECIPS2(:,NDTPRECCUR2))
