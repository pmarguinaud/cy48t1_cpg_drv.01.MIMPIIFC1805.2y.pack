...
@PROCESS NOCHECK
#endif
SUBROUTINE APLPAR(YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, &
...
REAL(KIND=JPRB) :: ZVETAH(0:YDCPG_OPTS%KFLEVG),ZNLAB(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG),ZNLABCVP(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG)
...
REAL(KIND=JPRB) :: ZMELNET(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG) ! net melting (-freezing) rate of ice
REAL(KIND=JPRB) :: ZMELGET(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG)! net melting (-freezing) rate of graupel
...
REAL(KIND=JPRB) :: ZDAER(YDCPG_OPTS%KFLEVG), ZHUC(YDCPG_OPTS%KFLEVG), ZBLH(YDCPG_OPTS%KLON)
...
REAL(KIND=JPRB) :: ZQG(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZDQG
...
REAL (KIND=JPRB)     :: ZPRC_DPRECIPS2 (YDCPG_OPTS%KLON, 1:YDCPG_OPTS%KDTPREC2)
REAL (KIND=JPRB)     :: ZPRC_DPRECIPS (YDCPG_OPTS%KLON, 1:YDCPG_OPTS%KDTPREC)
...
& LFLASH =>YDCFU%LFLASH, CGMIXLEN=>YDMODEL%YRML_PHY_MF%YRPHY%CGMIXLEN)
...
ZRDG_LCVQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)=ZRDG_CVGQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)

DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ZFLU_QSATS(JROF)=0.0_JPRB
ENDDO

CALL MF_PHYS_FPL_PART1 (YDCPG_DIM, YDCPG_OPTS, ZPFL_FPLCH, ZPFL_FPLSH, YDVARS%CPF%T0, YDVARS%SPF%T0, YDMODEL)
...
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART1 (YDCPG_DIM, YDCPG_OPTS, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH, ZSAV_FHPS, ZSAV_GZ0F,                    &
...
CALL APLPAR_INIT (YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, NTSSG, YSP_SBD%NLEVS, &
...
IF (LTWOTL) THEN
  IF (LAJUCV) THEN
    DO JLEV=1,YDCPG_OPTS%KFLEVG
      ...
      ENDDO
    ENDDO
    CALL ACAJUCV(YDMODEL%YRML_PHY_MF%YRPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,       &
...
!-------------------------------------------------
!
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
              & YDCPG_OPTS%KSTEP, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,               &
              & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDVARS%GEOMETRY%GELAM%T0,          &
              & YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%Q,  &
...
IF (LMPHYS) THEN
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(YDCPG_OPTS%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
  ...
  ELSE
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, ZQO3, .FALSE., YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, LO3ABC, YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
!     EPAISSEUR STD AEROSOLS
  ZAEO = AERCS1*ZVETAH(YDCPG_OPTS%KTDIA-1) + AERCS3*ZVETAH(YDCPG_OPTS%KTDIA-1)**3&
   & + AERCS5*ZVETAH(YDCPG_OPTS%KTDIA-1)**5
  DO JLEV = YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
...
IF(LCOEFKTKE.AND.YDCPG_OPTS%KSTEP == 0) THEN
  YDMF_PHYS_SURF%GSD_VH%PQSH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KFLEVG)
ENDIF
...
!     ----------------------------
  IF ( LTHERMO ) THEN
    CALL ACTQSAT ( YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTQSAT, YDCPG_OPTS%KFLEVG,           &
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP, YDPHY0, YDPHYDS, LLAROME, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                            &
     ...
     & YDMF_PHYS_BASE_STATE%V(:, YDCPG_OPTS%KFLEVG), ZTHETAS, YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                     &
     & ZFEV, ZFMDU, ZFMDV)
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      LLHMT=.FALSE.
      CALL ACSOL ( YDPHY, YDPHY1, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_SURF%GSD_VV%PARG,                     &
...
!
  ...
  CALL ARO_GROUND_DIAG_Z0( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1, YDCPG_DIM%KIDIA,        &
    ...
    IF (LMSE) THEN
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (YDCPG_OPTS%KSTEP == 0) THEN 
         CALL ARO_GROUND_DIAG_Z0( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1,                         &
        ...
        CALL ACTKEZOTLS ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,                 &
      ...
      ELSE
        CALL ACHMTLS ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,        &
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        CALL ACTKEHMT ( YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDSURF%YSD_VVD%NUMFLDS>=8.AND.LSOLV, &
        & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,           &
        ...
        &                                                                                                                     )
      ELSE
        CALL ACHMT ( YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDSURF%YSD_VVD%NUMFLDS>=8.AND.LSOLV,    &
        & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,           &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
        & YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, ZPFL_FPLSH,                                    &
        & ZPFL_FPLCH, ZDPHIT, ZDPHIV, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,      &
    ...
    IF (LPTKE) THEN
      YDMF_PHYS_BASE_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG) = MAX(YDMF_PHYS_BASE_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG),ETKE_MIN)
    ENDIF
    IF (LCOEFK_PTTE) THEN
      YDVARS%TTE%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG) = MAX(YDVARS%TTE%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG),ETKE_MIN)
    ENDIF

    IF(LCOEFKTKE) THEN
      ZCP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG) = RCPD*(1.0_JPRB+(RCPV/RCPD-1.0_JPRB)*(&
        & ZQV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)+ZQI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)+&
        & ZQL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)))
    ELSE
      ZCP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG) = YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_OPTS%KFLEVG)
    ENDIF

    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      !  computation of Ri*,Ri** for mixing lenth computation
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCOEF, YDCPG_OPTS%KFLEVG, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,  &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI,                   &
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      ENDDO
      CALL ACCLPH ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,               &
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      CALL ACMIXLENZ ( YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, 1, YDCPG_OPTS%KFLEVG,     &
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZLMU, ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZZLMT, ZLMT                                   &
      &                                                                                                                                                     )
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        LLMAF=.TRUE.
        CALL ACMIXELEN(YGFL, YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                            &
    ...
    IF(LCOEFKTKE) THEN
      ...
      ! ------------------------------------------------------------- 
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCOEF,                                     &
        ...
        & ZMSC_LSCPE, ZQV, ZQL, ZQI, ZFLU_QSAT, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%T,                             &
        & YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCOEF, YDCPG_OPTS%KFLEVG,                    &
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      ...
      ENDDO
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,          &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,   &
  ...
  IF (LRAFTKE) THEN
    ...
    ZDCAPE(:)=0._JPRB
    CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
    & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%CAPE,  &
    & ZDCAPE, YDMF_PHYS_BASE_STATE%TKE, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDVARS%GEOMETRY%OROG%T0, YDMF_PHYS%OUT%UGST,       &
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      CALL ACTKECOEFK ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCOEF, YDCPG_OPTS%KFLEVG,  &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCOEF, YDCPG_OPTS%KFLEVG,        &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,         &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
      & ZMSC_LSCPE, ZQV, ZQL, ZQI, ZFLU_QSAT, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%T,                   &
      & YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, ZPFL_FPLSH, ZPFL_FPLCH, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0,               &
      & YDMF_PHYS%OUT%GZ0H, ZBLH, ZKTROV, ZKUROV, ZKNROV, ZNBVNO, ZXTROV, ZXUROV, ZXPTKEROV)
    ENDIF
  ENDIF

  IF (LCVPPKF) THEN
    CALL ACVPPKF( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,        &
  ...
  IF (LEDKF) THEN
    ...
    CALL ARP_SHALLOW_MF( YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG,                   &
    & ZIMPL, TSPHY, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
...
! input dust scalar concentration in ppp from
  ...
  CALL ARO_MNHDUST (1, ILONMNH, YDCPG_OPTS%KFLEVG, NGFL_EXT, PDTPHY, ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, 1:NGFL_EXT),                &
  ...
  & NSWB_MNH, YDCPG_OPTS%KSTEP+1, ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, 1:NGFL_EXT), ZPIZA_DST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :), &
...
!DEC$ IVDEP
  ...
  & .OR.  ( (LRAY.OR.LRAYSP).AND.(.NOT.LRSTAER)) ) THEN
    ...
    IF ( ( (LRAYFM.AND.NAER /= 0) .OR.LRAY.OR.LRAYSP).AND.LLAERO )  THEN
      CALL RADAER ( YDMODEL%YRML_PHY_RAD%YREAERD, YDERAD, YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, &
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_OPTS%KSTEP,YDCFU%NFRRC) == 0) THEN
      CALL ACRANEB(YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                          &
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    ELSE
      ...
      ENDDO
    ENDIF
    CALL ACCVIMPGY ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,                   &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                 &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP,              &
    & ZMSC_LH, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQCI, ZQCL, ZQLIS, ZFLU_QSAT, ZMSC_QW, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%T, ZMSC_TW, YDMF_PHYS_BASE_STATE%U,                                 &
    & YDMF_PHYS_BASE_STATE%V, ZDSA_CPS, YDVARS%GEOMETRY%GM%T0, YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%OUT%DIFCQ,                               &
    ...
    & YDMF_PHYS%OUT%STRCV, ICIS, INLAB, INND, YDMF_PHYS_BASE_STATE%CVV)
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND ( YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                            &
    & NTPLUI, YDCPG_OPTS%KFLEVG, LLREDPR, ZHUC, ZVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,       &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,                     &
      ...
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDVARS%GEOMETRY%GM%T0, YDMF_PHYS_BASE_STATE%YGSP_RR%T,    &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    CALL ACNEBN ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTNEBU, YDCPG_OPTS%KFLEVG,                    &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                 &
...
!         Diagnostique de nebulosite partielle.
  CALL ACNPART(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTNEBU, YDCPG_OPTS%KFLEVG,   &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_OPTS%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    CALL ACRADCOEF ( YDRCOEF, YDPHY3, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, &
    & YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,     &
...
!DEC$ IVDEP
        ...
        ENDDO
      CASE(2)
        CALL ACRANEB2(YDERDI, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                   &
        & NTRADI, YDCPG_OPTS%KFLEVG, IJN, YDCPG_OPTS%KSTEP, YDCFU%NFRRC, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                  &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,   &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,                &
        & ZQO3, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, ZFLU_EMIS, YDVARS%GEOMETRY%GELAM%T0, YDVARS%GEOMETRY%GEMU%T0,                 &
        & ZRDG_MU0, ZRDG_MU0LU, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD, ZCLCT_RAD, YDMF_PHYS%OPT%GDEOSI,                      &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,      &
      & YDCPG_OPTS%KSW, ZALBD, ZALBP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,      &
      & YDCPG_MISC%NEB, ZQO3, ZAER, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZFLU_EMIS, ZRDG_MU0M,                      &
      & ZQV, ZFLU_QSAT, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQS, ZQR, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_BASE_STATE%T, &
      & YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS%RAD%EMTD, YDMF_PHYS%RAD%EMTU,                   &
    ...
    CALL RADHEAT ( YDERAD, YDERDI, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, &
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
    ...
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
                       & YDCPG_OPTS%KTDIA, NTSSG, YDVARS%GEOMETRY%GEMU%T0, ZRDG_MU0, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, &
                       & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO)
...
!DEC$ IVDEP
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, ZRDG_MU0, YDMF_PHYS%OUT%FRSO(:, YDCPG_OPTS%KFLEVG, 1), &
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    CALL ACVEG ( YDPHY, YDPHY1, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS%OUT%FRSO, &
    & ZQV, ZFLU_QSAT, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VF%PLSM,                    &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC, YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                   &
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        IF ( LMSE.AND.LCALLSFX ) THEN       
          ...
          ENDIF
        ENDIF
        CALL ACPTKE(YGFL, YDLDDH, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                                &
      ...
      CALL ACDIFV1 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                   &
      & YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,                                 &
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1, YDCPG_DIM%KIDIA,                                                     &
        & YDCPG_DIM%KFDIA, YDCPG_OPTS%KSTEP, IRR, NSW, NGFL_EXT, NDGUNG, NDGUXG, NDLUNG, NDLUXG,                                                                          &
        ...
        & YDVARS%GEOMETRY%RINDY%T0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_BASE_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                   &
        & YDMF_PHYS_BASE_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                                                    &
        & YDMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                                                    &
        & YDMF_PHYS_BASE_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                                                    &
        & ZSVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG, 1:NGFL_EXT),                                                                          &
        & ZCARDI, ZRHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),                                                      &
        & ZDTMSE, ZDEPTH_HEIGHT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG), ZZS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                                &
        ...
        & ZCFAQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), ZCFATH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),         &
        & ZCFAU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), ZCFBQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),          &
        & ZCFBTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), ZCFBU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG),         &
        & ZCFBV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG:YDCPG_OPTS%KFLEVG), YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                              &
        ...
        & ZFLU_EMIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), ZTSN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS%OUT%FRTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, 1)    &
        &                         )               !orographic shadowing

! Opposite water vapor flux
        ZFEVS(:)=-ZFEV(:)

        CALL ARO_GROUND_DIAG( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1, YDCPG_DIM%KIDIA,                                           &
        & YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, -1, NDGUNG, NDGUXG, NDLUNG, NDLUXG, LSURFEX_KFROM, ZZS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                         &
        & ZFEVS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_BASE_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_OPTS%KFLEVG),                                &
        & YDMF_PHYS_BASE_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_OPTS%KFLEVG), ZDEPTH_HEIGHT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_OPTS%KFLEVG),    &
        & YDMF_PHYS%OUT%FRTH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, 1), YDMF_PHYS%OUT%FRSO(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, 1), &
...
! calculation of variables for the old "ISBA" atmosphere scheme
        IF (.NOT. LELAM) THEN
          CALL ARO_GROUND_DIAG_2ISBA( YDCPG_DIM%KBL, YDCPG_OPTS%KGPCOMP, YDCPG_DIM%KFDIA-YDCPG_DIM%KIDIA+1,                                         &
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          CALL ACAA1 ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,    &
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          IF (LEDMFI) THEN
            ...
            ZCFBV_G(:,:) = ZCFBV(:,:)
          ENDIF   
          CALL ARP_GROUND_PARAM ( YDMCC, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,        &
          & YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, YSP_SBD%NLEVS, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, ZCP,                  &
          & YDMF_PHYS%OUT%FRSO, ZQV, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%T,                  &
      ...
      CALL ACDIFV2 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, &
      & YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, ZCFAQ, ZCFAS, ZCFAU, ZCFASV, ZCFBQ,                &
      & ZCFBS, ZCFBU, ZCFBV, ZCFBSV, ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZDSE, ZQT, YDMF_PHYS_BASE_STATE%U,           &
      & YDMF_PHYS_BASE_STATE%V, ZPOID, YDMF_PHYS_BASE_STATE%T, ZQL, ZQI, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,  &
      ...
      IF (L3DTURB) THEN
        ...
        & YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%TKE, ZTENDPTKE, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
        & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V,                &
        & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%DIV, YDMF_PHYS_BASE_STATE%VOR,                                &
        & YDVARS%U%DL, YDVARS%V%DL, YDMF_PHYS_BASE_STATE%YCPG_PHY%W, YDMF_PHYS_BASE_STATE%YCPG_PHY%WL,                               &
      ...
      IF (LCOEFKTKE) THEN
        ...
        ELSEIF (NDIFFNEB == 4) THEN
          ZCOEFA(:,:) = ZBNEBCVPP(:,:)
        ENDIF
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,           &
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTDIFU, YDCPG_OPTS%KFLEVG,                  &
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      ENDDO
      CALL ACPBLH ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTDIFU, YDCPG_OPTS%KFLEVG,                     &
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY, YDPHY1, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, &
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    CALL ACDRAG ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTDRAG, YDCPG_OPTS%KFLEVG,         &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    CALL ACPLUIE ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG,       &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZMSC_QW, YDMF_PHYS_BASE_STATE%T, &
  ...
  IF ( LSTRAS ) THEN
    CALL ACPLUIS ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG, &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZNEBS, ZQV,                    &
    & ZQLIS, ZMSC_QW, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%T, YDMF_PHYS%OUT%FCSQL,             &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      CALL ACTQSATS ( YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTQSAT, YDCPG_OPTS%KFLEVG, &
      ...
      & YDVARS%GEOMETRY%GM%T0, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1,                                &
      & ZQLIS0, ZNEBS0)
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG,            &
      & ZVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL,                            &
      & ZQS, ZQR, ZQG, ZQW, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZT, ZMSC_TW, ZNEBS, ZRHCRI, ZICEFR1,                           &
      & ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP, ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM,              &
      & ZFLU_NEIJ, YDVARS%GEOMETRY%GM%T0, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN,          &
    ...
    ELSE
      ...
      & ZVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL,                            &
      & ZQS, ZQR, ZQG, ZMSC_QW, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%T, ZMSC_TW,                           &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,              &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, ZRDG_CVGQ, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQSAT, ZQW, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,                   &
      & ZT, ZTW, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS_BASE_STATE%VOR, YDVARS%DAL%T0,                             &
      & YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZTAUX, YDVARS%GEOMETRY%RCORI%T0, YDVARS%GEOMETRY%GM%T0, ZDECRD_MF, YDMF_PHYS%OUT%DIFCQ,                           &
      & YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN,                    &
    ...
    ! Initialize Environment horizontal velocity
      ZU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YDMF_PHYS_BASE_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
      ZV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YDMF_PHYS_BASE_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
    ELSE
...
! PASS T9 CONDENSATES TO ACCSU
      ...
      & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, &
      & YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                 &
      & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,            &
    ...
    CALL ACUPU(YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, &
    & YDVARS%UAL%T0, YDVARS%UNEBH%T0, ZDETFI, ZPOID, ZIPOI, ZLHV, ZLHS, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,      &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    CALL APLMPHYS( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,              &
    & YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,                    &
   ...
   CALL ACUPM( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                 &
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      CALL ACNSDO(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                    &
    ...
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                  &
      & YDCPG_OPTS%KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,                      &
  ...
  !  ---------------------------------------------
    ...
    CALL ACUPD(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                   &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,      &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,     &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, &
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    CALL ACCVIMP_V3 (YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTCVIM, YDCPG_OPTS%KFLEVG,       &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,        &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,    &
    & ZRDG_CVGQ, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%LNPR, ZQV,                       &
    & ZMSC_QW, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%T,        &
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    ENDDO
    LLPTQ = .FALSE.
    CALL ACUPTQ ( YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, LLPTQ, YDMF_PHYS%OUT%FRSO,  &
    ...
    & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, 0, YDCPG_OPTS%KFLEVG, ZDXTDK, ISPPN2D, LLLAND,                    &
    & LLDSLPHY, TSPHY, ZVDIFTS, YDMF_PHYS_BASE_STATE%T, ZQV, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V,             &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    ZGAW(:)=0._JPRB
    CALL CULIGHT   ( YDEPHY, YGFL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,                            &
  ...
  IF ( LADJCLD ) THEN
    ...
    ZFRTH(:,:) = 0.0_JPRB
    LLPTQ = .TRUE.
    CALL ACUPTQ ( YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, LLPTQ, ZFRSO,                   &
    ...
    & YDMF_PHYS_BASE_STATE%Q, YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZTENHA, ZTENQVA )
  ENDIF

  IF ( LPROCLD ) THEN
    IF(LGPCMT) THEN
      ! Microphysics occurs in a shell between updraft and its resolved environment.
      ZNEBS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=1._JPRB-(1._JPRB-ZNEBS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))&
        & *(1._JPRB-ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))
      ZTMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=YDMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)
      ZQMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)&
        & *ZFLU_QSAT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)&
        & +(1._JPRB-ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))*ZQV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)
      ZQLIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=MAX(ZQLIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG),&
        & ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)&
        & *(ZQL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)+ZQI(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))&
        & +(1._JPRB-ZCSGC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))*ZQLIS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG))
      ZQC_DET_PCMT(:,:)=0._JPRB
    ELSE
      ! Microphysics occurs in the resolved state.
      ZTMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=YDMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)
      ZQMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)=ZQV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KTDIA:YDCPG_OPTS%KFLEVG)
    ENDIF

    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG,    &
    & ZTMIC, ZQMIC, ZQL, ZQI, ZQR, ZQS, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, &
    ...
    & YDMF_PHYS_BASE_STATE%YGSP_RR%T, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDVARS%GEOMETRY%GM%T0, ZVETAF, YDMF_PHYS%OUT%FCSQL, &
    & YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN,     &
  ...
  IF ( LNEBCO ) THEN
    CALL ACNEBC ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTNEBU, YDCPG_OPTS%KFLEVG,                  &
  ...
  ENDIF
  IF ( LGWDC ) THEN
    CALL ACDRAC ( YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTDRAG, YDCPG_OPTS%KFLEVG,        &
  ...
  IF ( LNORGWD ) THEN
    ...
    ZPRECGWD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA) = MAX(0.0_JPRB,                  &
      & YDMF_PHYS%OUT%FPLCL(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KFLEVG) + YDMF_PHYS%OUT%FPLCN(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_OPTS%KFLEVG))
    
    CALL ACNORGWD(YDNORGWD, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
...
!         SAUVEGARDE DES FLUX DE PRECIPITATION CONVECTIVE ET STRATIFORME.
  ...
  IF ( LCONDWT.AND.LPROCLD.AND..NOT.LGPCMT ) THEN
    CALL QNGCOR (YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTPLUI, YDCPG_OPTS%KFLEVG,                   &
...
!     ---------------------------
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,                  &
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME (YDMODEL%YRCST, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTDRME, YDCPG_OPTS%KFLEVG,  &
    & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%T,    &
    & ZQV, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%FRMH, ZMSC_FRMQ, YDMF_PHYS%OUT%STRMU, &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    CALL ACOZONE ( YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, NTOZON, YDCPG_OPTS%KFLEVG,       &
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    CALL ACDIFOZ ( YDRIP, YDPHY2, YDMODEL%YRML_PHY_MF%YRVDOZ, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,                  &
    ...
    & YDMF_PHYS_BASE_STATE%T, ZXTROV, ZFLU_NEIJ, YDVARS%GEOMETRY%GEMU%T0, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS%OUT%FCHOZ                 &
    &                                                                                    )
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     -----------------------------------------------------------------------
  ...
  IF (YDMODEL%YRML_DIAG%YRLDDH%LFLEXDIA) THEN
    CALL APLPAR_FLEXDIA (YDCPG_DIM, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, YDDDH, &
    & YDMF_PHYS_BASE_STATE)
  ENDIF

  IF (LECT.AND.LRAFTKE) THEN
     ! DCAPE due to precipitation evaporation.
     CALL ACEVADCAPE(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMF_PHYS%OUT%FPLSL, &
     ...
     CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, &
     & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS%OUT%CAPE,  &
     & ZDCAPE, ZTKE1, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDVARS%GEOMETRY%OROG%T0, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,      &
     & ZBLH, IMOC_CLPH)
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON,           &
...
ZZDEP=0.0_JPRB
  ...
  ENDDO
    ...
    CALL ARO_WETDEP(ILONMNH, YDCPG_OPTS%KFLEVG, NGFL_EXT, IKRR, PDTPHY, ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, 1:NGFL_EXT), &
    & ZZDEP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :), ZZI_PABSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :),                              &
    & ZZI_THM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :), ZZI_RHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :),                         &
    & YDCPG_OPTS%KSTEP+1, ZZI_RM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, :), ZEVAP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :),           &
    & ISPLITR               )
! return to tendency
   DO JGFL=1,NGFL_EXT
     DO JLON=1,YDCPG_OPTS%KLON
...
IF ( (.NOT. LGPCMT) ) THEN
   ...
   ZDE2MR(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)  = RD * YDMF_PHYS_BASE_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:) / YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
...
CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, ZPCLS, YDMF_PHYS%OUT%TCLS, YDMF_PHYS_BASE_STATE%Q(:, YDCPG_OPTS%KFLEVG), &
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_OPTS%KFLEVG
    CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF(:, JLEV), &
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS (YDPHY%YRDPRECIPS, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,                      &
...
IF (LINTFLEX) THEN
  CALL APLPAR2INTFLEX(YGFL, YDPHY, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG,                            &
...
!             --------------------------
CALL MF_PHYS_MOCON (YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS%OUT%MOCON, ZRDG_LCVQ, IMOC_CLPH, YDMF_PHYS_BASE_STATE)

! Store surface water flux P and E for water conservation
IF (LCORWAT) THEN
  CALL MF_PHYS_CORWAT (YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, &
...
IF (LOZONE) THEN
  ...
  !   variable.
  CALL CPOZO (YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, PDTPHY, YDMF_PHYS%OUT%FCHOZ, &
...
IF (NDPSFI == 1) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV, YDPHY, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_PHY0%PREHYD, &
...
IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
  CALL CPTEND_FLEX( YDLDDH, YDMDDH, YGFL, YDPHY, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG,   &
  & YDVARS%GEOMETRY%GNORDL%T0, YDVARS%GEOMETRY%GNORDM%T0, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, &
  & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, YDMF_PHYS_BASE_STATE%T,      &
  & YDMF_PHYS_BASE_STATE%YGSP_RR%T, PGFL, YLPROCSET, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV,                         &
...
ELSE
  CALL CPTEND_NEW( YDMODEL, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, YDVARS%GEOMETRY%GNORDL%T0,            &
...
IF (.NOT.LDCONFX) THEN
  IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
    CALL CPCHET (YDMF_PHYS, YDMF_PHYS_BASE_STATE, YDCPG_MISC, YDRIP, YDPHY, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, &
    & YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%KSTEP, ZMSC_FRMQ, ZDSA_CPS, ZTENDH, ZPTENDQ1,             &
...
IF (NDPSFI == 1) THEN
  CALL CPMVVPS(YDVAB, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, PDTPHY,               &
...
CALL CPUTQY_APLPAR_EXPL(YDCPG_DIM, YDCPG_OPTS, YDMF_PHYS_NEXT_STATE, YDMF_PHYS_BASE_STATE, YDVARS, YDPHY, PDTPHY, &
& ZTENDH, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDD, ZPTENDEFB11,      &
...
& YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG, PDTPHY, ZTENDGFL, YDCPG_SL1%ZVIEW, PGMVT1,                  &
& PGFLT1)

CALL MF_PHYS_FPL_PART2 (YDCPG_DIM, YDCPG_OPTS, ZPFL_FPLCH, ZPFL_FPLSH, YDVARS%CPF%T1, YDVARS%SPF%T1, YDMODEL)

!       2.9b Prognostic convection etc.
!            --------------------------

! TRANSFER NOT ADVECTED VARIABLES INTO PGFLT1
CALL MF_PHYS_TRANSFER (YDCPG_DIM, YDCPG_OPTS, YDVARS, YDMODEL)
...
IF ((.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,YDCPG_OPTS%KFLEVG
      ...
      ENDDO
    ENDDO
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_OPTS%KFLEVG,                        &
    ...
    CALL CPWTS(YDSURF, YDMODEL%YRML_AOC%YRMCC, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA,           &
  ...
  ENDIF
  IF(LNUDG)THEN
    CALL CPNUDG ( YDCPG_OPTS%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFNUDG, YDCPG_OPTS%KFLEVG, YDCPG_DIM%KBL,    &
...
CALL MF_PHYS_CVV (YDCPG_DIM, YDCPG_OPTS, YDVARS%CVV%T0, YDVARS%CVV%T1, YDMODEL)
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART2 (YDCPG_DIM, YDCPG_OPTS, ZSAV_DDAL, ZSAV_DDOM, ZSAV_ENTCH, ZSAV_FHPS, ZSAV_GZ0F,                    &
  & ZSAV_GZ0HF, ZSAV_HV, ZSAV_PBLH, ZSAV_QSH, ZSAV_UDAL, ZSAV_UDGRO, ZSAV_UDOM, ZSAV_UNEBH, YDMF_PHYS_SURF%GSD_VF%PZ0F, &
  & YDMF_PHYS_SURF%GSD_VH%PPBLH, YDMF_PHYS_SURF%GSD_VH%PQSH, YDMF_PHYS_SURF%GSD_VH%PSPSH, YDMF_PHYS_SURF%GSD_VK%PUDGRO, &
  & YDMF_PHYS_SURF%GSD_VV%PHV, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDVARS%DAL%T0, YDVARS%DOM%T0, YDVARS%UAL%T0,                 &
  & YDVARS%UEN%T0, YDVARS%UNEBH%T0, YDVARS%UOM%T0, YDSURF, YDMODEL)
...
CALL MF_PHYS_BAYRAD (YDCPG_DIM, YDCPG_OPTS, ZBAY_QRCONV, ZBAY_QSCONV, YDVARS%RCONV%T1, YDVARS%SCONV%T1, YDMODEL, &
& LAROME)
...
IF(LGSCM.OR.LMUSCLFA) THEN
  IF (LAROME) THEN
    ...
    ENDDO
  ENDIF
  CALL WRITEPHYSIO(YDGEOMETRY, YDCPG_MISC, YDCPG_PHY0,  YDMF_PHYS, YDCPG_DYN0, &
  & YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KFDIA, YDCPG_DIM%KIDIA,    &
  & YDCPG_OPTS%KGL1, YDCPG_OPTS%KGL2, YDCPG_DIM%KSTGLO, YDCPG_OPTS%KSTEP, NTSSG, YSP_SBD%NLEVS, YDVARS%GEOMETRY%GELAM%T0, &
  & YDVARS%GEOMETRY%GEMU%T0, YDVARS%GEOMETRY%GM%T0, YDVARS%GEOMETRY%OROG%T0, YDVARS%GEOMETRY%RCORI%T0, YDVARS%GEOMETRY%RATATH%T0, YDVARS%GEOMETRY%RATATX%T0,         &
...
CALL MF_PHYS_PRECIPS (YDCPG_DIM, YDCPG_OPTS, ZPRC_DPRECIPS, ZPRC_DPRECIPS2, YDMF_PHYS_SURF%GSD_XP%PPRECIP, YDMF_PHYS_SURF%GSD_XP2%PPRECIP2, YDMODEL)
