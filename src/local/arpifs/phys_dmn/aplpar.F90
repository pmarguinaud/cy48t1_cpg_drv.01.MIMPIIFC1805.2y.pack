...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     DO JLON=KIDIA,KFDIA
        ...
        YDMF_PHYS%OUT%FCLL(JLON,1)=YDMF_PHYS%OUT%FEVL(JLON,1)*PLH(JLON,KLEV)
        YDMF_PHYS%OUT%FCLN(JLON,1)=YDMF_PHYS%OUT%FEVN(JLON,1)*PLH(JLON,KLEV)
        PLHS(JLON)=FOLH(YDMF_PHYS_STATE%YGSP_RR%T(JLON),0.0_JPRB)
     ENDDO
...
!       Computation of ZRTI
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, PQSAT,&
       & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, PLSCPE,YDMF_PHYS%OUT%GZ0,&
       & ZMN2PP,ZMRIPP)
    ENDIF
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         ...
         & YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,ZMN2PP,ZFMGST,PFPLSH,PFPLCH,&
         & YDMF_PHYS%OUT%GZ0,YDMF_PHYS_STATE%YGSP_RR%T,PCDN,ZBLH,ZLMU,ZLMT,YDVARS%MXL%T0,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
         & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, PLSCPE,&
         & ZQV, ZQL, ZQI, PQSAT,&
      ...
      CALL ACMRIP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,KSTEP,&
       ...
       & ZQV, ZQL, ZQI,ZCP,&
       & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,PQSAT,PQW,PTW,&
       ...
       & YDMF_PHYS_STATE%YGSP_RR%T,YDMF_PHYS_SURF%GSD_VH%PQSH,PRS,PCPS,ZRTI,YDMF_PHYS%OUT%GZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, PLSCPE, ZQV, ZQL, ZQI, PQSAT,&
       & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, PFPLSH, PFPLCH, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH,&
      ...
      ELSE
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-PVEG(JLON)-PNEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & PVEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+PNEIJ(JLON)*YDMF_PHYS_STATE%YGSP_SG%A(JLON,1)
          ELSE
            ...
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-PNEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & YDMF_PHYS_STATE%YGSP_SG%A(JLON,1))+(PNEIJ(JLON)-ZNEIJV(JLON))*     &
      ...
      IF (LALBMERCLIM) THEN
        DO JSG=1,NSW
          DO JLON=KIDIA,KFDIA
            ...
            ZALBPMER=(1.0_JPRB+&
             & 0.5_JPRB*PMU0M(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))/   (&
             & 1.0_JPRB+PMU0M(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))**2
    ...
    ELSEIF (LRAY) THEN
      ...
      DO JLON=KIDIA,KFDIA
        ...
        ZALBDIR(JLON)=ZLAMB*YDMF_PHYS%OUT%ALB(JLON)+(1._JPRB-ZLAMB)*(1._JPRB+&
         & 0.5_JPRB*PMU0(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))/   (&
         & 1.0_JPRB+PMU0(JLON)*(1.0_JPRB/YDMF_PHYS%OUT%ALB(JLON)-1.0_JPRB))**2
      ENDDO
    ENDIF
  ...
  IF (LRAY.AND.NRAY == 1.AND.KNFRRC /= 0) THEN
    IF (MOD(KSTEP,KNFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       ...
       & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PMU0,PGEMU,PGELAM,PMU0LU,YDMF_PHYS_STATE%YGSP_RR%T,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      ! - INPUT  2D .
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDCPG_DYN0%CTY%VVEL(:,1:),YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,PLH,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,&
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,&
      & YDMF_PHYS_STATE%Q,ZQI,ZQL,YDMF_PHYS_STATE%R,YDMF_PHYS_STATE%S,ZQLIS,PQSAT,PQW,&
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,PCVGQ,YDMF_PHYS_STATE%T,PTW,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%TKE,ZTRA(:,:,INBTRA_DEP:INBTRA),ZSMOOTRAC(1:INBTRA),&
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
      & PGM, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, PNEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTNEBU,KLEV,&
     & YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,PLSCPE,ZQV,&
     & PQSAT,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%T,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS%OUT%GZ0,PFPLCH,YDMF_PHYS_SURF%GSD_VH%PPBLH,YDCPG_MISC%QS,YDMF_PHYS_STATE%YGSP_RR%T,&
...
!DEC$ IVDEP
        DO JLON=KIDIA,KFDIA
          IF ( YDMF_PHYS%OUT%FRSOPS(JLON) > RSUNDUR*PMU0(JLON) ) THEN
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         ...
         & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,YDMF_PHYS_STATE%T,&
         & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PGELAM,PGEMU,PMU0,PMU0LU,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD,ZCLCT_RAD,&
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(KIDIA,KFDIA,KLON,                             &
       & PMU0,YDMF_PHYS%OUT%FRSO(:,KLEV,1),YDMF_PHYS%OUT%FRSODS,YDMF_PHYS%OUT%FRSOPS,YDMF_PHYS%OUT%FRTH(:,KLEV,1),YDMF_PHYS%OUT%FRTHDS, &
       & YDMF_PHYS%OUT%MRT) 
    ENDIF
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDIFU,KLEV,KCSS,&
       ...
       & PCD, PCDN, ZCDROV, ZCHROV, ZCEROV, PCPS, YDMF_PHYS%OUT%CT,ZDQSTS,&
       & PEMIS, YDMF_PHYS_SURF%GSD_VH%PSPSH, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV,&
       & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, PNEIJ, YDCPG_MISC%QS, PQSATS, YDMF_PHYS_STATE%YGSP_SB%T, YDMF_PHYS_STATE%YGSP_RR%T, PVEG,&
       ...
       & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, PFEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV,&
       & YDMF_PHYS%OUT%FTR, PLHS, YDMF_PHYS%OUT%FRTH, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H)
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      IF (LCOEFKTKE) THEN
        ...
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KSTEP,&
          ...
          & ZDIFWQ,ZDIFWS,&
          & YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,YDMF_PHYS%OUT%FCS,PFEVI,YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       ...
       & YDMF_PHYS%OUT%FCS,YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,PLHS, YDMF_PHYS%OUT%GZ0,&
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      CALL ACPBLH ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       ...
       & YDMF_PHYS%OUT%FCS,YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,PLHS,ZRTI,YDMF_PHYS_SURF%GSD_VH%PPBLH)
    ENDIF
    ! ------------------------------------------------------------------
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       ...
       & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,ZQW,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,ZT,PTW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,PNEIJ,PGM,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD_MF,&
    ...
    ELSE
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       & ZVETAF,YDMF_PHYS_STATE%YCPG_DYN%PHI,&
       & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,PQW,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%T,PTW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,PNEIJ,PGM,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD_MF,&
       & YDMF_PHYS%OUT%FPFPSL,YDMF_PHYS%OUT%FPFPSN,YDMF_PHYS%OUT%FPFPSG,YDMF_PHYS%OUT%FCSQL,YDMF_PHYS%OUT%FCSQN,YDMF_PHYS%OUT%FPEVPSL,YDMF_PHYS%OUT%FPEVPSN,YDMF_PHYS%OUT%FPEVPSG,YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLSG,&
       & ZSEDIQL,ZSEDIQI,YDMF_PHYS%OUT%DIAGH)
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, PCVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, PQSAT, PQW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, PTW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%VOR, YDCPG_DYN0%CTY%VVEL(:,1:),&
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    CALL ACCVIMP_V3 (YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, PCVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, PQW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, PTW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,&
  ...
  IF(LNEBN.OR.LNEBR.OR.LRRGUST) THEN
    IF (LFPCOR) THEN
      IF (LCDDPRO) THEN
        DO JLEV=KTDIA-1,KLEV
          DO JLON=KIDIA,KFDIA
            PFPLCH(JLON,JLEV)=YDMF_PHYS%OUT%FPLCL(JLON,JLEV)+YDMF_PHYS%OUT%FPLCN(JLON,JLEV)
          ENDDO
        ENDDO
      ELSE
        DO JLEV=KTDIA-1,KLEV
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    CALL ACOZONE ( YDPHY2,YDTOPH,KIDIA,KFDIA,KLON,NTOZON,KLEV,KVCLIS,&
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,PKOZO,ZQO3(1,1),YDMF_PHYS_STATE%T,PMU0,&
     & YDMF_PHYS%OUT%FCHOZ)
