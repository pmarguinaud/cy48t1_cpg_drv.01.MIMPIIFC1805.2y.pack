...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZCGA_DST, ZTAUREL_DST, ZAERINDS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDCPG_GPAR%ZVIEW,                         &
      & YDCPG_GPAR%SWDIR, YDCPG_GPAR%SWDIF, YDMF_PHYS_TMP%RDG%MU0LU, YDMF_PHYS%OUT%ALB, YDMF_PHYS%RAD%RMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
