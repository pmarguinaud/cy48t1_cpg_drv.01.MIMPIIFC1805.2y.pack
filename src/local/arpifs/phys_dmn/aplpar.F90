...
!cdir unroll=8
  ...
  !                         FOR RAIN/SNOW/GRAUPEL PUT IN PFCQR/S/GNG
          ...
          ZQV0=PQ(JLON,JLEV)-ZIPOI(JLON,JLEV)*(0.0_JPRB- YDMF_PHYS%OUT%FCQNG(JLON,JLEV-1) &
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS,LLAROME,KIDIA,KFDIA,KLON,PAPHIF(:,KLEV),ZRHODREFM,YDMF_PHYS_SURF%GSD_SFO%PGROUP,&
        & ZTN,ZTSN,YDMF_PHYS_SURF%GSD_VF%PLSM,PQ(:,KLEV),PU(:,KLEV),PV(:,KLEV),ZTHETAS,YDMF_PHYS%OUT%FCS(KIDIA:KFDIA,1),ZFEV,ZFMDU,ZFMDV)
     DO JLON=KIDIA,KFDIA
        PTS(JLON)=ZTSN(JLON)
     ENDDO
     DO JLON=KIDIA,KFDIA
...
!DEC$ IVDEP
      DO JLON = KIDIA,KFDIA
        YDMF_PHYS%OUT%CT(JLON)=HSOL /(&
         & 1.0_JPRB+HSOLIWR*(PWS(JLON)+PWP(JLON,1))/(WSMX+WPMX)&
      ...
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (KSTEP == 0) THEN 
        ...
        CALL ACTKEZOTLS ( YDPHY0,KIDIA,KFDIA,KLON,KLEV,&
         & PAPHI, PAPHIF, PR, PT,&
         & YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, PTS, YDCPG_MISC%QS,&
         & PCDN, ZCDNH, PCPS, ZRTI, PRS)
         
      ELSE
        CALL ACHMTLS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
          & PAPHI,PAPHIF,PAPRS,PAPRSF,PR,PT,PU,PV,&
          & PTS,YDCPG_MISC%QS,ZDPHIT,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,LLCLS,&
...
!       Computation of ZRTI
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       ...
       & PR, PT, PU, PV, PLSCPE,YDMF_PHYS%OUT%GZ0,&
       & ZMN2PP,ZMRIPP)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      ENDDO
      DO JLON=KIDIA,KFDIA
        ZTHETAVS(JLON)=PTS(JLON)*(1.0_JPRB+RETV*YDCPG_MISC%QS(JLON))&
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      CALL ACMIXLENZ ( YDPHY,YDPHY0,KIDIA,KFDIA,KLON,1,KLEV,.FALSE.,&
       & PAPHI,PAPHIF,ZBLH,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZLMU,ZLMT)
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV, &
       & PAPHI,PAPHIF,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZBLH,ZLMU,ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV,&
       & PAPHI,PAPHIF,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZBLH,ZZLMT,ZLMT)
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         ...
         & YDMF_PHYS%OUT%GZ0,PTS,PCDN,ZBLH,ZLMU,ZLMT,YDVARS%MXL%T0,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
         ...
         & ZQV, ZQL, ZQI, PQSAT,&
         & PR, PT, PU, PV, ZLMU, ZLMT,YDMF_PHYS%OUT%GZ0,&
         & ZMRIPP)
      ENDIF
      ...
      CALL ACMRIP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,KSTEP,&
       ...
       & PTKE,YDVARS%TTE%T0,&
       & PTS,YDMF_PHYS_SURF%GSD_VH%PQSH,PRS,PCPS,ZRTI,YDMF_PHYS%OUT%GZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      ...
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
       ...
       & ZDPHIT,ZDPHIV,YDMF_PHYS%OUT%GZ0,YDMF_PHYS_SURF%GSD_VF%PLSM,PQSATS,ZCRTI,PTS,&
  ...
  IF (LRAFTKE) THEN
    ...
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, PU, PV, YDMF_PHYS%OUT%CAPE, ZDCAPE, PTKE, PAPHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,ZBLH,KCLPH)
  ENDIF
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       ...
       & PR, PT, PU, PV, PFPLSH, PFPLCH, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH,&
...
! Computation of the 2 znlab used in acbl89
    ...
    CALL ACTKE ( YDLDDH,YDMODEL%YRML_DIAG%YRMDDH,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTCOET,KLEV,&
     ...
     & PCD, PCH, YDMF_PHYS%OUT%GZ0, PTS, YDCPG_MISC%QS,&
     & YDCPG_MISC%QICE, YDCPG_MISC%QLI, PTKE, ZPRODTH_CVPP, ZNLAB, ZNLABCVP,&
    ...
    DO JLON=KIDIA,KFDIA
      IF (LSNV) THEN
        ...
        YDMF_PHYS%OUT%ALB(JLON)= (1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PVEG(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
         ...
         & MAX(YDMF_PHYS_SURF%GSD_VF%PALBSF(JLON),PALBNS(JLON,1))&
         & + YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*ZNEIJV(JLON) *&
         & MAX(ZALBV,PALBNS(JLON,1))&
        ...
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-PVEG(JLON)-PNEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & PVEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+PNEIJ(JLON)*PALBNS(JLON,1)
          ELSE
            ...
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-PNEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & PALBNS(JLON,1))+(PNEIJ(JLON)-ZNEIJV(JLON))*     &
             & YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*(YDMF_PHYS_SURF%GSD_VV%PALV(JLON)-PALBNS(JLON,1))
          ENDIF
  ...
  IF (LRAY.AND.NRAY == 1.AND.KNFRRC /= 0) THEN
    IF (MOD(KSTEP,KNFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       ...
       & PAPHIF,PAPRS,PAPRSF,PCP,PR,PDELP,YDCPG_MISC%NEB,&
       & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,PT,&
       & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PMU0,PGEMU,PGELAM,PMU0LU,PTS,&
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,LLREDPR,&
     & ZHUC,ZVETAF,PAPHI,PAPHIF,PAPRSF,PCP,PR,PDELP,YDCPG_MISC%RH,ZBLH,ZQV,ZQI,ZQL,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
      & PGM, PTS, YDCPG_MISC%QS, PNEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTNEBU,KLEV,&
     ...
     & YDMF_PHYS%OUT%GZ0,PFPLCH,YDMF_PHYS_SURF%GSD_VH%PPBLH,YDCPG_MISC%QS,PTS,&
...
!         Diagnostique de nebulosite partielle.
  CALL ACNPART(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
   & PAPHI,PAPHIF,PAPRSF,ZDECRD,YDCPG_MISC%NEB,&
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(KSTEP == 1).AND.LRCOEF ) THEN
    ...
    CALL ACRADCOEF ( YDRCOEF,YDPHY3,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & PAPRS,PAPRSF,PDELP,ZZNEB,ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,&
     & ZQO3,PT,&
...
!DEC$ IVDEP
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         ...
         & PAPRS,PAPRSF,PCP,PR,PDELP,YDCPG_MISC%NEB,&
         & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,PT,&
         & YDMF_PHYS%OUT%ALB,ZALBDIR,PEMIS,PGELAM,PGEMU,PMU0,PMU0LU,PTS,ZDECRD,ZCLCT_RAD,&
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV,YDMODEL,     &
       ...
       &  YDCPG_MISC%NEB  , ZQO3 , ZAER  , PDELP  , PEMIS , &
    ...
    &   KIDIA  , KFDIA  , KLON    , KLEV,&
    &   PAPRS  , PEMIS  , YDMF_PHYS%RAD%EMTD   , PMU0, ZQV  ,&
    &   ZTENT  , YDMF_PHYS%RAD%TRSW   , ZTRSOD , PTS , TSPHY,&
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU ,PMU0,PAPHIF,PDELP,YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU  ,PMU0,PAPHIF,PDELP,YDMF_PHYS%OUT%FRSO)
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    CALL ACVEG ( YDPHY,YDPHY1,KIDIA,KFDIA,KLON,KLEV,&
     & YDMF_PHYS%OUT%FRSO,ZQV,PQSAT,PT,&
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDIFU,KLEV,KCSS,&
       & PAPHI, PAPHIF, PCP, PDELP, YDMF_PHYS%OUT%FRSO, ZKTROV, ZKUROV, ZKNROV,&
       ...
       & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, PNEIJ, YDCPG_MISC%QS, PQSATS, PTP, PTS, PVEG,&
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      IF (LCOEFKTKE) THEN
        ...
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KSTEP,&
          ...
          & YDMF_PHYS%OUT%DIFTS,YDMF_PHYS%OUT%DIFTQ,PTKE,PTENDPTKE,&
...
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,KLEV,1)/PEMIS(JLON)+RSIGMA*PTS(JLON)**4
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY,YDPHY1,KIDIA, KFDIA, KLON, KTDIA, KLEV,&
     & PEMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, PNEIJ, ZQV, YDCPG_MISC%QS, PTS, ZCHROV,  ZDQSTS,&
...
! -----------------------------------------------------
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    CALL ACUPTQ ( KLON,KIDIA,KFDIA,KLEV,LLPTQ,YDMF_PHYS%OUT%FRSO,YDMF_PHYS%OUT%FRTH,&
     ...
     & YDMF_PHYS%OUT%FPEVPCL,YDMF_PHYS%OUT%FPEVPCN,YDMF_PHYS%OUT%FPFPCL,YDMF_PHYS%OUT%FPFPCN,PRDELP,PT,PQ,PTS,&
     & ZTENHA,ZTENQVA )
    ...
    CALL CUCALLN_MF &
     ...
     & PT,ZQV,PU,PV,ZLISUM,ZVERVEL,YDMF_PHYS%OUT%DIFTQ,YDMF_PHYS%OUT%DIFTS,&
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    CALL CULIGHT &
      ...
      &   PT    ,  ZLU   ,  ZMFU,    YDMF_PHYS%OUT%CAPE, &
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,KCSS,&
       ...
       & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, PFEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS_SURF%GSD_VF%PLSM, PSNS,&
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    CALL ACDIFOZ ( YDRIP,YDPHY2,YDMODEL%YRML_PHY_MF%YRVDOZ,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & PAPRSF,PDELP,YDMF_PHYS%OUT%FPLCL,YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FRSO,&
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=PDELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%QICE(KIDIA:KFDIA,1:KLEV)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAF,'VIR',YDDDH)
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=PDELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%QLI(KIDIA:KFDIA,1:KLEV)
      CALL NEW_ADD_FIELD_3D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAF,'VLR',YDDDH)
      ZTMPAF(KIDIA:KFDIA,1:KLEV)=PDELP(KIDIA:KFDIA,1:KLEV)*YDCPG_MISC%NEB(KIDIA:KFDIA,1:KLEV)
      ...
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*PTS(KIDIA:KFDIA)/YDMF_PHYS%OUT%CT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*RTINER*PTP(KIDIA:KFDIA,KCSS)/YDMF_PHYS%OUT%CT(KIDIA:KFDIA)
      ...
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*PWP(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSWP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*PSNS(KIDIA:KFDIA,1)
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     CALL ACEVADCAPE(KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLCL,YDMF_PHYS%OUT%FPLCN,PAPRS,PAPRSF,PT,PCP,PAPHIF,PAPHI,ZDCAPE)
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, PU, PV, YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, PAPHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, KCLPH)
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,PAPHI,PAPHIF,PAPRSF,&
      & PT,PR,YDCPG_MISC%QLI,YDCPG_MISC%QICE,ZQR,ZQS,ZQGM,YDMF_PHYS%OUT%VISICLD, YDMF_PHYS%OUT%VISIHYD,YDMF_PHYS%OUT%MXCLWC)
  ENDIF
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,ZPCLS,YDMF_PHYS%OUT%TCLS,&
  & PQ(:,KLEV),PQL(:,KLEV),PQI(:,KLEV),YDMF_PHYS%OUT%TPWCLS)
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS (YDPHY%YRDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,PAPHIF,&
