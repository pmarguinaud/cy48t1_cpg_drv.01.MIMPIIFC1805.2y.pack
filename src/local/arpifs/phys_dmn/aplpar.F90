...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & NTPLUI, YDCPG_OPTS%KFLEVG, LLREDPR, YDMODEL%YRML_PHY_MF%YRPHY0%RHUC, YDSTA%SVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI,    &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDCPG_MISC%QS, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDSTA%SVETAF,                         &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    ...
    & ZAIPCMT, YDCPG_MISC%NEB, ZNEBC0, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDMODEL%YRML_PHY_MF%YRPHY0%RHUC, YDSTA%SVETAF)
    DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
!DEC$ IVDEP
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
!     ---------------------------------
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & YDCPG_OPTS%KFLEVG, YDSTA%SVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,            &
...
! -----------------------------------------------------
  ...
  IF ( LPROCLD ) THEN
    ...
    & YDSTA%SVETAF, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL,            &
