...
ASSOCIATE(YDDIM=>YDGEOMETRY%YRDIM,YDGEM=>YDGEOMETRY%YRGEM, &
 & YDSTA=>YDGEOMETRY%YRSTA, YDCSGEOM => YDGEOMETRY%YRCSGEOM(YDCPG_DIM%KBL), &
 & YDGSGEOM=>YDGEOMETRY%YRGSGEOM(YDCPG_DIM%KBL), YDOROG=>YDGEOMETRY%YROROG(YDCPG_DIM%KBL), &
...
& ,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDGSGEOM%GELAM,YDGSGEOM%GEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_STATE%T,YDMF_PHYS_STATE%Q,YDMF_PHYS_STATE%YGSP_RR%T)
!     ------------------------------------------------------------------
...
IF (LMPHYS) THEN
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDGSGEOM%GEMU,ZROZ)
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,0)=1.E-9_JPRB
      ENDDO
      DO JLEV=YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG
...
!     ----------------------------
  IF ( LTHERMO ) THEN
    CALL ACTQSAT ( YDPHY,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTQSAT,YDCPG_DIM%KFLEVG,&
     ...
     & ZGEOSLC, YDAPLPAR_TMP%MSC%LH, YDAPLPAR_TMP%MSC%LSCPE, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDCPG_MISC%RH, YDAPLPAR_TMP%MSC%TW)
  ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS,LLAROME,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDMF_PHYS_STATE%YCPG_DYN%PHIF(:,YDCPG_DIM%KFLEVG),ZRHODREFM,YDMF_PHYS_SURF%GSD_SFO%PGROUP,&
        & ZTN,ZTSN,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_STATE%Q(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_STATE%U(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_STATE%V(:,YDCPG_DIM%KFLEVG),ZTHETAS,YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1),ZFEV,ZFMDU,ZFMDV)
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      CALL ACSOL ( YDPHY,YDPHY1,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,&
       ...
       & LLHMT,&
       & YDAPLPAR_TMP%DSA%C1,YDAPLPAR_TMP%DSA%C2,ZC3,ZCG,ZCN,YDMF_PHYS%OUT%CT,ZNEIJG,ZNEIJV,&
    ...
    IF (LMSE) THEN
      ...
      ELSE
        CALL ACHMTLS ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,&
          ...
          & ZNBVNO,ZMRIPP,YDAPLPAR_TMP%DSA%CPS,ZGWDCS,YDAPLPAR_TMP%DSA%LHS,ZPCLS,YDAPLPAR_TMP%FLU%CD,YDAPLPAR_TMP%FLU%CDN)
!       Computation of ZRTI
        DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        CALL ACTKEHMT ( YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDSURF%YSD_VVD%NUMFLDS>=8.AND.LSOLV,&
         ...
         & YDAPLPAR_TMP%PFL%FPLSH,YDAPLPAR_TMP%PFL%FPLCH,&
         ...
         & LLCLS, LLHMT,&
         & ZNBVNO,ZMRIPP,&
         & YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, YDAPLPAR_TMP%FLU%CH, ZCHROV, YDAPLPAR_TMP%DSA%CPS, ZDQSTS, ZGWDCS,&
         & YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZHQ, ZHU, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS%OUT%QCLS, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%QSATS, YDMF_PHYS%OUT%RHCLS,&
         & YDAPLPAR_TMP%DSA%RS, ZRTI, ZSTAB, YDMF_PHYS%OUT%TCLS, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS%OUT%NUCLS, YDMF_PHYS%OUT%NVCLS, ZPCLS, YDAPLPAR_TMP%FLU%VEG,&
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,YDCPG_DIM%KFLEVG,&
       & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,&
       & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDAPLPAR_TMP%MSC%LSCPE,YDMF_PHYS%OUT%GZ0,&
       & ZMN2PP,ZMRIPP)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      CALL ACCLPH ( YDPHY0,YDPHY2,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
       ...
       & YDAPLPAR_TMP%MOC%CLPH,YDMF_PHYS%OUT%CLPH,YDMF_PHYS%OUT%VEIN,ZUGST,ZVGST)
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      CALL ACMIXLENZ ( YDPHY,YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,1,YDCPG_DIM%KFLEVG,.FALSE.,&
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG, &
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      CALL ACMIXLENZ(YDPHY,YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,1,YDCPG_DIM%KFLEVG,.TRUE.,&
       & YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,ZBLH,YDMF_PHYS%OUT%GZ0,YDMF_PHYS%OUT%GZ0H,ZLMU,ZLMT)
      ZLMU2(:,:)=ZLMU(:,:)
      ZLMT2(:,:)=ZLMT(:,:)
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,IMLTYPE,&
         ...
         & YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,ZMN2PP,ZFMGST,YDAPLPAR_TMP%PFL%FPLSH,YDAPLPAR_TMP%PFL%FPLCH,&
         & YDMF_PHYS%OUT%GZ0,YDMF_PHYS_STATE%YGSP_RR%T,YDAPLPAR_TMP%FLU%CDN,ZBLH,ZLMU,ZLMT,YDVARS%MXL%T0,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      ! ------------------------------------------------------------- 
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,YDCPG_DIM%KFLEVG,&
         & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDAPLPAR_TMP%MSC%LSCPE,&
         & ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,&
         & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZLMU, ZLMT,YDMF_PHYS%OUT%GZ0,&
         & ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,&
       ...
       & ZQV, ZQL, ZQI,ZCP,&
       & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,YDAPLPAR_TMP%FLU%QSAT,YDAPLPAR_TMP%MSC%QW,YDAPLPAR_TMP%MSC%TW,&
       ...
       & YDMF_PHYS_STATE%YGSP_RR%T,YDMF_PHYS_SURF%GSD_VH%PQSH,YDAPLPAR_TMP%DSA%RS,YDAPLPAR_TMP%DSA%CPS,ZRTI,YDMF_PHYS%OUT%GZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      ...
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,&
       ...
       & ZDPHIT,ZDPHIV,YDMF_PHYS%OUT%GZ0,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR_TMP%FLU%QSATS,ZCRTI,YDMF_PHYS_STATE%YGSP_RR%T,&
       & LLHMT,YDAPLPAR_TMP%FLU%CD,YDAPLPAR_TMP%FLU%CDN,ZCDROV,ZCE,ZCEROV,YDAPLPAR_TMP%FLU%CH,ZCHROV,&
  ...
  IF (LRAFTKE) THEN
    ...
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,ZBLH,YDAPLPAR_TMP%MOC%CLPH)
  ENDIF
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,YDCPG_DIM%KFLEVG,&
       & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDAPLPAR_TMP%MSC%LSCPE, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,&
       & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH,&
  ...
  IF (LEDKF) THEN
    ...
    CALL ARP_SHALLOW_MF( YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,ZIMPL,TSPHY,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, &
...
! Computation of the 2 znlab used in acbl89
    ...
    CALL ACTKE ( YDLDDH,YDMODEL%YRML_DIAG%YRMDDH,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,NTCOET,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZQV, ZQIC, ZQLC, YDAPLPAR_TMP%MSC%LSCPE,&
     & YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CH, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS,&
     & YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDMF_PHYS_STATE%TKE, ZPRODTH_CVPP, ZNLAB, ZNLABCVP,&
     & ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZXTROV, ZXUROV,&
     & ZNBVNO, ZNEBS, ZQLIS, ZNEBS0, ZQLIS0, ZCOEFN , YDAPLPAR_TMP%PFL%FTKE, YDAPLPAR_TMP%PFL%FTKEI, ZTKE1,ZTPRDY,&
     & YDMF_PHYS%OUT%EDR,YDDDH)
...
! input dust scalar concentration in ppp from
  ...
  CALL ARO_MNHDUST (1,ILONMNH,YDCPG_DIM%KFLEVG,NGFL_EXT, PDT,&
                 & ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:,1:NGFL_EXT),&
                 & ZZZ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZDZZ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_PABSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_THM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_RHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & NSWB_MNH,&
                 & YDCPG_DIM%KSTEP+1,&
                 & ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:,1:NGFL_EXT),&
                 & ZPIZA_DST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZCGA_DST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZTAUREL_DST(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZAERD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:),&
                 & NGFL_EZDIAG,&
                 & ZZI_PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:)           )
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTRADI,YDCPG_DIM%KFLEVG,IJN,&
       ...
       & YDMF_PHYS%OUT%ALB,ZALBDIR,YDAPLPAR_TMP%FLU%EMIS,YDMF_PHYS_TMP%RDG%MU0,YDGSGEOM%GEMU,YDGSGEOM%GELAM,YDMF_PHYS_TMP%RDG%MU0LU,YDMF_PHYS_STATE%YGSP_RR%T,&
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    CALL ACCVIMPGY ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDAPLPAR_TMP%MSC%LH,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,ZQV,ZQCI,ZQCL,ZQLIS,YDAPLPAR_TMP%FLU%QSAT,YDAPLPAR_TMP%MSC%QW,&
     & YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDMF_PHYS_STATE%T,YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,&
     & YDAPLPAR_TMP%DSA%CPS,YDGSGEOM%GM,YDMF_PHYS_STATE%YGSP_RR%T,&
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,LLREDPR,&
     ...
     & YDAPLPAR_TMP%MSC%QW,YDMF_PHYS_STATE%T,ZNEBCH,YDGSGEOM%GM,YDMF_PHYS_STATE%YGSP_RR%T,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,INBTRA,&
      ! - INPUT  2D .
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDCPG_DYN0%CTY%VVEL(:,1:),YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDAPLPAR_TMP%MSC%LH,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,&
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,&
      & YDMF_PHYS_STATE%Q,ZQI,ZQL,YDMF_PHYS_STATE%R,YDMF_PHYS_STATE%S,ZQLIS,YDAPLPAR_TMP%FLU%QSAT,YDAPLPAR_TMP%MSC%QW,&
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDMF_PHYS_TMP%RDG%CVGQ,YDMF_PHYS_STATE%T,YDAPLPAR_TMP%MSC%TW,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%TKE,ZTRA(:,:,INBTRA_DEP:INBTRA),ZSMOOTRAC(1:INBTRA),&
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
      & YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    CALL ACNEBN ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTNEBU,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,ZQV,ZQL,ZQI,YDAPLPAR_TMP%FLU%QSAT,YDMF_PHYS_STATE%T,YDAPLPAR_TMP%PFL%FPLCH,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,&
     & ZUNEBH,&
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCOEF,NTNEBU,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDAPLPAR_TMP%MSC%LSCPE,ZQV,&
     & YDAPLPAR_TMP%FLU%QSAT,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%T,YDMF_PHYS_STATE%U,YDMF_PHYS_STATE%V,YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS%OUT%GZ0,YDAPLPAR_TMP%PFL%FPLCH,YDMF_PHYS_SURF%GSD_VH%PPBLH,YDCPG_MISC%QS,YDMF_PHYS_STATE%YGSP_RR%T,&
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    CALL ACRADCOEF ( YDRCOEF,YDPHY3,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS_TMP%RDG%MMU0,YDCPG_MISC%DHSF,YDAPLPAR_TMP%FLU%EMIS,&
     & ZAER,&
     & POMPAC,PAC,YDAPLPAR_TMP%RDT%COR,&
     & YDAPLPAR_TMP%RDT%RAB3C,YDAPLPAR_TMP%RDT%RAB3N,YDAPLPAR_TMP%RDT%RAB4C,YDAPLPAR_TMP%RDT%RAB4N,YDAPLPAR_TMP%RDT%RAB6C,YDAPLPAR_TMP%RDT%RAB6N,&
     & YDAPLPAR_TMP%RDT%RAT1C,YDAPLPAR_TMP%RDT%RAT1N,YDAPLPAR_TMP%RDT%RAT2C,YDAPLPAR_TMP%RDT%RAT2N,YDAPLPAR_TMP%RDT%RAT3C,YDAPLPAR_TMP%RDT%RAT3N,&
     & YDAPLPAR_TMP%RDT%RAT4C,YDAPLPAR_TMP%RDT%RAT4N,YDAPLPAR_TMP%RDT%RAT5C,YDAPLPAR_TMP%RDT%RAT5N)
  ENDIF
...
!DEC$ IVDEP
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTRADI,YDCPG_DIM%KFLEVG,IJN,YDCPG_DIM%KSTEP,YDCFU%NFRRC,&
         ...
         & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,YDMF_PHYS_STATE%T,&
         & YDMF_PHYS%OUT%ALB,ZALBDIR,YDAPLPAR_TMP%FLU%EMIS,YDGSGEOM%GELAM,YDGSGEOM%GEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_TMP%RDG%MU0LU,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD,ZCLCT_RAD,&
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV,YDMODEL,     &
       ...
       &  YDCPG_MISC%NEB  , ZQO3 , ZAER  , YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP  , YDAPLPAR_TMP%FLU%EMIS , &
       &  YDMF_PHYS_TMP%RDG%MU0M , ZQV  , YDAPLPAR_TMP%FLU%QSAT , YDCPG_MISC%QICE  , YDCPG_MISC%QLI  , & 
       ...
       &  YDGSGEOM%GELAM, YDGSGEOM%GEMU, PGPAR, YDMF_PHYS_TMP%RDG%MU0LU , YDMF_PHYS%OUT%ALB, YDMF_PHYS%RAD%RMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
    ...
    &   YDCPG_DIM%KIDIA  , YDCPG_DIM%KFDIA  , YDCPG_DIM%KLON    , YDCPG_DIM%KFLEVG,&
    &   YDMF_PHYS_STATE%YCPG_PHY%PREHYD  , YDAPLPAR_TMP%FLU%EMIS  , YDMF_PHYS%RAD%EMTD   , YDMF_PHYS_TMP%RDG%MU0, ZQV  ,&
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,YDGSGEOM%GEMU ,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,YDGSGEOM%GEMU  ,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)
...
!DEC$ IVDEP
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,                             &
       & YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS%OUT%FRSO(:,YDCPG_DIM%KFLEVG,1),YDMF_PHYS%OUT%FRSODS,YDMF_PHYS%OUT%FRSOPS,YDMF_PHYS%OUT%FRTH(:,YDCPG_DIM%KFLEVG,1),YDMF_PHYS%OUT%FRTHDS, &
       & YDMF_PHYS%OUT%MRT) 
    ENDIF
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    CALL ACVEG ( YDPHY,YDPHY1,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS%OUT%FRSO,ZQV,YDAPLPAR_TMP%FLU%QSAT,YDMF_PHYS_STATE%T,&
     & YDMF_PHYS_SURF%GSD_VV%PD2,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_SURF%GSD_VV%PIVEG,YDMF_PHYS_SURF%GSD_VV%PLAI,YDAPLPAR_TMP%FLU%NEIJ,YDAPLPAR_TMP%FLU%VEG,YDMF_PHYS_SURF%GSD_VV%PRSMIN,&
     & ZCHROV,ZGWDCS,ZWFC,YDMF_PHYS_STATE%YGSP_RR%FC,ZWWILT,YDMF_PHYS_STATE%YGSP_SB%Q,YDAPLPAR_TMP%FLU%QSATS,&
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDIFU,YDCPG_DIM%KFLEVG,YSP_SBD%NLEVS,&
       ...
       & YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCHROV, ZCEROV, YDAPLPAR_TMP%DSA%CPS, YDMF_PHYS%OUT%CT,ZDQSTS,&
       & YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VH%PSPSH, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV,&
       & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDAPLPAR_TMP%FLU%NEIJ, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%QSATS, YDMF_PHYS_STATE%YGSP_SB%T, YDMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,&
       ...
       & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV,&
       & YDMF_PHYS%OUT%FTR, YDAPLPAR_TMP%DSA%LHS, YDMF_PHYS%OUT%FRTH, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H)
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        CALL ACPTKE(YGFL,YDLDDH,YDMODEL%YRML_PHY_MF,&
          & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,&
          ...
          & YDAPLPAR_TMP%FLU%CD,YDMF_PHYS%OUT%GZ0,&
      ...
      CALL ACDIFV1 ( YGFL,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP,&
          ...
          & YDMF_PHYS_TMP%RDG%MU0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDMF_PHYS_TMP%RDG%MU0N(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDGSGEOM%GELAM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
          & YDGSGEOM%GEMU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),XSW_BANDS,&
...
! calculation of variables for the old "ISBA" atmosphere scheme
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          CALL ACAA1 ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          IF (LEDMFI) THEN
            ...
            ZCFBV_G(:,:) = ZCFBV(:,:)
          ENDIF   
          CALL ARP_GROUND_PARAM ( YDMCC,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YSP_SBD%NLEVS,&
           ...
           & YDAPLPAR_TMP%FLU%CD,YDAPLPAR_TMP%FLU%CDN,ZCDROV,ZCHROV,ZCEROV,YDAPLPAR_TMP%DSA%CPS,YDMF_PHYS%OUT%CT,ZDQSTS,YDAPLPAR_TMP%FLU%EMIS,YDMF_PHYS_SURF%GSD_VH%PSPSH,&
           & ZHQ,ZHTR,ZHU,YDMF_PHYS_SURF%GSD_VV%PHV,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_SURF%GSD_VV%PIVEG,YDAPLPAR_TMP%FLU%NEIJ,YDCPG_MISC%QS,YDAPLPAR_TMP%FLU%QSATS,&
           & YDMF_PHYS_STATE%YGSP_SB%T,YDMF_PHYS_STATE%YGSP_RR%T,YDAPLPAR_TMP%FLU%VEG,ZXDROV,ZXHROV,YDMF_PHYS_STATE%YGSP_RR%W,YDMF_PHYS_STATE%YGSP_RR%IC,&
           & ZDSE,&
           & ZCFAS,ZCFAU,&
           & ZCFBS,ZCFBU,ZCFBV,&
           ...
           & YDMF_PHYS%OUT%FCHSP,YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,YDMF_PHYS%OUT%FCS,YDAPLPAR_TMP%FLU%FEVI,YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
           & YDMF_PHYS%OUT%FEVV,YDMF_PHYS%OUT%FTR,YDAPLPAR_TMP%DSA%LHS,YDMF_PHYS_SURF%GSD_VH%PQSH,&
           & YDMF_PHYS%OUT%FRTH,&
      ...
      CALL ACDIFV2 ( YGFL,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
      ...
      IF (L3DTURB) THEN
        CALL ACTKECOEFKH(YDRIP,YDMODEL%YRML_PHY_MF,YDGEOMETRY%YREGEO,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
          ...
          & ZRTI, YDAPLPAR_TMP%FLU%CD, ZCDROV,&
          & LPTKE, YDAPLPAR_TMP%KUR%KUROV_H, YDAPLPAR_TMP%KUR%KTROV_H, ZRHS)
      ENDIF
      ...
      IF (LCOEFKTKE) THEN
        ...
        ELSEIF (NDIFFNEB == 4) THEN
          ZCOEFA(:,:) = ZBNEBCVPP(:,:)
        ENDIF
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YDCPG_DIM%KSTEP,&
          ...
          & ZDIFWQ,ZDIFWS,&
          & YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,YDMF_PHYS%OUT%FCS,YDAPLPAR_TMP%FLU%FEVI,YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDIFU,YDCPG_DIM%KFLEVG,&
       ...
       & YDMF_PHYS%OUT%FCS,YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,YDAPLPAR_TMP%DSA%LHS, YDMF_PHYS%OUT%GZ0,&
       & YDAPLPAR_TMP%MOC%CLPH,ZBLH,ZUGST,ZVGST)
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      CALL ACPBLH ( YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDIFU,YDCPG_DIM%KFLEVG,&
       ...
       & YDMF_PHYS%OUT%FCS,YDMF_PHYS%OUT%FCLL,YDMF_PHYS%OUT%FCLN,YDAPLPAR_TMP%DSA%LHS,ZRTI,YDMF_PHYS_SURF%GSD_VH%PPBLH)
    ENDIF
    ! ------------------------------------------------------------------
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY,YDPHY1,YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG,&
     & YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, ZQV, YDCPG_MISC%QS, YDMF_PHYS_STATE%YGSP_RR%T, ZCHROV,  ZDQSTS,&
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    CALL ACDRAG ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDRAG,YDCPG_DIM%KFLEVG,&
     ...
     & YDGSGEOM%RCORI,YDMF_PHYS_SURF%GSD_VF%PGETRL, ZGWDCS,&
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    CALL ACPLUIE ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
     ...
     & ZQV, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%T,&
  ...
  IF ( LSTRAS ) THEN
    CALL ACPLUIS ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZNEBS,ZQV,ZQLIS,YDAPLPAR_TMP%MSC%QW,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%T,&
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,LLREDPR,&
       & ZHUC,ZVETAF,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDCPG_MISC%RH,ZBLH,ZQV,ZQI,ZQL,&
       & ZQW,ZT,ZNEBCH,YDGSGEOM%GM,YDMF_PHYS_STATE%YGSP_RR%T,&
       & ZQLIS,ZNEBS,ZRHCRI,ZRH,ZQSATS,ZICEFR1,ZQLIS0,ZNEBS0)
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
       ...
       & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,ZQW,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,ZT,YDAPLPAR_TMP%MSC%TW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR_TMP%FLU%NEIJ,YDGSGEOM%GM,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD_MF,&
    ...
    ELSE
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
       & ZVETAF,YDMF_PHYS_STATE%YCPG_DYN%PHI,&
       & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,ZQV,ZQI,ZQL,ZQS,ZQR,ZQG,YDAPLPAR_TMP%MSC%QW,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,YDMF_PHYS_STATE%T,YDAPLPAR_TMP%MSC%TW,ZNEBS,&
       & ZRHCRI,ZICEFR1,ZQSATS,ZNEBCH,ZRHDFDA,ZRKTTEND,ZRKQVTEND,ZRKQCTEND,&
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR_TMP%FLU%NEIJ,YDGSGEOM%GM,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD_MF,&
       & YDMF_PHYS%OUT%FPFPSL,YDMF_PHYS%OUT%FPFPSN,YDMF_PHYS%OUT%FPFPSG,YDMF_PHYS%OUT%FCSQL,YDMF_PHYS%OUT%FCSQN,YDMF_PHYS%OUT%FPEVPSL,YDMF_PHYS%OUT%FPEVPSN,YDMF_PHYS%OUT%FPEVPSG,YDMF_PHYS%OUT%FPLSL,YDMF_PHYS%OUT%FPLSN,YDMF_PHYS%OUT%FPLSG,&
       & ZSEDIQL,ZSEDIQI,YDMF_PHYS%OUT%DIAGH)
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      CALL ACCVUD ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
       ...
       & YDVARS%DAL%T0,&
       & YDMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,YDGSGEOM%GM, ZDECRD_MF,&
...
! PASS T9 CONDENSATES TO ACCSU
      CALL ACCSU ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_EC%YRECUMF,YDMODEL%YRML_PHY_MF, &
        & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
        & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS_STATE%YCPG_DYN%RCP%R,&
        & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,&
        ...
        & YDMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,ZDECRD_MF,&
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    CALL APLMPHYS( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
     ...
     & ZTCORR,YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_STATE%YGSP_RR%T, ZDECRD_MF,&
     & ZFCQL, ZFCQI,&
   ...
   CALL ACUPM( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
...
! input/output  2d .
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      CALL ACNSDO(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
    ...
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
       & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,&
  ...
  !  ---------------------------------------------
    ...
    CALL ACUPD(YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
! input  2d .
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%VOR, YDCPG_DYN0%CTY%VVEL(:,1:),&
     & YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,&
     ...
     & YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS, ZGEOSLC, YDGSGEOM%GEMU)
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    CALL ACCVIMP_V3 (YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,&
     & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,&
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    CALL CUCALLN_MF &
     ...
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,ZGEOM1,ZGEOMH,YDGSGEOM%GM, &
  ...
  IF ( LPROCLD ) THEN
    ...
    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, YDGSGEOM%GM, ZVETAF,&
  ...
  IF ( LNORGWD ) THEN
    ...
    CALL ACNORGWD(YDNORGWD, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, TSPHY, &
                    & Z_PP, YDGSGEOM%GEMU, Z_TT, Z_UU, Z_VV, Z_VO, ZPRECGWD, &
                    & ZD_U, ZD_V)
...
!     ---------------------------
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YSP_SBD%NLEVS,&
       & YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, ZFPLSL, ZFPLSN, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,&
       & YDAPLPAR_TMP%DSA%C1, YDAPLPAR_TMP%DSA%C2, ZC3, ZCN, YDMF_PHYS%OUT%CT, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR, YDMF_PHYS_SURF%GSD_VV%PLAI, YDAPLPAR_TMP%FLU%NEIJ,&
       & YDAPLPAR_TMP%FLU%VEG, ZWFC, ZWPMX, YDMF_PHYS_STATE%YGSP_RR%FC, ZWLMX, ZWSEQ, ZWSMX,&
       & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_STATE%YGSP_SG%F,&
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME ( YDPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTDRME,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS%OUT%FRMH,YDAPLPAR_TMP%MSC%FRMQ,YDMF_PHYS%OUT%STRMU,YDMF_PHYS%OUT%STRMV,&
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    CALL ACOZONE ( YDPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTOZON,YDCPG_DIM%KFLEVG,KVCLIS,&
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,PKOZO,ZQO3(1,1),YDMF_PHYS_STATE%T,YDMF_PHYS_TMP%RDG%MU0,&
     & YDMF_PHYS%OUT%FCHOZ)

!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE

    CALL ACDIFOZ ( YDRIP,YDPHY2,YDMODEL%YRML_PHY_MF%YRVDOZ,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
     ...
     & YDAPLPAR_TMP%FLU%NEIJ,YDGSGEOM%GEMU,YDMF_PHYS_SURF%GSD_VV%PIVEG,&
     & YDMF_PHYS%OUT%FCHOZ)
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, YDAPLPAR_TMP%MOC%CLPH)
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,YDMF_PHYS_STATE%YCPG_DYN%PHI,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,&
...
ZZDEP=0.0_JPRB
  ...
  ENDDO
    ...
    CALL ARO_WETDEP(ILONMNH,YDCPG_DIM%KFLEVG,NGFL_EXT,IKRR, PDT,&
                 & ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:,1:NGFL_EXT),&
                 & ZZDEP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_PABSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_THM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ZZI_RHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & YDCPG_DIM%KSTEP+1,&
                 & ZZI_RM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:,:),&
                 & ZEVAP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:),&
                 & ISPLITR               )
! return to tendency
   DO JGFL=1,NGFL_EXT
     DO JLON=1,YDCPG_DIM%KLON
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS (YDPHY%YRDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDOROG%OROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTW,YDMF_PHYS_STATE%L,ZFPLC(:,YDCPG_DIM%KFLEVG),ZFPLS(:,YDCPG_DIM%KFLEVG),ZFPLSG(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_TMP%PRC%DPRECIPS(:,NDTPRECCUR))
 ...
 !Idem for an other time step and an other period
   ...
   CALL DPRECIPS(YDPHY%YRDPRECIPS,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDOROG%OROG,YDMF_PHYS%OUT%TPWCLS,YDMF_PHYS%OUT%DIAGH,YDMF_PHYS_STATE%YCPG_DYN%PHIF,&
      & ZDZZ,ZTW,YDMF_PHYS_STATE%L,ZFPLC(:,YDCPG_DIM%KFLEVG),ZFPLS(:,YDCPG_DIM%KFLEVG),ZFPLSG(:,YDCPG_DIM%KFLEVG),YDMF_PHYS_TMP%PRC%DPRECIPS2(:,NDTPRECCUR2))
