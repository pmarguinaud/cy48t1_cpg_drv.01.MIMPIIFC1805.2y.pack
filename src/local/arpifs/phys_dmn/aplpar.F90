...
& YDVARS, YDGMV, YDSURF, YDCFU, YDXFU, YDMODEL, LDCONFX, PDTPHY, PGFL, PGMVT1, PGFLT1, PTRAJ_PHYS, &
& YDDDH)
...
& LADJCLD=>YDPHY%LADJCLD, LGRAPRO=>YDPHY%LGRAPRO, RDECLI=>YDRIP%RDECLI, CMF_CLOUD=>YDPARAR%CMF_CLOUD,                                     &
& XSW_BANDS=>YDPARAR%XSW_BANDS, NSWB_MNH=>YDPARAR%NSWB_MNH, LMIXUV=>YDPARAR%LMIXUV, NTRADI=>YDTOPH%NTRADI,                                &
...
CALL SC2PRG(YEFB1%MP1,  ZTENDGFL, ZPTENDEFB11,  LDDUMM=.TRUE.)
CALL SC2PRG(YEFB2%MP1,  ZTENDGFL, ZPTENDEFB21,  LDDUMM=.TRUE.)
CALL SC2PRG(YEFB3%MP1,  ZTENDGFL, ZPTENDEFB31,  LDDUMM=.TRUE.)
CALL SC2PRG(YG%MP1,     ZTENDGFL, ZPTENDG1,     LDDUMM=.TRUE.)
CALL SC2PRG(YICONV%MP1, ZTENDGFL, ZPTENDICONV1, LDDUMM=.TRUE.)
CALL SC2PRG(YI%MP1,     ZTENDGFL, ZPTENDI1,     LDDUMM=.TRUE.)
CALL SC2PRG(YLCONV%MP1, ZTENDGFL, ZPTENDLCONV1, LDDUMM=.TRUE.)
CALL SC2PRG(YL%MP1,     ZTENDGFL, ZPTENDL1,     LDDUMM=.TRUE.)
CALL SC2PRG(YQ%MP1,     ZTENDGFL, ZPTENDQ1,     LDDUMM=.TRUE.)
CALL SC2PRG(YRCONV%MP1, ZTENDGFL, ZPTENDRCONV1, LDDUMM=.TRUE.)
CALL SC2PRG(YR%MP1,     ZTENDGFL, ZPTENDR1,     LDDUMM=.TRUE.)
CALL SC2PRG(YSCONV%MP1, ZTENDGFL, ZPTENDSCONV1, LDDUMM=.TRUE.)
CALL SC2PRG(YS%MP1,     ZTENDGFL, ZPTENDS1,     LDDUMM=.TRUE.)
CALL SC2PRG(YTKE%MP1,   ZTENDGFL, ZPTENDTKE1,   LDDUMM=.TRUE.)

CALL SC2PRG(1, YEZDIAG(:)%MP, YDMODEL%YRML_GCONF%YGFL%NGFL_EZDIAG, PGFL, ZP1EZDIAG, LDDUMM=.TRUE.)
...
!-------------------------------------------------
!
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,                      &
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDCPG_MISC%QS, ZFLU_NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF,                               &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    ...
    ELSE
      ...
      CALL ACDIFV1 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,                                 &
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        & NDLUNG, NDLUXG, LSURFEX_KFROM, LMPA, CCOUPLING, LDCONFX, NINDAT, ZRHGMT, ZSTATI, RSOVR,                                                                              &
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      CALL ACDIFV2 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,               &
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    CALL ACDNSHF(YDPHY, YDPHY1, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA,                         &
...
!     ---------------------------------
  ...
  IF ( LSTRAS ) THEN
    CALL ACPLUIS ( YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI,                &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI,                        &
      & YDCPG_OPTS%KFLEVG, ZVETAF, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,                  &
...
! -----------------------------------------------------
  ...
  IF ( LADJCLD ) THEN
    ...
    ZFRTH(:,:) = 0.0_JPRB
    LLPTQ = .TRUE.
    CALL ACUPTQ ( YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KFLEVG, LLPTQ,                  &
  ...
  IF ( LPROCLD ) THEN
    ...
    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, NTPLUI,                                      &
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME (YDMODEL%YRCST, YDPHY2, YDTOPH, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,           &
    ...
    & ZMSC_FRMQ, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV, RMESOT, RMESOQ, RMESOU, STTEM)
  ENDIF
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,              &
     & YDCPG_OPTS%KFLEVG, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, &
     & YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDVARS%GEOMETRY%OROG%T0,            &
     & YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, IMOC_CLPH)
...
CALL PPWETPOINT(YDPHY, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, ZPCLS, YDMF_PHYS%OUT%TCLS,                                      &
...
CALL CPUTQY_APLPAR_EXPL(YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS_NEXT_STATE, YDMF_PHYS_BASE_STATE, YDVARS, &
& YDPHY, PDTPHY, ZTENDH, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDU, ZTENDV, ZTENDD,          &
...
IF ((.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    ...
    CALL CPWTS(YDCPG_OPTS, YDMODEL%YRML_AOC%YRMCC, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA,    &
    & YDCPG_BNDS%KFDIA, YSP_SBD%NLEVS, PDTPHY, ZTDS_TDTS, ZTDS_TDTP, ZTDS_TDWS, ZTDS_TDWSI,                                 &
