!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDCPG_DIM,YDMF_PHYS_STATE,YDCPG_DYN0,YDCPG_MISC,YDMF_PHYS,&
...
IF(LGCHECKMV) CALL CHECKMV(YDRIP, YDPHY0, YDPHY2, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,          &
              & YDCPG_DIM%KSTEP, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, &
              & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0,                        &
              & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%Q, YDMF_PHYS_STATE%YGSP_RR%T                    &
              & )
!     ------------------------------------------------------------------
...
IF (LMPHYS) THEN
  ...
  ELSEIF ((LO3FL).AND.(NOZOCL == 1).AND.(LRAYFM)) THEN
    IF (MOD(YDCPG_DIM%KSTEP,NRADFR) == 0) THEN
      CALL RADOZCMF(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, &
      & YDGSGEOM%GEMU, ZROZ)
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZQO3(JLON,0)=1.E-9_JPRB
      ENDDO
  ...
  ELSE
    CALL SUOZON(YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, ZQO3, .FALSE., YDMF_PHYS_STATE%YCPG_PHY%PREHYD, &
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, LO3ABC, YDMF_PHYS_SURF%GSD_VC%PGROUP)
  ENDIF
...
IF(LCOEFKTKE.AND.YDCPG_DIM%KSTEP == 0) THEN
  YDMF_PHYS_SURF%GSD_VH%PQSH(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG)
ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTN(JLON) =YDMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)
     ENDDO
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        ZTHETAS(JLON)   = ZTSN(JLON)*(RATM/YDMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG))**RKAPPA
     ENDDO
     LLAROME = .FALSE.
     CALL SURF_IDEAL_FLUX(YDRIP, YDPHY0, YDPHYDS, LLAROME, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                  &
     & YDMF_PHYS_STATE%YCPG_DYN%PHIF(:, YDCPG_DIM%KFLEVG), ZRHODREFM, YDMF_PHYS_SURF%GSD_SFO%PGROUP,                          &
     & ZTN, ZTSN, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_STATE%Q(:, YDCPG_DIM%KFLEVG), YDMF_PHYS_STATE%U(:, YDCPG_DIM%KFLEVG), &
     & YDMF_PHYS_STATE%V(:, YDCPG_DIM%KFLEVG), ZTHETAS, YDMF_PHYS%OUT%FCS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1),                &
     & ZFEV, ZFMDU, ZFMDV)
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
     ...
     DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ...
        YDAPLPAR_TMP%DSA%LHS(JLON)=FOLH(YDMF_PHYS_STATE%YGSP_RR%T(JLON),0.0_JPRB)
     ENDDO
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS_SURF%GSD_VV%PLAI, YDMF_PHYS_STATE%YGSP_SG%A,  &
      & YDMF_PHYS_STATE%YGSP_SG%R, YDMF_PHYS_SURF%GSD_VV%PSAB, YDMF_PHYS_STATE%YGSP_SG%F, YDMF_PHYS_STATE%YGSP_RR%T,     &
      & YDMF_PHYS_SURF%GSD_VF%PVEG, YDMF_PHYS_STATE%YGSP_SB%Q, YDMF_PHYS_STATE%YGSP_SB%TL, YDMF_PHYS_STATE%YGSP_RR%W,    &
      & YDMF_PHYS_STATE%YGSP_RR%IC, LLHMT, YDAPLPAR_TMP%DSA%C1, YDAPLPAR_TMP%DSA%C2, ZC3, ZCG, ZCN, YDMF_PHYS%OUT%CT,    &
    ...
    IF (LMSE) THEN
      IF(LCOEFKTKE.AND.LCOEFKSURF) THEN
       ...
       IF (YDCPG_DIM%KSTEP == 0) THEN 
        ...
        & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,        &
        & YDMF_PHYS_STATE%T, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, &
      ...
      ELSE
        ...
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U,     &
        & YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, ZDPHIT, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, &
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        ...
        & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                          &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,                &
        & YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH,               &
        & ZDPHIT, ZDPHIV, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,                  &
        & YDMF_PHYS_SURF%GSD_VV%PHV, YDMF_PHYS_SURF%GSD_VF%PLSM, ZNEIJG, ZNEIJV, YDMF_PHYS_STATE%YGSP_SG%F,                      &
        & YDMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VF%PVEG, ZWFC, YDMF_PHYS_STATE%YGSP_RR%W, YDMF_PHYS_STATE%YGSP_RR%IC,    &
    ...
    IF (LPTKE) THEN
      YDMF_PHYS_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG) = MAX(YDMF_PHYS_STATE%TKE(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG),ETKE_MIN)
    ENDIF
    IF (LCOEFK_PTTE) THEN
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                    &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,         &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDAPLPAR_TMP%MSC%LSCPE, &
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      & YDCPG_DIM%KFLEVG, ZTHETAV, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%U, &
      & YDMF_PHYS_STATE%V, ZTHETAVS, YDAPLPAR_TMP%MOC%CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%VEIN,                &
      & ZUGST, ZVGST)
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      & .FALSE., YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, ZBLH, YDMF_PHYS%OUT%GZ0,       &
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, &
      & YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZLMU, ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, &
      & YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS%OUT%GZ0, YDMF_PHYS%OUT%GZ0H, ZBLH, ZZLMT, ZLMT)
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      CALL ACMIXLENZ(YDPHY, YDPHY0, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, 1, YDCPG_DIM%KFLEVG, &
      & .TRUE., YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, ZBLH, YDMF_PHYS%OUT%GZ0,      &
      & YDMF_PHYS%OUT%GZ0H, ZLMU, ZLMT)
      ZLMU2(:,:)=ZLMU(:,:)
      ZLMT2(:,:)=ZLMT(:,:)
      ...
      IF( LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, IMLTYPE, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,      &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%T, ZQV,                    &
        & ZQL, ZQI, YDMF_PHYS_STATE%R, YDMF_PHYS_STATE%S, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,          &
        & YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,   &
        & ZMN2PP, ZFMGST, YDAPLPAR_TMP%PFL%FPLSH, YDAPLPAR_TMP%PFL%FPLCH, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_STATE%YGSP_RR%T, &
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,           &
        & YDAPLPAR_TMP%MSC%LSCPE, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,                   &
        & YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0,                         &
        & ZMRIPP)
      ENDIF

      CALL ACMRIP(YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTCOEF, YDCPG_DIM%KFLEVG,                  &
      & YDCPG_DIM%KSTEP, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,              &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZQV, ZQL, ZQI, ZCP, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, &
      & YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,                            &
      & YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0,                 &
      & YDMF_PHYS_STATE%TKE, YDVARS%TTE%T0, YDMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PQSH, YDAPLPAR_TMP%DSA%RS,             &
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZCRTI(JLON) = 1.0_JPRB/(YDMF_PHYS_STATE%YGSP_RR%T(JLON)*YDAPLPAR_TMP%DSA%RS(JLON))
      ENDDO
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,         &
      & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                 &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%Q, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, &
      & ZDPHIT, ZDPHIV, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%QSATS, ZCRTI,                 &
      & YDMF_PHYS_STATE%YGSP_RR%T, LLHMT, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCE,                     &
  ...
  IF (LRAFTKE) THEN
    ...
    & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE,                &
    & ZDCAPE, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, &
...
!     ------------------------------------------------------------------
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(LCOEFKTKE) THEN
      ...
      & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,               &
      & ZFMTKE, ZFTTKE, ZAUTKE, ZATTKE, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U,       &
      & YDMF_PHYS_STATE%V, ZLMU, ZLMT, YDMF_PHYS%OUT%GZ0, ZKTROV, ZKUROV, ZKNROV, ZXTROV, ZXUROV,                   &
      & ZXPTKEROV)
    ELSE
  ...
  IF (LCVPPKF) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,          &
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, ZQV, ZQL, ZQI, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,      &
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%TKE, ZDIFCVPPQ,                 &
  ...
  IF (LEDKF) THEN
    ...
    & ZIMPL, TSPHY, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, &
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, CMF_UPDRAFT, CMF_CLOUD, LMIXUV, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,     &
    & YDMF_PHYS_STATE%T, ZQV, ZQL, ZQI, ZQR, ZQS, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,         &
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZEDMFQ, ZEDMFS, ZEDMFU, ZEDMFV, YDMF_PHYS_SURF%GSD_SFL%PGROUP(:, 1),    &
...
! Computation of the 2 znlab used in acbl89
    ...
    & YDCPG_DIM%KLON, NTCOEF, NTCOET, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,  &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,           &
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZQV,                   &
    & ZQIC, ZQLC, YDAPLPAR_TMP%MSC%LSCPE, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CH, YDMF_PHYS%OUT%GZ0,                &
    & YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDMF_PHYS_STATE%TKE,                 &
...
! input dust scalar concentration in ppp from
  ...
  CALL ARO_MNHDUST (1, ILONMNH, YDCPG_DIM%KFLEVG, NGFL_EXT, PDT, ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, 1:NGFL_EXT),                   &
  ...
  PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:)=ZZI_PEZDIAG(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:,:)
        ...
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%OUT%ALB(JLON)=(1.0_JPRB-YDAPLPAR_TMP%FLU%VEG(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & YDAPLPAR_TMP%FLU%VEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+YDAPLPAR_TMP%FLU%NEIJ(JLON)*YDMF_PHYS_STATE%YGSP_SG%A(JLON,1)
          ELSE
            ...
            YDMF_PHYS%OUT%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-YDAPLPAR_TMP%FLU%NEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & YDMF_PHYS_STATE%YGSP_SG%A(JLON,1))+(YDAPLPAR_TMP%FLU%NEIJ(JLON)-ZNEIJV(JLON))*     &
...
!DEC$ IVDEP
  ...
  & .OR.  ( (LRAY.OR.LRAYSP).AND.(.NOT.LRSTAER)) ) THEN
    ...
    IF ( ( (LRAYFM.AND.NAER /= 0) .OR.LRAY.OR.LRAYSP).AND.LLAERO )  THEN
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%T,    &
      & YDMF_PHYS_STATE%YGSP_RR%T, ZAESEA, ZAELAN, ZAESOO, ZAEDES, ZAESUL, ZAEVOL, ZAER, ZAERINDS                  &
      &                                  )
    ENDIF
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.YDCFU%NFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,YDCFU%NFRRC) == 0) THEN
      ...
      & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,         &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,     &
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,        &
      & ZQO3, YDMF_PHYS_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0,     &
      & YDGSGEOM%GEMU, YDGSGEOM%GELAM, YDMF_PHYS_TMP%RDG%MU0LU, YDMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS%OUT%FRSO, &
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                       &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,                    &
    & YDAPLPAR_TMP%MSC%LH, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQCI, ZQCL, ZQLIS, YDAPLPAR_TMP%FLU%QSAT,                   &
    & YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T,              &
    & YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDAPLPAR_TMP%DSA%CPS, YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, &
    ...
    & YDMF_PHYS%OUT%STRCV, ICIS, INLAB, INND, YDMF_PHYS_STATE%CVV)
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    & NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,  &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,             &
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%T, &
    & ZNEBCH, YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1,                    &
    & ZQLIS0, ZNEBS0)
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & INBTRA, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,                  &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDCPG_DYN0%CTY%VVEL(:, 1:),                           &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDAPLPAR_TMP%MSC%LH, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, YDMF_PHYS_STATE%Q, ZQI, ZQL, YDMF_PHYS_STATE%R, YDMF_PHYS_STATE%S,                    &
      & ZQLIS, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_TMP%RDG%CVGQ,           &
      & YDMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%TKE,                       &
      & ZTRA(:, :, INBTRA_DEP:INBTRA), ZSMOOTRAC(1:INBTRA), ZQLC, ZQIC, ZQRC, ZQSC, YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T,      &
...
!         Appel du calcul de nebulosite.
  ...
  IF(LNEBN) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZQL, ZQI, YDAPLPAR_TMP%FLU%QSAT,             &
    & YDMF_PHYS_STATE%T, YDAPLPAR_TMP%PFL%FPLCH, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZUNEBH, ZNEBS0, ZQLIS0, ZQLI_CVP, ZNEB_CVPP, ZQLI_CVPP,                         &
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    ...
    & NTCOEF, NTNEBU, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,                    &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDAPLPAR_TMP%MSC%LSCPE, ZQV,                   &
    & YDAPLPAR_TMP%FLU%QSAT, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,   &
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%GZ0, YDAPLPAR_TMP%PFL%FPLCH, &
    & YDMF_PHYS_SURF%GSD_VH%PPBLH, YDCPG_MISC%QS, YDMF_PHYS_STATE%YGSP_RR%T, ZKTROV, ZKUROV, ZNBVNO,                    &
...
!         Diagnostique de nebulosite partielle.
  ...
  & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,              &
...
!           radiation scheme
  ...
  IF ( LRAYSP .AND.(YDCPG_DIM%KSTEP == 1).AND.LRCOEF ) THEN
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
    & ZZNEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQO3, YDMF_PHYS_STATE%T, YDMF_PHYS_TMP%RDG%MMU0,                    &
    & YDCPG_MISC%DHSF, YDAPLPAR_TMP%FLU%EMIS, ZAER, POMPAC, PAC, YDAPLPAR_TMP%RDT%COR, YDAPLPAR_TMP%RDT%RAB3C,                &
...
!DEC$ IVDEP
      ...
      CASE(2)
        ...
        & NTRADI, YDCPG_DIM%KFLEVG, IJN, YDCPG_DIM%KSTEP, YDCFU%NFRRC, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,               &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,          &
        & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%NEB, ZQV, ZQCO2, YDCPG_MISC%QICE, YDCPG_MISC%QLI,             &
        & ZQO3, YDMF_PHYS_STATE%T, YDMF_PHYS%OUT%ALB, ZALBDIR, YDAPLPAR_TMP%FLU%EMIS, YDGSGEOM%GELAM,                 &
        & YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_TMP%RDG%MU0LU, YDMF_PHYS_STATE%YGSP_RR%T,                   &
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      ...
      & ZALBD, ZALBP, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDCPG_MISC%NEB,          &
      & ZQO3, ZAER, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_TMP%RDG%MU0M,             &
      & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDCPG_MISC%QICE, YDCPG_MISC%QLI, ZQS, ZQR, YDMF_PHYS_SURF%GSD_VF%PLSM,        &
      & YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%YGSP_RR%T, PGP2DSPP, PEZDIAG, YDMF_PHYS%RAD%EMTD, YDMF_PHYS%RAD%EMTU,  &
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS%RAD%EMTD, YDMF_PHYS_TMP%RDG%MU0, &
    & ZQV, ZTENT, YDMF_PHYS%RAD%TRSW, ZTRSOD, YDMF_PHYS_STATE%YGSP_RR%T, TSPHY, ZTRSODIR, ZTRSODIF,                        &
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,         &
              & YDCPG_DIM%KTDIA, KSGST, YDGSGEOM%GEMU, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_STATE%YCPG_DYN%PHIF, &
              & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO)
...
!     ----------------------
  ...
  IF (LSOLV.AND.(.NOT.LMSE)) THEN
    ...
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDMF_PHYS_STATE%T, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VF%PLSM,              &
    & YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS_SURF%GSD_VV%PLAI, YDAPLPAR_TMP%FLU%NEIJ, YDAPLPAR_TMP%FLU%VEG,              &
    & YDMF_PHYS_SURF%GSD_VV%PRSMIN, ZCHROV, ZGWDCS, ZWFC, YDMF_PHYS_STATE%YGSP_RR%FC, ZWWILT, YDMF_PHYS_STATE%YGSP_SB%Q, &
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      & NTDIFU, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,             &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS%OUT%FRSO,                           &
      & ZKTROV, ZKUROV, ZKNROV, ZQV, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U,            &
      & YDMF_PHYS_STATE%V, ZXTROV, ZXUROV, ZXPTKEROV, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, ZQL, ZQI, ZNEBDIFF, YDMF_PHYS_STATE%TKE, ZLMU, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, &
      ...
      & YDAPLPAR_TMP%FLU%QSATS, YDMF_PHYS_STATE%YGSP_SB%T, YDMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,               &
      & ZXDROV, ZXHROV, YDMF_PHYS_STATE%YGSP_RR%W, YDMF_PHYS_STATE%YGSP_RR%IC, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS,  &
      & YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTQN,        &
      & PTENDPTKE, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, &
    ...
    ELSE
      ...
      IF ( LPTKE ) THEN
        ...
        & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, &
        & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZKNROV, YDMF_PHYS_STATE%T,                &
        & YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,        &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%TKE,                           &
        & ZLMU, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZF_EPS, ZFUN_TTE, ZMRIPP, ZMRIFPP, ZRHS, ZMN2_ES,                       &
        & ZMN2_EQ, ZRRCOR, YDVARS%FQTUR%T0, YDVARS%FSTUR%T0, YDVARS%SHTUR%T0, YDAPLPAR_TMP%FLU%CD, YDMF_PHYS%OUT%GZ0,      &
        & ZKUROV, ZKTROV, ZXPTKEROV, ZLMLTILD, YDVARS%MXL%T0, ZLML, ZTH_FUN, ZWW_FUN, PTENDPTKE, YDVARS%TTE%T0,            &
        & PFTCNS)
      ENDIF

      CALL ACDIFV1 ( YGFL, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA,                 &
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, ZCP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & ZKTROV, ZKQROV, ZKUROV, ZQV, ZQL, ZQI, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T,                              &
      & YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZSVM, ZXTROV, ZXUROV, ZXDROV, ZXHROV, ZEDMFS, ZEDMFQ,                                &
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        & LSURFEX_KFROM, LMPA, CCOUPLING, LDXFUMSE, NINDAT, ZRHGMT, ZSTATI, RSOVR, RCODEC, RSIDEC, YDCSGEOM%RINDX(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),                                     &
        & YDCSGEOM%RINDY(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                       &
        & YDMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), YDMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), &
        & YDMF_PHYS_STATE%Q(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG), ZSVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG, 1:NGFL_EXT),  &
        & ZCARDI, ZRHODREFM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_STATE%YCPG_PHY%PREHYDF(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                     &
        & YDMF_PHYS_STATE%YCPG_PHY%PREHYD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG:YDCPG_DIM%KFLEVG),                                                                          &
...
! Opposite water vapor flux
        ...
        & ZFEVS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA), YDMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),                                     &
        & YDMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG), ZDEPTH_HEIGHT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, 1:YDCPG_DIM%KFLEVG),         &
...
! calculation of variables for the old "ISBA" atmosphere scheme
        ...
        IF (LDIFCONS.AND..NOT.LNODIFQC) THEN
          ...
          & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, ZCOEFN, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,        &
          & ZQL, ZQI, YDMF_PHYS_STATE%T, ZALPHA1, ZCOEFA, ZLVT, ZQICE)
        ENDIF
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YSP_SBD%NLEVS, YDMF_PHYS_STATE%YCPG_DYN%PHI, ZCP, YDMF_PHYS%OUT%FRSO,    &
          & ZQV, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,           &
          & ZXURO, ZXQRO, ZXTRO, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, ZQL, ZQI, ZNEBDIFF, YDAPLPAR_TMP%FLU%CD,              &
          & YDAPLPAR_TMP%FLU%CDN, ZCDROV, ZCHROV, ZCEROV, YDAPLPAR_TMP%DSA%CPS, YDMF_PHYS%OUT%CT,                       &
          & ZDQSTS, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VH%PSPSH, ZHQ, ZHTR, ZHU, YDMF_PHYS_SURF%GSD_VV%PHV,      &
          & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDAPLPAR_TMP%FLU%NEIJ, YDCPG_MISC%QS,              &
          & YDAPLPAR_TMP%FLU%QSATS, YDMF_PHYS_STATE%YGSP_SB%T, YDMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%VEG,         &
          & ZXDROV, ZXHROV, YDMF_PHYS_STATE%YGSP_RR%W, YDMF_PHYS_STATE%YGSP_RR%IC, ZDSE, ZCFAS,                         &
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, ZCFAQ, ZCFAS, ZCFAU, ZCFASV, ZCFBQ, ZCFBS,              &
      & ZCFBU, ZCFBV, ZCFBSV, ZKTROV, ZKQROV, ZKQLROV, ZKUROV, ZDSE, ZQT, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,    &
      & ZPOID, YDMF_PHYS_STATE%T, ZQL, ZQI, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZCOEFA, ZALPHA1,                   &
      & ZLVT, ZQICE, ZSFSV, YDMF_PHYS%OUT%FCS, ZFEV, ZFMDU, ZFMDV, ZTSN, ZXHROV, PDIFSV, YDMF_PHYS%OUT%DIFTQ,      &
      ...
      IF (L3DTURB) THEN
        ...
        & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%TKE, PTENDPTKE, YDMF_PHYS_STATE%YCPG_DYN%PHI,           &
        & YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, &
        & YDMF_PHYS_STATE%DIV, YDMF_PHYS_STATE%VOR, YDVARS%U%DL, YDVARS%V%DL, YDMF_PHYS_STATE%YCPG_PHY%W,                            &
        & YDMF_PHYS_STATE%YCPG_PHY%WL, YDMF_PHYS_STATE%YCPG_PHY%WM, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                                 &
        & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZLML, ZFHORM, ZFHORH,                                   &
      ...
      IF (LCOEFKTKE) THEN
        ...
        & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_DYN%PHI,                      &
        & YDMF_PHYS_STATE%YCPG_DYN%PHIF, ZCP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZKTROV, ZXTROV, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, &
        & YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZCHROV, ZXHROV, ZCOEFA, ZQV, ZQL, ZQI, YDMF_PHYS_STATE%U,                          &
        & YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,            &
        & ZLML, ZTH_FUN, ZWW_FUN, ZF_EPS, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS_STATE%TKE,                         &
        & PTENDPTKE, ZMN2_ES, ZMN2_EQ, ZMN2_DS, ZMN2_DQ, YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTQN,                             &
...
!DEC$ IVDEP
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,             &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V,              &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, &
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      DO JLON = YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        ZPREN(JLON)=YDMF_PHYS_STATE%YCPG_PHY%PREHYD(JLON,YDCPG_DIM%KFLEVG)
      ENDDO
      ...
      & YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, ZPREN, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDMF_PHYS%OUT%FCS, &
...
!DEC$ IVDEP
  DO JLON=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,YDCPG_DIM%KFLEVG,1)/YDAPLPAR_TMP%FLU%EMIS(JLON)+RSIGMA*YDMF_PHYS_STATE%YGSP_RR%T(JLON)**4
...
! HEAT FLUX WITH RESPECT TO SURFACE TEMPERATURE (PDERNSHF)
  ...
  IF(LMCC03)THEN
    ...
    & YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, ZQV, YDCPG_MISC%QS, YDMF_PHYS_STATE%YGSP_RR%T, &
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,        &
    & ZNBVNO, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDGSGEOM%RCORI,            &
 ...
 ! SAVE FOR TL/NL COEFS FROM VERT. DIFF AND GWD
  ...
  IF (LTRAJPS.AND.(LVDIFSPNL.OR.LGWDSPNL)) THEN
     ...
     & ZKTROV_SAVE, ZKUROV_SAVE, ZCDROV_SAVE, ZCHROV_SAVE, ZTRAJGWD, YDMF_PHYS_STATE%L, YDMF_PHYS_STATE%I, &
     & YDMF_PHYS_STATE%R, YDMF_PHYS_STATE%S, ZQLIS, ZNEBS)
  ENDIF
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%T, &
  ...
  IF ( LSTRAS ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZNEBS, ZQV, ZQLIS, YDAPLPAR_TMP%MSC%QW,      &
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FPLSL, &
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZQV, ZT, ZGEOSLC, ZLH,                &
      & ZLSCPE, ZQSAT, ZQW, ZRH, ZTW)
      CALL ACNEBCOND ( YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                  &
      & NTPLUI, YDCPG_DIM%KFLEVG, LLREDPR, ZHUC, ZVETAF, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,            &
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDCPG_MISC%RH, ZBLH, ZQV, ZQI, ZQL, ZQW, ZT, ZNEBCH, YDGSGEOM%GM,          &
      & YDMF_PHYS_STATE%YGSP_RR%T, ZQLIS, ZNEBS, ZRHCRI, ZRH, ZQSATS, ZICEFR1, ZQLIS0, ZNEBS0)
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NTPLUI, YDCPG_DIM%KFLEVG, &
      & ZVETAF, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,     &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                 &
      & ZQG, ZQW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, ZT, YDAPLPAR_TMP%MSC%TW, ZNEBS, ZRHCRI, ZICEFR1,                   &
      & ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP, ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM,   &
      & YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, ZDECRD_MF, YDMF_PHYS%OUT%FPFPSL,              &
    ...
    ELSE
      ...
      & ZVETAF, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,       &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, ZQV, ZQI, ZQL, ZQS, ZQR,                   &
      & ZQG, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW,              &
      & ZNEBS, ZRHCRI, ZICEFR1, ZQSATS, ZNEBCH, ZRHDFDA, ZRKTTEND, ZRKQVTEND, ZRKQCTEND, ZQLI_CVPP,                    &
      & ZNEB_CVPP, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T,          &
...
!cdir unroll=8
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      ...
      & YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI,         &
      & YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,                           &
      & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQSAT, ZQW, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,                           &
      & ZT, ZTW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%VOR, YDVARS%DAL%T0, YDMF_PHYS_STATE%YGSP_RR%T,               &
    ...
    ! Initialize Environment horizontal velocity
      ZU(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YDMF_PHYS_STATE%U(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
      ZV(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)=YDMF_PHYS_STATE%V(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,:)
    ELSE
...
! PASS T9 CONDENSATES TO ACCSU
      ...
      & YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, &
      & YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                           &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R,                      &
      & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                           &
      & ZQV, ZTCORR, YDMF_PHYS_STATE%I, YDMF_PHYS_STATE%L, ZQSAT, ZQW, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,                      &
      & ZT, ZTW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%VOR, YDCPG_DYN0%CTY%VVEL(:, 1:),                         &
      & YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT%FCSQN, YDMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI,                             &
    ...
    & YDVARS%UAL%T0, YDVARS%UNEBH%T0, ZDETFI, ZPOID, ZIPOI, ZLHV, ZLHS, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,           &
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, &
    & ZQV, ZQI, ZQL, ZQS, ZQR, ZQG, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, ZT, ZIPOI, ZDQ, ZDQM, ZLHS, ZLHV,                       &
    & ZNEBS, ZPOID, ZRCVOTT, ZTCORR, YDMF_PHYS_SURF%GSD_VF%PLSM, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_STATE%YGSP_RR%T,          &
   ...
   & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZLHS, ZLHV, YDCPG_DYN0%CTY%EVEL, ZIPOI,                        &
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,  &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,               &
      & ZQV, ZQI, ZQL, ZQR, ZQS, ZQW, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, ZSIGP, ZT, ZTW, ZU, ZV, YDCPG_DYN0%CTY%VVEL(:, 1:), &
    ...
    ELSE
      ...
      & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,       &
      & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,                      &
      & YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR, ZQV, ZQI, ZQL, ZQW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
  ...
  !  ---------------------------------------------
    ...
    & YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, ZLHS,                         &
    & ZLHV, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, ZIPOI, ZPOID, ZFALLR, ZFALLS, ZFALLG, YDMF_PHYS%OUT%FPEVPSL,                 &
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%XYB%ALPH, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,                      &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,                  &
    & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                        &
    & ZQV, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YDMF_PHYS_STATE%T, YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%VOR,                   &
    & YDCPG_DYN0%CTY%VVEL(:, 1:), YDGSGEOM%GM, YDMF_PHYS_STATE%YGSP_RR%T, ZTAUX, YDGSGEOM%RCORI, YDMF_PHYS%OUT%DIFCQ,      &
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP,                &
    & YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%XYB%LNPR,                      &
    & ZQV, YDAPLPAR_TMP%MSC%QW, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T,   &
    & YDAPLPAR_TMP%MSC%TW, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VH%PPBLH, &
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    & YDMF_PHYS%OUT%FPEVPCN, YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP,     &
    & YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%Q, YDMF_PHYS_STATE%YGSP_RR%T, ZTENHA, ZTENQVA )
    ...
    & LLDSLPHY, TSPHY, ZVDIFTS, YDMF_PHYS_STATE%T, ZQV, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, ZLISUM,                       &
    & ZVERVEL, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, &
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, ZGEOM1, ZGEOMH, YDGSGEOM%GM, ZCUCONVCA, ZGP2DSPPA, ZTENT,                             &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    & ZGAW, ZGAW, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_DYN%PHI,  &
    & YDMF_PHYS_STATE%YCPG_DYN%PHIF, LLLAND, YDMF_PHYS_STATE%T, ZLU, ZMFU, YDMF_PHYS%OUT%CAPE, YDMF_PHYS%OUT%FPLCL, &
  ...
  IF ( LADJCLD ) THEN
    ...
    & YDMF_PHYS%OUT%FPFPCL, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%T,              &
    & YDMF_PHYS_STATE%Q, YDMF_PHYS_STATE%YGSP_RR%T, ZTENHA, ZTENQVA )
  ENDIF
  ...
  IF ( LPROCLD ) THEN
    IF(LGPCMT) THEN
      ...
      ZTMIC(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)=YDMF_PHYS_STATE%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YDCPG_DIM%KTDIA:YDCPG_DIM%KFLEVG)
    ...
    & ZTMIC, ZQMIC, ZQL, ZQI, ZQR, ZQS, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,            &
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, ZNEBS, ZQLIS, ZNEB_CVPP,                         &
    & ZQLI_CVPP, ZQC_DET_PCMT, ZTENHA, ZTENQVA, LADJCLD, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YGSP_RR%T,       &
  ...
  IF ( LGWDC ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_PHY%PREHYD, ZNBVNO, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_STATE%U,                        &
    & YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH, &
  ...
  IF ( LNORGWD ) THEN
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        Z_PP(JLON, JLEV) = YDMF_PHYS_STATE%YCPG_PHY%PREHYDF(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_UU(JLON, JLEV) = YDMF_PHYS_STATE%U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VV(JLON, JLEV) = YDMF_PHYS_STATE%V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_TT(JLON, JLEV) = YDMF_PHYS_STATE%T(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        Z_VO(JLON, JLEV) = YDMF_PHYS_STATE%VOR(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_U(JLON, JLEV) = TSPHY * YDMF_PHYS_STATE%P1NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
        ZD_V(JLON, JLEV) = TSPHY * YDMF_PHYS_STATE%P2NOGW(JLON, YDCPG_DIM%KFLEVG - JLEV + 1)
      ENDDO
    ENDDO
    ...
    DO JLON = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      DO JLEV = 1, YDCPG_DIM%KFLEVG
        YDMF_PHYS_STATE%P1NOGW(JLON, JLEV) = ZD_U(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
        YDMF_PHYS_STATE%P2NOGW(JLON, JLEV) = ZD_V(JLON, YDCPG_DIM%KFLEVG - JLEV + 1) / TSPHY
      ENDDO
    ENDDO
...
!         SAUVEGARDE DES FLUX DE PRECIPITATION CONVECTIVE ET STRATIFORME.
  ...
  IF ( LCONDWT.AND.LPROCLD.AND..NOT.LGPCMT ) THEN
    ...
    & ZQV, ZQL, ZQI, YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL, &
...
!     ---------------------------
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      ...
      & ZWFC, ZWPMX, YDMF_PHYS_STATE%YGSP_RR%FC, ZWLMX, ZWSEQ, ZWSMX, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL,               &
      & YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN,                &
      & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_STATE%YGSP_SG%F, YDMF_PHYS_STATE%YGSP_SB%T, YDMF_PHYS_STATE%YGSP_RR%T,         &
      & YDMF_PHYS_STATE%YGSP_SB%Q, YDMF_PHYS_STATE%YGSP_SB%TL, YDMF_PHYS_STATE%YGSP_RR%W, YDMF_PHYS_STATE%YGSP_RR%IC,        &
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    ...
    & YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_STATE%T, ZQV, YDMF_PHYS_STATE%U, &
    & YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%FRMH, YDAPLPAR_TMP%MSC%FRMQ, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV,        &
...
!     ------------------------------------------------------------------
  IF ( LOZONE ) THEN
    ...
    & YDMODEL%YRML_PHY_G%YRDPHY%NVCLIS, YDMF_PHYS_STATE%YCPG_PHY%PREHYD, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP, &
    & PKOZO, ZQO3(1, 1), YDMF_PHYS_STATE%T, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS%OUT%FCHOZ)
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,               &
    & YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FRSO, ZKTROV, ZQO3(1, 1), YDMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, &
    & YDMF_PHYS_STATE%T, ZXTROV, YDAPLPAR_TMP%FLU%NEIJ, YDGSGEOM%GEMU, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS%OUT%FCHOZ     &
    & )
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     ------------------------------------------------------------------
  ...
  IF (LPHSPSH) THEN
    DO JLON=YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
       YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=(YDMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YDMF_PHYS_STATE%YGSP_RR%T(JLON)) * ((&
    ...
    ENDDO
    IF (LGRAPRO) THEN
      YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)+(YDMF_PHYS_STATE%T(JLON,YDCPG_DIM%KFLEVG)-YDMF_PHYS_STATE%YGSP_RR%T(JLON)) * (&
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)*RTINER*YDMF_PHYS_STATE%YGSP_SB%T(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,YSP_SBD%NLEVS)/YDMF_PHYS%OUT%CT(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA)
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     & YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS_STATE%YCPG_PHY%PREHYD,                      &
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_STATE%YCPG_DYN%PHIF, &
     & YDMF_PHYS_STATE%YCPG_DYN%PHI, ZDCAPE)
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU, YDPHY, YDPHY2, YDTOPH, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
     & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE,            &
     & ZDCAPE, ZTKE1, YDMF_PHYS_STATE%YCPG_DYN%PHIF, YDOROG%OROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,           &
     & ZBLH, YDAPLPAR_TMP%MOC%CLPH)
  ...
  IF (LXVISI.OR.LXVISI2) THEN
     ...
     & YDCPG_DIM%KTDIA, YDCPG_DIM%KFLEVG, YDMF_PHYS_STATE%YCPG_DYN%PHI, YDMF_PHYS_STATE%YCPG_DYN%PHIF,      &
     & YDMF_PHYS_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_STATE%T, YDMF_PHYS_STATE%YCPG_DYN%RCP%R, YDCPG_MISC%QLI, &
...
ZZDEP=0.0_JPRB
  ...
  ENDDO
    ...
    CALL ARO_WETDEP(ILONMNH, YDCPG_DIM%KFLEVG, NGFL_EXT, IKRR, PDT, ZZI_SVM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA, :, :, 1:NGFL_EXT), &
...
CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, ZPCLS, YDMF_PHYS%OUT%TCLS, YDMF_PHYS_STATE%Q(:, YDCPG_DIM%KFLEVG), &
& YDMF_PHYS_STATE%L(:, YDCPG_DIM%KFLEVG), YDMF_PHYS_STATE%I(:, YDCPG_DIM%KFLEVG), YDMF_PHYS%OUT%TPWCLS                                      &
& )
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    CALL PPWETPOINT(YDPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDMF_PHYS_STATE%YCPG_PHY%PREHYDF(:, JLEV), &
    & YDMF_PHYS_STATE%T(:, JLEV), YDMF_PHYS_STATE%Q(:, JLEV), YDMF_PHYS_STATE%L(:, JLEV), YDMF_PHYS_STATE%I(:, JLEV),   &
    & ZTW(:, JLEV))
  ENDDO
  ...
  IF (LDPRECIPS) THEN
   ...
   & YDOROG%OROG, YDMF_PHYS%OUT%TPWCLS, YDMF_PHYS%OUT%DIAGH, YDMF_PHYS_STATE%YCPG_DYN%PHIF, ZDZZ,                 &
   & ZTW, YDMF_PHYS_STATE%L, ZFPLC(:, YDCPG_DIM%KFLEVG), ZFPLS(:, YDCPG_DIM%KFLEVG), ZFPLSG(:, YDCPG_DIM%KFLEVG), &
