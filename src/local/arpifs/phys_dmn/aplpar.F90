!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDCPG_OPT,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV,YDMODEL,     &
       ...
       &  PGELAM, PGEMU, PGPAR, PMU0LU , YDMF_PHYS%OUT%ALB, PVRMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
    ...
    IF (LRAYLU)  YDMF_PHYS%OUT%FRSOLU(KIDIA:KFDIA)=PVRMOON(KIDIA:KFDIA)
