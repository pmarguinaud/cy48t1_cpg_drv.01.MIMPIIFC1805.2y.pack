...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      CALL ACDIFV2 (LSFORCS, YDMODEL%YRCST,  YGFL, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,&
...
! -----------------------------------------------------
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    CALL CUCALLN_MF (RPLDARE, RPLRG, YDCPG_OPTS%KSTEP, YDMODEL%YRML_PHY_EC%YRTHF, YDMODEL%YRCST, YDMODEL%YRML_PHY_RAD%YRERAD, &
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    ZGAW(:)=0._JPRB
    CALL CULIGHT (RPLDARE, RPLRG, YDMODEL%YRML_PHY_EC%YRTHF, YDMODEL%YRCST, YDEPHY, YGFL,                                         &
