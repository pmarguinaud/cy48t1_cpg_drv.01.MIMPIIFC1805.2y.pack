!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU,YDCFU,YDMODEL,KIDIA , KFDIA  , KLON      ,&
 & KTDIA  , KLEV   , KSTGLO ,&
 & KVCLIS , KVCLIV , KSTEP  ,&
 & KSGST  , KCSS   ,&
...
!cdir unroll=8
  ...
  !                         FOR RAIN/SNOW/GRAUPEL PUT IN PFCQR/S/GNG
          ...
          ZQV0=PQ(JLON,JLEV)-ZIPOI(JLON,JLEV)*(0.0_JPRB- YDMF_PHYS%FCQNG(JLON,JLEV-1) &
          & -YDMF_PHYS%FCQNNG(JLON,JLEV-1)-YDMF_PHYS%FCQLNG(JLON,JLEV-1)-YDMF_PHYS%FCQRNG(JLON,JLEV-1) &
          & -YDMF_PHYS%FCQSNG(JLON,JLEV-1)-YDMF_PHYS%FCQGNG(JLON,JLEV-1))
          ZQV(JLON,JLEV)=MAX(0.0_JPRB,ZQV0-ZDQC)
          ZDQV=MAX(0.0_JPRB,ZQV0-ZDQC)-PQ(JLON,JLEV)

          YDMF_PHYS%FCQNG(JLON,JLEV)=YDMF_PHYS%FCQNG(JLON,JLEV-1)-ZDQV*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQNNG(JLON,JLEV)=YDMF_PHYS%FCQNNG(JLON,JLEV-1)-ZDQI*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQLNG(JLON,JLEV)=YDMF_PHYS%FCQLNG(JLON,JLEV-1)-ZDQL*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQRNG(JLON,JLEV)=YDMF_PHYS%FCQRNG(JLON,JLEV-1)-ZDQR*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQSNG(JLON,JLEV)=YDMF_PHYS%FCQSNG(JLON,JLEV-1)-ZDQS*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQGNG(JLON,JLEV)=YDMF_PHYS%FCQGNG(JLON,JLEV-1)-ZDQG*ZPOID(JLON,JLEV)
        ENDDO
      ENDDO
    ELSE
      DO JLEV = 1, KLEV
    ...
    IF (LGPCMT) THEN
      DO JLEV = 1, KLEV
        DO JLON = KIDIA,KFDIA
          ...
          & -YDMF_PHYS%FCNEGQIC(JLON,JLEV-1)-YDMF_PHYS%FCNEGQLC(JLON,JLEV-1)&
          & -YDMF_PHYS%FCNEGQRC(JLON,JLEV-1)-YDMF_PHYS%FCNEGQSC(JLON,JLEV-1))
          ZQVI=MAX(0.0_JPRB,ZQV0-ZDQC)
          ZDQV=ZQVI-ZQV(JLON,JLEV)
          ZQV(JLON,JLEV)=ZQVI

          ZFCQVNG(JLON,JLEV)=ZFCQVNG(JLON,JLEV-1)-ZDQV*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCQNG(JLON,JLEV)=YDMF_PHYS%FCQNG(JLON,JLEV)+ZFCQVNG(JLON,JLEV)
          YDMF_PHYS%FCNEGQIC(JLON,JLEV)=YDMF_PHYS%FCNEGQIC(JLON,JLEV-1)-ZDQI*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCNEGQLC(JLON,JLEV)=YDMF_PHYS%FCNEGQLC(JLON,JLEV-1)-ZDQL*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCNEGQRC(JLON,JLEV)=YDMF_PHYS%FCNEGQRC(JLON,JLEV-1)-ZDQR*ZPOID(JLON,JLEV)
          YDMF_PHYS%FCNEGQSC(JLON,JLEV)=YDMF_PHYS%FCNEGQSC(JLON,JLEV-1)-ZDQS*ZPOID(JLON,JLEV)
        ENDDO
      ENDDO
      ZFCQVNG(:,:)=0.0_JPRB
    ENDIF
...
!     -------------------------------------------
  ...
  IF (LSFORCS) THEN  ! Surface forcing for 1D model MUSC
     ...
     CALL SURF_IDEAL_FLUX(YDRIP,YDPHY0,YDPHYDS,LLAROME,KIDIA,KFDIA,KLON,PAPHIF(:,KLEV),ZRHODREFM,YDMF_PHYS_SURF%GSD_SFO%PGROUP,&
        & ZTN,ZTSN,YDMF_PHYS_SURF%GSD_VF%PLSM,PQ(:,KLEV),PU(:,KLEV),PV(:,KLEV),ZTHETAS,YDMF_PHYS%FCS(KIDIA:KFDIA,1),ZFEV,ZFMDU,ZFMDV)
     DO JLON=KIDIA,KFDIA
        PTS(JLON)=ZTSN(JLON)
     ENDDO
     DO JLON=KIDIA,KFDIA
        ...
        YDMF_PHYS%FEVL(JLON,1)=ZFEV(JLON)*(1.0_JPRB-ZDELTA)
        YDMF_PHYS%FEVN(JLON,1)=ZFEV(JLON)*ZDELTA
        YDMF_PHYS%FCLL(JLON,1)=YDMF_PHYS%FEVL(JLON,1)*PLH(JLON,KLEV)
        YDMF_PHYS%FCLN(JLON,1)=YDMF_PHYS%FEVN(JLON,1)*PLH(JLON,KLEV)
  ...
  IF ( .NOT.LMSE ) THEN
    IF ( LSOLV ) THEN
      ...
      CALL ACSOL ( YDPHY,YDPHY1,KIDIA,KFDIA,KLON,&
       ...
       & LLHMT,&
       & PC1,PC2,ZC3,ZCG,ZCN,YDMF_PHYS%CT,ZNEIJG,ZNEIJV,&
...
!     VARIABLES PSEUDO-HISTORIQUES ASSOCIEES
    ...
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%GZ0     (JLON) = PGPAR(JLON,MGZ0)
      YDMF_PHYS%GZ0H    (JLON) = PGPAR(JLON,MGZ0H)
    ...
    IF ( LRAY ) THEN
      ...
      DO JLON=KIDIA,KFDIA
        YDMF_PHYS%ALB   (JLON) = PGPAR(JLON,MALBSCA)
    ...
    ELSE
      ...
      ENDDO
      DO JLON=KIDIA,KFDIA
        YDMF_PHYS%ALB(JLON)=0.0_JPRB
      ENDDO
      DO JSG=1,NSW
        DO JLON=KIDIA,KFDIA
          YDMF_PHYS%ALB(JLON)=YDMF_PHYS%ALB(JLON)+0.5*(ZALBP(JLON,JSG)+ZALBD(JLON,JSG))
        ENDDO
      ENDDO
      DO JLON=KIDIA,KFDIA
        YDMF_PHYS%ALB(JLON)=YDMF_PHYS%ALB(JLON)/REAL(NSW,JPRB)
      ENDDO
    ENDIF
...
!
  ...
  CALL ARO_GROUND_DIAG_Z0( KBL, KGPCOMP,&
          ...
          & LSURFEX_KFROM,&
          & YDMF_PHYS%GZ0(KIDIA:KFDIA), YDMF_PHYS%GZ0H(KIDIA:KFDIA))
  ENDIF
!*
!     ------------------------------------------------------------------
...
!       Computation of ZRTI
    ...
    ELSE
      IF (LCOEFKSURF) THEN
        CALL ACTKEHMT ( KIDIA,KFDIA,KLON,KLEV,KVCLIV>=8.AND.LSOLV,&
         ...
         & YDMF_PHYS%GZ0, YDMF_PHYS%GZ0H, ZHQ, ZHU, PNEIJ, YDMF_PHYS%QCLS, PQS, PQSATS, YDMF_PHYS%RHCLS,&
         & PRS, ZRTI, ZSTAB, YDMF_PHYS%TCLS, YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, YDMF_PHYS%NUCLS, YDMF_PHYS%NVCLS, ZPCLS, PVEG,&
         & ZXDROV, ZXHROV,YDMF_PHYS%UGST,YDMF_PHYS%VGST)
      ELSE
    ...
    IF(LCOEFK_RIS .AND. LCOEFKTKE) THEN
      ...
      CALL ACMRISS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       ...
       & PR, PT, PU, PV, PLSCPE,YDMF_PHYS%GZ0,&
       & ZMN2PP,ZMRIPP)
    ENDIF
    ...
    IF (CGMIXLEN == 'Z'  .OR. &
     ...
     &  CGMIXLEN == 'AYC'.AND.(.NOT.LECT)) THEN
      ...
      CALL ACCLPH ( YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & KCLPH,YDMF_PHYS%CLPH,YDMF_PHYS%VEIN,ZUGST,ZVGST)
      ZBLH(KIDIA:KFDIA)=YDMF_PHYS%CLPH(KIDIA:KFDIA)
      IF (.NOT.LRAFTUR) THEN
        YDMF_PHYS%UGST(KIDIA:KFDIA)=ZUGST(KIDIA:KFDIA)
        YDMF_PHYS%VGST(KIDIA:KFDIA)=ZVGST(KIDIA:KFDIA)
      ENDIF
    ELSE
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    IF(TRIM(CGMIXLEN) == 'Z') THEN
      ...
      CALL ACMIXLENZ ( YDPHY,YDPHY0,KIDIA,KFDIA,KLON,1,KLEV,.FALSE.,&
       & PAPHI,PAPHIF,ZBLH,YDMF_PHYS%GZ0,YDMF_PHYS%GZ0H,ZLMU,ZLMT)
    ...
    ELSEIF((TRIM(CGMIXLEN) == 'TMC').OR.(TRIM(CGMIXLEN) == 'AYC')) THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV, &
       & PAPHI,PAPHIF,YDMF_PHYS%GZ0,YDMF_PHYS%GZ0H,ZBLH,ZLMU,ZLMT)
    ...
    ELSEIF(TRIM(CGMIXLEN) == 'TM') THEN
      ...
      CALL ACMIXLENTM ( YDPHY0,KIDIA,KFDIA,KLON,KLEV,&
       & PAPHI,PAPHIF,YDMF_PHYS%GZ0,YDMF_PHYS%GZ0H,ZBLH,ZZLMT,ZLMT)
    ...
    ELSEIF((CGMIXLEN(1:2) == 'EL').AND.LPTKE) THEN
      ...
      IF( LCOEFK_RIS) THEN
        ...
        CALL ACMIXELEN(YGFL,YDPHY,YDPHY0,&
         ...
         & YDMF_PHYS%GZ0,PTS,PCDN,ZBLH,ZLMU,ZLMT,YDVARS%MXL%T0,ZLML,ZLMLTILD,ZRRCOR,LLMAF)
      ENDIF
    ...
    IF(LCOEFKTKE) THEN
      ...
      IF(LCOEFK_RIS) THEN
        CALL ACMRIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
         ...
         & ZQV, ZQL, ZQI, PQSAT,&
         & PR, PT, PU, PV, ZLMU, ZLMT,YDMF_PHYS%GZ0,&
         & ZMRIPP)
      ENDIF
      ...
      CALL ACMRIP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,KSTEP,&
       ...
       & PTKE,YDVARS%TTE%T0,&
       & PTS,YDMF_PHYS_SURF%GSD_VH%PQSH,PRS,PCPS,ZRTI,YDMF_PHYS%GZ0,&
       & LLCLS,&
       & ZMRIPP,ZMRIFPP,&
  ...
  IF ( LVDIF.OR.LHMTO.OR.LGWD ) THEN
    ...
    IF (LFLUSO.AND.(.NOT.LMSE)) THEN
      ...
      CALL ACFLUSO ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,&
       ...
       & ZDPHIT,ZDPHIV,YDMF_PHYS%GZ0,YDMF_PHYS_SURF%GSD_VF%PLSM,PQSATS,ZCRTI,PTS,&
       & LLHMT,PCD,PCDN,ZCDROV,ZCE,ZCEROV,PCH,ZCHROV,&
       & YDMF_PHYS%QCLS,YDMF_PHYS%RHCLS,YDMF_PHYS%TCLS,YDMF_PHYS%UCLS,YDMF_PHYS%VCLS,YDMF_PHYS%UGST,YDMF_PHYS%VGST)
    ELSE
      DO JLON=KIDIA,KFDIA
        ZCE   (JLON) = PCH   (JLON)
  ...
  IF (LRAFTKE) THEN
    YDMF_PHYS%CAPE(:)=0._JPRB
    ZDCAPE(:)=0._JPRB
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, PU, PV, YDMF_PHYS%CAPE, ZDCAPE, PTKE, PAPHIF, POROG, YDMF_PHYS%UGST, YDMF_PHYS%VGST,ZBLH,KCLPH)
  ENDIF
  ...
  IF ( (LVDIF.OR.LGWD).AND.(.NOT.(LNEBR.OR.LECT)) ) THEN
    ...
    ELSE
      CALL ACCOEFK ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,KLEV,&
       ...
       & PR, PT, PU, PV, PFPLSH, PFPLCH, ZLMU, ZLMT, YDMF_PHYS%GZ0, YDMF_PHYS%GZ0H, ZBLH,&
...
!DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      IF (LSNV) THEN
        ...
        ENDIF
        YDMF_PHYS%ALB(JLON)= (1.0_JPRB-YDMF_PHYS_SURF%GSD_VF%PVEG(JLON))*(1.0_JPRB-ZNEIJG(JLON)) *&
      ...
      ELSE
        IF (LVGSN) THEN
          IF (LZ0HSREL.AND.LCOEFKSURF) THEN
            ...
            YDMF_PHYS%ALB(JLON)=(1.0_JPRB-PVEG(JLON)-PNEIJ(JLON))*YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)+ &
             & PVEG(JLON)*YDMF_PHYS_SURF%GSD_VV%PALV(JLON)+PNEIJ(JLON)*PALBNS(JLON)
          ELSE
            ! old treatment, PNEIJ is snow fraction for bare ground
            YDMF_PHYS%ALB(JLON)=YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)-PNEIJ(JLON)*(YDMF_PHYS_SURF%GSD_VF%PALBF(JLON)- &
             & PALBNS(JLON))+(PNEIJ(JLON)-ZNEIJV(JLON))*     &
             & YDMF_PHYS_SURF%GSD_VF%PVEG(JLON)*(YDMF_PHYS_SURF%GSD_VV%PALV(JLON)-PALBNS(JLON))
          ENDIF

          YDMF_PHYS%ALB(JLON)=MIN(ABS(YDMF_PHYS_SURF%GSD_VV%PIVEG(JLON)-2._JPRB),1.0_JPRB) * YDMF_PHYS%ALB(JLON) +(&
           & 1.0_JPRB-MIN(ABS(YDMF_PHYS_SURF%GSD_VV%PIVEG(JLON)-2._JPRB),1.0_JPRB))&
           & * MAX(ALCRIN,YDMF_PHYS%ALB(JLON))
          YDMF_PHYS_SURF%GSP_SG%PT_T1(JLON,1)=YDMF_PHYS%ALB(JLON)
    ...
    IF (LRAYFM) THEN
      ...
      IF (LALBMERCLIM) THEN
        DO JSG=1,NSW
          DO JLON=KIDIA,KFDIA
            ZALBD(JLON,JSG)=YDMF_PHYS%ALB(JLON)
            ZALBPMER=(1.0_JPRB+&
             & 0.5_JPRB*PMU0M(JLON)*(1.0_JPRB/YDMF_PHYS%ALB(JLON)-1.0_JPRB))/   (&
             & 1.0_JPRB+PMU0M(JLON)*(1.0_JPRB/YDMF_PHYS%ALB(JLON)-1.0_JPRB))**2
            ZALBP(JLON,JSG)=YDMF_PHYS%ALB(JLON)*          YDMF_PHYS_SURF%GSD_VF%PLSM(JLON)+&
...
!DEC$ IVDEP
        ...
        DO JSG=1,NSW
          DO JLON=KIDIA,KFDIA
            ZALBD(JLON,JSG)=(1.0_JPRB-ZMERL(JLON))*YDMF_PHYS%ALB(JLON)+&
                                    & ZMERL(JLON) *ALBMED
            ZALBP(JLON,JSG)=(1.0_JPRB-ZMERL(JLON))*YDMF_PHYS%ALB(JLON)+&
             & ZMERL(JLON) *&
    ...
    ELSEIF (LRAY) THEN
      ...
      DO JLON=KIDIA,KFDIA
        ...
        ENDIF
        ZALBDIR(JLON)=ZLAMB*YDMF_PHYS%ALB(JLON)+(1._JPRB-ZLAMB)*(1._JPRB+&
         & 0.5_JPRB*PMU0(JLON)*(1.0_JPRB/YDMF_PHYS%ALB(JLON)-1.0_JPRB))/   (&
         & 1.0_JPRB+PMU0(JLON)*(1.0_JPRB/YDMF_PHYS%ALB(JLON)-1.0_JPRB))**2
      ENDDO
    ENDIF
  ...
  IF (LRAY.AND.NRAY == 1.AND.KNFRRC /= 0) THEN
    IF (MOD(KSTEP,KNFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       ...
       & YDMF_PHYS%ALB,ZALBDIR,PEMIS,PMU0,PGEMU,PGELAM,PMU0LU,PTS,&
       & YDMF_PHYS%FRSO,YDMF_PHYS%FRTH,&
       & ZFRSODS,YDMF_PHYS%FRSOPS,YDMF_PHYS%FRSOLU,YDMF_PHYS%FRTHDS,ZAER,&
       & ZMAK,ZMAN)
      DO JLON=KIDIA,KFDIA
        YDMF_PHYS%FRSOC(JLON,0)=YDMF_PHYS%FRSO(JLON,NTRADI-1,1)
        YDMF_PHYS%FRSOC(JLON,1)=YDMF_PHYS%FRSO(JLON,KLEV,1)
        YDMF_PHYS%FRTHC(JLON,0)=YDMF_PHYS%FRTH(JLON,NTRADI-1,1)
        YDMF_PHYS%FRTHC(JLON,1)=YDMF_PHYS%FRTH(JLON,KLEV,1)
      ENDDO
    ENDIF
  ENDIF
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    CALL ACCVIMPGY ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     ...
     & PCPS,PGM,PTS,&
     & YDMF_PHYS%DIFCQ,YDMF_PHYS%DIFCQL,YDMF_PHYS%DIFCQN,YDMF_PHYS%DIFCS,YDMF_PHYS%FCCQL,YDMF_PHYS%FCCQN,&
     & YDMF_PHYS%FPFPCL,YDMF_PHYS%FPFPCN,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,&
     & ZFHMLTSC,ZFHEVPPC,ZFPEVPPC,&
     & YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,ZNEBC,ZQLIC,YDMF_PHYS%STRCU,YDMF_PHYS%STRCV,&
     & ICIS,INLAB,&
     & INND,&
     & PCVV)
    ...
    DO JLEV = KTDIA, KLEV
      DO JLON = KIDIA, KFDIA
        YDMF_PHYS%DIFCS(JLON,JLEV) = YDMF_PHYS%DIFCS(JLON,JLEV) - ZFHEVPPC(JLON,JLEV)&
                                            & - ZFHMLTSC(JLON,JLEV)
        YDMF_PHYS%DIFCQ(JLON,JLEV) = YDMF_PHYS%DIFCQ(JLON,JLEV) + YDMF_PHYS%FPEVPCL(JLON,JLEV)&
                                            & + YDMF_PHYS%FPEVPCN(JLON,JLEV)
      ENDDO
    ENDDO
    YDMF_PHYS%FPEVPCL=0._JPRB
    YDMF_PHYS%FPEVPCN=0._JPRB
    IF (LGRAPRO) THEN
      YDMF_PHYS%FPEVPCG=0._JPRB
    ENDIF
    ...
    IF (.FALSE.) THEN
      DO JLEV=0,KLEV
        DO JLON=KIDIA,KFDIA
          YDMF_PHYS%DIFCQL(JLON,JLEV)=0.0_JPRB
          YDMF_PHYS%DIFCQN(JLON,JLEV)=0.0_JPRB
        ENDDO
      ENDDO
    ENDIF
  ENDIF !  (LCVPGY)
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      ! - OUTPUT 2D .
      & ZEDMFQ,YDMF_PHYS%DIFCQL,YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQLC, YDMF_PHYS%DIFCQIC,&
      & ZEDMFS,ZDIFCVTH,ZTMPPRODTH,&
      & YDMF_PHYS%FCCQL,YDMF_PHYS%FCCQN,ZEDMFU,ZEDMFV,ZSTRCTRA(:,:,INBTRA_DEP:INBTRA),&
      & YDMF_PHYS%FIMCC,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,&
      & YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,ZMF_UP,ZMU,ZMD,&
      & YDMF_PHYS%FPFPCL,YDMF_PHYS%FPFPCN,&
      & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC,&
      & YDMF_PHYS%FCNEGQLC,YDMF_PHYS%FCNEGQIC,YDMF_PHYS%FCNEGQRC,YDMF_PHYS%FCNEGQSC, &
      & INLAB,&
      & ZNEBC0,ZQLI_CVP,ZTU,ZQU,ZQC_DET_PCMT,ZCSGC,ZENTCH, &
      ! - OUTPUT 1D .
      & INND,YDMF_PHYS%CAPE,ZAIPCMT,ZALF_CAPE,ZALF_CVGQ,&
      ! - INPUT/OUTPUT 2D .
    ...
    ELSE
      ...
      & PGM, PTS, PQS, PNEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
      & YDMF_PHYS%DIFCQ,YDMF_PHYS%DIFCQL,YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQLC, YDMF_PHYS%DIFCQIC,&
      & YDMF_PHYS%DIFCS,ZDIFCVTH,ZTMPPRODTH,&
      & YDMF_PHYS%FCCQL,YDMF_PHYS%FCCQN,YDMF_PHYS%STRCU,YDMF_PHYS%STRCV,ZSTRCTRA(:,:,INBTRA_DEP:INBTRA),&
      & YDMF_PHYS%FIMCC,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,&
      & YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,ZMF_UP,ZMU,ZMD,&
      & YDMF_PHYS%FPFPCL,YDMF_PHYS%FPFPCN,&
      & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC,&
...
!DEC$ IVDEP
  ...
  IF ( LNEBR ) THEN
    CALL ACNEBR ( YDERAD,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCOEF,NTNEBU,KLEV,&
     ...
     & YDMF_PHYS%GZ0,PFPLCH,YDMF_PHYS_SURF%GSD_VH%PPBLH,PQS,PTS,&
...
!         Diagnostique de nebulosite partielle.
  CALL ACNPART(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
   ...
   & PCLCC=YDMF_PHYS%CLCC,PNEBC=ZNEBC0,PTOPC=YDMF_PHYS%CTOP)
...
!DEC$ IVDEP
        DO JLON=KIDIA,KFDIA
          IF ( YDMF_PHYS%FRSOPS(JLON) > RSUNDUR*PMU0(JLON) ) THEN
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         ...
         & ZQV,ZQCO2,PQICE,PQLI,ZQO3,PT,&
         & YDMF_PHYS%ALB,ZALBDIR,PEMIS,PGELAM,PGEMU,PMU0,PMU0LU,PTS,ZDECRD,ZCLCT_RAD,&
         & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
         & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
         & PGRPROX,PGMIXP,PGFLUXC,PGRSURF,YDMF_PHYS_SURF%GSD_VD%PSUND,&
         & YDMF_PHYS%FRSO,YDMF_PHYS%FRTH,&
         & YDMF_PHYS%FRSOC,YDMF_PHYS%FRTHC,ZFRSODS,YDMF_PHYS%FRSOPS,YDMF_PHYS%FRSOLU,YDMF_PHYS%FRTHDS,ZAER)
    ENDSELECT

    ! sum downward diffuse and direct solar radiation at surface
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%FRSODS(JLON)=ZFRSODS(JLON)+YDMF_PHYS%FRSOPS(JLON)
    ENDDO
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LRAYLU)  YDMF_PHYS%FRSOLU(KIDIA:KFDIA)=PVRMOON(KIDIA:KFDIA)
    ...
    &   YDMF_PHYS%FRSO  , YDMF_PHYS%FRTH  , YDMF_PHYS%FRSODS , YDMF_PHYS%FRTHDS,&
    &   ZCEMTR , ZCTRSO , YDMF_PHYS%FRSOC  , YDMF_PHYS%FRTHC,&
    &   ZSUDU  , ZSDUR  , ZDSRP  , ZSFSWDIR , ZSFSWDIF, YDMF_PHYS%FRSOPS, ZFRSODS, YDMF_PHYS%FRSOPT )


    ! ---- Take into account day duration depending on altitude.
    IF(LDAYD) CALL ACDAYD(YDRIP,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU ,PMU0,PAPHIF,PDELP,YDMF_PHYS%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,KIDIA,KFDIA,KLON,KLEV,KTDIA,KSGST,PGEMU  ,PMU0,PAPHIF,PDELP,YDMF_PHYS%FRSO)
    ...
    IF(.NOT.LMSE) THEN
      DO JLON=KIDIA,KFDIA
        ...
        YDMF_PHYS%FRSODS(JLON)=YDMF_PHYS%FRSO(JLON,KLEV,1)/(1.0_JPRB-ZALB(JLON))
      ENDDO
    ENDIF
    ! Compute Sunshine Duration (in seconds)
!DEC$ IVDEP
    DO JLON=KIDIA,KFDIA
      IF(YDMF_PHYS%FRSODS(JLON) >= RSUNDUR) THEN
  ...
  IF (LRAY.OR.LRAYFM ) THEN
    ...
    DO JLON = KIDIA, KFDIA
      YDMF_PHYS%FRSDNI(JLON)=YDMF_PHYS%FRSOPS(JLON)
  ...
  IF (LRAY.OR.LRAYFM) THEN
    ...
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%FRSGNI(JLON)=YDMF_PHYS%FRSDNI(JLON)+0.5_JPRB*(                         &
       & (1.0_JPRB+PMU0(JLON))*(YDMF_PHYS%FRSODS(JLON)-YDMF_PHYS%FRSOPS(JLON)       )+ &
       & (1.0_JPRB-PMU0(JLON))*(YDMF_PHYS%FRSODS(JLON)-YDMF_PHYS%FRSO  (JLON,KLEV,1)))
    ENDDO

    ! mean radiant temperature
    IF (LXMRT) THEN
      CALL MEAN_RAD_TEMP(KIDIA,KFDIA,KLON,                             &
       & PMU0,YDMF_PHYS%FRSO(:,KLEV,1),YDMF_PHYS%FRSODS,YDMF_PHYS%FRSOPS,YDMF_PHYS%FRTH(:,KLEV,1),YDMF_PHYS%FRTHDS, &
       & YDMF_PHYS%MRT) 
    ENDIF
...
!   Sauvegarde temporaire de l'ancien acdifus pour les besoins du Climat
    IF ( LACDIFUS ) THEN
      ...
      CALL ACDIFUS ( YDMCC,YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDIFU,KLEV,KCSS,&
       & PAPHI, PAPHIF, PCP, PDELP, YDMF_PHYS%FRSO, ZKTROV, ZKUROV, ZKNROV,&
       & ZQV, PRDELP, PT, PU, PV, ZXTROV, ZXUROV, ZXPTKEROV, PALPH,&
       & PAPRS, PR, ZQL, ZQI, ZNEBDIFF, PTKE, ZLMU, PLNPR,&
       & PCD, PCDN, ZCDROV, ZCHROV, ZCEROV, PCPS, YDMF_PHYS%CT,ZDQSTS,&
       ...
       & YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTS, YDMF_PHYS%FCQNG, YDMF_PHYS%STRTU, YDMF_PHYS%STRTV,&
       & YDMF_PHYS%DIFTQL, YDMF_PHYS%DIFTQN, PTENDPTKE,&
       & YDMF_PHYS%FCHSP, YDMF_PHYS%FCLL, YDMF_PHYS%FCLN, YDMF_PHYS%FCS, PFEVI, YDMF_PHYS%FEVL, YDMF_PHYS%FEVN, YDMF_PHYS%FEVV,&
       & YDMF_PHYS%FTR, PLHS, YDMF_PHYS%FRTH, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H)
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        ELSEIF (LRAY) THEN
          ...
          DO JLON=KIDIA,KFDIA
            ...
            ZSFSWDIR(JLON,1)=YDMF_PHYS%FRSOPS(JLON)
          ENDDO
        ENDIF
        ...
        CALL ARO_GROUND_PARAM( KBL, KGPCOMP,&
          ...
          & YDMF_PHYS%FRTHDS(KIDIA:KFDIA),ZSFSWDIF(KIDIA:KFDIA,1:NSW),&
          ...
          & YDMF_PHYS%FCS(KIDIA:KFDIA,1),ZFEV(KIDIA:KFDIA),&
          ...
          & YDMF_PHYS%FRTH(KIDIA:KFDIA,KLEV,1))               !orographic shadowing
...
! Opposite water vapor flux
        ...
        CALL ARO_GROUND_DIAG( KBL, KGPCOMP,&
          ...
          & PINDX(KIDIA:KFDIA), PINDY(KIDIA:KFDIA),&
          & PQS(KIDIA:KFDIA), YDMF_PHYS%GZ0(KIDIA:KFDIA),&
          & YDMF_PHYS%GZ0H(KIDIA:KFDIA), YDMF_PHYS%TCLS (KIDIA:KFDIA),&
          & YDMF_PHYS%QCLS(KIDIA:KFDIA), YDMF_PHYS%RHCLS(KIDIA:KFDIA),&
        ...
        DO JLON=KIDIA,KFDIA
          PGPAR(JLON,MGZ0)   = YDMF_PHYS%GZ0 (JLON)
          PGPAR(JLON,MGZ0H)  = YDMF_PHYS%GZ0H(JLON)
        ...
        DO JSG  = 1, KSGST+1
          DO JLEV = 0, KLEV
            DO JLON = KIDIA, KFDIA
              YDMF_PHYS%FRTH(JLON,JLEV,JSG)=YDMF_PHYS%FRTH(JLON,JLEV,JSG)+ZBUDTH(JLON)
            ENDDO
          ENDDO
        ENDDO
...
! calculation of variables for the old "ISBA" atmosphere scheme
      ...
      ELSEIF (.NOT.LMSE) THEN
        ...
        IF (.NOT. LSFORCS) THEN
          ...
          CALL ARP_GROUND_PARAM ( YDMCC,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KCSS,&
           & PAPHI,ZCP,YDMF_PHYS%FRSO,&
           & ZQV,PRDELP,PT,PU,PV,ZXURO,ZXQRO,ZXTRO,&
           & PAPRS,ZQL,ZQI,ZNEBDIFF,&
           & PCD,PCDN,ZCDROV,ZCHROV,ZCEROV,PCPS,YDMF_PHYS%CT,ZDQSTS,PEMIS,YDMF_PHYS_SURF%GSD_VH%PSPSH,&
           ...
           & YDMF_PHYS%FCHSP,YDMF_PHYS%FCLL,YDMF_PHYS%FCLN,YDMF_PHYS%FCS,PFEVI,YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
           & YDMF_PHYS%FEVV,YDMF_PHYS%FTR,PLHS,YDMF_PHYS_SURF%GSD_VH%PQSH,&
           & YDMF_PHYS%FRTH,&
           & YDMF_PHYS_SURF%GSD_VF%PZ0F,YDMF_PHYS_SURF%GSD_VV%PZ0H)

          DO JLON=KIDIA,KFDIA
            YDMF_PHYS%DIFTQ(JLON,KLEV)=ZDIFWQ(JLON)
            YDMF_PHYS%DIFTS(JLON,KLEV)=ZDIFWS(JLON)
          ENDDO
      ...
      CALL ACDIFV2 ( YGFL,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
        ...
        & ZSFSV,YDMF_PHYS%FCS,ZFEV,ZFMDU,ZFMDV,ZTSN,ZXHROV,&
        & PDIFSV,YDMF_PHYS%DIFTQ,YDMF_PHYS%DIFTS,YDMF_PHYS%STRTU,YDMF_PHYS%STRTV,YDMF_PHYS%DIFTQL,YDMF_PHYS%DIFTQN,YDMF_PHYS%DIFCQ,YDMF_PHYS%DIFCS,YDMF_PHYS%STRCU,YDMF_PHYS%STRCV,YDVARS%SHTUR%T0)

      IF (LEDKF) THEN
        IF (LSFORCS) THEN
        DO JLON=KIDIA,KFDIA
           YDMF_PHYS_SURF%GSD_SFL%PGROUP(JLON,1) = YDMF_PHYS%FCS(JLON,1)
           YDMF_PHYS_SURF%GSD_SFL%PGROUP(JLON,2) = ZFEV(JLON)
        ENDDO
        ELSE
          DO JLON=KIDIA,KFDIA             
            YDMF_PHYS_SURF%GSD_SFL%PGROUP(JLON,1) = YDMF_PHYS%DIFTS(JLON,KLEV)
            YDMF_PHYS_SURF%GSD_SFL%PGROUP(JLON,2) = YDMF_PHYS%DIFTQ(JLON,KLEV)
          ENDDO
        ENDIF   
      ENDIF
      ...
      IF (LCOEFKTKE) THEN
        ...
        CALL ACDIFV3 ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KSTEP,&
          ...
          & YDMF_PHYS%DIFTS,YDMF_PHYS%DIFTQ,PTKE,PTENDPTKE,&
          & ZMN2_ES,ZMN2_EQ,ZMN2_DS,ZMN2_DQ,&
          & YDMF_PHYS%DIFTQL,YDMF_PHYS%DIFTQN,&
          & ZDIFWQ,ZDIFWS,&
          & YDMF_PHYS%FCLL,YDMF_PHYS%FCLN,YDMF_PHYS%FCS,PFEVI,YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
          & YDMF_PHYS%GZ0,ZRTI,&
          & ZSC_FEVI,ZSC_FEVN,ZSC_FCLL,ZSC_FCLN,&
        ...
        IF (YFQTUR%LGP.AND.YFSTUR%LGP) THEN
          DO JLEV=KTDIA,KLEV
            DO JLON=KIDIA,KFDIA
              YDVARS%FQTUR%T0(JLON,JLEV)=YDMF_PHYS%DIFTQ(JLON,JLEV)+YDMF_PHYS%DIFTQL(JLON,JLEV)&
               &                                 +YDMF_PHYS%DIFTQN(JLON,JLEV)
              YDVARS%FSTUR%T0(JLON,JLEV)=YDMF_PHYS%DIFTS(JLON,JLEV)
            ENDDO
          ENDDO
        ENDIF
      ENDIF ! LCOEFKTKE
...
!----------------------------------------------------------------------------
    IF (LCVPPKF.OR.(LEDKF .AND. .NOT. LEDMFI)) THEN
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          YDMF_PHYS%DIFTQ   (JLON,JLEV) = YDMF_PHYS%DIFTQ(JLON,JLEV) + ZDIFCVPPQ(JLON,JLEV)
          YDMF_PHYS%DIFTS   (JLON,JLEV) = YDMF_PHYS%DIFTS(JLON,JLEV) + ZDIFCVPPS(JLON,JLEV)
          YDMF_PHYS%STRTU   (JLON,JLEV) = YDMF_PHYS%STRTU(JLON,JLEV) + ZDIFCVPPU(JLON,JLEV)
          YDMF_PHYS%STRTV   (JLON,JLEV) = YDMF_PHYS%STRTV(JLON,JLEV) + ZDIFCVPPV(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF
...
!DEC$ IVDEP
      DO JLON = KIDIA,KFDIA
        ...
        ZT(JLON,JLEV)=PT(JLON,JLEV)-ZIPOI(JLON,JLEV)/PCP(JLON,JLEV)&
         & *(YDMF_PHYS%DIFTS(JLON,JLEV)-YDMF_PHYS%DIFTS(JLON,JLEV-1))
        ZQX0=ZQI(JLON,JLEV)
        ZQX1=ZQX0-ZIPOI(JLON,JLEV)*(YDMF_PHYS%DIFTQN(JLON,JLEV)&
          & -YDMF_PHYS%DIFTQN(JLON,JLEV-1))
        ZQI(JLON,JLEV)=MAX(0.0_JPRB,ZQX1)
        ZDQI=MAX(0.0_JPRB,ZQX1)-ZQX1
        ZFCQING(JLON,JLEV)=ZFCQING(JLON,JLEV-1)-ZDQI*ZPOID(JLON,JLEV)
        YDMF_PHYS%FCQNNG(JLON,JLEV)=YDMF_PHYS%FCQNNG(JLON,JLEV)+ZFCQING(JLON,JLEV)
        ZQX0=ZQL(JLON,JLEV)
        ZQX1=ZQX0-ZIPOI(JLON,JLEV)*(YDMF_PHYS%DIFTQL(JLON,JLEV)&
          & -YDMF_PHYS%DIFTQL(JLON,JLEV-1))
        ZQL(JLON,JLEV)=MAX(0.0_JPRB,ZQX1)
        ZDQL= MAX(0.0_JPRB,ZQX1)-ZQX1
        ZFCQLNG(JLON,JLEV)=ZFCQLNG(JLON,JLEV-1)-ZDQL*ZPOID(JLON,JLEV)
        YDMF_PHYS%FCQLNG(JLON,JLEV)=YDMF_PHYS%FCQLNG(JLON,JLEV)+ZFCQLNG(JLON,JLEV)
        ZDQC=ZDQI+ZDQL

        ZQX0=ZQV(JLON,JLEV)
        ZQX1=ZQX0-ZIPOI(JLON,JLEV)*(YDMF_PHYS%DIFTQ(JLON,JLEV)&
          & -YDMF_PHYS%DIFTQ(JLON,JLEV-1))
        ...
        IF(LRKCDEV.AND.KSTEP>0) THEN
          ZDTRAD(JLON,JLEV)=ZIPOI(JLON,JLEV)/PCP(JLON,JLEV)* (&
            & YDMF_PHYS%FRSO(JLON,JLEV-1,1)-YDMF_PHYS%FRSO(JLON,JLEV,1)&
            & +YDMF_PHYS%FRTH(JLON,JLEV-1,1)-YDMF_PHYS%FRTH(JLON,JLEV,1) )
    ...
    IF((TRIM(CGMIXLEN) == 'TM').OR.(TRIM(CGMIXLEN) == 'TMC')) THEN
      ...
      CALL ACPBLHTM ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       ...
       & ZPCLS, YDMF_PHYS%TCLS,YDMF_PHYS%QCLS,&
       & YDMF_PHYS%FCS,YDMF_PHYS%FCLL,YDMF_PHYS%FCLN,PLHS, YDMF_PHYS%GZ0,&
       & KCLPH,ZBLH,ZUGST,ZVGST)
      YDMF_PHYS_SURF%GSD_VH%PPBLH(KIDIA:KFDIA)=ZBLH(KIDIA:KFDIA)
      YDMF_PHYS%CLPH(KIDIA:KFDIA)=ZBLH(KIDIA:KFDIA)
      IF (.NOT.LRAFTUR) THEN
        YDMF_PHYS%UGST(KIDIA:KFDIA)=ZUGST(KIDIA:KFDIA)
        YDMF_PHYS%VGST(KIDIA:KFDIA)=ZVGST(KIDIA:KFDIA)
      ENDIF
    ...
    IF(LNEBCO.AND.LNEBR)THEN
      ...
      CALL ACPBLH ( YDPHY0,KIDIA,KFDIA,KLON,NTDIFU,KLEV,&
       ...
       & YDMF_PHYS%STRTU, YDMF_PHYS%STRTV,&
       & ZPREN, PTS, PQS,&
       & YDMF_PHYS%FCS,YDMF_PHYS%FCLL,YDMF_PHYS%FCLN,PLHS,ZRTI,YDMF_PHYS_SURF%GSD_VH%PPBLH)
    ENDIF
    ! ------------------------------------------------------------------
...
!DEC$ IVDEP
  DO JLON=KIDIA,KFDIA
    IF(.NOT.LMSE)YDMF_PHYS%FRTHDS(JLON)=YDMF_PHYS%FRTH(JLON,KLEV,1)/PEMIS(JLON)+RSIGMA*PTS(JLON)**4
    IF(PMU0(JLON) <= 0.0_JPRB) THEN

!     NUIT : FLUX SOLAIRE NUL
!     NIGHT: SOLAR FLUX IS EQUAL TO ZERO

      YDMF_PHYS%FRSOPT(JLON)=0.0_JPRB
    ELSE
      YDMF_PHYS%FRSOPT(JLON)=RII0*PMU0(JLON)
    ENDIF
  ENDDO
...
!     ------------------------------------------------------------------
  IF ( LGWD ) THEN
    CALL ACDRAG ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTDRAG,KLEV,&
     ...
     & YDMF_PHYS%STRDU, YDMF_PHYS%STRDV,ZTRAJGWD)
  ENDIF
...
!     ---------------------------------
  ...
  IF ( LSTRA.AND.(.NOT.LSTRAPRO) ) THEN
    CALL ACPLUIE ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     & PAPRS, PDELP,&
     & ZQV, PQW, PT,&
     & YDMF_PHYS%FPFPSL,YDMF_PHYS%FPFPSN,YDMF_PHYS%FCSQL,YDMF_PHYS%FCSQN,YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN)
  ENDIF

  IF ( LSTRAS ) THEN
    CALL ACPLUIS ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     & PAPRSF,PDELP,ZNEBS,ZQV,ZQLIS,PQW,PR,PT,&
     & YDMF_PHYS%FCSQL,YDMF_PHYS%FCSQN,YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPFPSL,YDMF_PHYS%FPFPSN)
  ENDIF
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       ...
       & YDMF_PHYS%FPFPSL,YDMF_PHYS%FPFPSN,YDMF_PHYS%FPFPSG,YDMF_PHYS%FCSQL,YDMF_PHYS%FCSQN,YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPLSG,&
       & ZSEDIQL,ZSEDIQI,YDMF_PHYS%DIAGH)
    ELSE
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
       & ZVETAF,PAPHI,&
    ...
    DO JLEV=KTDIA,KLEV
      DO JLON=KIDIA,KFDIA
        ZDQL=ZIPOI(JLON,JLEV)* (&
          & YDMF_PHYS%FCSQL(JLON,JLEV)-YDMF_PHYS%FCSQL(JLON,JLEV-1))
        ZQL(JLON,JLEV)=ZQL(JLON,JLEV)+ZDQL
        ZDQI=ZIPOI(JLON,JLEV)* (&
          & YDMF_PHYS%FCSQN(JLON,JLEV)-YDMF_PHYS%FCSQN(JLON,JLEV-1))
      ...
      DO JLEV = KTDIA, KLEV
        DO JLON = KIDIA, KFDIA
          PCVGQ(JLON,JLEV) = PCVGQ(JLON,JLEV)*ZMOD(JLON) &
           & -RG*PRDELP(JLON,JLEV)&
           & *(YDMF_PHYS%DIFTQ(JLON,JLEV)-YDMF_PHYS%DIFTQ(JLON,JLEV-1)&
  ...
  !  -------------------------
    ...
    IF (.NOT.LCVCSD) THEN
      CALL ACCVUD ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCS,&
       & YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, ZATSLC,&
       & INLAB, YDVARS%UNEBH%T0, ZDETFI, ZTU, ZQU, ZUU, ZVU,&
       & INND,YDMF_PHYS%CAPE,YDMF_PHYS%CUCONVCA,YDMF_PHYS%NLCONVCA,&
...
! PASS T9 CONDENSATES TO ACCSU
      CALL ACCSU ( YDMODEL%YRML_PHY_EC%YRECUCONVCA,YDMODEL%YRML_PHY_EC%YRECUMF,YDMODEL%YRML_PHY_MF, &
        ...
        & PRDELP, ZT, ZTW, PU, PV, PVORT0,&
        & PVERVEL, &
        & YDMF_PHYS%FCSQL, YDMF_PHYS%FCSQN, &
        & PTS, ZTAUX, PRCORI,ZDECRD_MF,&
        & YDMF_PHYS_SURF%GSD_VK%PUDGRO, &
        & YDMF_PHYS%CUCONVCA,YDMF_PHYS%NLCONVCA,&
        & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCS,&
        & YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, ZATSLC,&
        & INLAB, YDVARS%UNEBH%T0, ZDETFI, ZTU, ZQU, ZUU, ZVU,&
        & INND,YDMF_PHYS%CAPE, &
...
!input/output  2d .
               ...
               & YDMF_PHYS%FCQNG,YDMF_PHYS%FCQNNG,YDMF_PHYS%FCQLNG)
...
!rb store "-B" state: no protection of Ncv
  ...
  !  ---------------------------------------------------------
    ...
    !    -------------------------------
        ZFCQL(JLON,JLEV)=YDMF_PHYS%FCSQL(JLON,JLEV)+YDMF_PHYS%FCCQL(JLON,JLEV)
        ZFCQI(JLON,JLEV)=YDMF_PHYS%FCSQN(JLON,JLEV)+YDMF_PHYS%FCCQN(JLON,JLEV)
        IF (LRCVOTT) THEN
          ZCONVC=MAX(0.0_JPRB, YDMF_PHYS%FCCQL(JLON,JLEV)+YDMF_PHYS%FCCQN(JLON,JLEV))
          ZAUXPRC(JLON)=ZAUXPRC(JLON)+&
         & MAX(0.0_JPRB,YDMF_PHYS%FCSQL(JLON,JLEV)+YDMF_PHYS%FCSQN(JLON,JLEV)&
         & -YDMF_PHYS%FCSQL(JLON,JLEV-1)-YDMF_PHYS%FCSQN(JLON,JLEV-1))
    ...
    CALL APLMPHYS( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     ...
     & YDMF_PHYS%FPFPSL,YDMF_PHYS%FPFPSN, YDMF_PHYS%FPFPSG, YDMF_PHYS%FPEVPSL, YDMF_PHYS%FPEVPSN, YDMF_PHYS%FPEVPSG, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN,YDMF_PHYS%FPLSG,&
     & ZSEDIQL,ZSEDIQI,ZMELNET,ZMELGET,YDMF_PHYS%DIAGH)
   ...
   CALL ACUPM( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
              ...
              & YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPFPSL,YDMF_PHYS%FPFPSN,YDMF_PHYS%FPFPSG,&
              & YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPLSG,&
              & ZFCQL,ZFCQI,YDMF_PHYS%FCCQL,YDMF_PHYS%FCCQN,&
! output 2D
              & ZOME, ZFHP,&
! input/output  2d .
              & ZQV,ZQL,ZQI,ZQR,ZQS,ZQG,ZT,&
              & YDMF_PHYS%FCSQL,YDMF_PHYS%FCSQN,&
              & YDMF_PHYS%FCQNG,YDMF_PHYS%FCQRNG,YDMF_PHYS%FCQSNG,YDMF_PHYS%FCQGNG)
  ...
  !  -------------------------
    ...
    IF (LNSDO) THEN
      CALL ACNSDO(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & ZATSLC,ZGEOSLC,&
       & YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPEVPSL, YDMF_PHYS%FPEVPSN,&
       & ZDIFCQD,ZDIFCQLD,ZDIFCQID,ZDIFCSD,&
       & YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,&
       & ZSTRCUD,ZSTRCVD,&
    ...
    ELSE
      CALL ACMODO(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
       ...
       & YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPLSG,&
       & ZDIFCQD,ZDIFCQLD,ZDIFCQID,ZDIFCSD,&
       & YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPEVPCG,&
       & ZSTRCUD,ZSTRCVD,&
       & YDVARS%DAL%T0,YDVARS%DOM%T0)
    ENDIF
  !  ---------------------------------------------
  ...
  !  ---------------------------------------------
    ...
    !   --------------------------------------------------
        ...
        YDMF_PHYS%DIFCS(JLON,JLEV) =YDMF_PHYS%DIFCS(JLON,JLEV) +ZDIFCSD(JLON,JLEV)
        YDMF_PHYS%DIFCQ(JLON,JLEV) =YDMF_PHYS%DIFCQ(JLON,JLEV) +ZDIFCQD(JLON,JLEV)
        YDMF_PHYS%DIFCQL(JLON,JLEV)=YDMF_PHYS%DIFCQL(JLON,JLEV)+ZDIFCQLD(JLON,JLEV)
        YDMF_PHYS%DIFCQN(JLON,JLEV)=YDMF_PHYS%DIFCQN(JLON,JLEV)+ZDIFCQID(JLON,JLEV)
        YDMF_PHYS%STRCU(JLON,JLEV)=YDMF_PHYS%STRCU(JLON,JLEV)+ZSTRCUD(JLON,JLEV)
        YDMF_PHYS%STRCV(JLON,JLEV)=YDMF_PHYS%STRCV(JLON,JLEV)+ZSTRCVD(JLON,JLEV)
      ENDDO
    ENDDO
...
! input  2d .
     ...
     & ZIPOI,ZPOID,&
     & ZFALLR,ZFALLS,ZFALLG,&
     & YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPEVPCG,ZFHP,&
     & ZDIFCSD,ZDIFCQD,ZDIFCQLD,ZDIFCQID,&
! input/output  2d .
     & ZQV,ZQL,ZQI,ZQR,ZQS,ZQG,ZT,YDVARS%UEN%T0,&
     & YDMF_PHYS%FCQNG,YDMF_PHYS%FCQNNG,YDMF_PHYS%FCQLNG,YDMF_PHYS%FCQRNG,YDMF_PHYS%FCQSNG,YDMF_PHYS%FCQGNG,&
     & YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPLSG)
...
!rb  store "+C": case with no protection of Ncv
  ...
  ! ---------------------------------------------
    DO JLEV=KTDIA-1,KLEV
      DO JLON=KIDIA, KFDIA
         YDMF_PHYS%FPLCL(JLON,JLEV)=ZSIGPC(JLON)*YDMF_PHYS%FPLSL(JLON,JLEV)
         YDMF_PHYS%FPLSL(JLON,JLEV)=YDMF_PHYS%FPLSL(JLON,JLEV)-YDMF_PHYS%FPLCL(JLON,JLEV)
         YDMF_PHYS%FPLCN(JLON,JLEV)=ZSIGPC(JLON)*YDMF_PHYS%FPLSN(JLON,JLEV)
         YDMF_PHYS%FPLSN(JLON,JLEV)=YDMF_PHYS%FPLSN(JLON,JLEV)-YDMF_PHYS%FPLCN(JLON,JLEV)
      ENDDO
    ENDDO
    IF (LGRAPRO) THEN
      DO JLEV=KTDIA-1,KLEV
        DO JLON=KIDIA, KFDIA
           YDMF_PHYS%FPLCG(JLON,KLEV)=0.0_JPRB !ZSIGPC(JLON)*PFPLSG(JLON,JLEV)
           YDMF_PHYS%FPLSG(JLON,JLEV)=YDMF_PHYS%FPLSG(JLON,JLEV)-YDMF_PHYS%FPLCG(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF
  ENDIF ! LCDDPRO
...
! -----------------------------------------------------
  ...
  IF ( LCVRA ) THEN
    ...
    IF (LSTRAPRO) THEN
      DO JLEV = KTDIA-1, KLEV
        DO JLON = KIDIA, KFDIA
          YDMF_PHYS%DIFTQ(JLON,JLEV)=YDMF_PHYS%DIFTQ(JLON,JLEV)+ZFCQVNG(JLON,JLEV)
        ENDDO
      ENDDO
    ENDIF
    ...
    CALL ACCVIMP ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     ...
     & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCS, YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN,&
     & YDMF_PHYS%FPEVPCL, YDMF_PHYS%FPEVPCN, YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, ZFPCOR, INLAB, YDMF_PHYS%CAPE, INND,&
     & YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTS, ZGEOSLC, PGEMU)
  ...
  ELSEIF (LCVRAV3) THEN
    ...
    CALL ACCVIMP_V3 (YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTCVIM,KLEV,&
     ...
     & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCS, YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN,&
     & YDMF_PHYS%FPEVPCL, YDMF_PHYS%FPEVPCN, YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, INLAB, INND,&
     & YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTS)
  ...
  ELSEIF (LCVTDK) THEN     ! <== IFS deep convection scheme
    ...
    ENDDO
    LLPTQ = .FALSE.
    CALL ACUPTQ ( KLON,KIDIA,KFDIA,KLEV,LLPTQ,YDMF_PHYS%FRSO,YDMF_PHYS%FRTH,&
     & YDMF_PHYS%DIFCQ,YDMF_PHYS%DIFCS,YDMF_PHYS%DIFTQ,YDMF_PHYS%DIFTS,YDMF_PHYS%FCCQL,YDMF_PHYS%FCCQN,YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,&
     & YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPFPCL,YDMF_PHYS%FPFPCN,PRDELP,PT,PQ,PTS,&
     & ZTENHA,ZTENQVA )
    ...
    CALL CUCALLN_MF &
     ...
     & PT,ZQV,PU,PV,ZLISUM,ZVERVEL,YDMF_PHYS%DIFTQ,YDMF_PHYS%DIFTS,&
     ...
     & ZLU,ZLUDE,ZLUDELI,ZSNDE,ZMFU,ZMFD,YDMF_PHYS%DIFCQ,YDMF_PHYS%DIFCS,ZFHPCL,ZFHPCN,&
     & YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,ZLRAIN,ZRSUD,YDMF_PHYS%STRCU,YDMF_PHYS%STRCV,ZFCQLF,ZFCQLI,&
     & ZMFUDE_RATE,ZMFDDE_RATE,YDMF_PHYS%CAPE,ZWMEAN,ZDIFF,0,ZCEN,ZTENC,ZSCAV)
    DO JLEV=0,KLEV
      DO JLON=KIDIA,KFDIA 
         ZFPCOR  (JLON,JLEV)=YDMF_PHYS%FPLCL(JLON,JLEV)+YDMF_PHYS%FPLCN(JLON,JLEV)
         YDMF_PHYS%FCCQL  (JLON,JLEV)=YDMF_PHYS%FPLCL(JLON,JLEV)
         YDMF_PHYS%FCCQN  (JLON,JLEV)=YDMF_PHYS%FPLCN(JLON,JLEV)
         YDMF_PHYS%FPFPCL (JLON,JLEV)=YDMF_PHYS%FPLCL(JLON,JLEV)
         YDMF_PHYS%FPFPCN (JLON,JLEV)=YDMF_PHYS%FPLCN(JLON,JLEV)
         YDMF_PHYS%FPEVPCL(JLON,JLEV)=0._JPRB
         YDMF_PHYS%FPEVPCN(JLON,JLEV)=0._JPRB
       ENDDO
    ENDDO
  ...
  IF(LFLASH .AND. LCVTDK) THEN
    ...
    CALL CULIGHT &
      ...
      &   PT    ,  ZLU   ,  ZMFU,    YDMF_PHYS%CAPE, &
      &   YDMF_PHYS%FPLCL,  YDMF_PHYS%FPLCN,&
      &   LLCUM ,  ICBOT ,  ICTOP,&
      ! Outputs
      &   LLLINOX, YDMF_PHYS%FLASH,  ZLIGH_CTG,  ZCTOPH, &
      &   ZPRECMX, ZICE,   ZCDEPTH,  ZWMFU) 
    ! LIGHTNING FLASH RATES ARE CONVERTED IN fl/km2/s BEFORE ENTERING CFU TIME ACCUMULATION.
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%FLASH(JLON)=YDMF_PHYS%FLASH(JLON)/86400._JPRB
    ENDDO
  ENDIF
  ...
  IF ( LPROCLD ) THEN
    ...
    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,NTPLUI,KLEV,&
     ...
     & YDMF_PHYS%FCSQL, YDMF_PHYS%FCSQN, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN,&
     & YDMF_PHYS%FPEVPSL, YDMF_PHYS%FPEVPSN, YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN, ZSEDIQL, ZSEDIQI )
      ...
      DO JLEV=KTDIA,KLEV
        DO JLON=KIDIA,KFDIA
          YDVARS%DAL%T0(JLON,JLEV)=( &
            & +YDMF_PHYS%FPEVPSL(JLON,JLEV)-YDMF_PHYS%FPEVPSL(JLON,JLEV-1)   &
            & +YDMF_PHYS%FPEVPSN(JLON,JLEV)-YDMF_PHYS%FPEVPSN(JLON,JLEV-1)   &
            & +YDMF_PHYS%FPEVPCL(JLON,JLEV)-YDMF_PHYS%FPEVPCL(JLON,JLEV-1)   &
            & +YDMF_PHYS%FPEVPCN(JLON,JLEV)-YDMF_PHYS%FPEVPCN(JLON,JLEV-1)) &
            & /RG*PRDELP(JLON,JLEV)
        ENDDO
      ENDDO
      IF(LMUSCLFA) CALL WRSCMR(NMUSCLFA,'PFPEVPSL0',YDMF_PHYS%FPEVPSL,KLON,KLEV+1)
      IF(LMUSCLFA) CALL WRSCMR(NMUSCLFA,'PFPEVPSN0',YDMF_PHYS%FPEVPSN,KLON,KLEV+1)
      IF(LMUSCLFA) CALL WRSCMR(NMUSCLFA,'PFPEVPCL0',YDMF_PHYS%FPEVPCL,KLON,KLEV+1)
      IF(LMUSCLFA) CALL WRSCMR(NMUSCLFA,'PFPEVPCN0',YDMF_PHYS%FPEVPCN,KLON,KLEV+1)
    ENDIF
  ENDIF

  IF ( LNEBCO ) THEN
    CALL ACNEBC ( YDPHY0,KIDIA,KFDIA,KLON,NTNEBU,KLEV,&
     & YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,INLAB,&
     & INND,&
  ...
  IF ( LGWDC ) THEN
    CALL ACDRAC ( YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,NTDRAG,KLEV,&
     ...
     & YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH,&
     & YDMF_PHYS%STRCU, YDMF_PHYS%STRCV)
  ENDIF
  ...
  IF ( LNORGWD ) THEN
    ...
    ZPRECGWD(KIDIA:KFDIA) = MAX(0.0_JPRB,                  &
      & YDMF_PHYS%FPLCL(KIDIA:KFDIA,KLEV) + YDMF_PHYS%FPLCN(KIDIA:KFDIA,KLEV))
    ...
    DO JLEV = 1, KLEV
       DO JLON = KIDIA, KFDIA
          ...
          YDMF_PHYS%STRCU(JLON, JLEV) = YDMF_PHYS%STRCU(JLON, JLEV) + ZFLX_LOTT_GWU(JLON, JLEV)
          YDMF_PHYS%STRCV(JLON, JLEV) = YDMF_PHYS%STRCV(JLON, JLEV) + ZFLX_LOTT_GWV(JLON, JLEV)          
    ...
    DO JLEV = 1, NORGWD_NNOVERDIF
       DO JLON = KIDIA, KFDIA
          YDMF_PHYS%STRTU(JLON, JLEV) = 0.0
          YDMF_PHYS%STRTV(JLON, JLEV) = 0.0          
       ENDDO
    ENDDO
...
!         SAUVEGARDE DES FLUX DE PRECIPITATION CONVECTIVE ET STRATIFORME.
  ...
  IF(LNEBN.OR.LNEBR.OR.LRRGUST) THEN
    IF (LFPCOR) THEN
      IF (LCDDPRO) THEN
        DO JLEV=KTDIA-1,KLEV
          DO JLON=KIDIA,KFDIA
            PFPLCH(JLON,JLEV)=YDMF_PHYS%FPLCL(JLON,JLEV)+YDMF_PHYS%FPLCN(JLON,JLEV)
          ENDDO
        ENDDO
      ELSE
        DO JLEV=KTDIA-1,KLEV
  ...
  IF(LRRGUST) THEN
    DO JLEV=KTDIA-1,KLEV
      DO JLON=KIDIA,KFDIA
        PFPLSH(JLON,JLEV)=YDMF_PHYS%FPLSL(JLON,JLEV)+YDMF_PHYS%FPLSN(JLON,JLEV)+YDMF_PHYS%FPLSG(JLON,JLEV)
      ENDDO
    ENDDO
  ENDIF
  ...
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      YDMF_PHYS%DIFTQL(JLON,JLEV)=YDMF_PHYS%DIFTQL(JLON,JLEV)+ZSEDIQL(JLON,JLEV)
      YDMF_PHYS%DIFTQN(JLON,JLEV)=YDMF_PHYS%DIFTQN(JLON,JLEV)+ZSEDIQI(JLON,JLEV)
      ZFPLSL (JLON,JLEV)=YDMF_PHYS%DIFTQL(JLON,JLEV)+YDMF_PHYS%FPLSL (JLON,JLEV)
      ZFPLSN (JLON,JLEV)=YDMF_PHYS%DIFTQN(JLON,JLEV)+YDMF_PHYS%FPLSN (JLON,JLEV)
    ENDDO
  ENDDO
  IF (LGRAPRO) THEN
    DO JLEV=KTDIA,KLEV
      DO JLON=KIDIA,KFDIA
        ZFPLSN (JLON,JLEV)=ZFPLSN(JLON,JLEV)+YDMF_PHYS%FPLSG(JLON,JLEV)
      ENDDO
    ENDDO
  ENDIF
  ...
  IF ( LCONDWT.AND.LPROCLD.AND..NOT.LGPCMT ) THEN
    CALL QNGCOR (YDPHY2,KIDIA, KFDIA, KLON, NTPLUI, KLEV,&
     & ZQV, ZQL, ZQI, PRDELP,&
     & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL,&
     ...
     & YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FCSQL, YDMF_PHYS%FCSQN, YDMF_PHYS%FCQNG )
...
!     ---------------------------
  IF ( LMSE ) THEN
    ...
    DO JLON=KIDIA,KFDIA
      PGPAR(JLON,MRAIN)=ZFPLSL(JLON,KLEV)+YDMF_PHYS%FPLCL(JLON,KLEV)
      PGPAR(JLON,MSNOW)=ZFPLSN(JLON,KLEV)+YDMF_PHYS%FPLCN(JLON,KLEV)
    ENDDO
  ...
  ELSE
    ...
    IF ( LSFHYD.AND.LSOLV ) THEN
      CALL ACDROV ( YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KLEV,KCSS,&
       & YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, ZFPLSL, ZFPLSN, YDMF_PHYS%FRSO, YDMF_PHYS%FRTH,&
       & PC1, PC2, ZC3, ZCN, YDMF_PHYS%CT, YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS%FEVV, YDMF_PHYS%FTR, YDMF_PHYS_SURF%GSD_VV%PLAI, PNEIJ,&
       & PVEG, ZWFC, ZWPMX, PWL, ZWLMX, ZWSEQ, ZWSMX,&
       & YDMF_PHYS%FCHSP, YDMF_PHYS%FCLL, YDMF_PHYS%FCLN, YDMF_PHYS%FCS, PFEVI, YDMF_PHYS%FEVL, YDMF_PHYS%FEVN, YDMF_PHYS_SURF%GSD_VF%PLSM, PSNS,&
       & PTP, PTS, PWP, PWPI, PWS, PWSI,&
       & YDMF_PHYS%FGEL, YDMF_PHYS%FGELS,&
       & YDMF_PHYS%FLWSP, YDMF_PHYS%FONTE, YDMF_PHYS%RUISP, YDMF_PHYS%RUISL, YDMF_PHYS%RUISS)
    ENDIF
...
!     ------------------------------------------------------------------
  IF ( LRRMES ) THEN
    CALL ACDRME ( YDPHY2,YDTOPH,KIDIA,KFDIA,KLON,NTDRME,KLEV,&
     ...
     & YDMF_PHYS%FRMH,PFRMQ,YDMF_PHYS%STRMU,YDMF_PHYS%STRMV,&
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    CALL ACDIFOZ ( YDRIP,YDPHY2,YDMODEL%YRML_PHY_MF%YRVDOZ,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
     & PAPRSF,PDELP,YDMF_PHYS%FPLCL,YDMF_PHYS%FPLSL,YDMF_PHYS%FRSO,&
  ...
  IF (LPHSPSH) THEN
    DO JLON=KIDIA, KFDIA
       YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=(PT(JLON,KLEV)-PTS(JLON)) * ((&
          & RCW-RCPD)*(YDMF_PHYS%FPLSL(JLON,KLEV)+YDMF_PHYS%FPLCL(JLON,KLEV))&
          & +(RCS-RCPD)*(YDMF_PHYS%FPLSN(JLON,KLEV)+YDMF_PHYS%FPLCN(JLON,KLEV)) )
    ENDDO
    IF (LGRAPRO) THEN
      YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)=YDMF_PHYS_SURF%GSD_VH%PSPSH(JLON)+(PT(JLON,KLEV)-PTS(JLON)) * (&
        &(RCS-RCPD)*(YDMF_PHYS%FPLSG(JLON,KLEV)+YDMF_PHYS%FPLCG(JLON,KLEV)))
    ENDIF
  ENDIF !LPHSPSH
...
!     -----------------------------------------------------------------------
  ...
  IF(LFLEXDIA) THEN
    IF (LDDH_OMP) THEN
      ...
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*PTS(KIDIA:KFDIA)/YDMF_PHYS%CT(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSTS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*RTINER*PTP(KIDIA:KFDIA,KCSS)/YDMF_PHYS%CT(KIDIA:KFDIA)
      ...
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%TCLS,'VTCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%QCLS,'VQCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%UCLS,'VUCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%VCLS,'VVCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%NUCLS,'VNUCLS',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%NVCLS,'VNVCLS',YDDDH,CDTYPE='S')
      ZTMPAS(KIDIA:KFDIA)=PAPHI(KIDIA:KFDIA,KLEV)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'VSPHI',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%GZ0,'VSGZ0',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%GZ0H,'VSGZH',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%ALB,'VSALB',YDDDH,CDTYPE='S')
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%CLPH,'VSCPH',YDDDH,CDTYPE='S')

      ! surface fluxes free stlye or not
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FRSO(KIDIA:KFDIA,KLEV,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSRAYSODW',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FRTH(KIDIA:KFDIA,KLEV,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSRAYTHUP',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FCLL(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSCHLATLI',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FCLN(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSCHLATNE',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FCS(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSCHLATSF',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FCHSP(KIDIA:KFDIA,KCSS)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSCHGSNPL',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FONTE(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FFONTESLI',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FPLSL(KIDIA:KFDIA,KLEV)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSPRELIGE',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FPLSN(KIDIA:KFDIA,KLEV)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSPRENEGE',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FPLCL(KIDIA:KFDIA,KLEV)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSPRELICO',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FPLCN(KIDIA:KFDIA,KLEV)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSPRENECO',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FEVL(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSEVAPLIQ',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FEVN(KIDIA:KFDIA,1)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSEVAPNEG',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%FLWSP(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FSLIQSNPL',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%RUISS(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FRUISSURF',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*YDMF_PHYS%RUISP(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FRUISSPRF',YDDDH)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,YDMF_PHYS%FRTHDS,'FSRAYTHDS',YDDDH)
      ZTMPAS(KIDIA:KFDIA)=-YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*RLMLT*YDMF_PHYS%FONTE(KIDIA:KFDIA)
      CALL NEW_ADD_FIELD_2D(YDMODEL%YRML_DIAG%YRMDDH,ZTMPAS,'FFONTESNE',YDDDH)

      ZEPSA = 1.E-4_JPRB
      DO JLON=KIDIA, KFDIA
        ZTMPAS(JLON)=YDMF_PHYS%FRSO(JLON,KLEV,1)/MAX(1.0_JPRB-YDMF_PHYS%ALB(JLON),ZEPSA)
      ENDDO
    ...
    ELSE  
      ...
      ! surface free-style variables
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%TCLS,'VTCLS','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%QCLS,'VQCLS','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%UCLS,'VUCLS','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%VCLS,'VVCLS','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%NUCLS,'VNUCLS','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%NVCLS,'VNVCLS','S','ARP',.TRUE.,.TRUE.)
      ZTMPAS(KIDIA:KFDIA)=PAPHI(KIDIA:KFDIA,KLEV)
      CALL ADD_FIELD_2D(YDLDDH,ZTMPAS,'VSPHI','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%GZ0,'VSGZ0','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%GZ0H,'VSGZH','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%ALB,'VSALB','S','ARP',.TRUE.,.TRUE.)
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%CLPH,'VSCPH','S','ARP',.TRUE.,.TRUE.)
      ...
      CALL ADD_FIELD_2D(YDLDDH,YDMF_PHYS%FRTHDS,'FSRAYTHDS','F','ARP',.TRUE.,.TRUE.)
      ZTMPAS(KIDIA:KFDIA)=-YDMF_PHYS_SURF%GSD_VF%PLSM(KIDIA:KFDIA)*RLMLT*YDMF_PHYS%FONTE(KIDIA:KFDIA)
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     CALL ACEVADCAPE(KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%FPLSL,YDMF_PHYS%FPLSN,YDMF_PHYS%FPLCL,YDMF_PHYS%FPLCN,PAPRS,PAPRSF,PT,PCP,PAPHIF,PAPHI,ZDCAPE)
     ! Gusts.
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, KIDIA,KFDIA, KLON, KLEV,&
      & YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, PU, PV, YDMF_PHYS%CAPE, ZDCAPE, ZTKE1, PAPHIF, POROG, YDMF_PHYS%UGST, YDMF_PHYS%VGST, ZBLH, KCLPH)
     YDMF_PHYS%CLPH(KIDIA:KFDIA)=MIN(XMAXLM,MAX(XMINLM,ZBLH(KIDIA:KFDIA)))
  ENDIF

  IF (LXVISI.OR.LXVISI2) THEN
     CALL ACVISIH(YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI,KIDIA,KFDIA,KLON,KTDIA,KLEV,PAPHI,PAPHIF,PAPRSF,&
      & PT,PR,PQLI,PQICE,ZQR,ZQS,ZQGM,YDMF_PHYS%VISICLD, YDMF_PHYS%VISIHYD,YDMF_PHYS%MXCLWC)
  ENDIF
...
ZZDEP=0.0_JPRB
  ...
  DO JLEV = 1, KLEV
    DO JLON = KIDIA,KFDIA
      ZEVAP(JLON,1,JLEV)=YDMF_PHYS%FPEVPSL(JLON,JLEV)&
       & +YDMF_PHYS%FPEVPCL(JLON,JLEV)
    ENDDO
  ENDDO
...
IF(LMUSCLFA) THEN
  DO JLEV=0,KLEV
    DO JLON=1,KLON
      ZFPLS(JLON,JLEV)=YDMF_PHYS%FPLSL(JLON,JLEV)+YDMF_PHYS%FPLSN(JLON,JLEV)
      ZFPLC(JLON,JLEV)=YDMF_PHYS%FPLCL(JLON,JLEV)+YDMF_PHYS%FPLCN(JLON,JLEV)
...
IF ( (.NOT. LGPCMT) ) THEN
   ...
   DO JLEV=1,KLEV
     DO JLON= KIDIA, KFDIA
       PQRCONV1(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%FPLCL(JLON,JLEV)) * ZCOEFRAIN(1)) ** ZCOEFRAIN(2) )
       PQSCONV1(JLON,JLEV)  = 0.001_JPRB * ( (ABS(YDMF_PHYS%FPLCN(JLON,JLEV)) * ZCOEFSNOW(1)) ** ZCOEFSNOW(2) )    
     ENDDO
   ENDDO
...
CALL PPWETPOINT(YDPHY,KIDIA,KFDIA,KLON,ZPCLS,YDMF_PHYS%TCLS,&
  & PQ(:,KLEV),PQL(:,KLEV),PQI(:,KLEV),YDMF_PHYS%TPWCLS)
...
IF (LDPRECIPS .OR. LDPRECIPS2) THEN
  ...
  DO JLON=1,KLON
      ZFPLS(JLON,KLEV)=YDMF_PHYS%FPLCN(JLON,KLEV)+YDMF_PHYS%FPLSN(JLON,KLEV)
      ZFPLC(JLON,KLEV)=YDMF_PHYS%FPLCL(JLON,KLEV)+YDMF_PHYS%FPLSL(JLON,KLEV)
  ...
  IF (LDPRECIPS) THEN
   ...
   CALL DPRECIPS (YDPHY%YRDPRECIPS,KIDIA,KFDIA,KLON,KLEV,POROG,YDMF_PHYS%TPWCLS,YDMF_PHYS%DIAGH,PAPHIF,&
