...
& ,YDMF_PHYS_STATE%YCPG_PHY%PREHYD,YDMF_PHYS_STATE%YCPG_PHY%PREHYDF,PGELAM,PGEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_SURF%GSD_VF%PLSM,YDMF_PHYS_STATE%T,YDMF_PHYS_STATE%Q,YDMF_PHYS_STATE%YGSP_RR%T)
!     ------------------------------------------------------------------
...
!       Computation of ZRTI
  ...
  IF (LRAFTKE) THEN
    ...
    CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, YDMF_PHYS_STATE%TKE, YDMF_PHYS_STATE%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST,ZBLH,YDAPLPAR_TMP%MOC%CLPH)
  ENDIF
...
!          Clear sky radiative fluxes    (Geleyn's scheme)
  ...
  IF (LRAY.AND.NRAY == 1.AND.KNFRRC /= 0) THEN
    IF (MOD(YDCPG_DIM%KSTEP,KNFRRC) == 0) THEN
      CALL ACRANEB(YDRIP,YDMODEL%YRML_PHY_MF, &
       ...
       & YDMF_PHYS%OUT%ALB,ZALBDIR,YDAPLPAR_TMP%FLU%EMIS,YDMF_PHYS_TMP%RDG%MU0,PGEMU,PGELAM,YDMF_PHYS_TMP%RDG%MU0LU,YDMF_PHYS_STATE%YGSP_RR%T,&
...
!      7.3.1 Shallow + Deep convection
  ...
  IF (LCVPGY) THEN
    ...
    CALL ACCVIMPGY ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTCVIM,YDCPG_DIM%KFLEVG,&
     ...
     & YDAPLPAR_TMP%DSA%CPS,PGM,YDMF_PHYS_STATE%YGSP_RR%T,&
...
! convective cloudiness in case we need protection of convective cloud water.
    ...
    CALL ACNEBCOND ( YDRIP,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,LLREDPR,&
     ...
     & YDAPLPAR_TMP%MSC%QW,YDMF_PHYS_STATE%T,ZNEBCH,PGM,YDMF_PHYS_STATE%YGSP_RR%T,&
...
! Rash-Kristiansson cloud water scheme - second part.
  ...
  IF(LGPCMT) THEN
    ...
    IF(LEDMFI) THEN
      ...
      & ZQLC, ZQIC, ZQRC, ZQSC, &
      ! - INPUT 1D .
      & PGM, YDMF_PHYS_STATE%YGSP_RR%T, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, ZVETAF, &
      ! - OUTPUT 2D .
...
!DEC$ IVDEP
      ...
      CASE(2)
        CALL ACRANEB2(YDERDI,YDRIP,YDMODEL%YRML_PHY_MF,&
         & YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTRADI,YDCPG_DIM%KFLEVG,IJN,YDCPG_DIM%KSTEP,KNFRRC,&
         ...
         & ZQV,ZQCO2,YDCPG_MISC%QICE,YDCPG_MISC%QLI,ZQO3,YDMF_PHYS_STATE%T,&
         & YDMF_PHYS%OUT%ALB,ZALBDIR,YDAPLPAR_TMP%FLU%EMIS,PGELAM,PGEMU,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_TMP%RDG%MU0LU,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD,ZCLCT_RAD,&
...
!    IF (NCALLRAD==3) ! <== not yet
    ...
    IF (LLCALLRAD) THEN
      CALL RECMWF(YDGEOMETRY%YRDIMV,YDMODEL,     &
       ...
       &  PGELAM, PGEMU, PGPAR, YDMF_PHYS_TMP%RDG%MU0LU , YDMF_PHYS%OUT%ALB, YDMF_PHYS%RAD%RMOON)
    ELSE
      IF (LMSE) THEN
      DO JSG=1,NSW
    ...
    IF(LDAYD) CALL ACDAYD(YDRIP,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,PGEMU ,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)

    ! ---- Correct solar absorption as a function of pmu0.
    IF(GRSO < 1._JPRB) CALL ACRSO(YDPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KFLEVG,YDCPG_DIM%KTDIA,KSGST,PGEMU  ,YDMF_PHYS_TMP%RDG%MU0,YDMF_PHYS_STATE%YCPG_DYN%PHIF,YDMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YDMF_PHYS%OUT%FRSO)
    ...
    ELSE
      ...
      IF ( LMSE.AND.LCALLSFX ) THEN
        ...
        CALL ARO_GROUND_PARAM( YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP,&
          ...
          & YDMF_PHYS_TMP%RDG%MU0(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),YDMF_PHYS_TMP%RDG%MU0N(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),PGELAM(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA),&
  ...
  IF (L3MT.OR.LSTRAPRO) THEN
    ...
    IF (LCOEFKTKE.OR.LLREDPR) THEN
      ...
      CALL ACCDEV ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
       ...
       & ZQLI_CVPP,ZNEB_CVPP,YDMF_PHYS_SURF%GSD_VF%PLSM,YDAPLPAR_TMP%FLU%NEIJ,PGM,YDMF_PHYS_STATE%YGSP_RR%T,ZDECRD_MF,&
...
! -----------------------------------------------------
  ...
  IF ( LPROCLD ) THEN
    ...
    CALL ACPLUIZ ( YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,YDCPG_DIM%KFLEVG,&
     ...
     & YDMF_PHYS_STATE%YGSP_RR%T, YDAPLPAR_TMP%FLU%NEIJ, YDMF_PHYS_SURF%GSD_VF%PLSM, PGM, ZVETAF,&
...
!           DIFFUSION TURBULENTE/DEPOT SEC DE L'OZONE
    ...
    CALL ACDIFOZ ( YDRIP,YDPHY2,YDMODEL%YRML_PHY_MF%YRVDOZ,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,YDCPG_DIM%KTDIA,YDCPG_DIM%KFLEVG,&
     ...
     & YDAPLPAR_TMP%FLU%NEIJ,PGEMU,YDMF_PHYS_SURF%GSD_VV%PIVEG,&
     & YDMF_PHYS%OUT%FCHOZ)
  ENDIF
!*
!     ------------------------------------------------------------------
...
!     -----------------------------------------------------------------------
  ...
  IF (LECT.AND.LRAFTKE) THEN
     ...
     ZQGM=ZEPSNEB
     CALL ACCLDIA(YDXFU,YDPHY,YDPHY2,YDTOPH, YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG,&
      & YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS_STATE%U, YDMF_PHYS_STATE%V, YDMF_PHYS%OUT%CAPE, ZDCAPE, ZTKE1, YDMF_PHYS_STATE%YCPG_DYN%PHIF, POROG, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, ZBLH, YDAPLPAR_TMP%MOC%CLPH)
