!OPTIONS XOPT(NOEVAL)
SUBROUTINE APLPARS(YDGEOMETRY, YDMODEL, KIDIA , KFDIA  , KLON  ,  KTDIA , KLEV   ,&
 & KSGST  , KSTEP  ,&
 !---------------------------------------------------------------------
 ! - INPUT .
...
ASSOCIATE(YDVAB=>YDGEOMETRY%YRVAB,YDVETA=>YDGEOMETRY%YRVETA,YDVFE=>YDGEOMETRY%YRVFE,YDSTA=>YDGEOMETRY%YRSTA, &
 ...
 & YDPHY=>YDMODEL%YRML_PHY_MF%YRPHY,YDTOPH=>YDMODEL%YRML_PHY_MF%YRTOPH,YDSIMPHL=>YDMODEL%YRML_PHY_MF%YRSIMPHL, &
...
ASSOCIATE(TEQK=>YDPHY0%TEQK, &
 ...
 & LNEIGE=>YDPHY%LNEIGE, LTLADDIA=>YDMODEL%YRML_PHY_RAD%YRRCOEF%LTLADDIA, &
 & STTEM=>YDSTA%STTEM)
!     ------------------------------------------------------------------
...
IF ( LVDIFSP) THEN
  ...
  CALL ACDIFSP(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
...
!     ------------------------------------------------------------------
IF ( LGWDSP ) THEN
  CALL ACDRAGL(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
...
IF ( LCVRASP ) THEN
  CALL ACCONV(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
...
!     ------------------------------------------------------------------
IF ( LSTRASP ) THEN
  CALL ACQWLSR(YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
   & PQ,PT,PAPRSF,PDELP,&
   & PFPLSL,PFPLSN)
ENDIF
...
!        COMPUTATION OF THE FLUXES
  ...
  CALL ACRADS(YDGEOMETRY%YRVAB,YDMODEL%YRML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,IJN,&
...
IF ( LRRMESSP ) THEN
  CALL ACDRME(YDMODEL%YRML_PHY_MF%YRPHY2,YDTOPH,KIDIA,KFDIA,KLON,KTDIA,KLEV,&
