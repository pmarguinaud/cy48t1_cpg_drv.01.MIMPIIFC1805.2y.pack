...
ASSOCIATE(YDSTA=>YDGEOMETRY%YRSTA,   YDPHY=>YDML_PHY_MF%YRPHY,YDTOPH=>YDML_PHY_MF%YRTOPH,YDSIMPHL=>YDML_PHY_MF%YRSIMPHL,   &
...
& WCRIN=>YDPHY1%WCRIN,   LSTRASP=>YDSIMPHL%LSTRASP, LRRMESSP=>YDSIMPHL%LRRMESSP,   LGWDSP=>YDSIMPHL%LGWDSP,  &
& LVDIFSP=>YDSIMPHL%LVDIFSP,   LCVRASP=>YDSIMPHL%LCVRASP, LRAYSP=>YDSIMPHL%LRAYSP,   LCONDWT=>YDPHY%LCONDWT, &
& LMPHYS=>YDPHY%LMPHYS,   LNEIGE=>YDPHY%LNEIGE, LTLADDIA=>YDRCOEF%LTLADDIA)
!     ------------------------------------------------------------------
...
!=PARALLEL
  ...
  ENDIF
  IF (LRAYSP.AND.(YDCPG_OPTS%NSTEP > 1.OR.LTLADDIA)) THEN
...
!        EVAPORATION OF THE SURFACE
  ...
  DO JLEV=KTDIA,YDCPG_OPTS%KFLEVG
    DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
      PCVGQ(JLON,JLEV)=PCVGQ(JLON,JLEV)/(1.0_JPRB+YDVARS%GEOMETRY%GM%T0(JLON)*TEQK)&
