...
ASSOCIATE(NTSSG=>YDDPHY%NTSSG, LTRAJPS=>YDSIMPHL%LTRAJPS,       &
...
IF (LRAYSP.AND.(YDCPG_DIM%KSTEP >= INSTEP_DEB .AND. YDCPG_DIM%KSTEP <= INSTEP_FIN)) THEN  
  CALL CPPSOLAN(YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDVARS%GEOMETRY%GEMU%T0, YDVARS%GEOMETRY%GELAM%T0, ZRDG_MMU0)
  IF (.NOT.LSDDH) THEN
...
CALL APLPARS(YDGEOMETRY, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,                &
