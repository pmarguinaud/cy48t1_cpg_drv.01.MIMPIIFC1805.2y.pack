...
@PROCESS NOCHECK
#endif
SUBROUTINE APLSIM(YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS, YDMF_PHYS_TMP, &
...
ASSOCIATE(TSPHY=>YDPHY2%TSPHY, NTSSG=>YDDPHY%NTSSG, YSP_SBD=>YDSURF%YSP_SBD, LTRAJPS=>YDSIMPHL%LTRAJPS,       &
...
& YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%Q, YDMF_PHYS_SURF%GSP_SG%PF_T1, YDMF_PHYS_SURF%GSP_RR%PT_T1,       &
& YDMF_PHYS_SURF%GSP_RR%PW_T1, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VF%PVEG, ZQS1)  

CALL APLPASSH (YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
& YLMF_PHYS_STATE%YCPG_PHY%PREHYD, YLMF_PHYS_STATE%Q, YLMF_PHYS_STATE%YGSP_SG%F, YLMF_PHYS_STATE%YGSP_RR%T,           &
& YLMF_PHYS_STATE%YGSP_RR%W, YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VF%PVEG, YDCPG_MISC%QS)  

CALL APLPARS(YDGEOMETRY, YDRCOEF, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON,               &
& 1, YDCPG_DIM%KFLEVG, NTSSG, YDCPG_DIM%KSTEP, YLMF_PHYS_STATE%YCPG_DYN%PHI, YLMF_PHYS_STATE%YCPG_PHY%PREHYD,          &
& YLMF_PHYS_STATE%YCPG_DYN%PHIF, YLMF_PHYS_STATE%YCPG_PHY%PREHYDF, YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,                  &
& YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%Q,      &
& YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, YDMF_PHYS_TMP%RDG%CVGQ, YLMF_PHYS_STATE%YGSP_SG%F, YLMF_PHYS_STATE%YGSP_RR%T,       &
...
& YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV,                &
& YDMF_PHYS%OUT%STRDU, YDMF_PHYS%OUT%STRDV, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%STRMU,              &
& YDMF_PHYS%OUT%STRMV, YDMF_PHYS%OUT%FRMH, YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%YCPG_DYN%PHIF,           &
& YLMF_PHYS_STATE%U, YLMF_PHYS_STATE%V, YLMF_PHYS_STATE%T, YLMF_PHYS_STATE%Q, YDCPG_MISC%QS, YLMF_PHYS_STATE%YGSP_RR%T, &
...
CALL CPUTQYS(YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDSTOPH, YDPHY2, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA,                   &
& YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG, PDTPHY, ISLB1U9, ISLB1V9, ISLB1T9, ISLB1GFL9, ZTENDH, ZTENDQ, YDMF_PHYS%OUT%TENDU,     &
& YDMF_PHYS%OUT%TENDV, YDMF_PHYS%FOR%U, YDMF_PHYS%FOR%V, YDMF_PHYS%FOR%T, YDMF_PHYS%FOR%Q, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP, &
