...
IF (LRAYSP.AND.(YDCPG_DIM%KSTEP >= INSTEP_DEB .AND. YDCPG_DIM%KSTEP <= INSTEP_FIN)) THEN  
  CALL CPPSOLAN(YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDVARS%GEOMETRY%GEMU%T0, YDVARS%GEOMETRY%GELAM%T0, &
  & ZRDG_MMU0)
  IF (.NOT.LSDDH) THEN
...
CALL CPUTQYS (YDCPG_DIM, YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDMF_PHYS, YDSTOPH, YDPHY2, &
& PDTPHY, ZTENDH, ZTENDQ)
