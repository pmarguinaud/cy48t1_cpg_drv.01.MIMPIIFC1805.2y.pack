...
IF (LRAYSP.AND.(YDCPG_DIM%KSTEP >= INSTEP_DEB .AND. YDCPG_DIM%KSTEP <= INSTEP_FIN)) THEN  
  CALL CPPSOLAN(YDGEOMETRY%YRDIM, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDGSGEOM%GEMU, YDGSGEOM%GELAM, ZRDG_MMU0&
  &     )
  IF (.NOT.LSDDH) THEN
...
& YDMF_PHYS_SURF%GSD_VF%PALBF, ZRDG_MU0, YDGSGEOM%GM, ZAC_HC, ZCOR, ZRAB3C, ZRAB3N,                        &
...
& YLMF_PHYS_BASE_STATE%YGSP_RR%T, YDOROG%OROG, YDMF_PHYS%OUT%TENDU, YDMF_PHYS%OUT%TENDV, ZTENDH,                        &
