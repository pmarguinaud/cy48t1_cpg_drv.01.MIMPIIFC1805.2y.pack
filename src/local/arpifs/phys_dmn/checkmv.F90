!OPTIONS XOPT(NOEVAL)
SUBROUTINE CHECKMV(YDRIP,YDPHY0,YDPHY2,KIDIA,KFDIA,KLON,KLEV,KSTEP,PAPHI,PAPHIF &
...
DO JLEV=1,KLEV
  DO JLON=KIDIA,KFDIA
    ...
    !-------------------------------------------------
    !
    ZTOACI=RTT+15._JPRB-0.0065*PAPHIF(JLON,JLEV)/RG
...
IF(ILEV > 0) THEN
  ...
  DO JLEV=1,KLEV
    WRITE(NULERR,FMT='(F8.2,F12.8,F10.1,F9.2,I5)') PT(ILON,JLEV),PQ(ILON,JLEV)&
      & ,PAPHIF(ILON,JLEV)/RG,PAPRSF(ILON,JLEV)/100._JPRB,JLEV
  ENDDO
  !
  !-------------------------------------------------
  ! Surface temperature.
