...
DO JLON=KIDIA,KFDIA
  DO JLEV=KLEV,ITOP(JLON),-1
    I1(JLON)=MAX(I1(JLON),JLEV*INT(MAX(0._JPRB,SIGN(1._JPRB,PTPW(JLON,JLEV)-YDCST%RTT))))
  ENDDO
ENDDO
...
DO JLON=KIDIA,KFDIA
  DO JLEV=I1(JLON),ITOP(JLON),-1
    I2(JLON)=MAX(I2(JLON),JLEV*INT(MAX(0._JPRB,SIGN(1._JPRB,YDCST%RTT-PTPW(JLON,JLEV)))))
  ENDDO
ENDDO
...
DO JLON=KIDIA,KFDIA
  DO JLEV=KLEV,ITOP(JLON),-1
    ZPROFNEG(JLON)=ZPROFNEG(JLON)+MAX(0._JPRB,SIGN(1._JPRB,PTPW(JLON,JLEV)-YDCST%RTT))
  ENDDO
ENDDO

DO JLON=KIDIA,KFDIA
  DO JLEV=KLEV,MIN(I1(JLON)+1,KLEV),-1
    ZANEG(JLON)=ZANEG(JLON)+PDZZ(JLON,JLEV)*(YDCST%RTT-PTPW(JLON,JLEV))
  ENDDO
ENDDO

DO JLON=KIDIA,KFDIA
  DO JLEV=I1(JLON),MIN(I2(JLON)+1,I1(JLON)),-1
     ZAPOS(JLON)=ZAPOS(JLON)+PDZZ(JLON,JLEV)*(PTPW(JLON,JLEV)-YDCST%RTT)
  ENDDO
ENDDO
