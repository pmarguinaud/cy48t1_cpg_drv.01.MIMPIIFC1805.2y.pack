...
IF (LNHQE) THEN
  CALL MF_PHYS_NHQE_PART1 (YDGEOMETRY, YDCPG_DIM, YDMF_PHYS, YDVARS, YDMODEL, PGFL)
ENDIF
...
IF (LLDIAB.OR.LSIMPH) THEN
  ...
  YDMF_PHYS%TMP%APLPAR%MSC%LCVQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS%TMP%APLPAR%MSC%CVGQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
ENDIF
...
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART1 (YDGEOMETRY, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL)
ENDIF

CALL INITAPLPAR (YDMF_PHYS, YDCPG_MISC, YDMF_PHYS_SURF, YGFL, YDARPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, YDCPG_DIM%KFLEVG, &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, ITDIA, NVCLIS,         &
   & YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, &
   ...
   CALL APLPAR_NEW(YDGEOMETRY, YDCPG_DIM, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF,        &
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, ITDIA, NVCLIS,         &
   & YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, &
   & YDCSGEOM%RINDY, LLXFUMSE, YDGSGEOM%RCORI, PGFL, PKOZO, PGPAR, YDGSGEOM%GELAM, YDGSGEOM%GEMU,     &
   & YDGSGEOM%GM, ZAC, ZAC_HC, YDOROG%OROG, ZDIFEXT, ICLPH, ZP1EZDIAG, ZTENDPTKE, ZTENDEXT_DEP,       &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
  DO JROF = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
    YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)/(YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,YDCPG_DIM%KFLEVG)-YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,ICLPH(JROF)-1))
  ENDDO
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KFLEVG,        &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      ...
      & YDCPG_DIM%KFLEVG, NSTEP, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZTENDH, ZPTENDQ1, &
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY0%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSEIF (LSLAG .AND. (.NOT.LTWOTL)) THEN
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSE
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
...
CALL MF_PHYS_TRANSFER (YDGEOMETRY, YDCPG_DIM, YDVARS, YDMODEL)
...
IF (LSIMPH) THEN
  ...
  CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,1,YDCPG_DIM%KFLEVG,NTSSG,NSTEP,&
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   & YDCFU, YDXFU, YDMODEL, 1, NSTEP, IMAXDRAFT, NTSSG,      &
  ...
  ELSE
    ...
    CALL CPUTQY_AROME(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,ZDT,IPGFL,IPTR,&
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  CALL MF_PHYS_SAVE_PHSURF_PART2 (YDGEOMETRY, YDCPG_DIM, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDSURF, YDMODEL)
ENDIF
...
IF (LNHQE) THEN
  CALL MF_PHYS_NHQE_PART2 (YDGEOMETRY, YDCPG_DIM, YDMF_PHYS, YDVARS)
ENDIF
