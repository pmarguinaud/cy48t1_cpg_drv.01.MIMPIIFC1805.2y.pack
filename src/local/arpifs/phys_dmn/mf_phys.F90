...
CALL INITAPLPAR (YDMF_PHYS, YDCPG_MISC, YDMF_PHYS_SURF, YGFL, YDARPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, NFLEVG, &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      IF (LAJUCV) THEN
        DO JLEV=1,NFLEVG
          ...
          ENDDO
        ENDDO
        CALL ACAJUCV(YDMODEL%YRML_PHY_MF%YRPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NTPLUI,NFLEVG,NTAJUC,&
   ...
   CALL APLPAR(YDGEOMETRY, YDCPG_DIM, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF,        &
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, ITDIA, NFLEVG, YDCPG_DIM%KSTGLO, NVCLIS,         &
  ...
  IF (LINTFLEX) THEN
    CALL APLPAR2INTFLEX(YGFL, YDPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%   &
...
IF (LLDIAB.AND.LOZONE.AND.(.NOT.LMPA)) THEN
  ...
  IF (LSLAG) THEN
    IF (LTWOTL) THEN
      CALL CPOZO (YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,YDMF_PHYS%OUT%FCHOZ,&
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_PHY0%PREHYD,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDCPG_MISC%QS,YDMF_PHYS%TMP%APLPAR%FLU%QSATS,YDCPG_MISC%QSOL)
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH, YDMDDH, YGFL, YDPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YDGSGEOM%GNORDL,      &
  ...
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG,        &
...
IF (LLDIAB.AND.(.NOT.LSIMPH)) THEN
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,&
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDPHY, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, PDTPHY,   &
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,NFLEVG
      ...
      ENDDO
    ENDDO
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, YDCPG_DIM%KLON, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YSP_SBD%NLEVS, PDTPHY, YDMF_PHYS% &
  ...
  IF(LNUDG)THEN
    ...
    CALL CPNUDG ( YDCPG_DIM%KLON, YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, NFNUDG, NFLEVG, YDCPG_DIM%KBL,&
     & XPNUDG,&
...
IF (LSIMPH) THEN
  ...
  CALL APLPASSH (YDPHY,YDMODEL%YRML_PHY_MF%YRPHY1,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,NFLEVG,&
  ...
  CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KLON,1,NFLEVG,NTSSG,NSTEP,&
...
IF (LSIMPH) THEN
  ...
  CALL CPTENDSM (YDPHY,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,&
   & YDMF_PHYS%OUT%DIFCQ,YDMF_PHYS%OUT%DIFCS,YDMF_PHYS%OUT%DIFTQ,YDMF_PHYS%OUT%DIFTS,&
   & YDMF_PHYS%OUT%FCCQL,YDMF_PHYS%OUT%FCCQN,YDMF_PHYS%OUT%FCSQL,YDMF_PHYS%OUT%FCSQN,&
...
!             ------------------------------------------
  ...
  CALL CPUTQYS(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,&
   & YDSTOPH, YDPHY2, &
   & YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,IPGFL,&
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   CALL APL_AROME(YDGEOMETRY, YDCPG_DIM, YLMF_PHYS_STATE, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDSURF, &
   & YDCFU, YDXFU, YDMODEL, YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, YDCPG_DIM%KLON, 1, NFLEVG, NSTEP, IMAXDRAFT, NTSSG,      &
  ...
  ELSE
    ...
    CALL CPUTQY_AROME(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,ZDT,IPGFL,IPTR,&
