...
IF (LL_SAVE_PHSURF) THEN
  IF(YSD_VV%YHV%LSET) YDMF_PHYS%TMP%SAV%HV(1:NPROMA)=YDMF_PHYS_SURF%GSD_VV%PHV(1:NPROMA)
  IF(YSD_VF%YZ0F%LSET) YDMF_PHYS%TMP%SAV%GZ0F(1:NPROMA)=YDMF_PHYS_SURF%GSD_VF%PZ0F(1:NPROMA)
  IF(YSD_VV%YZ0H%LSET) YDMF_PHYS%TMP%SAV%GZ0HF(1:NPROMA)=YDMF_PHYS_SURF%GSD_VV%PZ0H(1:NPROMA)
  IF(YSD_VH%YPBLH%LSET) YDMF_PHYS%TMP%SAV%PBLH(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PPBLH(1:NPROMA)
  IF(YSD_VH%YSPSH%LSET) YDMF_PHYS%TMP%SAV%FHPS(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PSPSH(1:NPROMA)
  IF(YSD_VH%YQSH%LSET) YDMF_PHYS%TMP%SAV%QSH(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PQSH(1:NPROMA)
  IF(YSD_VK%YUDGRO%LSET) YDMF_PHYS%TMP%SAV%UDGRO(1:NPROMA)=YDMF_PHYS_SURF%GSD_VK%PUDGRO(1:NPROMA)
  IF(LCVPRO.OR.LGPCMT) THEN
    ...
    ENDIF
  ENDIF
  IF(YUNEBH%LACTIVE) YDMF_PHYS%TMP%SAV%UNEBH(:,:)=YDVARS%UNEBH%T0(:,:)
  IF(YUEN%LACTIVE)   YDMF_PHYS%TMP%SAV%ENTCH(:,:)=YDVARS%UEN%T0(:,:)
ENDIF
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  IF(YSD_VV%YHV%LSET) YDMF_PHYS_SURF%GSD_VV%PHV(1:NPROMA)=YDMF_PHYS%TMP%SAV%HV(1:NPROMA)
  IF(YSD_VF%YZ0F%LSET) YDMF_PHYS_SURF%GSD_VF%PZ0F(1:NPROMA)=YDMF_PHYS%TMP%SAV%GZ0F(1:NPROMA)
  IF(YSD_VV%YZ0H%LSET) YDMF_PHYS_SURF%GSD_VV%PZ0H(1:NPROMA)=YDMF_PHYS%TMP%SAV%GZ0HF(1:NPROMA)
  IF(YSD_VH%YPBLH%LSET) YDMF_PHYS_SURF%GSD_VH%PPBLH(1:NPROMA)=YDMF_PHYS%TMP%SAV%PBLH(1:NPROMA)
  IF(YSD_VH%YSPSH%LSET) YDMF_PHYS_SURF%GSD_VH%PSPSH(1:NPROMA)=YDMF_PHYS%TMP%SAV%FHPS(1:NPROMA)
  IF(YSD_VH%YQSH%LSET)  YDMF_PHYS_SURF%GSD_VH%PQSH(1:NPROMA)=YDMF_PHYS%TMP%SAV%QSH(1:NPROMA)
  IF(YSD_VK%YUDGRO%LSET) YDMF_PHYS_SURF%GSD_VK%PUDGRO(1:NPROMA)=YDMF_PHYS%TMP%SAV%UDGRO(1:NPROMA)
  IF(LCVPRO.OR.LGPCMT) THEN
    ...
    ENDIF
  ENDIF
  IF(YUNEBH%LACTIVE) YDVARS%UNEBH%T0(:,:)=YDMF_PHYS%TMP%SAV%UNEBH(:,:)
  IF(YUEN%LACTIVE)   YDVARS%UEN%T0(:,:)=YDMF_PHYS%TMP%SAV%ENTCH(:,:)
ENDIF
