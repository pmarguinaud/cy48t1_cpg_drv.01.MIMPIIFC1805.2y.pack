...
IF (LL_SAVE_PHSURF) THEN
  !* outline MF_PHYS_SAVE_PHSURF_PROLOG
  IF(YDSURF%YSD_VV%YHV%LSET) YDMF_PHYS%TMP%SAV%HV(1:NPROMA)=YDMF_PHYS_SURF%GSD_VV%PHV(1:NPROMA)
  IF(YDSURF%YSD_VF%YZ0F%LSET) YDMF_PHYS%TMP%SAV%GZ0F(1:NPROMA)=YDMF_PHYS_SURF%GSD_VF%PZ0F(1:NPROMA)
  IF(YDSURF%YSD_VV%YZ0H%LSET) YDMF_PHYS%TMP%SAV%GZ0HF(1:NPROMA)=YDMF_PHYS_SURF%GSD_VV%PZ0H(1:NPROMA)
  IF(YDSURF%YSD_VH%YPBLH%LSET) YDMF_PHYS%TMP%SAV%PBLH(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PPBLH(1:NPROMA)
  IF(YDSURF%YSD_VH%YSPSH%LSET) YDMF_PHYS%TMP%SAV%FHPS(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PSPSH(1:NPROMA)
  IF(YDSURF%YSD_VH%YQSH%LSET) YDMF_PHYS%TMP%SAV%QSH(1:NPROMA)=YDMF_PHYS_SURF%GSD_VH%PQSH(1:NPROMA)
  IF(YDSURF%YSD_VK%YUDGRO%LSET) YDMF_PHYS%TMP%SAV%UDGRO(1:NPROMA)=YDMF_PHYS_SURF%GSD_VK%PUDGRO(1:NPROMA)
  IF(LCVPRO.OR.LGPCMT) THEN
    YDMF_PHYS%TMP%SAV%UDAL(:,:)=YDVARS%UAL%T0(:,:)
    YDMF_PHYS%TMP%SAV%UDOM(:,:)=YDVARS%UOM%T0(:,:)
    IF(LCDDPRO) THEN
      YDMF_PHYS%TMP%SAV%DDAL(:,:)=YDVARS%DAL%T0(:,:)
      YDMF_PHYS%TMP%SAV%DDOM(:,:)=YDVARS%DOM%T0(:,:)
    ENDIF
  ENDIF
  IF(YDMODEL%YRML_GCONF%YGFL%YUNEBH%LACTIVE) YDMF_PHYS%TMP%SAV%UNEBH(:,:)=YDVARS%UNEBH%T0(:,:)
  IF(YDMODEL%YRML_GCONF%YGFL%YUEN%LACTIVE)   YDMF_PHYS%TMP%SAV%ENTCH(:,:)=YDVARS%UEN%T0(:,:)
  !* end outline
ENDIF

CALL INITAPLPAR (   YDMF_PHYS, YDCPG_MISC, YDMF_PHYS_SURF, YGFL, YDARPHY, KST, &
& KEND, NPROMA, NFLEVG, NTSSG, YSP_SBD%NLEVS, ZDIFEXT, ZFRMQ, ZCPS, ZLHS, ZRS, &
& ZLH, ZLSCPE, ZQSAT, ZQW, ZTW, ZCD, ZCDN, ZCH, ZC1, ZC2, ZEMIS, ZFEVI, ZFTKE, &
& ZFTKEI, ZFEFB1, ZFEFB2, ZFEFB3, ZNEIJ, ZVEG, ZQSATS, ICLPH, ZEDR)
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   & YDGSGEOM%RCORI, PGFL, ZCVGQ, PKOZO, ZFPLCH, ZFPLSH, PGPAR, ZMU0, ZMU0LU,     &
   & ZMU0M, ZMU0N, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, ZAC, ZAC_HC,       &
   & ZMMU0, YDOROG%OROG, ZDIFEXT, ZFRMQ, ZCPS, ZLHS, ZRS, ZLH, ZLSCPE, ZQRCONV,   &
   & ZQSCONV, ZQSAT, ZQW, ZTW, ZCD, ZCDN, ZCH, ZC1, ZC2, ZEMIS, ZFEVI, ZFTKE,     &
   & ZFTKEI, ZFEFB1, ZFEFB2, ZFEFB3, ZNEIJ, ZVEG, ZQSATS, ICLPH, ZDPRECIPS,       &
   & ZDPRECIPS2, ZP1EZDIAG, ZTENDPTKE, ZKUROV_H, ZKTROV_H, ZTENDEXT_DEP,          &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  DO JLEV=1,NFLEVG
    DO JROF = KST, KEND
      YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-YLMF_PHYS_STATE%Q(JROF,JLEV)*&
...
& YDMF_PHYS%TMP%RDT%COR  , YDMF_PHYS%TMP%RDT%RAB3C, YDMF_PHYS%TMP%RDT%RAB3N, YDMF_PHYS%TMP%RDT%RAB4C, &
& YDMF_PHYS%TMP%RDT%RAB4N, YDMF_PHYS%TMP%RDT%RAB6C, YDMF_PHYS%TMP%RDT%RAB6N, YDMF_PHYS%TMP%RDT%RAT1C, &
& YDMF_PHYS%TMP%RDT%RAT1N, YDMF_PHYS%TMP%RDT%RAT2C, YDMF_PHYS%TMP%RDT%RAT2N, YDMF_PHYS%TMP%RDT%RAT3C, &
& YDMF_PHYS%TMP%RDT%RAT3N, YDMF_PHYS%TMP%RDT%RAT4C, YDMF_PHYS%TMP%RDT%RAT4N, YDMF_PHYS%TMP%RDT%RAT5C, &
& YDMF_PHYS%TMP%RDT%RAT5N, ZAC_HC)
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  ELSE
    ...
    & KEND, NFLEVG, YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, ZDIFEXT, ZFRMQ, ZFTKE,     &
    & ZFTKEI, ZFEFB1, ZFEFB2, ZFEFB3, ZCPS, ZFHP, ZFP, ZTENDU, ZTENDV, ZTENDH,   &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      ...
      & NPROMA, KST, KEND, NFLEVG, NSTEP, ZFRMQ, ZCPS, ZTENDH, ZPTENDQ1,    &
    ...
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0,           &
       ...
       & KEND, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, ZMU0, YDOROG%OROG,    &
       & YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, ZCVGQ, ZLCVQ, ZFPLCH, &
       & ZFPLSH  )
...
IF (LSIMPH) THEN
  ...
  CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,KST,KEND,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
   ...
   & YLMF_PHYS_STATE%U,YLMF_PHYS_STATE%V,YLMF_PHYS_STATE%T,YLMF_PHYS_STATE%Q,YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,ZCVGQ,&
   ...
   & ZAC_HC,YDMF_PHYS%TMP%RDT%COR,&
   & YDMF_PHYS%TMP%RDT%RAB3C,YDMF_PHYS%TMP%RDT%RAB3N,&
   & YDMF_PHYS%TMP%RDT%RAB4C,YDMF_PHYS%TMP%RDT%RAB4N,&
   & YDMF_PHYS%TMP%RDT%RAB6C,YDMF_PHYS%TMP%RDT%RAB6N,&
   & YDMF_PHYS%TMP%RDT%RAT1C,YDMF_PHYS%TMP%RDT%RAT1N,&
   & YDMF_PHYS%TMP%RDT%RAT2C,YDMF_PHYS%TMP%RDT%RAT2N,&
   & YDMF_PHYS%TMP%RDT%RAT3C,YDMF_PHYS%TMP%RDT%RAT3N,&
   & YDMF_PHYS%TMP%RDT%RAT4C,YDMF_PHYS%TMP%RDT%RAT4N,&
   & YDMF_PHYS%TMP%RDT%RAT5C,YDMF_PHYS%TMP%RDT%RAT5N,&
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   & YDOROG%OROG, YDGSGEOM%GM, ZMU0, ZMU0LU, ZMU0M, ZMU0N, YDGSGEOM%GECLO,     &
   & YDGSGEOM%GESLO, PGP2DSDT, ZGP2DSPP, ZEDR, PGPAR, ZTENDT, ZTENDGFLR(1, 1,  &
   & IRR), ZTENDW, ZTENDGFLR(1, 1, IPTRLIMA), ZTENDGFLR(1, 1, IPTRTKE),        &
   & ZTENDGFLR(1, 1, IEFB1), ZTENDGFLR(1, 1, IEFB2), ZTENDGFLR(1, 1, IEFB3),   &
   & ZTENDGFLR(1, 1, IPTREXT), ZDPRECIPS, ZDPRECIPS2, ZP1EZDIAG, YLPROCSET,    &
   & YDDDH )
...
!   if relevant.
IF (LL_SAVE_PHSURF) THEN
  !* outline MF_PHYS_SAVE_PHSURF_EPILOG
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & NLEVS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, ZMU0, YDOROG%OROG,         &
  & YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, ZCVGQ, ZLCVQ, YDGSGEOM%     &
  & GECLO, YDGSGEOM%GESLO, ZFTKE, ZFEFB1, ZFEFB2, ZFEFB3, ZFRMQ, ZFEVI, ZCD,      &
  & ZCDN, ZCH, ZC1, ZC2, ZEMIS, ZNEIJ, ZVEG, ZCPS, ZLHS, ZRS, ZLH, ZLSCPE, ZQSAT, &
  & ZQW, ZTW, ZQSATS, ICLPH, ZFPLCH, ZFPLSH  )
ENDIF
