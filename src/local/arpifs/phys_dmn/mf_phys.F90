...
INTEGER(KIND=JPIM) :: IPQ,IPO3,IPTREXT,IPTR_CONT,IEFB1,IEFB2,IEFB3
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      ...
      & YDCPG_DIM%KFLEVG, YDCPG_DIM%KSTEP, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZTENDH, ZPTENDQ1, &
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   & YDCFU, YDXFU, YDMODEL, IMAXDRAFT, NTSSG,      &
