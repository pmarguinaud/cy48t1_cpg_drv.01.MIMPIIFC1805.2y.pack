...
@PROCESS NOCHECK
#endif
SUBROUTINE MF_PHYS(YDGEOMETRY, YDCPG_DIM, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,  &
& YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDGMV, YDSURF, YDCFU, YDXFU, &
& YDMODEL, KGL1, KGL2, LDCONFX, PDTPHY, &
...
REAL(KIND=JPRB) :: ZGWT1(YDGEOMETRY%YRDIM%NPROMA,0:YDGEOMETRY%YRDIMV%NFLEVG)     ! vertical velocity calculated by cputqy_arome before convertion in d
...
& NPROMA, NTSSG=>YDDPHY%NTSSG, NVCLIS=>YDDPHY%NVCLIS, LMDUST=>YDARPHY%LMDUST, LMPA=>YDARPHY%LMPA,     &
...
& YDTOPH%NTAJUC, NTPLUI=>YDTOPH%NTPLUI, NUMFLDS=>YGFL%NUMFLDS, LAGPHY=>YDEPHY%LAGPHY, LEPHYS=>YDEPHY  &
& %LEPHYS, LDPRECIPS=>YDPHY%LDPRECIPS, LDPRECIPS2=>YDPHY%LDPRECIPS2, NDTPRECCUR=>YDPRECIPS%           &
...
IBLK=(YDCPG_DIM%KSTGLO-1)/NPROMA+1
INSTEP_DEB=1
INSTEP_FIN=1
...
IF (LNHQE) THEN
  CALL MF_PHYS_NHQE_PART1 (YDGEOMETRY, YDMF_PHYS, YDVARS, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, PGFL)
ENDIF

IF (LRAYSP.AND.(NSTEP >= INSTEP_DEB .AND. NSTEP <= INSTEP_FIN)) THEN  
  CALL CPPSOLAN(YDGEOMETRY%YRDIM,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDGSGEOM%GEMU,YDGSGEOM%GELAM,YDMF_PHYS%TMP%RDG%MMU0)
  IF (.NOT.LSDDH) THEN
...
IF (LLDIAB.OR.LSIMPH) THEN
  ...
  & YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0, YDMF_PHYS%TMP%RDG%MU0, YDMF_PHYS%TMP%RDG%MU0LU, YDMF_PHYS%  &
  & TMP%RDG%MU0M, YDMF_PHYS%TMP%RDG%MU0N, YDMF_PHYS%TMP%APLPAR%MSC%CVGQ)
  YDMF_PHYS%TMP%APLPAR%MSC%LCVQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=YDMF_PHYS%TMP%APLPAR%MSC%CVGQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
ENDIF
...
CALL INITAPLPAR (YDMF_PHYS, YDCPG_MISC, YDMF_PHYS_SURF, YGFL, YDARPHY, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NPROMA, NFLEVG, &
& NTSSG, YSP_SBD%NLEVS, ZDIFEXT, ICLPH, ZEDR)
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      IF (LAJUCV) THEN
        DO JLEV=1,NFLEVG
          DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            YDMF_PHYS%TMP%APLPAR%ADJ%TAUX(JROF,JLEV)=YDVARS%T%T0(JROF,JLEV)
          ENDDO
        ENDDO
        CALL ACAJUCV(YDMODEL%YRML_PHY_MF%YRPHY0,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NPROMA,NTPLUI,NFLEVG,NTAJUC,&
         & YDCPG_PHY0%PREHYD,YDCPG_PHY0%XYB%ALPH,YDCPG_PHY0%XYB%DELP,&
         & YDCPG_PHY0%XYB%LNPR,YDVARS%T%T0)
        DO JLEV=1,NFLEVG
          DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            YDMF_PHYS%TMP%APLPAR%ADJ%DTAJU(JROF,JLEV)=YDVARS%T%T0(JROF,JLEV)-YDMF_PHYS%TMP%APLPAR%ADJ%TAUX(JROF,JLEV)
          ENDDO
        ENDDO
      ENDIF
   ELSE
   ...
   CALL APLPAR(YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF,        &
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NPROMA, ITDIA, NFLEVG, YDCPG_DIM%KSTGLO, NVCLIS,         &
   & YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, &
   & YDCSGEOM%RINDY, LLXFUMSE, YDGSGEOM%RCORI, PGFL, PKOZO, PGPAR, YDGSGEOM%GELAM, YDGSGEOM%GEMU,     &
   & YDGSGEOM%GM, ZAC, ZAC_HC, YDOROG%OROG, ZDIFEXT, ICLPH, ZP1EZDIAG, ZTENDPTKE, ZTENDEXT_DEP,       &
   & PTRAJ_PHYS, ZEDR, YDDDH, PFTCNS, ZGP2DSPP)

   ELSE

   CALL APLPAR_NEW  (YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF,  &
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NPROMA, ITDIA, NFLEVG, YDCPG_DIM%KSTGLO, NVCLIS,         &
   & YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, &
   & YDCSGEOM%RINDY, LLXFUMSE, YDGSGEOM%RCORI, PGFL, PKOZO, PGPAR, YDGSGEOM%GELAM, YDGSGEOM%GEMU,     &
   & YDGSGEOM%GM, ZAC, ZAC_HC, YDOROG%OROG, ZDIFEXT, ICLPH, ZP1EZDIAG, ZTENDPTKE, ZTENDEXT_DEP,       &
   ...
   ENDIF
    ...
    IF (LTWOTL) THEN
      IF (LAJUCV) THEN
        DO JLEV=1,NFLEVG
          DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            YDVARS%T%T0(JROF,JLEV)=YDMF_PHYS%TMP%APLPAR%ADJ%TAUX(JROF,JLEV)
          ENDDO
        ENDDO
        DO JLEV=1,NFLEVG
          DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
            PB1(JROF,ISLB1T9+JLEV-NFLSA)=PB1(JROF,ISLB1T9+JLEV-NFLSA)+YDMF_PHYS%TMP%APLPAR%ADJ%DTAJU(JROF,JLEV)
          ENDDO
        ENDDO
      ENDIF
    ELSE
  ...
  IF (LINTFLEX) THEN
    CALL APLPAR2INTFLEX(YGFL, YDPHY, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%   &
    ...
    & YDMF_PHYS%OUT%FCQSNG, YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%FRMH, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ,   &
    ...
    & YDMF_PHYS%TMP%APLPAR%PFL%FTKE, ZTENDPTKE, ZTENDEXT, ZTENDEXT_DEP, YLPROCSET )
  ENDIF
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  DO JLEV=1,NFLEVG
    DO JROF = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)+(YDMF_PHYS%TMP%APLPAR%MSC%LCVQ(JROF,JLEV)-YLMF_PHYS_STATE%Q(JROF,JLEV)*&
       & YLMF_PHYS_STATE%DIV(JROF,JLEV))*YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP(JROF,JLEV)&
       & *MAX(0,SIGN(1,JLEV-ICLPH(JROF)))  
    ENDDO
  ENDDO
  DO JROF = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
    YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)/(YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,NFLEVG)-YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,ICLPH(JROF)-1))
  ENDDO
ENDIF

! Store surface water flux P and E for water conservation
IF (LCORWAT) THEN
  CALL MF_PHYS_CORWAT (YDMF_PHYS, YDMF_PHYS_SURF, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG)
ENDIF
...
& YDMF_PHYS%TMP%APLPAR%RDT%COR  , YDMF_PHYS%TMP%APLPAR%RDT%RAB3C, YDMF_PHYS%TMP%APLPAR%RDT%RAB3N, YDMF_PHYS%TMP%APLPAR%RDT%RAB4C, &
& YDMF_PHYS%TMP%APLPAR%RDT%RAB4N, YDMF_PHYS%TMP%APLPAR%RDT%RAB6C, YDMF_PHYS%TMP%APLPAR%RDT%RAB6N, YDMF_PHYS%TMP%APLPAR%RDT%RAT1C, &
& YDMF_PHYS%TMP%APLPAR%RDT%RAT1N, YDMF_PHYS%TMP%APLPAR%RDT%RAT2C, YDMF_PHYS%TMP%APLPAR%RDT%RAT2N, YDMF_PHYS%TMP%APLPAR%RDT%RAT3C, &
& YDMF_PHYS%TMP%APLPAR%RDT%RAT3N, YDMF_PHYS%TMP%APLPAR%RDT%RAT4C, YDMF_PHYS%TMP%APLPAR%RDT%RAT4N, YDMF_PHYS%TMP%APLPAR%RDT%RAT5C, &
& YDMF_PHYS%TMP%APLPAR%RDT%RAT5N, ZAC_HC)
ENDIF
...
IF (LLDIAB.AND.LOZONE.AND.(.NOT.LMPA)) THEN
  ...
  IF (LSLAG) THEN
    IF (LTWOTL) THEN
      CALL CPOZO (NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,YDMF_PHYS%OUT%FCHOZ,&
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_PHY0%PREHYD,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDCPG_MISC%QS,YDMF_PHYS%TMP%APLPAR%FLU%QSATS,YDCPG_MISC%QSOL)
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH, YDMDDH, YGFL, YDPHY, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YDGSGEOM%GNORDL,      &
    ...
    & YDMF_PHYS%OUT%FHPCL, YDMF_PHYS%OUT%FHPCN, YDMF_PHYS%OUT%FHPSL, YDMF_PHYS%OUT%FHPSN, YDMF_PHYS%&
    & TMP%APLPAR%MSC%FHP, YDMF_PHYS%TMP%APLPAR%PFL%FP, YDMF_PHYS%OUT%FEPFP, YDMF_PHYS%OUT%FCMPCQ,   &
    & YDMF_PHYS%OUT%FCMPSN, YDMF_PHYS%OUT%FCMPSL, YDDDH )
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG,        &
    & YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, ZDIFEXT, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR  &
    & %PFL%FTKE, YDMF_PHYS%TMP%APLPAR%PFL%FTKEI, YDMF_PHYS%TMP%APLPAR%PFL%FEFB1, YDMF_PHYS%TMP%       &
    & APLPAR%PFL%FEFB2, YDMF_PHYS%TMP%APLPAR%PFL%FEFB3, YDMF_PHYS%TMP%APLPAR%DSA%CPS, YDMF_PHYS%TMP%  &
    & APLPAR%MSC%FHP, YDMF_PHYS%TMP%APLPAR%PFL%FP, ZTENDU, ZTENDV, ZTENDH, ZPTENDQ1, ZPTENDI1,        &
    & ZPTENDL1, ZPTENDLCONV1, ZPTENDICONV1, ZPTENDRCONV1, ZPTENDSCONV1, ZPTENDR1, ZPTENDS1, ZPTENDG1, &
    & ZPTENDTKE1, ZPTENDEFB11, ZPTENDEFB21, ZPTENDEFB31, ZTENDEXT, YDDDH)
  ENDIF
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      CALL CPCHET(    YDMF_PHYS, YLMF_PHYS_STATE, YDCPG_MISC, YDRIP, YDPHY, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA,        &
      & NFLEVG, NSTEP, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZTENDH, ZPTENDQ1, &
      & ZPTENDI1, ZPTENDL1, ZPTENDR1, ZPTENDS1, YDGSGEOM%GEMU, YDGSGEOM%GELAM  )
    ENDIF
    
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS,     &
       & YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KFDIA, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%&
       & GM, YDMF_PHYS%TMP%RDG%MU0, YDOROG%OROG, YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, &
       & YDMF_PHYS%TMP%APLPAR%MSC%CVGQ, YDMF_PHYS%TMP%APLPAR%MSC%LCVQ, YDMF_PHYS%TMP%APLPAR%PFL%   &
       & FPLCH, YDMF_PHYS%TMP%APLPAR%PFL%FPLSH  )
...
IF (LLDIAB.AND.(.NOT.LSIMPH)) THEN
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY0%PREHYD(:,NFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSEIF (LSLAG .AND. (.NOT.LTWOTL)) THEN
      CALL CPMVVPS(YDVAB,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,NFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSE
      CALL CPMVVPS(YDVAB,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,&
       & YDMF_PHYS%TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,NFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDPHY, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, PDTPHY,   &
...
CALL MF_PHYS_TRANSFER (YDGEOMETRY, YDVARS, YDMODEL, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA)
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,NFLEVG
      DO JROF=YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA
        YDMF_PHYS%TMP%APLPAR%PFL%FPLSN(JROF,JLEV)=YDMF_PHYS%OUT%FPLSN(JROF,JLEV)+YDMF_PHYS%OUT%FPLSG(JROF,JLEV)
      ENDDO
    ENDDO
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NFLEVG, YSP_SBD%NLEVS, PDTPHY, YDMF_PHYS% &
    & OUT%FPLCL, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%TMP%APLPAR%PFL%FPLSN, YDMF_PHYS&
    & %OUT%FRSO, YDMF_PHYS%OUT%FRTH, YDMF_PHYS_SURF%GSP_SG%PA_T1, YDMF_PHYS%OUT%CT, YDMF_PHYS%TMP%  &
    & APLPAR%DSA%C1, YDMF_PHYS%TMP%APLPAR%DSA%C2, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS&
    & %OUT%FCLN, YDMF_PHYS%OUT%FCS, YDMF_PHYS%TMP%APLPAR%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT&
    ...
    & PF_T1, YDMF_PHYS%TMP%APLPAR%FLU%VEG, YDMF_PHYS%TMP%APLPAR%TDS%TDTS, YDMF_PHYS%TMP%APLPAR%TDS% &
    & TDTP, YDMF_PHYS%TMP%APLPAR%TDS%TDWS, YDMF_PHYS%TMP%APLPAR%TDS%TDWSI, YDMF_PHYS%TMP%APLPAR%TDS%&
    & TDWP, YDMF_PHYS%TMP%APLPAR%TDS%TDWPI, YDMF_PHYS%TMP%APLPAR%TDS%TDWL, YDMF_PHYS%TMP%APLPAR%TDS%&
    & TDSNS, YDMF_PHYS%TMP%APLPAR%TDS%TDALBNS, YDMF_PHYS%TMP%APLPAR%TDS%TDRHONS)  

    CALL CPWTS(YDSURF, YDMODEL%YRML_AOC%YRMCC, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, NPROMA, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA,  &
    & YSP_SBD%NLEVS, PDTPHY, YDMF_PHYS%TMP%APLPAR%TDS%TDTS, YDMF_PHYS%TMP%APLPAR%TDS%TDTP, YDMF_PHYS  &
    & %TMP%APLPAR%TDS%TDWS, YDMF_PHYS%TMP%APLPAR%TDS%TDWSI, YDMF_PHYS%TMP%APLPAR%TDS%TDWP, YDMF_PHYS  &
    & %TMP%APLPAR%TDS%TDWPI, YDMF_PHYS%TMP%APLPAR%TDS%TDWL, YDMF_PHYS%TMP%APLPAR%TDS%TDSNS,           &
    & YDMF_PHYS%TMP%APLPAR%TDS%TDALBNS, YDMF_PHYS%TMP%APLPAR%TDS%TDRHONS, YDMF_PHYS_SURF%GSD_VP%PTPC, &
  ...
  IF(LNUDG)THEN
    ...
    CALL CPNUDG ( NPROMA, YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA, NFNUDG, NFLEVG, IBLK,&
     & XPNUDG,&
...
IF (LSIMPH) THEN
  ...
  CALL APLPASSH (YDPHY,YDMODEL%YRML_PHY_MF%YRPHY1,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NPROMA,NFLEVG,&
  ...
  CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
   ...
   & YLMF_PHYS_STATE%U,YLMF_PHYS_STATE%V,YLMF_PHYS_STATE%T,YLMF_PHYS_STATE%Q,YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,YDMF_PHYS%TMP%APLPAR%MSC%CVGQ,&
   & YLMF_PHYS_STATE%YGSP_SG%F,YLMF_PHYS_STATE%YGSP_RR%T,YDCPG_MISC%QS,&
   & YDMF_PHYS_SURF%GSP_RR%PT_T1,YDMF_PHYS%TMP%APLPAR%FLU%QS1,&
   ...
   & YDMF_PHYS_SURF%GSD_VF%PALBF,YDMF_PHYS%TMP%RDG%MU0,YDGSGEOM%GM,&
   & ZAC_HC,YDMF_PHYS%TMP%APLPAR%RDT%COR,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAB3C,YDMF_PHYS%TMP%APLPAR%RDT%RAB3N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAB4C,YDMF_PHYS%TMP%APLPAR%RDT%RAB4N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAB6C,YDMF_PHYS%TMP%APLPAR%RDT%RAB6N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAT1C,YDMF_PHYS%TMP%APLPAR%RDT%RAT1N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAT2C,YDMF_PHYS%TMP%APLPAR%RDT%RAT2N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAT3C,YDMF_PHYS%TMP%APLPAR%RDT%RAT3N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAT4C,YDMF_PHYS%TMP%APLPAR%RDT%RAT4N,&
   & YDMF_PHYS%TMP%APLPAR%RDT%RAT5C,YDMF_PHYS%TMP%APLPAR%RDT%RAT5N,&
...
IF (LSIMPH) THEN
  ...
  CALL CPTENDSM (YDPHY,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,&
   & YDMF_PHYS%OUT%DIFCQ,YDMF_PHYS%OUT%DIFCS,YDMF_PHYS%OUT%DIFTQ,YDMF_PHYS%OUT%DIFTS,&
   & YDMF_PHYS%OUT%FCCQL,YDMF_PHYS%OUT%FCCQN,YDMF_PHYS%OUT%FCSQL,YDMF_PHYS%OUT%FCSQN,&
...
!             ------------------------------------------
  ...
  CALL CPUTQYS(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,&
   & YDSTOPH, YDPHY2, &
   & NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,PDTPHY,IPGFL,&
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   CALL APL_AROME(YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDSURF, &
   & YDCFU, YDXFU, YDMODEL, YDCPG_DIM%KBL, YDCPG_DIM%KGPCOMP, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA, NPROMA, 1, NFLEVG, NSTEP, IMAXDRAFT, NTSSG,      &
   & YDCFU%NFRRC, PDTPHY, LLXFUMSE, YDCSGEOM%RINDX, YDCSGEOM%RINDY, YDGSGEOM%GEMU, YDGSGEOM%GELAM,    &
   & YDOROG%OROG, YDGSGEOM%GM, YDGSGEOM%GECLO, YDGSGEOM%GESLO, PGP2DSDT, ZGP2DSPP, ZEDR, PGPAR,       &
   & ZTENDT, ZTENDGFLR(:, :, IRR), ZTENDW, ZTENDGFLR(:, :, IPTRLIMA), ZTENDGFLR(:, :, IPTRTKE),       &
   & ZTENDGFLR(:, :, IEFB1), ZTENDGFLR(:, :, IEFB2), ZTENDGFLR(:, :, IEFB3), ZTENDGFLR(:, :, IPTREXT  &
   & ), ZP1EZDIAG, YLPROCSET, YDDDH )
  ...
  IF(LNHDYN) THEN
    ...
    IF (LGWADV) THEN
      ZTENDD(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=ZGWT1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
    ELSE

      ! * Provide the appropriate version of (RT) at t+dt for GNHGW2SVDAROME:
      IF (L_RDRY_VD) THEN
        ! Use Rd because "dver" is currently defined with Rd.
        ZRTT1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)=RD*ZTT1(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:NFLEVG)
      ELSE
  ...
  ELSE
    ...
    CALL CPUTQY_AROME(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,NPROMA,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,NFLEVG,ZDT,IPGFL,IPTR,&
...
IF (L3DTURB) THEN
  DO JLEV=1,NFLEVG
    PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2KAPPAM+JLEV-1)=YDMF_PHYS%TMP%APLPAR%KUR%KUROV_H(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
    PB2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,MSLB2KAPPAH+JLEV-1)=YDMF_PHYS%TMP%APLPAR%KUR%KTROV_H(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,JLEV)
  ENDDO
ENDIF
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, YDCPG_DIM%KFDIA, YDCPG_DIM%KIDIA, KGL1, KGL2,        &
  & YDCPG_DIM%KSTGLO, NSTEP, NTSSG, YSP_SBD%      NLEVS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, YDOROG%    &
...
IF (LDPRECIPS) THEN
  YDMF_PHYS_SURF%GSD_XP%PPRECIP(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,NDTPRECCUR)=YDMF_PHYS%TMP%PRC%DPRECIPS(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,NDTPRECCUR)
ENDIF

IF (LDPRECIPS2) THEN
  YDMF_PHYS_SURF%GSD_XP2%PPRECIP2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,NDTPRECCUR2)=YDMF_PHYS%TMP%PRC%DPRECIPS2(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,NDTPRECCUR2)
ENDIF

! Restore Tt and grad(Tt) for NHQE model.
IF (LNHQE) THEN
  CALL MF_PHYS_NHQE_PART2 (YDGEOMETRY, YDMF_PHYS, YDVARS, YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA)
ENDIF
