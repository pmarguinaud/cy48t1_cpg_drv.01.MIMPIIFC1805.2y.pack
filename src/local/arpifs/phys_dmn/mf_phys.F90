...
IF (LLDIAB.OR.LSIMPH) THEN
  CALL CPPHINP(YDGEOMETRY,YDMODEL,KST,KEND,&
   ...
   & YDMF_PHYS%TMP%RDG%MU0,YDMF_PHYS%TMP%RDG%MU0LU,YDMF_PHYS%TMP%RDG%MU0M,YDMF_PHYS%TMP%RDG%MU0N,YDMF_PHYS%TMP%APLPAR%MSC%CVGQ)
...
& KEND, NPROMA, NFLEVG, NTSSG, YSP_SBD%NLEVS, ZDIFEXT, &
& ICLPH, ZEDR)
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   CALL APLPAR(YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS,    &
   & YDMF_PHYS_SURF, YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, KST, KEND, NPROMA,    &
   & ITDIA, NFLEVG, KSTGLO, NVCLIS, YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, &
   & KBL, KGPCOMP, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, YDCSGEOM%RINDY, LLXFUMSE, &
   & YDGSGEOM%RCORI, PGFL, PKOZO, PGPAR, &
   & YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, ZAC, ZAC_HC,       &
   & YDOROG%OROG, ZDIFEXT, &
   & ICLPH, &
   & ZP1EZDIAG, ZTENDPTKE, ZTENDEXT_DEP,          &
   & PTRAJ_PHYS, ZEDR, YDDDH, PFTCNS, ZGP2DSPP)

   ELSE

   CALL APLPAR_NEW &
   &(YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_DYN0, YDCPG_MISC, YDMF_PHYS,    &
   & YDMF_PHYS_SURF, YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, KST, KEND, NPROMA,    &
   & ITDIA, NFLEVG, KSTGLO, NVCLIS, YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, &
   & KBL, KGPCOMP, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, YDCSGEOM%RINDY, LLXFUMSE, &
   & YDGSGEOM%RCORI, PGFL, PKOZO, PGPAR, &
  ...
  IF (LINTFLEX) THEN
    CALL APLPAR2INTFLEX(YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,&
      ...
      & YDMF_PHYS%OUT%FCQNG , YDMF_PHYS%OUT%FRMH  , YDMF_PHYS%TMP%APLPAR%MSC%FRMQ  , YDMF_PHYS%OUT%FRSO  , YDMF_PHYS%OUT%FRTH  ,&
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
     & YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP ,&
     & YLMF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,&
     & YLMF_PHYS_STATE%U,YLMF_PHYS_STATE%V,YLMF_PHYS_STATE%T,YLMF_PHYS_STATE%YGSP_RR%T,&
     & PGFL,&
     & YLPROCSET,&
     ...
     & YDMF_PHYS%TMP%APLPAR%MSC%FHP   ,YDMF_PHYS%TMP%APLPAR%PFL%FP   ,  YDMF_PHYS%OUT%FEPFP, YDMF_PHYS%OUT%FCMPCQ, YDMF_PHYS%OUT%FCMPSN, YDMF_PHYS%OUT%FCMPSL,YDDDH )
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, NPROMA, KST, &
    & KEND, NFLEVG, YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, ZDIFEXT, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR%PFL%FTKE,     &
    & YDMF_PHYS%TMP%APLPAR%PFL%FTKEI, YDMF_PHYS%TMP%APLPAR%PFL%FEFB1, YDMF_PHYS%TMP%APLPAR%PFL%FEFB2, YDMF_PHYS%TMP%APLPAR%PFL%FEFB3, YDMF_PHYS%TMP%APLPAR%DSA%CPS, YDMF_PHYS%TMP%APLPAR%MSC%FHP, YDMF_PHYS%TMP%APLPAR%PFL%FP, ZTENDU, ZTENDV, ZTENDH,   &
    & ZPTENDQ1, ZPTENDI1, ZPTENDL1, ZPTENDLCONV1, ZPTENDICONV1, ZPTENDRCONV1,    &
    & ZPTENDSCONV1, ZPTENDR1, ZPTENDS1, ZPTENDG1, ZPTENDTKE1, ZPTENDEFB11,       &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      CALL CPCHET(    YDMF_PHYS, YLMF_PHYS_STATE, YDCPG_MISC, YDRIP, YDPHY, &
      & NPROMA, KST, KEND, NFLEVG, NSTEP, YDMF_PHYS%TMP%APLPAR%MSC%FRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZTENDH, ZPTENDQ1,    &
      & ZPTENDI1, ZPTENDL1, ZPTENDR1, ZPTENDS1, YDGSGEOM%GEMU, YDGSGEOM%GELAM  )
    ENDIF
    
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0,           &
       & YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF,     &
       & KEND, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, YDMF_PHYS%TMP%RDG%MU0, YDOROG%OROG,    &
       & YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, YDMF_PHYS%TMP%APLPAR%MSC%CVGQ, YDMF_PHYS%TMP%APLPAR%MSC%LCVQ, YDMF_PHYS%TMP%APLPAR%PFL%FPLCH, &
       & YDMF_PHYS%TMP%APLPAR%PFL%FPLSH  )
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,YDPHY,NPROMA,KST,KEND,NFLEVG,PDTPHY,IPGFL,&
   ...
   & YLMF_PHYS_STATE%YCPG_DYN%RCP%CP,YLMF_PHYS_STATE%YCPG_PHY%XYB%DELP,YLMF_PHYS_STATE%T,YLMF_PHYS_STATE%U,YLMF_PHYS_STATE%V,&
   & PB1, PGMVT1, PGFLT1,&
   & YDMF_PHYS%OUT%FDIS)
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    ...
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, NPROMA, KST, KEND, NFLEVG, YSP_SBD%NLEVS, PDTPHY,&
     & YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%TMP%APLPAR%PFL%FPLSN,&
     & YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,&
     & YDMF_PHYS_SURF%GSP_SG%PA_T1,YDMF_PHYS%OUT%CT, YDMF_PHYS%TMP%APLPAR%DSA%C1, YDMF_PHYS%TMP%APLPAR%DSA%C2,&
     & YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL, YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS,&
     & YDMF_PHYS%TMP%APLPAR%FLU%FEVI,YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN,&
     ...
     & YDMF_PHYS%OUT%RUISL, YDMF_PHYS%OUT%RUISP, YDMF_PHYS%OUT%RUISS, YDMF_PHYS_SURF%GSP_SG%PF_T1, YDMF_PHYS%TMP%APLPAR%FLU%VEG,&
     & YDMF_PHYS%TMP%APLPAR%TDS%TDTS, YDMF_PHYS%TMP%APLPAR%TDS%TDTP, YDMF_PHYS%TMP%APLPAR%TDS%TDWS, YDMF_PHYS%TMP%APLPAR%TDS%TDWSI, YDMF_PHYS%TMP%APLPAR%TDS%TDWP, YDMF_PHYS%TMP%APLPAR%TDS%TDWPI, YDMF_PHYS%TMP%APLPAR%TDS%TDWL,&
     & YDMF_PHYS%TMP%APLPAR%TDS%TDSNS, YDMF_PHYS%TMP%APLPAR%TDS%TDALBNS, YDMF_PHYS%TMP%APLPAR%TDS%TDRHONS)  
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   CALL APL_AROME(YDGEOMETRY, YLMF_PHYS_STATE, YDCPG_MISC, YDMF_PHYS,          &
   & YDMF_PHYS_SURF, YDVARS, YDSURF, YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KST, &
   & KEND, NPROMA, 1, NFLEVG, NSTEP, IMAXDRAFT, NTSSG, YDCFU%NFRRC, PDTPHY,    &
   & LLXFUMSE, YDCSGEOM%RINDX, YDCSGEOM%RINDY, YDGSGEOM%GEMU, YDGSGEOM%GELAM,  &
   & YDOROG%OROG, YDGSGEOM%GM, YDGSGEOM%GECLO,     &
   & YDGSGEOM%GESLO, PGP2DSDT, ZGP2DSPP, ZEDR, PGPAR, ZTENDT, ZTENDGFLR(:, :,  &
   & IRR), ZTENDW, ZTENDGFLR(:, :, IPTRLIMA), ZTENDGFLR(:, :, IPTRTKE),        &
   & ZTENDGFLR(:, :, IEFB1), ZTENDGFLR(:, :, IEFB2), ZTENDGFLR(:, :, IEFB3),   &
   & ZTENDGFLR(:, :, IPTREXT), ZP1EZDIAG, YLPROCSET,    &
   & YDDDH )
...
IF(LGSCM.OR.LMUSCLFA) THEN
  IF (LAROME) THEN
    ...
    ENDDO
  ENDIF
  CALL WRITEPHYSIO(YDGEOMETRY, YDCPG_MISC, YDCPG_PHY0, YDCPG_PHY9, YDMF_PHYS,     &
  & YDCPG_DYN0, YDCPG_DYN9, YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP,        &
  & YDMODEL%YRML_PHY_MF, KEND, KST, KGL1, KGL2, KSTGLO, NSTEP, NTSSG, YSP_SBD%    &
  & NLEVS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, YDOROG%OROG,         &
  & YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, YDGSGEOM%     &
  & GECLO, YDGSGEOM%GESLO, &
  & &
  & ICLPH  )
ENDIF
