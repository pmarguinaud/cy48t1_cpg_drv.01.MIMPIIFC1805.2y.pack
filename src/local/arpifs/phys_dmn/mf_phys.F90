...
@PROCESS NOCHECK
#endif
SUBROUTINE MF_PHYS(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDGMV,YDSURF,YDCFU,YDXFU,YDMODEL,&
 !---------------------------------------------------------------------
 ! - INPUT and INOUT.
...
REAL(KIND=JPRB), POINTER :: ZP1EXT(:,:), ZP1EXT9(:,:), ZPFQTUR(:,:)
...
ZPCPF       => YDVARS%CPF%T0
ZPCPF1      => YDVARS%CPF%T1
ZPCVGQ      => YDVARS%CVGQ%T0
ZPCVGQL     => YDVARS%CVGQ%DL
ZPCVGQM     => YDVARS%CVGQ%DM
ZPCVV       => YDVARS%CVV%T0
ZPCVV1      => YDVARS%CVV%T1
ZPCVV9      => YDVARS%CVV%T9
ZPDAL       => YDVARS%DAL%T0
ZPDAL1      => YDVARS%DAL%T1
ZPDOM       => YDVARS%DOM%T0
ZPDOM1      => YDVARS%DOM%T1
ZPEFB1      => YDVARS%EFB1%T0
CALL SC2PRG(YEFB1%MP1 ,ZTENDGFL ,ZPTENDEFB11)
ZPEFB19     => YDVARS%EFB1%T9
ZPEFB2      => YDVARS%EFB2%T0
CALL SC2PRG(YEFB2%MP1 ,ZTENDGFL ,ZPTENDEFB21)
ZPEFB29     => YDVARS%EFB2%T9
ZPEFB3      => YDVARS%EFB3%T0
CALL SC2PRG(YEFB3%MP1 ,ZTENDGFL ,ZPTENDEFB31)
ZPEFB39     => YDVARS%EFB3%T9
ZP1EXT      => YDVARS%EXT(1)%T0
ZP1EXT9     => YDVARS%EXT(1)%T9
ZPFQTUR     => YDVARS%FQTUR%T0
ZPFQTUR1    => YDVARS%FQTUR%T1
ZPFSTUR     => YDVARS%FSTUR%T0
ZPFSTUR1    => YDVARS%FSTUR%T1
ZPG         => YDVARS%G%T0
CALL SC2PRG(YG%MP1    ,ZTENDGFL ,ZPTENDG1)
ZPG9        => YDVARS%G%T9
ZPH         => YDVARS%H%T0
ZPH9        => YDVARS%H%T9
ZPICONV     => YDVARS%ICONV%T0
CALL SC2PRG(YICONV%MP1,ZTENDGFL ,ZPTENDICONV1)
ZPI         => YDVARS%I%T0
CALL SC2PRG(YI%MP1    ,ZTENDGFL ,ZPTENDI1)
ZPI9        => YDVARS%I%T9
ZPIRAD1     => YDVARS%IRAD%T1
ZPLCONV     => YDVARS%LCONV%T0
CALL SC2PRG(YLCONV%MP1,ZTENDGFL ,ZPTENDLCONV1)
ZP1LIMA     => YDVARS%LIMA(1)%T0
ZP1LIMA9    => YDVARS%LIMA(1)%T9
ZPL         => YDVARS%L%T0
CALL SC2PRG(YL%MP1    ,ZTENDGFL ,ZPTENDL1)
ZPL9        => YDVARS%L%T9
ZPLRAD1     => YDVARS%LRAD%T1
ZPMXL       => YDVARS%MXL%T0
ZPMXL1      => YDVARS%MXL%T1
ZP1NOGW     => YDVARS%NOGW(1)%T0
ZP1NOGW9    => YDVARS%NOGW(1)%T9
ZP2NOGW     => YDVARS%NOGW(2)%T0
ZP2NOGW9    => YDVARS%NOGW(2)%T9
ZPO3        => YDVARS%O3%T0
ZPO31       => YDVARS%O3%T1
ZPO39       => YDVARS%O3%T9
ZPQ         => YDVARS%Q%T0
...
ZPQL        => YDVARS%Q%DL
ZPQM        => YDVARS%Q%DM
ZPRCONV     => YDVARS%RCONV%T0
ZPRCONV1    => YDVARS%RCONV%T1
CALL SC2PRG(YRCONV%MP1,ZTENDGFL ,ZPTENDRCONV1)
ZPRKTH      => YDVARS%RKTH%T0
ZPRKTH1     => YDVARS%RKTH%T1
ZPRKTQC     => YDVARS%RKTQC%T0
ZPRKTQC1    => YDVARS%RKTQC%T1
ZPRKTQV     => YDVARS%RKTQV%T0
ZPRKTQV1    => YDVARS%RKTQV%T1
ZPR         => YDVARS%R%T0
CALL SC2PRG(YR%MP1    ,ZTENDGFL ,ZPTENDR1)
ZPR9        => YDVARS%R%T9
ZPSCONV     => YDVARS%SCONV%T0
ZPSCONV1    => YDVARS%SCONV%T1
CALL SC2PRG(YSCONV%MP1,ZTENDGFL ,ZPTENDSCONV1)
ZPSHTUR     => YDVARS%SHTUR%T0
ZPSHTUR1    => YDVARS%SHTUR%T1
ZPS         => YDVARS%S%T0
CALL SC2PRG(YS%MP1    ,ZTENDGFL ,ZPTENDS1)
ZPS9        => YDVARS%S%T9
ZPSPF       => YDVARS%SPF%T0
ZPSPF1      => YDVARS%SPF%T1
ZPSRC       => YDVARS%SRC%T0
ZPSRC1      => YDVARS%SRC%T1
ZPSRC9      => YDVARS%SRC%T9
ZPTKE       => YDVARS%TKE%T0
CALL SC2PRG(YTKE%MP1  ,ZTENDGFL ,ZPTENDTKE1)
ZPTKE9      => YDVARS%TKE%T9
ZPTTE       => YDVARS%TTE%T0
ZPTTE1      => YDVARS%TTE%T1
ZPUAL       => YDVARS%UAL%T0
ZPUAL1      => YDVARS%UAL%T1
ZPUEN       => YDVARS%UEN%T0
ZPUEN1      => YDVARS%UEN%T1
ZPUNEBH     => YDVARS%UNEBH%T0
ZPUNEBH1    => YDVARS%UNEBH%T1
ZPUOM       => YDVARS%UOM%T0
ZPUOM1      => YDVARS%UOM%T1
...
ZDIXEDR     => YDMF_PHYS_SURF%GSD_DI%PXEDR

ZSFLSFL1    => YDMF_PHYS_SURF%GSD_SFL%PGROUP 
ZSFOSFO1    => YDMF_PHYS_SURF%GSD_SFO%PGROUP

ZVADES      => YDMF_PHYS_SURF%GSD_VA%PDES
ZVALAN      => YDMF_PHYS_SURF%GSD_VA%PLAN
ZVASEA      => YDMF_PHYS_SURF%GSD_VA%PSEA
ZVASOO      => YDMF_PHYS_SURF%GSD_VA%PSOO
ZVASUL      => YDMF_PHYS_SURF%GSD_VA%PSUL
ZVAVOL      => YDMF_PHYS_SURF%GSD_VA%PVOL

ZVCVC1      => YDMF_PHYS_SURF%GSD_VC%PGROUP

ZVDSUND     => YDMF_PHYS_SURF%GSD_VD%PSUND
ZVFALBF     => YDMF_PHYS_SURF%GSD_VF%PALBF
ZVFALBSF    => YDMF_PHYS_SURF%GSD_VF%PALBSF
ZVFEMISF    => YDMF_PHYS_SURF%GSD_VF%PEMISF
ZVFGETRL    => YDMF_PHYS_SURF%GSD_VF%PGETRL
ZVFLSM      => YDMF_PHYS_SURF%GSD_VF%PLSM
ZVFNUDM     => YDMF_PHYS_SURF%GSD_VF%PNUDM
ZVFVEG      => YDMF_PHYS_SURF%GSD_VF%PVEG
ZVFVRLAN    => YDMF_PHYS_SURF%GSD_VF%PVRLAN
ZVFVRLDI    => YDMF_PHYS_SURF%GSD_VF%PVRLDI
ZVFZ0F      => YDMF_PHYS_SURF%GSD_VF%PZ0F
ZVFZ0RLF    => YDMF_PHYS_SURF%GSD_VF%PZ0RLF
ZVHBCCH     => YDMF_PHYS_SURF%GSD_VH%PBCCH
ZVHPBLH     => YDMF_PHYS_SURF%GSD_VH%PPBLH
ZVHQSH      => YDMF_PHYS_SURF%GSD_VH%PQSH
ZVHSCCH     => YDMF_PHYS_SURF%GSD_VH%PSCCH
ZVHSPSH     => YDMF_PHYS_SURF%GSD_VH%PSPSH
ZVHTCCH     => YDMF_PHYS_SURF%GSD_VH%PTCCH
ZVKUDGRO    => YDMF_PHYS_SURF%GSD_VK%PUDGRO
ZVPTPC      => YDMF_PHYS_SURF%GSD_VP%PTPC
ZVPWPC      => YDMF_PHYS_SURF%GSD_VP%PWPC
ZVVALV      => YDMF_PHYS_SURF%GSD_VV%PALV
ZVVARG      => YDMF_PHYS_SURF%GSD_VV%PARG
ZVVD2       => YDMF_PHYS_SURF%GSD_VV%PD2
ZVVHV       => YDMF_PHYS_SURF%GSD_VV%PHV
ZVVIVEG     => YDMF_PHYS_SURF%GSD_VV%PIVEG
ZVVLAI      => YDMF_PHYS_SURF%GSD_VV%PLAI
ZVVRSMIN    => YDMF_PHYS_SURF%GSD_VV%PRSMIN
ZVVSAB      => YDMF_PHYS_SURF%GSD_VV%PSAB
ZVVZ0H      => YDMF_PHYS_SURF%GSD_VV%PZ0H
ZRRFC0      => YDMF_PHYS_SURF%GSP_RR%PFC_T0
ZRRFC1      => YDMF_PHYS_SURF%GSP_RR%PFC_T1
ZRRFC9      => YDMF_PHYS_SURF%GSP_RR%PFC_T9
ZRRIC0      => YDMF_PHYS_SURF%GSP_RR%PIC_T0
ZRRIC1      => YDMF_PHYS_SURF%GSP_RR%PIC_T1
ZRRIC9      => YDMF_PHYS_SURF%GSP_RR%PIC_T9
ZRRT0       => YDMF_PHYS_SURF%GSP_RR%PT_T0
ZRRT1       => YDMF_PHYS_SURF%GSP_RR%PT_T1
ZRRT9       => YDMF_PHYS_SURF%GSP_RR%PT_T9
ZRRW0       => YDMF_PHYS_SURF%GSP_RR%PW_T0
ZRRW1       => YDMF_PHYS_SURF%GSP_RR%PW_T1
ZRRW9       => YDMF_PHYS_SURF%GSP_RR%PW_T9
ZSBQ0       => YDMF_PHYS_SURF%GSP_SB%PQ_T0
ZSBQ1       => YDMF_PHYS_SURF%GSP_SB%PQ_T1
ZSBQ9       => YDMF_PHYS_SURF%GSP_SB%PQ_T9
ZSBTL0      => YDMF_PHYS_SURF%GSP_SB%PTL_T0
ZSBTL1      => YDMF_PHYS_SURF%GSP_SB%PTL_T1
ZSBTL9      => YDMF_PHYS_SURF%GSP_SB%PTL_T9
ZSBT0       => YDMF_PHYS_SURF%GSP_SB%PT_T0
ZSBT1       => YDMF_PHYS_SURF%GSP_SB%PT_T1
ZSBT9       => YDMF_PHYS_SURF%GSP_SB%PT_T9
ZSGA0       => YDMF_PHYS_SURF%GSP_SG%PA_T0
ZSGA1       => YDMF_PHYS_SURF%GSP_SG%PA_T1
ZSGA9       => YDMF_PHYS_SURF%GSP_SG%PA_T9
ZSGF0       => YDMF_PHYS_SURF%GSP_SG%PF_T0
ZSGF1       => YDMF_PHYS_SURF%GSP_SG%PF_T1
ZSGF9       => YDMF_PHYS_SURF%GSP_SG%PF_T9
ZSGR0       => YDMF_PHYS_SURF%GSP_SG%PR_T0
ZSGR1       => YDMF_PHYS_SURF%GSP_SG%PR_T1
ZSGR9       => YDMF_PHYS_SURF%GSP_SG%PR_T9
ZSGT1       => YDMF_PHYS_SURF%GSP_SG%PT_T1
ZT0DIV      => YDVARS%DIV%T0
ZT0SP       => YDVARS%SP%T0
ZT0SPD      => YDVARS%SPD%T0
ZT0SPDL     => YDVARS%SPD%DL
ZT0SPDM     => YDVARS%SPD%DM
ZT0SPL      => YDVARS%SP%DL
ZT0SPM      => YDVARS%SP%DM
ZT0T        => YDVARS%T%T0
ZT0TL       => YDVARS%T%DL
...
ZT0V        => YDVARS%V%T0
ZT0VL       => YDVARS%V%DL
ZT0VOR      => YDVARS%VOR%T0
ZT1SP       => YDVARS%SP%T1
ZT9DIV      => YDVARS%DIV%T9
ZT9SP       => YDVARS%SP%T9
...
IF (LLDIAB.OR.LSIMPH) THEN
  CALL CPPHINP(YDGEOMETRY,YDMODEL,KST,KEND,&
   ...
   & PXYB0(1,1,YYTXYB0_PHY%M_RDELP),PCTY0(1,0,YYTCTY0%M_EVEL),YDVARS%CVGQ%T0,&
...
CALL INITAPLPAR ( YGFL, YDARPHY, KST, KEND, NPROMA, NFLEVG, NTSSG, YSP_SBD%NLEVS,&
  ...
  & YDMF_PHYS%DIFCQ , YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS , ZDIFEXT, YDMF_PHYS%DIFTQ , YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL,&
  & YDMF_PHYS%DIFTS , YDMF_PHYS%FCCQL , YDMF_PHYS%FCCQN , YDMF_PHYS%FCSQL , YDMF_PHYS%FCSQN , YDMF_PHYS%FCQNG , YDMF_PHYS%FCQNNG,&
  & YDMF_PHYS%FCQLNG, YDMF_PHYS%FCQRNG, YDMF_PHYS%FCQSNG,YDMF_PHYS%FCQGNG,&
  & YDMF_PHYS%FPLCL , YDMF_PHYS%FPLCN , YDMF_PHYS%FPLCG , YDMF_PHYS%FPLCH, YDMF_PHYS%FPLSL , YDMF_PHYS%FPLSN ,YDMF_PHYS%FPLSG ,&
  & YDMF_PHYS%FPLSH, YDMF_PHYS%FRSO  , YDMF_PHYS%FRSOC ,&
  & YDMF_PHYS%FRTH  , YDMF_PHYS%FRTHC , YDMF_PHYS%STRCU , YDMF_PHYS%STRCV , YDMF_PHYS%STRDU , YDMF_PHYS%STRDV , YDMF_PHYS%STRTU ,&
  & YDMF_PHYS%STRTV , YDMF_PHYS%STRMU , YDMF_PHYS%STRMV , YDMF_PHYS%FRMH  , ZFRMQ  ,&
  & YDMF_PHYS%DIFCQLC,YDMF_PHYS%DIFCQIC,YDMF_PHYS%FIMCC,&
  & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC, YDMF_PHYS%FCNEGQLC,YDMF_PHYS%FCNEGQIC,YDMF_PHYS%FCNEGQRC,YDMF_PHYS%FCNEGQSC,&
  & YDMF_PHYS%FCHOZ , ZCPS   , ZLHS   ,&
  & ZRS    , ZLH    , ZLSCPE , PNEB   , PQICE  , PQLI   , ZQSAT  ,&
  & ZQW    , PRH    , ZTW    , YDMF_PHYS%ALB , ZCD    , ZCDN   , ZCH    ,&
  & ZC1    , ZC2    , YDMF_PHYS%CT    , ZEMIS  , YDMF_PHYS%FCHSP , YDMF_PHYS%FCLL  , YDMF_PHYS%FCLN  ,&
  & YDMF_PHYS%FCS   , ZFEVI  , YDMF_PHYS%FEVL  , YDMF_PHYS%FEVN  , YDMF_PHYS%FEVV  , YDMF_PHYS%FLASH , YDMF_PHYS%FTR   , YDMF_PHYS%FLWSP ,&
  & YDMF_PHYS%FONTE , YDMF_PHYS%FGEL  , YDMF_PHYS%FGELS ,&
  & YDMF_PHYS%FRSGNI, YDMF_PHYS%FRSDNI, YDMF_PHYS%FRSODS, YDMF_PHYS%FRSOPS, YDMF_PHYS%FRSOPT, YDMF_PHYS%FRSOLU, YDMF_PHYS%FRTHDS,&
  & YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN,YDMF_PHYS%FPFPSG, YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN,YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPEVPCL,&
  & YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPEVPCG,ZFTKE  , ZFTKEI , ZFEFB1 , ZFEFB2   , ZFEFB3,&
  & YDMF_PHYS%GZ0   , YDMF_PHYS%GZ0H  , ZNEIJ  , ZVEG   , PQS    , ZQSATS , YDMF_PHYS%RUISL ,&
  & YDMF_PHYS%RUISP , YDMF_PHYS%RUISS , YDMF_PHYS%UCLS  , YDMF_PHYS%VCLS  , YDMF_PHYS%NUCLS , YDMF_PHYS%NVCLS , YDMF_PHYS%TCLS  , YDMF_PHYS%MRT,&
  & YDMF_PHYS%QCLS  , YDMF_PHYS%RHCLS , PCLCT  , YDMF_PHYS%CLCH  , YDMF_PHYS%CLCM  , YDMF_PHYS%CLCL  , YDMF_PHYS%CLCC  ,&
  & YDMF_PHYS%CAPE  , YDMF_PHYS%CTOP  , ICLPH  , YDMF_PHYS%CLPH  , YDMF_PHYS%VEIN  , YDMF_PHYS%UGST  , YDMF_PHYS%VGST  ,&
  & YDMF_PHYS%DIAGH , ZEDR, YDMF_PHYS%VISICLD, YDMF_PHYS%VISIHYD, YDMF_PHYS%MXCLWC)
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      ...
      IF (LAJUCV) THEN
        ...
        CALL ACAJUCV(YDMODEL%YRML_PHY_MF%YRPHY0,KST,KEND,NPROMA,NTPLUI,NFLEVG,NTAJUC,&
         ...
         & PXYB0(1,1,YYTXYB0_PHY%M_LNPR),YDVARS%T%T0)
        DO JLEV=1,NFLEVG
          DO JROF=KST,KEND
      ...
      CALL APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
       ...
       & PHI0  , PRE0  , PHIF0 , PRE0F ,PXYB0(1,1,YYTXYB0_PHY%M_ALPH),&
       & PXYB0(1,1,YYTXYB0_PHY%M_DELP),&
       & PXYB0(1,1,YYTXYB0_PHY%M_LNPR),PXYB0(1,1,YYTXYB0_PHY%M_RDELP),&
       & YDGSGEOM%RCORI, ZP1EXT,&
       & YDVARS%U%T0,YDVARS%V%T0,YDVARS%T%T0,&
       & YDVARS%Q%T0,YDVARS%I%T0,YDVARS%L%T0,&
       & YDVARS%S%T0,YDVARS%R%T0,YDVARS%G%T0,YDVARS%TKE%T0,&
       & YDVARS%EFB1%T0,YDVARS%EFB2%T0,YDVARS%EFB3%T0,&
       & YDVARS%CVV%T0,YDVARS%O3%T0,ZP1CHEM,ZP1NOGW,ZP2NOGW, PGFL, &
       & YDVARS%VOR%T0,&
       & PRCP0(1,1,YYTRCP0%M_CP), ZCVGQ  ,PRCP0(1,1,YYTRCP0%M_R), PKOZO  , ZFPLCH , ZFPLSH ,&
       & PCTY0(1,0,YYTCTY0%M_EVEL),&
       & PGPAR , &
       & YDMF_PHYS_SURF%GSP_SG%PF_T0 , YDMF_PHYS_SURF%GSP_SG%PA_T0, YDMF_PHYS_SURF%GSP_SG%PR_T0,&
       & YDMF_PHYS_SURF%GSP_SB%PT_T0 , YDMF_PHYS_SURF%GSP_RR%PT_T0 , YDMF_PHYS_SURF%GSP_RR%PFC_T0 ,&
       & YDMF_PHYS_SURF%GSP_SB%PQ_T0 , YDMF_PHYS_SURF%GSP_SB%PTL_T0, YDMF_PHYS_SURF%GSP_RR%PW_T0  ,&
       ...
       & ZMRAT5C,ZMRAT5N,&
       & YDOROG%OROG,&
       & PWT0,YDVARS%DIV%T0,YDVARS%U%DL,YDVARS%V%DL,PWT0L,PWT0M,&
       & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
       & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
       & PGRPROX,PGMIXP,PGFLUXC,PGRSURF,&
       & ZDIFEXT, &
    ...
    ELSE
      ...
      ITDIA=1_JPIM
      CALL APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,  YDXFU, YDCFU,YDMODEL,  KST,    KEND   , NPROMA ,&
       & ITDIA  , NFLEVG  , KSTGLO ,&
       & NVCLIS , YSD_VVD%NUMFLDS ,&
       & NSTEP  ,&
       & NTSSG  , YSP_SBD%NLEVS   ,&
       & KBL    , KGPCOMP, YDCFU%NFRRC, PDTPHY,YDCSGEOM%RINDX,YDCSGEOM%RINDY, LLXFUMSE,&
       & PHI9  , PRE9  , PHIF9 , PRE9F , PXYB9(1,1,YYTXYB9_PHY%M_ALPH) , &
       & PXYB9(1,1,YYTXYB9_PHY%M_DELP),&
       & PXYB9(1,1,YYTXYB9_PHY%M_LNPR),PXYB9(1,1,YYTXYB9_PHY%M_RDELP),&
       & YDGSGEOM%RCORI, ZP1EXT9,&
       & YDVARS%U%T9,YDVARS%V%T9,YDVARS%T%T9,&
       & YDVARS%Q%T9,YDVARS%I%T9,YDVARS%L%T9,&
       & YDVARS%S%T9,YDVARS%R%T9,YDVARS%G%T9,YDVARS%TKE%T9,&
       & YDVARS%EFB1%T9,YDVARS%EFB2%T9,YDVARS%EFB3%T9,&
       & YDVARS%CVV%T9,YDVARS%O3%T9,ZP1CHEM9,ZP1NOGW9,ZP2NOGW9, PGFL, &
       & YDVARS%VOR%T9,&
       & PRCP9(1,1,YYTRCP9%M_CP), ZCVGQ  ,PRCP9(1,1,YYTRCP9%M_R), PKOZO  , ZFPLCH, ZFPLSH  ,&
       & PCTY0(1,0,YYTCTY0%M_EVEL),&
       & PGPAR , &
       & YDMF_PHYS_SURF%GSP_SG%PF_T9 , YDMF_PHYS_SURF%GSP_SG%PA_T9, YDMF_PHYS_SURF%GSP_SG%PR_T9,&
       & YDMF_PHYS_SURF%GSP_SB%PT_T9 , YDMF_PHYS_SURF%GSP_RR%PT_T9 , YDMF_PHYS_SURF%GSP_RR%PFC_T9 ,&
       & YDMF_PHYS_SURF%GSP_SB%PQ_T9 , YDMF_PHYS_SURF%GSP_SB%PTL_T9, YDMF_PHYS_SURF%GSP_RR%PW_T9  ,&
       ...
       & ZMRAT5C,ZMRAT5N,&
       & YDOROG%OROG,&
       & PWT9,YDVARS%DIV%T9,YDVARS%U%DL,YDVARS%V%DL,ZWT9L,ZWT9M,&
       & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
       & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
       & PGRPROX,PGMIXP,PGFLUXC,PGRSURF,&
       & ZDIFEXT, &
  ...
  IF (LINTFLEX) THEN
    CALL APLPAR2INTFLEX(YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,&
      & YDMF_PHYS%DIFCQ , YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS ,ZDIFEXT,&
      & YDMF_PHYS%DIFTQ , YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL, YDMF_PHYS%DIFTS ,&
      & YDMF_PHYS%FCCQL , YDMF_PHYS%FCCQN , YDMF_PHYS%FCSQL , YDMF_PHYS%FCSQN ,&
      & YDMF_PHYS%FPLSL , YDMF_PHYS%FPLSN , YDMF_PHYS%FPLCL,  YDMF_PHYS%FPLCN,&
      ...
      & YDMF_PHYS%FCQLNG, YDMF_PHYS%FCQNNG, YDMF_PHYS%FCQRNG, YDMF_PHYS%FCQSNG,&
      & YDMF_PHYS%FCQNG , YDMF_PHYS%FRMH  , ZFRMQ  , YDMF_PHYS%FRSO  , YDMF_PHYS%FRTH  ,&
      & YDMF_PHYS%STRCU , YDMF_PHYS%STRCV , YDMF_PHYS%STRDU , YDMF_PHYS%STRDV ,&
      & YDMF_PHYS%STRTU , YDMF_PHYS%STRTV , YDMF_PHYS%STRMU , YDMF_PHYS%STRMV ,&
      & YDMF_PHYS%DIFCQLC,YDMF_PHYS%DIFCQIC,YDMF_PHYS%FIMCC,&
      & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC, YDMF_PHYS%FCNEGQLC,YDMF_PHYS%FCNEGQIC,YDMF_PHYS%FCNEGQRC,YDMF_PHYS%FCNEGQSC,&
      & ZFTKE,&
...
!             --------------------------
YDMF_PHYS%MOCON(KST:KEND)=0.0_JPRB
IF (LLDIAB.AND.(.NOT.LMPA)) THEN

  IF (LTWOTL) THEN
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-YDVARS%Q%T0(JROF,JLEV)*&
         & YDVARS%DIV%T0(JROF,JLEV))*PXYB0(JROF,JLEV,YYTXYB0_PHY%M_DELP)&
         & *MAX(0,SIGN(1,JLEV-ICLPH(JROF)))  
      ENDDO
    ENDDO
    DO JROF = KST, KEND
      YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)/(PRE0(JROF,NFLEVG)-PRE0(JROF,ICLPH(JROF)-1))
    ENDDO
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-YDVARS%Q%T9(JROF,JLEV)*&
         & YDVARS%DIV%T0(JROF,JLEV))*PXYB9(JROF,JLEV,YYTXYB9_PHY%M_DELP)&
         & *MAX(0,SIGN(1,JLEV-ICLPH(JROF)))  
      ENDDO
    ENDDO
    DO JROF = KST, KEND
      YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)/(PRE9(JROF,NFLEVG)-PRE9(JROF,ICLPH(JROF)-1))
    ENDDO
  ENDIF
ENDIF
...
IF (LCORWAT) THEN
  YDMF_PHYS_SURF%GSD_VH%PPSL(KST:KEND) = YDMF_PHYS%FPLSL(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPCL(KST:KEND) = YDMF_PHYS%FPLCL(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPSN(KST:KEND) = YDMF_PHYS%FPLSN(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPCN(KST:KEND) = YDMF_PHYS%FPLCN(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PEVA(KST:KEND) = YDMF_PHYS%FEVN(KST:KEND,1)-YDMF_PHYS%FEVL(KST:KEND,1)
ENDIF
...
IF (LLDIAB.AND.LOZONE.AND.(.NOT.LMPA)) THEN
  ...
  IF (LSLAG) THEN
    IF (LTWOTL) THEN
      CALL CPOZO (NPROMA,KST,KEND,NFLEVG,PDTPHY,YDMF_PHYS%FCHOZ,&
  ...
  ELSE
    CALL CPOZO (NPROMA,KST,KEND,NFLEVG,PDTPHY,YDMF_PHYS%FCHOZ,&
     & YDVARS%O3%T1,PXYB9(1,1,YYTXYB9_PHY%M_RDELP))
  ENDIF
ENDIF
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,NPROMA,KST,KEND,PRE0,YDMF_PHYS_SURF%GSP_RR%PT_T0,PQS,ZQSATS,PQSOL)
ENDIF
  ...
  IF (LTWOTL) THEN
    ...
    IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
      CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
       ...
       & PGFL,&
       & YLPROCSET,&
       & YDMF_PHYS%TENDU , YDMF_PHYS%TENDV , ZTENDH , ZTENDGFL,&
       & YDMF_PHYS%FHSCL ,YDMF_PHYS%FHSCN,YDMF_PHYS%FHSSL,YDMF_PHYS%FHSSN,&
       & YDMF_PHYS%FHPCL ,YDMF_PHYS%FHPCN,YDMF_PHYS%FHPSL,YDMF_PHYS%FHPSN,&
       & ZFHP   ,ZFP   ,  YDMF_PHYS%FEPFP, YDMF_PHYS%FCMPCQ, YDMF_PHYS%FCMPSN, YDMF_PHYS%FCMPSL,YDDDH )
    ELSE
      CALL CPTEND_NEW( YDMODEL, NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
       & YDMF_PHYS%DIFCQ , YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS ,ZDIFEXT,&
       & YDMF_PHYS%DIFTQ , YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL, YDMF_PHYS%DIFTS ,&
       & YDMF_PHYS%FCCQL , YDMF_PHYS%FCCQN , YDMF_PHYS%FCSQL , YDMF_PHYS%FCSQN ,&
       & YDMF_PHYS%FPLSL , YDMF_PHYS%FPLSN , YDMF_PHYS%FPLSG , YDMF_PHYS%FPLCL,  YDMF_PHYS%FPLCN, YDMF_PHYS%FPLCG,&
       & YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPEVPCG,&
       & YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN, YDMF_PHYS%FPFPSG, YDMF_PHYS%FPFPCL ,YDMF_PHYS%FPFPCN ,&
       & YDMF_PHYS%FCQLNG, YDMF_PHYS%FCQNNG, YDMF_PHYS%FCQRNG, YDMF_PHYS%FCQSNG, YDMF_PHYS%FCQGNG ,&
       & YDMF_PHYS%FCQNG , YDMF_PHYS%FRMH  , ZFRMQ  , YDMF_PHYS%FRSO  , YDMF_PHYS%FRTH  ,&
       & YDMF_PHYS%STRCU , YDMF_PHYS%STRCV , YDMF_PHYS%STRDU , YDMF_PHYS%STRDV ,&
       & YDMF_PHYS%STRTU , YDMF_PHYS%STRTV , YDMF_PHYS%STRMU , YDMF_PHYS%STRMV ,&
       & YDMF_PHYS%DIFCQLC,YDMF_PHYS%DIFCQIC,YDMF_PHYS%FIMCC,&
       ...
       & YDMF_PHYS%FHSCL ,YDMF_PHYS%FHSCN,YDMF_PHYS%FHSSL,YDMF_PHYS%FHSSN,YDMF_PHYS%FHSSG,&
       & YDMF_PHYS%FHPCL ,YDMF_PHYS%FHPCN,YDMF_PHYS%FHPCG,YDMF_PHYS%FHPSL,YDMF_PHYS%FHPSN,YDMF_PHYS%FHPSG,&
       & ZFHP   ,ZFP   ,  YDMF_PHYS%FEPFP, YDMF_PHYS%FCMPCQ, YDMF_PHYS%FCMPSN, YDMF_PHYS%FCMPSL,&
       & YDMF_PHYS%TENDU , YDMF_PHYS%TENDV , ZTENDU, ZTENDV, ZTENDH ,&
...
!     PFEPFP was ZFEPFP in CPTEND_NEW, before, ZFEPFP still in CPFHPFS
      DO JLEV= 0, NFLEVG 
        DO JROF = 1, NPROMA
          YDMF_PHYS%FEPFP(JROF,JLEV) = 0.0_JPRB
          YDMF_PHYS%FCMPCQ(JROF,JLEV) = 0.0_JPRB
          YDMF_PHYS%FCMPSN(JROF,JLEV) = 0.0_JPRB
          YDMF_PHYS%FCMPSL(JROF,JLEV) = 0.0_JPRB
        ENDDO
      ENDDO
    ENDIF
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      IF (LTWOTL) THEN
        ...
        & YDMF_PHYS%FHSCL , YDMF_PHYS%FHSCN , YDMF_PHYS%FHSSL , YDMF_PHYS%FHSSN ,&
        & YDMF_PHYS%FHPCL , YDMF_PHYS%FHPCN , YDMF_PHYS%FHPSL , YDMF_PHYS%FHPSN ,&
        & YDMF_PHYS%DIFCQ , YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS ,&
        & YDMF_PHYS%DIFTQ , YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL, YDMF_PHYS%DIFTS ,&
        & YDMF_PHYS%FCCQL , YDMF_PHYS%FCCQN , YDMF_PHYS%FCSQL , YDMF_PHYS%FCSQN ,&
        & YDMF_PHYS%FPLCL , YDMF_PHYS%FPLCN , YDMF_PHYS%FPLSL , YDMF_PHYS%FPLSN ,&
        & YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN, YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN,&
        & YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,&
        & YDMF_PHYS%FRMH  , ZFRMQ  , YDMF_PHYS%FRSO  , YDMF_PHYS%FRTH  ,&
        & YDMF_PHYS%STRCU , YDMF_PHYS%STRCV , YDMF_PHYS%STRDU , YDMF_PHYS%STRDV ,&
        & YDMF_PHYS%STRTU , YDMF_PHYS%STRTV , YDMF_PHYS%STRMU , YDMF_PHYS%STRMV ,&
        & PXYB0(1,1,YYTXYB0_PHY%M_RDELP), PRCP0(1,1,YYTRCP0%M_CP),&
        & YDVARS%T%T0,YDVARS%Q%T0,YDVARS%I%T0,YDVARS%L%T0,&
        & ZCPS   , YDMF_PHYS_SURF%GSP_RR%PT_T0 , PQS  ,&
        & YDMF_PHYS%TENDU , YDMF_PHYS%TENDV , ZTENDH ,&
        & ZPTENDQ1,ZPTENDI1,ZPTENDL1,&
        & ZPTENDR1,ZPTENDS1,&
      ...
      ELSE
        ...
        & ZCPS   , YDMF_PHYS_SURF%GSP_RR%PT_T9  , PQS    ,&
        & YDMF_PHYS%TENDU , YDMF_PHYS%TENDV , ZTENDH ,&
        & ZPTENDQ1,ZPTENDI1,ZPTENDL1,&
        & ZPTENDR1,ZPTENDS1,&
    ...
    IF (GCHETN%LPROFV)&
     ...
     & YDMF_PHYS_SURF%GSD_VV%PLAI, YDMF_PHYS%CT, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,&
     ...
     & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS, YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL,&
     & YDMF_PHYS%DIFTS, YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FCSQL, YDMF_PHYS%FCSQN, YDMF_PHYS%FCQNG, YDMF_PHYS%FCQNNG,&
     & YDMF_PHYS%FCQLNG, YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN, YDMF_PHYS%FRSO, YDMF_PHYS%FRTH,&
     & YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, YDMF_PHYS%STRDU, YDMF_PHYS%STRDV, YDMF_PHYS%STRTU, YDMF_PHYS%STRTV, YDMF_PHYS%STRMU, YDMF_PHYS%STRMV,&
     & YDMF_PHYS%FRMH, YDMF_PHYS%FCHOZ, PNEB, PQICE, PQLI, PRH,&
     & YDMF_PHYS%FCS, YDMF_PHYS%FCLL, YDMF_PHYS%FCLN, YDMF_PHYS%FEVL, YDMF_PHYS%FEVN, YDMF_PHYS%FEVV, YDMF_PHYS%FTR, YDMF_PHYS%FLWSP, YDMF_PHYS%FONTE,&
     & YDMF_PHYS%FGEL, YDMF_PHYS%FGELS, YDMF_PHYS%FRSODS, YDMF_PHYS%FRSOPS, YDMF_PHYS%FRSOPT, YDMF_PHYS%FRTHDS,&
     & PQS, YDMF_PHYS%RUISL, YDMF_PHYS%RUISP, YDMF_PHYS%RUISS,&
     & YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, YDMF_PHYS%TCLS, YDMF_PHYS%QCLS, YDMF_PHYS%RHCLS,&
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,PRE0(1,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & PCTY0(1,0,YYTCTY0%M_EVEL),PCTY0(1,0,YYTCTY0%M_PSDVBC),PB1(1,MSLB1SP9))
    ELSEIF (LSLAG .AND. (.NOT.LTWOTL)) THEN
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,PRE9(1,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & PCTY0(1,0,YYTCTY0%M_EVEL),PCTY0(1,0,YYTCTY0%M_PSDVBC),PB1(1,MSLB1SP9))
    ELSE
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,PRE9(1,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & PCTY0(1,0,YYTCTY0%M_EVEL),PCTY0(1,0,YYTCTY0%M_PSDVBC),YDVARS%SP%T1)
    ENDIF
  ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,YDPHY,NPROMA,KST,KEND,NFLEVG,PDTPHY,IPGFL,&
   ...
   & ZTENDH, ZTENDT, YDMF_PHYS%TENDU, YDMF_PHYS%TENDV, ZTENDU, ZTENDV, ZTENDD, ZTENDGFL,&
   & PRCP0(1,1,YYTRCP0%M_CP),PXYB0(1,1,YYTXYB0_PHY%M_DELP),YDVARS%T%T0,YDVARS%U%T0,YDVARS%V%T0,&
   & PRCP9(1,1,YYTRCP9%M_CP),PXYB9(1,1,YYTXYB9_PHY%M_DELP),YDVARS%T%T9,YDVARS%U%T9,YDVARS%V%T9,&
   & PB1, PGMVT1, PGFLT1,&
   & YDMF_PHYS%FDIS)
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,NFLEVG
      DO JROF=KST,KEND
        ZFPLSN(JROF,JLEV)=YDMF_PHYS%FPLSN(JROF,JLEV)+YDMF_PHYS%FPLSG(JROF,JLEV)
      ENDDO
    ENDDO
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, NPROMA, KST, KEND, NFLEVG, YSP_SBD%NLEVS, PDTPHY,&
     & YDMF_PHYS%FPLCL, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLCN, ZFPLSN,&
     & YDMF_PHYS%FRSO, YDMF_PHYS%FRTH,&
     & YDMF_PHYS_SURF%GSP_SG%PA_T1,YDMF_PHYS%CT, ZC1, ZC2,&
     & YDMF_PHYS%FCHSP, YDMF_PHYS%FCLL, YDMF_PHYS%FCLN, YDMF_PHYS%FCS,&
     & ZFEVI,YDMF_PHYS%FEVL, YDMF_PHYS%FEVN,&
     & YDMF_PHYS%FEVV, YDMF_PHYS%FGEL, YDMF_PHYS%FGELS, YDMF_PHYS%FLWSP, YDMF_PHYS%FONTE, YDMF_PHYS%FTR,&
     & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSP_SG%PR_T1,&
     & YDMF_PHYS%RUISL, YDMF_PHYS%RUISP, YDMF_PHYS%RUISS, YDMF_PHYS_SURF%GSP_SG%PF_T1, ZVEG,&
  ...
  IF(LNUDG)THEN
    ...
    CALL CPNUDG ( NPROMA, KST,KEND, NFNUDG, NFLEVG, IBLK,&
     ...
     & YDVARS%V%T0,PRE0(1,NFLEVG),YDGSGEOM%GM,YDMF_PHYS_SURF%GSD_VF%PLSM)
  ENDIF
ENDIF
...
IF (LSIMPH) THEN
  ...
  IF (LTWOTL) THEN
    ...
    CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,KST,KEND,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
     ...
     & YDVARS%U%T0,YDVARS%V%T0,YDVARS%T%T0,YDVARS%Q%T0,PRCP0(1,1,YYTRCP0%M_CP),ZCVGQ,&
     & YDMF_PHYS_SURF%GSP_SG%PF_T0,YDMF_PHYS_SURF%GSP_RR%PT_T0,PQS,&
     ...
     & YDMF_PHYS%STRMU,YDMF_PHYS%STRMV,YDMF_PHYS%FRMH)
  ...
  ELSE
    ...
    CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,KST,KEND,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
     ...
     & YDVARS%U%T9,YDVARS%V%T9,YDVARS%T%T9,YDVARS%Q%T9,PRCP9(1,1,YYTRCP9%M_CP),ZCVGQ,&
     & YDMF_PHYS_SURF%GSP_SG%PF_T0,YDMF_PHYS_SURF%GSP_RR%PT_T9,PQS,&
     & YDMF_PHYS_SURF%GSP_RR%PT_T1,ZQS1,&
     & YDMF_PHYS_SURF%GSD_VF%PGETRL,YDMF_PHYS_SURF%GSD_VF%PLSM,&
     & YDMF_PHYS_SURF%GSD_VF%PZ0F,YDMF_PHYS_SURF%GSD_VF%PVRLAN,YDMF_PHYS_SURF%GSD_VF%PVRLDI,&
     & YDMF_PHYS_SURF%GSD_VF%PALBF,ZMU0,YDGSGEOM%GM,&
...
IF (LSIMPH) THEN
  ...
  IF (LTWOTL) THEN
    CALL CPTENDSM (YDPHY,NPROMA,KST,KEND,NFLEVG,&
     ...
     & YDMF_PHYS%FRSO,YDMF_PHYS%FRTH,&
     & YDMF_PHYS%STRCU,YDMF_PHYS%STRCV,YDMF_PHYS%STRDU,YDMF_PHYS%STRDV,&
     & YDMF_PHYS%STRTU,YDMF_PHYS%STRTV,&
     & YDMF_PHYS%STRMU,YDMF_PHYS%STRMV,YDMF_PHYS%FRMH,&
     & PXYB0(1,1,YYTXYB0_PHY%M_RDELP),PHIF0,&
     & YDVARS%U%T0,YDVARS%V%T0,YDVARS%T%T0,YDVARS%Q%T0,&
     & PQS,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDOROG%OROG,&
     & YDMF_PHYS%TENDU,YDMF_PHYS%TENDV,ZTENDH,ZTENDQ,&
     & YDMF_PHYS%FHPCL,YDMF_PHYS%FHPCN,YDMF_PHYS%FHPSL,YDMF_PHYS%FHPSN,&
     & YDMF_PHYS%FHSCL,YDMF_PHYS%FHSCN,YDMF_PHYS%FHSSL,YDMF_PHYS%FHSSN)  
  ELSE
    CALL CPTENDSM (YDPHY,NPROMA,KST,KEND,NFLEVG,&
     ...
     & PQS,YDMF_PHYS_SURF%GSP_RR%PT_T9,YDOROG%OROG,&
     & YDMF_PHYS%TENDU,YDMF_PHYS%TENDV,ZTENDH,ZTENDQ,&
     & YDMF_PHYS%FHPCL,YDMF_PHYS%FHPCN,YDMF_PHYS%FHPSL,YDMF_PHYS%FHPSN,&
     & YDMF_PHYS%FHSCL,YDMF_PHYS%FHSCN,YDMF_PHYS%FHSSL,YDMF_PHYS%FHSSN)  
  ENDIF
...
IF (LSIMPH) THEN
  ...
  CALL CPUTQYS(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,&
   ...
   & ZTENDH, ZTENDQ, YDMF_PHYS%TENDU, YDMF_PHYS%TENDV,&
   & PFORCEU, PFORCEV, PFORCET, PFORCEQ, &
   & PRCP0(1,1,YYTRCP0%M_CP),PXYB0(1,1,YYTXYB0_PHY%M_DELP),YDVARS%T%T0,YDVARS%U%T0,YDVARS%V%T0,&
   & PRCP9(1,1,YYTRCP9%M_CP),PXYB9(1,1,YYTXYB9_PHY%M_DELP),YDVARS%T%T9,YDVARS%U%T9,YDVARS%V%T9,&
   & PB1, PGMVT1, PGFLT1,&
...
IF (LMPA) THEN
  ...
  IF(LTWOTL) THEN
    CALL APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     & 1      , NFLEVG , NSTEP  ,&
     ...
     & YDVARS%G%T0,YDVARS%H%T0,ZP1LIMA,YDVARS%TKE%T0,&
     & YDVARS%EFB1%T0,YDVARS%EFB2%T0,YDVARS%EFB3%T0,&
     & YDVARS%SRC%T0,ZP1EXT,&
     & YDVARS%U%T0,YDVARS%V%T0, PWT0,ZEDR,&
     & PFORCEU,PFORCEV,PFORCET,PFORCEQ,&
     ...
     & YLPROCSET,YDDDH )
  ELSE    
    CALL APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     & 1      , NFLEVG , NSTEP  ,&
     & IMAXDRAFT, NTSSG, YDCFU%NFRRC, PDTPHY, LLXFUMSE,YDCSGEOM%RINDX,YDCSGEOM%RINDY,&
     & YDGSGEOM%GEMU,YDGSGEOM%GELAM,YDOROG%OROG,YDGSGEOM%GM,&
     & ZMU0,ZMU0LU,ZMU0M,ZMU0N,&
     ...
     & YDVARS%U%T9,YDVARS%V%T9, PWT9,ZEDR ,&
     & PFORCEU,PFORCEV,PFORCET,PFORCEQ,&
     & PGPAR,PEMTD,PEMTU,PTRSW,&
     & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
     & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
  ...
  ELSE
    ...
    CALL CPUTQY_AROME(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,NPROMA,KST,KEND,NFLEVG,ZDT,IPGFL,IPTR,&
       ...
       & ZTENDT, ZTENDGFLR(:,:,1), YDMF_PHYS%TENDU, YDMF_PHYS%TENDV, ZTENDD ,&
       & PB1, PGMVT1, PGFLT1)
  ENDIF
...
IF(LGSCM.OR.LMUSCLFA) THEN
  IF (LAROME) THEN
    DO JLEV=1,NFLEVG
      DO JROF=KST,KEND
        PNEB(JROF,JLEV)=YDVARS%A%T1(JROF,JLEV)
      ENDDO
    ENDDO
  ENDIF
  CALL WRITEPHYSIO(YDGEOMETRY,YDSURF,YDDPHY,YDRIP,YDMODEL%YRML_PHY_MF,&
       ...
       & YDMF_PHYS_SURF%GSD_VV%PD2, YDMF_PHYS_SURF%GSD_VV%PIVEG, YDMF_PHYS_SURF%GSD_VV%PLAI, YDMF_PHYS%CT,&
       & YDMF_PHYS_SURF%GSD_VV%PALV, YDMF_PHYS%ALB, YDMF_PHYS_SURF%GSD_VF%PALBF, YDMF_PHYS_SURF%GSD_VF%PALBSF, YDMF_PHYS_SURF%GSP_SG%PT_T1,&
       ...
       & YDMF_PHYS_SURF%GSP_RR%PT_T9, YDMF_PHYS_SURF%GSP_SB%PT_T9, YDMF_PHYS_SURF%GSP_RR%PFC_T9, YDMF_PHYS_SURF%GSP_RR%PW_T9,&
       & YDMF_PHYS_SURF%GSP_RR%PIC_T9, YDMF_PHYS_SURF%GSP_SB%PQ_T9, YDMF_PHYS_SURF%GSP_SB%PTL_T9, YDMF_PHYS_SURF%GSP_SG%PF_T9,&
       & YDMF_PHYS_SURF%GSP_SG%PA_T0, YDMF_PHYS_SURF%GSP_SG%PR_T0, PCTY0(1,1,YYTCTY0%M_VVEL),&
       & PEMTD , PEMTU ,PTRSW ,YDGSGEOM%GECLO,YDGSGEOM%GESLO,&
       & YDMF_PHYS%DIFCQ, YDMF_PHYS%DIFCQN, YDMF_PHYS%DIFCQL, YDMF_PHYS%DIFCS, YDMF_PHYS%DIFTQ, YDMF_PHYS%DIFTQN, YDMF_PHYS%DIFTQL,&
       & YDMF_PHYS%DIFTS, YDMF_PHYS%FCCQL, YDMF_PHYS%FCCQN, YDMF_PHYS%FCSQL, YDMF_PHYS%FCSQN, YDMF_PHYS%FCQNG, YDMF_PHYS%FCQNNG,&
       & YDMF_PHYS%FCQLNG, YDMF_PHYS%FCQRNG, YDMF_PHYS%FCQSNG, YDMF_PHYS%FPLCL, YDMF_PHYS%FPLCN, YDMF_PHYS%FPLSL, YDMF_PHYS%FPLSN, YDMF_PHYS%FPLSG, YDMF_PHYS%FPLSH,&
       & YDMF_PHYS%FPFPSL, YDMF_PHYS%FPFPSN, YDMF_PHYS%FPFPCL, YDMF_PHYS%FPFPCN, YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPCL, YDMF_PHYS%FPEVPCN,&
       & ZFTKE, ZFEFB1, ZFEFB2, ZFEFB3, YDMF_PHYS%FRSO, YDMF_PHYS%FRSOC, YDMF_PHYS%FRTH, YDMF_PHYS%FRTHC, YDMF_PHYS%FRSOLU,&
       & YDMF_PHYS%STRCU, YDMF_PHYS%STRCV, YDMF_PHYS%STRDU, YDMF_PHYS%STRDV, YDMF_PHYS%STRTU, YDMF_PHYS%STRTV, YDMF_PHYS%STRMU, YDMF_PHYS%STRMV,&
       & YDMF_PHYS%FRMH, ZFRMQ, YDMF_PHYS%FCHOZ, PNEB, PQICE, PQLI, PRH,&
       & YDMF_PHYS%FCS, YDMF_PHYS%FCLL, YDMF_PHYS%FCLN, YDMF_PHYS%FEVL, YDMF_PHYS%FEVN, ZFEVI, YDMF_PHYS%FEVV, YDMF_PHYS%FTR, YDMF_PHYS%FLWSP, YDMF_PHYS%FONTE,&
       & YDMF_PHYS%FGEL, YDMF_PHYS%FGELS, YDMF_PHYS%FCHSP, YDMF_PHYS%FRSODS, YDMF_PHYS%FRSOPS, YDMF_PHYS%FRSOPT, YDMF_PHYS%FRTHDS,&
       & ZCD, ZCDN, ZCH, ZC1, ZC2, ZEMIS, YDMF_PHYS%GZ0   , YDMF_PHYS%GZ0H  , ZNEIJ  , ZVEG,&
       & ZCPS, ZLHS, ZRS, ZLH, ZLSCPE, ZQSAT, ZQW, ZTW,&
       & PQS, ZQSATS, YDMF_PHYS%RUISL, YDMF_PHYS%RUISP, YDMF_PHYS%RUISS,&
       & YDMF_PHYS%UCLS, YDMF_PHYS%VCLS, YDMF_PHYS%TCLS, YDMF_PHYS%QCLS, YDMF_PHYS%RHCLS,&
       & PCLCT, YDMF_PHYS%CLCH, YDMF_PHYS%CLCM, YDMF_PHYS%CLCL, YDMF_PHYS%CLCC,&
       & YDMF_PHYS%CAPE  , YDMF_PHYS%CTOP, ICLPH  , YDMF_PHYS%CLPH  , YDMF_PHYS%UGST  , YDMF_PHYS%VGST,&
       & ZFPLCH, ZFPLSH,&
       & YDMF_PHYS_SURF%GSD_VH%PTCCH,  YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH, YDMF_PHYS_SURF%GSD_VH%PPBLH )
ENDIF
