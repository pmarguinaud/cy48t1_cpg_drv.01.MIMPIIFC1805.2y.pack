...
@PROCESS NOCHECK
#endif
SUBROUTINE MF_PHYS(YDGEOMETRY,YDVARS,YDGMV,YDSURF,YDCFU,YDXFU,YDMODEL,&
 !---------------------------------------------------------------------
 ! - INPUT and INOUT.
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      ...
      ITDIA=1_JPIM
      CALL APLPAR(YDGEOMETRY,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
       & ITDIA  , NFLEVG  , KSTGLO,&
...
IF (LMPA) THEN
  ...
  IF(LTWOTL) THEN
    CALL APL_AROME(YDGEOMETRY,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     & 1      , NFLEVG , NSTEP  ,&
