...
& KEND, NPROMA, NFLEVG, NTSSG, YSP_SBD%NLEVS, ZDIFEXT, ZFRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, YDMF_PHYS%TMP%APLPAR%DSA%LHS, YDMF_PHYS%TMP%APLPAR%DSA%RS, &
& ZLH, ZLSCPE, YDMF_PHYS%TMP%APLPAR%FLU%QSAT, ZQW, ZTW, YDMF_PHYS%TMP%APLPAR%FLU%CD, YDMF_PHYS%TMP%APLPAR%FLU%CDN, YDMF_PHYS%TMP%APLPAR%FLU%CH, YDMF_PHYS%TMP%APLPAR%DSA%C1, YDMF_PHYS%TMP%APLPAR%DSA%C2, YDMF_PHYS%TMP%APLPAR%FLU%EMIS, YDMF_PHYS%TMP%APLPAR%FLU%FEVI, YDMF_PHYS%TMP%APLPAR%PFL%FTKE, &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   & YDGSGEOM%RCORI, PGFL, ZCVGQ, PKOZO, YDMF_PHYS%TMP%APLPAR%PFL%FPLCH, YDMF_PHYS%TMP%APLPAR%PFL%FPLSH, PGPAR, YDMF_PHYS%TMP%RDG%MU0, YDMF_PHYS%TMP%RDG%MU0LU,     &
   & YDMF_PHYS%TMP%RDG%MU0M, YDMF_PHYS%TMP%RDG%MU0N, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, ZAC, ZAC_HC,       &
   & YDMF_PHYS%TMP%RDG%MMU0, YDOROG%OROG, ZDIFEXT, ZFRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, YDMF_PHYS%TMP%APLPAR%DSA%LHS, YDMF_PHYS%TMP%APLPAR%DSA%RS, ZLH, ZLSCPE, ZQRCONV,   &
   & ZQSCONV, YDMF_PHYS%TMP%APLPAR%FLU%QSAT, ZQW, ZTW, YDMF_PHYS%TMP%APLPAR%FLU%CD, YDMF_PHYS%TMP%APLPAR%FLU%CDN, YDMF_PHYS%TMP%APLPAR%FLU%CH, YDMF_PHYS%TMP%APLPAR%DSA%C1, YDMF_PHYS%TMP%APLPAR%DSA%C2, YDMF_PHYS%TMP%APLPAR%FLU%EMIS, YDMF_PHYS%TMP%APLPAR%FLU%FEVI, YDMF_PHYS%TMP%APLPAR%PFL%FTKE,     &
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
     ...
     & ZFHP   ,YDMF_PHYS%TMP%APLPAR%PFL%FP   ,  YDMF_PHYS%OUT%FEPFP, YDMF_PHYS%OUT%FCMPCQ, YDMF_PHYS%OUT%FCMPSN, YDMF_PHYS%OUT%FCMPSL,YDDDH )
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, NPROMA, KST, &
    & KEND, NFLEVG, YDGSGEOM%GNORDL, YDGSGEOM%GNORDM, ZDIFEXT, ZFRMQ, YDMF_PHYS%TMP%APLPAR%PFL%FTKE,     &
    & YDMF_PHYS%TMP%APLPAR%PFL%FTKEI, YDMF_PHYS%TMP%APLPAR%PFL%FEFB1, YDMF_PHYS%TMP%APLPAR%PFL%FEFB2, YDMF_PHYS%TMP%APLPAR%PFL%FEFB3, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZFHP, YDMF_PHYS%TMP%APLPAR%PFL%FP, ZTENDU, ZTENDV, ZTENDH,   &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      ...
      & NPROMA, KST, KEND, NFLEVG, NSTEP, ZFRMQ, YDMF_PHYS%TMP%APLPAR%DSA%CPS, ZTENDH, ZPTENDQ1,    &
    ...
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0,           &
       ...
       & YDGSGEOM%RCORI, YDCSGEOM%RATATH, YDCSGEOM%RATATX, ZCVGQ, ZLCVQ, YDMF_PHYS%TMP%APLPAR%PFL%FPLCH, &
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & GECLO, YDGSGEOM%GESLO, YDMF_PHYS%TMP%APLPAR%PFL%FTKE, YDMF_PHYS%TMP%APLPAR%PFL%FEFB1, YDMF_PHYS%TMP%APLPAR%PFL%FEFB2, YDMF_PHYS%TMP%APLPAR%PFL%FEFB3, ZFRMQ, YDMF_PHYS%TMP%APLPAR%FLU%FEVI, YDMF_PHYS%TMP%APLPAR%FLU%CD,      &
  & YDMF_PHYS%TMP%APLPAR%FLU%CDN, YDMF_PHYS%TMP%APLPAR%FLU%CH, YDMF_PHYS%TMP%APLPAR%DSA%C1, YDMF_PHYS%TMP%APLPAR%DSA%C2, YDMF_PHYS%TMP%APLPAR%FLU%EMIS, YDMF_PHYS%TMP%APLPAR%FLU%NEIJ, YDMF_PHYS%TMP%APLPAR%FLU%VEG, YDMF_PHYS%TMP%APLPAR%DSA%CPS, YDMF_PHYS%TMP%APLPAR%DSA%LHS, YDMF_PHYS%TMP%APLPAR%DSA%RS, ZLH, ZLSCPE, YDMF_PHYS%TMP%APLPAR%FLU%QSAT, &
  & ZQW, ZTW, YDMF_PHYS%TMP%APLPAR%FLU%QSATS, ICLPH, YDMF_PHYS%TMP%APLPAR%PFL%FPLCH, YDMF_PHYS%TMP%APLPAR%PFL%FPLSH  )
ENDIF
