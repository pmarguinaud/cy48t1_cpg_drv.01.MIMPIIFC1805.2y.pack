  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      ...
      CALL APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
       ...
       & PTPWCLS,ZDPRECIPS,ZDPRECIPS2,PVISICLD,PVISIHYDRO,PMXCLWC,&
       & ZTENDPTKE, ZKUROV_H, ZKTROV_H,&
       & PDERNSHF, ZTENDEXT_DEP, PTRAJ_PHYS,&
       & ZEDR, YDDDH, &
       & PFTCNS,&
       & ZGP2DSPP)
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  IF (LTWOTL) THEN
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-ZPQ(JROF,JLEV)*&
  ...
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-ZPQ9(JROF,JLEV)*&
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    ...
    IF (GCHETN%LPROFV)&
     ...
     & ZVVLAI, YDMF_PHYS%CT, ZVFZ0F, ZVVZ0H, ZVFZ0RLF,&
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    ...
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, NPROMA, KST, KEND, NFLEVG, YSP_SBD%NLEVS, PDTPHY,&
     ...
     & ZVFLSM, ZSGR1,&
     & YDMF_PHYS%RUISL, YDMF_PHYS%RUISP, YDMF_PHYS%RUISS, ZSGF1, ZVEG,&
...
IF (LTRAJPS) THEN
  IF (LTWOTL) THEN
    ...
    PTRAJ_PHYS%PSNSMF(KST:KEND)=ZSGF0(KST:KEND,1)
  ELSE
    ...
    PTRAJ_PHYS%PSNSMF(KST:KEND)=ZSGF9(KST:KEND,1)
  ENDIF
...
IF (LMPA) THEN
  ...
  IF(LTWOTL) THEN
    CALL APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     ...
     & PFEVL,PFEVN,PCLPH,&
     & PDIAGH,PFLASH,PTPWCLS, ZDPRECIPS, ZDPRECIPS2,&
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  CALL WRITEPHYSIO(YDGEOMETRY,YDSURF,YDDPHY,YDRIP,YDMODEL%YRML_PHY_MF,&
       ...
       & ZVVD2, ZVVIVEG, ZVVLAI, YDMF_PHYS%CT,&
       & ZVVALV, YDMF_PHYS%ALB, ZVFALBF, ZVFALBSF, ZSGT1,&
       & ZVFEMISF,&
