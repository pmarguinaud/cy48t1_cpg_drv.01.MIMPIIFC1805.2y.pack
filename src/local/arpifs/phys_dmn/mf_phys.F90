...
IF (LLDIAB .AND. (.NOT. LMPA)) THEN
  ...
  CALL MF_PHYS_FPL_PART1 (YDCPG_DIM, YDMF_PHYS, YDVARS, YDMODEL)
...
& NTSSG, YSP_SBD%NLEVS, ZDIFEXT, ZEDR)
  ...
  DO JLEV=1,YDCPG_DIM%KFLEVG
    DO JROF = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
      YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)+(YDMF_PHYS%TMP%APLPAR%MSC%LCVQ(JROF,JLEV)-YLMF_PHYS_STATE%Q(JROF,JLEV)*&
       ...
       & *MAX(0,SIGN(1,JLEV-YDMF_PHYS%TMP%APLPAR%MOC%CLPH(JROF)))  
    ENDDO
  ENDDO
  DO JROF = YDCPG_DIM%KIDIA, YDCPG_DIM%KFDIA
    YDMF_PHYS%OUT%MOCON(JROF) = YDMF_PHYS%OUT%MOCON(JROF)/(YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,YDCPG_DIM%KFLEVG)-YLMF_PHYS_STATE%YCPG_PHY%PREHYD(JROF,YDMF_PHYS%TMP%APLPAR%MOC%CLPH(JROF)-1))
  ENDDO
ENDIF
...
IF (LLDIAB .AND. (.NOT. LMPA)) THEN
  CALL MF_PHYS_FPL_PART2 (YDCPG_DIM, YDMF_PHYS, YDVARS, YDMODEL)
ENDIF
