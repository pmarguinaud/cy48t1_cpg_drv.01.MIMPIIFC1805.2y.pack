...
TYPE (MF_PHYS_STATE_TYPE) :: YL_MF_PHYS_STATE
...
CALL YL_MF_PHYS_STATE%INIT (LTWOTL, YDCPG_DYN0, YDCPG_DYN9, YDCPG_PHY0, YDCPG_PHY9, YDVARS, YDMF_PHYS_SURF, &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   ITDIA=1_JPIM
   CALL APLPAR(YDGEOMETRY,YL_MF_PHYS_STATE,YDCPG_DYN0,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
    & ITDIA  , NFLEVG  , KSTGLO,&
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
     & YL_MF_PHYS_STATE%YCPG_PHY%XYB%DELP ,&
     & YL_MF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YL_MF_PHYS_STATE%YCPG_DYN%RCP%CP,&
     & YL_MF_PHYS_STATE%U,YL_MF_PHYS_STATE%V,YL_MF_PHYS_STATE%T,YL_MF_PHYS_STATE%YGSP_RR%T,&
     & PGFL,&
     & YLPROCSET,&
  ...
  ELSE
    CALL CPTEND_NEW( YDMODEL, NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
     ...
     & YDMF_PHYS%OUT%DIFCQLC,YDMF_PHYS%OUT%DIFCQIC,YDMF_PHYS%OUT%FIMCC,&
     & YDMF_PHYS%OUT%FEDQLC, YDMF_PHYS%OUT%FEDQIC, YDMF_PHYS%OUT%FEDQRC, YDMF_PHYS%OUT%FEDQSC, YDMF_PHYS%OUT%FCNEGQLC,YDMF_PHYS%OUT%FCNEGQIC,YDMF_PHYS%OUT%FCNEGQRC,YDMF_PHYS%OUT%FCNEGQSC,&
     & ZFTKE  , ZFTKEI,  ZFEFB1 , ZFEFB2 , ZFEFB3 ,YL_MF_PHYS_STATE%YCPG_PHY%XYB%DELP ,&
     & YL_MF_PHYS_STATE%YCPG_PHY%XYB%RDELP, YL_MF_PHYS_STATE%YCPG_DYN%PHIF  , YL_MF_PHYS_STATE%YCPG_DYN%RCP%CP,&
     & YL_MF_PHYS_STATE%U,YL_MF_PHYS_STATE%V,YL_MF_PHYS_STATE%T,&
     & YL_MF_PHYS_STATE%Q,YL_MF_PHYS_STATE%I,YL_MF_PHYS_STATE%L,&
     & YDVARS%LCONV%T0,YDVARS%ICONV%T0,YDVARS%RCONV%T0,YDVARS%SCONV%T0,&
     & YL_MF_PHYS_STATE%R,YL_MF_PHYS_STATE%S,YL_MF_PHYS_STATE%G,&
     & ZCPS   , YL_MF_PHYS_STATE%YGSP_RR%T  ,&
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   CALL APL_AROME(YDGEOMETRY,YL_MF_PHYS_STATE,YDCPG_MISC,YDMF_PHYS,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
   & 1      , NFLEVG , NSTEP  ,&
