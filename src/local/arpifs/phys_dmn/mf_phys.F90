...
@PROCESS NOCHECK
#endif
SUBROUTINE MF_PHYS(YDGEOMETRY,YDCPG_PHY0,YDCPG_PHY9,YDMF_PHYS,YDCPG_DYN0,YDCPG_DYN9,&
...
CALL SC2PRG(1,YCHEM(:)%MP,PGFL  ,ZP1CHEM)     !  YDVARS%CHEM(1)%T0
...
CALL SC2PRG(1,YEXT(:)%MP,PGFL   ,ZP1EXT)      !  YDVARS%EXT(1)%T0
CALL SC2PRG(1,YEXT(:)%MP9,PGFL  ,ZP1EXT9)     !  YDVARS%EXT(1)%T9
...
CALL SC2PRG(1,YLIMA(:)%MP,PGFL  ,ZP1LIMA)     !  YDVARS%LIMA(1)%T0
CALL SC2PRG(1,YLIMA(:)%MP9,PGFL ,ZP1LIMA9)    !  YDVARS%LIMA(1)%T9
CALL SC2PRG(YL%MP1    ,ZTENDGFL ,ZPTENDL1)
CALL SC2PRG(1,YNOGW(:)%MP,PGFL  ,ZP1NOGW)     !  YDVARS%NOGW(1)%T0
CALL SC2PRG(1,YNOGW(:)%MP9,PGFL ,ZP1NOGW9)    !  YDVARS%NOGW(1)%T9 
CALL SC2PRG(2,YNOGW(:)%MP,PGFL  ,ZP2NOGW)     !  YDVARS%NOGW(2)%T0
...
IF (LLDIAB.OR.LSIMPH) THEN
  CALL CPPHINP(YDGEOMETRY,YDMODEL,KST,KEND,&
   ...
   & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),YDVARS%CVGQ%T0,&
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      ...
      IF (LAJUCV) THEN
        ...
        CALL ACAJUCV(YDMODEL%YRML_PHY_MF%YRPHY0,KST,KEND,NPROMA,NTPLUI,NFLEVG,NTAJUC,&
         & YDCPG_PHY0%PREHYD,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_ALPH),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),&
         & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_LNPR),YDVARS%T%T0)
        DO JLEV=1,NFLEVG
          DO JROF=KST,KEND
      ...
      CALL APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
       ...
       & YDCPG_DYN0%PHI  , YDCPG_PHY0%PREHYD  , YDCPG_DYN0%PHIF , YDCPG_PHY0%PREHYDF ,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_ALPH),&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_LNPR),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),&
       ...
       & YDVARS%VOR%T0,&
       & YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP), ZCVGQ  ,YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_R), PKOZO  , ZFPLCH , ZFPLSH ,&
       & YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),&
       & PGPAR , &
       ...
       & YDCPG_DYN0%CTY%ZVIEW(:,1:,YYTCTY0%M_VVEL),&
       & PEMTD , PEMTU ,PTRSW ,&
       & PRMOON ,&
       ...
       & ZMRAT5C,ZMRAT5N,&
       & YDOROG%OROG,&
       & YDCPG_PHY0%W,YDVARS%DIV%T0,YDVARS%U%DL,YDVARS%V%DL,YDCPG_PHY0%WL,YDCPG_PHY0%WM,&
       & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
       & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
       & PGRPROX,PGMIXP,PGFLUXC,PGRSURF,&
       & ZDIFEXT, &
    ...
    ELSE
      ...
      CALL APLPAR(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,  YDXFU, YDCFU,YDMODEL,  KST,    KEND   , NPROMA ,&
       ...
       & NTSSG  , YSP_SBD%NLEVS   ,&
       & KBL    , KGPCOMP, YDCFU%NFRRC, PDTPHY,YDCSGEOM%RINDX,YDCSGEOM%RINDY, LLXFUMSE,&
       & YDCPG_DYN9%PHI  , YDCPG_PHY9%PREHYD  , YDCPG_DYN9%PHIF , YDCPG_PHY9%PREHYDF , YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_ALPH) , &
       & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP),&
       & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_LNPR),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP),&
       ...
       & YDVARS%VOR%T9,&
       & YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP), ZCVGQ  ,YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_R), PKOZO  , ZFPLCH, ZFPLSH  ,&
       & YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),&
       & PGPAR , &
       ...
       & ZMRAT5C,ZMRAT5N,&
       & YDOROG%OROG,&
       & YDCPG_PHY9%W,YDVARS%DIV%T9,YDVARS%U%DL,YDVARS%V%DL,ZWT9L,ZWT9M,&
       & PGDEOSI,PGUEOSI,PGMU0,PGMU0_MIN,PGMU0_MAX,&
       & PGDEOTI,PGDEOTI2,PGUEOTI,PGUEOTI2,PGEOLT,PGEOXT,&
       & PGRPROX,PGMIXP,PGFLUXC,PGRSURF,&
       & ZDIFEXT, &
    ...
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-YDVARS%Q%T0(JROF,JLEV)*&
         & YDVARS%DIV%T0(JROF,JLEV))*YDCPG_PHY0%XYB%ZVIEW(JROF,JLEV,YYTXYB0_PHY%M_DELP)&
         & *MAX(0,SIGN(1,JLEV-ICLPH(JROF)))  
      ENDDO
    ENDDO
    DO JROF = KST, KEND
      YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)/(YDCPG_PHY0%PREHYD(JROF,NFLEVG)-YDCPG_PHY0%PREHYD(JROF,ICLPH(JROF)-1))
    ENDDO
  ELSE
    DO JLEV=1,NFLEVG
      DO JROF = KST, KEND
        YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)+(ZLCVQ(JROF,JLEV)-YDVARS%Q%T9(JROF,JLEV)*&
         & YDVARS%DIV%T0(JROF,JLEV))*YDCPG_PHY9%XYB%ZVIEW(JROF,JLEV,YYTXYB9_PHY%M_DELP)&
         & *MAX(0,SIGN(1,JLEV-ICLPH(JROF)))  
      ENDDO
    ENDDO
    DO JROF = KST, KEND
      YDMF_PHYS%MOCON(JROF) = YDMF_PHYS%MOCON(JROF)/(YDCPG_PHY9%PREHYD(JROF,NFLEVG)-YDCPG_PHY9%PREHYD(JROF,ICLPH(JROF)-1))
    ENDDO
  ENDIF
ENDIF
  ...
  IF (LSLAG) THEN
    IF (LTWOTL) THEN
      CALL CPOZO (NPROMA,KST,KEND,NFLEVG,PDTPHY,YDMF_PHYS%FCHOZ,&
       & PB1(1,ISLB1GFL9+IPO3+1-NFLSA),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP))  
    ELSE
      CALL CPOZO (NPROMA,KST,KEND,NFLEVG,PDTPHY,YDMF_PHYS%FCHOZ,&
       & PB1(1,ISLB1GFL9+IPO3+1-NFLSA),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP))  
    ENDIF
  ELSE
    CALL CPOZO (NPROMA,KST,KEND,NFLEVG,PDTPHY,YDMF_PHYS%FCHOZ,&
     & YDVARS%O3%T1,YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP))
  ENDIF
ENDIF
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,NPROMA,KST,KEND,YDCPG_PHY0%PREHYD,YDMF_PHYS_SURF%GSP_RR%PT_T0,PQS,ZQSATS,PQSOL)
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LTWOTL) THEN
    ...
    IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
      CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP) ,&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP), YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP),&
    ...
    ELSE
      CALL CPTEND_NEW( YDMODEL, NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
       ...
       & YDMF_PHYS%DIFCQLC,YDMF_PHYS%DIFCQIC,YDMF_PHYS%FIMCC,&
       & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC, YDMF_PHYS%FCNEGQLC,YDMF_PHYS%FCNEGQIC,YDMF_PHYS%FCNEGQRC,YDMF_PHYS%FCNEGQSC,&
       & ZFTKE  , ZFTKEI,  ZFEFB1 , ZFEFB2 , ZFEFB3 ,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP) ,&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP), YDCPG_DYN0%PHIF  , YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP),&
       & YDVARS%U%T0,YDVARS%V%T0,YDVARS%T%T0,&
       & YDVARS%Q%T0,YDVARS%I%T0,YDVARS%L%T0,&
  ...
  ELSE
    IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
      CALL CPTEND_FLEX( YDLDDH,YDMDDH,YGFL,YDPHY,NPROMA, KST, KEND, NFLEVG,YDGSGEOM%GNORDL,YDGSGEOM%GNORDM,&
       & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP) ,&
       & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP), YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP),&
    ...
    ELSE
      ...
      & YDMF_PHYS%FPEVPSL,YDMF_PHYS%FPEVPSN,YDMF_PHYS%FPEVPSG,YDMF_PHYS%FPEVPCL,YDMF_PHYS%FPEVPCN,YDMF_PHYS%FPEVPCG,&
       ...
       & YDMF_PHYS%DIFCQLC,YDMF_PHYS%DIFCQIC,YDMF_PHYS%FIMCC,&
       & YDMF_PHYS%FEDQLC, YDMF_PHYS%FEDQIC, YDMF_PHYS%FEDQRC, YDMF_PHYS%FEDQSC, YDMF_PHYS%FCNEGQLC,YDMF_PHYS%FCNEGQIC,YDMF_PHYS%FCNEGQRC,YDMF_PHYS%FCNEGQSC,&
       & ZFTKE  , ZFTKEI,  ZFEFB1 , ZFEFB2 , ZFEFB3 , YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP) ,&
       & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP), YDCPG_DYN9%PHIF  , YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP),&
       & YDVARS%U%T9,YDVARS%V%T9,YDVARS%T%T9,&
       & YDVARS%Q%T9,YDVARS%I%T9,YDVARS%L%T9,&
       & YDVARS%LCONV%T0,YDVARS%ICONV%T0,YDVARS%RCONV%T0,YDVARS%SCONV%T0,&
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,YDCPG_PHY0%PREHYD(:,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_PSDVBC),PB1(1,MSLB1SP9))
    ELSEIF (LSLAG .AND. (.NOT.LTWOTL)) THEN
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,YDCPG_PHY9%PREHYD(:,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_PSDVBC),PB1(1,MSLB1SP9))
    ELSE
      CALL CPMVVPS(YDVAB,NPROMA,KST,KEND,NFLEVG,PDTPHY,&
       & ZFP,YDCPG_PHY9%PREHYD(:,NFLEVG),YDMF_PHYS%FEVL,YDMF_PHYS%FEVN,&
       & YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_EVEL),YDCPG_DYN0%CTY%ZVIEW(:,:,YYTCTY0%M_PSDVBC),YDVARS%SP%T1)
    ENDIF
  ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV,YDGMV,YGFL,YDPTRSLB1,YDPHY,NPROMA,KST,KEND,NFLEVG,PDTPHY,IPGFL,&
   ...
   & YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),YDVARS%T%T0,YDVARS%U%T0,YDVARS%V%T0,&
   & YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP),YDVARS%T%T9,YDVARS%U%T9,YDVARS%V%T9,&
   & PB1, PGMVT1, PGFLT1,&
   & YDMF_PHYS%FDIS)
...
IF (LSIMPH) THEN
  ...
  IF (LTWOTL) THEN
    ...
    CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,KST,KEND,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
     & YDCPG_DYN0%PHI,YDCPG_PHY0%PREHYD,YDCPG_DYN0%PHIF,YDCPG_PHY0%PREHYDF,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),&
     & YDVARS%U%T0,YDVARS%V%T0,YDVARS%T%T0,YDVARS%Q%T0,YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP),ZCVGQ,&
  ...
  ELSE
    ...
    CALL APLPARS(YDGEOMETRY,YDRCOEF,YDMODEL%YRML_PHY_MF,KST,KEND,NPROMA,1,NFLEVG,NTSSG,NSTEP,&
     & YDCPG_DYN9%PHI,YDCPG_PHY9%PREHYD,YDCPG_DYN9%PHIF,YDCPG_PHY9%PREHYDF,YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP),&
     & YDVARS%U%T9,YDVARS%V%T9,YDVARS%T%T9,YDVARS%Q%T9,YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP),ZCVGQ,&
...
IF (LSIMPH) THEN
  ...
  IF (LTWOTL) THEN
    CALL CPTENDSM (YDPHY,NPROMA,KST,KEND,NFLEVG,&
     ...
     & YDMF_PHYS%STRMU,YDMF_PHYS%STRMV,YDMF_PHYS%FRMH,&
     & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),YDCPG_DYN0%PHIF,&
  ...
  ELSE
    CALL CPTENDSM (YDPHY,NPROMA,KST,KEND,NFLEVG,&
     ...
     & YDMF_PHYS%STRTU,YDMF_PHYS%STRTV,&
     & YDMF_PHYS%STRMU,YDMF_PHYS%STRMV,YDMF_PHYS%FRMH,&
     & YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP),YDCPG_DYN9%PHIF,&
...
IF (LMPA) THEN
  ...
  IF(LTWOTL) THEN
    CALL APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     ...
     & PGP2DSDT, ZGP2DSPP, &
     & YDCPG_DYN0%PHI,YDCPG_DYN0%PHIF,YDCPG_PHY0%PRE,YDCPG_PHY0%PREF,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),&
     & YDVARS%T%T0,YDVARS%Q%T0,&
     & YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP),YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_R),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_ALPH),YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_LNPR),&
  ...
  ELSE    
    CALL APL_AROME(YDGEOMETRY,YDMF_PHYS,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     ...
     & YDGSGEOM%GECLO,YDGSGEOM%GESLO,&
     & PGP2DSDT, ZGP2DSPP, &
     & YDCPG_DYN9%PHI,YDCPG_DYN9%PHIF,YDCPG_PHY9%PRE,YDCPG_PHY9%PREF,YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_RDELP),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_DELP),&
     & YDVARS%T%T9,YDVARS%Q%T9,&
     & YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_CP),YDCPG_DYN9%RCP%ZVIEW(:,:,YYTRCP9%M_R),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_ALPH),YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_LNPR),&
  ...
  IF(LNHDYN) THEN
    ...
    ELSE
      ...
      ELSE
        ...
        DO JLEV=1,NFLEVG
          DO JROF=KST,KEND
            ZRTT1(JROF,JLEV)=YDCPG_DYN0%RCP%ZVIEW(JROF,JLEV,YYTRCP0%M_R)*ZTT1(JROF,JLEV)
          ENDDO
        ENDDO
      ENDIF
      ...
      ! * Do conversion:
      IF (LSLAG.AND.LTWOTL) THEN
        CALL GNHGW2SVDAROME(YDGEOMETRY,KST,KEND,YDCPG_PHY0%PREHYDF,YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_LNPR),ZRTT1,YDCPG_PHY0%PREF,ZGWT1,&
         & ZTENDD)  
      ELSE
        CALL GNHGW2SVDAROME(YDGEOMETRY,KST,KEND,YDCPG_PHY9%PREHYDF,YDCPG_PHY9%XYB%ZVIEW(:,:,YYTXYB9_PHY%M_LNPR),ZRTT1,YDCPG_PHY9%PREF,ZGWT1,&
         & ZTENDD)  
      ENDIF
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  CALL WRITEPHYSIO(YDGEOMETRY,YDSURF,YDDPHY,YDRIP,YDMODEL%YRML_PHY_MF,&
       ...
       & YDGSGEOM%GELAM,YDGSGEOM%GEMU,&
       & YDGSGEOM%GM, ZMU0,YDOROG%OROG,YDCPG_DYN0%OROGL,YDCPG_DYN0%OROGM,YDGSGEOM%RCORI,YDCSGEOM%RATATH,YDCSGEOM%RATATX,&
       & YDCPG_DYN0%PHI  , YDCPG_PHY0%PREHYD  , YDCPG_DYN0%PHIF , YDCPG_PHY0%PREHYDF , YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_ALPH), YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_DELP),&
       & YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_LNPR), YDCPG_PHY0%XYB%ZVIEW(:,:,YYTXYB0_PHY%M_RDELP),&
       ...
       & YDVARS%EFB1%T0,YDVARS%EFB2%T0,YDVARS%EFB3%T0,&
       & YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_CP), YDCPG_DYN0%RCP%ZVIEW(:,:,YYTRCP0%M_R),&
       & YDVARS%U%T0,YDVARS%V%T0,YDVARS%VOR%T0,YDVARS%DIV%T0, ZCVGQ, ZLCVQ,&
       & YDMF_PHYS_SURF%GSP_RR%PT_T9, YDMF_PHYS_SURF%GSP_SB%PT_T9, YDMF_PHYS_SURF%GSP_RR%PFC_T9, YDMF_PHYS_SURF%GSP_RR%PW_T9,&
       & YDMF_PHYS_SURF%GSP_RR%PIC_T9, YDMF_PHYS_SURF%GSP_SB%PQ_T9, YDMF_PHYS_SURF%GSP_SB%PTL_T9, YDMF_PHYS_SURF%GSP_SG%PF_T9,&
       & YDMF_PHYS_SURF%GSP_SG%PA_T0, YDMF_PHYS_SURF%GSP_SG%PR_T0, YDCPG_DYN0%CTY%ZVIEW(:,1:,YYTCTY0%M_VVEL),&
