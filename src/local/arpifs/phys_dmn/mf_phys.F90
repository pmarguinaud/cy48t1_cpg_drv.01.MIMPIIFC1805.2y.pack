...
@PROCESS NOCHECK
#endif
SUBROUTINE MF_PHYS(YDGEOMETRY,YDCPG_DYN0,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDGMV,YDSURF,YDCFU,YDXFU,YDMODEL,&
 !---------------------------------------------------------------------
 ! - INPUT and INOUT.
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
    ...
    IF (LTWOTL) THEN
      ...
      ITDIA=1_JPIM
      CALL APLPAR(YDGEOMETRY,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF, YDXFU, YDCFU, YDMODEL, KST    , KEND   , NPROMA ,&
       & ITDIA  , NFLEVG  , KSTGLO,&
    ...
    ELSE
      ...
      ITDIA=1_JPIM
      CALL APLPAR(YDGEOMETRY,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,  YDXFU, YDCFU,YDMODEL,  KST,    KEND   , NPROMA ,&
       & ITDIA  , NFLEVG  , KSTGLO ,&
       & NVCLIS , YSD_VVD%NUMFLDS ,&
       & NSTEP  ,&
       & NTSSG  , YSP_SBD%NLEVS   ,&
...
IF (LCORWAT) THEN
  YDMF_PHYS_SURF%GSD_VH%PPSL(KST:KEND) = PFPLSL(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPCL(KST:KEND) = PFPLCL(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPSN(KST:KEND) = PFPLSN(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PPCN(KST:KEND) = PFPLCN(KST:KEND,NFLEVG)
  YDMF_PHYS_SURF%GSD_VH%PEVA(KST:KEND) = PFEVN(KST:KEND,1)-PFEVL(KST:KEND,1)
ENDIF
...
IF (LMPA) THEN
  ...
  IF(LTWOTL) THEN
    CALL APL_AROME(YDGEOMETRY,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     & 1      , NFLEVG , NSTEP  ,&
  ...
  ELSE    
    CALL APL_AROME(YDGEOMETRY,YDCPG_DYN9,YDMF_PHYS_SURF,YDVARS,YDSURF,YDCFU,YDXFU,YDMODEL, KBL, KGPCOMP, KST  , KEND   , NPROMA ,&
     & 1      , NFLEVG , NSTEP  ,&
     & IMAXDRAFT, NTSSG, YDCFU%NFRRC, PDTPHY, LLXFUMSE,YDCSGEOM%RINDX,YDCSGEOM%RINDY,&
     & YDGSGEOM%GEMU,YDGSGEOM%GELAM,YDOROG%OROG,YDGSGEOM%GM,&
     & ZMU0,ZMU0LU,ZMU0M,ZMU0N,&
