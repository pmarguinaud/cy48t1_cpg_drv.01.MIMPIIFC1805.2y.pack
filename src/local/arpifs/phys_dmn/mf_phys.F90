...
CALL INITAPLPAR ( &
  ...
  & YGFL, YDARPHY, KST, KEND, NPROMA, NFLEVG, NTSSG, YSP_SBD%NLEVS,&
  & ZDIFEXT, &
  & ZFRMQ  ,&
  & ZCPS   , ZLHS   ,&
  & ZRS    , ZLH    , ZLSCPE , ZQSAT  ,&
  & ZQW    , ZTW    , ZCD    , ZCDN   , ZCH    ,&
  & ZC1    , ZC2    , ZEMIS  , &
  & ZFEVI  , &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      CALL CPCHET(  &
      & YDMF_PHYS, YLMF_PHYS_STATE, YDCPG_MISC, &
      & YDRIP,YDPHY, NPROMA, KST, KEND, NFLEVG, NSTEP,&
      & ZFRMQ  , &
      & ZCPS   , &
      & ZTENDH ,&
      & ZPTENDQ1,ZPTENDI1,ZPTENDL1,&
    ...
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY,& 
     & YDCPG_MISC,YDMF_PHYS,YDCPG_DYN0,YDMF_PHYS_SURF,YDVARS,&
     & YDSURF,YDDPHY,YDRIP,YDMODEL%YRML_PHY_MF,&
     & KEND,&
     & YDGSGEOM%GELAM,YDGSGEOM%GEMU,&
     & YDGSGEOM%GM,ZMU0,YDOROG%OROG,YDGSGEOM%RCORI,YDCSGEOM%RATATH,YDCSGEOM%RATATX,&
     & ZCVGQ, ZLCVQ,&
     & ZFPLCH, ZFPLSH&
     & )
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  CALL WRITEPHYSIO(YDGEOMETRY,&
       ...
       & ZCVGQ, ZLCVQ,&
       & YDGSGEOM%GECLO,YDGSGEOM%GESLO,&
       & ZFTKE, ZFEFB1, ZFEFB2, ZFEFB3, &
       & ZFRMQ, &
       & ZFEVI, &
       & ZCD, ZCDN, ZCH, ZC1, ZC2, ZEMIS, ZNEIJ  , ZVEG,&
