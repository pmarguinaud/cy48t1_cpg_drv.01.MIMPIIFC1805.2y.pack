...
& YDMODEL, KBL, KGPCOMP, KST, KEND, KGL1, KGL2, KSTGLO, LDCONFX, PDTPHY, KIBL, &
...
& YDGEOMETRY%YRCSGEOM(KIBL), YDGSGEOM=>YDGEOMETRY%YRGSGEOM(KIBL), YDOROG=>YDGEOMETRY%YROROG(KIBL),  &
...
IF (LRAYSP.AND.(NSTEP >= INSTEP_DEB .AND. NSTEP <= INSTEP_FIN)) THEN  
  CALL CPPSOLAN(YDGEOMETRY%YRDIM,KST,KEND,YDGSGEOM%GEMU,YDGSGEOM%GELAM,YDMF_PHYS%TMP%RDG%MMU0)
  IF (.NOT.LSDDH) THEN
...
IF (LLDIAB.OR.LSIMPH) THEN
  CALL CPPHINP(YDGEOMETRY, YDMODEL, KST, KEND, YDGSGEOM%GEMU, YDGSGEOM%GELAM, YDVARS%U%T0, YDVARS%V% &
  ...
  YDMF_PHYS%TMP%APLPAR%MSC%LCVQ(KST:KEND,1:NFLEVG)=YDMF_PHYS%TMP%APLPAR%MSC%CVGQ(KST:KEND,1:NFLEVG)
ENDIF
...
CALL INITAPLPAR (YDMF_PHYS, YDCPG_MISC, YDMF_PHYS_SURF, YGFL, YDARPHY, KST, KEND, NPROMA, NFLEVG, &
...
IF (LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  ENDDO
   ...
   & YDVARS, YDSURF, YDXFU, YDCFU, YDMODEL, KST, KEND, NPROMA, ITDIA, NFLEVG, KSTGLO, NVCLIS,         &
   & YSD_VVD%NUMFLDS, NSTEP, NTSSG, YSP_SBD%NLEVS, KBL, KGPCOMP, YDCFU%NFRRC, PDTPHY, YDCSGEOM%RINDX, &
  ...
  IF (LINTFLEX) THEN
    CALL APLPAR2INTFLEX(YGFL, YDPHY, NPROMA, KST, KEND, NFLEVG, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%   &
...
IF (LCORWAT) THEN
  CALL MF_PHYS_CORWAT (YDMF_PHYS, YDMF_PHYS_SURF, KST, KEND, NFLEVG)
ENDIF
...
IF (LRCOEF.AND.(NSTEP == 1).AND.LLDIAB.AND.(.NOT.LMPA)) THEN
  ...
  CALL WRRADCOEF(YDGEOMETRY,YDRCOEF,KST,KEND,KSTGLO,IFIELDSS, &
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,NPROMA,KST,KEND,YDCPG_PHY0%PREHYD,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDCPG_MISC%QS,YDMF_PHYS%TMP%APLPAR%FLU%QSATS,YDCPG_MISC%QSOL)
ENDIF
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  IF (LINTFLEX.AND.(.NOT.LDCONFX)) THEN
    CALL CPTEND_FLEX( YDLDDH, YDMDDH, YGFL, YDPHY, NPROMA, KST, KEND, NFLEVG, YDGSGEOM%GNORDL,      &
  ...
  ELSE
    CALL CPTEND_NEW(   YDMF_PHYS, YLMF_PHYS_STATE, YDVARS, YDMODEL, NPROMA, KST, KEND, NFLEVG,        &
...
!               ----------------------------------
  ...
  IF (.NOT.LDCONFX) THEN
    IF ((GCHETN%LFREQD).OR.(GCHETN%LCOORD).OR.(GCHETN%LPROFV)) THEN
      CALL CPCHET(    YDMF_PHYS, YLMF_PHYS_STATE, YDCPG_MISC, YDRIP, YDPHY, NPROMA, KST, KEND,        &
    ...
    IF (GCHETN%LPROFV)&
     & CALL PROFILECHET(YDGEOMETRY, YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS,     &
       & YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, KEND, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%&
...
IF (LLDIAB.AND.(.NOT.LSIMPH).AND.(.NOT.LMPA)) THEN
  ...
  CALL CPUTQY(YDGEOMETRY%YRDIMV, YDGMV, YGFL, YDPTRSLB1, YDPHY, NPROMA, KST, KEND, NFLEVG, PDTPHY,   &
...
CALL MF_PHYS_TRANSFER (YDGEOMETRY, YDVARS, YDMODEL, KST, KEND)
...
IF (LLDIAB.AND.LMPHYS.AND.(.NOT.LMPA).AND.(.NOT.LSFORCS)) THEN
  ...
  IF (.NOT.LMSE) THEN
    DO JLEV=0,NFLEVG
      ...
      ENDDO
    ENDDO
    CALL CPTENDS( YDMODEL%YRML_PHY_MF, NPROMA, KST, KEND, NFLEVG, YSP_SBD%NLEVS, PDTPHY, YDMF_PHYS% &
    ...
    CALL CPWTS(YDSURF, YDMODEL%YRML_AOC%YRMCC, YDPHY, YDMODEL%YRML_PHY_MF%YRPHY1, NPROMA, KST, KEND,  &
...
IF (LTRAJPS) THEN
  ...
  PTRAJ_PHYS%PTSMF(KST:KEND) =YLMF_PHYS_STATE%YGSP_RR%T(KST:KEND)
  PTRAJ_PHYS%PSNSMF(KST:KEND)=YLMF_PHYS_STATE%YGSP_SG%F(KST:KEND,1)
...
IF (LMPA) THEN
  ...
  ENDIF
   ...
   & YDCFU, YDXFU, YDMODEL, KBL, KGPCOMP, KST, KEND, NPROMA, 1, NFLEVG, NSTEP, IMAXDRAFT, NTSSG,      &
...
IF (L3DTURB) THEN
  DO JLEV=1,NFLEVG
    PB2(KST:KEND,MSLB2KAPPAM+JLEV-1)=YDMF_PHYS%TMP%APLPAR%KUR%KUROV_H(KST:KEND,JLEV)
    PB2(KST:KEND,MSLB2KAPPAH+JLEV-1)=YDMF_PHYS%TMP%APLPAR%KUR%KTROV_H(KST:KEND,JLEV)
  ENDDO
ENDIF
...
IF(LGSCM.OR.LMUSCLFA) THEN
  ...
  & YDMF_PHYS_SURF, YDVARS, YDSURF, YDDPHY, YDRIP, YDMODEL%YRML_PHY_MF, KEND, KST, KGL1, KGL2,        &
  & KSTGLO, NSTEP, NTSSG, YSP_SBD%      NLEVS, YDGSGEOM%GELAM, YDGSGEOM%GEMU, YDGSGEOM%GM, YDOROG%    &
...
IF (LDPRECIPS) THEN
  YDMF_PHYS_SURF%GSD_XP%PPRECIP(KST:KEND,NDTPRECCUR)=YDMF_PHYS%TMP%PRC%DPRECIPS(KST:KEND,NDTPRECCUR)
ENDIF

IF (LDPRECIPS2) THEN
  YDMF_PHYS_SURF%GSD_XP2%PPRECIP2(KST:KEND,NDTPRECCUR2)=YDMF_PHYS%TMP%PRC%DPRECIPS2(KST:KEND,NDTPRECCUR2)
ENDIF
