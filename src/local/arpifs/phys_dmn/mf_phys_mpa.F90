...
IF (LLDIAB) THEN
  ...
  & YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0, YDMF_PHYS_TMP%RDG%MU0, YDMF_PHYS_TMP%RDG%MU0LU, YDMF_PHYS%  &
  & TMP%RDG%MU0M, YDMF_PHYS_TMP%RDG%MU0N, YDMF_PHYS_TMP%APLPAR%MSC%CVGQ)
  YDMF_PHYS_TMP%APLPAR%MSC%LCVQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)=YDMF_PHYS_TMP%APLPAR%MSC%CVGQ(YDCPG_DIM%KIDIA:YDCPG_DIM%KFDIA,1:YDCPG_DIM%KFLEVG)
ENDIF
...
IF (LLDIAB.AND.(NDPSFI == 1)) THEN
  CALL CPQSOL(YDGEOMETRY%YRDIMV,YDPHY,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_PHY0%PREHYD,YDMF_PHYS_SURF%GSP_RR%PT_T0,YDCPG_MISC%QS,YDMF_PHYS_TMP%APLPAR%FLU%QSATS,YDCPG_MISC%QSOL)
ENDIF
  ...
  IF (NDPSFI == 1) THEN
    IF (LSLAG .AND. LTWOTL) THEN
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS_TMP%APLPAR%PFL%FP,YDCPG_PHY0%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSEIF (LSLAG .AND. (.NOT.LTWOTL)) THEN
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS_TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
       & YDCPG_DYN0%CTY%EVEL,YDCPG_DYN0%CTY%PSDVBC,PB1(1,MSLB1SP9))
    ELSE
      CALL CPMVVPS(YDVAB,YDCPG_DIM%KLON,YDCPG_DIM%KIDIA,YDCPG_DIM%KFDIA,YDCPG_DIM%KFLEVG,PDTPHY,&
       & YDMF_PHYS_TMP%APLPAR%PFL%FP,YDCPG_PHY9%PREHYD(:,YDCPG_DIM%KFLEVG),YDMF_PHYS%OUT%FEVL,YDMF_PHYS%OUT%FEVN,&
...
ENDIF
 ...
 CALL APL_AROME(YDGEOMETRY, YDCPG_DIM, YLMF_PHYS_STATE, YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_TMP, YDMF_PHYS_SURF, YDVARS, YDSURF, &
