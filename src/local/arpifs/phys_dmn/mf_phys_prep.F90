...
YDCPG_PHY0%W(:,:)=YDCPG_DYN0%GWFT(:,:)*ZUSG

IF (LNHDYN .AND. YRDYNA%NVDVAR==5) THEN
  ! compute w from W:
  DO JLEV=1,NFLEVG
...
IF (LMPA.AND.(.NOT.LTWOTL)) THEN
  YDCPG_PHY9%W(:,:)=YDCPG_DYN9%GWFT(:,:)*ZUSG
  IF (LNHDYN .AND. (YRDYNA%NVDVAR==5)) THEN
    ! compute w from W:
    DO JLEV=1,NFLEVG
      DO JROF=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
...
IF (LNHDYN) THEN
  YDCPG_PHY0%WL(:,:)=YDCPG_DYN0%GWFL(:,:)*ZUSG
  YDCPG_PHY0%WM(:,:)=YDCPG_DYN0%GWFM(:,:)*ZUSG
