...
ASSOCIATE (LCDDPRO=>YDPHY%LCDDPRO, LCVPRO=>YDPHY%LCVPRO, LGPCMT=>YDPHY%LGPCMT)
  ...
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VV%YHV%LSET) PSAV_HV(1:YDCPG_OPTS%KLON)=PGSD_VV_PHV(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VF%YZ0F%LSET) PSAV_GZ0F(1:YDCPG_OPTS%KLON)=PGSD_VF_PZ0F(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VV%YZ0H%LSET) PSAV_GZ0HF(1:YDCPG_OPTS%KLON)=PGSD_VV_PZ0H(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VH%YPBLH%LSET) PSAV_PBLH(1:YDCPG_OPTS%KLON)=PGSD_VH_PPBLH(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VH%YSPSH%LSET) PSAV_FHPS(1:YDCPG_OPTS%KLON)=PGSD_VH_PSPSH(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VH%YQSH%LSET) PSAV_QSH(1:YDCPG_OPTS%KLON)=PGSD_VH_PQSH(1:YDCPG_OPTS%KLON)
  IF(YDCPG_OPTS%YRSURF_OPTS%YSD_VK%YUDGRO%LSET) PSAV_UDGRO(1:YDCPG_OPTS%KLON)=PGSD_VK_PUDGRO(1:YDCPG_OPTS%KLON)
