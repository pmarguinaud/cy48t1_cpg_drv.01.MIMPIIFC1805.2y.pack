...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RFAC_TWOC) THEN
  ...
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   ...
   DO JL=KIDIA,KFDIA
    PEZDIAG_POS(JL,JKO) = PGP2DSPP(JL,YSPP%MP_RFAC_TWOC)
    PEZDIAG_POS(JL,JKE) = ZFAC_TWO_COEF(JL)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RZC_H) THEN
  ...
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   ...
   JKE=2*YSPP%MP_RZC_H
   DO JL=KIDIA,KFDIA
    PEZDIAG_POS(JL,JKO) = PGP2DSPP(JL,YSPP%MP_RZC_H)
    PEZDIAG_POS(JL,JKE) = ZC_H(JL)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RZL_INF) THEN
  ...
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   ...
   DO JL=KIDIA,KFDIA
    PEZDIAG_POS(JL,JKO) = PGP2DSPP(JL,YSPP%MP_RZL_INF)
    PEZDIAG_POS(JL,JKE) = ZL_INF(JL)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
