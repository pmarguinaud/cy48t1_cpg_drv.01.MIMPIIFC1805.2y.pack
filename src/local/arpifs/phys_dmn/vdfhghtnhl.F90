...
REAL(KIND=JPRB)    :: ZFACW,ZFACI,ZESDP,ZCOR
!cstep/GL ---------------------------------------------
...
REAL(KIND=JPRB) ::    ZSTAR(KLON), ZCHICRIT(KLON,0:KLEV),ZMEANCHICRIT(KLON)   , &
                    ...
                    & ZEXNERH(KLON,0:KLEV), ZQVENH(KLON,0:KLEV), &
                    & ZTHTVENH(KLON,0:KLEV), ZTHTLUH(KLON,0:KLEV), &
                    & ZTHTENH(KLON,0:KLEV), &
                    & ZTHTVUH(KLON,0:KLEV), ZTHTLEH(KLON,0:KLEV), ZESATU(KLON,0:KLEV),&
...
REAL(KIND=JPRB) ::    ZQSATM, ZSATDEF, &
                    ...
                    & ZDZRHO, ZPFLXTOT, ZPEVAPUP, ZFAC, ZUPMELT, ZUPMELTTEND, ZPGENTOT
...
REAL(KIND=JPRB) ::    ZZI(KLON),ZB1(KLON),ZCOUNT(KLON),ZTOP,ZBASE, &
...
ASSOCIATE(RTAUMEL=>YDECUMF%RTAUMEL, &
 & YSURF=>YDEPHY%YSURF,LHARATU=>YDPARAR%LHARATU,LTOTPREC=>YDPARAR%LTOTPREC )
...
! Setup SPP patterns 
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_CLDDPTH) THEN
  IF (YSPP_CONFIG%LLNN_MEAN1.OR.YSPP_CONFIG%LLNN_MEAN1_CLDDPTH) THEN
    ZMU = -0.5_JPRB * (YSPP_CONFIG%CMPERT_CLDDPTH * YSPP_CONFIG%SDEV)**2
  ELSE
    ZMU = 0._JPRB
  ENDIF
  DO JL=KIDIA,KFDIA
    ZVAL = ZCLDDEPTH*EXP(ZMU+YSPP_CONFIG%CMPERT_CLDDPTH*PGP2DSPP(JL,YSPP%MP_CLDDPTH))
    ZFRMIN(JL,1) = MAX(YSPP_CONFIG%CLIP_CLDDPTH(1),MIN(ZVAL,YSPP_CONFIG%CLIP_CLDDPTH(2)))
  ENDDO
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   JKO=2*YSPP%MP_CLDDPTH-1
   JKE=2*YSPP%MP_CLDDPTH
   DO JL=KIDIA,KFDIA
    PEZDIAG(JL,JKO,YSPP_CONFIG%IEZDIAG_POS) = PGP2DSPP(JL,YSPP%MP_CLDDPTH)
    PEZDIAG(JL,JKE,YSPP_CONFIG%IEZDIAG_POS) = ZFRMIN(JL,1)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
  ...
  ENDDO
ENDIF
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_CLDDPTHDP) THEN
  IF (YSPP_CONFIG%LLNN_MEAN1.OR.YSPP_CONFIG%LLNN_MEAN1_CLDDPTHDP) THEN
    ZMU = -0.5_JPRB * (YSPP_CONFIG%CMPERT_CLDDPTHDP * YSPP_CONFIG%SDEV)**2
  ELSE
    ZMU = 0._JPRB
  ENDIF
  DO JL=KIDIA,KFDIA
    ZVAL = ZCLDDEPTHDP*EXP(ZMU+YSPP_CONFIG%CMPERT_CLDDPTHDP*PGP2DSPP(JL,YSPP%MP_CLDDPTHDP))
    ZFRMIN(JL,2) = MAX(YSPP_CONFIG%CLIP_CLDDPTHDP(1),MIN(ZVAL,YSPP_CONFIG%CLIP_CLDDPTHDP(2)))
  ENDDO
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   JKO=2*YSPP%MP_CLDDPTHDP-1
   JKE=2*YSPP%MP_CLDDPTHDP
   DO JL=KIDIA,KFDIA
    PEZDIAG(JL,JKO,YSPP_CONFIG%IEZDIAG_POS) = PGP2DSPP(JL,YSPP%MP_CLDDPTHDP)
    PEZDIAG(JL,JKE,YSPP_CONFIG%IEZDIAG_POS) = ZFRMIN(JL,2)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
...
!*
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZEXNERH(JL,JK) = (PAPHM1(JL,JK)/ZPRES_0)**ZKAPPA
    ZTHTENH(JL,JK) = (ZTENH(JL,JK)/ZEXNERH(JL,JK)) 
    ZTHTVENH(JL,JK) = (ZTENH(JL,JK)/ZEXNERH(JL,JK))&
  ...
  DO JD = 2,KDRAFT
    ...
    !-- Release the updraft --
      ...
      CALL VDFPARCELHL(YDEPHLI,YDPARAR,KIDIA,KFDIA,KLON,KLEV,KDRAFT,PGEOH,PGEOM1,PAPHM1,PUM1,PVM1,ZQTM1,ZSLGM1,ZTVEN,PUUH, &
        & PVUH,PSLGUH,&
PQTUH,ZWU2H,ZQCUH,ZBUOF,ZQUH,ZTUH,ZEPS,PZPLCL,KPLCL,PZPTOP,KPTOP,KPLZB,JD,ZUPGENL,ZUPGENN,ZTAUEPS,ZW2THRESH,LLDONE,KPBLTYPE)
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF ( KPBLTYPE(JL)==2 .OR. KPBLTYPE(JL)==3) THEN
      ...
      IF (  PZPLCL(JL,3) < 0._JPRB .OR. KPLCL(JL,3)<KPTOP(JL,3) .OR. KPLCL(JL,3)==0  .OR. KPTOP(JL,3)==0 ) THEN
        KPBLTYPE(JL) = 1
  ...
  DO JD = 3,KDRAFT  !moist updrafts only
    ...
    DO JK=2,KLEV
      DO JL=KIDIA,KFDIA
        ...
        ZPGENTOT = ZUPGENL(JL,JK,JD) + ZUPGENN(JL,JK,JD)
   ...
   IF(LTOTPREC)THEN
     ...
     DO JL=KIDIA,KFDIA
      ...
      ZTOP=KPTOP(JL,3)
      ZBASE=KPLCL(JL,3)
      IF(ZTOP > 0._JPRB .AND. ZBASE > 0._JPRB)THEN
       ITOP=INT(ZTOP,KIND=JPIM)
       IBASE=INT(ZBASE,KIND=JPIM)
       DO JK=ITOP,IBASE
         ...
         IF ( KPBLTYPE(JL)/=0 .AND. ZFRACB(JL,JD)>0._JPRB) THEN
          ...
          IF (JD == 3 .AND. JK >= KPLCL(JL,3) ) THEN
           ! do only below LCL !!
            ...
            PMFLX(JL,JK,JD)  = 0.3_JPRB * 0.1_JPRB * ZWSTAR(JL) *&
  ...
  DO JL=KIDIA,KFDIA
    ...
    DO JK=KLEV-1,1,-1
      IF (KPTOP(JL,2) <= JK .AND. KPBLTYPE(JL) > 0) THEN
        ZEXNERH(JL,JK) = (PAPHM1(JL,JK)/ZPRES_0)**ZKAPPA
        PTHTVUH(JL,JK,2) = (ZTUH(JL,JK,2)/ ZEXNERH(JL,JK))&
   ...
   DO JK=KPLCL(JL,3),KPTOP(JL,3),-1
        ...
        ZTHTLUH(JL,JK) = (ZTUH(JL,JK,3) / ZEXNERH(JL,JK)) -&
                        & ((RLVTT/(RCPD*ZEXNERH(JL,JK)))*ZQCUH(JL,JK,3))
        !  N.B. ZQCUH is initialized with ql and qi from the environment!?!

        ZTHTVUH(JL,JK) = (ZTUH(JL,JK,3)/ ZEXNERH(JL,JK))&
                       &  * (1._JPRB + RETV * ZQUH(JL,JK,3)&
                       &    - ZQCUH(JL,JK,3)) 
        PTHTVUH(JL,JK,3)=ZTHTVUH(JL,JK)
        ZTHTLEH(JL,JK) = (ZTENH(JL,JK) / ZEXNERH(JL,JK)) -&
                      & (RLVTT/(RCPD*ZEXNERH(JL,JK)))*&
                      & ZQLENH(JL,JK)
        ZESATU(JL,JK) = 0.61078_JPRB * EXP(17.2694_JPRB *&
                      &  (ZTUH(JL,JK,3)-RTT)/(ZTUH(JL,JK,3)-35.86_JPRB))

        ZQSATU(JL,JK) = FOEEWM(ZTUH(JL,JK,3))/PAPHM1(JL,JK)
        ZQSATU(JL,JK) = MIN(0.5_JPRB,ZQSATU(JL,JK))
        ZQSATU(JL,JK) = ZQSATU(JL,JK)/(1.0_JPRB-RETV*ZQSATU(JL,JK))
        ZDQSDTU(JL,JK) = (0.622_JPRB * RLVTT * ZQSATU(JL,JK))/&
                      & (RD * ZTUH(JL,JK,3) * ZTUH(JL,JK,3)) 
        ZGAMMA(JL,JK) = (RLVTT/RCPD)*ZDQSDTU(JL,JK)
        ZDUMFUNC(JL,JK) = (1._JPRB/(1._JPRB+ZGAMMA(JL,JK)))*&
                      & ((ZQTENH(JL,JK)-PQTUH(JL,JK,3)) -&
                      & ZEXNERH(JL,JK) * ZDQSDTU(JL,JK) *&
        ...
        ZCHICRIT(JL,JK) = (ZTHTVENH(JL,JK)-ZTHTVUH(JL,JK))/&
                      ...
                      & PQTUH(JL,JK,3) - (1._JPRB + RETV)*&
                      & ZQCUH(JL,JK,3)) + (RLVTT/(ZEXNERH(JL,JK)*RCPD))*&
            &  ZDUMFUNC(JL,JK))
...
IF (LHARATU) THEN
    ...
    DO JL=KIDIA,KFDIA
     ...
     DO JK=1,KLEV-1
       ...
       IF (ZWSTAR(JL)*PZPTOP(JL,2).GT.0._JPRB) THEN
         ...
         PBUOY_COR (JL,JK) = ZTVEXCSURF(JL,2)*PWU(JL,JK)/ ZWSTAR(JL) / PZPTOP(JL,2) * RG / PTM1(JL,KLEV)
       ENDIF
    ENDDO
   ENDDO
  ...
  DO JK=1,KLEV
    DO JL=KIDIA,KFDIA
      ! ZQSVAR(JL,JK) = FOEEWM(PTM1(JL,JK))/PAPM1(JL,JK)
      ! CGL fix to avoid unrealistic values in stratosphere
      ZQSVAR(JL,JK) = FOEEWM(PTM1(JL,JK))/MAX(PAPM1(JL,JK),5000._JPRB)
      ...
      ZFACW=R5LES/((PTM1(JL,JK)-R4LES)**2)
      ZFACI=R5IES/((PTM1(JL,JK)-R4IES)**2)
      ZFAC=ZALFAW(JL,JK)*ZFACW+(1.0_JPRB-ZALFAW(JL,JK))*ZFACI
      ! ZESDP=FOEEWM(PTM1(JL,JK))/PAPM1(JL,JK)
      ! CGL fix to avoid unrealistic values in stratosphere
      ZESDP=FOEEWM(PTM1(JL,JK))/MAX(PAPM1(JL,JK),5000._JPRB)
...
CALL VDFEXCUHL(YDVDF,YDEPHY,KIDIA,KFDIA,KLON,KLEV,PTMST,PUM1,PVM1,PTM1,PQM1,PLM1,PIM1,ZSLGM1,ZQTM1,PKMFL,PKHFL,PKQFL,PAPHM1,&
 & PAPM1,PGEOM1,PGEOH,ZZI,KPBLTYPE,KDRAFT,ZQSVAR,ZDQSDTEMP,PBUOY_COR,ZENCASC,PWU, &
 & PGP2DSPP,KGFL_EZDIAG,PEZDIAG,PTKE,PMFLX,ZLENGTH_M,ZLENGTH_H)
