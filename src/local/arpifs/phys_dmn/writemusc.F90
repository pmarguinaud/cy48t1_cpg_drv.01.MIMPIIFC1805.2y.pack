SUBROUTINE WRITEMUSC(&
  ...
  &CDLOCK, &
  &PRDG_CVGQ, PRDG_MU0, PDSA_C1, PDSA_C2, PDSA_CPS, PDSA_LHS, PDSA_RS, PFLU_CD, PFLU_CDN, &
  &PFLU_CH, PFLU_EMIS, PFLU_FEVI, PFLU_NEIJ, PFLU_QSAT, PFLU_QSATS, PFLU_VEG, KMOC_CLPH, &
  &PMSC_FRMQ, PMSC_LH, PMSC_LSCPE, PMSC_QW, PMSC_TW, PPFL_FEFB1, PPFL_FEFB2, PPFL_FEFB3, PPFL_FTKE)
...
REAL (KIND=JPRB), INTENT (IN)         :: PDSA_C1 (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PDSA_C2 (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PDSA_CPS (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PDSA_LHS (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PDSA_RS (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_CD (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_CDN (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_CH (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_EMIS (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_FEVI (KLON, KTSSG+1)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_NEIJ (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_QSAT (KLON, KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_QSATS (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PFLU_VEG (KLON)
INTEGER (KIND=JPIM), INTENT (INOUT)   :: KMOC_CLPH (KLON)
REAL (KIND=JPRB), INTENT (IN)         :: PMSC_FRMQ (KLON, 0:KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PMSC_LH (KLON, KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PMSC_LSCPE (KLON, KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PMSC_QW (KLON, KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PMSC_TW (KLON, KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PPFL_FEFB1 (KLON, 0:KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PPFL_FEFB2 (KLON, 0:KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PPFL_FEFB3 (KLON, 0:KLEV)
REAL (KIND=JPRB), INTENT (IN)         :: PPFL_FTKE (KLON, 0:KLEV)
...
CALL LFAECRI(IUSCM,'KCLPH',KMOC_CLPH(2),1)
...
DO JLEV=KTDIA,KLEV
   ...
   ZTHETAL(JLON,JLEV)=ZTHETA(JLON,JLEV)*(1.-(PMSC_LH(JLON,JLEV)*ZQC(JLON,JLEV))/(YDCPG_DYN0%RCP%CP(JLON,JLEV)*YDVARS%T%T0(JLON,JLEV)))
...
IF (.NOT.LAROME) THEN 
   CALL WRSCMR(IUSCM,'PQSAT',PFLU_QSAT,KLON,KLEV)
ENDIF
...
IF (.NOT.LAROME) THEN
   ...
   CALL WRSCMR(IUSCM,'PLH',PMSC_LH,KLON,KLEV)
   ...
   CALL WRSCMR(IUSCM,'PFTKE',PPFL_FTKE,KLON,KLEV+1)
   IF (LEFB) THEN
      CALL WRSCMR(IUSCM,'PFEFB1',PPFL_FEFB1,KLON,KLEV+1)
      CALL WRSCMR(IUSCM,'PFEFB2',PPFL_FEFB2,KLON,KLEV+1)
      CALL WRSCMR(IUSCM,'PFEFB3',PPFL_FEFB3,KLON,KLEV+1)
   ENDIF   
   ...
   CALL WRSCMR(IUSCM,'PFRMQ',PMSC_FRMQ,KLON,KLEV+1)
   CALL WRSCMR(IUSCM,'PFCHOZ',YDMF_PHYS%OUT%FCHOZ,KLON,KLEV+1)
ENDIF
IF (.NOT.LMSE) THEN
CALL ECR1D(IUSCM,'PLHS',PDSA_LHS,1,KLON)
ENDIF
CALL ECR1D(IUSCM,'PEMIS',PFLU_EMIS,1,KLON)
CALL ECR1D(IUSCM,'PFEVL',YDMF_PHYS%OUT%FEVL,1,KLON)
CALL ECR1D(IUSCM,'PFEVI',PFLU_FEVI,1,KLON)
CALL ECR1D(IUSCM,'PFEVN',YDMF_PHYS%OUT%FEVN,1,KLON)
IF ((.NOT.LMSE).AND.(LSOLV)) THEN  
  CALL ECR1D(IUSCM,'PCD',PFLU_CD,1,KLON)
  CALL ECR1D(IUSCM,'PCDN',PFLU_CDN,1,KLON)
  CALL ECR1D(IUSCM,'PCH',PFLU_CH,1,KLON)
  CALL ECR1D(IUSCM,'PSNS',YDMF_PHYS_SURF%GSP_SG%PF_T9,1,KLON)
  CALL ECR1D(IUSCM,'PCPS',PDSA_CPS,1,KLON)
  CALL ECR1D(IUSCM,'PRS',PDSA_RS,1,KLON)
  CALL WRSCMR(IUSCM,'PLSCPE',PMSC_LSCPE,KLON,KLEV)
  CALL WRSCMR(IUSCM,'PQW',PMSC_QW,KLON,KLEV)
  CALL WRSCMR(IUSCM,'PTW',PMSC_TW,KLON,KLEV)
...
!CALL ECR1D(IUSCM,'PGM',PGM,1,KLON)
  CALL ECR1D(IUSCM,'PC1',PDSA_C1,1,KLON)
  CALL ECR1D(IUSCM,'PC2',PDSA_C2,1,KLON)
  ...
  CALL ECR1D(IUSCM,'PNEIJ',PFLU_NEIJ,1,KLON)
  CALL ECR1D(IUSCM,'PVEG',PFLU_VEG,1,KLON)
  CALL ECR1D(IUSCM,'PQSATS',PFLU_QSATS,1,KLON)
...
IF (.NOT.LAROME) THEN
   ...
   &/(ZRHO(JLON,KLEV)*PMSC_LH(JLON,KLEV)))
   ENDDO
...
DO JLON=KIDIA,KFDIA
   ...
   DO JLEV=KTDIA,KLEV
      IF(YDCPG_DYN0%PHIF(JLON,JLEV)/RG >= 0._JPRB .AND.&
        & YDCPG_DYN0%PHIF(JLON,JLEV)/RG < 15000._JPRB ) THEN
        ZVAL=(PFLU_QSAT(JLON,JLEV)-YDVARS%Q%T0(JLON,JLEV))*1000._JPRB
        ZMEAN_SAT_DEF(JLON)=ZMEAN_SAT_DEF(JLON)+ZVAL*YDCPG_PHY0%XYB%DELP(JLON,JLEV)/RG
        ZDELTAPSG=ZDELTAPSG+YDCPG_PHY0%XYB%DELP(JLON,JLEV)/RG
      ENDIF
   ENDDO
...
DO JLON=KIDIA,KFDIA
   ...
   DO JLEV=KTDIA,KLEV
      IF(YDCPG_DYN0%PHIF(JLON,JLEV)/RG >= 2500._JPRB&
        ...
        ZVAL=(PFLU_QSAT(JLON,JLEV)-YDVARS%Q%T0(JLON,JLEV))*1000.
        ZMEAN_SAT_DEF(JLON)=ZMEAN_SAT_DEF(JLON)+ZVAL*YDCPG_PHY0%XYB%DELP(JLON,JLEV)/RG
        ZDELTAPSG=ZDELTAPSG+YDCPG_PHY0%XYB%DELP(JLON,JLEV)/RG
      ENDIF
   ENDDO
