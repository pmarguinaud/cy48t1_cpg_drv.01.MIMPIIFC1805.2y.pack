...
IF (LMUSCLFA) THEN
   ...
   & KCSS, YDCPG_DYN0%PHI, YDCPG_PHY0%PREHYD, YDCPG_DYN0%PHIF, YDCPG_PHY0%PREHYDF, YDCPG_PHY0%XYB%ALPH,                        &
   & YDMF_PHYS_SURF%GSD_VV%PARG, YDMF_PHYS_SURF%GSD_VV%PD2, YDCPG_PHY0%XYB%DELP, YDMF_PHYS_SURF%GSD_VV%PIVEG,                  &
   & YDMF_PHYS_SURF%GSD_VV%PLAI, YDCPG_PHY0%XYB%LNPR, YDCPG_PHY0%XYB%RDELP, YDMF_PHYS_SURF%GSD_VV%PRSMIN,                      &
   & YDMF_PHYS_SURF%GSD_VV%PSAB, YDMF_PHYS_SURF%GSD_VV%PZ0H, YDVARS%U%T0, YDVARS%V%T0, YDVARS%T%T0, YDVARS%Q%T0,               &
   & YDVARS%I%T0, YDVARS%L%T0, YDVARS%S%T0, YDVARS%R%T0, YDVARS%G%T0, YDVARS%TKE%T0, YDVARS%EFB1%T0,                           &
   & YDVARS%EFB2%T0, YDVARS%EFB3%T0, YDCPG_DYN0%RCP%CP, YDMF_PHYS_TMP%RDG%CVGQ, YDCPG_DYN0%RCP%R, YDMF_PHYS_SURF%GSD_VH%PPBLH, &
   & YDMF_PHYS_SURF%GSP_SG%PF_T9, YDMF_PHYS_SURF%GSP_SG%PA_T0, YDMF_PHYS_SURF%GSP_SG%PR_T0, YDMF_PHYS_SURF%GSP_SB%PT_T9,       &
   & YDMF_PHYS_SURF%GSP_RR%PT_T9, YDMF_PHYS_SURF%GSP_RR%PFC_T9, YDMF_PHYS_SURF%GSP_SB%PQ_T9, YDMF_PHYS_SURF%GSP_SB%PTL_T9,     &
   & YDMF_PHYS_SURF%GSP_RR%PW_T9, YDMF_PHYS_SURF%GSP_RR%PIC_T9, YDMF_PHYS_SURF%GSD_VV%PHV, YDCPG_DYN0%CTY%VVEL(:, 1:),         &
   & YDMF_PHYS%RAD%EMTD, YDMF_PHYS%RAD%EMTU, YDMF_PHYS%RAD%TRSW, PCLON, PSLON, YDMF_PHYS_SURF%GSD_VV%PALV,                     &
   & YDMF_PHYS_SURF%GSD_VF%PALBF, YDMF_PHYS_SURF%GSD_VF%PALBSF, YDMF_PHYS_SURF%GSD_VF%PEMISF, YDMF_PHYS_SURF%GSD_VF%PGETRL,    &
   & YDMF_PHYS_SURF%GSD_VF%PLSM, YDMF_PHYS_SURF%GSD_VF%PVEG, YDMF_PHYS_SURF%GSD_VF%PZ0F, YDMF_PHYS_SURF%GSD_VF%PZ0RLF,         &
   & YDMF_PHYS_SURF%GSD_VF%PVRLAN, YDMF_PHYS_SURF%GSD_VF%PVRLDI, YDMF_PHYS_TMP%RDG%MU0, PGELAM,                                &
   & PGEMU, PGM, POROG, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCS,                  &
   ...
   & YDMF_PHYS%OUT%FCQLNG, YDMF_PHYS%OUT%FCQRNG, YDMF_PHYS%OUT%FCQSNG, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN,               &
   & YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLSG, YDMF_PHYS%OUT%FPLSH, YDMF_PHYS%OUT%FRSO,                   &
   & YDMF_PHYS%OUT%FRSOC, YDMF_PHYS%OUT%FRTH, YDMF_PHYS%OUT%FRTHC, YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV,                   &
   & YDMF_PHYS%OUT%STRDU, YDMF_PHYS%OUT%STRDV, YDMF_PHYS%OUT%STRTU, YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%STRMU,                  &
   & YDMF_PHYS%OUT%STRMV, YDMF_PHYS%OUT%FRMH, YDAPLPAR_TMP%MSC%FRMQ, YDMF_PHYS%OUT%FCHOZ, YDAPLPAR_TMP%DSA%CPS,                &
   & YDAPLPAR_TMP%DSA%LHS, YDAPLPAR_TMP%DSA%RS, YDAPLPAR_TMP%MSC%LH, YDAPLPAR_TMP%MSC%LSCPE, YDCPG_MISC%NEB,                   &
   & YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%MSC%QW, YDCPG_MISC%RH, YDAPLPAR_TMP%MSC%TW,          &
   & YDMF_PHYS%OUT%ALB, YDAPLPAR_TMP%FLU%CD, YDAPLPAR_TMP%FLU%CDN, YDAPLPAR_TMP%FLU%CH, YDAPLPAR_TMP%DSA%C1,                   &
   & YDAPLPAR_TMP%DSA%C2, YDMF_PHYS%OUT%CT, YDAPLPAR_TMP%FLU%EMIS, YDMF_PHYS%OUT%FCHSP, YDMF_PHYS%OUT%FCLL,                    &
   & YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FCS, YDAPLPAR_TMP%FLU%FEVI, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN,                     &
   & YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR, YDMF_PHYS%OUT%FLWSP, YDMF_PHYS%OUT%FONTE, YDMF_PHYS_SURF%GSP_SG%PT_T1,             &
   & YDMF_PHYS%OUT%FGEL, YDMF_PHYS%OUT%FGELS, YDMF_PHYS%OUT%FRSODS, YDMF_PHYS%OUT%FRSOPS, YDMF_PHYS%OUT%FRSOPT,                &
   & YDMF_PHYS%OUT%FRSOLU, YDMF_PHYS%OUT%FRTHDS, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN, YDMF_PHYS%OUT%FPFPCL,             &
   & YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN,         &
   & YDAPLPAR_TMP%PFL%FTKE, YDAPLPAR_TMP%PFL%FEFB1, YDAPLPAR_TMP%PFL%FEFB2, YDAPLPAR_TMP%PFL%FEFB3, YDMF_PHYS%OUT%GZ0,         &
   & YDMF_PHYS%OUT%GZ0H, YDAPLPAR_TMP%FLU%NEIJ, YDAPLPAR_TMP%FLU%VEG, YDCPG_MISC%QS, YDAPLPAR_TMP%FLU%QSATS,                   &
   & YDMF_PHYS%OUT%RUISL, YDMF_PHYS%OUT%RUISP, YDMF_PHYS%OUT%RUISS, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS,                    &
   & YDMF_PHYS%OUT%TCLS, YDMF_PHYS%OUT%QCLS, YDMF_PHYS%OUT%RHCLS, YDCPG_MISC%CLCT, YDMF_PHYS%OUT%CLCH,                         &
   & YDMF_PHYS%OUT%CLCM, YDMF_PHYS%OUT%CLCL, YDMF_PHYS%OUT%CLCC, YDMF_PHYS%OUT%CAPE, YDMF_PHYS%OUT%CTOP,                       &
   & YDAPLPAR_TMP%MOC%CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST, 'WRITEMUSC', YDMF_PHYS_TMP, YDAPLPAR_TMP)
