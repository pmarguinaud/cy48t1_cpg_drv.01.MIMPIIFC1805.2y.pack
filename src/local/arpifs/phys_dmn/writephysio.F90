SUBROUTINE WRITEPHYSIO(YDGEOMETRY,&
 ...
 & PCLON  , PSLON, PRDG_CVGQ, PRDG_LCVQ, PRDG_MU0, YDAPLPAR_TMP )
...
IF (LLTIME_OK) THEN
  DO JLON=KST,KEND
    ...
    IF (LLOK) THEN
      ...
      & YDAPLPAR_TMP%PFL%FPLCH, YDAPLPAR_TMP%PFL%FPLSH)
    ENDIF
  ENDDO
ENDIF
...
IF (LMUSCLFA) THEN
   ...
   & KCSS, PCLON, PSLON, PGELAM, PGEMU, PGM, POROG, 'WRITEMUSC', PRDG_CVGQ, PRDG_MU0, YDAPLPAR_TMP%DSA%C1, &
   & YDAPLPAR_TMP%DSA%C2, YDAPLPAR_TMP%DSA%CPS, YDAPLPAR_TMP%DSA%LHS, YDAPLPAR_TMP%DSA%RS, YDAPLPAR_TMP%FLU%CD, &
   & YDAPLPAR_TMP%FLU%CDN, YDAPLPAR_TMP%FLU%CH, YDAPLPAR_TMP%FLU%EMIS, YDAPLPAR_TMP%FLU%FEVI, &
   & YDAPLPAR_TMP%FLU%NEIJ, YDAPLPAR_TMP%FLU%QSAT, YDAPLPAR_TMP%FLU%QSATS, YDAPLPAR_TMP%FLU%VEG, &
   & YDAPLPAR_TMP%MOC%CLPH, YDAPLPAR_TMP%MSC%FRMQ, YDAPLPAR_TMP%MSC%LH, YDAPLPAR_TMP%MSC%LSCPE, &
   & YDAPLPAR_TMP%MSC%QW, YDAPLPAR_TMP%MSC%TW, YDAPLPAR_TMP%PFL%FEFB1, YDAPLPAR_TMP%PFL%FEFB2, &
   & YDAPLPAR_TMP%PFL%FEFB3, YDAPLPAR_TMP%PFL%FTKE)
