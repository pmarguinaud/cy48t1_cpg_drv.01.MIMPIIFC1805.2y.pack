...
IF (LLTIME_OK) THEN
  DO JLON=KST,KEND
    ...
    IF (LLOK) THEN
      ...
      ! -------------------------------------------------
      !
      CALL WRITEPROFILE(YDGEOMETRY, YDSURF, YDDPHY, YDRIP, YDML_PHY_MF, 'Profile.', JLON,                                     &
      & ZLAT_MODELE, ZLON_MODELE, ZD, PGELAM, PGEMU, PGM, YDMF_PHYS_TMP%RDG%MU0, POROG, YDCPG_DYN0%OROGL,                     &
      ...
      & YDVARS%VOR%T0, YDVARS%DIV%T0, YDMF_PHYS_TMP%RDG%CVGQ, YDMF_PHYS_TMP%RDG%LCVQ, YDMF_PHYS_SURF%GSP_RR%PT_T9,            &
      & YDMF_PHYS_SURF%GSP_SB%PT_T9, YDMF_PHYS_SURF%GSP_RR%PFC_T9, YDMF_PHYS_SURF%GSP_RR%PW_T9, YDMF_PHYS_SURF%GSP_RR%PIC_T9, &
      & YDMF_PHYS_SURF%GSP_SB%PQ_T9, YDMF_PHYS_SURF%GSP_SB%PTL_T9, YDMF_PHYS_SURF%GSP_SG%PF_T9, YDMF_PHYS%OUT%DIFCQ,          &
      & YDMF_PHYS%OUT%DIFCQN, YDMF_PHYS%OUT%DIFCQL, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%DIFTQ, YDMF_PHYS%OUT%DIFTQN,           &
      & YDMF_PHYS%OUT%DIFTQL, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FCSQL,             &
      & YDMF_PHYS%OUT%FCSQN, YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%FCQNNG, YDMF_PHYS%OUT%FCQLNG, YDMF_PHYS%OUT%FPLCL,            &
      & YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,                &
      & YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV, YDMF_PHYS%OUT%STRDU, YDMF_PHYS%OUT%STRDV, YDMF_PHYS%OUT%STRTU,              &
      & YDMF_PHYS%OUT%STRTV, YDMF_PHYS%OUT%STRMU, YDMF_PHYS%OUT%STRMV, YDMF_PHYS%OUT%FRMH, YDMF_PHYS%OUT%FCHOZ,               &
      & YDCPG_MISC%NEB, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDCPG_MISC%RH, YDMF_PHYS%OUT%FCS, YDMF_PHYS%OUT%FCLL,                &
      & YDMF_PHYS%OUT%FCLN, YDMF_PHYS%OUT%FEVL, YDMF_PHYS%OUT%FEVN, YDMF_PHYS%OUT%FEVV, YDMF_PHYS%OUT%FTR,                    &
      & YDMF_PHYS%OUT%FLWSP, YDMF_PHYS%OUT%FONTE, YDMF_PHYS%OUT%FGEL, YDMF_PHYS%OUT%FGELS, YDMF_PHYS%OUT%FRSODS,              &
      & YDMF_PHYS%OUT%FRSOPS, YDMF_PHYS%OUT%FRSOPT, YDMF_PHYS%OUT%FRTHDS, YDCPG_MISC%QS, YDMF_PHYS%OUT%RUISL,                 &
      & YDMF_PHYS%OUT%RUISP, YDMF_PHYS%OUT%RUISS, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS%OUT%TCLS,                 &
      & YDMF_PHYS%OUT%QCLS, YDMF_PHYS%OUT%RHCLS, YDCPG_MISC%CLCT, YDMF_PHYS%OUT%CLCH, YDMF_PHYS%OUT%CLCM,                     &
      & YDMF_PHYS%OUT%CLCL, YDMF_PHYS%OUT%CLCC, YDAPLPAR_TMP%PFL%FPLCH, YDAPLPAR_TMP%PFL%FPLSH, YDMF_PHYS_SURF%GSD_VH%PTCCH,  &
      & YDMF_PHYS_SURF%GSD_VH%PSCCH, YDMF_PHYS_SURF%GSD_VH%PBCCH, YDMF_PHYS_SURF%GSD_VH%PPBLH)
    ENDIF
  ENDDO
ENDIF
