    ...
    DO JL=KIDIA,KFDIA
      IF(LDCUM(JL)) THEN
        ...
        ZDTDT(JL,JK)=ZDP(JL,JK)*ZORCPD*&
         ...
         & +YDCST%RLMLT*PLGLAC(JL,JK)&
         & -YDCST%RLMLT*PDPMEL(JL,JK)&
       ! & -ZALV*(PMFUL(JL,JK+1)-PMFUL(JL,JK)-&
       ! & PLUDE(JL,JK)-PDMFUP(JL,JK)))
         & -ZALV*(PMFUL(JL,JK+1)-PMFUL(JL,JK)-PDMFUP(JL,JK))&
         & +YDCST%RLVTT*PLUDELI(JL,JK,1)+YDCST%RLSTT*PLUDELI(JL,JK,2))
  ...
  ELSE
    DO JL=KIDIA,KFDIA
      IF(LDCUM(JL)) THEN
        ...
        ENDIF
        ZDTDT(JL,JK)=-ZDP(JL,JK)*ZORCPD*&
         & (ZMFUS(JL,JK)+ZMFDS(JL,JK)+YDCST%RLMLT*PDPMEL(JL,JK)-ZALV*&
         ...
         DO JL=KIDIA,KFDIA
           ...
           IF(LLCUMBAS(JL,JK)) THEN
             ...
             ELSE
               ZB(JL,JK)=1.0_JPRB
             ENDIF
             ZZP=YDCST%RG*(PMFU(JL,JK)+RMFADVWDD*PMFD(JL,JK))/(PAP(JL,JK)-PAP(JL,IM))*PTSPHY*ZADVW(JL)
