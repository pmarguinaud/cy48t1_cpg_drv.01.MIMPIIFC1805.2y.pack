SUBROUTINE CUININ &
 & (YDCST,   YDEPHLI, YDECUMF,  KIDIA,    KFDIA,    KLON,    KLEV,&
...
DO JK=2,KLEV
  DO JL=KIDIA,KFDIA
    PTENH(JL,JK)=(MAX(YDCST%RCPD*PTEN(JL,JK-1)+PGEO(JL,JK-1),&
     & YDCST%RCPD*PTEN(JL,JK)+PGEO(JL,JK))-PGEOH(JL,JK))*ZORCPD  
...
!orig   IF(JK.GE.KLEV-1) GO TO 130
  ...
  IF(LPHYLIN)THEN
    ICALL=0
    CALL CUADJTQS &
     & ( YRTHF, YDCST, KIDIA,    KFDIA,    KLON,    KLEV,     IK,&
