!OPTIONS XOPT(NOEVAL)
SUBROUTINE ACRADS(YDVAB, YDML_PHY_MF,KIDIA,KFDIA,KLON,KTDIA,KLEV,KJN,&
 !-----------------------------------------------------------------------
 ! - INPUT  2D .
...
DO JLEV=KTDIA,KLEV
  DO JLON=KIDIA,KFDIA
    ZZBB(JLON,JLEV-1)=RSIGMA*(PT(JLON,JLEV)*PT(JLON,JLEV))&
     & *(PT(JLON,JLEV)*PT(JLON,JLEV))  
  ENDDO
ENDDO
DO JLON=KIDIA,KFDIA
  ZZBB(JLON,KLEV)=RSIGMA*(PTS(JLON)*PTS(JLON))*(PTS(JLON)*PTS(JLON))
ENDDO
