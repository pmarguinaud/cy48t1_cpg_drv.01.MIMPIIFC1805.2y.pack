...
! SPP mean of perturbation distributions
...
CALL LIQUID_EFFECTIVE_RADIUS(YDMODEL%YRML_PHY_RAD%YRERAD, &
     &  YDMODEL%YRML_PHY_EC%YRECLDP,YDMODEL%YRML_GCONF%YGFL, &
...
IF (YSPP_CONFIG%LSPP .AND. YSPP_CONFIG%LPERT_ZDECORR) THEN
  ...
  DO JLON = KIDIA,KFDIA
    ZDECORR_LEN_KM(JLON) = ZDECORR_LEN_KM(JLON) &
         &  * EXP(ZMU_ZDECORR + YSPP_CONFIG%CMPERT_ZDECORR*PPERT(JLON,YSPP%MPZDECORRRAD))
  ENDDO
ENDIF
...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_ZSIGQCW) THEN
  ...
  ENDIF
  DO JLON = KIDIA,KFDIA
    ZFACTOR = EXP(ZMU_ZSIGQCW + YSPP_CONFIG%CMPERT_ZSIGQCW*PPERT(JLON,YSPP%MPZSIGQCWRAD))
    DO JLEV = 1,KLEV
...
!$OMP CRITICAL
    ...
    WRITE(CL_FILE_NAME, '(A,I0,A,I0,A)') '/home/rd/parr/ifs_dump/bad_inputs_', &
    ...
    WRITE(CL_FILE_NAME, '(A,I0,A,I0,A)') '/home/rd/parr/ifs_dump/bad_outputs_', &
         &  MPL_MYRANK(), '_', N_BAD_FLUXES, '.nc'
    WRITE(NULERR,*) '  Writing ', TRIM(CL_FILE_NAME)
...
!$OMP CRITICAL
  ...
  WRITE(CL_FILE_NAME, '(A,I0,A,I0,A)') '/home/rd/parr/ifs_dump/inputs_', &
  ...
  CALL SAVE_INPUTS(TRIM(CL_FILE_NAME), RAD_CONFIG, SINGLE_LEVEL, &
       &  THERMODYNAMICS, GAS, YLCLOUD, AEROSOL, &
       &  LAT=ASIN(PGEMU)*180.0/RPI, LON=PGELAM*180.0/RPI, IVERBOSE=3)
  WRITE(CL_FILE_NAME, '(A,I0,A,I0,A)') '/home/rd/parr/ifs_dump/outputs_', &
       &  MPL_MYRANK(), '_', N_OUTPUT_FLUXES, '.nc'
  WRITE(NULERR,*) '  Writing ', TRIM(CL_FILE_NAME)
  CALL SAVE_FLUXES(TRIM(CL_FILE_NAME), RAD_CONFIG, THERMODYNAMICS, FLUX, IVERBOSE=3)
!$OMP END CRITICAL
