...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RSWINHF) THEN
  ...
  IF ( YSPP_CONFIG%IEZDIAG_POS > 0 ) THEN
   ...
   DO JL=KIDIA,KFDIA
    PEZDIAG_POS(JL,JKO) = PGP2DSPP(JL,YSPP%MP_RSWINHF)
    PEZDIAG_POS(JL,JKE) = ZRSWINHF(JL)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
...
IF (YSPP_CONFIG%LSPP.AND.YSPP_CONFIG%LPERT_RLWINHF) THEN
  ...
  DO JL=KIDIA,KFDIA
    PEZDIAG_POS(JL,JKO) = PGP2DSPP(JL,YSPP%MP_RLWINHF)
    PEZDIAG_POS(JL,JKE) = ZRLWINHF(JL)
   ENDDO
  ENDIF
ELSE
  DO JL=KIDIA,KFDIA
...
!*         4.2     TRANSFORM FLUXES TO MODEL HISTORICAL VARIABLES
  ...
  IF (YDARPHY%LMSE) THEN
    IF (NSW==6) THEN
      DO JSW=1,NSW
        PGPAR(KIDIA:KFDIA,YDPARAR%MSWDIR+JSW-1)= PTRSODIR(KIDIA:KFDIA,JSW)
        PGPAR(KIDIA:KFDIA,YDPARAR%MSWDIF+JSW-1)= PTRSODIF(KIDIA:KFDIA,JSW)
      ENDDO
    ELSEIF (NSW==1) THEN
      PGPAR(KIDIA:KFDIA,YDPARAR%MSWDIR)=PFSDNN(KIDIA:KFDIA)+PFSDNV(KIDIA:KFDIA)
      PGPAR(KIDIA:KFDIA,YDPARAR%MSWDIF)=0.0_JPRB
    ENDIF
  ENDIF
