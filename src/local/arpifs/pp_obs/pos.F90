...
IF (LLDYN(1)) THEN
  IF (LNHDYN) THEN
    ...
    CALL GNHPRE(YRDYNA%NPDVAR,YDGEOMETRY,KPROMA,KFLEVG,KST,KND,ZPDT0,ZPRESF,&
     & PNHPREF=ZNHPREF,PNHPPI=ZNHPPI,PRNHPPI=ZRNHPPI,PDEP=ZPDEP)
    ! total nh pressure at half level
    CALL GNHPREH(LVERTFE, LVFE_ECMWF, YRDYNA%NPDVAR, YDGEOMETRY,KPROMA,KFLEVG,KST,KND,ZPDT0,ZPRESH,&
    ...
    DO JLEV=1,KFLEVG
      ...
      ENDDO
    ENDDO
    CALL GNHGRPRE(YRDYNA%NPDVAR,YDGEOMETRY,KPROMA,KFLEVG,KST,KND,ZXYB(1,1,YYTXYBPP%M_RTGR),ZSP0L,ZSP0M,&
...
IF (LLDYN(4)) THEN
  ...
  CALL GPHLWI(YDDIMV,KPROMA,KST,KND,ZXYB(1,1,YYTXYBPP%M_LNPR),&
       & ZXYB(1,1,YYTXYBPP%M_ALPH),ZUVH(1,1,YYTHWPP%M_WWI),LDVERINT=YRDYNA%LVEREGINT)
  ...
  CALL GPGW(LNHDYN, LVERTFE, LVFE_GW, YDGEOMETRY,KFLEVG,KPROMA,KST,KND,LLGWF,LLGDWI,&
