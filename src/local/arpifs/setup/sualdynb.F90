...
IF (LSTRHD) THEN
  ...
  IVTHS=0
  IF ((.NOT.YRDYNA%LSLHD_STATIC).AND.YRDYNA%LSLHD_W) IVTHS=IVTHS+2
  IF ((.NOT.YRDYNA%LSLHD_STATIC).AND.LNHDYN.AND.YRDYNA%LSLHD_SVD) IVTHS=IVTHS+1
  IF ((.NOT.YRDYNA%LSLHD_STATIC).AND.YRDYNA%LNHX.AND.YRDYNA%LSLHD_SVD) IVTHS=IVTHS+1
  IL=IVTHS*NFLEVL
  IF(IVTHS>0) THEN
...
ENDIF
IF((.NOT.YRDYNA%LSLHD_STATIC).AND.YRDYNA%LSLHD_W) THEN
...
ENDIF
IF((.NOT.YRDYNA%LSLHD_STATIC).AND.YRDYNA%LSLHD_SVD.AND.LNHDYN) THEN
