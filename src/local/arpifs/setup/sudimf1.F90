...
IF(LR3D .OR. ANY(SPREAD(NCONF,1,2) == (/701,901/))) THEN  
  ...
  LSPT=.TRUE.
  IF(LSLAG) THEN
    IF (LNHDYN.OR.(YRDYNA%LSLHD.AND..NOT.YRDYNA%LSLHD_STATIC)&
      & .OR.(NCONF == 131.AND.LJC).OR.YRDYNA%L3DTURB.OR.YRDYNA%LCOMAD.OR.LDIAGTURB_EC) THEN
      LUVDER=.TRUE.
    ELSE
      LUVDER=.FALSE.
    ENDIF
  ...
  IF(NCONF==1.AND..NOT.(LELAM.OR.LNHDYN)) THEN
    LVOR=((.NOT.LSLAG).OR.(LSLAG.AND.YRDYNA%LSLHD)).AND.LADER
  ELSE
    LVOR=LADER
  ENDIF
ELSEIF(LR2D) THEN  
