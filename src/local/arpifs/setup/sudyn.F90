...
RDAMPVDS = 0._JPRB
RDAMPHDS = 1._JPRB
IF(YRDYNA%LSLHD_W .OR. YRDYNA%LSLHD_SVD) RDAMPHDS= 1.5_JPRB - RNLGINC * 0.5_JPRB
IF(YRDYNA%LSLHD_W) THEN
  RDAMPDIVS= 1._JPRB
  RDAMPVORS= 5._JPRB
ENDIF
IF(YRDYNA%LSLHD_SVD) RDAMPVDS = 15._JPRB
...
IF ((YRDYNA%LSLHD_W.OR.YRDYNA%LSLHD_T.OR.YRDYNA%LSLHD_SVD).AND.(.NOT.(YRDYNA%LSLHD_STATIC.OR.LSLHDSPONGE))) SDRED=1._JPRB
...
IF (LECMWF) THEN
  ...
  ELSE
    ...
    ENDIF
    NITMP=2
    IF (YRDYNA%LSETTLST.OR.YRDYNA%LSETTLSV) THEN
      IF (NCONF == 1) THEN
        NITMP=5
...
LVWENO_W   = .FALSE.
LVWENO_T   = LECMWF.AND.YRDYNA%LRHSVWENO
LVWENO_SP  = .FALSE.
LVWENO_SPD = .FALSE.
LVWENO_SVD = .FALSE.
...
IF (YRDYNA%LSLHD) THEN
  ...
  IF ((SDRED < 0.0_JPRB ).OR.(SDRED > 1.0_JPRB )) THEN
    ...
    CALL ABOR1('SUDYN: ABOR1 CALLED')
  ENDIF
  IF (.NOT.LELAM .AND. YRDYNA%LSLHD_SVD .AND. (RDAMPVDS /= 0._JPRB)) THEN
    WRITE(KULOUT,*)&
...
IF (YRDYNA%LSLHD.AND.(SLHDHOR /= 0._JPRB).AND.(NCONF == 131.OR.NCONF == 401&
 &  .OR.NCONF == 501.OR.NCONF == 601.OR.NCONF == 801).AND.&
 &  (.NOT.YRDYNA%LSLHD_STATIC)) THEN
...
IF (LR2D) THEN
  ...
  IF (YRDYNA%LSETTLS.AND..NOT.(NWLAG == 3.AND.NVLAG == 3.AND.LTWOTL)) THEN
...
ELSE
  ...
  IF ((.NOT.LMPHYS).AND.(NWLAG==4 .OR. NTLAG==4 .OR. NSVDLAG==4)) THEN
    ...
    CALL ABOR1('SUDYN: ABOR1 CALLED')
  ENDIF
  IF (YRDYNA%LSETTLS.AND.YRDYNA%LPC_CHEAP.AND.(NWLAG/=3 .OR. NTLAG/=3 .OR. NSPDLAG/=3 .OR. NSVDLAG/=3)) THEN
  ...
  IF ((NSPLTHOI /= 0).AND.LSLPHY) THEN
    ...
    CALL ABOR1('SUDYN: ABOR1 CALLED')
  ENDIF
  IF ((NSPLTHOI == 1).AND.(YRDYNA%SLHDEPSH == 0.0_JPRB).AND.&
    & ((.NOT.YRDYNA%LSLHD_OLD).AND.(YRDYNA%SLHDKMAX == YRDYNA%SLHDKMIN))) THEN
  ...
  ENDIF
  ! * Use LSETTLS=T when not coded?
  IF (YRDYNA%LSETTLS.AND..NOT.(NWLAG >= 3.AND.NTLAG >= 3.AND.NVLAG == 3.AND.LTWOTL)) THEN  
    WRITE(KULOUT,*) 'LSETTLS ONLY WORKS WITH NWLAG=NTLAG=NVLAG=3,(4) LTWOTL=T'
    CALL ABOR1('SUDYN: ABOR1 CALLED')
  ENDIF
  IF (LNHDYN.AND.YRDYNA%LSETTLS.AND..NOT.(NSPDLAG == 3.AND.NSVDLAG >= 3.AND.LTWOTL)) THEN  
...
IF (NSITER < 0) THEN
  ...
  CALL ABOR1('SUDYN: ABOR1 CALLED')
ENDIF
IF( NSITER==0.AND.YRDYNA%LPC_FULL )THEN
  WRITE(KULOUT,*) 'No iterations NSITER=0 but LPC_FULL is .T.'
  CALL ABOR1('SUDYN: ABOR1 CALLED')
ENDIF
IF( NSITER>0.AND..NOT.YRDYNA%LPC_FULL)THEN
  WRITE(KULOUT,*) 'Dynamics with NSITER>0 needs LPC_FULL'          
  CALL ABOR1('SUDYN: ABOR1 CALLED')
ENDIF
IF (LSETFSTAT.AND.YRDYNA%LMIXETTLS) THEN
...
IF (YRDYNA%LRUBC .AND. NSITER /= 1) THEN
...
IF (YRDYNA%L3DTURB.AND.(.NOT.LMPHYS)) THEN
...
IF (YRDYNA%LSLTVWENO.OR.YRDYNA%LRHSVWENO) THEN
...
IF (LLSTRSG .AND. LSLDP_CURV) THEN
  ...
  CALL ABOR1('SUDYN: ABOR1 CALLED')
ENDIF
IF (YRDYNA%LSLHD .AND. YRDYNA%LRHSVWENO) THEN
  IF (LSLHDSPONGE) THEN
    ...
    IF (YRDYNA%SLHDKREF /= 0._JPRB) THEN
  ...
  ENDIF
ENDIF
IF ((LVWENO_W.OR.LVWENO_T.OR.LVWENO_SP.OR.LVWENO_SPD.OR.LVWENO_SVD).AND.(.NOT.YRDYNA%LRHSVWENO)) THEN
...
IF (LSLAG) THEN
  ...
  IF (YRDYNA%L3DTURB) NSLDIMK=NSLDIMK+2
...
ELSE IF (NSTOP > 0.OR.LDFI.OR.LSDDH.OR.LWCOU) THEN
  ...
  ! Test of convergence of the iterative Helmholtz solver if needed
    IF (YRDYNA%NDLNPR /= 1.AND.NFLEVG>1) CALL SUNHSI_TESTCONV(YDMODEL%YRCST,YDGEOMETRY,YDRIP,YDDYN)
