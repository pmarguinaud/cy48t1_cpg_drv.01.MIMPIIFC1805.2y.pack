...
!**** *SUNH_VERTFE1D*  - Setup VERTical Finite Element Derivative based on
!                        linear (1) elements: first order derivative.
...
! indefiniteINTEGRAL(EJ*EI')dx
FDD(XX,XA,XB,YA,YB)=XX*( XA*YB + XX*XB*YB/2._JPRB )
...
IF (LHOOK) CALL DR_HOOK('SUNH_VERTFE1D',0,ZHOOK_HANDLE)
...
DO JR=1,NFLEVG
  DO JC=2,NFLEVG-1
    YDVFE%RDERI(JR,JC)=ZDERI(JR+1,JC+1)
  ENDDO
  YDVFE%RDERI(JR,1)=ZDERI(JR+1,2)+ZW1*ZDERI(JR+1,1)
  YDVFE%RDERI(JR,2)=ZDERI(JR+1,3)+ZW2*ZDERI(JR+1,1)
  YDVFE%RDERI(JR,II-1)=ZDERI(JR+1,II  )+ZW2*ZDERI(JR+1,II+2)
  YDVFE%RDERI(JR,II  )=ZDERI(JR+1,II+1)+ZW1*ZDERI(JR+1,II+2)
ENDDO

IF(MYPROC == 1.AND.(NPRINTLEV >= 1)) THEN

  WRITE(NULOUT,*) ""
  WRITE(NULOUT,*) " PRINTINGS IN SUNH_VERTFE1D "
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('SUNH_VERTFE1D',1,ZHOOK_HANDLE)
