SUBROUTINE SUNH_VERTFE3DD(YDGEOMETRY)

!**** *SUNH_VERTFE3DD*  - Setup VERTical Finite Element Derivative based on
!                         cubic (3) elements
...
REAL(KIND=JPRB) :: ZA1, ZA2, ZA3, ZA4, ZB1, ZB2, ZB3, ZB4
...
! indefiniteINTEGRAL(EJ*EI'')dx                '
!-----------
! Ej   = ZA1 + ZA2*X +  ZA3*X*X +    ZA4*X*X*X
! Ei'' =            2*ZB3     + 6* ZB4*X
!-----------

FDD(XX,ZA1,ZA2,ZA3,ZA4,ZB1,ZB2,ZB3,ZB4)=&
 & ZA1*XX*(2._JPRB*ZB3 + 3._JPRB*ZB4*XX)&
 & + XX*XX/30._JPRB*(30._JPRB*ZA2*(ZB3+2._JPRB*ZB4*XX)&
 & + XX*(&
 & 5._JPRB*ZA3*(4._JPRB*ZB3+9._JPRB*ZB4*XX)&
 &    + 3._JPRB*ZA4*XX*(5._JPRB*ZB3+12._JPRB*ZB4*XX)))
...
IF (LHOOK) CALL DR_HOOK('SUNH_VERTFE3DD',0,ZHOOK_HANDLE)
...
DO JR=1,NFLEVG
  DO JC=3,NFLEVG-2
    YDVFE%RDDERI(JR,JC)=ZDERI(JR+2,JC+2)
  ENDDO
  YDVFE%RDDERI(JR,1)=ZDERI(JR+2,3)+ZW1T*ZDERI(JR+2,2)&
   & -ZDERI(JR+2,1)/(YDVETA%VFE_ETAF(2)-YDVETA%VFE_ETAF(1))
  YDVFE%RDDERI(JR,2)=ZDERI(JR+2,4)+ZW2T*ZDERI(JR+2,2)&
   & +ZDERI(JR+2,1)/(YDVETA%VFE_ETAF(2)-YDVETA%VFE_ETAF(1))
  YDVFE%RDDERI(JR,II-1)=ZDERI(JR+2,II+1)+ZW2B*ZDERI(JR+2,II+3)&
   & +ZDERI(JR+2,II+4)/(YDVETA%VFE_ETAF(II-1)-YDVETA%VFE_ETAF(II))
  YDVFE%RDDERI(JR,II  )=ZDERI(JR+2,II+2)+ZW1B*ZDERI(JR+2,II+3)&
   & -ZDERI(JR+2,II+4)/(YDVETA%VFE_ETAF(II-1)-YDVETA%VFE_ETAF(II))
ENDDO
...
IF(MYPROC == 1.AND.(NPRINTLEV >= 1)) THEN
  ...
  WRITE(NULOUT,*) ""
  WRITE(NULOUT,*) " PRINTINGS IN SUNH_VERTFE3DD "
...
END ASSOCIATE
END ASSOCIATE
IF (LHOOK) CALL DR_HOOK('SUNH_VERTFE3DD',1,ZHOOK_HANDLE)
END SUBROUTINE SUNH_VERTFE3DD
