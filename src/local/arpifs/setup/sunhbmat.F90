SUBROUTINE SUNHBMAT(YDGEOMETRY,YDDYN)
...
USE YOMCST   , ONLY : RG, RD, RCPD, RCVD, RPI
USE YOMCVER  , ONLY : LVERTFE, NVFE_ORDER
...
CALL SISEVE(YDGEOMETRY,YDDYN,1,NFLEVG,ZD,SIFAC,NFLEVG)
...
ELSE
  ...
  CALL SISEVE(YDGEOMETRY,YDDYN,1,NFLEVG,ZSIBIN,ZSIBOUT,NFLEVG)
...
IF (LVERTFE .AND. LOUTPUT .AND. NPRINTLEV>=1) THEN
  ...
  DO JL1=1,NFLEVG
    ...
    ! analytical solution
    ZANA(JL1)  = ZX*(6.0_JPRB*RPI*COS(ZA)**2*SIN(ZA)**2 &
     & - 2.0_JPRB*RPI*SIN(ZA)**4 + ZX*(6.0_JPRB*RPI**2*COS(ZA)**3*SIN(ZA) &
     & - 10.0_JPRB*RPI**2*COS(ZA)*SIN(ZA)**3))
  ENDDO

  CALL SISEVE(YDGEOMETRY,YDDYN,1,NFLEVG,ZFUNC,ZVFE,1)
