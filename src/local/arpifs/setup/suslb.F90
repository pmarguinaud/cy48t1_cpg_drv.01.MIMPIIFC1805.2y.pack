...
LLSLIN_SI= LRSIDDH .OR. YRDYNA%LSLINL
LLSETTLS = YRDYNA%LSETTLS .AND. YRDYNA%LPC_CHEAP
IU = NULOUT
IF (LLP) WRITE (IU,*)
...
IF (LNHDYN.AND.YRDYNA%LGWADV.AND.LVERTFE.AND.LVFE_GW) THEN
  MSLB2GWF = IPTX
...
!          for terms specific to diagnostic BBC in NH
  ...
  IF (YRDYNA%LRDBBC .AND. (NSVDLAG == 2)) THEN
    MSLB1DBBC9 = IPTX
...
!          for terms specific to diagnostic BBC in NH
  ...
  IF (YRDYNA%LRDBBC .AND. (NSVDLAG >= 3)) THEN
    MSLB1DBBC9 = IPTX
    IF (LLP) WRITE(IU,'(1X,A14,2I4)') 'MSLB1DBBC9   = ',IPTX,ILVP2
    IPTX     = IPTX + ILVP2
    MSLB1DPHI9 = IPTX
...
!        2.13 Computation of RPARSL1 for 2D continuity equation terms.
  ...
  ! diagnostic LRDBBC
  IF (YRDYNA%LRDBBC) YDMODEL%YRML_DYN%YRPTRSLB1%RPARSL1(MSLB1GWS9)=1.0_JPRB
