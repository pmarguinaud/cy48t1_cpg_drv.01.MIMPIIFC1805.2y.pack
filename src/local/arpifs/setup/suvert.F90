...
IF (NFLEVG > 1) THEN 
  IF (LVERTFE) THEN
    ...
    DO JLEV=1,NFLEVG
      YDVAB%VRATH(JLEV)=(YDVAB%VBH(JLEV)*VP00/(YDVAB%VAH(JLEV)+YDVAB%VBH(JLEV)*VP00))**YRDYNA%REXP_VRAT
      YDVAB%VRATF(JLEV)=(YDVAB%VBF(JLEV)*VP00/(YDVAB%VAF(JLEV)+YDVAB%VBF(JLEV)*VP00))**YRDYNA%REXP_VRAT
    ENDDO
  ...
  DO JLEV=1,NFLEVG
    YDVAB%VRATF(JLEV)=((YDVAB%VBH(JLEV)+YDVAB%VBH(JLEV-1))*VP00 &
     & /(YDVAB%VAH(JLEV)+YDVAB%VAH(JLEV-1)+(YDVAB%VBH(JLEV)+YDVAB%VBH(JLEV-1))*VP00))**YRDYNA%REXP_VRAT
  ENDDO
  YDVAB%VRATF(1)=MIN(0.25_JPRB,YDVAB%VRATF(1))
  IF (NFLEVG>2) YDVAB%VRATF(2)=MIN(0.75_JPRB,YDVAB%VRATF(2))
ELSE
