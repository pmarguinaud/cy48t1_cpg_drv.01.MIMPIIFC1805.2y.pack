MODULE MODD_SPP_TYPE
 ...
 SUBROUTINE SET_SPP_TYPE(TSPP,LLNN_MEAN1,LLNN_MEAN1_SELF, &
  ...
  REAL, DIMENSION(KLON,KLEV), OPTIONAL, INTENT(INOUT) :: PEZDIAG_POS
  ...
  IF ( PRESENT(PEZDIAG_POS) .AND. NEZDIAG_POS > 0 ) THEN
   ...
   JKE=2*MP_SELF
   DO JI=1,KLON
    PEZDIAG_POS(JI,JKO) = TSPP%PGP2DSPP(JI)
   ENDDO
   DO JI=1,KLON
    ZVAL = PREFVAL*EXP(ZMU+TSPP%CMPERT*TSPP%PGP2DSPP(JI))
    PEZDIAG_POS(JI,JKE) = MAX(TSPP%CLIP(1),MIN(ZVAL,TSPP%CLIP(2)))
   ENDDO
  ENDIF
